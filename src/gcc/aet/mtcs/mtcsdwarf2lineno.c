...
static void mtcsDwarf2LinenoInit(MtcsDwarf2Lineno *self)
{
   MtcsDebug *mtcsDebug=(MtcsDebug*)self;
   //由Mtcsdwarf2Out设定 mtcsDebug->init = init_cb;
   mtcsDebug->finish = finish_cb;
   mtcsDebug->early_finish = earlyFinish_cb;
   //由Mtcsdwarf2Out设定 mtcsDebug->assembly_start = assembly_start;
   ...
   //由Mtcsdwarf2Out设定  mtcsDebug->source_line =sourceLine_cb;
...
{
   MtcsDwarf2Lineno *self = n_slice_alloc0 (sizeof(MtcsDwarf2Lineno));
   mtcs_component_set_mode((MtcsComponent *)self,mtcsMode);
   mtcs_dwarf2_out_init((MtcsDwarf2Out *)self);
   mtcsDwarf2LinenoInit(self);
   return self;
}
