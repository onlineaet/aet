...
#include "aet/aetmangle.h"
#include "aet/classparser.h"
#include "aet/classimpl.h"
#include "aet/aetinfo.h"
#include "aet/classmgr.h"
#include "aet/classref.h"
#include "aet/classinit.h"
#include "aet/classcast.h"
#include "aet/genericimpl.h"
#include "aet/classutil.h"
#include "aet/newobject.h"
#include "aet/genericcall.h"
#include "aet/funcmgr.h"
#include "aet/genericutil.h"
#include "aet/blockmgr.h"
#include "aet/genericexpand.h"
#include "aet/genericquery.h"
...
static void c_lex_one_token (c_parser *parser, c_token *token, bool raw = false)
{
     ...
     switch (token->type){
         case CPP_NAME:
         {
	         ...
	         if(C_IS_RESERVED_WORD (token->value)){
	              ...
	              if (rid_code == RID_CXX_COMPAT_WARN){
		             ...
		             n_debug("c_lex_one_token 很少进这里 rid_code >= RID_FIRST_ADDR_SPACE && rid_code <= RID_LAST_ADDR_SPACE rid_code:%d\n",rid_code);
		             addr_space_t as;
					 ...
					 /* We found an Objective-C "pq" keyword (in, out,
		               ...
		               if (parser->objc_pq_context){
		                  ...
		                  break;
		 	               ...
		 	               n_debug("c_lex_one_token 22 rid_code:%d\n",rid_code);
				...
				*/
					...
					/* We found one of the Objective-C "@" keywords (defs,
		             ...
		             //n_debug("c_lex_one_token 33 如果是关键词 跳到out there 333 rid_code:%d",rid_code);
		             token->type = CPP_KEYWORD;
		             token->keyword = rid_code;
		             break;
	             }
	    ...
	    /* Objective-C class names are in the same namespace as
	       ...
	       declarations.  */
	    	  ...
	    	  if(class_impl_set_class_or_enum_type(classImpl,token)){
	    	     break;
	    	  }
	      }
 ...
 out:
//  printf("c_lex_one_token 66 完成 raw:%d vec_safe_ength:%d --- %s, %s, %d\n",
//		  raw,vec_safe_length (parser->raw_tokens),__FILE__, __FUNCTION__, __LINE__);
...
c_parser_consume_token (c_parser *parser)
{
  ...
  if (parser->tokens != &parser->tokens_buf[0])
    parser->tokens++;
	 ...
	 // int index=parser->tokens_avail;
     // parser->tokens[index-2] = parser->tokens[index-1];
      parser->tokens[0] = parser->tokens[1];
      if (parser->tokens_avail >= 3){
          parser->tokens[1] = parser->tokens[2];
	  if (parser->tokens_avail >= 4)
		parser->tokens[2] = parser->tokens[3];
	  if(parser->tokens_avail >= 5) //zclei++
		  parser->tokens[3] = parser->tokens[4];//zclei++
	  if(parser->tokens_avail >= 6) //zclei++
			parser->tokens[4] = parser->tokens[5];//zclei++
	  if(parser->tokens_avail >= 7) //zclei++
			parser->tokens[5] = parser->tokens[6];//zclei++
	  if(parser->tokens_avail >= 8) //zclei++
			parser->tokens[6] = parser->tokens[7];//zclei++
	  if(parser->tokens_avail >= 9) //zclei++
		   parser->tokens[7] = parser->tokens[8];//zclei++
	  if(parser->tokens_avail >= 10) //zclei++
		   parser->tokens[8] = parser->tokens[9];//zclei++
	  if(parser->tokens_avail >= 11) //zclei++
		   parser->tokens[9] = parser->tokens[10];//zclei++
	  if(parser->tokens_avail >= 12) //zclei++
		   parser->tokens[10] = parser->tokens[11];//zclei++
	  if(parser->tokens_avail >= 13) //zclei++
		   parser->tokens[11] = parser->tokens[12];//zclei++
	  if(parser->tokens_avail >= 14) //zclei++
		   parser->tokens[12] = parser->tokens[13];//zclei++
	  if(parser->tokens_avail >= 15) //zclei++
		   parser->tokens[13] = parser->tokens[14];//zclei++
	  if(parser->tokens_avail >= 16) //zclei++
		   parser->tokens[14] = parser->tokens[15];//zclei++
	  if(parser->tokens_avail >= 17) //zclei++
		   parser->tokens[15] = parser->tokens[16];//zclei++
	  if(parser->tokens_avail >= 18) //zclei++
		   parser->tokens[16] = parser->tokens[17];//zclei++
	  if(parser->tokens_avail >= 19) //zclei++
		   parser->tokens[17] = parser->tokens[18];//zclei++
	  if(parser->tokens_avail >= 20) //zclei++
		   parser->tokens[18] = parser->tokens[19];//zclei++
	  if(parser->tokens_avail >= 21) //zclei++
		   parser->tokens[19] = parser->tokens[20];//zclei++
	  if(parser->tokens_avail >= 22) //zclei++
		   parser->tokens[20] = parser->tokens[21];//zclei++
	  if(parser->tokens_avail >= 23) //zclei++
		   parser->tokens[21] = parser->tokens[22];//zclei++
	  if(parser->tokens_avail >= 24) //zclei++
		   parser->tokens[22] = parser->tokens[23];//zclei++
	  if(parser->tokens_avail >= 25) //zclei++
		   parser->tokens[23] = parser->tokens[24];//zclei++
	  if(parser->tokens_avail >= 26) //zclei++
		   parser->tokens[24] = parser->tokens[25];//zclei++
	  if(parser->tokens_avail >= 27) //zclei++
		   parser->tokens[25] = parser->tokens[26];//zclei++
	  if(parser->tokens_avail >= 28) //zclei++
		   parser->tokens[26] = parser->tokens[27];//zclei++
	  if(parser->tokens_avail >= 29) //zclei++
		   parser->tokens[27] = parser->tokens[28];//zclei++
	  if(parser->tokens_avail >= 30) //zclei++
		   parser->tokens[28] = parser->tokens[29];//zclei++
    }
  }
  parser->tokens_avail--;
}
   ...
   if (c_parser_next_token_is (parser, CPP_EOF)){
      ...
      do{
	   ...
	   //printf("外部声明结束 --- 并处理static count:%d %s, %s, %d \n", testcount++,__FILE__, __FUNCTION__, __LINE__);
        class_impl_compile_over(classImpl);
   }
  ...
  switch (c_parser_peek_token (parser)->type){
    case CPP_KEYWORD:
      switch (c_parser_peek_token (parser)->keyword){
        ...
        break;
    case CPP_SEMICOLON:
     pedwarn (c_parser_peek_token (parser)->location, OPT_Wpedantic,"ISO C does not allow extra %<;%> outside of a function");
...
static void c_parser_declaration_or_fndef (c_parser *parser, bool fndef_ok,
			       ...
			       bool *fallthru_attr_p)
{
  ...
  tree genericDefine=NULL_TREE;//zclei
  tree funcGeneric=NULL_TREE;//zclei 泛型函数
  ...
  if(parser->isGenericState){
		if(c_parser_next_token_is (parser, CPP_NAME)){
		   ...
		   if(generic_util_valid_id(token->value)){
			   ...
			   char *genStr=IDENTIFIER_POINTER(token->value);
		       c_parser_consume_token (parser);
			   generic_expand_replace(generic_expand_get(),genStr);
		   }
		}
  }
  ...
  if (nested && !specs->declspecs_seen_p){
	  n_debug("分析声明说明符 dddd 22vvvvv： nested:%d ", nested);
  ...
  genericDefineModel=generic_impl_pop_generic_from_declspecs(generic_impl_get(),specs);
  ...
  n_debug("分析声明说明符 33：finish_declspecs storage_class:%d", specs->storage_class);
  ...
  if (c_parser_next_token_is (parser, CPP_SEMICOLON)){
	 ...
	 }else if (empty_ok && !(have_attrs && specs->non_std_attrs_seen_p))
	     shadow_tag (specs);
     else{
    ...
    class_parser_over_enum(classParser,specs);//zclei 估计是枚举也可能不是
    return;
  }
  ...
  if (empty_ok  && specs->typespec_kind == ctsk_tagdef  && c_parser_next_token_starts_declspecs (parser)
      ...
      switch (c_parser_peek_token (parser)->type){
	     ...
	     case CPP_MINUS:
	       if (c_parser_objc_diagnose_bad_element_prefix (parser, specs))
	          return;
	       if (specs->attrs){
		       warning_at (c_parser_peek_token (parser)->location, OPT_Wattributes,"prefix attributes are ignored for methods");
	     ...
	     case RID_AT_IMPLEMENTATION:
	       {
	          if (c_parser_objc_diagnose_bad_element_prefix (parser, specs))
	             return;
	          if (specs->attrs){
		          warning_at (c_parser_peek_token (parser)->location, OPT_Wattributes,"prefix attributes are ignored for implementations");
		          specs->attrs = NULL_TREE;
	          }
	          c_parser_objc_class_definition (parser, NULL_TREE);
	          return;
  ...
  if(parser->isAet && !genericBlockFuncDefine && nested==0){
	  ...
	  nboolean ok=class_impl_add_self_to_param(classImpl,funcGenericModel!=NULL);
	  if(ok){
      ...
      if (c_parser_next_token_is (parser, CPP_EQ)
	     ...
	     if (c_parser_next_token_is_keyword (parser, RID_ATTRIBUTE)){
	         ...
	         if (c_parser_next_token_is (parser, CPP_OPEN_BRACE)){
		       ...
		       error_at (here, "attributes should be specified before the declarator in a function definition");
		       c_parser_skip_to_end_of_block_or_statement (parser);
		       return;
		    }
	     }
	     if (c_parser_next_token_is (parser, CPP_EQ)){
	         ...
	         if (auto_type_p){
		         ...
		         if (TREE_CODE (init.value) == COMPONENT_REF  && DECL_C_BIT_FIELD (TREE_OPERAND (init.value, 1)))
		        ...
		        /* As with typeof, remove all qualifiers from atomic types.  */
		         ...
		         bool vm_type = variably_modified_type_p (init_type,NULL_TREE);
		         if (vm_type)
			     ...
			     new_object_parser(new_object_get(),d,genericDefineModel);
		         init_loc = c_parser_peek_token (parser)->location;
		         rich_location richloc (line_table, init_loc);
		         ...
		         finish_init ();
		         if(genblock)
			    	block_mgr_set_lhs(block_mgr_get(),NULL_TREE);
	         ...
	         if (d != error_mark_node){
		            ...
		            if(new_object_ctor(new_object_get(),d)){
		                ;
		   		    }else{
		     ...
		     if(parser->isAet && !genericBlockFuncDefine && nested==0){
		  		  ...
		  		  setFundecl=func_mgr_change_class_impl_func_decl(func_mgr_get(),declarator,classImpl->className);
		  	 }
	         ...
	         if (omp_declare_simd_clauses.exists ()){
	          ...
	          if (oacc_routine_data)
		           c_finish_oacc_routine (oacc_routine_data, d, false);
	          if (d){
				 if(parser->isAet && setFundecl && !genericBlockFuncDefine && nested==0){
			        ...
			        func_mgr_set_class_impl_func_decl(func_mgr_get(),d,classImpl->className);
				 }
				 finish_decl (d, UNKNOWN_LOCATION, NULL_TREE,NULL_TREE, asm_name);
			     n_debug("test test----11 context:%p extern:%d static:%d pub:%d ",DECL_CONTEXT(d),DECL_EXTERNAL(d),TREE_STATIC(d),TREE_PUBLIC(d));
				 if(parser->isAet && setFundecl){
					 ;
				 }else{
					 n_debug("分析声明说明符 120 :为变量设泛型 count:%d",testcount);
					  if(genericDefineModel){
						 printf("分析声明说明符 121 泛型变量有效 11\n");
						 c_aet_set_generics_model(d,genericDefineModel);
						 generic_impl_check_var(generic_impl_get(),d,genericDefineModel);
		              }
	    		 }
	     ...
	     if(c_parser_next_token_is (parser, CPP_COMMA)){
			 ...
			 if (auto_type_p){
				 error_at (here,"%<__auto_type%> may only be used with a single declarator");
				 c_parser_skip_to_end_of_block_or_statement (parser);
				 return;
			 }
			 c_parser_consume_token (parser);
...
//	     }
	 ...
	 function definitions either.  */
      ...
      while (c_parser_next_token_is_not (parser, CPP_EOF)  && c_parser_next_token_is_not (parser, CPP_OPEN_BRACE))
	      c_parser_declaration_or_fndef (parser, false, false, false,true, false, NULL, vNULL);
	  n_debug("分析声明说明符 128：保存函数定义的参数据 count:%d declarator:%p", testcount,declarator);
      ...
      /* If the definition was marked with __GIMPLE then parse the
	     ...
	     in_late_binary_op = saved;
		  ...
		  n_debug("分析声明说明符 131：对函数内部的复合语句进行分析 count:%d declarator:%p", testcount,declarator);
		  	aet_print_token(c_parser_peek_token (parser));
      ...
      if (nested){
		  ...
		  n_debug("分析声明说明符 132： 内部函数 count:%d declarator:%p funcName:%s", testcount,declarator,IDENTIFIER_POINTER(DECL_NAME(decl)));
	     ...
	     if (fnbody)
	       add_stmt (fnbody);
	     finish_function (endloc);
	     class_impl_finish_function(class_impl_get(),fndecl);
	  }
  ...
  while (c_parser_next_token_is (parser, CPP_NAME)
	 ...
	 has simply forgotten a semicolon, so we exit.  */
	  ...
	  /* If we cannot accept a type, and the next token must start one,
	     ...
	     if (seen_type || !c_parser_next_tokens_start_typename (parser, la)){
	      ...
	      /* Now at an unknown typename (C_ID_ID), a C_ID_TYPENAME or a C_ID_CLASSNAME.  */
	      c_parser_consume_token (parser);
	      seen_type = true;
	      attrs_ok = true;
	      if (kind == C_ID_ID){
	         ...
	         tree record=TREE_TYPE(t.spec);
	         n_debug("类型 是 t.spec:%p %s value :%s record:%p %s",t.spec,
	        		 get_tree_code_name(TREE_CODE(t.spec)),IDENTIFIER_POINTER(value),record,get_tree_code_name(TREE_CODE(record )));
	      } else{
	      ...
	      aet_print_token(c_parser_peek_token (parser));
		  if(c_parser_next_token_is (parser, CPP_LESS)){ // zclei
			   ...
			   if(secondToken->type==CPP_NAME || secondToken->type==CPP_QUERY){
				  if(secondToken->type==CPP_NAME){
					 if(generic_util_valid_all(secondToken->value)){
						//printf("发现泛型语句 c_parser_declspecs----是 ? A-Z specs:%p\n",specs);
				 ...
				 //printf("发现泛型语句 c_parser_declspecs----是 类型\n");
				 generic_impl_push_generic_from_declspecs(generic_impl_get(),specs);
			 }
		  }
          continue;
	...
	case RID_THREAD:
	 ...
	 // tree v=c_parser_peek_token (parser)->value;
	  ...
	  c_parser_consume_token (parser);
	  break;
	case RID_AET_PACKAGE:
		c_parser_consume_token (parser); //consume package
		break;
	case RID_AET_CLASS:
	case RID_AET_INTERFACE:
		 n_debug("声明说明符语法 100 AET CLASS说明符： count:%d keyword:%d specs:%p id_kind!=C_ID_CLASSNAME:%d",
		  			  count,c_parser_peek_token (parser)->keyword,specs,
		 ...
		 if(c_parser_peek_token (parser)->id_kind!=C_ID_CLASSNAME){
			  ...
			  c_parser_consume_token (parser); //consume typedef
	 		  ...
	 		  t=class_parser_parser_class_specifier(classParser);//c_parser_peek_token 在class_parser_parser_class_specifier中被consume
...
out:
  if (end_std_attr_ok && c_parser_nth_token_starts_std_attributes (parser, 1)){
    ...
    end_at_defs:
	 ...
	 elsewhere.  */
	  ...
	  /* Stop if at the end of the struct or union contents.  */
	     if (c_parser_next_token_is (parser, CPP_CLOSE_BRACE)){
			  n_debug("分析struct 44 下一个token是CPP_CLOSE_BRACE break count:%d ",testcount);
	          c_parser_consume_token (parser);
	          break;
	      }
...
//				 testcount);
	     ...
	     n_debug("分析struct 55xxx -- %p %p",contents,DECL_LANG_SPECIFIC(contents));
      ...
      n_debug("分析struct 77 完成finish_struct count:%d--- ret.spec:%p",testcount,ret.spec);
	  aet_print_token(c_parser_peek_token (parser));
 ...
 * 方法中循环的
 */
static tree c_parser_struct_declaration (c_parser *parser)
{
  ...
  if (c_parser_next_token_is_keyword (parser, RID_EXTENSION)){
      ...
      restore_extension_diagnostics (ext);
      n_debug("struct内的声明 -------00 RID_EXTENSION 返回");
      return decl;
  }
...
c_parser_typeof_specifier (c_parser *parser)
{
  ...
  if (!parens.require_open (parser))
   ...
   generic_expand_parser_typeof(generic_expand_get());
...
struct c_declarator *c_parser_declarator (c_parser *parser, bool type_seen_p, c_dtr_syn kind,bool *seen_id)
{
  /* Parse any initial pointer part.  */
    if (c_parser_next_token_is (parser, CPP_MULT)){
       struct c_declspecs *quals_attrs = build_null_declspecs ();
       multCount++;
 	   n_debug("解析声明符：00 是*号 --- specs:%p multCount:%d", quals_attrs,multCount);
       struct c_declarator *inner;
       c_parser_consume_token (parser);
       c_parser_declspecs (parser, quals_attrs, false, false, true,false, false, true, false, cla_prefer_id);
       inner = c_parser_declarator (parser, type_seen_p, kind, seen_id);
 	   n_debug("解析声明符：11 是*号 --- inner:%p multCount:%d", inner,multCount);
       if (inner == NULL)
	      return NULL;
       else{
    	  struct c_declarator *pointer= make_pointer_declarator (quals_attrs, inner);
    	  n_debug("解析声明符：22 返回指针类型的c_declarator  新的declarator:%p inner:%p specs:%p multCount:%d",pointer,inner,quals_attrs,multCount);
    	  return pointer;
       }
   }
  /* Now we have a direct declarator, direct abstract declarator or
     nothing (which counts as a direct abstract declarator here).  */
   n_debug("解析声明符：33 进入  c_parser_direct_declarator---multCount:%d", multCount);
...
static struct c_declarator *c_parser_direct_declarator (c_parser *parser, bool type_seen_p, c_dtr_syn kind,
			    bool *seen_id)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_PAREN)){
      ...
      attrs = c_parser_gnu_attributes (parser);
      if (kind != C_DTR_NORMAL && (c_parser_next_token_starts_declspecs (parser) ||
    		  (!have_gnu_attrs  && c_parser_nth_token_starts_std_attributes (parser, 1))
	      ...
	      if (args == NULL)
	        return NULL;
	         ...
	         if (!(args->types  && args->types != error_mark_node && TREE_CODE (TREE_VALUE (args->types)) == IDENTIFIER_NODE)
		        && c_parser_nth_token_starts_std_attributes (parser, 1)){
...
static struct c_declarator *c_parser_direct_declarator_inner (c_parser *parser, bool id_present,struct c_declarator *inner)
{
  /* Parse a sequence of array declarators and parameter lists.  */
  if (c_parser_next_token_is (parser, CPP_OPEN_SQUARE)  && !c_parser_nth_token_starts_std_attributes (parser, 1)){
      ...
      c_parser_consume_token (parser);
      c_parser_declspecs (parser, quals_attrs, false, false, true,false, false, false, false, cla_prefer_id);
	 ...
	 dimension.  */
      ...
      n_debug("inner直接解析声明 22 ");
       ...
       return c_parser_direct_declarator_inner (parser, id_present, inner);
        ...
        attrs = c_parser_gnu_attributes (parser);
        //在这里加入self
        args = c_parser_parms_declarator (parser, id_present, attrs,have_gnu_attrs);
        if (args == NULL){
           ...
           //这里的inner的kind是
	       struct c_declarator *temp=inner;
	       inner = build_function_declarator (args, inner);
...
static struct c_arg_info *c_parser_parms_declarator (c_parser *parser, bool id_list_ok, tree attrs,bool have_gnu_attrs)
{
  push_scope ();
  ...
  while (true){
      ...
      if (parm == NULL)
	     bad_parm = true;
	      ...
	      tree decl=push_parm_decl (parm, &expr);//zclei 该方法原来没有返回什，为了支持泛型现在加上
	      if(genericsModel!=NULL){
	    	  n_debug("参数设泛型了-------	等完成函数声明后再检查泛型是否有效。");
...
static struct c_parm *c_parser_parameter_declaration (c_parser *parser, tree attrs,bool have_gnu_attrs)
{
  struct c_declspecs *specs;
  struct c_declarator *declarator;
  tree prefix_attrs;
  ...
  if (c_parser_next_token_is_keyword (parser, RID_AET_GOTO)){
   ...
   //printf("创建c_parm -- %d %d type:%d token->id_kind:%d\n",c_parser_next_token_starts_declspecs (parser),
  ...
  if (!c_parser_next_token_starts_declspecs (parser) && !c_parser_nth_token_starts_std_attributes (parser, 1)){
      ...
      if (parser->error)
	    return NULL;
   	  //printf("创建c_parm 11  have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);

      c_parser_set_source_position_from_token (token);
   	  //printf("创建c_parm 22  have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);

      if (c_parser_next_tokens_start_typename (parser, cla_prefer_type)){
       	  //printf("创建c_parm 33  have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);
	     ...
	     if (const char *suggestion = hint.suggestion ()){
	         ...
	         richloc.add_fixit_replace (suggestion);
	       	  //printf("创建c_parm 44  have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);

	         error_at (&richloc,"unknown type name %qE; did you mean %qs?",token->value, suggestion);
	     } else{
	       	  //printf("创建c_parm 55  have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);

	        error_at (token->location, "unknown type name %qE", token->value);
	       	 // printf("创建c_parm 66  have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);
	 ...
	 should be a different message.  */
   	 ...
   	 // printf("创建c_parm 77  have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);
  ...
  location_t start_loc = c_parser_peek_token (parser)->location;
  specs = build_null_declspecs ();
  //printf("创建c_parm 88  have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);
...
//  }
  ...
  */
  ...
  while (id_declarator && id_declarator->kind != cdk_id){
	 //printf("创建c_parm 99  have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);
  ...
  //printf("创建c_parm 100 have_gnu_attrs:%d %s, %s, %d\n",have_gnu_attrs,__FILE__, __FUNCTION__, __LINE__);
...
c_parser_initializer (c_parser *parser)
{
  if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
  ...
  else
    {
      ...
      if (TREE_CODE (ret.value) != STRING_CST  && TREE_CODE (ret.value) != COMPOUND_LITERAL_EXPR)
...
again:
    ...
    while (c_parser_next_token_is_not (parser, CPP_CLOSE_BRACE)){
       ...
       if (c_parser_next_token_is_keyword (parser, RID_CASE)
	   ...
	   }else if (!last_label && c_parser_next_token_is_keyword (parser, RID_EXTENSION)){
		  ...
		  /* External pragmas, and some omp pragmas, are not associated
			 ...
			 were ignored.  */
	      if (c_parser_pragma (parser,last_label ? pragma_stmt : pragma_compound,NULL))
...
static void c_parser_statement_after_labels (c_parser *parser, bool *if_p, vec<tree> *chain)
{
    location_t loc = c_parser_peek_token (parser)->location;
    tree stmt = NULL_TREE;
    ...
    class_impl_replace_self_call_at_statement_after_labels(classImpl);//zclei 解决self()调用
    ...
    switch (c_parser_peek_token (parser)->type){
          ...
          switch (c_parser_peek_token (parser)->keyword){
	         ...
	         case RID_RETURN:
	            ...
	            n_debug("语句标签后 33 分析return isAet:%d nest:%d isCtor:%d,",
	            		parser->isAet,classImpl->nest,classImpl->isConstructor);
		           aet_print_token(c_parser_peek_token (parser));
	         ...
	         case RID_ATTRIBUTE:
	            {
	               ...
	               if (attribute_fallthrough_p (attrs)){
	                  ...
	                  warning_at (loc, OPT_Wattributes, "only attribute %<fallthrough%> can be applied to a null statement");
	               break;
	            }
	         case RID_AET_NEW:
	         ...
	         case RID_AET_SUPER:
		         ...
		         class_impl_parser_super(classImpl);
		         break;
       ...
       case CPP_PRAGMA:
          ...
          break;
       default:
     	  n_debug("语句标签后 99 把整个赋值表达式语句的树结构挂接到语句列表上 ");
          expr_stmt:
             stmt = c_finish_expr_stmt (loc, c_parser_expression_conv (parser).value);
             //zclei
             class_impl_in_finish_stmt(classImpl,stmt);
          expect_semicolon:
             c_parser_skip_until_found (parser, CPP_SEMICOLON, "expected %<;%>");
...
static struct c_expr c_parser_conditional_expression (c_parser *parser, struct c_expr *after,tree omp_atomic_lhs)
{
  ...
  while (true){
      ...
      switch (ocode){
		case TRUTH_ANDIF_EXPR:
		  ...
		  stack[sp].expr= convert_lvalue_to_rvalue (stack[sp].loc,stack[sp].expr, true, true);
		  stack[sp].expr.value = c_objc_common_truthvalue_conversion(stack[sp].loc, default_conversion (stack[sp].expr.value));
		  c_inhibit_evaluation_warnings += (stack[sp].expr.value== truthvalue_false_node);
		...
		case TRUTH_ORIF_EXPR:
		  ...
		  stack[sp].expr= convert_lvalue_to_rvalue (stack[sp].loc,stack[sp].expr, true, true);
		  stack[sp].expr.value = c_objc_common_truthvalue_conversion(stack[sp].loc, default_conversion (stack[sp].expr.value));
		  c_inhibit_evaluation_warnings += (stack[sp].expr.value== truthvalue_true_node);
		  set_c_expr_source_range (&stack[sp].expr, src_range);
		  break;
		default:
		  break;
...
static struct c_expr c_parser_cast_expression (c_parser *parser, struct c_expr *after)
{
  ...
  /* If the expression begins with a parenthesized type name, it may
    ...
    if (c_parser_next_token_is (parser, CPP_OPEN_PAREN) && c_token_starts_typename (c_parser_peek_2nd_token (parser))){
      ...
      /* Save casted types in the function's used types hash table.  */
       ...
       if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
	      return c_parser_postfix_expression_after_paren_type (parser, type_name,cast_loc);
	   ...
	   expr.value= generic_impl_cast(generic_impl_get(),type_name,expr.value);
       ret.value = c_cast_expr (cast_loc, type_name, expr.value);
       ret.value=class_impl_cast(classImpl,type_name,ret.value);
       generic_impl_ready_check_cast(generic_impl_get(),type_name,ret.value);
       ...
       return ret;
        ...
        //n_debug("c_parser_cast_expression 33 强转表达式 进入一元表达式处理流程 ");
        return c_parser_unary_expression (parser);
    }
}
  ...
  switch (c_parser_peek_token (parser)->type)
	...
	case RID_AET_GOTO:
		printf("RID_AET_GOTO at c_parser_unary_expression\n");
		{
		   ...
		   if(!re && action==GOTO_STATIC_CONTINUE_COMPILE)
	          return c_parser_postfix_expression (parser);
		   else
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_PAREN) && c_token_starts_typename (c_parser_peek_2nd_token (parser)))
    {
	 ...
	 starting with a compound literal.  */
      ...
      if (c_parser_next_token_is (parser, CPP_OPEN_BRACE)){
	     expr = c_parser_postfix_expression_after_paren_type (parser,type_name,expr_loc);
...
static struct c_expr c_parser_postfix_expression (c_parser *parser)
{
    ...
    switch (c_parser_peek_token (parser)->type){
       case CPP_NUMBER:
          expr.value = c_parser_peek_token (parser)->value;
      	  n_debug("c_parser_postfix_expression 11 这是数字 后缀表达式 %s ",get_tree_code_name(TREE_CODE(expr.value )));
       ...
       case CPP_NAME:
          ...
          switch (c_parser_peek_token (parser)->id_kind){
	         case C_ID_ID:
	         {
	        	tree id = c_parser_peek_token (parser)->value;
		   	    c_parser_consume_token (parser);
		   	    GenericModel *genericDefineModel=NULL;
				if(c_parser_peek_token (parser)->type== CPP_LESS){
					genericDefineModel=class_impl_get_func_generic_model(classImpl,id);
				}
				...
				aet_print_token(c_parser_peek_token (parser));
				expr= class_impl_process_expression(classImpl,expr,loc,genericDefineModel,id,isFun,&action);
				n_debug("c_parser_postfix_expression 33 后缀表达式 CPP_NAME:%s action:%d ",IDENTIFIER_POINTER(id),action);
				if(action==2){
			    ...
			    if(action==3){
                    n_debug("c_parser_postfix_expression 66 这是在aet中调用implicitly函数:%s action:%d ",
                                        IDENTIFIER_POINTER(id),action);
                    set_c_expr_source_range (&expr, tok_range);
                    break;
			    }
       ...
       case CPP_OPEN_PAREN:
          ...
          if (c_parser_peek_2nd_token (parser)->type == CPP_OPEN_BRACE){
	         ...
	         c_parser_consume_token (parser);
	         n_debug("c_parser_postfix_expression 66 后缀表达式 CPP_OPEN_PAREN expr.value:%p ",
		          	  	        		expr.value);
	         expr = c_parser_expression (parser);
       ...
       case CPP_KEYWORD:
          switch (c_parser_peek_token (parser)->keyword){
	         ...
	         case RID_AET_OBJECT://zclei
		         expr = class_impl_parser_object(classImpl);
	        	 break;
	         case RID_VA_ARG:
	         {
			...
			/* Parse the second argument to __builtin_offsetof.  We
			   ...
			   accept sub structure and sub array references.  */
	            if (c_parser_next_token_is (parser, CPP_NAME)){
		           ...
		           offsetof_ref = build_component_ref(loc, offsetof_ref, comp_tok->value, comp_tok->location);
		           c_parser_consume_token (parser);

		           while (c_parser_next_token_is (parser, CPP_DOT) || c_parser_next_token_is (parser,CPP_OPEN_SQUARE)
		              ...
		              if (c_parser_next_token_is (parser, CPP_DEREF)){
			             ...
			             c_parser_skip_until_found (parser, CPP_CLOSE_SQUARE,"expected %<]%>");
			             n_debug("c_parser_postfix_expression 77 数组引用");
	         ...
	         case RID_BUILTIN_TGMATH:
	         {
	            ...
	            *p = convert_lvalue_to_rvalue (loc, *p, true, true);
	            ...
	            for (unsigned int j = 1; j < num_functions; j++){
		           ...
		           if (this_nargs != nargs){
		              error_at ((*cexpr_list)[j].get_location (),"argument %u of %<__builtin_tgmath%> has wrong number of arguments", j + 1);
		              expr.set_error ();
		              goto out;
		           }
	            }
	            ...
	            for (unsigned int j = 1; j < num_functions; j++){
		           ...
		           if (ret != parm_first[0]){
		              ...
		              if (!SCALAR_FLOAT_TYPE_P (parm_first[0])	&& !COMPLEX_FLOAT_TYPE_P (parm_first[0])){
			             error_at ((*cexpr_list)[0].get_location (),"invalid type-generic return type for argument %u of %<__builtin_tgmath%>",1);
			             expr.set_error ();
			             goto out;
		              }
		              if (!SCALAR_FLOAT_TYPE_P (ret)&& !COMPLEX_FLOAT_TYPE_P (ret)){
			             error_at ((*cexpr_list)[j].get_location (),"invalid type-generic return type for argument %u of %<__builtin_tgmath%>",j + 1);
			             expr.set_error ();
			             goto out;
		              }
		           }
		           ...
		           FOREACH_FUNCTION_ARGS (type, t, iter){
		              ...
		              if (t != parm_first[argpos]){
			             ...
			             if (!SCALAR_FLOAT_TYPE_P (parm_first[argpos])  && !COMPLEX_FLOAT_TYPE_P (parm_first[argpos])){
			                error_at ((*cexpr_list)[0].get_location (),"invalid type-generic type for argument %u of argument %u of %<__builtin_tgmath%>",
			                		argpos, 1);
			                expr.set_error ();
			                goto out;
			             }
			             if (!SCALAR_FLOAT_TYPE_P (t)  && !COMPLEX_FLOAT_TYPE_P (t)) {
			                error_at ((*cexpr_list)[j].get_location (),"invalid type-generic type for argument %u of argument %u of %<__builtin_tgmath%>",
			                		argpos, j + 1);
			                expr.set_error ();
			                goto out;
			             }
		              ...
		              argpos++;
		           } //end FOREACH_FUNCTION_ARGS (type, t, iter)
	            }//end unsigned int j = 1; j < num_functions; j++
			...
			/* Determine the type of the relevant parameter for each
	           ...
	           for (unsigned int j = 0; j < num_functions; j++){
	            ...
	            FOR_EACH_VEC_ELT (tg_type, i, t){
		           ...
		           if (tg_types.add (t)){
		              error_at ((*cexpr_list)[i].get_location (),"duplicate type-generic parameter type for function argument %u of %<__builtin_tgmath%>",
			             i + 1);
		              expr.set_error ();
		              goto out;
		           }
	            ...
	            for (unsigned int j = 0; j < num_functions; j++){
		           ...
		           tree type = TREE_TYPE (TREE_TYPE ((*cexpr_list)[j].value));
		           tree ret = TYPE_MAIN_VARIANT (TREE_TYPE (type));
		           if ((parm_kind[0] == tgmath_complex && ret != exp_type) || (parm_kind[0] == tgmath_real && ret != exp_real_type)){
		              error_at ((*cexpr_list)[j].get_location (),"bad return type for function argument %u of %<__builtin_tgmath%>", j + 1);
		              expr.set_error ();
		              goto out;
		           }
		           argpos = 1;
		           FOREACH_FUNCTION_ARGS (type, t, iter){
		              if (t == void_type_node)
		                 break;
		              t = TYPE_MAIN_VARIANT (t);
		              if ((parm_kind[argpos] == tgmath_complex && t != exp_type)|| (parm_kind[argpos] == tgmath_real  && t != exp_real_type)){
			             error_at ((*cexpr_list)[j].get_location (), "bad type for argument %u of function argument %u of %<__builtin_tgmath%>", argpos, j + 1);
			             expr.set_error ();
			             goto out;
		              }
		              argpos++;
	            ...
	            for (unsigned int j = 1; j <= nargs; j++){
		           ...
		           if (!INTEGRAL_TYPE_P (type)  && !SCALAR_FLOAT_TYPE_P (type)  && TREE_CODE (type) != COMPLEX_TYPE){
		              error_at (ce->get_location (),"invalid type of argument %u of type-generic function", j);
		              expr.set_error ();
		              goto out;
		           }
		           if (DECIMAL_FLOAT_TYPE_P (type)){
		              ...
		              if (all_complex){
			             error_at (ce->get_location (),"decimal floating-point argument %u to complex-only type-generic function", j);
			             expr.set_error ();
			             goto out;
		              }else if (all_binary){
			             error_at (ce->get_location (),"decimal floating-point argument %u to binary-only type-generic function", j);
			             expr.set_error ();
			             goto out;
		              }else if (arg_complex){
			             error_at (ce->get_location (),"both complex and decimal floating-point arguments to type-generic function");
			             expr.set_error ();
			             goto out;
		              }else if (arg_binary){
			             error_at (ce->get_location (),"both binary and decimal floating-point arguments to type-generic function");
			             expr.set_error ();
			             goto out;
		              }
		              ...
		              if (COMPLEX_FLOAT_TYPE_P (type))
		                 arg_binary = true;
		              if (all_decimal){
			             error_at (ce->get_location (),"complex argument %u to decimal-only type-generic function", j);
			             expr.set_error ();
			             goto out;
		              ...
		              arg_binary = true;
		              if (all_decimal){
			             error_at (ce->get_location (),"binary argument %u to decimal-only type-generic function", j);
			             expr.set_error ();
			             goto out;
		              }else if (arg_int_decimal){
			             error_at (ce->get_location (),"both binary and decimal floating-point arguments to type-generic function");
	...
	case RID_AET_SUPER:
	  ...
	  expr=class_impl_parser_super_at_postfix_expression(classImpl,expr);
	  break;
	default:
	  c_parser_error (parser, "expected expression");
	  expr.set_error ();
...
static struct c_expr c_parser_postfix_expression_after_primary (c_parser *parser,location_t expr_loc,struct c_expr expr)
{
       ...
       switch (c_parser_peek_token (parser)->type){
	        ...
	        /* Function call.  */
	         ...
	         if(class_impl_is_aet_class_component_ref_call(classImpl,expr)){
	        	 ...
	        	 //printf("加self之前的exprlist 00 %p %d\n",exprlist,exprlist->length());
	              ...
	              // printf("加self之前的exprlist 11 %p %d\n",exprlist,exprlist->length());
	         }
	         expr=class_impl_replace_func_id(classImpl,expr,exprlist,origtypes,arg_loc,expr_loc);
	         orig_expr = expr;
	         n_debug("c_parser_postfix_expression_after_primary 33 函数参数处理完成 expr.value:%s origtypes:%p 参数个数:%d",
	        		 get_tree_code_name(TREE_CODE(expr.value)),origtypes,vec_safe_length (exprlist));
	         mark_exp_read (expr.value);
	         ...
	         if (TREE_CODE (expr.value) == FUNCTION_DECL){
	            ...
	            if (warn_absolute_value  && fndecl_built_in_p (expr.value, BUILT_IN_NORMAL)  && vec_safe_length (exprlist) == 1)
		           warn_for_abs (expr_loc, expr.value, (*exprlist)[0]);
	         }

	         start = expr.get_start ();
	         finish = parser->tokens_buf[0].get_finish ();
	         expr.value= c_build_function_call_vec (expr_loc, arg_loc, expr.value,exprlist, origtypes);
	         n_debug("c_parser_postfix_expression_after_primary 33 整合 expr.value:%p %s origtypes:%p",
	        		 expr.value,get_tree_code_name(TREE_CODE(expr.value)),origtypes);
	         expr.value=generic_call_convert_generic_to_user(generic_call_get(),expr.value);
	         expr=class_impl_nameles_call(classImpl,expr);
	         set_c_expr_source_range (&expr, start, finish);
	         expr.original_code = ERROR_MARK;
	         if (TREE_CODE (expr.value) == INTEGER_CST   && TREE_CODE (orig_expr.value) == FUNCTION_DECL
	      ...
	      case CPP_DOT:
	         /* Structure element reference.  */
	    	  n_debug("c_parser_postfix_expression_after_primary 44 DOT引用. %p %s ",
	         ...
	         c_parser_consume_token (parser);
	         if(class_dot_is_class_ref(classImpl->classDot,expr.value)){
	       	     n_debug("c_parser_postfix_expression_after_primary 55 DOT引用.name:%s是一个aet class %p %s ",
	             ...
	             expr.value = build_component_ref (op_loc, expr.value, ident, comp_loc);
	         }
	         set_c_expr_source_range (&expr, start, finish);
	         expr.original_code = ERROR_MARK;
	         if (TREE_CODE (expr.value) != COMPONENT_REF)
	      ...
	      {
	         /* Structure element reference.  */
	    	  n_debug("c_parser_postfix_expression_after_primary 44 指针引用-> %p %s ",
	    			  expr.value, get_tree_code_name(TREE_CODE(expr.value)));
	         c_parser_consume_token (parser);
	         ...
	         if (c_parser_next_token_is (parser, CPP_NAME)){
	            ...
	            ident = comp_tok->value;
	            comp_loc = comp_tok->location;
		    	n_debug("c_parser_postfix_expression_after_primary 55 指针引用-> name:%s expr.value:%p %s ",
		    			IDENTIFIER_POINTER(ident),expr.value,get_tree_code_name(TREE_CODE(expr.value)));
		     }else{
	            c_parser_error (parser, "expected identifier");
	            expr.set_error ();
	         ...
	         if(class_ref_is_class_ref(classImpl->classRef,expr.value)){
	            ...
	            expr.value = build_component_ref (op_loc,build_indirect_ref (op_loc,expr.value,RO_ARROW),ident, comp_loc);
	            n_debug("c_parser_postfix_expression_after_primary 66 指针引用-> name:%s expr.value:%p ",
	         		    			IDENTIFIER_POINTER(ident),expr.value);
	         }
	         set_c_expr_source_range (&expr, start, finish);
	         expr.original_code = ERROR_MARK;
	         if (TREE_CODE (expr.value) != COMPONENT_REF)
	      ...
	      case CPP_PLUS_PLUS:
	         /* Postincrement.  */
	    	  n_debug("c_parser_postfix_expression_after_primary 77 CPP_PLUS_PLUS %s ",
	    	  		    			get_tree_code_name(TREE_CODE(expr.value)));
	         start = expr.get_start ();
	         finish = c_parser_peek_token (parser)->get_finish ();
	         c_parser_consume_token (parser);
	         expr = default_function_array_read_conversion (expr_loc, expr);
	         expr.value = build_unary_op (op_loc, POSTINCREMENT_EXPR,expr.value, false);
	         set_c_expr_source_range (&expr, start, finish);
	         expr.original_code = ERROR_MARK;
	         expr.original_type = NULL;
	         break;
	      case CPP_MINUS_MINUS:
	         ...
	         c_parser_consume_token (parser);
	         expr = default_function_array_read_conversion (expr_loc, expr);
	         expr.value = build_unary_op (op_loc, POSTDECREMENT_EXPR,expr.value, false);
	         set_c_expr_source_range (&expr, start, finish);
	         expr.original_code = ERROR_MARK;
	         expr.original_type = NULL;
	         break;
...
static struct c_expr c_parser_expression (c_parser *parser)
{
...
static struct c_expr c_parser_expression_conv (c_parser *parser)
{
  ...
  if(expr.value && (VAR_P(expr.value) || TREE_CODE(expr.value)==COMPONENT_REF) && c_aet_get_is_modify_stack_new(expr.value)){
	  ...
	  c_aet_set_modify_stack_new(expr.value,0);
...
c_parse_file (void)
{
  ...
  *the_parser = tparser;
  ...
  //zclei
  aet_print_set_parser(the_parser);
  classParser=class_parser_get();
  class_parser_set_parser(classParser,the_parser);
  classImpl=class_impl_get();
  class_impl_set_parser(classImpl,the_parser);
...
tree c_c_parser_std_attribute_specifier_sequence (c_parser *parser)
{
	return c_parser_std_attribute_specifier_sequence(parser);
}

tree c_c_parser_gnu_attributes (c_parser *parser)
{
	return c_parser_gnu_attributes(parser);
}
...
bool c_c_parser_nth_token_starts_std_attributes (c_parser *parser,unsigned int n)
{
	return c_parser_nth_token_starts_std_attributes(parser,n);
}

void c_c_parser_static_assert_declaration_no_semi (c_parser *parser)
{
	c_parser_static_assert_declaration_no_semi(parser);
}


void c_c_parser_skip_to_end_of_block_or_statement (c_parser *parser)
{
    c_parser_skip_to_end_of_block_or_statement(parser);
}
...
struct c_expr c_c_parser_expr_no_commas (c_parser *parser, struct c_expr *after,tree omp_atomic_lhs)
{
	return c_parser_expr_no_commas(parser,after,omp_atomic_lhs);
}
...
bool c_c_parser_pragma (c_parser *parser, int pc/*enum pragma_context*/, bool *if_p)
{
	return c_parser_pragma(parser,pc,if_p);
}

void c_c_parser_asm_definition (c_parser *parser)
{
	c_parser_asm_definition(parser);
}

tree c_c_parser_compound_statement (c_parser *parser, location_t *endlocp)
{
	return c_parser_compound_statement(parser,endlocp);
}


void c_c_parser_static_assert_declaration (c_parser *parser)
{
	c_parser_static_assert_declaration(parser);
}
...
struct c_expr c_c_parser_initializer (c_parser *parser)
{
	return c_parser_initializer(parser);
}

void c_c_parser_declaration_or_fndef (c_parser *parser, bool fndef_ok,
			       bool static_assert_ok, bool empty_ok,
			       bool nested, bool start_attr_ok,
			       tree *objc_foreach_object_declaration,
			       vec<c_token> omp_declare_simd_clauses,
			       ...
			       bool *fallthru_attr_p)
{
	c_parser_declaration_or_fndef(parser,fndef_ok,static_assert_ok,
			empty_ok,nested,start_attr_ok,objc_foreach_object_declaration,
