...
clobber_group * clobber_info::recompute_group ()
{
   ...
   clobber_group *group = splay_tree::splay_and_search (this, nullptr,find_group);
...
void resource_info::print_identifier (pretty_printer *pp) const
{
   if (is_mem ())
      pp_string (pp, "mem");
   else{
...
// See the comment above the declaration.
void resource_info::print_context (pretty_printer *pp) const
{
   ...
   if (mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,regno)){
      if (const char *name = mtcsReg->hardRegs.x_reg_names/*!reg_names*/[regno]){
         pp_space (pp);
         pp_left_paren (pp);
         pp_string (pp, name);
         if (mode != mtcsMode->modes.M_BLKmode){
            pp_colon (pp);
            pp_string (pp, mtcs_mode_get_name/*!GET_MODE_NAME*/(mtcsMode,mode));
         }
         pp_right_paren (pp);
      }
   }else if (is_reg ()){
...
// See the comment above the declaration.
void resource_info::print (pretty_printer *pp) const
{
   print_identifier (pp);
   print_context (pp);
}
...
void access_info::print_prefix_flags (pretty_printer *pp) const
{
   if (m_is_temp)
...
void access_info::print_properties_on_new_lines (pretty_printer *pp) const
{
...
// link information.
inline bool use_info::check_integrity ()
{
...
// See the comment above the declaration.
void use_info::print_location (pretty_printer *pp) const
{
  if (is_in_phi ())
...
// See the comment above the declaration.
void use_info::print_def (pretty_printer *pp) const
{
...
// See the comment above the declaration.
void use_info::print (pretty_printer *pp, unsigned int flags) const
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsMode *mtcsMode=mtcsTarget->mtcsMode;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
...
// See the comment above the declaration.
void def_info::print_identifier (pretty_printer *pp) const
{
...
// See the comment above the declaration.
void def_info::print_location (pretty_printer *pp) const
{
  insn ()->print_identifier_and_location (pp);
}

// See the comment above the declaration.
void clobber_info::print (pretty_printer *pp, unsigned int flags) const
{
   print_prefix_flags (pp);
   if (is_call_clobber ())
      pp_string (pp, "call ");
...
// See the comment above the declaration.
void set_info::print_uses_on_new_lines (pretty_printer *pp) const
{
   ...
   if (m_use_tree){
      ...
      pp_newline_and_indent (pp, 2);
      auto print_use = [](pretty_printer *pp,splay_tree_node<use_info *> *node)
      {
         pp_string (pp, "use by ");
...
// See the comment above the declaration.
void set_info::print (pretty_printer *pp, unsigned int flags) const
{
   print_prefix_flags (pp);
   pp_string (pp, "set ");
   print_identifier (pp);
...
// See the comment above the declaration.
void phi_info::print (pretty_printer *pp, unsigned int flags) const
{
   print_prefix_flags (pp);
...
// See the comment above the declaration.
void set_node::print (pretty_printer *pp) const
{
   pp_access (pp, first_def ());
}

// See the comment above the declaration.
clobber_info *clobber_group::prev_clobber (insn_info *insn) const
{
...
// See the comment above the declaration.
clobber_info * clobber_group::next_clobber (insn_info *insn) const
{
   auto &tree = const_cast<clobber_tree &> (m_clobber_tree);
   int comparison = lookup_clobber (tree, insn);
   if (comparison >= 0)
...
// See the comment above the declaration.
void clobber_group::print (pretty_printer *pp) const
{
...
// See the comment above the declaration.
def_info * def_lookup::prev_def (insn_info *insn) const
{
   if (mux && comparison == 0)
...
// See the comment above the declaration.
def_info *def_lookup::next_def (insn_info *insn) const
{
   if (mux && comparison == 0)
      if (auto *node = mux.dyn_cast<def_node *> ())
         if (auto *group = dyn_cast<clobber_group *> (node))
...
clobber_group *function_info::need_clobber_group (clobber_info *clobber)
{
...
// otherwise.
def_node *function_info::need_def_node (def_info *def)
{
...
// the splay tree.
def_splay_tree function_info::need_def_splay_tree (def_info *last)
{
...
// find_access_index.
int mtcs_rtl_ssa::lookup_def (def_splay_tree &tree, insn_info *insn)
{
...
int mtcs_rtl_ssa::lookup_clobber (clobber_tree &tree, insn_info *insn)
{
...
// details.
def_lookup function_info::find_def (resource_info resource, insn_info *insn)
{
...
void function_info::insert_def_before (def_info *def, def_info *before)
{
   gcc_checking_assert (!def->has_def_links () && *before->insn () > *def->insn ());
...
void function_info::insert_def_after (def_info *def, def_info *after)
{
   gcc_checking_assert (!def->has_def_links () && *after->insn () < *def->insn ());
...
void function_info::remove_def_from_list (def_info *def)
{
...
void function_info::add_clobber (clobber_info *clobber, clobber_group *group)
{
...
// CLOBBER->resource ().
void function_info::remove_clobber (clobber_info *clobber, clobber_group *group)
{
...
void function_info::prepend_clobber_to_group (clobber_info *clobber, clobber_group *group)
{
...
// CLOBBER is not currently part of any group.
void function_info::append_clobber_to_group (clobber_info *clobber,clobber_group *group)
{
...
void function_info::merge_clobber_groups (clobber_info *clobber1,
                 clobber_info *clobber2, def_info *last)
{
   if (clobber1->is_in_group () && clobber2->is_in_group ()){
      clobber_group *group1 = clobber1->group ();
      clobber_group *group2 = clobber2->group ();
      gcc_checking_assert (clobber1 == group1->last_clobber () && clobber2 == group2->first_clobber ());
...
// the defs together.
std::array<clobber_group *, 2> function_info::split_clobber_group (clobber_group *group, insn_info *insn)
{
   // Search for either the previous or next clobber in the group.
   // The result is less than zero if CLOBBER should come before NEIGHBOR
   // or greater than zero if CLOBBER should come after NEIGHBOR.
...
void function_info::append_def (def_info *def)
{
...
void function_info::add_def (def_info *def)
{
   gcc_checking_assert (!def->has_def_links ()
         && !def->m_is_temp
...
void function_info::remove_def (def_info *def)
{
   def_info **head = &m_defs[def->regno () + 1];
   def_info *first = *head;
...
void function_info::need_use_splay_tree (set_info *def)
{
   if (!def->m_use_tree)
...
static inline int compare_use_insns (insn_info *insn1, insn_info *insn2)
{
...
int mtcs_rtl_ssa::lookup_use (splay_tree<use_info *> &tree, insn_info *insn)
{
...
void function_info::insert_use_before (use_info *use, use_info *before)
{
...
//
// This routine should not be used for inserting phi uses.
void function_info::insert_use_after (use_info *use, use_info *after)
{
   set_info *def = use->def ();
...
void function_info::add_use (use_info *use)
{
...
void function_info::reparent_use (use_info *use, set_info *new_def)
{
   remove_use (use);
   use->set_def (new_def);
   add_use (use);
...
void function_info::remove_use (use_info *use)
{
   set_info *def = use->def ();
   if (!def)
   return;
...
def_array function_info::insert_temp_clobber (obstack_watermark &watermark,
...
// See the comment above the declaration.
bool function_info::remains_available_at_insn (const set_info *set, insn_info *insn)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,set->regno ())
   && mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&m_clobbered_by_calls, set->regno ()))
...
// See the comment above the declaration.
bool function_info::remains_available_on_exit (const set_info *set, bb_info *bb)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,set->regno ())
   && mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&m_clobbered_by_calls, set->regno ())){
      insn_info *search_insn = (set->bb () == bb  ? set->insn () : bb->head_insn ());
   ...
   return (set->is_last_def ()  || *set->next_def ()->insn () > *bb->end_insn ());
}
...
use_info *function_info::make_use_available (use_info *use, bb_info *bb, bool will_be_debug_use)
{
   set_info *def = use->def ();
   if (!def)
      return use;
...
// See the comment above the declaration.
use_array function_info::make_uses_available (obstack_watermark &watermark,
                use_array uses, bb_info *bb,  bool will_be_debug_uses)
{
...
set_info *function_info::create_set (obstack_watermark &watermark,
            insn_info *insn, resource_info resource)
{
...
use_info *function_info::create_use (obstack_watermark &watermark,insn_info *insn,set_info *set)
{
...
// represent ACCESS1.
static bool can_merge_accesses (access_info *access1, access_info *access2)
{
   if (access1 == access2)
      return true;
...
// See the comment above the declaration.
access_array mtcs_rtl_ssa::merge_access_arrays_base (obstack_watermark &watermark,
               access_array accesses1, access_array accesses2)
{
   if (accesses1.empty ())
      return accesses2;
   if (accesses2.empty ())
...
// See the comment above the declaration.
access_array mtcs_rtl_ssa::insert_access_base (obstack_watermark &watermark,
...
// See the comment above the declaration.
use_array mtcs_rtl_ssa::remove_uses_of_def (obstack_watermark &watermark, use_array uses,
              def_info *def)
{
...
// See the comment above the declaration.
access_array mtcs_rtl_ssa::remove_note_accesses_base (obstack_watermark &watermark,
...
// See the comment above the declaration.
bool mtcs_rtl_ssa::accesses_reference_same_resource (access_array accesses1,
...
// See the comment above the declaration.
bool mtcs_rtl_ssa::insn_clobbers_resources (insn_info *insn, access_array accesses)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (insn->is_call () && accesses_include_hard_registers (accesses)){
      mtcs_function_abi abi = mtcs_func_abi_insn_callee_abi/*!insn_callee_abi*/(mtcsFuncAbi,insn->rtl ());
      for (const access_info *access : accesses){
         if (!mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,access->regno ()))
            break;
...
void mtcs_rtl_ssa::pp_resource (pretty_printer *pp, resource_info resource)
{
   resource.print (pp);
}

// Print ACCESS to PP.  FLAGS is a bitmask of PP_ACCESS_* flags.
void mtcs_rtl_ssa::pp_access (pretty_printer *pp, const access_info *access,unsigned int flags)
{
   if (!access)
      pp_string (pp, "<null>");
...
void mtcs_rtl_ssa::pp_accesses (pretty_printer *pp, access_array accesses,unsigned int flags)
{
   if (accesses.empty ())
   pp_string (pp, "none");
   else{
...
// Print NODE to PP.
void mtcs_rtl_ssa::pp_def_node (pretty_printer *pp, const def_node *node)
{
   if (!node)
      pp_string (pp, "<null>");
...
// Print MUX to PP.
void mtcs_rtl_ssa::pp_def_mux (pretty_printer *pp, def_mux mux)
{
   if (auto *node = mux.dyn_cast<def_node *> ())
      pp_def_node (pp, node);
   else
...
// Print DL to PP.
void mtcs_rtl_ssa::pp_def_lookup (pretty_printer *pp, def_lookup dl)
{
...
// Print TREE to PP.
void mtcs_rtl_ssa::pp_def_splay_tree (pretty_printer *pp, def_splay_tree tree)
{
