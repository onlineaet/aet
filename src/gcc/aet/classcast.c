...
/**
 * 创建函数调用语句 dynamic_cast_iface 定义在AObject.h
 */
...
{
   if(TREE_CODE(declOrModify)==VAR_DECL || TREE_CODE(declOrModify)==MODIFY_EXPR
   ||  TREE_CODE(declOrModify)==COMPONENT_REF || TREE_CODE(declOrModify)==ARRAY_REF){
...
static tree createOffsetof(char *className,char *face,location_t loc,nboolean minus)
{
	  ...
	  // printf("class_cast createOffsetof 00 接口所属类:%s iface:%s info:%p minus:%d\n",className,face,info,minus);
	   ...
	   offsetof_ref = build_component_ref(loc, offsetof_ref, faceId, loc,UNKNOWN_LOCATION);
...
////--------------------------接口转类------------------
/**
 * 第一参数 p1 = ifaceObj->_iface_common_var.atClass123
 */
static  vec<tree, va_gc> *createDynamicIfaceToClassParms(location_t loc,tree p1,char *ifaceSysName,char *destSysName)
{
   tree p2=class_util_create_string_actual_param(loc,ifaceSysName);
   tree p3=class_util_create_string_actual_param(loc,destSysName);


   expanded_location xloc;
   xloc = expand_location(loc);
   //n_debug("位置是:%d %d %s\n",xloc.line, xloc.column,xloc.file);
   tree p4=class_util_create_string_actual_param(loc,xloc.file);//当不能动态转化时报错在那个文件。
   tree p5=class_util_create_integer_actual_param(loc,xloc.line);
   tree p6=class_util_create_integer_actual_param(loc,xloc.column);

   vec<tree, va_gc> *parmVec;
   parmVec = make_tree_vector ();
   vec_safe_push(parmVec,p1);
   vec_safe_push(parmVec,p2);
   vec_safe_push(parmVec,p3);
   vec_safe_push(parmVec,p4);
   vec_safe_push(parmVec,p5);
   vec_safe_push(parmVec,p6);
   return parmVec;
}

/**
 * 创建函数调用语句 dynamic_cast_iface 定义在AObject.h
 * static inline void* dynamic_iface_to_class(AObject *atClass,char *ifaceSysName,char *destSysName,char *compileFile,int line,int column)
 *
 */
static tree buildDynamicIfaceToClassExpr(location_t loc,tree op,char *ifaceSysName,char *destSysName)
{
   ...
   tree funcName=aet_utils_create_ident(AET_DYNAMIC_IFACE_TO_CLASS_FUNC_NAME);
   tree decl=lookup_name(funcName);
   vec<tree, va_gc> *parms=createDynamicIfaceToClassParms(loc,firstParm,ifaceSysName,destSysName);
...
/**
 * 转化类到接口或接口转类
   不能直接转化接口到类，因为不知道被转的类是否实现的接口
   class$ A;
   class$ B extends$ A implements$ C
   C c=new$ B();
   A *a=(A*)c;
   A并未实现接口C,但C其实是B,B又继承自A，所以c是可以转成A的，关键是要知道C类实现B继承自A
   可以通过c的接口变量_iface_common_var.atClass,知道实现它的类
 */
static void castClassToInterface(location_t loc,char *rightName,char *leftName,tree nopexpr,tree op)
{
   ...
   n_debug("class_cast_parser_decl castClassToInterface 00 %s %s info:%p %p\n",leftName,rightName,leftInfo,rightInfo);
   ...
   if(lhs && !rhs){
      ...
      if(belongClass!=NULL){
         ...
         if(isIfaceToClass(TREE_TYPE(nopexpr),op)){
            ClassName *belongClass=class_mgr_find_interface(class_mgr_get(),&leftInfo->className,&rightInfo->className);
            if(belongClass==NULL){
               error_at(loc,"类%qs并未实现接口%qs。",leftInfo->className.sysName,rightInfo->className.sysName);
               return ;
            }
            tree newvalue=ifaceToClass(op,TREE_TYPE(nopexpr));
            TREE_OPERAND (nopexpr, 0)=newvalue;
         }else{
            //匹配 A *=(A*)B; B是接口，A没有实现接口B
            ...
            //newvalu = (AObject*)c->_iface_common_var.atClass
             tree p1=ifaceToClass(op,aobjectPointerType);
             tree newvalue=buildDynamicIfaceToClassExpr(loc,p1,rightName,leftName);
             TREE_OPERAND (nopexpr, 0)=newvalue;
         }
      }
   }else if(lhs && rhs){
      /* IOData实现两个接口 NOutputData NInputData
      IOData *var=new$ IOData();
      NOutputData *iface1=var;
      NInputData *iface2=(NInputData*)iface1;
      op是 iface1 ,通过 ifaceToClass转成了void *, void*=var
      最后创建调用 dynamic_cast_iface 表达式。
      */
      op = ifaceToClass(op,ptr_type_node);
      op=buildDynamicClassToIfaceExpr(loc,op,leftName);
      TREE_OPERAND (nopexpr, 0)=op;
   }
}
...
static void setNopExpr(ClassCast *self ,location_t loc,tree decl,tree nopexpr,char *declClassName)
{
   enum tree_code  nopexprTypeCode=TREE_CODE(TREE_TYPE(nopexpr));
   tree op=TREE_OPERAND (nopexpr, 0);
   n_debug("class_cast_parser_decl setNopExpr  %s %s",get_tree_code_name(nopexprTypeCode),get_tree_code_name(TREE_CODE(op)));
 ...
 *如果变量是void* 不管，如果是Class 如果赋值是接口，要处理，如果是接口 赋值是Class要处理
  */
void   class_cast_in_finish_decl(ClassCast *self ,location_t loc,tree decl)
{
 ...
 *    从设setNopExpr 再设setVarDecl-----
 */
void  class_cast_in_finish_stmt(ClassCast *self ,location_t loc,tree stmt)
{
   ...
   if((TREE_CODE(lt)==VAR_DECL || TREE_CODE(lt)==COMPONENT_REF || TREE_CODE(lt)==ARRAY_REF )
         && TREE_CODE(rt)==NOP_EXPR){
      setNopExpr(self,loc,lt,rt,declClassName);
...
static tree buildClassToIface(tree op,tree castToType)
{
   ...
   tree componetRef = build_component_ref (loc,selfref,ifaceVarName, loc,UNKNOWN_LOCATION);//
 ...
 *    class$ A implements Iface1,Iface2
      A *a=new$ A();
      Iface1 *face1=(Iface1*)a;
      Iface2 *face2=(Iface2*)((A*)face1);
      区配Iface2 *face2=(Iface2*)((A*)face1);
      (Iface2*)((A*)face1);这句在经过c_parser_cast_expression-->c_cast_expr后op的类型变成了void *不是A*
 */
static int getMethodForIfaceConv(tree castToType,tree op)
{
   ...
   if(interface!=NULL && sysName==NULL && TREE_TYPE(op)==ptr_type_node){
      //处理 Iface2 *face2=(Iface2*)((A*)face1);
   ...
   ClassName *belongClass=class_mgr_find_interface(class_mgr_get(),&rightInfo->className,&leftInfo->className);
   if(belongClass==NULL){
      n_debug("getMethodForIfaceConv --- 00 需要动态转化 %s %s",interface,sysName);
      return CLASS_TO_IFACE_DYNAMIC;
   }
...
static nboolean isIfaceToClass(tree castToType,tree op)
{
   ...
   if(!class_info_is_interface(leftInfo) && class_info_is_interface(rightInfo)){
      ...
      //没有直接关系
//      if(belongClass==NULL){
//         nboolean indirect=class_mgr_have_indirect_relationship(class_mgr_get(),rightInfo,leftInfo);
//         printf("isIfaceToClass 没有直接关系，是否有间接关系 %s %s indirect:%d\n",leftSysName,rightSysName,indirect);
//         return indirect;
//      }
...
static tree buildDynamicClassToIfaceExpr(location_t loc,tree op,char *ifaceSysName)
{
   ...
   tree funcName=aet_utils_create_ident(AET_DYNAMIC_CLASS_TO_IFACE_FUNC_NAME);
   tree decl=lookup_name(funcName);
 ...
 * 这时从temp应该找到最下层的类abc。由abc再来找到接口。否则出错。
 * 如果temp中没有接口，这时进入动态转
 */
static tree convertClassToInterface(location_t loc,tree castToType,tree expr)
{
   ...
   if(TREE_CODE(expr)==NOP_EXPR || TREE_CODE(expr)==NON_LVALUE_EXPR){
      ...
      if(TREE_CODE(op)==VAR_DECL || TREE_CODE(op)==ADDR_EXPR || TREE_CODE(op)==PARM_DECL ||
         ...
         || TREE_CODE(op)==ARRAY_REF || TREE_CODE(op)==INDIRECT_REF ){
         int castMethod=getMethodForIfaceConv(castToType,op);
         n_debug("转接口的方法是:%d interface:%s\n",castMethod,interface);
   ...
   || TREE_CODE(expr)==COMPONENT_REF || TREE_CODE(expr)==TARGET_EXPR){
      int castMethod=getMethodForIfaceConv(castToType,expr);
      n_debug("转接口的方法是:%d interface:%s\n",castMethod,interface);
      if(castMethod==CLASS_TO_IFACE_STATIC){
 ...
 * 如果被转的是指针类型的Class。
 */
tree class_cast_cast(ClassCast *self,location_t loc,struct c_type_name *type_name,tree expr)
{
   if (!type_name)
      return expr;
   tree type = groktypename (type_name, NULL, NULL);
   char *sysName=class_util_get_class_name(type);//这是一个aet 类吗？
   n_debug("class_cast_cast 00 sysName:%s\n",sysName);
   if(!sysName)
      return expr;
   ...
   if(class_info_is_class(leftInfo) || class_info_is_abstract_class(leftInfo)){
      if(TREE_CODE(expr)==NOP_EXPR){
         ...
         if(isIfaceToClass(type,op)){
            ...
            n_debug("class_cast_cast 接口转类 接口：%s 转化到类:%s\n",rightSysName,leftInfo->className.sysName);
            ...
            if(rightSysName){
               ClassInfo *rightInfo=class_mgr_get_class_info(class_mgr_get(),rightSysName);
               if(class_info_is_interface(rightInfo) && !class_info_is_root(leftInfo)){
                  n_debug("class_cast_cast 22 sysName:%s rightSysName:%s\n",sysName,rightSysName);
                  //匹配 （(A*)B) B是接口，A没有实现接口B
                  ClassName *rootClassName=class_mgr_get_class_name_by_user(class_mgr_get(),AET_ROOT_OBJECT);
                  tree id=aet_utils_create_ident(rootClassName->sysName);
                  tree aobjectType=lookup_name(id);
                  aobjectType=TREE_TYPE(aobjectType);
                 ...
                 //newvalu = (AObject*)c->_iface_common_var.atClass
                  tree p1=ifaceToClass(op,aobjectPointerType);
                  tree newvalue=buildDynamicIfaceToClassExpr(loc,p1,rightSysName,sysName);
                  TREE_OPERAND (expr, 0)=newvalue;
               }
            }
         }
      }
   }else{
      n_debug("class_cast_cast 类转接口 expr:%p left:%s sysName:%s\n",expr,leftInfo->className.sysName,sysName);
...
/**
 * 接口转类
 * actualParm->_iface_common_var._atClass123
 * IFACE_COMMON_STRUCT_VAR_NAME = "_iface_common_var"
 * IFACE_AT_CLASS = "_atClass123"
 *
 */
static tree ifaceToClass(tree actualParm,tree formulaType)
{
   ...
   tree componetTwo = build_component_ref (loc,actualRef,_iface_common_var, loc,UNKNOWN_LOCATION);//
   tree atClassVarName=aet_utils_create_ident(IFACE_AT_CLASS);
   tree componetRef = build_component_ref (loc,componetTwo,atClassVarName, loc,UNKNOWN_LOCATION);//
...
{
   int castMethod=getMethodForIfaceConv(interfaceType,expr);
