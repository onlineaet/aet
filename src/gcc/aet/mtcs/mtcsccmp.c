...
static void mtcsCcmpInit(MtcsCcmp *self)
{
}
...
//原型 get_compare_parts ccmp.cc
static void getCompareParts (tree t, int *up, rtx_code *rcode,
           tree *rhs1, tree *rhs2)
{
  tree_code code;
  gimple *g = get_gimple_for_ssa_name (t);
...
static rtx expand_ccmp_next (MtcsCcmp *self,tree op, tree_code code, rtx prev,
          rtx_insn **prep_seq, rtx_insn **gen_seq)
{
  ...
  getCompareParts(op, &unsignedp, &rcode, &rhs1, &rhs2);
  return mtcsTarget->gen_ccmp_next/*!targetm.gen_ccmp_next*/(mtcsTarget,prep_seq, gen_seq, prev, rcode,
...
/* Expand conditional compare gimple G.  A typical CCMP sequence is like:
   ...
   GEN_SEQ returns all compare insns.  */
static rtx expand_ccmp_expr_1(MtcsCcmp *self,gimple *g, rtx_insn **prep_seq, rtx_insn **gen_seq)
{
  MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
  MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
  ...
  if (ccmp_tree_comparison_p (op0, bb)){
      if (ccmp_tree_comparison_p (op1, bb)){
          ...
          getCompareParts (op0, &unsignedp0, &rcode0,&logical_op0_rhs1, &logical_op0_rhs2);

          getCompareParts (op1, &unsignedp1, &rcode1,&logical_op1_rhs1, &logical_op1_rhs2);

          rtx_insn *prep_seq_1, *gen_seq_1;
          tmp = mtcsTarget->gen_ccmp_first/*!targetm.gen_ccmp_first*/(mtcsTarget,&prep_seq_1, &gen_seq_1, rcode0,
                        logical_op0_rhs1, logical_op0_rhs2);
          if (tmp != NULL){
              ret = expand_ccmp_next(self,op1, code, tmp, &prep_seq_1, &gen_seq_1);
          ...
          if (tmp == NULL || cost1 < COSTS_N_INSNS (25))
            tmp2 = mtcsTarget->gen_ccmp_first/*!targetm.gen_ccmp_first*/(mtcsTarget,
                    &prep_seq_2, &gen_seq_2, rcode1,logical_op1_rhs1, logical_op1_rhs2);
          if (!tmp && !tmp2)
            return NULL_RTX;
          if (tmp2 != NULL){
              ret2 = expand_ccmp_next(self,op0, code, tmp2, &prep_seq_2,&gen_seq_2);
          ...
          *gen_seq = gen_seq_1;
          ...
          tmp = expand_ccmp_expr_1(self,gs1, prep_seq, gen_seq);
          if (!tmp)
            return NULL_RTX;
          return expand_ccmp_next(self,op0, code, tmp, prep_seq, gen_seq);
      }
  }else{
      ...
      tmp = expand_ccmp_expr_1(self,gs0, prep_seq, gen_seq);
      if (!tmp)
          return NULL_RTX;
      return expand_ccmp_next(self,op1, code, tmp, prep_seq, gen_seq);
  }
}
...
rtx mtcs_ccmp_expand_ccmp_expr (MtcsCcmp *self,gimple *g, machine_mode mode)
{
  ...
  tmp = expand_ccmp_expr_1 (self,g, &prep_seq, &gen_seq);
...
#endif
      icode = mtcs_opinit_optab_handler/*!optab_handler*/(mtcsOpinit,cstore_optab, cc_mode);
      if (icode != CODE_FOR_nothing){
          rtx target = mtcs_emit_gen_reg_rtx/*!gen_reg_rtx*/(mtcsEmit,mode);

          mtcs_emit_emit_insn/*!emit_insn*/(mtcsEmit,prep_seq);
          mtcs_emit_emit_insn/*!emit_insn*/(mtcsEmit,gen_seq);

          tmp = mtcs_expmed_emit_cstore/*!emit_cstore*/(mtcsExpmed,target, icode, cmp_code, cc_mode, cc_mode,
...
{
    MtcsCcmp *self = n_slice_alloc0 (sizeof(MtcsCcmp));
