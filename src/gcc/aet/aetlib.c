...
//一个文件中会有多个类实现，所以先取该文件所有匹配FUNC_CEHCK_GLOBAL_VAR_NAME_PREFIX的变量
...
typedef struct _VarData{
	...
	char *data;
	int size;
	int compress;//是不是压缩数据
}VarData;

static nboolean addLibData(AetLib *self,char *varName,char *value,int size,int compress)
{
   int i;
   for(i=0;i<self->resultArray->len;i++){
      VarData *item=n_ptr_array_index(self->resultArray,i);
      if(!strcmp(item->varName,varName))
         return FALSE;
   }
   VarData *vd=(VarData *)n_slice_new0(VarData);
   vd->varName=varName;
   vd->data=value;
   vd->size=size;
   vd->compress=compress;
   n_ptr_array_add(self->resultArray,vd);
   return TRUE;
}
...
static void createGlobalVarBySingleFile_new(AetLib *self,char *fileName)
{
   ...
   for(j=0;j<result->len;j++){
      char *varName=n_ptr_array_index(result,j);
      int size=0;
      char *value=getVarValue(varName,fileName,&size);
      if(!addLibData(self,varName,value,size,TRUE)){
         n_free(value);
      }
   }
...
static void varContentWrite(AetLib *self)
{
   ...
   for(i=0;i<len;i++){
      VarData *item=(VarData *)n_ptr_array_index(self->resultArray,i);
      //fprintf(stderr,"aetlib.c varContentWrite -- %d %s compress:%d size:%d\n",i,item->varName,item->compress,item->size);
      if(item->compress){
         char *text=middle_file_decode(item->data,item->size);
         n_string_append(codes,text);
         n_free(text);
      }else{
         n_string_append(codes,item->data);
      }
      n_string_append(codes,"\n");
   }
   self->buffer=n_string_free(codes,FALSE);
}

static void aetLibInit(AetLib *self)
{
...
static char *readIface(char *buffer)
{
   ...
   char *c=buffer;
   while(strstr(c,IFACE_START)){
      char *start=strstr(c,IFACE_START);
      //printf("r0 is :%s\n",start);
      char *n=start+strlen(IFACE_START);
    //  printf("r1 is :%s\n",n);
      char *end=strstr(n,IFACE_END);
     // printf("r2 is :%s\n",end);
      int len=strlen(n);
      int remain=strlen(end);
      char ret[len-remain+1];
      memcpy(ret,n,len-remain);
      ret[len-remain]='\0';
      n_string_append(ifaces,ret);
      c = end+strlen(IFACE_END);
   }
...
{
   if(!self->haveIfaceData){
      self->implIfaces=readIface(self->buffer);
      self->haveIfaceData=TRUE;
   }
   if(self->implIfaces==NULL)
      return FALSE;
   return strstr(self->implIfaces,sysName)!=NULL;
}

NPtrArray *aet_lib_get_generic_objs(AetLib *self)
{
   if(!self->haveGenericObjs){
      self->genObjArray=generic_graph_read(self->buffer);
      self->haveGenericObjs=TRUE;
   }
   return self->genObjArray;
}
...
static char *readClassIfaceImplInfo(char *buffer)
{
   if(buffer==NULL)
      return NULL;
   NString *ifaces=n_string_new("");
...
{
   if(!self->haveClassIfaceImplInfo){
      self->classIfaceImplInfo=readClassIfaceImplInfo(self->buffer);
