  ...
  switch (code)
   ...
   if (pred_name[0] != 0)
     {
       pred = lookup_predicate (pred_name);
       if (!pred){
         fprintf(stderr,"不知道的 predicate --- %s\n",pred_name);
   ...
   if (special_predicate_operand_p (src)
     ...
     error_at (info->loc, "mode mismatch in set: %smode vs %smode",
          mtcs_gen_get_mode_name/*!GET_MODE_NAME*/(mtcs_gen_get(),dmode), mtcs_gen_get_mode_name/*!GET_MODE_NAME*/(mtcs_gen_get(),smode));
   ...
   error_at (info->loc, "operand to label_ref %smode not VOIDmode",
        mtcs_gen_get_mode_name/*!GET_MODE_NAME*/(mtcs_gen_get(),GET_MODE (XEXP (pattern, 0))));
      break;
      ...
      if (GET_MODE (pattern) != VOIDmode)
   {
     ...
     machine_mode emode
       =   mtcs_gen_is_vector_p/*!VECTOR_MODE_P*/(mtcs_gen_get(),mode) ?   mtcs_gen_get_inner/*!GET_MODE_INNER*/(mtcs_gen_get(),mode) : mode;
     if (GET_CODE (XEXP (pattern, 1)) == PARALLEL)
       {
         int expected = 1;
         unsigned int nelems;
         if (  mtcs_gen_is_vector_p/*!VECTOR_MODE_P*/(mtcs_gen_get(),mode)
        && !mtcs_gen_get_nunits/*!GET_MODE_NUNITS*/(mtcs_gen_get(),mode).is_constant (&expected))
      error_at (info->loc,
           "vec_select with variable-sized mode %s",
           mtcs_gen_get_mode_name/*!GET_MODE_NAME*/(mtcs_gen_get(),mode));
         else if (XVECLEN (XEXP (pattern, 1), 0) != expected)
      error_at (info->loc,
           "vec_select parallel with %d elements, expected %d",
           XVECLEN (XEXP (pattern, 1), 0), expected);
         else if (  mtcs_gen_is_vector_p/*!VECTOR_MODE_P*/(mtcs_gen_get(),imode)
             && mtcs_gen_get_nunits/*!GET_MODE_NUNITS*/(mtcs_gen_get(),imode).is_constant (&nelems))
      {
        int i;
     ...
     if (imode != VOIDmode && !  mtcs_gen_is_vector_p/*!VECTOR_MODE_P*/(mtcs_gen_get(),imode))
       error_at (info->loc, "%smode of first vec_select operand is not a "
             "vector mode", mtcs_gen_get_mode_name/*!GET_MODE_NAME*/(mtcs_gen_get(),imode));
     else if (imode != VOIDmode &&   mtcs_gen_get_inner/*!GET_MODE_INNER*/(mtcs_gen_get(),imode) != emode)
       error_at (info->loc, "element mode mismatch between vec_select "
             "%smode and its operand %smode",
            mtcs_gen_get_mode_name/*!GET_MODE_NAME*/(mtcs_gen_get(),emode),
            mtcs_gen_get_mode_name/*!GET_MODE_NAME*/(mtcs_gen_get(),  mtcs_gen_get_inner/*!GET_MODE_INNER*/(mtcs_gen_get(),imode)));
   }
      break;
...
safe_predicate_mode (const struct pred_data *pred, machine_mode mode)
{
  ...
  if (mtcs_gen_get_class/*!GET_MODE_CLASS*/(mtcs_gen_get(),mode) == MODE_INT
...
static void
write_header (NString *outFile, const char *header_filename)
{
...
#include \"regs.h\"\n");
  ...
  n_string_append_printf(outFile, "#include \"%s\"\n", header_filename);
  ...
  if(mtcs_gen_is_ptx(mtcs_gen_get())){
     ...
     n_string_append(outFile,"#include \"../mtcsptxrecog.h\"\n");
...
printf_indent (NString *f, unsigned int indent, const char *format, ...)
{
  va_list ap;
  va_start (ap, format);
  n_string_append_printf(f, "%*s", indent, "");
  n_string_append_vprintf(f, format, ap);
  va_end (ap);
}
...
static void
change_state (NString *f, output_state *os, position *pos, unsigned int indent)
{
...
static void
print_code (NString *f, enum rtx_code code)
{
  const char *p;
  for (p = GET_RTX_NAME (code); *p; p++)
     n_string_append_printf (f, "%c", TOUPPER (*p));
}
...
static void
print_host_wide_int (NString *f, uint64_t val)
{
  uint64_t min = uint64_t (1) << (HOST_BITS_PER_WIDE_INT - 1);
  if (val == min)
     n_string_append_printf(f, "(" HOST_WIDE_INT_PRINT_DEC_C " - 1)", val + 1);
  else
     n_string_append_printf(f, HOST_WIDE_INT_PRINT_DEC_C, val);
}

/* Print the C expression for actual parameter PARAM.  */

static void
print_parameter_value (NString *f, const parameter &param)
{
  if (param.is_param)
    n_string_append_printf(f, "i%d", (int) param.value + 1);
  else
    switch (param.type)
      {
      case parameter::UNSET:
   ...
   break;
      ...
      case parameter::MODE:
         n_string_append_printf(f, "E_%smode", mtcs_gen_get_mode_name/*!GET_MODE_NAME*/(mtcs_gen_get(),(machine_mode) param.value));
   break;

      case parameter::INT:
         n_string_append_printf(f, "%d", (int) param.value);
   break;

      case parameter::UINT:
         n_string_append_printf(f, "%u", (unsigned int) param.value);
   break;
...
static void
print_test_rtx (NString *f, output_state *os, const rtx_test &test)
{
  if (test.pos_operand >= 0)
     n_string_append_printf(f, "operands[%d]", test.pos_operand);
  else
     n_string_append_printf(f, "x%d", os->id_to_var[test.pos->id]);
}

/* Print the C expression for non-boolean test TEST.  */

static void
print_nonbool_test (NString *f, output_state *os, const rtx_test &test)
{
   const char *platName=mtcs_gen_get_platform_name(mtcs_gen_get());//zclei
  switch (test.kind)
    {
    case rtx_test::CODE:
       n_string_append(f, "GET_CODE (");
      print_test_rtx (f, os, test);
      n_string_append(f, ")");
      break;

    case rtx_test::MODE:
       n_string_append(f, "GET_MODE (");
      print_test_rtx (f, os, test);
      n_string_append(f, ")");
      break;

    case rtx_test::VECLEN:
       n_string_append(f, "XVECLEN (");
      print_test_rtx (f, os, test);
      n_string_append(f, ", 0)");
      break;

    case rtx_test::INT_FIELD:
       n_string_append(f, "XINT (");
      print_test_rtx (f, os, test);
      n_string_append_printf(f, ", %d)", test.u.opno);
      break;

    case rtx_test::REGNO_FIELD:
       n_string_append(f, "REGNO (");
      print_test_rtx (f, os, test);
      n_string_append(f, ")");
      break;

    case rtx_test::SUBREG_FIELD:
       n_string_append(f, "SUBREG_BYTE (");
      print_test_rtx (f, os, test);
      n_string_append(f, ")");
      break;

    case rtx_test::WIDE_INT_FIELD:
       n_string_append(f, "XWINT (");
      print_test_rtx (f, os, test);
      n_string_append_printf(f, ", %d)", test.u.opno);
      break;
   ...
   n_string_append_printf(f, "%s_pattern%d (", platName,routine->pattern_id);
   const char *sep = "";
   if (test.pos)
     {
       print_test_rtx (f, os, test);
   ...
   if (routine->insn_p)
     {
      n_string_append_printf(f, "%sinsn", sep);
       sep = ", ";
     }
   if (routine->pnum_clobbers_p)
     {
      n_string_append_printf(f, "%spnum_clobbers", sep);
       sep = ", ";
     }
   for (unsigned int i = 0; i < test.u.pattern->params.length (); ++i)
     {
      n_string_append_printf(f, "%s\n", sep);
...
static void
print_label_value (NString *f, const rtx_test &test, bool is_param,
         uint64_t value)
{
...
//zclei
static void printMtcsPreds(NString *f,const char *funcName)
{
    const char *platName=mtcs_gen_get_platform_name(mtcs_gen_get());//zclei
    const char *platNameObject=mtcs_gen_get_platform_object_name(mtcs_gen_get());//zclei
    char re[64];
    sprintf(re,"mtcs_%s_preds_",platName);
    if(strstr(funcName,re)){
       n_string_append_printf(f,"(Mtcs%sPreds *)mtcsTarget->mtcsPreds,",platNameObject);
    }else if( strstr(funcName,"mtcs_preds_") ){
       n_string_append(f,"mtcsTarget->mtcsPreds,");
    }
}
...
static void
print_test (NString *f, output_state *os, const rtx_test &test, bool is_param,
       uint64_t value, bool invert_p)
{
  switch (test.kind)
    ...
    case rtx_test::PATTERN:
      ...
      n_string_append_printf(f, " %s ", invert_p ? "!=" : "==");
      print_label_value (f, test, is_param, value);
      break;

    case rtx_test::SUBREG_FIELD:
       n_string_append_printf(f, "%s (", invert_p ? "maybe_ne" : "known_eq");
      print_nonbool_test (f, os, test);
      n_string_append(f, ", ");
      print_label_value (f, test, is_param, value);
      n_string_append(f, ")");
    ...
    case rtx_test::SAVED_CONST_INT:
      ...
      print_test_rtx (f, os, test);
      n_string_append_printf(f, " %s const_int_rtx[MAX_SAVED_CONST_INT + ",
          invert_p ? "!=" : "==");
    ...
    case rtx_test::PEEP2_COUNT:
      gcc_assert (!is_param && value == 1);
      n_string_append_printf(f, "peep2_current_count %s %d", invert_p ? "<" : ">=",
          test.u.min_len);
      break;
    ...
    case rtx_test::VECLEN_GE:
      ...
      n_string_append(f, "XVECLEN (");
      print_test_rtx (f, os, test);
      n_string_append_printf(f, ", 0) %s %d", invert_p ? "<" : ">=", test.u.min_len);
      break;

    case rtx_test::PREDICATE:
      gcc_assert (!is_param && value == 1);
      //fprintf(stderr,"mtcsgenrecog.c print_test %s\n",test.u.predicate.data->name);
      n_string_append_printf(f, "%s%s (", invert_p ? "!" : "", test.u.predicate.data->name);
      printMtcsPreds(f,test.u.predicate.data->name);//zclei
      print_test_rtx (f, os, test);
      n_string_append(f, ", ");
      ...
      n_string_append(f, ")");
      //fprintf(stderr,"mtcsgenrecog.c print_test 11 %s\n",test.u.predicate.data->name);

      break;

    case rtx_test::DUPLICATE:
      gcc_assert (!is_param && value == 1);
      n_string_append_printf(f, "%srtx_equal_p (", invert_p ? "!" : "");
      print_test_rtx (f, os, test);
      n_string_append_printf(f, ", operands[%d])", test.u.opno);
      break;

    case rtx_test::HAVE_NUM_CLOBBERS:
      gcc_assert (!is_param && value == 1);
      n_string_append_printf(f, "pnum_clobbers %s NULL", invert_p ? "==" : "!=");
      break;
    ...
    case rtx_test::C_TEST:
      ...
      if (invert_p)
         n_string_append(f, "!");
      mtcs_gen_print_c_condition/*!rtx_reader_ptr->print_c_condition*/(mtcs_gen_get(),f,test.u.string);
      break;
...
static exit_state print_decision (NString *f, output_state *, decision *,
              unsigned int, bool);
...
static exit_state
print_state (NString *f, output_state *os, state *s, unsigned int indent,
        bool is_final)
{
...
/* Print the code for subroutine call ACCEPTANCE (for which partial_p
   ...
   match.  */
static const char *
print_subroutine_call (NString *f, const acceptance_type &acceptance)
{
   const char *platName=mtcs_gen_get_platform_name(mtcs_gen_get());//zclei
  switch (acceptance.type)
    {
    ...
    case RECOG:
      n_string_append_printf(f, "%s_recog_%d (x1, insn, pnum_clobbers)",
          platName,acceptance.u.subroutine_id);
      return ">= 0";

    case SPLIT:
       n_string_append_printf(f, "split_%d (x1, insn)", acceptance.u.subroutine_id);
      return "!= NULL_RTX";

    case PEEPHOLE2:
       n_string_append_printf(f, "peephole2_%d (x1, insn, pmatch_len_)",
...
static exit_state
print_acceptance (NString *f, const acceptance_type &acceptance,
        unsigned int indent, bool is_final)
{
  ...
  switch (acceptance.type)
   ...
   printf_indent (f, indent, "*pnum_clobbers = %d;\n",
    ...
    case SPLIT:
      printf_indent (f, indent, "return %s_gen_split_%d (insn, operands);\n",
            platName,acceptance.u.full.code);//zclei
      return ES_RETURNED;
...
static exit_state
print_decision (NString *f, output_state *os, decision *d, unsigned int indent,
      bool is_final)
{
  uint64_t label;
  ...
  if (os->type == SUBPATTERN
      ...
      else
   {
     ...
     printf_indent (f, indent + 2, "return res");
     if (base != 0)
        n_string_append_printf(f, " + %d", base);
     n_string_append_printf(f, "; /* [%d, %d] */\n", base, base + count - 1);
     return ES_FALLTHROUGH;
   }
    }
...
static void
print_subroutine_start (NString *f, output_state *os, state *s, position *root)
{
      ...
      if (num_vars > 2)
   {
     for (unsigned int i = 2; i < num_vars; ++i)
       /* Print 8 rtx variables to a line.  */
        n_string_append_printf(f, "%s x%d",
          i == 2 ? "  rtx" : (i - 2) % 8 == 0 ? ";\n  rtx" : ",", i);
     n_string_append(f, ";\n");
   }
  ...
  if (os->type == SUBPATTERN || os->type == RECOG)
     n_string_append(f, "  int res ATTRIBUTE_UNUSED;\n");
  else
     n_string_append(f, "  rtx_insn *res ATTRIBUTE_UNUSED;\n");
}

/* Output the definition of pattern routine ROUTINE.  */

static void
print_pattern (NString *outFile, output_state *os, pattern_routine *routine,
          bool in_header = false)
{
  const char *platName=mtcs_gen_get_platform_name(mtcs_gen_get());//zclei

  if (!in_header)
    n_string_append_printf(outFile, "\nint\n%s_pattern%d (", platName,routine->pattern_id);
  else
     n_string_append_printf(outFile, "\nextern int\n%s_pattern%d (", platName,routine->pattern_id);
  const char *sep = "";
  ...
  if (routine->pos)
    {
     n_string_append_printf(outFile, "%srtx x1", sep);
      sep = ", ";
    }
  ...
  if (routine->insn_p)
    ...
    so add an ATTRIBUTE_UNUSED just in case.  */
     n_string_append_printf(outFile, "%srtx_insn *insn ATTRIBUTE_UNUSED", sep);
      sep = ", ";
    }
  if (routine->pnum_clobbers_p)
    {
     n_string_append_printf(outFile, "%sint *pnum_clobbers", sep);
      sep = ", ";
    }
  /* Add the "i" parameters.  */
  for (unsigned int i = 0; i < routine->param_types.length (); ++i)
    {
     n_string_append_printf(outFile, "%s%s i%d", sep,
  ...
  os->type = SUBPATTERN;
  print_subroutine_start (outFile, os, routine->s, routine->pos);
  print_state (outFile, os, routine->s, 2, true);
  n_string_append(outFile, "}\n");
}
...
static void
print_subroutine (NString *f, output_state *os, state *s, int proc_id,
        bool in_header = false)
{
  const char *platName=mtcs_gen_get_platform_name(mtcs_gen_get());//zclei
  n_string_append(f, "\n");
  ...
  switch (os->type)
    ...
    case RECOG:
      if (proc_id)
         n_string_append_printf(f, "%s int\n%s_recog_%d", specifier, platName,proc_id);
      else
         n_string_append_printf(f, "%s int\n%s_recog", specifier,platName);
      n_string_append_printf(f, " (rtx x1 ATTRIBUTE_UNUSED,\n"
    ...
    case SPLIT:
      if (proc_id)
         n_string_append_printf(f, "%s rtx_insn *\nsplit_%d", specifier, proc_id);
      else
         n_string_append_printf(f, "%s rtx_insn *\n%s_split_insns", specifier,platName);//zclei
      n_string_append_printf(f, " (rtx x1 ATTRIBUTE_UNUSED, "
    ...
    case PEEPHOLE2:
      if (proc_id)
         ...
         n_string_append_printf(f, "%s rtx_insn *\n%s_peephole2_insns", specifier,platName);//zclei
      n_string_append_printf(f, " (rtx x1 ATTRIBUTE_UNUSED,\n"
          "\trtx_insn *insn ATTRIBUTE_UNUSED,\n"
  ...
  if (proc_id == 0)
    {
     n_string_append(f, "  recog_data.insn = NULL;\n");
    }
...
static void
print_subroutine_group (NString *f, NString *header, output_state *os,
...
/**
 * 替换recog_data. -->mtcsTarget->mtcsRecog.recog_data.
 */
static void replaceRecogData(NString *src)
{
   n_string_replace(src,"recog_data.","mtcsTarget->mtcsRecog->recog_data.",-1);
}
...
main (int argc, const char **argv)
{
   ...
   progname = "mtcsgenrecog";

   if (!init_rtx_reader_args_cb (argc, argv, mtcs_gen_handle_arg/*!handle_arg*/))
   ...
   snprintf (header_name, 255, "%s-insn-recog.h", platName);
   char headerFileName[255];
   sprintf(headerFileName,"%s/%s/gen/%s",saveRootPath,platName,header_name);
   FILE *header = fopen (headerFileName, "w");
   fprintf(stderr,"mtcsgenrecog.c 11 %s header:%p headerFileName:%s\n",header_name,header,headerFileName);
   NString *cFile=n_string_sized_new(1024*1024);
   NString *hFile=n_string_sized_new(10*1024);

   write_header (cFile, header_name);
   n_string_append(cFile,mtcs_gen_create_target_code(mtcs_gen_get(),"recog"));//zclei
   ...
   while (read_md_rtx (&info)){
      ...
      switch (GET_CODE (def)){
         case DEFINE_INSN:
         {
            ...
            if (GET_CODE (pattern) == PARALLEL    && remove_clobbers (&acceptance, &pattern))
               match_pattern (&insn_root, &info, pattern, acceptance);
            break;
         }
         ...
         case DEFINE_SPLIT:
            ...
            "(rtx_insn *, rtx *);\n", info.index);*/
            n_string_append_printf(hFile,"extern rtx_insn *%s_gen_split_%d (rtx_insn *, rtx *);\n",platName,info.index);//zclei
         ...
         case DEFINE_PEEPHOLE2:
            ...
            pattern matches.  The definition comes from insn-emit.cc.  */
            n_string_append_printf(hFile, "extern rtx_insn *gen_peephole2_%d "
            "(rtx_insn *, rtx *);\n", info.index);
            break;
   ...
   n_string_append_printf(cFile, "%s", "\n\n");
   ...
   if (use_pattern_routines_p){
      ...
      print_pattern (hFile, &os, routine, true);

      FOR_EACH_VEC_ELT (patterns, i, routine){
         print_pattern (cFile, &os, routine);
      }
   }
   /* Print out the matching routines.  */
   print_subroutine_group (cFile, hFile, &os, RECOG, &insn_root);
   print_subroutine_group (cFile, hFile, &os, SPLIT, &split_root);
   print_subroutine_group (cFile, hFile, &os, PEEPHOLE2, &peephole2_root);
   fprintf(header,"%s",hFile->str);
   fclose (header);
   replaceRecogData(cFile);
   mtcs_gen_replace_recog(mtcs_gen_get(),cFile);
   mtcs_gen_replace_plat_preds(mtcs_gen_get(),cFile);
   mtcs_gen_replace_common_preds(mtcs_gen_get(),cFile);
   fprintf(stdout,"%s",cFile->str);
   fflush (stdout);
   fclose (stdout);
