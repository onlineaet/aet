...
#define uncond_jump_length   (self->x_uncond_jump_length)
...
static void find_traces_1_round (MtcsBBReorder *self,int, profile_count, struct trace *, int *,
             int, bb_heap_t **, int);
static basic_block copy_bb (MtcsBBReorder *self,basic_block, edge, basic_block, int);
static long bb_to_key (MtcsBBReorder *self,basic_block);
static bool better_edge_p (MtcsBBReorder *self,const_basic_block, const_edge, profile_probability,
            profile_count, profile_probability, profile_count,const_edge);
static bool copy_bb_p (MtcsBBReorder *self,const_basic_block, int);

/* Return the trace number in which BB was visited.  */
static int bb_visited_trace (MtcsBBReorder *self,const_basic_block bb)
{
   gcc_assert (bb->index < self->array_size);
   return self->bbd[bb->index].visited;
}

/* This function marks BB that it was visited in trace number TRACE.  */
static void mark_bb_visited (MtcsBBReorder *self,basic_block bb, int trace)
{
   self->bbd[bb->index].visited = trace;
   if (self->bbd[bb->index].heap){
      self->bbd[bb->index].heap->delete_node (self->bbd[bb->index].node);
      self->bbd[bb->index].heap = NULL;
      self->bbd[bb->index].node = NULL;
   }
}
...
static bool push_to_next_round_p (const_basic_block bb, int round, int number_of_rounds,
            profile_count count_th)
{
   ...
   block_not_hot_enough = (bb->count < count_th  || probably_never_executed_bb_p (cfun, bb));

   if (there_exists_another_round  && block_not_hot_enough)
      return true;
   else
      return false;
}
...
/* Find the traces for Software Trace Cache.  Chain each trace through
   ...
   traces to TRACES.  */
static void find_traces (MtcsBBReorder *self,int *n_traces, struct trace *traces)
{
   int i;
   ...
   self->max_entry_count = profile_count::zero ();
   FOR_EACH_EDGE (e, ei, ENTRY_BLOCK_PTR_FOR_FN (cfun)->succs){
      self->bbd[e->dest->index].heap = heap;
      self->bbd[e->dest->index].node = heap->insert (bb_to_key(self,e->dest), e->dest);
      if (e->dest->count > self->max_entry_count)
         self->max_entry_count = e->dest->count;
   }
   /* Find the traces.  */
   for (i = 0; i < number_of_rounds; i++){
      ...
      count_threshold = self->max_entry_count.apply_scale (exec_threshold[i], 1000);

      find_traces_1_round(self,REG_BR_PROB_BASE * branch_threshold[i] / 1000,
   ...
   if (dump_file){
      for (i = 0; i < *n_traces; i++){
         basic_block bb;
         fprintf (dump_file, "Trace %d (round %d):  ", i + 1, traces[i].round + 1);
...
static basic_block rotate_loop (MtcsBBReorder *self,edge back_edge, struct trace *trace, int trace_n)
{
   basic_block bb;
   ...
   do{
      ...
      FOR_EACH_EDGE (e, ei, bb->succs)
         ...
         && bb_visited_trace(self,e->dest) != trace_n
         && (e->flags & EDGE_CAN_FALLTHRU)
         && !(e->flags & EDGE_COMPLEX)){
            if (is_preferred){
               /* The best edge is preferred.  */
               if (!bb_visited_trace(self,e->dest) || self->bbd[e->dest->index].start_of_trace >= 0){
   ...
   if (best_bb){
      ...
      if (back_edge->dest == trace->first){
         ...
         for (prev_bb = trace->first;  prev_bb->aux != back_edge->dest; prev_bb = (basic_block) prev_bb->aux)
            ;
         ...
         if (single_succ_p (prev_bb)){
            ...
            in the end.  */
            if (any_condjump_p (BB_END (header)) && copy_bb_p(self,header, 0)  && !CROSSING_JUMP_P (BB_END (header)))
               copy_bb(self,header, single_succ_edge (prev_bb), prev_bb, trace_n);
         }
      }
   }else{
...
static void find_traces_1_round (MtcsBBReorder *self,int branch_th, profile_count count_th,
           ...
           bb_heap_t **heap, int number_of_rounds)
{
   ...
   while (!(*heap)->empty ()){
      ...
      if (!for_size  && push_to_next_round_p (bb, round, number_of_rounds,count_th)){
         int key = bb_to_key(self,bb);
         self->bbd[bb->index].heap = new_heap;
         self->bbd[bb->index].node = new_heap->insert (key, bb);

         if (dump_file)
            fprintf (dump_file, "  Possible start point of next round: %d (key: %d)\n", bb->index, key);
         continue;
      }
      ...
      trace->round = round;
      trace->length = 0;
      self->bbd[bb->index].in_trace = *n_traces;
      (*n_traces)++;
      ...
      do{
         ...
         best_edge = NULL;
         mark_bb_visited(self,bb, *n_traces);
         trace->length++;
         ...
         FOR_EACH_EDGE (e, ei, bb->succs){
            ...
            if (bb_visited_trace(self,e->dest) && bb_visited_trace(self,e->dest) != *n_traces)
               continue;
            ...
            if (better_edge_p(self,bb, e, prob, count, best_prob, best_count,best_edge)){
               best_edge = e;
               best_prob = prob;
               best_count = count;
            }
         ...
         if (best_edge  && EDGE_COUNT (best_edge->dest->preds) >= 2  && copy_bb_p(self,best_edge->dest, 0)){
         ...
         if (best_edge && for_size  && (EDGE_COUNT (best_edge->dest->succs) > 1  || EDGE_COUNT (best_edge->dest->preds) > 1))
            best_edge = NULL;

         /* Add all non-selected successors to the heaps.  */
         FOR_EACH_EDGE (e, ei, bb->succs){
            if (e == best_edge  || e->dest == EXIT_BLOCK_PTR_FOR_FN (cfun)  || bb_visited_trace(self,e->dest))
               continue;

            key = bb_to_key(self,e->dest);

            if (self->bbd[e->dest->index].heap){
               /* E->DEST is already in some heap.  */
               if (key != self->bbd[e->dest->index].node->get_key ()){
                  if (dump_file){
                     fprintf (dump_file,"Changing key for bb %d from %ld to %ld.\n",
                     e->dest->index,(long) self->bbd[e->dest->index].node->get_key (),key);
                  }
                  self->bbd[e->dest->index].heap->replace_key (self->bbd[e->dest->index].node, key);
               }
            }else{
               ...
               || e->count () < count_th){
                  ...
                  if (!for_size && push_to_next_round_p (e->dest, round, number_of_rounds, count_th))
                     which_heap = new_heap;
               }

               self->bbd[e->dest->index].heap = which_heap;
               self->bbd[e->dest->index].node = which_heap->insert (key, e->dest);

               if (dump_file){
                  fprintf (dump_file,"  Possible start of %s round: %d (key: %ld)\n",
                        (which_heap == new_heap) ? "next" : "this", e->dest->index, (long) key);
               }
            }
         }

         if (best_edge) /* Suitable successor was found.  */{
            if (bb_visited_trace(self,best_edge->dest) == *n_traces){
               /* We do nothing with one basic block loops.  */
               if (best_edge->dest != bb){
                  if (best_edge->count ()  > best_edge->dest->count.apply_scale (4, 5)){
                     /* The loop has at least 4 iterations.  If the loop
                     header is not the first block of the function
                     we can rotate the loop.  */

                     if (best_edge->dest != ENTRY_BLOCK_PTR_FOR_FN (cfun)->next_bb){
                        if (dump_file){
                           fprintf (dump_file,"Rotating loop %d - %d\n", best_edge->dest->index, bb->index);
                        }
                        bb->aux = best_edge->dest;
                        self->bbd[best_edge->dest->index].in_trace =  (*n_traces) - 1;
                        bb = rotate_loop(self,best_edge, trace, *n_traces);
                     }
                  }else{
                     /* The loop has less than 4 iterations.  */

                     if (single_succ_p (bb) && copy_bb_p(self,best_edge->dest, optimize_edge_for_speed_p (best_edge))){
                        bb = copy_bb(self,best_edge->dest, best_edge, bb,
                        *n_traces);
                        trace->length++;
                     }
                  }
               ...
               */
               ...
               FOR_EACH_EDGE (e, ei, bb->succs)
                  ...
                  && (e->flags & EDGE_CAN_FALLTHRU)
                  && !(e->flags & EDGE_COMPLEX)
                  && !bb_visited_trace(self,e->dest)
                  && single_pred_p (e->dest)
                  && !(e->flags & EDGE_CROSSING)
                  && single_succ_p (e->dest)
                  && (single_succ_edge (e->dest)->flags  & EDGE_CAN_FALLTHRU)
                  && !(single_succ_edge (e->dest)->flags & EDGE_COMPLEX)
                  && single_succ (e->dest) == best_edge->dest
                  && (e->dest->count * 2  >= best_edge->count () || for_size)){
                     best_edge = e;
                     if (dump_file)
      ...
      trace->last = bb;
      self->bbd[trace->first->index].start_of_trace = *n_traces - 1;
      if (self->bbd[trace->last->index].end_of_trace != *n_traces - 1){
         self->bbd[trace->last->index].end_of_trace = *n_traces - 1;
         ...
         FOR_EACH_EDGE (e, ei, trace->last->succs)
            if (EDGE_FREQUENCY (e) > self->bbd[e->dest->index].priority)
               self->bbd[e->dest->index].priority = EDGE_FREQUENCY (e);
      }
      ...
      FOR_EACH_EDGE (e, ei, bb->succs){
         if (e->dest == EXIT_BLOCK_PTR_FOR_FN (cfun) || bb_visited_trace(self,e->dest))
            continue;
...
static basic_block copy_bb (MtcsBBReorder *self,basic_block old_bb, edge e, basic_block bb, int trace)
{
   basic_block new_bb;
   ...
   if (dump_file)
      fprintf (dump_file, "Duplicated bb %d (created bb %d)\n", old_bb->index, new_bb->index);

   if (new_bb->index >= self->array_size || last_basic_block_for_fn (cfun) > self->array_size){
      int i;
      int new_size;

      new_size = MAX (last_basic_block_for_fn (cfun), new_bb->index + 1);
      new_size = GET_ARRAY_SIZE (new_size);
      self->bbd = XRESIZEVEC (bbro_basic_block_data, self->bbd, new_size);
      for (i = self->array_size; i < new_size; i++){
         self->bbd[i].start_of_trace = -1;
         self->bbd[i].end_of_trace = -1;
         self->bbd[i].in_trace = -1;
         self->bbd[i].visited = 0;
         self->bbd[i].priority = -1;
         self->bbd[i].heap = NULL;
         self->bbd[i].node = NULL;
      }
      self->array_size = new_size;

      if (dump_file){
         fprintf (dump_file, "Growing the dynamic array to %d elements.\n",self->array_size);
      }
   }

   gcc_assert (!bb_visited_trace(self,e->dest));
   mark_bb_visited(self,new_bb, trace);
   new_bb->aux = bb->aux;
   bb->aux = new_bb;

   self->bbd[new_bb->index].in_trace = trace;

   return new_bb;
}

/* Compute and return the key (for the heap) of the basic block BB.  */
static long bb_to_key (MtcsBBReorder *self,basic_block bb)
{
   edge e;
   edge_iterator ei;
   ...
   if (BB_PARTITION (bb) == BB_COLD_PARTITION  || probably_never_executed_bb_p (cfun, bb))
      return BB_FREQ_MAX;

   /* Prefer blocks whose predecessor is an end of some trace
   or whose predecessor edge is EDGE_DFS_BACK.  */
   int priority = self->bbd[bb->index].priority;
   if (priority == -1){
      priority = 0;
      FOR_EACH_EDGE (e, ei, bb->preds){
         if ((e->src != ENTRY_BLOCK_PTR_FOR_FN (cfun)
         && self->bbd[e->src->index].end_of_trace >= 0)
      ...
      self->bbd[bb->index].priority = priority;
   }
...
static bool better_edge_p (MtcsBBReorder *self,const_basic_block bb, const_edge e, profile_probability prob,
          ...
          profile_count best_count, const_edge cur_best_edge)
{
   ...
   if (optimize_function_for_size_p (cfun))
      return !cur_best_edge || cur_best_edge->dest->index > e->dest->index;
   ...
   && prob > profile_probability::guessed_never ()))
      ...
      if (count < best_count - diff_count || (!best_count.initialized_p () && count.nonzero_p ()))
...
/* Return true when the edge E is better than the temporary best edge
   ...
   is better.  */
static bool connect_better_edge_p (MtcsBBReorder *self,const_edge e, bool src_index_p, int best_len,
             const_edge cur_best_edge, struct trace *traces)
{
   ...
   if (optimize_function_for_size_p (cfun)){
      ...
      b_index = src_index_p ? cur_best_edge->src->index : cur_best_edge->dest->index;
      /* The smaller one is better to keep the original order.  */
   ...
   if (src_index_p){
      ...
      if (e->count () > cur_best_edge->count ())
         ...
         /* The edge has lower probability than the temporary best edge.  */
         is_better_edge = false;
      else if (traces[self->bbd[e_index].end_of_trace].length > best_len)
      ...
      if (e->probability > cur_best_edge->probability)
         ...
         /* The edge has lower probability than the temporary best edge.  */
         is_better_edge = false;
      else if (traces[self->bbd[e_index].start_of_trace].length > best_len)
         /* The edge and the temporary best edge have equivalent probabilities.
         The edge with longer trace is better.  */
         is_better_edge = true;
      else
...
static void connect_traces (MtcsBBReorder *self,int n_traces, struct trace *traces)
{
   ...
   count_threshold = self->max_entry_count.apply_scale (DUPLICATION_THRESHOLD, 1000);
   ...
   if (mtcsRtlData/*!crtl*/->has_bb_partition)
      for (i = 0; i < n_traces && !two_passes; i++)
         if (BB_PARTITION (traces[0].first) != BB_PARTITION (traces[i].first))
            two_passes = true;
   ...
   for (i = 0; i < n_traces || (two_passes && current_pass == 1) ; i++){
      ...
      if (two_passes && BB_PARTITION (traces[t].first) != current_partition)
         continue;
      ...
      for (t2 = t; t2 > 0;){
         ...
         FOR_EACH_EDGE (e, ei, traces[t2].first->preds){
            ...
            && (e->flags & EDGE_CAN_FALLTHRU)
            && !(e->flags & EDGE_COMPLEX)
            && self->bbd[si].end_of_trace >= 0
            && !connected[self->bbd[si].end_of_trace]
            && (BB_PARTITION (e->src) == current_partition)
            && connect_better_edge_p(self,e, true, best_len, best, traces)){
               best = e;
               best_len = traces[self->bbd[si].end_of_trace].length;
            }
         }
         if (best){
            best->src->aux = best->dest;
            t2 = self->bbd[best->src->index].end_of_trace;
            connected[t2] = true;
      ...
      while (1){
         ...
         FOR_EACH_EDGE (e, ei, traces[t].last->succs){
            ...
            && (e->flags & EDGE_CAN_FALLTHRU)
            && !(e->flags & EDGE_COMPLEX)
            && self->bbd[di].start_of_trace >= 0
            && !connected[self->bbd[di].start_of_trace]
            && (BB_PARTITION (e->dest) == current_partition)
            && connect_better_edge_p(self,e, false, best_len, best, traces)){
               best = e;
               best_len = traces[self->bbd[di].start_of_trace].length;
            }
         }
         ...
         if (for_size){
            ...
            if (last_trace != self->bbd[best->dest->index].start_of_trace - 1)
               break;

            if (dump_file)
               fprintf (dump_file, "Connection: %d %d\n",best->src->index, best->dest->index);

            t = self->bbd[best->dest->index].start_of_trace;
            ...
            FOR_EACH_EDGE (e, ei, traces[t].last->succs)
               ...
               && (!best || e->probability > best->probability)){
                  ...
                  if (self->bbd[e->dest->index].start_of_trace >= 0
                  && traces[self->bbd[e->dest->index].start_of_trace].length == 1){
                     best = e;
                     try_copy = true;
                     continue;
                  }
                  ...
                  FOR_EACH_EDGE (e2, ei, e->dest->succs){
                     ...
                     || (e2->probability == best2->probability
                     && traces[self->bbd[di].start_of_trace].length
                     > best2_len)))){
                        best = e;
                        best2 = e2;
                        if (e2->dest != EXIT_BLOCK_PTR_FOR_FN (cfun))
                           best2_len = traces[self->bbd[di].start_of_trace].length;
                        else
                           best2_len = INT_MAX;
                        next_bb = e2->dest;
                        try_copy = true;
            ...
            || best->count () >= count_threshold))){
               ...
               if (dump_file){
                  fprintf (dump_file, "Connection: %d %d ",traces[t].last->index, best->dest->index);
                  if (!next_bb)
                     fputc ('\n', dump_file);
               ...
               new_bb = copy_bb(self,best->dest, best, traces[t].last, t);
               traces[t].last = new_bb;
               if (next_bb && next_bb != EXIT_BLOCK_PTR_FOR_FN (cfun)){
                  t = self->bbd[next_bb->index].start_of_trace;
                  traces[last_trace].last->aux = traces[t].first;
                  connected[t] = true;
                  last_trace = t;
               }else
...
static bool copy_bb_p (MtcsBBReorder *self,const_basic_block bb, int code_may_grow)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   return false;
   if (!mtcs_cfg_context_can_duplicate_block_p/*!can_duplicate_block_p*/(mtcsCfgContext,bb))
      return false;
   ...
   if (code_may_grow && optimize_bb_for_speed_p (bb))
      max_size *= mtcsOptionsItem->x_param_max_grow_copy_bb_insns;

   FOR_BB_INSNS (bb, insn){
      if (INSN_P (insn)){
         size += mtcs_final_get_attr_min_length/*!get_attr_min_length*/(mtcsFinal,insn);
         if (size > max_size)
            break;
      }
   }
   ...
   if (dump_file){
      fprintf (dump_file, "Block %d can't be copied because its size = %u.\n", bb->index, size);
   }
...
int mtcs_bb_reorder_get_uncond_jump_length (MtcsBBReorder *self)
{
   ...
   rtx_code_label *label = mtcs_emit_emit_label/*!emit_label*/(mtcsEmit,mtcs_rtl_gen_label_rtx/*!gen_label_rtx*/(mtcsRTL));
   rtx_insn *jump =mtcs_emit_emit_jump_insn/*!emit_jump_insn*/(mtcsEmit,
         target_rtx_gen_jump/*!targetm.gen_jump*/(mtcsMachine->tmrtx,label));
   length = mtcs_final_get_attr_min_length/*!get_attr_min_length*/(mtcsFinal,jump);
...
static basic_block create_eh_forwarder_block (MtcsBBReorder *self,rtx_code_label *new_label, basic_block old_bb)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   old_bb = mtcs_cfg_context_split_block_after_labels/*!split_block_after_labels*/(mtcsCfgContext,old_bb)->dest;

   /* Put the new label and a jump in the new basic block.  */
   rtx_insn *label = mtcs_emit_emit_label/*!emit_label*/(mtcsEmit,new_label);
   rtx_code_label *old_label = mtcs_cfg_rtl_block_label/*!block_label*/(mtcsCfgRtl,old_bb);
   rtx_insn *jump = mtcs_emit_emit_jump_insn/*!emit_jump_insn*/(mtcsEmit,
         target_rtx_gen_jump/*!targetm.gen_jump*/(mtcsMachine->tmrtx,old_label));
   ...
   basic_block new_bb = mtcs_cfg_context_create_basic_block/*!create_basic_block*/(mtcsCfgContext,label, jump, last_bb);
   new_bb->aux = last_bb->aux;
   new_bb->count = old_bb->count;
   last_bb->aux = new_bb;

   mtcs_cfg_rtl_emit_barrier_after_bb/*!emit_barrier_after_bb*/(mtcsCfgRtl,new_bb);

   mtcs_cfg_make_single_succ_edge/*!make_single_succ_edge*/(mtcsCfg,new_bb, old_bb, 0);
...
static void sjlj_fix_up_crossing_landing_pad (MtcsBBReorder *self,basic_block old_bb)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL  *mtcsRTL = mtcs_target_get_rtl(mtcsTarget);
   ...
   rtx_code_label *new_label =mtcs_rtl_gen_label_rtx/*!gen_label_rtx*/(mtcsRTL);
   LABEL_PRESERVE_P (new_label) = 1;

   /* Create the forwarder block.  */
   basic_block new_bb = create_eh_forwarder_block(self,new_label, old_bb);
   ...
   for (ei = ei_start (old_bb->preds); (e = ei_safe_edge (ei)) != NULL; )
      if (e->src != new_bb && BB_PARTITION (e->src) == BB_PARTITION (new_bb)){
         ...
         mtcs_cfg_redirect_edge_succ/*!redirect_edge_succ*/(mtcsCfg,e, new_bb);
      }else
         ei_next (&ei);
}
...
/* The landing pad OLD_LP, in block OLD_BB, has edges from both partitions.
   Add a new landing pad that will just jump to the old one and split the
   edges so that no EH edge crosses partitions.  */
static void dw2_fix_up_crossing_landing_pad (MtcsBBReorder *self,eh_landing_pad old_lp, basic_block old_bb)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL  *mtcsRTL = mtcs_target_get_rtl(mtcsTarget);
   ...
   new_lp->post_landing_pad = old_lp->post_landing_pad;
   new_lp->landing_pad = mtcs_rtl_gen_label_rtx/*!gen_label_rtx*/(mtcsRTL);
   LABEL_PRESERVE_P (new_lp->landing_pad) = 1;

   /* Create the forwarder block.  */
   basic_block new_bb = create_eh_forwarder_block(self,new_lp->landing_pad, old_bb);
   ...
   for (ei = ei_start (old_bb->preds); (e = ei_safe_edge (ei)) != NULL; )
      if (e->src != new_bb && BB_PARTITION (e->src) == BB_PARTITION (new_bb)){
         ...
         XEXP (note, 0) = mtcs_rtl_GEN_INT/*!GEN_INT*/(mtcsRTL,new_lp->index);
...
static unsigned int sanitize_hot_paths (MtcsBBReorder *self, bool walk_up, unsigned int cold_bb_count,
                    vec<basic_block> *bbs_in_hot_partition)
{
   ...
   while (! hot_bbs_to_check.is_empty ()   && cold_bb_count){
      basic_block bb = hot_bbs_to_check.pop ();
      vec<edge, va_gc> *edges = walk_up ? bb->preds : bb->succs;
      edge e;
      edge_iterator ei;
      profile_probability highest_probability = profile_probability::uninitialized ();
      ...
      FOR_EACH_EDGE (e, ei, edges){
         ...
         if (e->probability == profile_probability::never () || e->count () == profile_count::zero ())
            continue;
         ...
         if (!highest_probability.initialized_p () || e->probability > highest_probability)
      ...
      FOR_EACH_EDGE (e, ei, edges){
         ...
         if (dump_file)
            fprintf (dump_file, "Promoting bb %i to hot partition to sanitize profile of bb %i in %s walk\n",
                        reach_bb->index,  bb->index, walk_up ? "backward" : "forward");
         cold_bb_count--;
...
static vec<edge> find_rarely_executed_basic_blocks_and_crossing_edges (MtcsBBReorder *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsMachine *mtcsMachine=mtcs_target_get_machine(mtcsTarget);
   ...
   if (cold_bb_count){
      mark_dfs_back_edges ();
      cold_bb_count = sanitize_hot_paths(self,true, cold_bb_count,&bbs_in_hot_partition);
      if (cold_bb_count)
         sanitize_hot_paths(self,false, cold_bb_count, &bbs_in_hot_partition);
   ...
   if (cfun->eh->lp_array){
      const bool sjlj  = (target_common_except_unwind_info/*!targetm_common.except_unwind_info*/(mtcsMachine->common,
      ...
      FOR_EACH_VEC_ELT (*cfun->eh->lp_array, i, lp){
         ...
         if (lp == NULL || lp->landing_pad == NULL_RTX  || !LABEL_P (lp->landing_pad))
            continue;
         ...
         if (all_same)
            ...
            sjlj_fix_up_crossing_landing_pad(self,bb);
         else
            dw2_fix_up_crossing_landing_pad(self,lp, bb);
...
static void set_edge_can_fallthru_flag (MtcsBBReorder *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   FOR_EACH_BB_FN (bb, cfun){
      ...
      if (!mtcs_dojump_invert_jump/*!invert_jump*/(mtcsDojump,bb_end_jump, JUMP_LABEL (bb_end_jump), 0))
         continue;
      mtcs_dojump_invert_jump/*!invert_jump*/(mtcsDojump,bb_end_jump, JUMP_LABEL (bb_end_jump), 0);
...
static void add_labels_and_missing_jumps (MtcsBBReorder *self,vec<edge> crossing_edges)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   FOR_EACH_VEC_ELT (crossing_edges, i, e){
      ...
      rtx_code_label *label = mtcs_cfg_rtl_block_label/*!block_label*/(mtcsCfgRtl,dest);
      ...
      if (mtcs_cfg_build_control_flow_insn_p/*!control_flow_insn_p*/(mtcsCfgBuild,BB_END (src)))
         continue;

      /* Make sure there's only one successor.  */
      gcc_assert (single_succ_p (src));

      new_jump = emit_jump_insn_after (target_rtx_gen_jump/*!targetm.gen_jump*/(mtcsMachine->tmrtx,label), BB_END (src));
      BB_END (src) = new_jump;
      JUMP_LABEL (new_jump) = label;
      LABEL_NUSES (label) += 1;

      mtcs_cfg_rtl_emit_barrier_after_bb/*!emit_barrier_after_bb*/(mtcsCfgRtl,src);
...
static void fix_up_fall_thru_edges (MtcsBBReorder *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   FOR_EACH_BB_FN (cur_bb, cfun){
      ...
      if (succ1  && (succ1->flags & EDGE_FALLTHRU)) {
         fall_thru = succ1;
         cond_jump = succ2;
      }else if (succ2  && (succ2->flags & EDGE_FALLTHRU)){
         fall_thru = succ2;
         cond_jump = succ1;
         ...
         if (fall_thru->flags & EDGE_CROSSING){
            ...
            if (cond_jump){
               ...
               if (!cond_jump_crosses){
                  ...
                  rtx_code_label *fall_thru_label = mtcs_cfg_rtl_block_label/*!block_label*/(mtcsCfgRtl,fall_thru->dest);

                  if (old_jump && fall_thru_label){
                     rtx_jump_insn *old_jump_insn = dyn_cast <rtx_jump_insn *> (old_jump);
                     if (old_jump_insn)
                        invert_worked = mtcs_dojump_invert_jump/*!invert_jump*/(mtcsDojump,old_jump_insn,fall_thru_label, 0);
                  }

                  if (invert_worked){
                     fall_thru->flags &= ~EDGE_FALLTHRU;
                     cond_jump->flags |= EDGE_FALLTHRU;
                     mtcs_cfg_rtl_update_br_prob_note/*!update_br_prob_note*/(mtcsCfgRtl,cur_bb);
            ...
            if (cond_jump_crosses || !invert_worked){
               ...
               basic_block new_bb = mtcs_cfg_context_force_nonfallthru/*!force_nonfallthru*/(mtcsCfgContext,fall_thru);
               ...
               if (new_bb){
                  ...
                  gcc_assert (BB_PARTITION (new_bb) == BB_PARTITION (cur_bb));
                  ...
                  if (EDGE_COUNT (cur_bb->succs) > old_count){
                     ...
                     gcc_checking_assert (JUMP_P (j)  && (asm_noperands (PATTERN (j))  > 0));
               ...
               mtcs_cfg_rtl_emit_barrier_after_bb/*!emit_barrier_after_bb*/(mtcsCfgRtl,new_bb ? new_bb : cur_bb);
            }
         }
      }
   }
...
static basic_block find_jump_block (MtcsBBReorder *self,basic_block jump_dest)
{
   ...
   FOR_EACH_EDGE (e, ei, jump_dest->preds)
      if (e->flags & EDGE_CROSSING){
         ...
         if (LABEL_P (BB_HEAD (src)))
            for (insn = BB_HEAD (src); !INSN_P (insn) && insn != NEXT_INSN (BB_END (src)); insn = NEXT_INSN (insn)){
               if (INSN_P (insn)  && insn == BB_END (src) && JUMP_P (insn)   && !any_condjump_p (insn)){
                  source_bb = src;
                  break;
               }
            }
...
static void fix_crossing_conditional_branches (MtcsBBReorder *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   FOR_EACH_BB_FN (cur_bb, cfun){
      ...
      if (crossing_edge){
         ...
         if (set_src && (GET_CODE (set_src) == IF_THEN_ELSE)){
            rtx_jump_insn *old_jump_insn = as_a <rtx_jump_insn *> (old_jump);
            ...
            new_bb = find_jump_block(self,crossing_edge->dest);

            if (new_bb)
               new_label = mtcs_cfg_rtl_block_label/*!block_label*/(mtcsCfgRtl,new_bb);
            else{
               basic_block last_bb;
               ...
               new_label = mtcs_rtl_gen_label_rtx/*!gen_label_rtx*/(mtcsRTL);
               mtcs_emit_emit_label/*!emit_label*/(mtcsEmit,new_label);

               gcc_assert (GET_CODE (old_label) == LABEL_REF);
               old_jump_target = old_jump_insn->jump_target ();
               new_jump = as_a <rtx_jump_insn *> (mtcs_emit_emit_jump_insn/*!emit_jump_insn*/(mtcsEmit,
                     target_rtx_gen_jump/*!targetm.gen_jump*/(mtcsMachine->tmrtx,old_jump_target)));
               new_jump->set_jump_target (old_jump_target);

               last_bb = EXIT_BLOCK_PTR_FOR_FN (cfun)->prev_bb;
               new_bb = mtcs_cfg_context_create_basic_block/*!create_basic_block*/(mtcsCfgContext,new_label, new_jump, last_bb);
               new_bb->aux = last_bb->aux;
               last_bb->aux = new_bb;
            ...
            mtcs_dojump_redirect_jump/*!redirect_jump*/(mtcsDojump,old_jump_insn, new_label, 0);
            /* Remove crossing_edge as predecessor of 'dest'.  */
            dest = crossing_edge->dest;
            mtcs_cfg_redirect_edge_succ/*!redirect_edge_succ*/(mtcsCfg,crossing_edge, new_bb);
            ...
            if (EDGE_COUNT (new_bb->succs) == 0)
               new_edge =  mtcs_cfg_make_single_succ_edge/*!make_single_succ_edge*/(mtcsCfg,new_bb, dest, 0);
            else
...
static void fix_crossing_unconditional_branches (MtcsBBReorder *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgRtl *mtcsCfgRtl=mtcs_target_get_cfg_rtl(mtcsTarget);
   ...
   FOR_EACH_BB_FN (cur_bb, cfun){
      ...
      if (JUMP_P (last_insn)  && (succ->flags & EDGE_CROSSING)){
         gcc_assert (!any_condjump_p (last_insn));

         /* Make sure the jump is not already an indirect or table jump.  */
         if (!computed_jump_p (last_insn) && !tablejump_p (last_insn, NULL, NULL) && asm_noperands (PATTERN (last_insn)) < 0){
            /* We have found a "crossing" unconditional branch.  Now
            we must convert it to an indirect jump.  First create
            reference of label, as target for jump.  */

            label = JUMP_LABEL (last_insn);
            label_addr = gen_rtx_LABEL_REF (mtcs_mode_get_Pmode(mtcsMode), label);
            LABEL_NUSES (label) += 1;

            /* Get a register to use for the indirect jump.  */

            new_reg = mtcs_emit_gen_reg_rtx/*!gen_reg_rtx*/(mtcsEmit,mtcs_mode_get_Pmode(mtcsMode));

            /* Generate indirect the jump sequence.  */

            mtcs_emit_start_sequence/*!start_sequence*/(mtcsEmit);
            mtcs_expr_emit_move_insn/*!emit_move_insn*/(mtcsExpr,new_reg, label_addr);
            mtcs_optabs_emit_indirect_jump/*!emit_indirect_jump*/(mtcsOptabs,new_reg);
            indirect_jump_sequence = mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData);
            mtcs_emit_end_sequence/*!end_sequence*/(mtcsEmit);

            /* Make sure every instruction in the new jump sequence has
            its basic block set to be cur_bb.  */

            for (cur_insn = indirect_jump_sequence; cur_insn; cur_insn = NEXT_INSN (cur_insn)){
            ...
            mtcs_emit_emit_insn_before/*!emit_insn_before*/(mtcsEmit,indirect_jump_sequence, last_insn);
            mtcs_cfg_rtl_delete_insn/*!delete_insn*/(mtcsCfgRtl,last_insn);
...
static void reorder_basic_blocks_software_trace_cache (MtcsBBReorder *self)
{
   if (dump_file)
   ...
   if (uncond_jump_length == 0)
      uncond_jump_length = mtcs_bb_reorder_get_uncond_jump_length/*!get_uncond_jump_length*/(self);

   /* We need to know some information for each basic block.  */
   self->array_size = GET_ARRAY_SIZE (last_basic_block_for_fn (cfun));
   self->bbd = XNEWVEC (bbro_basic_block_data, self->array_size);
   ...
   n_traces = 0;
   find_traces(self,&n_traces, traces);
   connect_traces(self,n_traces, traces);
   FREE (traces);
   FREE (self->bbd);
}
...
static void reorder_basic_blocks_simple (MtcsBBReorder *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   int current_partition = BB_PARTITION (last_tail == ENTRY_BLOCK_PTR_FOR_FN (cfun)
         ? EDGE_SUCC (ENTRY_BLOCK_PTR_FOR_FN (cfun), 0)->dest : last_tail);
...
static void reorder_basic_blocks (MtcsBBReorder *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   set_edge_can_fallthru_flag(self);
   mark_dfs_back_edges ();

   switch (mtcsOptionsItem->x_flag_reorder_blocks_algorithm){
      case REORDER_BLOCKS_ALGORITHM_SIMPLE:
         reorder_basic_blocks_simple(self);
         break;

      case REORDER_BLOCKS_ALGORITHM_STC:
         reorder_basic_blocks_software_trace_cache(self);
         break;
   ...
   mtcs_cfg_rtl_relink_block_chain/*!relink_block_chain*/(mtcsCfgRtl,/*stay_in_cfglayout_mode=*/true);
   ...
   mtcsRtlData/*!crtl*/->bb_reorder_complete = true;
}
...
static bool maybe_duplicate_computed_goto (MtcsBBReorder *self,basic_block bb, int max_size)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   FOR_BB_INSNS (bb, insn)
      if (INSN_P (insn)){
         max_size -= mtcs_final_get_attr_min_length/*!get_attr_min_length*/(mtcsFinal,insn);
         if (max_size < 0)
            return false;
      }
   ...
   for (ei = ei_start (bb->preds); (e = ei_safe_edge (ei)); ){
      ...
      bool can_dup_more = mtcs_cfg_context_can_duplicate_block_p/*!can_duplicate_block_p*/(mtcsCfgContext,pred);

      /* Make a copy of BB, merge it into PRED.  */
      basic_block copy = mtcs_cfg_context_duplicate_block/*!duplicate_block*/(mtcsCfgContext,bb, e, NULL);
      mtcs_cfg_rtl_emit_barrier_after_bb/*!emit_barrier_after_bb*/(mtcsCfgRtl,copy);
      mtcs_rtl_reorder_insns_nobb/*!reorder_insns_nobb*/(mtcsRTL,BB_HEAD (copy), BB_END (copy), BB_END (pred));
      mtcs_cfg_context_merge_blocks/*!merge_blocks*/(mtcsCfgContext,pred, copy);

      changed = true;

      /* Try to merge the resulting merged PRED into further predecessors.  */
      if (can_dup_more)
         maybe_duplicate_computed_goto(self,pred, max_size);
   }
...
static void duplicate_computed_gotos (MtcsBBReorder *self,function *fun)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   int max_size = uncond_jump_length * mtcsOptionsItem->x_param_max_goto_duplication_insns;
   ...
   FOR_EACH_BB_FN (bb, fun)
      if (computed_jump_p (BB_END (bb)) && mtcs_cfg_context_can_duplicate_block_p/*!can_duplicate_block_p*/(mtcsCfgContext,bb))
         changed |= maybe_duplicate_computed_goto(self,bb, max_size);
...
static void mtcsBBReorderInit(MtcsBBReorder *self)
{
...
{
   MtcsBBReorder *self = n_slice_alloc0 (sizeof(MtcsBBReorder));
   mtcs_component_set_mode((MtcsComponent*)self,mtcsMode);
   mtcsBBReorderInit(self);
   return self;
}
...
static nboolean bb_reorder_gate_cb(MtcsPass *mtcsPass,function *fun)
{
   ...
   return (mtcsOptionsItem->x_optimize > 0
         && (mtcsOptionsItem->x_flag_reorder_blocks || mtcsOptionsItem->x_flag_reorder_blocks_and_partition));
}

static nuint bb_reorder_execute_cb(MtcsPass *mtcsPass,function *func)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(mtcsPass);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgCleanup *mtcsCfgCleanup=mtcs_target_get_cfg_cleanup(mtcsTarget);
   ...
   MtcsBBReorder *mtcsBBReorder =mtcs_rtl_pass_mgr_get_bb_reorder(mtcsRtlPassMgr);

   basic_block bb;

   /* Last attempt to optimize CFG, as scheduling, peepholing and insn
   splitting possibly introduced more crossjumping opportunities.  */
   mtcs_cfg_rtl_cfg_layout_initialize/*!cfg_layout_initialize*/(mtcsCfgRtl,CLEANUP_EXPENSIVE);

   reorder_basic_blocks(mtcsBBReorder);
   mtcs_cfg_cleanup_cleanup_cfg/*!cleanup_cfg*/(mtcsCfgCleanup,CLEANUP_EXPENSIVE | CLEANUP_NO_PARTITIONING);
...
static void mtcsPassBBReorderInit(MtcsPassBBReorder *self)
{
   MtcsPass *mtcsPass= (MtcsPass *)self;
   mtcsPass->execute =bb_reorder_execute_cb;
   mtcsPass->gate =bb_reorder_gate_cb;
...
{
   MtcsPassBBReorder *self = n_slice_alloc0 (sizeof(MtcsPassBBReorder));
   mtcs_component_set_mode((MtcsComponent*)self,mtcsMode);
   mtcs_pass_init((MtcsPass *)self,RTL_PASS,"bbro");
   mtcsPassBBReorderInit(self);
   return self;
}
...
static nboolean comp_gotos_gate_cb(MtcsPass *mtcsPass,function *fun)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(mtcsPass);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsOptions *mtcsOptions=mtcs_target_get_options(mtcsTarget);
   ...
   if (mtcsTarget/*!targetm.cannot_modify_jumps_p*/->cannot_modify_jumps_p (mtcsTarget))
      return false;
   return (mtcsOptionsItem->x_optimize > 0
       && mtcsOptionsItem->x_flag_expensive_optimizations
       && ! optimize_function_for_size_p (fun));
}

static nuint comp_gotos_execute_cb(MtcsPass *mtcsPass,function *func)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(mtcsPass);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRtlPassMgr *mtcsRtlPassMgr=mtcs_target_get_rtl_pass_mgr(mtcsTarget);
   ...
   duplicate_computed_gotos (mtcsBBReorder,func);
   return 0;

}

static void mtcsPassCompGotosInit(MtcsPassCompGotos *self)
{
   MtcsPass *mtcsPass= (MtcsPass *)self;
   mtcsPass->execute =comp_gotos_execute_cb;
   mtcsPass->gate =comp_gotos_gate_cb;
   mtcs_pass_set_properties(mtcsPass,
      0,/* properties_required */
      0, /* properties_provided */
      0 /* properties_destroyed */);
...
{
   MtcsPassCompGotos *self = n_slice_alloc0 (sizeof(MtcsPassCompGotos));
   mtcs_component_set_mode((MtcsComponent*)self,mtcsMode);
   mtcs_pass_init((MtcsPass *)self,RTL_PASS,"compgotos");
   mtcsPassCompGotosInit(self);
   return self;
}

//原型 NEXT_PASS (pass_partition_blocks, 1); RTL_PASS bb-reorder.cc bbpart y 有条件执行 flag_reorder_blocks_and_partition
static nboolean bb_part_gate_cb(MtcsPass *mtcsPass,function *fun)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(mtcsPass);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsOptions *mtcsOptions=mtcs_target_get_options(mtcsTarget);
   ...
   return (mtcsOptionsItem->x_flag_reorder_blocks_and_partition
...
static nuint bb_part_execute_cb(MtcsPass *mtcsPass,function *func)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(mtcsPass);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   if (n_basic_blocks_for_fn (func) <= NUM_FIXED_BLOCKS + 1)
      return 0;

   mtcs_dfcore_df_set_flags/*!df_set_flags*/(mtcsDfcore,DF_DEFER_INSN_RESCAN);

   crossing_edges = find_rarely_executed_basic_blocks_and_crossing_edges(mtcsBBReorder);
   if (!crossing_edges.exists ())
      /* Make sure to process deferred rescans and clear changeable df flags.  */
      return TODO_df_finish;

   mtcsRtlData/*!crtl*/->has_bb_partition = true;

   /* Make sure the source of any crossing edge ends in a jump and the
   destination of any crossing edge has a label.  */
   add_labels_and_missing_jumps(mtcsBBReorder,crossing_edges);

   /* Convert all crossing fall_thru edges to non-crossing fall
   thrus to unconditional jumps (that jump to the original fall
   through dest).  */
   fix_up_fall_thru_edges(mtcsBBReorder);

   /* If the architecture does not have conditional branches that can
   span all of memory, convert crossing conditional branches into
   crossing unconditional branches.  */
   if (!HAS_LONG_COND_BRANCH)
      fix_crossing_conditional_branches(mtcsBBReorder);
   ...
   if (!HAS_LONG_UNCOND_BRANCH)
      fix_crossing_unconditional_branches(mtcsBBReorder);
...
static void mtcsPassBBPartInit(MtcsPassBBPart *self)
{
   MtcsPass *mtcsPass= (MtcsPass *)self;
   mtcsPass->execute =bb_part_execute_cb;
   mtcsPass->gate =bb_part_gate_cb;
   mtcs_pass_set_properties(mtcsPass,
...
{
   MtcsPassBBPart *self = n_slice_alloc0 (sizeof(MtcsPassBBPart));
   mtcs_component_set_mode((MtcsComponent*)self,mtcsMode);
   mtcs_pass_init((MtcsPass *)self,RTL_PASS,"bbpart");
   mtcsPassBBPartInit(self);
   return self;
}
