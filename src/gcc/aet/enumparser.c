...
static int getIndex(EnumData **datas,int len,ClassName *className)
{
    ...
    for(i=0;i<len;i++){
       ...
       if(!strcmp(item->sysName,className->sysName)){
           printf("enumparser.c getIndex %d :%s\n",i,className->sysName);
          return i;
       }
    }
    return -1;
 ...
 * 只有这两种情况允许不用类.(AObject.Enum)方式访问。
...
nboolean  enum_parser_set_enum_type(EnumParser *self,c_token *who)
{
   ...
   if(count==1){
      ...
      if(index>=0){
         ...
         tree newValue=aet_utils_create_ident(item->typedefName);
         who->value=newValue;
         n_debug("有多个枚举类型:。把token的名字改了-xxx---id:%s typedefName:%s file:%s item:%p index:%d\n",
...
static struct c_typespec c_parser_enum_specifier (EnumParser *self,ClassName *className,EnumData **eData)
{
   ...
   attrs = aet_parser_c_parser_gnu_attributes/*!c_c_parser_gnu_attributes*/(self->parser);
   ...
   && c_token_starts_typename (c_parser_peek_2nd_token (parser))){
      pedwarn_c11 (enum_loc, OPT_Wpedantic, "ISO C does not support specifying %<enum%> underlying types before C23");
      if (ident){
         ...
         if (!ENUM_FIXED_UNDERLYING_TYPE_P (ret.spec))
            error_at (enum_loc, "%<enum%> declared both with and without fixed underlying type");
      ...
      aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(self->parser,
            specs, false, true, false, false, false,false, true, cla_prefer_id);
      finish_declspecs (specs);
      if (specs->default_int_p)
         error_at (enum_loc, "no %<enum%> underlying type specified");
      else if (TREE_CODE (specs->type) != INTEGER_TYPE  && TREE_CODE (specs->type) != BOOLEAN_TYPE){
      ...
      if (ident){
         ...
         if (ENUM_FIXED_UNDERLYING_TYPE_P (ret.spec) && ENUM_UNDERLYING_TYPE (ret.spec) == NULL_TREE){
            ...
            layout_type (ret.spec);
               ...
               && !comptypes (fixed_underlying_type,ENUM_UNDERLYING_TYPE (ret.spec))){
            error_at (enum_loc, "%<enum%> underlying type incompatible with previous declaration");
   ...
   *eData=enumData;
   if (c_parser_next_token_is (parser, CPP_OPEN_BRACE)){
      ...
      while (true){
         ...
         /* Set the location in case we create a decl now.  */
         aet_parser_c_parser_set_source_position_from_token/*!c_parser_set_source_position_from_token*/(token);
   ...
   if (fixed_underlying_type == NULL_TREE){
      ...
      if (pedantic && !COMPLETE_TYPE_P (ret.spec)){
         gcc_assert (ident);
         pedwarn (enum_loc, OPT_Wpedantic,"ISO C forbids forward references to %<enum%> types");
      }
   }
   return ret;
}
...
void enum_parser_build_dot (EnumParser *self, location_t loc,struct c_expr *expr)
{
   ...
   n_debug("enum_parser_build_dot 访问枚举:%s %s %s elem->value code:%s\n",enumData->sysName,enumData->origName,enumData->enumName,
         get_tree_code_name(TREE_CODE(TREE_VALUE(elem->value))));
...
void   enum_parser_build_class_dot_enum (EnumParser *self, location_t loc,char *sysName,char *origEnumName,struct c_expr *expr)
{
	...
	//	expr->value =TREE_VALUE(elem->value);
        ...
        error_at (input_location, "枚举%qs没有%qs元素。",enumData->origName,IDENTIFIER_POINTER(token->value));
        expr->set_error ();
        return;
	}else{
	   c_parser_error (parser, "expected identifier");
