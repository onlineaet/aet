...
void aet_utils_create_in_class_iface_var(char *ifaceName,char *result)
{
   ...
   unsigned int hash=aet_utils_create_hash(temp,strlen(temp));
...
c_token      *aet_utils_create_const_token(c_token *token,location_t start_loc)
{
   token->type = CPP_KEYWORD;
   token->id_kind=C_ID_NONE;
   token->keyword = RID_CONST;
   token->pragma_kind = PRAGMA_NONE;
   token->value = aet_utils_create_identifier("const",5);
   C_SET_RID_CODE(token->value,RID_CONST);
   TREE_LANG_FLAG_0 (token->value) = 1;
   token->location = start_loc;
   return token;
}
...
c_token  *aet_utils_create_attribute_token(c_token *token,location_t start_loc)
{
   token->type = CPP_KEYWORD;
   token->id_kind=C_ID_NONE;
   token->keyword = RID_ATTRIBUTE;
   token->pragma_kind = PRAGMA_NONE;
   token->value=aet_utils_create_identifier("__attribute__",13);
   C_SET_RID_CODE(token->value,RID_ATTRIBUTE);
   TREE_LANG_FLAG_0 (token->value) = 1;
   token->location = start_loc;
   return token;
}

c_token      *aet_utils_create_unsigned_token(c_token *token,location_t start_loc)
{
   token->type = CPP_KEYWORD;
   token->id_kind=C_ID_NONE;
   token->keyword = RID_UNSIGNED;
   token->pragma_kind = PRAGMA_NONE;
   token->value = aet_utils_create_identifier("unsigned",8);
   C_SET_RID_CODE(token->value,RID_UNSIGNED);
   TREE_LANG_FLAG_0 (token->value) = 1;
   token->location = start_loc;
   return token;
}

c_token      *aet_utils_create_long_token(c_token *token,location_t start_loc)
{
   token->type = CPP_KEYWORD;
   token->id_kind=C_ID_NONE;
   token->keyword = RID_LONG;
   token->pragma_kind = PRAGMA_NONE;
   token->value = aet_utils_create_identifier("long",4);
   C_SET_RID_CODE(token->value,RID_LONG);
   TREE_LANG_FLAG_0 (token->value) = 1;
   token->location = start_loc;
   return token;
}
...
void  aet_utils_create_key_token(c_token *token,location_t start_loc,int key,const char *keyName)
{
   token->type = CPP_KEYWORD;
   token->id_kind=C_ID_NONE;
   token->keyword = key;
   token->pragma_kind = PRAGMA_NONE;
   token->value=aet_utils_create_identifier(keyName,strlen(keyName));
   C_SET_RID_CODE(token->value,key);
   TREE_LANG_FLAG_0 (token->value) = 1;
   token->location = start_loc;
   return token;
}
...
/**
 * 取出变量名的类名和变量名
 * _V14debug_ASecond29MAX_COUNTEw
 * ‘_V‘是前缀
 */
int aet_utils_get_orgi_var_and_class_name(char *srcVarName,char *className,char *varName)
{
   if(srcVarName[0]!='_')
      return 0;
   if(srcVarName[1]!='V')
      return 0;
    if(!n_ascii_isdigit(srcVarName[2]))
      return 0;
    char classNameLen[strlen(srcVarName)];
    int i=2;
    int count=0;
    while(n_ascii_isdigit(srcVarName[i])){
      classNameLen[count++]=srcVarName[i];
        i++;
    }
    classNameLen[count]='\0';
    int len=atoi((const char*)classNameLen);
    memcpy(className,srcVarName+i,len);
    className[len]='\0';
    i+=len;
    count=0;
    while(n_ascii_isdigit(srcVarName[i])){
    ...
    classNameLen[count]='\0';
    len=atoi((const char*)classNameLen);
    memcpy(varName,srcVarName+i,len);
    varName[len]='\0';
    return len;
}
...
/**
 * 在class声明最后加入一个初始化方法
 * package_Abc_init_1234ergR5678_package_Abc
 */
char* aet_utils_create_init_method(char *sysName)
{
   char buffer[256];
   unsigned int hash=aet_utils_create_hash(sysName,strlen(sysName));
  // sprintf(result,"%s%u",temp,hash);
  // sprintf(buffer,"%s_%s_%s",sysName,AET_INIT_GLOBAL_METHOD_STRING,sysName);
   sprintf(buffer,"%s_%s_%u",sysName,AET_INIT_GLOBAL_METHOD_STRING,hash);
...
/**
 * 创建类初始化super数据的函数名
 * void TSecond_init_global_super_data_1_TSecond(unsigned long *addr,char **names,unsigned long **parentAddr,char ***parentNames)
 */
char* aet_utils_create_init_global_super_func_name(char *sysName)
{
   return n_strdup_printf("%s_%s_%s",sysName,AET_INIT_GLOBAL_SUPER_FUNC_NAME,sysName);
}
//static void TSecond_init_inner_super_data_2_TSecond()
char* aet_utils_create_init_inner_super_func_name(char *sysName)
{
   return n_strdup_printf("%s_%s_%s",sysName,AET_INIT_INNER_SUPER_FUNC_NAME,sysName);
}
...
static int addAetBuiltinCodes(cpp_reader *pfile, const char *str,size_t len,location_t loc,nboolean forceLocation)
{
   ...
   location_t oldInput=input_location;
   int count=0;
   location_t old=loc;
   if(forceLocation){
      bool ret=pfile->line_table->pure_location_p (loc);
      while(!ret){
         //expanded_location xloc;
        // xloc = expand_location(loc);
        // n_warning("addAetBuiltinCodes++ 位置无效---尝试loc+1----------- %d %d loc:%d count:%d\n",xloc.line,xloc.column,loc,count);
         loc+=1;
         ret=pfile->line_table->pure_location_p (loc);
         if(count>130){
            n_warning("没有找到正确的位置 count:%d :%s 文件:%s ",count,str,in_fnames[0]);
            break;
         }
         count++;
      }
      ...
      while(!ret){
          //expanded_location xloc;
          // xloc = expand_location(loc);
         //  n_warning("addAetBuiltinCodes-- 位置无效-XXX--尝试loc-1----------- %d %d loc:%d count:%d\n",xloc.line,xloc.column,loc,count);
         loc--;
         ret=pfile->line_table->pure_location_p (loc);
         if(count>139){
            n_warning("没有找到正确的位置 11  count:%d :%s 文件:%s ",count,str,in_fnames[0]);
            break;
         }
         count++;
      }
...
{
   char temp[255];
   unsigned int hash=aet_utils_create_hash(userName,strlen(userName));
   sprintf(temp,"_e0%lu%s%lu%s%u",strlen(sysName),sysName,strlen(userName),userName,hash);
...
{
   char temp[255];
   unsigned int hash=aet_utils_create_hash(origElement,strlen(origElement));
   sprintf(temp,"_e1%lu%s%lu%s%u",strlen(sysName),sysName,strlen(origElement),origElement,hash);
   return n_strdup(temp);
}
 ...
 * 是否是类创建，初始化，泛型等外部函数
 */
nboolean aet_utils_is_inner_function(char *funcName)
{
