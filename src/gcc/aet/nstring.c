...
NString *n_string_assign (NString  *string, const nchar *rval)
{
   n_return_val_if_fail (string != NULL, NULL);
   ...
   */
   if (N_UNLIKELY (val >= string->str && val <= string->str + string->len)){
      ...
      if (len_unsigned > precount)
         memcpy (string->str + pos_unsigned + precount,val + /* Already moved: */ precount +
         /* Space opened up: */ len_unsigned, len_unsigned - precount);
   }else{
         n_string_maybe_expand (string, len_unsigned);
...
NString *n_string_erase (NString *string,nssize   pos,nssize   len)
{
   ...
   if (len < 0)
      ...
      if (pos_unsigned + len_unsigned < string->len)
         memmove (string->str + pos_unsigned, string->str + pos_unsigned + len_unsigned,
...
int n_string_last_indexof(NString *self,const char *str)
{
 ...
 NString *n_string_new (const nchar *init)
 {
   NString *self;
   if (init == NULL || *init == '\0')
 	  self = n_string_sized_new (64);
   else{
       nint len;
       len = strlen (init);
       self = n_string_sized_new (2*len + 2);//多分配一倍的大小
