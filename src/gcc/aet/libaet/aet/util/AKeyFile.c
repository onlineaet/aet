...
impl$ AKeyFile{
   ...
   */
   ...
   static void parseKeyValuePair(const char  *line,asize length,AError **error){
      ...
      if (currentGroup == NULL || currentGroup->name == NULL){
         ...
         a_error_set (error, ERROR_DOMAIN,KeyFileError.GROUP_NOT_FOUND,"文件内容并不是从组开始的。");
         return;
      }
      ...
      */
      ...
      if (currentGroup && currentGroup->name && strcmp(startGroup->name,currentGroup->name)== 0
   ...
   static void parseData(const char *data,asize  length,AError **error){
      ...
      while (i < length){
         if (data[i] == '\n'){
            //printf("parseData 换行 %d %s\n",strBuffer->length(),strBuffer->getString());
   ...
   static void   set_not_found_key_error (const char *group_name,const char *key,AError  **error){
      a_error_printf(error, ERROR_DOMAIN,KeyFileError.KEY_NOT_FOUND,
            "Key file does not have key “%s” in group “%s”",key, group_name);
   }
   ...
   */
   ...
   public$ char    *getValue (int groupIndex,const char *key,AError **error){
      if(groupIndex<0 || groupIndex>=groups->length()){
         if(error)
            a_error_printf(error, ERROR_DOMAIN,-2,"下标越界。groupIndex:%d AList length:%d",groupIndex,groups->length());
         return NULL;
      }
      KeyFileGroup *gp=(KeyFileGroup *)groups->get(groupIndex);
      KeyFileKeyValuePair *pair = lookupKeyValuePair(gp, key);
      if(pair)
         return pair->value;
      return NULL;
   }
   ...
   /**
    * 找到第一个key对应的value,从第一个组开始查找
    */
   char *getFirstValue (const char *key,AError **error){
      ListIterator *iter=groups->initIter();
      apointer value=NULL;
      while(iter->hasNext(&value)){
         KeyFileGroup *gp=(KeyFileGroup *)value;
   ...
   */
   ...
   public$ const char *getGroupName(int index){
      int nums = groups->length();
      if(index<0 || index>nums){
         return NULL;
      }
      KeyFileGroup *g= groups->get(index);
      return g->name;
   }
