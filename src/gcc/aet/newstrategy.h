...
struct _NewStrategy
{
   ...
   NPtrArray *mtcsPlatformAndDevnum;
};

char      *new_strategy_recursion_init_parent_generic_info(NewStrategy *self,char *refVarName,
                     ClassName *refClassName,nboolean deref);
...
void       new_strategy_new_object_from_stack_no_name(NewStrategy *self,char *varName,GenericModel *genericDefine,
		                  char *ctorStr,ClassName *className,NString *codes,tree mtcsplatformType);
