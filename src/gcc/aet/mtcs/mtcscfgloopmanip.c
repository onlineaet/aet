...
static void copy_loops_to (MtcsCfgLoopManip *self,class loop **, int,class loop *);
static void loop_redirect_edge (MtcsCfgLoopManip *self,edge, basic_block);
static void remove_bbs (MtcsCfgLoopManip *self,basic_block *, int);
...
static bool rpe_enum_p (const_basic_block bb, const void *data)
{
...
static void remove_bbs (MtcsCfgLoopManip *self,basic_block *bbs, int nbbs)
{
   ...
   for (i = 0; i < nbbs; i++)
      mtcs_cfg_context_delete_basic_block/*!delete_basic_block*/(mtcsCfgContext,bbs[i]);
}
...
/* Find path -- i.e. the basic blocks dominated by edge E and put them
   ...
   path is returned.  */
static int find_path (edge e, basic_block **bbs)
{
   gcc_assert (EDGE_COUNT (e->dest->preds) <= 1);

   /* Find bbs in the path.  */
   *bbs = XNEWVEC (basic_block, n_basic_blocks_for_fn (cfun));
   return dfs_enumerate_from (e->dest, 0, rpe_enum_p, *bbs,n_basic_blocks_for_fn (cfun), e->dest);
}
...
static void fix_bb_placements (basic_block from, bool *irred_invalidated, bitmap loop_closed_ssa_invalidated)
{
   ...
   if (base_loop == current_loops->tree_root || from == base_loop->header)
      return;
   ...
   *qbeg = from;
   ...
   while (qbeg != qend){
      ...
      if (from->loop_father->header == from){
         ...
         if (!fix_loop_placement (from->loop_father, irred_invalidated,loop_closed_ssa_invalidated))
            continue;
      ...
      FOR_EACH_EDGE (e, ei, from->preds){
         ...
         if (pred->loop_father != base_loop && (nca == base_loop || nca != pred->loop_father))
...
bool mtcs_cfg_loop_manip_remove_path (MtcsCfgLoopManip *self,edge e, bool *irred_invalidated, bitmap loop_closed_ssa_invalidated)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
   ...
   if (!mtcs_cfg_context_can_remove_branch_p/*!can_remove_branch_p*/(mtcsCfgContext,e))
      return false;
   ...
   if (!single_pred_p (e->dest))
      e = single_pred_edge (mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,e));
   ...
   for (i = 0; i < nrem; i++){
      FOR_EACH_EDGE (ae, ei, rem_bbs[i]->succs)
         if (ae->dest != EXIT_BLOCK_PTR_FOR_FN (cfun)  && !bitmap_bit_p (seen, ae->dest->index)){
   ...
   /* Remove the path.  */
   from = e->src;
   mtcs_cfg_context_remove_branch/*!remove_branch*/(mtcsCfgContext,e);
   dom_bbs.create (0);
   ...
   for (i = 0; i < n_bord_bbs; i++){
      ...
      for (ldom = first_dom_son (CDI_DOMINATORS, bb); ldom; ldom = next_dom_son (CDI_DOMINATORS, ldom))
   ...
   fix_loop_placements (from->loop_father, irred_invalidated,loop_closed_ssa_invalidated);

   if (local_irred_invalidated  && loops_state_satisfies_p (LOOPS_HAVE_MARKED_IRREDUCIBLE_REGIONS))
...
void mtcs_cfg_loop_manip_add_loop (MtcsCfgLoopManip *self,class loop *loop, class loop *outer)
{
   ...
   mtcs_cfg_loop_manip_place_new_loop/*!place_new_loop*/(self,cfun, loop);
   ...
   for (i = 0; i < n; i++){
      ...
      if (loop_outer (subloop) == outer  && subloop->header == bbs[i]){
                   ...
                   edge exit_edge, profile_count desired_count)
{
   ...
   if (desired_count > exit_edge->src->count  && exit_edge->src->count.differs_from_p (desired_count)){
      if (dump_file){
         fprintf (dump_file, ";; Source bb of loop %i has count ",loop->num);
         exit_edge->src->count.dump (dump_file, cfun);
         fprintf (dump_file, " which is smaller then desired count of exitting loop ");
   ...
   if (!exit_edge->src->count.nonzero_p ()){
      if (dump_file){
         fprintf (dump_file, ";; Not updating exit edge probability"
               " in loop %i since profile is zero ",loop->num);
      }
      return NULL;
   }
   set_edge_probability_and_rescale_others (exit_edge, desired_count.probability_in (exit_edge->src->count));
   ...
   FOR_EACH_EDGE (e, ei, exit_edge->src->succs)
      if (!(e->flags & EDGE_FAKE) && !loop_exit_edge_p (loop, e)) {
         if (found){
            other_edge = NULL;
            break;
         }
   ...
   if (other_edge && other_edge->dest == loop->latch){
      if (single_pred_p (loop->latch))
         loop->latch->count = loop->latch->count + old_exit_count - exit_edge->count ();
   }else
      /* If there are multiple blocks, just scale.  */
      mtcs_cfg_loop_manip_scale_dominated_blocks_in_loop/*!scale_dominated_blocks_in_loop*/(self,loop, exit_edge->src,
          ...
          gcov_type iteration_bound)
{
   if (!(p == profile_probability::always ())){
      if (dump_file && (dump_flags & TDF_DETAILS)){
         fprintf (dump_file, ";; Scaling loop %i with scale ",loop->num);
         p.dump (dump_file);
         fprintf (dump_file, "\n");
      }
      /* Scale the probabilities.  */
      mtcs_cfg_loop_manip_scale_loop_frequencies/*!scale_loop_frequencies*/(self,loop, p);
   }
   ...
   if (dump_file && (dump_flags & TDF_DETAILS)){
      fprintf (dump_file, ";; Guessed iterations of loop %i is %f. New upper bound %i.\n",
            loop->num, iterations.to_double (),(int)iteration_bound);
   }
   ...
   profile_probability scale_prob = (count_in * (iteration_bound + 1)).probability_in (loop->header->count);
   if (dump_file && (dump_flags & TDF_DETAILS)){
      fprintf (dump_file, ";; Scaling loop %i with scale ",loop->num);
   ...
   mtcs_cfg_loop_manip_scale_loop_frequencies/*!scale_loop_frequencies*/(self,loop, scale_prob);
   mtcs_cfg_loop_manip_update_loop_exit_probability_scale_dom_bbs/*!update_loop_exit_probability_scale_dom_bbs*/(self,
   loop, exit_edge,unadjusted_exit_count);
}
...
static void update_dominators_in_loop (class loop *loop)
{
   ...
   for (i = 0; i < loop->num_nodes; i++){
      basic_block ldom;
      for (ldom = first_dom_son (CDI_DOMINATORS, body[i]); ldom; ldom = next_dom_son (CDI_DOMINATORS, ldom))
...
edge mtcs_cfg_loop_manip_create_empty_if_region_on_edge (MtcsCfgLoopManip *self,edge entry_edge, tree condition)
{
   ...
   cond_bb = mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,entry_edge);

   /* Insert condition in cond_bb.  */
   gsi = gsi_last_bb (cond_bb);
   simple_cond = force_gimple_operand_gsi (&gsi, condition, true, NULL,false, GSI_NEW_STMT);
   cond_stmt = gimple_build_cond_from_tree (simple_cond, NULL_TREE, NULL_TREE);
   gsi = gsi_last_bb (cond_bb);
   gsi_insert_after (&gsi, cond_stmt, GSI_NEW_STMT);

   join_bb = mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,single_succ_edge (cond_bb));

   e_true = single_succ_edge (cond_bb);
   true_bb = mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,e_true);

   e_false = mtcs_cfg_make_edge/*!make_edge*/(mtcsCfg,cond_bb, join_bb, 0);
   false_bb = mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,e_false);
            ...
            class loop *outer)
{
   ...
   loop_header = mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,entry_edge);
   loop_latch = mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,single_succ_edge (loop_header));
   succ_bb = single_succ (loop_latch);
   mtcs_cfg_make_edge/*!make_edge*/(mtcsCfg,loop_header, succ_bb, 0);
   mtcs_cfg_context_redirect_edge_succ_nodup/*!redirect_edge_succ_nodup*/(mtcsCfgContext,single_succ_edge (loop_latch), loop_header);
   ...
   mtcs_cfg_loop_manip_add_loop/*!add_loop*/(self,loop, outer);

   /* TODO: Fix counts.  */
   mtcs_cfg_loop_manip_scale_loop_frequencies/*!scale_loop_frequencies*/(self,loop, profile_probability::even ());
   ...
   create_iv (initial_value, PLUS_EXPR, stride, iv, loop, &gsi, false,iv_before, iv_after);

   /* Insert loop exit condition.  */
   cond_expr = gimple_build_cond (LT_EXPR, *iv_before, upper_bound, NULL_TREE, NULL_TREE);

   exit_test = gimple_cond_lhs (cond_expr);
   exit_test = force_gimple_operand_gsi (&gsi, exit_test, true, NULL, false, GSI_NEW_STMT);
   gimple_cond_set_lhs (cond_expr, exit_test);
   gsi = gsi_last_bb (exit_e->src);
   gsi_insert_after (&gsi, cond_expr, GSI_NEW_STMT);

   mtcs_cfg_context_split_block_after_labels/*!split_block_after_labels*/(mtcsCfgContext,loop_header);
   ...
   bitmap loop_closed_ssa_invalidated)
{
   ...
   mtcs_cfg_context_remove_edge/*!remove_edge*/(mtcsCfgContext,single_succ_edge (latch));
...
static void fix_loop_placements (class loop *loop, bool *irred_invalidated,
           bitmap loop_closed_ssa_invalidated)
{
   class loop *outer;
   ...
   while (loop_outer (loop)){
      ...
      if (!fix_loop_placement (loop, irred_invalidated, loop_closed_ssa_invalidated))
         break;
      ...
      fix_bb_placements (loop_preheader_edge (loop)->src, irred_invalidated, loop_closed_ssa_invalidated);
      loop = outer;
   }
}
...
void mtcs_cfg_loop_manip_copy_loop_info (MtcsCfgLoopManip *self,class loop *loop, class loop *target)
{
   ...
   target->nb_iterations_likely_upper_bound = loop->nb_iterations_likely_upper_bound;
   ...
   target->warned_aggressive_loop_optimizations |= loop->warned_aggressive_loop_optimizations;
...
class loop * mtcs_cfg_loop_manip_duplicate_loop (MtcsCfgLoopManip *self,class loop *loop, class loop *target, class loop *after)
{
   ...
   class loop *cloop;
   cloop = alloc_loop ();
   mtcs_cfg_loop_manip_place_new_loop/*!place_new_loop*/(self,cfun, cloop);
   mtcs_cfg_loop_manip_copy_loop_info/*!copy_loop_info*/(self,loop, cloop);
   /* Mark the new loop as copy of LOOP.  */
   mtcs_cfg_set_loop_copy/*!set_loop_copy*/(mtcsCfg,loop, cloop);
   /* Add it to target.  */
...
void mtcs_cfg_loop_manip_duplicate_subloops (MtcsCfgLoopManip *self,class loop *loop, class loop *target)
{
   ...
   for (aloop = loop->inner; aloop; aloop = aloop->next){
      cloop = mtcs_cfg_loop_manip_duplicate_loop/*!duplicate_loop*/(self,aloop, target, tail);
      tail = cloop;
      gcc_assert(!tail->next);
      mtcs_cfg_loop_manip_duplicate_subloops/*!duplicate_subloops*/(self,aloop, cloop);
   }
}
...
static void copy_loops_to (MtcsCfgLoopManip *self,class loop **copied_loops, int n, class loop *target)
{
   ...
   for (tail = target->inner; tail && tail->next; tail = tail->next)
      ;
   for (i = 0; i < n; i++){
      aloop = mtcs_cfg_loop_manip_duplicate_loop/*!duplicate_loop*/(self,copied_loops[i], target, tail);
      tail = aloop;
      gcc_assert(!tail->next);
      mtcs_cfg_loop_manip_duplicate_subloops/*!duplicate_subloops*/(self,copied_loops[i], aloop);
   }
}

/* Redirects edge E to basic block DEST.  */
static void loop_redirect_edge (MtcsCfgLoopManip *self,edge e, basic_block dest)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgContext *mtcsCfgContext=mtcs_target_get_cfg_context(mtcsTarget);
...
bool mtcs_cfg_loop_manip_can_duplicate_loop_p (MtcsCfgLoopManip *self,const class loop *loop)
{
   ...
   int ret;
   basic_block *bbs = get_loop_body (loop);
   ret = mtcs_cfg_context_can_copy_bbs_p/*!can_copy_bbs_p*/(mtcsCfgContext,bbs, loop->num_nodes);
   free (bbs);
   return ret;
}
                ...
                edge orig, vec<edge> *to_remove, int flags)
{
   ...
   n_debug("mtcscfgloopmanip.c duplicate_loop_body_to_header_edge 00 header:%p n:%d\n",loop->header,n);

   /* Check whether duplication is possible.  */
   if (!mtcs_cfg_context_can_copy_bbs_p/*!can_copy_bbs_p*/(mtcsCfgContext,bbs, loop->num_nodes)){
      free (bbs);
      return false;
   }
   ...
   if (flags & DLTHE_FLAG_UPDATE_FREQ){
      ...
      if (orig && orig->probability.initialized_p () && !(orig->probability == profile_probability::always ())){
         ...
         for (i = 0; i < n; i++){
            if (bbs[i] != orig->src && dominated_by_p (CDI_DOMINATORS, bbs[i], orig->src))
         ...
         if (after_exit_den.nonzero_p ()){
            auto_vec<edge> exits = get_loop_exit_edges (loop);
            for (edge ex : exits)
               if (ex != orig && dominated_by_p (CDI_DOMINATORS, ex->src, orig->src))
                  new_count_le -= ex->count ().apply_scale (after_exit_num - after_exit_den, after_exit_den);
         }
      }
      profile_probability prob_pass_wont_exit = new_count_le.probability_in (count_in);
      ...
      if (!count_in.nonzero_p ()){
         prob_pass_thru = profile_probability::always ().apply_scale (1, 2);
         prob_pass_wont_exit = profile_probability::always ().apply_scale (1, 2);
      }

      scale_step = XNEWVEC (profile_probability, ndupl);

      for (i = 1; i <= ndupl; i++)
         scale_step[i - 1] = bitmap_bit_p (wont_exit, i) ? prob_pass_wont_exit  : prob_pass_thru;
      ...
      number of iterations.  */
         ...
         profile_probability prob_pass_main = bitmap_bit_p (wont_exit, 0) ? prob_pass_wont_exit: prob_pass_thru;
   ...
   mtcs_cfg_set_loop_copy/*!set_loop_copy*/(mtcsCfg,loop, target);
   ...
   n_debug("mtcscfgloopmainip.c duplicate_loop_body_to_header_edge 11 loop:%p currentloops:%p header:%p n:%d ndupl:%d place_after:%p\n",
         loop,current_loops,loop->header,n,ndupl,place_after);

   for (j = 0; j < ndupl; j++){
      /* Copy loops.  */
      copy_loops_to(self,orig_loops, n_orig_loops, target);
      n_debug("mtcscfgloopmanip.c duplicate_loop_body_to_header_edge 22 j:%d ndupl:%d n:%d header:%p\n",j,ndupl,n,loop->header);
      /* Copy bbs.  */
      mtcs_cfg_context_copy_bbs/*!copy_bbs*/(mtcsCfgContext,bbs, n, new_bbs, spec_edges, 2, new_spec_edges, loop,place_after, true);
      place_after = new_spec_edges[SE_LATCH]->src;
      n_debug("mtcscfgloopmanip.c duplicate_loop_body_to_header_edge 22aa j:%d ndupl:%d n:%d header:%p\n",j,ndupl,n,loop->header);
      ...
      if (add_irreducible_flag){
         ...
         for (i = 0; i < n; i++){
            ...
            if ((ae->dest->flags & BB_DUPLICATED) && (ae->src->loop_father == target || ae->dest->loop_father == target))
               ae->flags |= EDGE_IRREDUCIBLE_LOOP;
         }
         for (i = 0; i < n; i++)
            new_bbs[i]->flags &= ~BB_DUPLICATED;
      }
      n_debug("mtcscfgloopmanip.c duplicate_loop_body_to_header_edge 22bb j:%d ndupl:%d n:%d header:%p is_latch:%d\n",
            j,ndupl,n,loop->header,is_latch);

      /* Redirect the special edges.  */
      if (is_latch){
         mtcs_cfg_context_redirect_edge_and_branch_force/*!redirect_edge_and_branch_force*/(mtcsCfgContext,latch_edge, new_bbs[0]);
         mtcs_cfg_context_redirect_edge_and_branch_force/*!redirect_edge_and_branch_force*/(mtcsCfgContext,
               new_spec_edges[SE_LATCH],loop->header);
         set_immediate_dominator (CDI_DOMINATORS, new_bbs[0], latch);
         latch = loop->latch = new_bbs[n - 1];
         e = latch_edge = new_spec_edges[SE_LATCH];
      }else{
         mtcs_cfg_context_redirect_edge_and_branch_force/*!redirect_edge_and_branch_force*/(mtcsCfgContext,e, new_bbs[0]);
         mtcs_cfg_context_redirect_edge_and_branch_force/*!redirect_edge_and_branch_force*/(mtcsCfgContext,
               new_spec_edges[SE_LATCH],loop->header);
      ...
      if (orig && bitmap_bit_p (wont_exit, j + 1)){
         ...
         if (bbs_to_scale && scale_after_exit){
            ...
            scale_bbs_frequencies_profile_count (new_bbs + i, 1, after_exit_num,after_exit_den);
         }
      }
   ...
   if (orig && bitmap_bit_p (wont_exit, 0)){
      ...
      if (bbs_to_scale && scale_after_exit){
         EXECUTE_IF_SET_IN_BITMAP (bbs_to_scale, 0, i, bi)
               scale_bbs_frequencies_profile_count (bbs + i, 1, after_exit_num,after_exit_den);
      }
   }
   ...
   for (i = 0; i < n; i++){
      ...
      FOR_EACH_VEC_ELT (dom_bbs, j, dominated){
         if (flow_bb_inside_loop_p (loop, dominated))
            continue;
         dom_bb = nearest_common_dominator (CDI_DOMINATORS, first_active[i], first_active_latch);
...
basic_block mtcs_cfg_loop_mainip_create_preheader (MtcsCfgLoopManip *self,class loop *loop, int flags)
{
   ...
   if (nentry == 1){
      ...
      if (single_entry->src == ENTRY_BLOCK_PTR_FOR_FN (cfun))
         ...
         if ((flags & CP_SIMPLE_PREHEADERS) && ((single_entry->flags & EDGE_COMPLEX)
         ...
         preheader ends with a jump or has predecessors from loop.  */
          ...
          && (JUMP_P (BB_END (single_entry->src)) || has_preds_from_loop (single_entry->src, loop)))
            need_forwarder_block = true;
      }
   ...
   || (single_entry->flags & EDGE_CROSSING) == 0))
      dummy = mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,single_entry);
   else{
   ...
   if (flags & CP_FALLTHRU_PREHEADERS)
      gcc_assert ((single_succ_edge (dummy)->flags & EDGE_FALLTHRU) && !JUMP_P (BB_END (dummy)));
...
void mtcs_cfg_loop_mainip_create_preheader (MtcsCfgLoopManip *self,int flags)
{
   ...
   for (auto loop : loops_list (cfun, 0))
      mtcs_cfg_loop_mainip_create_preheader/*!create_preheader*/(self,loop, flags);
...
void mtcs_cfg_loop_mainip_force_single_succ_latches (MtcsCfgLoopManip *self)
{
   ...
   for (auto loop : loops_list (cfun, 0)){
      ...
      mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,e);
   }
...
/* This function is called from loop_version.  It splits the entry edge
  ...
  IFN_LOOP_DIST_ALIAS.  */
static basic_block lv_adjust_loop_entry_edge (MtcsCfgLoopManip *self,basic_block first_head, basic_block second_head,
            ...
            profile_probability else_prob)
{
   ...
   new_head = mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,e);

   lv_add_condition_to_bb (first_head, second_head, new_head, cond_expr);

   /* Don't set EDGE_TRUE_VALUE in RTL mode, as it's invalid there.  */
   e = single_succ_edge (new_head);
   e1 = mtcs_cfg_make_edge/*!make_edge*/(mtcsCfg,new_head, first_head,current_ir_type () == IR_GIMPLE ? EDGE_TRUE_VALUE : 0);
         ...
         bool place_after)
{
   ...
   if (!mtcs_cfg_context_cfg_hook_duplicate_loop_body_to_header_edge/*!cfg_hook_duplicate_loop_body_to_header_edge*/
   (mtcsCfgContext,loop, entry, 1, NULL, NULL, NULL, 0)){
      entry->flags |= irred_flag;
      return NULL;
   }

   /* 2) loopify the duplicated new loop. */
   latch_edge = single_succ_edge (mtcs_cfg_get_bb_copy/*!get_bb_copy*/(mtcsCfg,loop->latch));
   nloop = alloc_loop ();
   class loop *outer = loop_outer (latch_edge->dest->loop_father);
   edge new_header_edge = single_pred_edge (mtcs_cfg_get_bb_copy/*!get_bb_copy*/(mtcsCfg,loop->header));
   nloop->header = new_header_edge->dest;
   nloop->latch = latch_edge->src;
   loop_redirect_edge(self,latch_edge, nloop->header);

   /* Compute new loop.  */
   mtcs_cfg_loop_manip_add_loop/*!add_loop*/(self,nloop, outer);
   mtcs_cfg_loop_manip_copy_loop_info/*!copy_loop_info*/(self,loop, nloop);
   mtcs_cfg_set_loop_copy/*!set_loop_copy*/(mtcsCfg,loop, nloop);
   ...
   cond_bb =  lv_adjust_loop_entry_edge(self,first_head, second_head,
   ...
   mtcs_cfg_loop_manip_scale_loop_frequencies/*!scale_loop_frequencies*/(self, loop, then_scale);
   mtcs_cfg_loop_manip_scale_loop_frequencies/*!scale_loop_frequencies*/(self, nloop, else_scale);
   update_dominators_in_loop (loop);
   ...
   one successor.  */
   mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,loop_preheader_edge (loop));
   mtcs_cfg_context_split_edge/*!split_edge*/(mtcsCfgContext,loop_preheader_edge (nloop));

   return nloop;
}

static void mtcsCfgLoopManipInit(MtcsCfgLoopManip *self)
{
}

MtcsCfgLoopManip *mtcs_cfg_loop_manip_new(MtcsMode *mtcsMode)
{
   MtcsCfgLoopManip *self = n_slice_alloc0 (sizeof(MtcsCfgLoopManip));
   mtcs_component_set_mode((MtcsComponent *)self,mtcsMode);
   mtcsCfgLoopManipInit(self);
   return self;
}
