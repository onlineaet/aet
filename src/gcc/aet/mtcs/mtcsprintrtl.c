...
static FILE *dumpFile=fopen("temprtl.txt","w");
...
mtcs_rtx_writer::mtcs_rtx_writer (FILE *outf, int ind, bool simple, bool compact,
         mtcs_rtx_reuse_manager *reuse_manager ATTRIBUTE_UNUSED,void *mtcsTarget)
: m_outfile (outf), m_indent (ind), m_sawclose (false),
  m_in_call_function_usage (false), m_simple (simple), m_compact (compact)
  , m_rtx_reuse_manager (reuse_manager),target(mtcsTarget)
{
}
...
mtcs_rtx_reuse_manager::mtcs_rtx_reuse_manager () : m_next_id (0)
{
}
...
void mtcs_rtx_reuse_manager::preprocess (const_rtx x)
{
...
bool mtcs_rtx_reuse_manager::has_reuse_id (const_rtx x, int *out)
{
...
bool mtcs_rtx_reuse_manager::seen_def_p (int reuse_id)
{
...
void mtcs_rtx_reuse_manager::set_seen_def (int reuse_id)
{
...
{
   fputc (' ', outfile);
   print_generic_expr (outfile, CONST_CAST_TREE (expr), dump_flags | TDF_SLIM);
}
...
void mtcs_rtx_writer::print_rtx_operand_code_0 (const_rtx in_rtx ATTRIBUTE_UNUSED,
                  int idx ATTRIBUTE_UNUSED)
{
   MtcsTarget *mtcsTarget=(MtcsTarget *)target;
   ...
   if (idx == 1 && GET_CODE (in_rtx) == SYMBOL_REF){
      ...
      switch (NOTE_KIND (in_rtx)){
         ...
         case NOTE_INSN_CFI:
            fputc ('\n', m_outfile);
            mtcs_dwarf2_cfi_output_cfi_directive/*!output_cfi_directive*/(mtcsDwarf2Cfi,m_outfile,NOTE_CFI (in_rtx));
            fputc ('\t', m_outfile);
            break;
         ...
         {
            expanded_location xloc = expand_location (NOTE_MARKER_LOCATION (in_rtx));
   ...
   }else if (idx == 7 && JUMP_P (in_rtx) && JUMP_LABEL (in_rtx) != NULL  && !m_compact){
...
void mtcs_rtx_writer::print_rtx_operand_code_e (const_rtx in_rtx, int idx)
{
   m_indent += 2;
...
void mtcs_rtx_writer::print_rtx_operand_codes_E_and_V (const_rtx in_rtx, int idx)
{
   MtcsTarget *mtcsTarget=(MtcsTarget *)target;
   MtcsMode *mtcsMode=mtcsTarget->mtcsMode;
   m_indent += 2;
   ...
   && !mtcs_mode_get_nunits/*!GET_MODE_NUNITS*/(mtcsMode,GET_MODE (in_rtx)).is_constant ()
      ...
      for (int j = 0; j < XVECLEN (in_rtx, idx); j++){
         ...
         if (j == barrier){
            ...
            if (!CONST_VECTOR_STEPPED_P (in_rtx))
               ...
               fprintf (m_outfile, "stepped (interleave %d) [",CONST_VECTOR_NPATTERNS (in_rtx));
            m_indent += 2;
         }
...
void mtcs_rtx_writer::print_rtx_operand_code_L (const_rtx in_rtx, int idx)
{
   if (idx == 4 && INSN_P (in_rtx)){
      ...
      if (INSN_HAS_LOCATION (in_insn)){
         ...
         fprintf (m_outfile, " \"%s\":%i:%i", xloc.file, xloc.line,xloc.column);
      ...
      if (ASM_OPERANDS_SOURCE_LOCATION (in_rtx) != UNKNOWN_LOCATION)
         fprintf (m_outfile, " %s:%i",LOCATION_FILE (ASM_OPERANDS_SOURCE_LOCATION (in_rtx)),
      ...
      if (ASM_INPUT_SOURCE_LOCATION (in_rtx) != UNKNOWN_LOCATION)
         fprintf (m_outfile, " %s:%i",LOCATION_FILE (ASM_INPUT_SOURCE_LOCATION (in_rtx)),
...
void mtcs_rtx_writer::print_rtx_operand_code_i (const_rtx in_rtx, int idx)
{
   MtcsTarget *mtcsTarget=(MtcsTarget *)target;
   MtcsMode *mtcsMode=mtcsTarget->mtcsMode;
   ...
   MtcsUnspec *mtcsUnspec=mtcs_target_get_unspec(mtcsTarget);
   ...
   }else if (mtcs_unspec_get_unspecv_num/*!NUM_UNSPECV_VALUES*/(mtcsUnspec) > 0
   && idx == 1
   && GET_CODE (in_rtx) == UNSPEC_VOLATILE
   && XINT (in_rtx, 1) >= 0
   && XINT (in_rtx, 1) < NUM_UNSPECV_VALUES)
      fprintf (m_outfile, " %s", mtcs_unspec_get_unspecv_string/*!unspecv_strings[XINT (in_rtx, 1)]*/(mtcsUnspec,XINT (in_rtx, 1)));
   else if (mtcs_unspec_get_unspec_num/*!NUM_UNSPEC_VALUES*/(mtcsUnspec) > 0
   && idx == 1
   && (GET_CODE (in_rtx) == UNSPEC
   || GET_CODE (in_rtx) == UNSPEC_VOLATILE)
   && XINT (in_rtx, 1) >= 0
   && XINT (in_rtx, 1) < NUM_UNSPEC_VALUES)
      fprintf (m_outfile, " %s", mtcs_unspec_get_unspec_string/*!unspec_strings[XINT (in_rtx, 1)]*/(mtcsUnspec,XINT (in_rtx, 1)));
   else{
      ...
      if (mtcsOptionsItem->x_flag_dump_unnumbered && (is_insn || NOTE_P (in_rtx)))
         fputc ('#', m_outfile);
      else
         fprintf (m_outfile, " %d", value);

      if (is_insn && &INSN_CODE (in_rtx) == &XINT (in_rtx, idx)
      && XINT (in_rtx, idx) >= 0 && (name = mtcs_output_get_insn_name/*!get_insn_name*/(mtcsOutput,XINT (in_rtx, idx))) != NULL)
...
void mtcs_rtx_writer::print_rtx_operand_code_r (const_rtx in_rtx)
{
   MtcsTarget *mtcsTarget=(MtcsTarget *)target;
   MtcsMode *mtcsMode=mtcsTarget->mtcsMode;
   ...
   int lastVirtualRegister=mtcs_reg_get_last_virtual_regno(mtcsReg);
   int  firstVirtualRegister= mtcs_reg_get_first_virtual_register(mtcsReg);
   ...
   if (regno <= lastVirtualRegister/*!LAST_VIRTUAL_REGISTER*/ && !m_compact)
      fprintf (m_outfile, " %d", regno);
   if (regno < firstPseudoRegister/*!FIRST_PSEUDO_REGISTER*/)
      fprintf (m_outfile, " %s", mtcsReg->hardRegs.x_reg_names/*!reg_names*/[regno]);
   else if (regno <= lastVirtualRegister/*!LAST_VIRTUAL_REGISTER*/){
      if (regno == mtcsReg->normalHardRegsNum.virtual_incoming_args_regnum/*!VIRTUAL_INCOMING_ARGS_REGNUM*/)
         fprintf (m_outfile, " virtual-incoming-args");
      else if (regno == mtcsReg->normalHardRegsNum.virtual_stack_vars_regnum/*!VIRTUAL_STACK_VARS_REGNUM*/)
         fprintf (m_outfile, " virtual-stack-vars");
      else if (regno == mtcsReg->normalHardRegsNum.virtual_stack_dynamic_regnum/*!VIRTUAL_STACK_DYNAMIC_REGNUM*/)
         fprintf (m_outfile, " virtual-stack-dynamic");
      else if (regno == mtcsReg->normalHardRegsNum.virtual_outgoing_args_regnum/*!VIRTUAL_OUTGOING_ARGS_REGNUM*/)
         fprintf (m_outfile, " virtual-outgoing-args");
      else if (regno == mtcsReg->normalHardRegsNum.virtual_cfa_regnum/*!VIRTUAL_CFA_REGNUM*/)
         fprintf (m_outfile, " virtual-cfa");
      else if (regno == mtcsReg->normalHardRegsNum.virtual_preferred_stack_boundary_regnum/*!VIRTUAL_PREFERRED_STACK_BOUNDARY_REGNUM*/)
         fprintf (m_outfile, " virtual-preferred-stack-boundary");
      else
         fprintf (m_outfile, " virtual-reg-%d", regno-firstVirtualRegister/*!FIRST_VIRTUAL_REGISTER*/);
   }else
      if (mtcsOptionsItem->x_flag_dump_unnumbered && is_insn)
         fputc ('#', m_outfile);
      else if (m_compact){
         ...
         gcc_assert (regno > lastVirtualRegister/*!LAST_VIRTUAL_REGISTER*/);
         fprintf (m_outfile, " <%d>", regno - (lastVirtualRegister/*!LAST_VIRTUAL_REGISTER*/ + 1));
      }else
         fprintf (m_outfile, " %d", regno);
   ...
   if (REG_ATTRS (in_rtx)){
      ...
      if (REG_EXPR (in_rtx))
         mtcs_print_mem_expr/*!print_mem_expr*/(m_outfile, REG_EXPR (in_rtx));
...
void mtcs_rtx_writer::print_rtx_operand_code_u (const_rtx in_rtx, int idx)
{
   MtcsTarget *mtcsTarget=(MtcsTarget *)target;
   MtcsMode *mtcsMode=mtcsTarget->mtcsMode;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (GET_CODE (in_rtx) == LABEL_REF){
      if (subc == NOTE  && NOTE_KIND (sub) == NOTE_INSN_DELETED_LABEL){
         if (mtcsOptionsItem->x_flag_dump_unnumbered)
   ...
   if (mtcsOptionsItem->x_flag_dump_unnumbered  || (mtcsOptionsItem->x_flag_dump_unnumbered_links && idx <= 1
   && (INSN_P (in_rtx) || NOTE_P (in_rtx)  || LABEL_P (in_rtx) || BARRIER_P (in_rtx))))
      fputs (" #", m_outfile);
   else
...
void mtcs_rtx_writer::print_rtx_operand (const_rtx in_rtx, int idx)
{
   const char *format_ptr = GET_RTX_FORMAT (GET_CODE (in_rtx));
   fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 00 %p,idx:%d %d %s %s\n",in_rtx,
         idx,format_ptr[idx],GET_RTX_NAME(GET_CODE(in_rtx)),format_ptr);
   ...
   switch (format_ptr[idx]){
      ...
      case '0':
         fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 11 print_rtx_operand_code_0\n");

         print_rtx_operand_code_0 (in_rtx, idx);
         break;

      case 'e':
         fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 22 print_rtx_operand_code_e\n");
      ...
      case 'E':
      case 'V':
         fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 33 print_rtx_operand_codes_E_and_V V\n");

         print_rtx_operand_codes_E_and_V (in_rtx, idx);
         break;

      case 'w':
         fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 44 print_rtx_operand_code_w m_simple:%d m_compact:%d\n",m_simple,m_compact);
         if (! m_simple)
            fprintf (m_outfile, " ");
         fprintf (m_outfile, HOST_WIDE_INT_PRINT_DEC, XWINT (in_rtx, idx));
         if (! m_simple && !m_compact)
            fprintf (m_outfile, " [" HOST_WIDE_INT_PRINT_HEX "]",(unsigned HOST_WIDE_INT) XWINT (in_rtx, idx));
         break;

      case 'i':
         fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 55 print_rtx_operand_code_i\n");
      ...
      case 'r':
         fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 66 print_rtx_operand_code_r\n");
      ...
      case 'u':
         fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 77 print_rtx_operand_code_u\n");
         print_rtx_operand_code_u (in_rtx, idx);
         break;

      case 't':
         fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 88 print_rtx_operand_code_t\n");

         if (idx == 0 && GET_CODE (in_rtx) == DEBUG_IMPLICIT_PTR)
            mtcs_print_mem_expr/*!print_mem_expr*/(m_outfile, DEBUG_IMPLICIT_PTR_DECL (in_rtx));
         else if (idx == 0 && GET_CODE (in_rtx) == DEBUG_PARAMETER_REF)
            mtcs_print_mem_expr/*!print_mem_expr*/(m_outfile, DEBUG_PARAMETER_REF_DECL (in_rtx));
         else
            dump_addr (m_outfile, " ", XTREE (in_rtx, idx));//dump_addr print-tree.h
         break;
      ...
      case 'B':
         fprintf(dumpFile,"mtcsprintrtl.c print_rtx_operand 99 print_rtx_operand_code_b m_compact:%d\n",m_compact);
...
bool mtcs_rtx_writer::operand_has_default_value_p (const_rtx in_rtx, int idx)
{
   const char *format_ptr = GET_RTX_FORMAT (GET_CODE (in_rtx));
...
void mtcs_rtx_writer::print_rtx (const_rtx in_rtx)
{
   MtcsTarget *mtcsTarget=(MtcsTarget *)target;
   MtcsMode *mtcsMode=mtcsTarget->mtcsMode;
   ...
   int idx = 0;
   if(in_rtx)
   fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 00 %p  %d %s %s\n",
         in_rtx,GET_CODE(in_rtx),GET_RTX_NAME(GET_CODE(in_rtx)),GET_RTX_FORMAT (GET_CODE (in_rtx)));
   else
      fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 00 in_rtx是空的\n");
   if (m_sawclose){
      if (m_simple)
         fputc (' ', m_outfile);
      else
   ...
   if (m_compact && is_a <const rtx_insn *, const struct rtx_def> (in_rtx)){
      ...
      fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 22 code == CODE_LABEL:%d :%s\n",code == CODE_LABEL,GET_RTX_NAME (code));
   }else if (m_simple && CONST_INT_P (in_rtx)){
      ; /* no code.  */
      fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 33  no code\n");

   }else{
      rtx_code code = GET_CODE (in_rtx);
      fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 44  code:%d :%s\n",code ,GET_RTX_NAME (code));
      fprintf (m_outfile, "%s", GET_RTX_NAME (GET_CODE (in_rtx)));
   }
   if (! m_simple){
      rtx_code code = GET_CODE (in_rtx);

      fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 55  code:%d :%s\n",code ,GET_RTX_NAME (code));
      ...
      && !m_in_call_function_usage){
         fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 66  :%s--\n",GET_REG_NOTE_NAME (GET_MODE (in_rtx)));

         fprintf (m_outfile, ":%s", GET_REG_NOTE_NAME (GET_MODE (in_rtx)));

      /* For other rtl, print the mode if it's not VOID.  */
      }else if (GET_MODE (in_rtx) != VOIDmode){
         fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 77  :%s--\n",
               mtcs_mode_get_name/*!GET_MODE_NAME*/(mtcsMode,GET_MODE (in_rtx)));

         fprintf (m_outfile, ":%s", mtcs_mode_get_name/*!GET_MODE_NAME*/(mtcsMode,GET_MODE (in_rtx)));
      }
      ...
      if (GET_CODE (in_rtx) == VAR_LOCATION){
         if (TREE_CODE (PAT_VAR_LOCATION_DECL (in_rtx)) == STRING_CST)
            ...
            mtcs_print_mem_expr/*!print_mem_expr*/(m_outfile, PAT_VAR_LOCATION_DECL (in_rtx));
         fputc (' ', m_outfile);
         fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 99  :%s--\n",
               mtcs_mode_get_name/*!GET_MODE_NAME*/(mtcsMode,GET_MODE (in_rtx)));

         print_rtx (PAT_VAR_LOCATION_LOC (in_rtx));
         if (PAT_VAR_LOCATION_STATUS (in_rtx) == VAR_INIT_STATUS_UNINITIALIZED)
   ...
   if (CONST_DOUBLE_AS_FLOAT_P (in_rtx))
      idx = 5;
   fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 100\n");
   ...
   fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 101 %p  %d %s %s limit:%d idx:%d\n",
         in_rtx,GET_CODE(in_rtx),GET_RTX_NAME(GET_CODE(in_rtx)),GET_RTX_FORMAT (GET_CODE (in_rtx)),limit,idx);
   if (m_compact)
      while (limit > idx && operand_has_default_value_p (in_rtx, limit - 1))
         limit--;

   fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 102 %p  %d %s %s limit:%d idx:%d\n",
         in_rtx,GET_CODE(in_rtx),GET_RTX_NAME(GET_CODE(in_rtx)),GET_RTX_FORMAT (GET_CODE (in_rtx)),limit,idx);
   ...
   fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 103 %p  %d %s %s limit:%d idx:%d\n",
         in_rtx,GET_CODE(in_rtx),GET_RTX_NAME(GET_CODE(in_rtx)),GET_RTX_FORMAT (GET_CODE (in_rtx)),limit,idx);

   switch (GET_CODE (in_rtx)){
      case MEM:
         fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 104 mem :[0  S1 A8]\n");
         if (UNLIKELY (final_insns_dump_p))
            fprintf (m_outfile, " [");
         else
            fprintf (m_outfile, " [" HOST_WIDE_INT_PRINT_DEC,(HOST_WIDE_INT) mtcs_rtl_get_mem_alias/*!MEM_ALIAS_SET*/(mtcsRTL,in_rtx));

         if (mtcs_rtl_get_mem_expr/*!MEM_EXPR*/(mtcsRTL,in_rtx))
            mtcs_print_mem_expr/*!print_mem_expr*/(m_outfile, mtcs_rtl_get_mem_expr/*!MEM_EXPR*/(mtcsRTL, in_rtx));
         else
            fputc (' ', m_outfile);

         if (mtcs_rtl_is_mem_offset_known_p/*!MEM_OFFSET_KNOWN_P*/(mtcsRTL,in_rtx)){
            fprintf (m_outfile, "+");
            print_poly_int (m_outfile, mtcs_rtl_get_mem_offset/*!MEM_OFFSET*/(mtcsRTL,in_rtx));
         }

         if (mtcs_rtl_is_mem_size_known_p/*!MEM_SIZE_KNOWN_P*/(mtcsRTL,in_rtx)){
            fprintf (m_outfile, " S");
            print_poly_int (m_outfile, mtcs_rtl_get_mem_size/*!MEM_SIZE*/(mtcsRTL,in_rtx));
         }

         if (mtcs_rtl_get_mem_align/*!MEM_ALIGN*/(mtcsRTL,in_rtx) != 1)
            fprintf (m_outfile, " A%u", mtcs_rtl_get_mem_align/*!MEM_ALIGN*/(mtcsRTL,in_rtx));

         if (!ADDR_SPACE_GENERIC_P (mtcs_rtl_get_mem_addr_space/*!MEM_ADDR_SPACE*/(mtcsRTL,in_rtx)))
            fprintf (m_outfile, " AS%u", mtcs_rtl_get_mem_addr_space/*!MEM_ADDR_SPACE*/(mtcsRTL,in_rtx));

         fputc (']', m_outfile);
         break;

      case CONST_DOUBLE:
         if (mtcs_mode_is_float_p/*!FLOAT_MODE_P*/(mtcsMode,GET_MODE (in_rtx))){
            char s[60];
      ...
      case CONST_POLY_INT:
         fprintf (m_outfile, " [");
         print_dec (CONST_POLY_INT_COEFFS (in_rtx)[0], m_outfile, SIGNED);//print_dec poly-int.h
   ...
   fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtx 105 %p  %d %s %s limit:%d idx:%d\n",
         in_rtx,GET_CODE(in_rtx),GET_RTX_NAME(GET_CODE(in_rtx)),GET_RTX_FORMAT (GET_CODE (in_rtx)),limit,idx);
...
void mtcs_rtx_writer::finish_directive ()
{
  fprintf (m_outfile, ")\n");
  m_sawclose = false;
}
...
void mtcs_print_inline_rtx (FILE *outf, const_rtx x, int ind)
{
  ...
  mtcs_rtx_writer w (outf, ind, false, false, NULL,mtcsTarget);
  w.print_rtx (x);
}

/* Call this function from the debugger to see what X looks like.  */
//原型 debug_rtx rtl.h print-rtl.cc
DEBUG_FUNCTION void mtcs_debug_rtx (const_rtx x)
{
  MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
  mtcs_rtx_writer w (stderr, 0, false, false, NULL,mtcsTarget);
  w.print_rtx (x);
  fprintf (stderr, "\n");
}
...
DEBUG_FUNCTION void mtcs_debug (const rtx_def &ref)
{
   mtcs_debug_rtx/*!debug_rtx*/(&ref);
}

//原型 debug rtl.h print-rtl.cc
DEBUG_FUNCTION void mtcs_debug (const rtx_def *ptr)
{
  if (ptr)
     mtcs_debug/*!debug*/(*ptr);
  else
    fprintf (stderr, "<nil>\n");
}
...
DEBUG_VARIABLE int mtcs_debug_rtx_count = 0;   /* 0 is treated as equivalent to 1 */
...
DEBUG_FUNCTION void mtcs_debug_rtx_list (const rtx_insn *x, int n)
{
   int i,count;
   const rtx_insn *insn;
...
DEBUG_FUNCTION void mtcs_debug_rtx_range (const rtx_insn *start, const rtx_insn *end)
{
   while (1){
      mtcs_debug_rtx/*!debug_rtx*/(start);
      fprintf (stderr, "\n");
      if (!start || start == end)
         break;
...
DEBUG_FUNCTION const rtx_insn * mtcs_debug_rtx_find (const rtx_insn *x, int uid)
{
   while (x != 0 && INSN_UID (x) != uid)
      x = NEXT_INSN (x);
      if (x != 0){
         mtcs_debug_rtx_list/*!debug_rtx_list*/(x, mtcs_debug_rtx_count);
         return x;
      }else{
...
void mtcs_rtx_writer::print_rtl (const_rtx rtx_first)
{
   const rtx_insn *tmp_rtx;

   if (rtx_first == 0){
      fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtl 00 rtx是空的\n");
      fputs (print_rtx_head, m_outfile);
      fputs ("(nil)\n", m_outfile);
   }else{
      fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtl 00 %d %s\n",GET_CODE (rtx_first),GET_RTX_NAME (GET_CODE (rtx_first)));
      ...
      switch (GET_CODE (rtx_first)){
         ...
         case BARRIER:
            ...
            for (tmp_rtx = as_a <const rtx_insn *> (rtx_first);
               tmp_rtx != 0;tmp_rtx = NEXT_INSN (tmp_rtx)){
               fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtl 11 code:%d\n",GET_CODE (rtx_first));
         ...
         default:
            fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtl 22 print_rtx_head:%s\n",print_rtx_head);
            fputs (print_rtx_head, m_outfile);
            print_rtx (rtx_first);
            fprintf(dumpFile,"mtcsprintrtl.c mtcs_rtx_writer::print_rtl XX22 print_rtx_head:%s\n",print_rtx_head);
...
void mtcs_print_rtl (FILE *outf, const_rtx rtx_first)
{
   if(!n_log_is_debug_file(NULL,NULL))
      return;
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   mtcs_rtx_writer w (outf, 0, false, false, NULL,mtcsTarget);
...
void mtcs_rtx_writer::print_rtl_single_with_indent (const_rtx x, int ind)
{
...
void mtcs_print_simple_rtl (FILE *outf, const_rtx x)
{
   if(!n_log_is_debug_file(NULL,NULL))
      return;
  MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
  mtcs_rtx_writer w (outf, 0, true, false, NULL,mtcsTarget);
  w.print_rtl (x);
}
...
static void print_exp (pretty_printer *pp, const_rtx x, int verbose)
{
   ...
   switch (GET_CODE (x)){
      ...
      case UNSPEC_VOLATILE:
      {
         ...
         for (i = 0; i < XVECLEN (x, 0); i++){
            if (i != 0)
               pp_comma (pp);
            mtcs_print_pattern(pp, XVECEXP (x, 0, i), verbose);
         }
         pp_string (pp, "] ");
   ...
   for (i = 0; i < 4; i++){
      ...
      if (op[i]){
         if (fun && i != 0)
            pp_comma (pp);
         mtcs_print_value(pp, op[i], verbose);
      }
   }
...
void mtcs_print_value (pretty_printer *pp, const_rtx x, int verbose)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsMode *mtcsMode=mtcsTarget->mtcsMode;
   MtcsReg *mtcsReg = mtcs_target_get_reg(mtcsTarget);
   ...
   switch (GET_CODE (x)){
      case CONST_INT:
         pp_scalar (pp, HOST_WIDE_INT_PRINT_HEX,(unsigned HOST_WIDE_INT) INTVAL (x));
         break;
      ...
      case CONST_WIDE_INT:
      {
         ...
         for (i = CONST_WIDE_INT_NUNITS (x) - 1; i >= 0; i--){
            pp_string (pp, sep);
            sep = ",";
            sprintf (tmp, HOST_WIDE_INT_PRINT_HEX,(unsigned HOST_WIDE_INT) CONST_WIDE_INT_ELT (x, i));
            pp_string (pp, tmp);
         }
         pp_greater (pp);
      }
      ...
      case CONST_DOUBLE:
         if (mtcs_mode_is_float_p/*!FLOAT_MODE_P*/(mtcsMode,GET_MODE (x))){
            ...
            pp_printf (pp, "<%wx,%wx>", (unsigned HOST_WIDE_INT) CONST_DOUBLE_LOW (x),
      ...
      case STRICT_LOW_PART:
         ...
         mtcs_print_value(pp, XEXP (x, 0), verbose);
         pp_right_paren (pp);
         break;
      case REG:
         if (REGNO (x) < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)){
            if (ISDIGIT (mtcsReg->hardRegs.x_reg_names/*!reg_names*/[REGNO (x)][0]))
               pp_modulo (pp);
            pp_string (pp, mtcsReg->hardRegs.x_reg_names/*!reg_names*/[REGNO (x)]);
         }else
            pp_printf (pp, "r%d", REGNO (x));
         if (verbose)
            pp_printf (pp, ":%s", mtcs_mode_get_name/*!GET_MODE_NAME*/(mtcsMode,GET_MODE (x)));
         break;
      case SUBREG:
         mtcs_print_value(pp, SUBREG_REG (x), verbose);
         pp_printf (pp, "#");
...
void mtcs_print_pattern (pretty_printer *pp, const_rtx x, int verbose)
{
   if (! x){
      pp_string (pp, "(nil)");
      return;
   ...
   switch (GET_CODE (x)){
      case SET:
         mtcs_print_value(pp, SET_DEST (x), verbose);
         pp_equal (pp);
         mtcs_print_value(pp, SET_SRC (x), verbose);
         break;
      case RETURN:
      case SIMPLE_RETURN:
      case EH_RETURN:
      ...
      case VAR_LOCATION:
         pp_string (pp, "loc ");
         mtcs_print_value(pp, PAT_VAR_LOCATION_LOC (x), verbose);
         break;
      case COND_EXEC:
         pp_left_paren (pp);
         if (GET_CODE (COND_EXEC_TEST (x)) == NE
         && XEXP (COND_EXEC_TEST (x), 1) == const0_rtx)
         mtcs_print_value(pp, XEXP (COND_EXEC_TEST (x), 0), verbose);
            else if (GET_CODE (COND_EXEC_TEST (x)) == EQ  && XEXP (COND_EXEC_TEST (x), 1) == const0_rtx){
            pp_exclamation (pp);
            mtcs_print_value(pp, XEXP (COND_EXEC_TEST (x), 0), verbose);
         } else
            mtcs_print_value(pp, COND_EXEC_TEST (x), verbose);
         pp_string (pp, ") ");
         mtcs_print_pattern(pp, COND_EXEC_CODE (x), verbose);
         break;
      case PARALLEL:
      {
         int i;
      ...
      case SEQUENCE:
      {
         ...
         if (INSN_P (seq->element (0))){
            ...
            for (int i = 0; i < seq->len (); i++)
               mtcs_print_insn_with_notes(pp, seq->insn (i));
            pp_printf (pp, "%s      ", save_print_rtx_head);
            print_rtx_head = save_print_rtx_head;
         }else{
            for (int i = 0; i < seq->len (); i++){
               mtcs_print_pattern(pp, seq->element (i), verbose);
               pp_semicolon (pp);
            }
         }
         pp_right_brace (pp);
      ...
      case ADDR_VEC:
         for (int i = 0; i < XVECLEN (x, 0); i++){
            mtcs_print_value(pp, XVECEXP (x, 0, i), verbose);
            pp_semicolon (pp);
         }
         break;
      case ADDR_DIFF_VEC:
         for (int i = 0; i < XVECLEN (x, 1); i++){
            mtcs_print_value(pp, XVECEXP (x, 1, i), verbose);
            pp_semicolon (pp);
         }
         break;
      case TRAP_IF:
         pp_string (pp, "trap_if ");
         mtcs_print_value(pp, TRAP_CONDITION (x), verbose);
         break;
      case UNSPEC:
      case UNSPEC_VOLATILE:
      /* Fallthru -- leave UNSPECs to print_exp.  */
      default:
         mtcs_print_value(pp, x, verbose);
   }
}           /* print_pattern */
...
void mtcs_print_insn (pretty_printer *pp, const rtx_insn *x, int verbose)
{
   if (verbose){
   ...
   switch (GET_CODE (x)){
      case INSN:
         mtcs_print_pattern(pp, PATTERN (x), verbose);
         break;
      ...
      case DEBUG_INSN:
      {
         ...
         if (DECL_P (INSN_VAR_LOCATION_DECL (x))){
            ...
            if (id)
               ...
               sprintf (idbuf, "D#%i",DEBUG_TEMP_UID (INSN_VAR_LOCATION_DECL (x)));
               name = idbuf;
            }else{
               sprintf (idbuf, "D.%i", DECL_UID (INSN_VAR_LOCATION_DECL (x)));
               name = idbuf;
            }
         }
         ...
         if (VAR_LOC_UNKNOWN_P (INSN_VAR_LOCATION_LOC (x)))
            ...
            mtcs_print_pattern(pp, INSN_VAR_LOCATION_LOC (x), verbose);
      }
      break;
      ...
      case CALL_INSN:
         if (GET_CODE (PATTERN (x)) == PARALLEL)
            mtcs_print_pattern(pp, XVECEXP (PATTERN (x), 0, 0), verbose);
         else
            mtcs_print_pattern(pp, PATTERN (x), verbose);
         break;
      case CODE_LABEL:
      ...
      case NOTE:
      {
         ...
         switch (NOTE_KIND (x)){
            ...
            case NOTE_INSN_VAR_LOCATION:
               pp_left_brace (pp);
               mtcs_print_pattern(pp, NOTE_VAR_LOCATION (x), verbose);
               pp_right_brace (pp);
               break;
...
void mtcs_print_insn_with_notes (pretty_printer *pp, const rtx_insn *x)
{
   pp_string (pp, print_rtx_head);
   mtcs_print_insn(pp, x, 1);
   pp_newline (pp);
   if (INSN_P (x) && REG_NOTES (x))
      for (rtx note = REG_NOTES (x); note; note = XEXP (note, 1)){
         ...
         if (GET_CODE (note) == INT_LIST)
            ...
            mtcs_print_pattern(pp, XEXP (note, 0), 1);
         pp_newline (pp);
      }
}
...
void mtcs_dump_value_slim (FILE *f, const_rtx x, int verbose)
{
   pretty_printer rtl_slim_pp;
   rtl_slim_pp.set_output_stream(f);
   mtcs_print_value(&rtl_slim_pp, x, verbose);
   pp_flush (&rtl_slim_pp);
}

/* Emit a slim dump of X (an insn) to the file F, including any register
   note attached to the instruction.  */
void mtcs_dump_insn_slim (FILE *f, const rtx_insn *x)
{
   pretty_printer rtl_slim_pp;
   rtl_slim_pp.set_output_stream(f);
   mtcs_print_insn_with_notes(&rtl_slim_pp, x);
   pp_flush (&rtl_slim_pp);
}
...
void mtcs_dump_rtl_slim (FILE *f, const rtx_insn *first, const rtx_insn *last,
          int count, int flags ATTRIBUTE_UNUSED)
{
   ...
   for (insn = first; (insn != NULL) && (insn != tail) && (count != 0);insn = NEXT_INSN (insn)){
      mtcs_print_insn_with_notes(&rtl_slim_pp, insn);
      if (count > 0)
         count--;
   }
...
void mtcs_rtl_dump_bb_for_graph (pretty_printer *pp, basic_block bb)
{
   rtx_insn *insn;
   bool first = true;
   /* TODO: inter-bb stuff.  */
   FOR_BB_INSNS (bb, insn){
      ...
      first = false;
      mtcs_print_insn_with_notes(pp, insn);
...
const char * mtcs_str_pattern_slim (const_rtx x)
{
   pretty_printer rtl_slim_pp;
   mtcs_print_pattern(&rtl_slim_pp, x, 0);
   return ggc_strdup (pp_formatted_text (&rtl_slim_pp));
}

/* Emit a slim dump of X (an insn) to stderr.  */
extern void mtcs_debug_insn_slim (const rtx_insn *);
DEBUG_FUNCTION void mtcs_debug_insn_slim (const rtx_insn *x)
{
  mtcs_dump_insn_slim(stderr, x);
}

/* Same as above, but using dump_rtl_slim.  */
extern void mtcs_debug_rtl_slim (FILE *, const rtx_insn *, const rtx_insn *, int, int);
DEBUG_FUNCTION void mtcs_debug_rtl_slim (const rtx_insn *first, const rtx_insn *last, int count,
      int flags)
{
  mtcs_dump_rtl_slim(stderr, first, last, count, flags);
}

extern void mtcs_debug_bb_slim (basic_block);
DEBUG_FUNCTION void mtcs_debug_bb_slim (basic_block bb)
{
  debug_bb (bb, TDF_SLIM | TDF_BLOCKS);
}

extern void mtcs_debug_bb_n_slim (int);
DEBUG_FUNCTION void mtcs_debug_bb_n_slim (int n)
{
  basic_block bb = BASIC_BLOCK_FOR_FN (cfun, n);
  mtcs_debug_bb_slim (bb);
}

static void printbb(basic_block block,nboolean mtcs)
{
   rtx_insn *insn;
   int i=0;
   FOR_BB_INSNS (block, insn){
      if (!INSN_P (insn))
         continue;
      n_debug("块中的指令 i:%d block:%p index:%d flags:%d insn:%p\n",
...
/**
 * 打印函数中的块中指令，分为打印主机和MTCS两类
 */
void mtcs_print_func_rtl(struct function *fn,const char *explain)
{
   if(!n_log_is_debug_file(NULL,NULL))
      return;
   nboolean running=mtcs_compile_is_compiling(mtcs_compile_get());
   basic_block bb;
   n_debug("打印函数 %s 块中的RTL。%s\n",IDENTIFIER_POINTER(DECL_NAME(fn->decl)),explain?explain:"");
   FOR_ALL_BB_FN (bb, fn)
      printbb(bb,running);
}
