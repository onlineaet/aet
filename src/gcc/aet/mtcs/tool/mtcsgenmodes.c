...
create_modes (void)
{
#undef  HAVE_EXTRA_MODES
#define HAVE_EXTRA_MODES 1
#undef  EXTRA_MODES_FILE
   if(strcmp(platName,"ptx")==0){
#define  EXTRA_MODES_FILE "aet/mtcs/ptx/nvptx-modes.def"
   }
...
emit_max_int (void)
{
  ...
  printf ("#define %s_BITS_PER_UNIT (%d)\n",upperPlatName, bits_per_unit);
  ...
  if (max_bitsize_mode_any_int == 0)
	...
	if (max < i->bytesize)
      ...
      if (max > mmax)
	mmax = max;
      printf ("#define %s_MAX_BITSIZE_MODE_ANY_INT (%d*BITS_PER_UNIT)\n", upperPlatName,mmax);
    }
  else
    printf ("#define %s_MAX_BITSIZE_MODE_ANY_INT %d\n", upperPlatName,max_bitsize_mode_any_int);
  ...
  if (max_bitsize_mode_any_mode == 0)
	...
	for (i = modes[j]; i; i = i->next)
	  if (mmax < i->bytesize)
	    mmax = i->bytesize;
      printf ("#define %s_MAX_BITSIZE_MODE_ANY_MODE (%d*BITS_PER_UNIT)\n", upperPlatName,mmax);
    }
  else
    printf ("#define %s_MAX_BITSIZE_MODE_ANY_MODE %d\n",upperPlatName,
...
emit_insn_modes_h (void)
{
  ...
  printf ("/* Generated automatically from machmode.def and %s\n",extraFile);


  printf ("\
   by mtcsgenmodes.  */\n\
\n\
#ifndef %s_INSN_MODES_H\n\
#define %s_INSN_MODES_H\n\
\n\
enum %s_machine_mode\n{\n",upperPlatName,upperPlatName,platName);

  for (c = 0; c < MAX_MODE_CLASS; c++)
    for (m = modes[c]; m; m = m->next)
      {
	int count_ = printf ("  %s_%smode,",upperPlatName, m->name);
...
//	printf ("#endif\n");
  ...
  for (c = 0; c < MAX_MODE_CLASS; c++)
      ...
      if (first && first->boolean)
	{
	  ...
	  printf ("  %s_MIN_MODE_BOOL = %s_%smode,\n", upperPlatName,upperPlatName,first->name);
	  ...
	  printf ("  %s_MAX_MODE_BOOL = %s_%smode,\n\n",  upperPlatName,upperPlatName,last_bool->name);
	}

      if (first && last)
	printf ("  %s_MIN_%s = %s_%smode,\n  %s_MAX_%s = %s_%smode,\n\n",
	      upperPlatName,mode_class_names[c],upperPlatName, first->name,
	      upperPlatName,mode_class_names[c],upperPlatName, last->name);
      else
	printf ("  %s_MIN_%s = %s_%smode,\n  %s_MAX_%s = %s_%smode,\n\n",
	      upperPlatName, mode_class_names[c], upperPlatName,void_mode->name,
	      upperPlatName,mode_class_names[c], upperPlatName,void_mode->name);
    }

  printf ("\
  %s_NUM_MACHINE_MODES = %s_MAX_MACHINE_MODE\n\
  ...
  for (c = 0; c < MAX_MODE_CLASS; c++)
    {
      printf ("#define %s_NUM_%s ", upperPlatName,mode_class_names[c]);
      if (modes[c])
	printf ("(%s_MAX_%s - %s_MIN_%s + 1)\n", upperPlatName,mode_class_names[c],upperPlatName,
		mode_class_names[c]);
      else
	printf ("0\n");
    }
  ...
  printf ("#define %s_CONST_MODE_NUNITS%s\n", upperPlatName,adj_nunits ? "" : " const");
  printf ("#define %s_CONST_MODE_PRECISION%s\n", upperPlatName,adj_nunits ? "" : " const");
  printf ("#define %s_CONST_MODE_SIZE%s\n",
        upperPlatName,adj_bytesize || adj_nunits ? "" : " const");
  printf ("#define %s_CONST_MODE_UNIT_SIZE%s\n", upperPlatName,adj_bytesize ? "" : " const");
  printf ("#define %s_CONST_MODE_BASE_ALIGN%s\n", upperPlatName,adj_alignment ? "" : " const");
#if 0 /* disabled for backward compatibility, temporary */
  printf ("#define %s_CONST_REAL_FORMAT_FOR_MODE%s\n", upperPlatName,adj_format ? "" :" const");
#endif
  printf ("#define %s_CONST_MODE_IBIT%s\n", upperPlatName,adj_ibit ? "" : " const");
  printf ("#define %s_CONST_MODE_FBIT%s\n", upperPlatName,adj_fbit ? "" : " const");
  printf ("#define %s_CONST_MODE_MASK%s\n", upperPlatName,adj_nunits ? "" : " const");
  emit_max_int ();
  ...
  printf ("#define %s_NUM_INT_N_ENTS %d\n",upperPlatName, n_int_n_ents);

  printf ("#define %s_NUM_POLY_INT_COEFFS %d\n",upperPlatName, NUM_POLY_INT_COEFFS);
  printf("\n");
  printf("extern const char ** %s_get_modes_name();\n",platName);
  printf("extern const unsigned char * %s_get_mode_class();\n",platName);
  printf("extern poly_uint16 * %s_get_mode_precision();\n",platName);
  printf("extern poly_uint16 * %s_get_mode_size();\n",platName);
  printf("extern poly_uint16 * %s_get_mode_nunits();\n",platName);
  printf("extern const unsigned short * %s_get_mode_next();\n",platName);
  printf("extern const unsigned short * %s_get_mode_wider();\n",platName);
  printf("extern const unsigned short * %s_get_mode_2xwider();\n",platName);
  printf("extern const unsigned short * %s_get_mode_complex();\n",platName);
  printf("extern const unsigned HOST_WIDE_INT * %s_get_mode_mask_array();\n",platName);
  printf("extern const unsigned short * %s_get_mode_inner();\n",platName);
  printf("extern const unsigned char * %s_get_mode_unit_size();\n",platName);
  printf("extern const unsigned short * %s_get_mode_unit_precision();\n",platName);
  printf("extern unsigned short * %s_get_mode_base_align();\n",platName);
  printf("extern const unsigned short * %s_get_class_narrowest_mode();\n",platName);
  printf("extern const struct real_format ** %s_get_real_format_for_mode();\n",platName);
  printf("extern void %s_init_adjust_machine_modes();\n",platName);
  printf("extern unsigned char * %s_get_mode_ibit();\n",platName);
  printf("extern unsigned char * %s_get_mode_fbit();\n",platName);
  printf("extern const int_n_data_t %s_int_n_data[%s_NUM_INT_N_ENTS];\n",platName,upperPlatName);
  printf ("\
\n\
#endif /* %s-insn-modes.h */\n",platName);
}
...
emit_insn_modes_inline_h (void)
{
  ...
  printf ("\
   by mtcsgenmodes.  */\n\
\n\
#ifndef %s_INSN_MODES_INLINE_H\n\
#define %s_INSN_MODES_INLINE_H\n",upperPlatName,upperPlatName);
...
emit_mode_name (void)
{
  ...
  printf ("\nstatic const char *const %s_mode_name[%s_NUM_MACHINE_MODES] =\n{\n",platName,upperPlatName);

  for_all_modes (c, m)
    printf ("  \"%s\",\n", m->name);

  print_closer ();
  printf("\nconst char ** %s_get_modes_name()\n{\n",platName);
  printf("     return %s_mode_name;\n",platName);
  printf("}\n");
}
...
emit_mode_class (void)
{
  ...
  printf ("\nstatic const unsigned char  %s_mode_class[%s_NUM_MACHINE_MODES] =\n{\n",platName,upperPlatName);

  for_all_modes (c, m)
    tagged_printf ("%s", mode_class_names[m->cl], m->name);

  print_closer ();
  printf("\nconst unsigned char * %s_get_mode_class()\n{\n",platName);
  printf("     return %s_mode_class;\n",platName);
  printf("}\n");
}
...
emit_mode_precision (void)
{
 ...
 // print_maybe_const_decl ("%spoly_uint16", "mode_precision",  "NUM_MACHINE_MODES", adj_nunits);
  printf ("\nstatic %spoly_uint16 %s_mode_precision[%s_NUM_MACHINE_MODES] = \n{\n",
        adj_nunits ? "" : "const ",platName,upperPlatName);
  ...
  print_closer ();
  printf("\npoly_uint16 * %s_get_mode_precision()\n{\n",platName);
  printf("     return %s_mode_precision;\n",platName);
  printf("}\n");
}
...
emit_mode_size (void)
{
  ...
  //print_maybe_const_decl ("%spoly_uint16", "mode_size", "NUM_MACHINE_MODES", adj_nunits || adj_bytesize);
  printf ("\nstatic %spoly_uint16 %s_mode_size[%s_NUM_MACHINE_MODES] = \n{\n",
        adj_nunits || adj_bytesize ? "" : "const ",platName,upperPlatName);

  for_all_modes (c, m)
    tagged_printf ("{ %u" ZERO_COEFFS " }", m->bytesize, m->name);

  print_closer ();
  printf("\npoly_uint16 * %s_get_mode_size()\n{\n",platName);
  printf("     return %s_mode_size;\n",platName);
  printf("}\n");
}
...
emit_mode_nunits (void)
{
  ...
  //print_maybe_const_decl ("%spoly_uint16", "mode_nunits", "NUM_MACHINE_MODES", adj_nunits);
  printf ("\nstatic %spoly_uint16 %s_mode_nunits[%s_NUM_MACHINE_MODES] = \n{\n",
        adj_nunits ? "" : "const ",platName,upperPlatName);
  ...
  print_closer ();
  printf("\npoly_uint16 * %s_get_mode_nunits()\n{\n",platName);
  printf("     return %s_mode_nunits;\n",platName);
  printf("}\n");
}
...
emit_mode_wider (void)
{
  ...
  printf ("\nstatic const unsigned short %s_mode_next[%s_NUM_MACHINE_MODES] =\n{\n",platName,upperPlatName);

  for_all_modes (c, m){
     //tagged_printf ("E_%smode", m->wider ? m->wider->name : void_mode->name, m->name);
     int count_ = printf ("  %s_%smode,", upperPlatName,m->wider ? m->wider->name : void_mode->name);
     printf ("%*s/* %s */\n", 27 - count_, "", m->name);
  }

  print_closer ();
  printf("\nconst unsigned short* %s_get_mode_next()\n{\n",platName);
  printf("     return %s_mode_next;\n",platName);
  printf("}\n");

  //print_decl ("unsigned short", "mode_wider", "NUM_MACHINE_MODES");
  printf ("\nstatic const unsigned short %s_mode_wider[%s_NUM_MACHINE_MODES] =\n{\n",platName,upperPlatName);
  ...
  for_all_modes (c, m)
	...
	for (m2 = m->wider; m2 && m2 != void_mode; m2 = m2->wider)
      ...
      if (m2 == void_mode)
	m2 = 0;
      //tagged_printf ("E_%smode", m2 ? m2->name : void_mode->name, m->name);
      int count_ = printf ("  %s_%smode,", upperPlatName,m2  ? m2->name : void_mode->name);
       printf ("%*s/* %s */\n", 27 - count_, "", m->name);
    }

  print_closer ();
  printf("\nconst unsigned short* %s_get_mode_wider()\n{\n",platName);
  printf("     return %s_mode_wider;\n",platName);
  printf("}\n");

  //print_decl ("unsigned short", "mode_2xwider", "NUM_MACHINE_MODES");
  printf ("\nstatic const unsigned short %s_mode_2xwider[%s_NUM_MACHINE_MODES] =\n{\n",platName,upperPlatName);
  ...
  print_closer ();
  printf("\nconst unsigned short* %s_get_mode_2xwider()\n{\n",platName);
  printf("     return %s_mode_2xwider;\n",platName);
  printf("}\n");
}
...
emit_mode_complex (void)
{
  ...
  printf ("\nstatic const unsigned short %s_mode_complex[%s_NUM_MACHINE_MODES] =\n{\n",platName,upperPlatName);

  for_all_modes (c, m){
    //tagged_printf ("E_%smode",m->complex ? m->complex->name : void_mode->name,m->name);
    int count_ = printf ("  %s_%smode,", upperPlatName,m->complex  ? m->complex->name : void_mode->name);
    printf ("%*s/* %s */\n", 27 - count_, "", m->name);
  }

  print_closer ();
  printf("\nconst unsigned short* %s_get_mode_complex()\n{\n",platName);
  printf("     return %s_mode_complex;\n",platName);
  printf("}\n");
}
...
emit_mode_mask (void)
{
  ...
  //print_maybe_const_decl ("%sunsigned HOST_WIDE_INT", "mode_mask_array","NUM_MACHINE_MODES", adj_nunits);
  printf ("\nstatic %sunsigned HOST_WIDE_INT %s_mode_mask_array[%s_NUM_MACHINE_MODES] = \n{\n",
        adj_nunits  ? "" : "const ",platName,upperPlatName);
...
#define MODE_MASK(m)                          \\\n\
  ...
  print_closer ();
  printf("\nconst unsigned HOST_WIDE_INT* %s_get_mode_mask_array()\n{\n",platName);
  printf("     return %s_mode_mask_array;\n",platName);
  printf("}\n");
}
...
emit_mode_inner (void)
{
  ...
  printf ("\nstatic const unsigned short %s_mode_inner[%s_NUM_MACHINE_MODES] =\n{\n",platName,upperPlatName);

  for_all_modes (c, m){
   // tagged_printf ("E_%smode", c != MODE_PARTIAL_INT && m->component ? m->component->name : m->name, m->name);
    int count_ = printf ("  %s_%smode,", upperPlatName,c != MODE_PARTIAL_INT && m->component ? m->component->name : m->name);
    printf ("%*s/* %s */\n", 27 - count_, "", m->name);
  }
  print_closer ();
  printf("\nconst unsigned short* %s_get_mode_inner()\n{\n",platName);
  printf("     return %s_mode_inner;\n",platName);
  printf("}\n");
}
...
emit_mode_unit_size (void)
{
  ...
  //print_maybe_const_decl ("%sunsigned char", "mode_unit_size","NUM_MACHINE_MODES", adj_bytesize);
  printf ("\nstatic %sunsigned char %s_mode_unit_size[%s_NUM_MACHINE_MODES] = \n{\n",
        adj_bytesize  ? "" : "const ",platName,upperPlatName);
  for_all_modes (c, m){
    tagged_printf ("%u", c != MODE_PARTIAL_INT && m->component ? m->component->bytesize : m->bytesize, m->name);
  }
  print_closer ();
  printf("\nconst unsigned char* %s_get_mode_unit_size()\n{\n",platName);
  printf("     return %s_mode_unit_size;\n",platName);
  printf("}\n");
}
...
emit_mode_unit_precision (void)
{
  ...
  printf ("\nstatic const unsigned short %s_mode_unit_precision[%s_NUM_MACHINE_MODES] =\n{\n",platName,upperPlatName);
  ...
  print_closer ();
  printf("\nconst unsigned short* %s_get_mode_unit_precision()\n{\n",platName);
  printf("     return %s_mode_unit_precision;\n",platName);
  printf("}\n");
}
...
emit_mode_base_align (void)
{
  ...
  //print_maybe_const_decl ("%sunsigned short", "mode_base_align", "NUM_MACHINE_MODES", adj_alignment);
  printf ("\nstatic %sunsigned short %s_mode_base_align[%s_NUM_MACHINE_MODES] = \n{\n",
        adj_alignment  ? "" : "const ",platName,upperPlatName);

  for_all_modes (c, m)
    tagged_printf ("%u", m->alignment, m->name);

  print_closer ();
  printf("\nunsigned short* %s_get_mode_base_align()\n{\n",platName);
  printf("     return %s_mode_base_align;\n",platName);
  printf("}\n");
}
...
emit_class_narrowest_mode (void)
{
  ...
  printf ("\nstatic const unsigned short %s_class_narrowest_mode[MAX_MODE_CLASS] =\n{\n",platName);
  ...
  for (c = 0; c < MAX_MODE_CLASS; c++)
	...
	m = m->next;
      ...
      int count_ = printf (" %s_MIN_%s,", upperPlatName,mode_class_names[c]);
      printf ("%*s/* %s */\n", 27 - count_, "", comment_name);
    }

  print_closer ();
  printf("\nconst unsigned short* %s_get_class_narrowest_mode()\n{\n",platName);
  printf("     return %s_class_narrowest_mode;\n",platName);
  printf("}\n");
}
...
//	      "+ MAX_MODE_DECIMAL_FLOAT - MIN_MODE_DECIMAL_FLOAT + 1");
  printf("#ifdef CONFIG_NLIB_HOST\n"
          "static const struct real_format *%s_real_format_for_mode[0];\n",platName);
  printf("#else\n");

  printf ("\nstatic const struct real_format *\n %s_real_format_for_mode["
         "%s_MAX_MODE_FLOAT - %s_MIN_MODE_FLOAT + 1 "
         "+ %s_MAX_MODE_DECIMAL_FLOAT - %s_MIN_MODE_DECIMAL_FLOAT + 1] = \n{\n",
         platName,upperPlatName,upperPlatName,upperPlatName,upperPlatName);
#endif
  ...
  print_closer ();
  printf("#endif\n");
  printf("\nconst struct real_format ** %s_get_real_format_for_mode()\n{\n",platName);
  printf("     return %s_real_format_for_mode;\n",platName);
  printf("}\n");
}
...
emit_mode_adjustments (void)
{
  ...
  if (adj_nunits)
    printf ("\n"
	    ...
	    "{\n"
	    "  unsigned int precision;\n"
	    "  if (%s_mode_precision/*!GET_MODE_PRECISION (mode)*/[mode].is_constant (&precision)\n"
	    "      && precision < HOST_BITS_PER_WIDE_INT)\n"
	    "    %s_mode_mask_array[mode] = (HOST_WIDE_INT_1U << precision) - 1;"
	    "\n"
	    "  else\n"
	    "    %s_mode_mask_array[mode] = HOST_WIDE_INT_M1U;\n"
	    "}\n",platName,platName,platName);

  printf ("\
\nvoid\
\n%s_init_adjust_machine_modes (void)\
\n{\
\n  poly_uint16 ps ATTRIBUTE_UNUSED;\n\
  ...
  for (a = adj_nunits; a; a = a->next)
    {
      ...
      printf ("    int old_factor = vector_element_size"
	      " (%s_mode_precision[%s_%smode], %s_mode_nunits[%s_%smode]);\n",
	      platName,upperPlatName,m->name, platName,upperPlatName,m->name);
      printf ("    %s_mode_precision[%s_%smode] = ps * old_factor;\n", platName,upperPlatName,m->name);
      printf ("    if (!multiple_p (%s_mode_precision[%s_%smode],"
	      " BITS_PER_UNIT, &%s_mode_size[%s_%smode]))\n",  platName,upperPlatName,m->name,  platName,upperPlatName,m->name);
      printf ("      %s_mode_size[%s_%smode] = -1;\n",  platName,upperPlatName,m->name);
      printf ("    %s_mode_nunits[%s_%smode] = ps;\n",  platName,upperPlatName,m->name);
      printf ("    adjust_mode_mask (%s_%smode);\n", upperPlatName,m->name);
      printf ("  }\n");
    }
  ...
  for (a = adj_bytesize; a; a = a->next)
	...
	case MODE_VECTOR_UACCUM:
	  printf ("  ps = %s;\n", a->adjustment);
	  printf ("  s = %s_mode_unit_size[E_%smode];\n",platName, a->mode->name);
	  break;

	default:
	  printf ("  ps = s = %s;\n", a->adjustment);
	  printf ("  %s_mode_unit_size[%s_%smode] = s;\n", platName,upperPlatName,a->mode->name);
	  break;
	}
      printf ("  %s_mode_size[%s_%smode] = ps;\n",platName,upperPlatName, a->mode->name);
      printf ("  %s_mode_base_align[%s_%smode] = known_alignment (ps);\n",platName,upperPlatName,
	      a->mode->name);
      ...
      for (m = a->mode->contained; m; m = m->next_cont)
	{
	  switch (m->cl)
	    ...
	    case MODE_COMPLEX_INT:
	    case MODE_COMPLEX_FLOAT:
	      printf ("  %s_mode_size[%s_%smode] = 2*s;\n", platName,upperPlatName,m->name);
	      printf ("  %s_mode_unit_size[%s_%smode] = s;\n", platName,upperPlatName,m->name);
	      printf ("  %s_mode_base_align[%s_%smode] = s & (~s + 1);\n",platName,upperPlatName,
		      m->name);
	      break;
	    ...
	    case MODE_VECTOR_ACCUM:
	    case MODE_VECTOR_UACCUM:
	      printf ("  %s_mode_size[%s_%smode] = %d * ps;\n",platName,upperPlatName,
		      m->name, m->ncomponents);
	      printf ("  %s_mode_unit_size[%s_%smode] = s;\n", platName,upperPlatName,m->name);
	      printf ("  %s_mode_base_align[%s_%smode]"
  ...
  for (a = adj_alignment; a; a = a->next)
    {
      printf ("\n  /* %s:%d */\n  s = %s;\n",
	      a->file, a->line, a->adjustment);
      printf ("  %s_mode_base_align[%s_%smode] = s;\n", platName,upperPlatName,a->mode->name);
      ...
      for (m = a->mode->contained; m; m = m->next_cont)
	{
	  switch (m->cl)
	    ...
	    case MODE_VECTOR_ACCUM:
	    case MODE_VECTOR_UACCUM:
	      printf ("  %s_mode_base_align[%s_%smode] = %d*s;\n",platName,upperPlatName,
		      m->name, m->ncomponents);
	      break;
  ...
  for (a = adj_fbit; a; a = a->next)
    {
      printf ("\n  /* %s:%d */\n  s = %s;\n",
	      a->file, a->line, a->adjustment);
      printf ("  %s_mode_fbit[%s_%smode] = s;\n", platName,upperPlatName,a->mode->name);
    }

  /* Real mode formats don't have to propagate anywhere.  */
  for (a = adj_format; a; a = a->next)
    printf ("\n  /* %s:%d */\n  REAL_MODE_FORMAT (%s_%smode) = %s;\n",
	    a->file, a->line, upperPlatName, a->mode->name, a->adjustment);
  ...
  for (a = adj_precision; a; a = a->next)
	...
	case MODE_VECTOR_BOOL:
	  printf ("\n  /* %s:%d.  */\n  ps = %s;\n", a->file, a->line,
		  a->adjustment);
	  printf ("  %s_mode_precision[%s_%smode] = ps;\n", platName,upperPlatName,a->mode->name);
	  break;
	default:
  ...
  for_all_modes (c, m)
    printf ("\n  gcc_assert (maybe_ne (%s_mode_size[%s_%smode], -1));\n",platName,upperPlatName,
	    m->name);
...
emit_mode_ibit (void)
{
  ...
  //print_maybe_const_decl ("%sunsigned char", "mode_ibit", "NUM_MACHINE_MODES", adj_ibit);
  printf ("\nstatic %sunsigned char %s_mode_ibit[%s_NUM_MACHINE_MODES] = \n{\n",
        adj_ibit ? "" : "const ",platName,upperPlatName);

  for_all_modes (c, m)
    tagged_printf ("%u", m->ibit, m->name);

  print_closer ();
  printf("\n unsigned char * %s_get_mode_ibit()\n{\n",platName);
  printf("     return %s_mode_ibit;\n",platName);
  printf("}\n");
}
...
emit_mode_fbit (void)
{
  ...
  //print_maybe_const_decl ("%sunsigned char", "mode_fbit", "NUM_MACHINE_MODES", adj_fbit);
  printf ("\nstatic %sunsigned char %s_mode_fbit[%s_NUM_MACHINE_MODES] = \n{\n",
        adj_fbit ? "" : "const ",platName,upperPlatName);

  for_all_modes (c, m)
    tagged_printf ("%u", m->fbit, m->name);

  print_closer ();
  printf("\n unsigned char * %s_get_mode_fbit()\n{\n",platName);
  printf("     return %s_mode_fbit;\n",platName);
  printf("}\n");
}
...
emit_mode_int_n (void)
{
 ...
 // print_decl ("int_n_data_t", "int_n_data", "");
  printf ("\n const int_n_data_t %s_int_n_data[%s_NUM_INT_N_ENTS] =\n{\n",platName,upperPlatName);
  ...
  for (i = 0; i < n_modes; i ++)
    {
      ...
      printf ("{ (machine_mode)%s_%smode },", upperPlatName,m->name);
      printf(" },\n");
    }
...
main (int argc, char **argv)
{
  ...
  if(argc!=4){
     error ("usage: %s extra-def -Wptx [-h|-c] > file", progname);
     return FATAL_EXIT_CODE;
  }
  extraFile = argv[1];
  platName=argv[2]+2; //-Wptx
  upperPlatName=xstrdup(platName);
  char c=platName[0];
  int count=0;
  while(c!='\0'){
      upperPlatName[count++]=TOUPPER(c);
      c=platName[count];
  }
  ...
  fprintf(stderr,"mtcsgenmodes 参数 %s %s %d\n",extraFile,platName,gen_header);
