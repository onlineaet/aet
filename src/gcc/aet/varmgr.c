...
static void defineGlobalVar(VarEntity *item)
{
   ...
   if(aet_utils_valid_tree(item->init_original_type) && TREE_CODE(item->init_original_type)==REAL_TYPE){
      item->init=build_real(item->init_original_type, item->realCst);
   }
...
static void eachDefine(VarMgr *self,NPtrArray *array,char *sysName,NString *codes)
{
   ...
   for(i=0;i<array->len;i++){
      ...
      if(entity->isStatic){
         ...
         if(strcmp(entity->orgiName,AET_IS_MTCS_CLASS_VAR_NAME)==0){
            nboolean isMtcsClass =class_mgr_is_mtcs_class(class_mgr_get(),&info->className);
            //printf("进这里赋值mtcs :%s isMtcs:%d\n",sysName,isMtcsClass);
            entity->init= build_int_cst (char_type_node, isMtcsClass?1:0);
         }
 ...
 * 这是在impl$编译完后做的事
 */
char *var_mgr_define_class_static_var(VarMgr *self,ClassName *implClassName)
{
...
//	c_token *t=c_parser_peek_token (self->parser->parser);
//	aet_print_token(t);
//	if(codes->len>0){
//		//要编译与下内容
//		printf("要编译与下内容 %s\n",codes->str);
...
nboolean   var_mgr_change_static_decl(VarMgr *self,ClassName *className,struct c_declspecs *specs,struct c_declarator *declarator)
{
   ...
   nboolean exits=existsVar(self,varName,className);
   if(exits){
      error_at (id_loc,"%qs 有同名的变量名 ！", varName);
      n_free(varName);
      return FALSE;
   }
		...
		struct c_expr *initExpr,ClassPermissionType permi,nboolean isFinal)
{
   ...
   n_debug("var_mgr_set_static_decl 11 静态变量：%s 所在的类:%s item:%p\n",mangleName,className->sysName,item);
	...
	//解决BUG @1      ggc_freed        addr: 7f253959b930
	if(initExpr && aet_utils_valid_tree(initExpr->value)){
	   if(TREE_CODE(initExpr->value)==REAL_CST){
        item->realCst =TREE_REAL_CST (initExpr->value);
        item->init_original_type=TREE_TYPE(initExpr->value);
	   }else{
 ...
 * component变量的原始名字
 */
tree var_mgr_get_static_const_init(VarMgr *self,ClassName *className,tree component)
{
   VarEntity *item=var_mgr_get_static_entity(self,className,component);
   if(item==NULL)
      return NULL;
   return var_entity_get_const_init(item);
}
...
VarEntity *var_mgr_get_entity_by_mangle(VarMgr *self,char *sysName,char *mangle)
{
   NPtrArray *array=(NPtrArray *)n_hash_table_lookup(self->mgrHash,sysName);
   if(array==NULL)
       return NULL;
   ...
   for(i=0;i<len;i++){
     VarEntity *item=n_ptr_array_index(array,i);
     if(item->mangleVarName && strcmp(mangle,item->mangleVarName)==0)
         return item;
   }
   return NULL;
}
 ...
 * 返回是常量的初始值
 */
tree var_entity_get_const_init(VarEntity *self)
{
     tree value = self->init;
     if(aet_utils_valid_tree(self->init_original_type) && TREE_CODE(self->init_original_type)==REAL_TYPE){
        value = build_real(self->init_original_type, self->realCst);
        n_debug("var_mgr_get_const 11 对小数点的处理 item->isConst:%d\n",self->isConst);
     }
     n_debug("判断静态变量的值是不是常数----\n");
