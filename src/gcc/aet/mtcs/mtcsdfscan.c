...
static void mtcsDfscanInit(MtcsDfscan *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   self->elim_reg_set.count=mtcs_reg_get_hard_reg_element_count(mtcsReg);
...
static void df_ref_record (MtcsDfscan *self,enum df_ref_class, class df_collection_rec *,
            rtx, rtx *,
            basic_block, struct df_insn_info *,
            enum df_ref_type, int ref_flags);
static void df_def_record_1 (MtcsDfscan *self,class df_collection_rec *, rtx *,
              basic_block, struct df_insn_info *,
              int ref_flags);
static void df_defs_record (MtcsDfscan *self,class df_collection_rec *, rtx,
             basic_block, struct df_insn_info *,
             int ref_flags);
static void df_uses_record (MtcsDfscan *self,class df_collection_rec *,
             rtx *, enum df_ref_type,
             basic_block, struct df_insn_info *,
             int ref_flags);

static void df_install_ref_incremental (MtcsDfscan *self,df_ref);
static void df_insn_refs_collect (MtcsDfscan *self,class df_collection_rec*, basic_block, struct df_insn_info *);
static void df_canonize_collection_rec (class df_collection_rec *);//不移植

static void df_get_regular_block_artificial_uses (MtcsDfscan *self,bitmap);
static void df_get_eh_block_artificial_uses (MtcsDfscan *self,bitmap);

static void df_record_entry_block_defs (MtcsDfscan *self,bitmap);
static void df_record_exit_block_uses (MtcsDfscan *self,bitmap);
static void df_get_entry_block_def_set (MtcsDfscan *self,bitmap);
static void df_grow_ref_info (struct df_ref_info *, unsigned int);//不移植
static void df_ref_chain_delete_du_chain (MtcsDfscan *self,df_ref);
static void df_ref_chain_delete (MtcsDfscan *self,df_ref);

static void df_refs_add_to_chains (MtcsDfscan *self,class df_collection_rec *,
               basic_block, rtx_insn *, unsigned int);

static bool df_insn_refs_verify (MtcsDfscan *self,class df_collection_rec *, basic_block,rtx_insn *, bool);
static void df_entry_block_defs_collect (MtcsDfscan *self,class df_collection_rec *, bitmap);
static void df_exit_block_uses_collect (MtcsDfscan *self,class df_collection_rec *, bitmap);
static void df_install_ref (MtcsDfscan *self,df_ref, struct df_reg_info *,struct df_ref_info *, bool);

static int df_ref_compare (df_ref, df_ref);//不移植
static int df_ref_ptr_compare (const void *, const void *);//不移植
static int df_mw_compare (const df_mw_hardreg *, const df_mw_hardreg *);//不移植
static int df_mw_ptr_compare (const void *, const void *);//不移植
static void df_insn_info_delete (MtcsDfscan *self,unsigned int);
...
static const unsigned int copy_all = copy_defs | copy_uses | copy_eq_uses| copy_mw;
...
{
  struct df_scan_problem_data *problem_data  = (struct df_scan_problem_data *) df_scan->problem_data;
  n_debug("mtcsdfscan.c df_scan_free_internal 00 df:%p df_scan:%p df->regs_inited:%d\n",df,df_scan,df->regs_inited);

  free (df->def_info.refs);
  free (df->def_info.begin);
  free (df->def_info.count);
  memset (&df->def_info, 0, (sizeof (struct df_ref_info)));
  n_debug("mtcsdfscan.c df_scan_free_internal 11 df:%p\n",df,df_scan,df->regs_inited);

  free (df->use_info.refs);
  free (df->use_info.begin);
  free (df->use_info.count);
  memset (&df->use_info, 0, (sizeof (struct df_ref_info)));
  n_debug("mtcsdfscan.c df_scan_free_internal 22 df:%p\n",df,df_scan,df->regs_inited);
  ...
  df->regs_size = 0;
  DF_REG_SIZE (df) = 0;
  n_debug("mtcsdfscan.c df_scan_free_internal 33 df:%p\n",df,df_scan,df->regs_inited);
  ...
  n_debug("mtcsdfscan.c df_scan_free_internal 44 df:%p\n",df,df_scan,df->regs_inited);
  ...
  n_debug("mtcsdfscan.c df_scan_free_internal 55 df:%p\n",df,df_scan,df->regs_inited);
  ...
  n_debug("mtcsdfscan.c df_scan_free_internal 66 df:%p\n",df,df_scan,df->regs_inited);
...
static void df_scan_free_bb_info (MtcsDfscan *self,basic_block bb, void *vbb_info)
{
  ...
  FOR_BB_INSNS (bb, insn)
    if (INSN_P (insn))
      df_insn_info_delete(self,INSN_UID (insn));

  if (bb_index < df_scan->block_info_size)
    bb_info = df_scan_get_bb_info (bb_index);

  /* Get rid of any artificial uses or defs.  */
  df_ref_chain_delete_du_chain(self,bb_info->artificial_defs);
  df_ref_chain_delete_du_chain(self,bb_info->artificial_uses);
  df_ref_chain_delete(self,bb_info->artificial_defs);
  df_ref_chain_delete(self,bb_info->artificial_uses);
...
{
   n_debug("mtcsdfscan.c df_scan_free---\n");
   ...
   n_debug("mtcsdfscan.c df_scan_free---11\n");

   free (df_scan);
}

/* Dump the preamble for DF_SCAN dump. */
static void df_scan_start_dump (MtcsDfscan *self,FILE *file ATTRIBUTE_UNUSED)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,
         file, mtcs_bitmap_view<HardRegSet/*!HARD_REG_SET*/> (abi/*!eh_edge_abi.*/->full_reg_clobbers ()));
   fprintf (file, ";;  hardware regs used \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &df->hardware_regs_used);
   fprintf (file, ";;  regular block artificial uses \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &df->regular_block_artificial_uses);
   fprintf (file, ";;  eh block artificial uses \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &df->eh_block_artificial_uses);
   fprintf (file, ";;  entry block defs \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, df->entry_block_defs);
   fprintf (file, ";;  exit block uses \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, df->exit_block_uses);
   fprintf (file, ";;  regs ever live \t");
   for (i = 0; i < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); i++)
      if (mtcs_dfscan_df_regs_ever_live_p/*!df_regs_ever_live_p*/(self,i))
         fprintf (file, " %d [%s]", i, mtcsReg->hardRegs.x_reg_names/*!reg_names*/[i]);
   fprintf (file, "\n;;  ref usage \t");
...
static void df_scan_start_block (basic_block bb, FILE *file)
{
   ...
   if (bb_info){
      ...
      mtcs_dfcore_df_refs_chain_dump/*!df_refs_chain_dump*/(mtcsDfcore,bb_info->artificial_defs, true, file);
      fprintf (file, "\n;; bb %d artificial_uses: ", bb->index);
      mtcs_dfcore_df_refs_chain_dump/*!df_refs_chain_dump*/(mtcsDfcore,bb_info->artificial_uses, true, file);
      fprintf (file, "\n");
   }
#if 0
   {
      ...
      FOR_BB_INSNS (bb, insn)
      if (INSN_P (insn))
         mtcs_dfcore_df_insn_debug/*!df_insn_debug*/(mtcsDfcore,insn, false, file);
   }
#endif
}

//原型 df_scan_alloc df.h df-scan.cc
static void df_scan_alloc_cb (bitmap all_blocks ATTRIBUTE_UNUSED)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfscan *mtcsDfscan=mtcs_target_get_dfscan(mtcsTarget);
   mtcs_dfscan_df_scan_alloc(mtcsDfscan,all_blocks);
}

static void df_scan_start_dump_cb (FILE *file ATTRIBUTE_UNUSED)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfscan *mtcsDfscan=mtcs_target_get_dfscan(mtcsTarget);
   df_scan_start_dump(mtcsDfscan,file);
}

static void df_scan_free_bb_info_cb(basic_block bb, void *vbb_info)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfscan *mtcsDfscan=mtcs_target_get_dfscan(mtcsTarget);
   df_scan_free_bb_info(mtcsDfscan,bb,vbb_info);
}
...
static const struct df_problem problem_SCAN =
{
  DF_SCAN,                    /* Problem id.  */
  DF_NONE,                    /* Direction.  */
  df_scan_alloc_cb,           /* Allocate the problem specific data.  */
  NULL,                       /* Reset global information.  */
  df_scan_free_bb_info_cb,    /* Free basic block info.  */
  ...
  df_scan_start_dump_cb,       /* Debugging.  */
...
void mtcs_dfscan_df_scan_add_problem (MtcsDfscan *self)
{
  ...
  mtcs_dfcore_df_add_problem/*!df_add_problem*/(mtcsDfcore,&problem_SCAN);
}

/* Grow the ref information.  */
static void df_grow_ref_info (struct df_ref_info *ref_info, unsigned int new_size)
{
   if (ref_info->refs_size < new_size){
      ref_info->refs = XRESIZEVEC (df_ref, ref_info->refs, new_size);
      memset (ref_info->refs + ref_info->refs_size, 0,(new_size - ref_info->refs_size) *sizeof (df_ref));
...
void mtcs_dfscan_df_scan_blocks (MtcsDfscan *self)
{
   ...
   basic_block bb;
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 00\n");
   df->def_info.ref_order = DF_REF_ORDER_NO_TABLE;
   df->use_info.ref_order = DF_REF_ORDER_NO_TABLE;

   df_get_regular_block_artificial_uses(self,&df->regular_block_artificial_uses);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 11\n");

   df_get_eh_block_artificial_uses(self,&df->eh_block_artificial_uses);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 22\n");

   bitmap_ior_into (&df->eh_block_artificial_uses, &df->regular_block_artificial_uses);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 33\n");

   /* ENTRY and EXIT blocks have special defs/uses.  */
   df_get_entry_block_def_set(self,df->entry_block_defs);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 44\n");

   df_record_entry_block_defs(self,df->entry_block_defs);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 55\n");

   mtcs_dfscan_df_get_exit_block_use_set/*!df_get_exit_block_use_set*/(self,df->exit_block_uses);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 66\n");

   df_record_exit_block_uses(self,df->exit_block_uses);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 77\n");

   mtcs_dfcore_df_set_bb_dirty/*!df_set_bb_dirty*/(mtcsDfcore,BASIC_BLOCK_FOR_FN (cfun, ENTRY_BLOCK));
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 88\n");

   mtcs_dfcore_df_set_bb_dirty/*!df_set_bb_dirty*/(mtcsDfcore,BASIC_BLOCK_FOR_FN (cfun, EXIT_BLOCK));
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 99\n");

   /* Regular blocks */
   FOR_EACH_BB_FN (bb, cfun){
      unsigned int bb_index = bb->index;
      n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_blocks 100 %p %p\n",mtcsMode,mtcsTarget);

      mtcs_dfscan_df_bb_refs_record/*!df_bb_refs_record*/(self,bb_index, true);
   }
}
...
void mtcs_dfscan_df_uses_create (MtcsDfscan *self,rtx *loc, rtx_insn *insn, int ref_flags)
{
  ...
  df_uses_record(self,NULL, loc, DF_REF_REG_USE, BLOCK_FOR_INSN (insn),DF_INSN_INFO_GET (insn),ref_flags);
}

static void df_install_ref_incremental (MtcsDfscan *self,df_ref ref)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   ...
   df_install_ref(self,ref, reg_info[DF_REF_REGNO (ref)], ref_info, add_to_table);
   ...
   *ref_ptr = ref;
   ...
   if (dump_file){
      ...
      mtcs_dfcore_df_ref_debug/*!df_ref_debug*/(mtcsDfcore,ref, dump_file);
   }
#endif
...
static void df_free_ref (df_ref ref)
{
   ...
   switch (DF_REF_CLASS (ref)){
      ...
      case DF_REF_ARTIFICIAL:
         problem_data->ref_artificial_pool->remove((df_artificial_ref *) (ref));
         break;

      case DF_REF_REGULAR:
         problem_data->ref_regular_pool->remove((df_regular_ref *) (ref));
         break;
   }
}
...
static void df_reg_chain_unlink (MtcsDfscan *self,df_ref ref)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (df_chain && DF_REF_CHAIN (ref))
      mtcs_dfproblems_df_chain_unlink/*!df_chain_unlink*/(mtcsDfproblems,ref);

   reg_info->n_refs--;
   if (DF_REF_FLAGS_IS_SET (ref, DF_HARD_REG_LIVE)){
      gcc_assert (DF_REF_REGNO (ref) < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg));
   ...
   if (prev)
      ...
      n_debug("mtcsdfscan.c df_reg_chain_unlink 00  reg_info:%p %d this_ref:%p\n",reg_info,next);
...
struct df_insn_info * df_insn_create_insn_record (MtcsDfscan *self,rtx_insn *insn)
{
   struct df_scan_problem_data *problem_data= (struct df_scan_problem_data *) df_scan->problem_data;
   struct df_insn_info *insn_rec;

   mtcs_dfscan_df_grow_insn_info/*!df_grow_insn_info*/(self);
...
static void df_ref_chain_delete_du_chain (MtcsDfscan *self,df_ref ref)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsDfproblems *mtcsDfproblems=mtcs_target_get_dfproblems(mtcsTarget);
   ...
   for (; ref; ref = DF_REF_NEXT_LOC (ref))
      ...
      if (DF_REF_CHAIN (ref))
         mtcs_dfproblems_df_chain_unlink/*!df_chain_unlink*/(mtcsDfproblems,ref); //df-problems.cc define
}

/* Delete all refs in the ref chain.  */
static void df_ref_chain_delete (MtcsDfscan *self,df_ref ref)
{
   df_ref next;
   for (; ref; ref = next){
      next = DF_REF_NEXT_LOC (ref);
      df_reg_chain_unlink(self,ref);
   }
}
...
static void df_insn_info_free_fields (MtcsDfscan *self,df_insn_info *insn_info)
{
  ...
  if (df_chain){
      df_ref_chain_delete_du_chain(self,insn_info->defs);
      df_ref_chain_delete_du_chain(self,insn_info->uses);
      df_ref_chain_delete_du_chain(self,insn_info->eq_uses);
  }

  df_ref_chain_delete(self,insn_info->defs);
  df_ref_chain_delete(self,insn_info->uses);
  df_ref_chain_delete(self,insn_info->eq_uses);
}
...
static void df_insn_info_delete (MtcsDfscan *self,unsigned int uid)
{
   ...
   if (insn_info){
      struct df_scan_problem_data *problem_data = (struct df_scan_problem_data *) df_scan->problem_data;
      df_insn_info_free_fields(self,insn_info);
...
void mtcs_dfscan_df_insn_delete (MtcsDfscan *self,rtx_insn *insn)
{
   ...
   mtcs_dfcore_df_grow_bb_info/*!df_grow_bb_info*/(mtcsDfcore,df_scan);
   mtcs_dfsan_df_grow_reg_info/*!df_grow_reg_info*/(self);
...
bool mtcs_dfscan_df_insn_rescan (MtcsDfscan *self,rtx_insn *insn)
{
   ...
   if (df->changeable_flags & DF_NO_INSN_RESCAN)
      return false;
   n_debug("mtcsdfscan.c mtcs_dfscan_df_insn_rescan 00 %p\n",df?df->def_regs[0]->reg_chain:NULL);
   mtcs_dfcore_df_grow_bb_info/*!df_grow_bb_info*/(mtcsDfcore,df_scan);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_insn_rescan 00ttxx %p\n",df?df->def_regs[0]->reg_chain:NULL);

   mtcs_dfsan_df_grow_reg_info/*!df_grow_reg_info*/(self);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_insn_rescan 00xx %p\n",df?df->def_regs[0]->reg_chain:NULL);
   ...
   if (df->changeable_flags & DF_DEFER_INSN_RESCAN){
      if (!insn_info){
         n_debug("mtcsdfscan.c mtcs_dfscan_df_insn_rescan 11 %p\n",df?df->def_regs[0]->reg_chain:NULL);

         insn_info = df_insn_create_insn_record(self,insn);
         insn_info->defs = 0;
         insn_info->uses = 0;
         insn_info->eq_uses = 0;
   ...
   if (insn_info){
      int luid;
      n_debug("mtcsdfscan.c mtcs_dfscan_df_insn_rescan 11xz:%p\n",df?df->def_regs[0]->reg_chain:NULL);

      bool the_same = df_insn_refs_verify(self,&collection_rec, bb, insn, false);
      n_debug("mtcsdfscan.c mtcs_dfscan_df_insn_rescan 22 bb:%p the_same:%d\n",bb,the_same);
      ...
      n_debug("mtcsdfscan.c mtcs_dfscan_df_insn_rescan 33 bb:%p\n",bb);

      struct df_insn_info *insn_info = df_insn_create_insn_record(self,insn);
      df_insn_refs_collect(self,&collection_rec, bb, insn_info);
      if (dump_file)
         fprintf (dump_file, "scanning new insn with uid = %d.\n", uid);
   }
   n_debug("mtcsdfscan.c mtcs_dfscan_df_insn_rescan 44 bb:%p %p\n",bb,df?df->def_regs[0]->reg_chain:NULL);

   df_refs_add_to_chains(self,&collection_rec, bb, insn, copy_all);
   if (!DEBUG_INSN_P (insn))
      mtcs_dfcore_df_set_bb_dirty/*!df_set_bb_dirty*/(mtcsDfcore,bb);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_insn_rescan 55 bb:%p\n",bb);
...
bool mtcs_dfscan_df_insn_rescan_debug_internal (MtcsDfscan *self,rtx_insn *insn)
{
   ...
   gcc_assert (DEBUG_INSN_P (insn)  && VAR_LOC_UNKNOWN_P (INSN_VAR_LOCATION_LOC (insn)));
   if (!df)
      return false;
...
void mtcs_dfscan_df_insn_rescan_all (MtcsDfscan *self)
{
   ...
   if (df->changeable_flags & DF_DEFER_INSN_RESCAN){
      mtcs_dfcore_df_clear_flags/*!df_clear_flags*/(mtcsDfcore,DF_DEFER_INSN_RESCAN);
      defer_insn_rescan = true;
   }

   auto_bitmap tmp (&mtcsDfcore->df_bitmap_obstack);
   ...
   FOR_EACH_BB_FN (bb, cfun){
      rtx_insn *insn;
      FOR_BB_INSNS (bb, insn){
         mtcs_dfscan_df_insn_rescan/*!df_insn_rescan*/(self,insn);
      }
   }
...
void mtcs_dfscan_df_process_deferred_rescans (MtcsDfscan *self)
{
   ...
   if (df->changeable_flags & DF_NO_INSN_RESCAN){
      n_debug("mtcsdfscan.c df_process_deferred_rescans 00\n");
      mtcs_dfcore_df_clear_flags/*!df_clear_flags*/(mtcsDfcore,DF_NO_INSN_RESCAN);
      no_insn_rescan = true;
   }

   if (df->changeable_flags & DF_DEFER_INSN_RESCAN){
      n_debug("mtcsdfscan.c df_process_deferred_rescans 11\n");
      mtcs_dfcore_df_clear_flags/*!df_clear_flags*/(mtcsDfcore,DF_DEFER_INSN_RESCAN);
      defer_insn_rescan = true;
   }
   ...
   EXECUTE_IF_SET_IN_BITMAP (tmp, 0, uid, bi){
      struct df_insn_info *insn_info = DF_INSN_UID_SAFE_GET (uid);
      if (insn_info){
         n_debug("mtcsdfscan.c df_process_deferred_rescans 22 uid:%d\n",uid);
         mtcs_print_rtl_single(stderr,insn_info->insn);
         df_insn_info_delete(self,uid);
      }
   }
   ...
   EXECUTE_IF_SET_IN_BITMAP (tmp, 0, uid, bi){
      struct df_insn_info *insn_info = DF_INSN_UID_SAFE_GET (uid);
      if (insn_info){
         n_debug("mtcsdfscan.c df_process_deferred_rescans 33\n");
         mtcs_dfscan_df_insn_rescan/*!df_insn_rescan*/(self,insn_info->insn);
         n_debug("mtcsdfscan.c df_process_deferred_rescans 33xx\n");
   ...
   EXECUTE_IF_SET_IN_BITMAP (tmp, 0, uid, bi){
      struct df_insn_info *insn_info = DF_INSN_UID_SAFE_GET (uid);
      if (insn_info){
         n_debug("mtcsdfscan.c df_process_deferred_rescans 44\n");
         mtcs_dfscan_df_notes_rescan/*!df_notes_rescan*/(self,insn_info->insn);
      }
   }
   ...
   bitmap_clear (&df->insns_to_rescan);
   bitmap_clear (&df->insns_to_notes_rescan);
   n_debug("mtcsdfscan.c df_process_deferred_rescans 55 no_insn_rescan:%d defer_insn_rescan:%d df->redo_entry_and_exit:%d\n",
         no_insn_rescan,defer_insn_rescan,df->redo_entry_and_exit);

   if (no_insn_rescan)
      mtcs_dfcore_df_set_flags/*!df_set_flags*/(mtcsDfcore,DF_NO_INSN_RESCAN);
   if (defer_insn_rescan)
      mtcs_dfcore_df_set_flags/*!df_set_flags*/(mtcsDfcore,DF_DEFER_INSN_RESCAN);
   n_debug("mtcsdfscan.c df_process_deferred_rescans 66\n");

   /* If someone changed regs_ever_live during this pass, fix up the
   entry and exit blocks.  */
   if (df->redo_entry_and_exit){
      n_debug("mtcsdfscan.c df_process_deferred_rescans 66aa mtcs_dfscan_df_update_entry_exit_and_calls\n");

      mtcs_dfscan_df_update_entry_exit_and_calls/*!df_update_entry_exit_and_calls*/(self);
      df->redo_entry_and_exit = false;
   }
   n_debug("mtcsdfscan.c df_process_deferred_rescans 77\n");
...
static void df_reorganize_refs_by_reg_by_reg (MtcsDfscan *self,struct df_ref_info *ref_info,
              bool include_defs,
              bool include_uses,
              bool include_eq_uses)
{
   ...
   if (df->changeable_flags & DF_NO_HARD_REGS){
      start = mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg);
      memset (ref_info->begin, 0, sizeof (int) * mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg));
      memset (ref_info->count, 0, sizeof (int) * mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg));
   }else
      start = 0;
...
/* Take build ref table for either the uses or defs from the reg-use
   ...
   function.  */
static void df_reorganize_refs_by_reg_by_insn (MtcsDfscan *self,struct df_ref_info *ref_info,
               bool include_defs,
               bool include_uses,
               bool include_eq_uses)
{
   ...
   unsigned int offset = 0;
   unsigned int r;
   unsigned int start = (df->changeable_flags & DF_NO_HARD_REGS)
         ? mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg) : 0;
   ...
   EXECUTE_IF_SET_IN_BITMAP (df->blocks_to_analyze, 0, bb_index, bi){
      ...
      if (include_defs)
         FOR_EACH_ARTIFICIAL_DEF (def, bb_index){
            unsigned int regno = DF_REF_REGNO (def);
            if (regno >= start){
               unsigned int id = ref_info->begin[regno] + ref_info->count[regno]++;
               DF_REF_ID (def) = id;
...
/* Take build ref table for either the uses or defs from the reg-use
   or reg-def chains.  */
static void df_reorganize_refs_by_reg (MtcsDfscan *self,struct df_ref_info *ref_info,
            bool include_defs,
            bool include_uses,
            bool include_eq_uses)
{
   if (df->analyze_subset)
      df_reorganize_refs_by_reg_by_insn(self,ref_info, include_defs,include_uses, include_eq_uses);
   else
      df_reorganize_refs_by_reg_by_reg(self,ref_info, include_defs,include_uses, include_eq_uses);
}


/* Add the refs in REF_VEC to the table in REF_INFO starting at OFFSET.  */
static unsigned int df_add_refs_to_table (MtcsDfscan *self,unsigned int offset,
            struct df_ref_info *ref_info, df_ref ref)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   for (; ref; ref = DF_REF_NEXT_LOC (ref))
      if (!(df->changeable_flags & DF_NO_HARD_REGS)
      || (DF_REF_REGNO (ref) >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))){
         ref_info->refs[offset] = ref;
         DF_REF_ID (ref) = offset++;
      }
   return offset;
...
static unsigned int df_reorganize_refs_by_insn_bb (MtcsDfscan *self,basic_block bb, unsigned int offset,
                struct df_ref_info *ref_info,
                ...
                bool include_eq_uses)
{
   ...
   if (include_defs)
      offset = df_add_refs_to_table(self,offset, ref_info,df_get_artificial_defs (bb->index));
   if (include_uses)
      offset = df_add_refs_to_table(self,offset, ref_info,df_get_artificial_uses (bb->index));

   FOR_BB_INSNS (bb, insn)
      if (INSN_P (insn)){
         unsigned int uid = INSN_UID (insn);
         if (include_defs)
            offset = df_add_refs_to_table(self,offset, ref_info, DF_INSN_UID_DEFS (uid));
         if (include_uses)
            offset = df_add_refs_to_table(self,offset, ref_info, DF_INSN_UID_USES (uid));
         if (include_eq_uses)
            offset = df_add_refs_to_table(self,offset, ref_info, DF_INSN_UID_EQ_USES (uid));
      }
   return offset;
}
...
static void df_reorganize_refs_by_insn (MtcsDfscan *self,struct df_ref_info *ref_info,
             bool include_defs, bool include_uses, bool include_eq_uses)
{
   basic_block bb;
   unsigned int offset = 0;
   ...
   if (df->blocks_to_analyze){
      ...
      EXECUTE_IF_SET_IN_BITMAP (df->blocks_to_analyze, 0, index, bi){
         offset = df_reorganize_refs_by_insn_bb(self,BASIC_BLOCK_FOR_FN (cfun,index),
               offset, ref_info,include_defs, include_uses,include_eq_uses);
      }

      ref_info->table_size = offset;
   }else{
      FOR_ALL_BB_FN (bb, cfun)
            offset = df_reorganize_refs_by_insn_bb(self,bb, offset, ref_info,include_defs, include_uses,include_eq_uses);
      ref_info->table_size = offset;
   }
}
...
{
   enum df_ref_order order=(enum df_ref_order)dforder;
   if (order == df->use_info.ref_order)
      return;

   switch (order){
      case DF_REF_ORDER_BY_REG:
         df_reorganize_refs_by_reg(self,&df->use_info, false, true, false);
         break;

      case DF_REF_ORDER_BY_REG_WITH_NOTES:
         df_reorganize_refs_by_reg(self,&df->use_info, false, true, true);
         break;

      case DF_REF_ORDER_BY_INSN:
         df_reorganize_refs_by_insn(self,&df->use_info, false, true, false);
         break;

      case DF_REF_ORDER_BY_INSN_WITH_NOTES:
         df_reorganize_refs_by_insn(self,&df->use_info, false, true, true);
         break;
...
void mtcs_dfscan_df_maybe_reorganize_def_refs (MtcsDfscan *self,int dforder/*!enum df_ref_order order*/)
{
   ...
   switch (order){
      case DF_REF_ORDER_BY_REG:
         df_reorganize_refs_by_reg(self,&df->def_info, true, false, false);
         break;

      case DF_REF_ORDER_BY_INSN:
         df_reorganize_refs_by_insn(self,&df->def_info, true, false, false);
         break;
...
void mtcs_dfscan_df_insn_change_bb (MtcsDfscan *self,rtx_insn *insn, basic_block new_bb)
{
   ...
   if (!DEBUG_INSN_P (insn))
      mtcs_dfcore_df_set_bb_dirty/*!df_set_bb_dirty*/(mtcsDfcore,new_bb);
   if (old_bb){
      if (dump_file)
         fprintf (dump_file, "  from %d to %d\n",
      old_bb->index, new_bb->index);
      if (!DEBUG_INSN_P (insn))
         mtcs_dfcore_df_set_bb_dirty/*!df_set_bb_dirty*/(mtcsDfcore,old_bb);
   }else
      if (dump_file)
...
static void df_ref_change_reg_with_loc_1 (MtcsDfscan *self,struct df_reg_info *old_df,
               struct df_reg_info *new_df, unsigned int new_regno, rtx loc)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   ...
   while (the_ref){
      ...
      && (*DF_REF_LOC (the_ref) == loc)){
         ...
         DF_REF_REG (the_ref) = mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[new_regno];
         ...
         n_debug("mtcsdfscan.c df_ref_change_reg_with_loc_1 00 old_df:%p %d\n",old_df,old_df->n_refs);
         old_df->n_refs--;
         ...
         n_debug("mtcsdfscan.c df_ref_change_reg_with_loc_1 11 new_df:%p %d the_ref:%p\n",new_df,new_df->n_refs,the_ref);

         new_df->n_refs++;
         if (DF_REF_BB (the_ref))
            mtcs_dfcore_df_set_bb_dirty/*!df_set_bb_dirty*/(mtcsDfcore,DF_REF_BB (the_ref));
         ...
         if (*ref_ptr != the_ref){
            ...
            *ref_ptr = next;
         }else if (DF_REF_NEXT_LOC (the_ref)  && df_ref_compare (the_ref, DF_REF_NEXT_LOC (the_ref)) > 0){
...
void mtcs_dfscan_df_ref_change_reg_with_loc (MtcsDfscan *self,rtx loc, unsigned int new_regno)
{
   ...
   if (df){
      mtcs_dfsan_df_grow_reg_info/*!df_grow_reg_info*/(self);
      df_ref_change_reg_with_loc_1(self,DF_REG_DEF_GET (old_regno),DF_REG_DEF_GET (new_regno),new_regno, loc);
      df_ref_change_reg_with_loc_1(self,DF_REG_USE_GET (old_regno),DF_REG_USE_GET (new_regno), new_regno, loc);
      df_ref_change_reg_with_loc_1(self,DF_REG_EQ_USE_GET (old_regno),DF_REG_EQ_USE_GET (new_regno),new_regno, loc);
   }
   mtcs_rtl_set_mode_and_regno/*!set_mode_and_regno*/(mtcsRTL,loc, GET_MODE (loc), new_regno);
}
   ...
   if (insn_info){
      ...
      for (note = REG_NOTES (insn); note; note = XEXP (note, 1)){
         switch (REG_NOTE_KIND (note)){
            case REG_EQUIV:
            case REG_EQUAL:
            df_uses_record(self,&collection_rec,
      ...
      df_refs_add_to_chains(self,&collection_rec, bb, insn, copy_eq_uses);
   }else
      mtcs_dfscan_df_insn_rescan/*!df_insn_rescan*/(self,insn);
...
//不移植
static int df_ref_compare (df_ref ref1, df_ref ref2)
{
   if (DF_REF_CLASS (ref1) != DF_REF_CLASS (ref2))
   ...
   if (DF_REF_CLASS (ref1) != DF_REF_ARTIFICIAL   && DF_REF_LOC (ref1) != DF_REF_LOC (ref2))
      return (int)DF_REF_ORDER (ref1) - (int)DF_REF_ORDER (ref2);
   ...
   if (DF_REF_FLAGS (ref1) != DF_REF_FLAGS (ref2)){
      ...
      first.  */
      if (DF_REF_FLAGS_IS_SET (ref1, DF_REF_MW_HARDREG) == DF_REF_FLAGS_IS_SET (ref2, DF_REF_MW_HARDREG))
...
//不移植
static int df_ref_ptr_compare (const void *r1, const void *r2)
{
...
static void df_sort_and_compress_refs (vec<df_ref, va_heap> *ref_vec)
{
   ...
   for (i=0; i<count-dist; i++){
      ...
      while (i + dist + 1 < count  && df_ref_equal_p ((*ref_vec)[i],(*ref_vec)[i + dist + 1])){
...
//不移植
static int df_mw_compare (const df_mw_hardreg *mw1, const df_mw_hardreg *mw2)
{
...
//不移植
static int df_mw_ptr_compare (const void *m1, const void *m2)
{
  return df_mw_compare (*(const df_mw_hardreg *const *) m1,*(const df_mw_hardreg *const *) m2);
}
...
static void df_sort_and_compress_mws (vec<df_mw_hardreg *, va_heap> *mw_vec)
{
   ...
   for (i=0; i<count-dist; i++){
      /* Find the next ref that is not equal to the current ref.  */
      while (i + dist + 1 < count && df_mw_equal_p ((*mw_vec)[i], (*mw_vec)[i + dist + 1])){
...
static void df_install_ref (MtcsDfscan *self,df_ref this_ref,
      struct df_reg_info *reg_info, struct df_ref_info *ref_info,bool add_to_table)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   n_debug("mtcsdfscan.c df_install_ref 00 regno:%d new_df:%p %d this_ref:%p\n",regno,reg_info,reg_info->n_refs,this_ref);

   reg_info->reg_chain = this_ref;
   reg_info->n_refs++;

   if (DF_REF_FLAGS_IS_SET (this_ref, DF_HARD_REG_LIVE)){
      gcc_assert (regno < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg));
      df->hard_regs_live_count[regno]++;
   }

   gcc_checking_assert (DF_REF_NEXT_REG (this_ref) == NULL && DF_REF_PREV_REG (this_ref) == NULL);
...
static df_ref df_install_refs (MtcsDfscan *self,basic_block bb,
       ...
       bool is_notes)
{
   ...
   if (count){
      ...
      FOR_EACH_VEC_ELT (*old_vec, ix, this_ref){
         DF_REF_NEXT_LOC (this_ref) = (ix + 1 < old_vec->length ()? (*old_vec)[ix + 1]: NULL);
         df_install_ref(self,this_ref, reg_info[DF_REF_REGNO (this_ref)],ref_info, add_to_table);
      }
      return (*old_vec)[0];
   }else
      return 0;
...
static void df_refs_add_to_chains (MtcsDfscan *self,class df_collection_rec *collection_rec,
             basic_block bb, rtx_insn *insn, unsigned int flags)
{
   if (insn){
      ...
      if (flags & copy_defs){
         ...
         insn_rec->defs = df_install_refs(self,bb, &collection_rec->def_vec,df->def_regs, &df->def_info, false);
      }
      if (flags & copy_uses){
         gcc_checking_assert (!insn_rec->uses);
         insn_rec->uses = df_install_refs(self,bb, &collection_rec->use_vec,df->use_regs, &df->use_info, false);
      }
      if (flags & copy_eq_uses){
         gcc_checking_assert (!insn_rec->eq_uses);
         insn_rec->eq_uses = df_install_refs(self,bb, &collection_rec->eq_use_vec,df->eq_use_regs, &df->use_info, true);
      }
      if (flags & copy_mw){
         gcc_checking_assert (!insn_rec->mw_hardregs);
         insn_rec->mw_hardregs  = df_install_mws (&collection_rec->mw_vec);
      }
   }else{
      struct df_scan_bb_info *bb_info = df_scan_get_bb_info (bb->index);

      gcc_checking_assert (!bb_info->artificial_defs);
      bb_info->artificial_defs = df_install_refs(self,bb, &collection_rec->def_vec,df->def_regs, &df->def_info, false);
      gcc_checking_assert (!bb_info->artificial_uses);
      bb_info->artificial_uses = df_install_refs(self,bb, &collection_rec->use_vec,df->use_regs,   &df->use_info, false);
   }
}
...
static df_ref df_ref_create_structure (MtcsDfscan *self,enum df_ref_class cl,
          class df_collection_rec *collection_rec,
          rtx reg, rtx *loc,
          ...
          int ref_flags)
{
   ...
   const unsigned int regno = REGNO (GET_CODE (reg) == SUBREG ? SUBREG_REG (reg) : reg);
   struct df_scan_problem_data *problem_data = (struct df_scan_problem_data *) df_scan->problem_data;
   df_ref this_ref;
   ...
   && cl != DF_REF_ARTIFICIAL  && !DEBUG_INSN_P (info->insn)){
      if (ref_type == DF_REF_REG_DEF){
         if (!DF_REF_FLAGS_IS_SET (this_ref, DF_REF_MAY_CLOBBER))
            DF_REF_FLAGS_SET (this_ref, DF_HARD_REG_LIVE);
      }else if (!(mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&self->elim_reg_set, regno)
   ...
   if (collection_rec){
      if (DF_REF_REG_DEF_P (this_ref))
         ...
         n_debug("mtcsdfscan.c df_ref_create_structure 真正存入数据到 use_vec this_ref:%p\n",this_ref);
         collection_rec->use_vec.safe_push (this_ref);
      }
   }else
      df_install_ref_incremental(self,this_ref);
...
static void df_ref_record (MtcsDfscan *self,enum df_ref_class cl,
          class df_collection_rec *collection_rec,
          rtx reg, rtx *loc,
          ...
          int ref_flags)
{
   ...
   if (regno <  mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)){
      ...
      df_ref ref;
      //n_debug("mtcsdfscan.c df_ref_record 00: regno:%d\n",regno);

      if (GET_CODE (reg) == SUBREG){
         int off = mtcs_rtlanal_subreg_regno_offset/*!subreg_regno_offset*/(mtcsRtlanal,regno,
               GET_MODE (SUBREG_REG (reg)),SUBREG_BYTE (reg), GET_MODE (reg));
         unsigned int nregno = regno + off;
         endregno = nregno + mtcs_rtlanal_subreg_nregs/*!subreg_nregs*/(mtcsRtlanal,reg);
         //n_debug("mtcsdfscan.c df_ref_record 11: regno:%d off:%d nregno:%d endregno:%d\n",regno,off,nregno,endregno);
         ...
         n_debug("mtcsdfscan.c df_ref_record 22: regno:%d endregno:%d\n",regno,endregno);
      ...
      if (collection_rec && (endregno != regno + 1) && insn_info){
         ...
         //n_debug("mtcsdfscan.c df_ref_record 33: regno:%d endregno:%d refflags:%d\n",regno,endregno,ref_flags);
      ...
      for (i = regno; i < endregno; i++){
         //n_debug("mtcsdfscan.c df_ref_record 44: regno:%d endregno:%d refflags:%d i:%d\n",regno,endregno,ref_flags,i);

         ref = df_ref_create_structure(self,cl, collection_rec,
               mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[i], loc,bb, insn_info, ref_type, ref_flags);
         gcc_assert (ORIGINAL_REGNO (DF_REF_REG (ref)) == i);
      }
   }else{
      //n_debug("mtcsdfscan.c df_ref_record 55: regno:%d refflags:%d\n",regno,ref_flags);

      df_ref_create_structure(self,cl, collection_rec, reg, loc, bb, insn_info,ref_type, ref_flags);
   }
}
...
static void df_def_record_1 (MtcsDfscan *self,class df_collection_rec *collection_rec,
                 rtx *loc, basic_block bb, struct df_insn_info *insn_info, int flags)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (GET_CODE (dst) == PARALLEL){
      ...
      for (i = XVECLEN (dst, 0) - 1; i >= 0; i--){
         ...
         df_def_record_1(self,collection_rec, &XEXP (temp, 0),bb, insn_info, flags);
      }
      return;
   }
   ...
   if (REG_P (dst)) {
      df_ref_record(self,DF_REF_REGULAR, collection_rec, dst, loc, bb, insn_info, DF_REF_REG_DEF, flags);
      /* We want to keep sp alive everywhere - by making all
      writes to sp also use of sp. */
      if (REGNO (dst) == mtcs_reg_get_stack_pointer_regnum/*!STACK_POINTER_REGNUM*/(mtcsReg))
         df_ref_record(self,DF_REF_BASE, collection_rec, dst, NULL, bb, insn_info, DF_REF_REG_USE, flags);
   }else if (GET_CODE (dst) == SUBREG && REG_P (SUBREG_REG (dst))){
      if (mtcs_rtlanal_read_modify_subreg_p/*!read_modify_subreg_p*/(mtcsRtlanal,dst))
...
static void df_defs_record (MtcsDfscan *self,class df_collection_rec *collection_rec,
                rtx x, basic_block bb, struct df_insn_info *insn_info,int flags)
{
   ...
   switch (code){
      case SET:
         df_def_record_1(self,collection_rec, &SET_DEST (x), bb, insn_info, flags);
         break;

      case CLOBBER:
         flags |= DF_REF_MUST_CLOBBER;
         df_def_record_1(self,collection_rec, &XEXP (x, 0), bb, insn_info, flags);
         break;

      case COND_EXEC:
         df_defs_record(self,collection_rec, COND_EXEC_CODE (x),bb, insn_info, DF_REF_CONDITIONAL);
         break;

      case PARALLEL:
         for (i = 0; i < XVECLEN (x, 0); i++)
            df_defs_record(self,collection_rec, XVECEXP (x, 0, i),bb, insn_info, flags);
         break;
      default:
...
/* Set bits in *DEFS for hard registers found in the rtx DST, which is the
   ...
   df_defs_record_1.  */
static void df_find_hard_reg_defs_1 (MtcsDfscan *self,rtx dst, HardRegSet/*!HARD_REG_SET*/ *defs)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (GET_CODE (dst) == PARALLEL){
      ...
      for (i = XVECLEN (dst, 0) - 1; i >= 0; i--){
         rtx temp = XVECEXP (dst, 0, i);
         gcc_assert (GET_CODE (temp) == EXPR_LIST);
         df_find_hard_reg_defs_1(self,XEXP (temp, 0), defs);
      }
      return;
   }
   ...
   /* At this point if we do not have a reg or a subreg, just return.  */
   if (REG_P (dst) && mtcs_reg_is_hard_rtx/*!HARD_REGISTER_P*/(mtcsReg,dst))
      mtcs_reg_set_hard_reg_bit/*!SET_HARD_REG_BIT*/(mtcsReg,defs, REGNO (dst));
   else if (GET_CODE (dst) == SUBREG  && REG_P (SUBREG_REG (dst)) && mtcs_reg_is_hard_rtx/*!HARD_REGISTER_P*/(mtcsReg,dst))
      mtcs_reg_set_hard_reg_bit/*!SET_HARD_REG_BIT*/(mtcsReg,defs, REGNO (SUBREG_REG (dst)));
}

/* Set bits in *DEFS for hard registers defined in the pattern X.  This
   has to match the logic in df_defs_record.  */
static void df_find_hard_reg_defs (MtcsDfscan *self,rtx x, HardRegSet/*!HARD_REG_SET*/ *defs)
{
   RTX_CODE code = GET_CODE (x);
   int i;
   ...
   switch (code){
      case SET:
         df_find_hard_reg_defs_1(self,SET_DEST (x), defs);
         break;

      case CLOBBER:
         df_find_hard_reg_defs_1(self,XEXP (x, 0), defs);
         break;

      case COND_EXEC:
         df_find_hard_reg_defs(self,COND_EXEC_CODE (x), defs);
         break;

      case PARALLEL:
         for (i = 0; i < XVECLEN (x, 0); i++)
            df_find_hard_reg_defs(self,XVECEXP (x, 0, i), defs);
         break;
      default:
         /* No DEFs to record in other cases */
         break;
...
static void df_uses_record (MtcsDfscan *self,class df_collection_rec *collection_rec,
                rtx *loc, enum df_ref_type ref_type, basic_block bb, struct df_insn_info *insn_info,int flags)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (!x)
      return;
   code = GET_CODE (x);
   //n_debug("mtcsdfscan.c df_uses_record 00 收集 use code:%d %s\n",code,GET_RTX_NAME(code));
   switch (code){
      case LABEL_REF:
      case SYMBOL_REF:
      case CONST:
      ...
      case CLOBBER:
         ...
         as being used.  */
         if (MEM_P (XEXP (x, 0)))
            df_uses_record(self,collection_rec,&XEXP (XEXP (x, 0), 0),DF_REF_REG_MEM_STORE,bb, insn_info,flags);

         /* If we're clobbering a REG then we have a def so ignore.  */
         return;

      case MEM:
         df_uses_record(self,collection_rec,&XEXP (x, 0), DF_REF_REG_MEM_LOAD,bb, insn_info, flags & DF_REF_IN_NOTE);
         return;
      ...
      case SUBREG:
         ...
         if (!REG_P (SUBREG_REG (x))){
            loc = &SUBREG_REG (x);
            df_uses_record(self,collection_rec, loc, ref_type, bb, insn_info, flags);
            return;
         }
      /* Fall through */

      case REG:
         df_ref_record(self,DF_REF_REGULAR, collection_rec,x, loc, bb, insn_info,ref_type, flags);
         return;

      case SIGN_EXTRACT:
      case ZERO_EXTRACT:
      {
         df_uses_record(self,collection_rec,&XEXP (x, 1), ref_type, bb, insn_info, flags);
         df_uses_record(self,collection_rec,&XEXP (x, 2), ref_type, bb, insn_info, flags);
         ...
         df_uses_record(self,collection_rec,&XEXP (x, 0), ref_type, bb, insn_info, flags);
         return;
      }
         break;
      ...
      case SET:
      {
         ...
         df_uses_record(self,collection_rec, &SET_SRC (x), DF_REF_REG_USE, bb, insn_info, flags);
        // n_debug("mtcsdfscan.c df_uses_record 11 收集 GET_CODE (dst) code:%d %s\n",GET_CODE (dst),GET_RTX_NAME(GET_CODE (dst)));

         switch (GET_CODE (dst)){
            case SUBREG:
               if (mtcs_rtlanal_read_modify_subreg_p/*!read_modify_subreg_p*/(mtcsRtlanal,dst)){
                  df_uses_record(self,collection_rec, &SUBREG_REG (dst),DF_REF_REG_USE, bb,
                        insn_info,flags | DF_REF_READ_WRITE | DF_REF_SUBREG);
                  break;
               }
            /* Fall through.  */
            case REG:
            ...
            case PC:
               break;
            case MEM:
               df_uses_record(self,collection_rec, &XEXP (dst, 0),DF_REF_REG_MEM_STORE, bb, insn_info, flags);
               break;
            case STRICT_LOW_PART:
            {
               ...
               SUBREG.  */
               dst = XEXP (dst, 0);
               df_uses_record(self,collection_rec,(GET_CODE (dst) == SUBREG) ? &SUBREG_REG (dst) : temp,
                     DF_REF_REG_USE, bb, insn_info, DF_REF_READ_WRITE | DF_REF_STRICT_LOW_PART);
            }
               break;
            case ZERO_EXTRACT:
            {
               df_uses_record(self,collection_rec, &XEXP (dst, 1),DF_REF_REG_USE, bb, insn_info, flags);
               df_uses_record(self,collection_rec, &XEXP (dst, 2),DF_REF_REG_USE, bb, insn_info, flags);
               if (GET_CODE (XEXP (dst,0)) == MEM)
                  df_uses_record(self,collection_rec, &XEXP (dst, 0),DF_REF_REG_USE, bb, insn_info,flags);
               else
                  df_uses_record(self,collection_rec, &XEXP (dst, 0),DF_REF_REG_USE, bb, insn_info,
      ...
      case ASM_INPUT:
      {
         ...
         if (code == ASM_OPERANDS){
            ...
            for (j = 0; j < ASM_OPERANDS_INPUT_LENGTH (x); j++)
               df_uses_record(self,collection_rec, &ASM_OPERANDS_INPUT (x, j),DF_REF_REG_USE, bb, insn_info, flags);
            return;
         }
         break;
      }
      ...
      case VAR_LOCATION:
         df_uses_record(self,collection_rec,&PAT_VAR_LOCATION_LOC (x),DF_REF_REG_USE, bb, insn_info, flags);
         return;
      ...
      case POST_MODIFY:
         ...
         df_ref_record(self,DF_REF_REGULAR, collection_rec, XEXP (x, 0), &XEXP (x, 0),
               bb, insn_info,DF_REF_REG_DEF, flags | DF_REF_READ_WRITE | DF_REF_PRE_POST_MODIFY);
   ...
   /* Recursively scan the operands of this expression.  */
   {
      ...
      for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--){
         if (fmt[i] == 'e'){
            ...
            df_uses_record(self,collection_rec, &XEXP (x, i), ref_type,bb, insn_info, flags);
         }else if (fmt[i] == 'E'){
            int j;
            for (j = 0; j < XVECLEN (x, i); j++)
               df_uses_record(self,collection_rec, &XVECEXP (x, i, j), ref_type,bb, insn_info, flags);
         }
      }
   }
...
static void df_get_conditional_uses (MtcsDfscan *self,class df_collection_rec *collection_rec)
{
   unsigned int ix;
   df_ref ref;
   FOR_EACH_VEC_ELT (collection_rec->def_vec, ix, ref){
      if (DF_REF_FLAGS_IS_SET (ref, DF_REF_CONDITIONAL)){
         df_ref use;
         use = df_ref_create_structure(self,DF_REF_CLASS (ref), collection_rec, DF_REF_REG (ref),
         DF_REF_LOC (ref), DF_REF_BB (ref), DF_REF_INSN_INFO (ref), DF_REF_REG_USE,DF_REF_FLAGS (ref) & ~DF_REF_CONDITIONAL);
...
static void df_get_call_refs (MtcsDfscan *self,class df_collection_rec *collection_rec,
                  basic_block bb,
                  struct df_insn_info *insn_info,
                  int flags)
{
   ...
   unsigned int i;
   HardRegSet/*!HARD_REG_SET*/ defs_generated={mtcs_reg_get_hard_reg_element_count(mtcsReg)};

   mtcs_reg_clear_hard_reg_set/*!CLEAR_HARD_REG_SET*/(&defs_generated);
   df_find_hard_reg_defs(self,PATTERN (insn_info->insn), &defs_generated);
   is_sibling_call = SIBLING_CALL_P (insn_info->insn);
   mtcs_function_abi callee_abi = mtcs_func_abi_insn_callee_abi/*!insn_callee_abi*/(mtcsFuncAbi,insn_info->insn);
   n_debug("mtcsdfscan.c df_get_call_refs ----- %p %p %p %p\n",mtcsMode,mtcsTarget,mtcsFuncAbi,callee_abi.mtcsFuncAbi);

   for (i = 0; i < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); i++){
      if (i == mtcs_reg_get_stack_pointer_regnum/*!STACK_POINTER_REGNUM*/(mtcsReg) && !FAKE_CALL_P (insn_info->insn))
         /* The stack ptr is used (honorarily) by a CALL insn.  */
         df_ref_record(self,DF_REF_BASE, collection_rec, mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[i],
               NULL, bb, insn_info, DF_REF_REG_USE,DF_REF_CALL_STACK_USAGE | flags);
      else if (mtcsReg->global_regs/*!global_regs*/[i]){
      ...
      && !mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&defs_generated, i)
      && (!is_sibling_call
      || !bitmap_bit_p (df->exit_block_uses, i)
      || refers_to_regno_p (i, mtcsRtlData/*!crtl*/->return_rtx)))
         df_ref_record(self,DF_REF_BASE, collection_rec, mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[i],
               NULL, bb, insn_info, DF_REF_REG_DEF,DF_REF_MAY_CLOBBER | flags);
   }

   /* Record the registers used to pass arguments, and explicitly
   noted as clobbered.  */
   for (note = CALL_INSN_FUNCTION_USAGE (insn_info->insn); note;note = XEXP (note, 1)){
      if (GET_CODE (XEXP (note, 0)) == USE)
         df_uses_record(self,collection_rec, &XEXP (XEXP (note, 0), 0),DF_REF_REG_USE, bb, insn_info, flags);
      else if (GET_CODE (XEXP (note, 0)) == CLOBBER){
         if (REG_P (XEXP (XEXP (note, 0), 0))){
            unsigned int regno = REGNO (XEXP (XEXP (note, 0), 0));
         if (!mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&defs_generated, regno))
            df_defs_record(self,collection_rec, XEXP (note, 0), bb,insn_info, flags);
         }else
            df_uses_record(self,collection_rec, &XEXP (note, 0),DF_REF_REG_USE, bb, insn_info, flags);
      }
   }
...
static void df_insn_refs_collect (MtcsDfscan *self,class df_collection_rec *collection_rec,
            basic_block bb, struct df_insn_info *insn_info)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   /* Process REG_EQUIV/REG_EQUAL notes.  */
   for (note = REG_NOTES (insn_info->insn); note; note = XEXP (note, 1)){
      n_debug("mtcsdfscan.c df_insn_refs_collect 收集 00 REG_NOTE_KIND (note):%d %d %d %d\n",
            REG_NOTE_KIND (note),REG_EQUIV,REG_EQUAL,REG_NON_LOCAL_GOTO);
      switch (REG_NOTE_KIND (note)){
         case REG_EQUIV:
         case REG_EQUAL:
            df_uses_record(self,collection_rec,&XEXP (note, 0), DF_REF_REG_USE,bb, insn_info, DF_REF_IN_NOTE);
            break;
         case REG_NON_LOCAL_GOTO:
            /* The frame ptr is used by a non-local goto.  */
            df_ref_record(self,DF_REF_BASE, collection_rec,
               mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg)],
               NULL, bb, insn_info,DF_REF_REG_USE, 0);

            if (!mtcs_reg_hard_frame_pointer_is_frame_pointer/*!HARD_FRAME_POINTER_IS_FRAME_POINTER*/(mtcsReg))
               df_ref_record(self,DF_REF_BASE, collection_rec,
                  mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[mtcs_reg_get_hard_frame_pointer_regnum/*!HARD_FRAME_POINTER_REGNUM*/(mtcsReg)],
                  NULL, bb, insn_info,DF_REF_REG_USE, 0);
            break;
         default:
            break;
   ...
   if (CALL_P (insn_info->insn))
      df_get_call_refs(self,collection_rec, bb, insn_info, flags);
   /* Record other defs.  These should be mostly for DF_REF_REGULAR, so
   that a qsort on the defs is unnecessary in most cases.  */
   df_defs_record(self,collection_rec,PATTERN (insn_info->insn), bb, insn_info, 0);
   /* Record the register uses.  */
   n_debug("mtcsdfscan.c df_insn_refs_collect 11 做uses收集 \n");

   df_uses_record(self,collection_rec,&PATTERN (insn_info->insn), DF_REF_REG_USE, bb, insn_info, 0);
   /* DF_REF_CONDITIONAL needs corresponding USES. */
   if (is_cond_exec)
      df_get_conditional_uses(self,collection_rec);

   df_canonize_collection_rec (collection_rec);//不移植
}
...
/* Collect all artificial refs at the block level for BB and add them
   to COLLECTION_REC.  */
static void df_bb_refs_collect (MtcsDfscan *self,class df_collection_rec *collection_rec, basic_block bb)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (bb->index == ENTRY_BLOCK){
      n_debug("mtcsdfscan.c df_bb_refs_collect 收集 00 是 ENTRY_BLOCK bb:%p\n",bb);
      df_entry_block_defs_collect(self,collection_rec, df->entry_block_defs);
      return;
   }else if (bb->index == EXIT_BLOCK){
      n_debug("mtcsdfscan.c df_bb_refs_collect 收集 11 是 EXIT_BLOCK bb:%p\n",bb);

      df_exit_block_uses_collect(self,collection_rec, df->exit_block_uses);
      return;
   }

   if (bb_has_eh_pred (bb)){
      n_debug("mtcsdfscan.c df_bb_refs_collect 收集 22 bb_has_eh_pred bb:%p\n",bb);

      unsigned int i;
      /* Mark the registers that will contain data for the handler.  */
      for (i = 0; ; ++i){
         unsigned regno = mtcs_reg_get_eh_return_data_regno/*!EH_RETURN_DATA_REGNO*/(mtcsReg,i);
         n_debug("mtcsdfscan.c df_bb_refs_collect 收集 33 bb_has_eh_pred i:%d regno:%d bb:%p\n",i,regno,bb);

         if (regno == INVALID_REGNUM)
            break;
         df_ref_record(self,DF_REF_ARTIFICIAL, collection_rec,
               mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[regno], NULL,bb, NULL, DF_REF_REG_DEF, DF_REF_AT_TOP);
      }
   }
   ...
   if (bb->flags & BB_NON_LOCAL_GOTO_TARGET){
      n_debug("mtcsdfscan.c df_bb_refs_collect 收集 44 bb->flags:%d bb:%p\n",bb->flags,bb);

      df_ref_record(self,DF_REF_ARTIFICIAL, collection_rec,
            mtcs_rtl_get_hard_frame_pointer_rtx/*!hard_frame_pointer_rtx*/(mtcsRTL), NULL,bb, NULL, DF_REF_REG_DEF, DF_REF_AT_TOP);
   }

   /* Add the artificial uses.  */
   if (bb->index >= NUM_FIXED_BLOCKS){
      n_debug("mtcsdfscan.c df_bb_refs_collect 收集 55 bb->index >= NUM_FIXED_BLOCKS bb->flags:%d bb:%p\n",bb->flags,bb);

      bitmap_iterator bi;
      unsigned int regno;
      bitmap au = bb_has_eh_pred (bb)? &df->eh_block_artificial_uses: &df->regular_block_artificial_uses;

      EXECUTE_IF_SET_IN_BITMAP (au, 0, regno, bi){
         n_debug("mtcsdfscan.c df_bb_refs_collect 收集 66 bb->index >= NUM_FIXED_BLOCKS bb->flags:%d bb:%p\n",bb->flags,bb);

         df_ref_record(self,DF_REF_ARTIFICIAL, collection_rec,
               mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[regno], NULL,bb, NULL, DF_REF_REG_USE, 0);
      }
   }
...
void mtcs_dfscan_df_bb_refs_record (MtcsDfscan *self,int bb_index, bool scan_insns)
{
   ...
   if (scan_insns)
      ...
      FOR_BB_INSNS (bb, insn){
         ...
         if (INSN_P (insn)){
            ...
            DF_INSN_INFO_LUID (insn_info) = luid++;
            df_insn_refs_collect(self,&collection_rec, bb, DF_INSN_INFO_GET (insn));
            df_refs_add_to_chains(self,&collection_rec, bb, insn, copy_all);
         }
         DF_INSN_INFO_LUID (insn_info) = luid;
      }
   ...
   df_bb_refs_collect(self,&collection_rec, bb);
   df_refs_add_to_chains(self,&collection_rec, bb, NULL, copy_all);
...
static void df_get_regular_block_artificial_uses (MtcsDfscan *self,bitmap regular_block_artificial_uses)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg =mtcs_target_get_reg(mtcsTarget);
   ...
   {
      unsigned int picreg = mtcs_reg_get_pic_offset_table_regnum/*!PIC_OFFSET_TABLE_REGNUM*/(mtcsReg);

      /* Any reference to any pseudo before reload is a potential
      reference of the frame pointer.  */
      bitmap_set_bit (regular_block_artificial_uses,mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg));
      ...
      && mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[argPointerRegNum/*!ARG_POINTER_REGNUM*/])
         bitmap_set_bit (regular_block_artificial_uses, argPointerRegNum/*!ARG_POINTER_REGNUM*/);

      /* Any constant, or pseudo with constant equivalences, may
      require reloading from memory using the pic register.  */
      if (picreg != INVALID_REGNUM && mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[picreg])
         bitmap_set_bit (regular_block_artificial_uses, picreg);
   }
   /* The all-important stack pointer must always be live.  */
   bitmap_set_bit (regular_block_artificial_uses, mtcs_reg_get_stack_pointer_regnum/*!STACK_POINTER_REGNUM*/(mtcsReg));
...
static void df_get_eh_block_artificial_uses (MtcsDfscan *self,bitmap eh_block_artificial_uses)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg =mtcs_target_get_reg(mtcsTarget);
   ...
   if (reload_completed){
      if (mtcsRtlData->x_frame_pointer_needed/*!frame_pointer_needed*/){
         bitmap_set_bit (eh_block_artificial_uses, mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg));
         if (!mtcs_reg_hard_frame_pointer_is_frame_pointer/*!HARD_FRAME_POINTER_IS_FRAME_POINTER*/(mtcsReg))
      ...
      if (mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg) !=argPointerRegNum/*!ARG_POINTER_REGNUM*/
      && mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[argPointerRegNum/*!ARG_POINTER_REGNUM*/])
         bitmap_set_bit (eh_block_artificial_uses, argPointerRegNum/*!ARG_POINTER_REGNUM*/);
   }
}
...
static void df_get_entry_block_def_set (MtcsDfscan *self,bitmap entry_block_defs)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg =mtcs_target_get_reg(mtcsTarget);
   ...
   for (i = 0; i < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); i++){
      ...
      if (mtcs_func_is_function_arg_regno/*!FUNCTION_ARG_REGNO_P*/(mtcsFunc,i))
         bitmap_set_bit (entry_block_defs, mtcs_reg_get_incoming_regno/*!INCOMING_REGNO*/(mtcsReg,i));
   }

   /* The always important stack pointer.  */
   bitmap_set_bit (entry_block_defs, mtcs_reg_get_stack_pointer_regnum/*!STACK_POINTER_REGNUM*/(mtcsReg));

   /* Once the prologue has been generated, all of these registers
   should just show up in the first regular block.  */
   if (target_rtx_have_prologue/*!targetm.have_prologue*/(mtcsMachine->tmrtx) && epilogue_completed){
      /* Defs for the callee saved registers are inserted so that the
      pushes have some defining location.  */
      for (i = 0; i < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); i++)
         if (!mtcsRtlData/*!crtl*/->abi->clobbers_full_reg_p (i)
         && !mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[i]
         && mtcs_dfscan_df_regs_ever_live_p/*!df_regs_ever_live_p*/(self,i))
            bitmap_set_bit (entry_block_defs, i);
   }

   r = target_calls_struct_value_rtx/*!targetm.calls.struct_value_rtx*/(mtcsMachine->calls,current_function_decl, true);
   if (r && REG_P (r))
      bitmap_set_bit (entry_block_defs, REGNO (r));

   /* If the function has an incoming STATIC_CHAIN, it has to show up
   in the entry def set.  */
   r = mtcs_calls_rtx_for_static_chain/*!rtx_for_static_chain*/(mtcsCalls,current_function_decl, true);
   if (r && REG_P (r))
      bitmap_set_bit (entry_block_defs, REGNO (r));
   ...
   if ((!reload_completed) || mtcsRtlData->x_frame_pointer_needed/*!frame_pointer_needed*/){
      /* Any reference to any pseudo before reload is a potential
      reference of the frame pointer.  */
      bitmap_set_bit (entry_block_defs, mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg));

      /* If they are different, also mark the hard frame pointer as live.  */
      if (!mtcs_reg_hard_frame_pointer_is_frame_pointer/*!HARD_FRAME_POINTER_IS_FRAME_POINTER*/(mtcsReg)
      && !LOCAL_REGNO (mtcs_reg_get_hard_frame_pointer_regnum/*!HARD_FRAME_POINTER_REGNUM*/(mtcsReg)))
         bitmap_set_bit (entry_block_defs, mtcs_reg_get_hard_frame_pointer_regnum/*!HARD_FRAME_POINTER_REGNUM*/(mtcsReg));
   }
   ...
   if (!reload_completed){
      ...
      if (mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg) != argPointerRegNum/*!ARG_POINTER_REGNUM*/
      && mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[argPointerRegNum/*!ARG_POINTER_REGNUM*/])
         bitmap_set_bit (entry_block_defs, argPointerRegNum/*!ARG_POINTER_REGNUM*/);
...
//#ifdef INCOMING_RETURN_ADDR_RTX //host=1 nvptx=0
//   n_debug("mtcsdfscan.c INCOMING_RETURN_ADDR_RTX----\n");
//   if (REG_P (INCOMING_RETURN_ADDR_RTX))
//   bitmap_set_bit (entry_block_defs, REGNO (INCOMING_RETURN_ADDR_RTX));
//#endif

   mtcsTarget/*!targetm.extra_live_on_entry*/->extra_live_on_entry(mtcsTarget,entry_block_defs);
}
...
static void df_entry_block_defs_collect (MtcsDfscan *self,class df_collection_rec *collection_rec,
              bitmap entry_block_defs)
{
   ...
   EXECUTE_IF_SET_IN_BITMAP (entry_block_defs, 0, i, bi){
      n_debug("mtcsdfscan.c df_entry_block_defs_collect i:%d\n",i);
      df_ref_record(self,DF_REF_ARTIFICIAL, collection_rec, mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[i], NULL,
...
/* Record the (conservative) set of hard registers that are defined on
   entry to the function.  */
static void df_record_entry_block_defs (MtcsDfscan *self,bitmap entry_block_defs)
{
  class df_collection_rec collection_rec;
  df_entry_block_defs_collect(self,&collection_rec, entry_block_defs);
  /* Process bb_refs chain */
  df_refs_add_to_chains(self,&collection_rec,BASIC_BLOCK_FOR_FN (cfun, ENTRY_BLOCK), NULL,copy_defs);
}
...
void mtcs_dfscan_df_update_entry_block_defs (MtcsDfscan *self)
{
   ...
   auto_bitmap refs (&mtcsDfcore->df_bitmap_obstack);
   df_get_entry_block_def_set(self,refs);
   ...
   if (changed){
      df_record_entry_block_defs(self,refs);
...
void mtcs_dfscan_df_get_exit_block_use_set (MtcsDfscan *self,bitmap exit_block_uses)
{
   ...
   bitmap_set_bit (exit_block_uses, mtcs_reg_get_stack_pointer_regnum/*!STACK_POINTER_REGNUM*/(mtcsReg));
   ...
   if ((!reload_completed) || mtcsRtlData->x_frame_pointer_needed/*!frame_pointer_needed*/){
      bitmap_set_bit (exit_block_uses, mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg));

      /* If they are different, also mark the hard frame pointer as live.  */
      if (!mtcs_reg_hard_frame_pointer_is_frame_pointer/*!HARD_FRAME_POINTER_IS_FRAME_POINTER*/(mtcsReg)
      && !LOCAL_REGNO (mtcs_reg_get_hard_frame_pointer_regnum/*!HARD_FRAME_POINTER_REGNUM*/(mtcsReg)))
         bitmap_set_bit (exit_block_uses, mtcs_reg_get_hard_frame_pointer_regnum/*!HARD_FRAME_POINTER_REGNUM*/(mtcsReg));
   }

   /* Many architectures have a GP register even without flag_pic.
   Assume the pic register is not in use, or will be handled by
   other means, if it is not fixed.  */
   if (!mtcs_reg_get_pic_offset_table_reg_call_clobbered/*!PIC_OFFSET_TABLE_REG_CALL_CLOBBERED*/(mtcsReg)
   && picreg != INVALID_REGNUM
   && mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[picreg])
   ...
   for (i = 0; i < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); i++)
      if (mtcsReg->global_regs/*!global_regs*/[i] || mtcs_dfscan_df_epilogue_uses_p/*!df_epilogue_uses_p*/(self,i))
         bitmap_set_bit (exit_block_uses, i);

   if (target_rtx_have_epilogue/*!targetm.have_epilogue*/(mtcsMachine->tmrtx)  && epilogue_completed){
      /* Mark all call-saved registers that we actually used.  */
      for (i = 0; i < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); i++)
         if (mtcs_dfscan_df_regs_ever_live_p/*!df_regs_ever_live_p*/(self,i)
         && !LOCAL_REGNO (i) && !mtcsRtlData/*!crtl*/->abi->clobbers_full_reg_p (i))
            bitmap_set_bit (exit_block_uses, i);
   }

   /* Mark the registers that will contain data for the handler.  */
   if (reload_completed && mtcsRtlData/*!crtl*/->calls_eh_return)
        //gcc15 改为      IOR_REG_SET_HRS (exit_block_uses, eh_return_data_regs);
//      for (i = 0; ; ++i){
//         unsigned regno = mtcs_reg_get_eh_return_data_regno/*!EH_RETURN_DATA_REGNO*/(mtcsReg,i);
//         if (regno == INVALID_REGNUM)
//            break;
...
//      }
      MTCS_IOR_REG_SET_HRS(exit_block_uses,mtcsReg->hardRegs.x_eh_return_data_regs);

   //#ifdef EH_RETURN_STACKADJ_RTX //host=1 nvptx=0
   ...
   if ((!target_rtx_have_epilogue/*!targetm.have_epilogue*/(mtcsMachine->tmrtx)
         || ! epilogue_completed) && mtcsRtlData/*!crtl*/->calls_eh_return){
   ...
   mtcs_func_diddle_return_value/*!diddle_return_value*/(mtcsFunc,df_mark_reg, (void*) exit_block_uses);
}
...
static void df_exit_block_uses_collect (MtcsDfscan *self,class df_collection_rec *collection_rec, bitmap exit_block_uses)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   EXECUTE_IF_SET_IN_BITMAP (exit_block_uses, 0, i, bi)
      df_ref_record(self,DF_REF_ARTIFICIAL, collection_rec,
            mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[i], NULL,EXIT_BLOCK_PTR_FOR_FN (cfun), NULL, DF_REF_REG_USE, 0);
   ...
   if (mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg) != argPointerRegNum/*!ARG_POINTER_REGNUM*/
   && reload_completed
   && !bitmap_bit_p (exit_block_uses, argPointerRegNum/*!ARG_POINTER_REGNUM*/)
   && bb_has_eh_pred (EXIT_BLOCK_PTR_FOR_FN (cfun))
   && mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[argPointerRegNum/*!ARG_POINTER_REGNUM*/])
      df_ref_record(self,DF_REF_ARTIFICIAL, collection_rec,
            mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[argPointerRegNum/*!ARG_POINTER_REGNUM*/],
            NULL,EXIT_BLOCK_PTR_FOR_FN (cfun), NULL, DF_REF_REG_USE, 0);
...
static void df_record_exit_block_uses (MtcsDfscan *self,bitmap exit_block_uses)
{
   class df_collection_rec collection_rec;
   df_exit_block_uses_collect(self,&collection_rec, exit_block_uses);
   /* Process bb_refs chain */
   df_refs_add_to_chains(self,&collection_rec,BASIC_BLOCK_FOR_FN (cfun, EXIT_BLOCK),NULL,copy_uses);
}
...
void mtcs_dfscan_df_update_exit_block_uses (MtcsDfscan *self)
{
   ...
   auto_bitmap refs (&mtcsDfcore->df_bitmap_obstack);
   mtcs_dfscan_df_get_exit_block_use_set/*!df_get_exit_block_use_set*/(self,refs);
   ...
   if (changed){
      df_record_exit_block_uses(self,refs);
...
void mtcs_dfscan_df_hard_reg_init (MtcsDfscan *self)
{
   ...
   */
   ...
   mark_used_regs.  */
   mtcs_reg_clear_hard_reg_set/*!CLEAR_HARD_REG_SET*/(&self->elim_reg_set);
   for (i = 0; i < mtcsReg->elimiableRegsCount/*!(int) ARRAY_SIZE (eliminables)*/; i++)
      mtcs_reg_set_hard_reg_bit/*!SET_HARD_REG_BIT*/(mtcsReg,&self->elim_reg_set, eliminables[i].from);
...
{
   basic_block bb;
   n_debug("mtcsdfscan.c mtcs_dfscan_df_update_entry_exit_and_calls 00 reg_chain:%p\n",
        df?df->def_regs[0]->reg_chain:NULL);
   mtcs_dfscan_df_update_entry_block_defs/*!df_update_entry_block_defs*/(self);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_update_entry_exit_and_calls 11 reg_chain:%p\n",
        df?df->def_regs[0]->reg_chain:NULL);
   mtcs_dfscan_df_update_exit_block_uses/*df_update_exit_block_uses*/(self);
   n_debug("mtcsdfscan.c mtcs_dfscan_df_update_entry_exit_and_calls 22 reg_chain:%p\n",
        df?df->def_regs[0]->reg_chain:NULL);
   ...
   FOR_EACH_BB_FN (bb, cfun){
      ...
      FOR_BB_INSNS (bb, insn){
         if (INSN_P (insn) && CALL_P (insn)){
            n_debug("mtcsdfscan.c mtcs_dfscan_df_update_entry_exit_and_calls 33 reg_chain:%p\n",
                 df?df->def_regs[0]->reg_chain:NULL);
            mtcs_dfscan_df_insn_rescan/*!df_insn_rescan*/(self,insn);
            n_debug("mtcsdfscan.c mtcs_dfscan_df_update_entry_exit_and_calls 44 reg_chain:%p\n",
                 df?df->def_regs[0]->reg_chain:NULL);
         }
      }
   }
...
{
  return self->regs_ever_live[regno];
}
...
{
  if (self->regs_ever_live[regno] == value)
    return;

  self->regs_ever_live[regno] = value;
  if (df)
         ...
         bool is_def, bool is_eq_use)
{
   ...
   for (ref = refs; ref; ref = DF_REF_NEXT_REG (ref)){
      ...
      n_debug("mtcsdfscan.c df_reg_chain_mark 00 ref:%p flags:%d is_def:%d is_eq_use:%d df_chain:%p regno:%d %d\n",
...
//不移植
static bool df_refs_verify (const vec<df_ref, va_heap> *new_rec, df_ref old_rec,
      bool abort_if_fail)
{
   unsigned int ix;
   df_ref new_ref;
   ...
   FOR_EACH_VEC_ELT (*new_rec, ix, new_ref){
      ...
      //n_debug("mtcsdfscan.c df_refs_verify 00 %p %d\n",old_rec,abort_if_fail);
      ...
      //n_debug("mtcsdfscan.c df_refs_verify 11 %p %d\n",old_rec,abort_if_fail);
...
static bool df_mws_verify (const vec<df_mw_hardreg *, va_heap> *new_rec,
          struct df_mw_hardreg *old_rec,bool abort_if_fail)
{
   unsigned int ix;
...
static bool df_insn_refs_verify (MtcsDfscan *self,class df_collection_rec *collection_rec,
           basic_block bb,rtx_insn *insn, bool abort_if_fail)
{
  bool ret1, ret2, ret3;
  unsigned int uid = INSN_UID (insn);
  struct df_insn_info *insn_info = DF_INSN_INFO_GET (insn);
  n_debug("mtcsdfscan.c df_insn_refs_verify 00 reg_chain:%p\n",df?df->def_regs[0]->reg_chain:NULL);

  df_insn_refs_collect(self,collection_rec, bb, insn_info);
  n_debug("mtcsdfscan.c df_insn_refs_verify 11 reg_chain:%p\n",df?df->def_regs[0]->reg_chain:NULL);
  ...
  ret1 = df_refs_verify (&collection_rec->def_vec, DF_INSN_UID_DEFS (uid),abort_if_fail);
  if (!ret1 && !abort_if_fail)
    return false;
  n_debug("mtcsdfscan.c df_insn_refs_verify 22 reg_chain:%p\n",df?df->def_regs[0]->reg_chain:NULL);

  ret2 = df_refs_verify (&collection_rec->use_vec, DF_INSN_UID_USES (uid),abort_if_fail);
  if (!ret2 && !abort_if_fail)
    return false;
  n_debug("mtcsdfscan.c df_insn_refs_verify 33 reg_chain:%p\n",df?df->def_regs[0]->reg_chain:NULL);

  ret3 = df_refs_verify (&collection_rec->eq_use_vec, DF_INSN_UID_EQ_USES (uid),abort_if_fail);
  if (!ret3 && !abort_if_fail)
    return false;
  n_debug("mtcsdfscan.c df_insn_refs_verify 44 reg_chain:%p\n",df?df->def_regs[0]->reg_chain:NULL);

  if (! df_mws_verify (&collection_rec->mw_vec, DF_INSN_UID_MWS (uid),abort_if_fail)){
     n_debug("mtcsdfscan.c df_insn_refs_verify 55 reg_chain:%p\n",df?df->def_regs[0]->reg_chain:NULL);
    return false;
  }
  return (ret1 && ret2 && ret3);
}

static void printfFlags(basic_block bb)
{
   struct df_scan_bb_info *bbinfo= df_scan_get_bb_info (bb->index);
   df_ref adef=bbinfo->artificial_defs;
   n_debug("mtcsdfscan.c printfFlags 00 bb:%p bb->index:%d adef:%p\n",bb,bb->index,adef);
   if(adef){
      struct df_artificial_ref  dfaref=adef->artificial_ref;
      struct df_base_ref base =dfaref.base;
      n_debug("mtcsdfscan.c printfFlags 11 bb->index:%d flags:%d def:%p\n",bb->index,base.flags,adef);
   }
}
...
static bool df_bb_verify (MtcsDfscan *self,basic_block bb)
{
   rtx_insn *insn;
   struct df_scan_bb_info *bb_info = df_scan_get_bb_info (bb->index);
   class df_collection_rec collection_rec;
   ...
   gcc_assert (bb_info);
   n_debug("mtcsdfscan.c df_bb_verify 00 bb:%p bb->index:%d\n",bb,bb->index);
   printfFlags(bb);
   /* Scan the block, one insn at a time, from beginning to end.  */
   FOR_BB_INSNS_REVERSE (bb, insn){
      if (!INSN_P (insn))
         continue;
      n_debug("mtcsdfscan.c df_bb_verify 开始refs 认证 bb:%p bb->index:%d insn:%p\n",bb,bb->index,insn);
      mtcs_print_rtl_single(stderr,insn);
      df_insn_refs_verify(self,&collection_rec, bb, insn, true);
      df_free_collection_rec (&collection_rec);
   }
   n_debug("mtcsdfscan.c df_bb_verify 11 bb:%p bb->index:%d\n",bb,bb->index);
   printfFlags(bb);
   /* Do the artificial defs and uses.  */
   df_bb_refs_collect(self,&collection_rec, bb);
   n_debug("mtcsdfscan.c df_bb_verify 22 bb:%p bb->index:%d\n",bb,bb->index);
   printfFlags(bb);
...
static bool df_entry_block_bitmap_verify (MtcsDfscan *self,bool abort_if_fail)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   ...
   auto_bitmap entry_block_defs (&mtcsDfcore->df_bitmap_obstack);
   df_get_entry_block_def_set(self,entry_block_defs);

   is_eq = bitmap_equal_p (entry_block_defs, df->entry_block_defs);

   if (!is_eq && abort_if_fail){
      fprintf (stderr, "entry_block_defs = ");
      mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,stderr, entry_block_defs);
      fprintf (stderr, "df->entry_block_defs = ");
      mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,stderr, df->entry_block_defs);
      gcc_assert (0);
   }
...
static bool df_exit_block_bitmap_verify (MtcsDfscan *self,bool abort_if_fail)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   ...
   auto_bitmap exit_block_uses (&mtcsDfcore->df_bitmap_obstack);
   mtcs_dfscan_df_get_exit_block_use_set/*!df_get_exit_block_use_set*/(self,exit_block_uses);

   is_eq = bitmap_equal_p (exit_block_uses, df->exit_block_uses);

   if (!is_eq && abort_if_fail){
      fprintf (stderr, "exit_block_uses = ");
      mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,stderr, exit_block_uses);
      fprintf (stderr, "df->exit_block_uses = ");
      mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,stderr, df->exit_block_uses);
      gcc_assert (0);
   }
...
void mtcs_dfscan_df_scan_verify (MtcsDfscan *self)
{
   ...
   if (!df)
      return;
   n_debug("mtcsdfscan.c df_scan_verify 00 DF_REG_SIZE (df):%d\n",DF_REG_SIZE (df));
   FOR_ALL_BB_FN (bb, cfun){
      printfFlags (bb);
   }
...
//   n_debug("mtcsdfscan.c df_scan_verify 11 DF_REG_SIZE (df):%d\n",DF_REG_SIZE (df));
//
//   FOR_ALL_BB_FN (bb, cfun){
//      n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_verify 00 bb %p\n",bb);
//      printfFlags (bb);
//   }
   ...
   auto_bitmap regular_block_artificial_uses (&mtcsDfcore->df_bitmap_obstack);
   auto_bitmap eh_block_artificial_uses (&mtcsDfcore->df_bitmap_obstack);

   df_get_regular_block_artificial_uses(self,regular_block_artificial_uses);
   df_get_eh_block_artificial_uses(self,eh_block_artificial_uses);

   bitmap_ior_into (eh_block_artificial_uses,regular_block_artificial_uses);

   /* Check artificial_uses bitmaps didn't change. */
   gcc_assert (bitmap_equal_p (regular_block_artificial_uses, &df->regular_block_artificial_uses));
   gcc_assert (bitmap_equal_p (eh_block_artificial_uses,&df->eh_block_artificial_uses));

   /* Verify entry block and exit block. These only verify the bitmaps,
   the refs are verified in df_bb_verify.  */
   df_entry_block_bitmap_verify(self,true);
   df_exit_block_bitmap_verify(self,true);
...
bool mtcs_dfscan_df_epilogue_uses_p ( MtcsDfscan *self,unsigned int regno)
{
  ...
  return (mtcs_func_epilogue_uses/*!EPILOGUE_USES*/(mtcsFunc,regno)
     || mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&mtcsRtlData/*!crtl*/->must_be_zero_on_return, regno));
}
...
void mtcs_dfsan_df_grow_reg_info (MtcsDfscan *self)
{
   ...
   if(df && df->def_regs)
      n_debug("mtcsdfscan.c mtcs_dfsan_df_grow_reg_info 00 max_reg:%d df->regs_size:%d reg_chain:%p\n",
           max_reg,df->regs_size,df->def_regs[0]?df->def_regs[0]->reg_chain:NULL);
   struct df_scan_problem_data *problem_data = (struct df_scan_problem_data *) df_scan->problem_data;
   unsigned int i;

   if (df->regs_size < new_size){
      if(df && df->def_regs)
         n_debug("mtcsdfscan.c mtcs_dfsan_df_grow_reg_info 11 max_reg:%d df->regs_size:%d reg_chain:%p\n",
              max_reg,df->regs_size,df->def_regs[0]?df->def_regs[0]->reg_chain:NULL);
      new_size += new_size / 4;
      ...
      df->eq_use_regs = XRESIZEVEC (struct df_reg_info *, df->eq_use_regs,new_size);
  ...
  // if(df && df->def_regs)
    //  n_debug("mtcsdfscan.c mtcs_dfsan_df_grow_reg_info 22 max_reg:%d df->regs_size:%d reg_chain:%p regs_inited:%d df:%p df_scan:%p\n",
         //  max_reg,df->regs_size,df->def_regs[0]?df->def_regs[0]->reg_chain:NULL,df->regs_inited,df,df_scan);
   for (i = df->regs_inited; i < max_reg; i++){
      struct df_reg_info *reg_info;
     // if(df && df->def_regs)
       //  n_debug("mtcsdfscan.c mtcs_dfsan_df_grow_reg_info 33 max_reg:%d df->regs_size:%d reg_chain:%p\n",
           //  max_reg,df->regs_size,df->def_regs[0]?df->def_regs[0]->reg_chain:NULL);
      // TODO
   ...
   n_debug("mtcsdfscan.c mtcs_dfsan_df_grow_reg_info 44 max_reg:%d df:%p\n",max_reg,df);
...
void mtcs_dfscan_df_grow_insn_info (MtcsDfscan *self)
{
  ...
  unsigned int new_size = mtcs_rtl_data_get_max_uid/*!get_max_uid*/(mtcsRtlData) + 1;
...
void mtcs_dfscan_df_scan_alloc (MtcsDfscan *self,bitmap all_blocks ATTRIBUTE_UNUSED)
{
   ...
   everything apart.  */
   n_debug("mtcsdfscan.c mtcs_dfscan_df_scan_alloc 00 df_scan->problem_data:%p\n",df_scan->problem_data);
   if (df_scan->problem_data)
      df_scan_free_internal ();
   ...
   problem_data->ref_base_pool = new object_allocator<df_base_ref> ("df_scan ref base");
   problem_data->ref_artificial_pool = new object_allocator<df_artificial_ref> ("df_scan ref artificial");
   problem_data->ref_regular_pool = new object_allocator<df_regular_ref> ("df_scan ref regular");
   problem_data->insn_pool = new object_allocator<df_insn_info> ("df_scan insn");
   problem_data->reg_pool = new object_allocator<df_reg_info> ("df_scan reg");
   problem_data->mw_reg_pool = new object_allocator<df_mw_hardreg> ("df_scan mw_reg");
...
void mtcs_dfscan_df_compute_regs_ever_live (MtcsDfscan *self,bool reset)
{
   ...
   if (reset)
      memset (self->regs_ever_live, 0, sizeof (self->regs_ever_live));
   n_debug("mtcsdfscan.c mtcs_dfscan_df_compute_regs_ever_live 00 reset:%d changed:%d\n",reset,changed);

   for (i = 0; i < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); i++)
      if ((!self->regs_ever_live[i]) && df_hard_reg_used_p (i)){
         n_debug("mtcsdfscan.c mtcs_dfscan_df_compute_regs_ever_live 11 reset:%d\n",reset);
         self->regs_ever_live[i] = true;
         changed = true;
      }
   if (changed)
      mtcs_dfscan_df_update_entry_exit_and_calls/*!df_update_entry_exit_and_calls*/(self);
...
{
   MtcsDfscan *self = n_slice_alloc0 (sizeof(MtcsDfscan));
