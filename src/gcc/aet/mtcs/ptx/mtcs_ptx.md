...
(define_c_enum "unspec" [
   ...
   UNSPEC_SHUFFLE
   UNSPEC_BR_UNIFIED
   ;;支持nvcc shfl_xor_sync 如果还用 UNSPEC_SHUFFLE有冲突。
   UNSPEC_SHFL_XOR_SYNC
   ;; sqrt是标准rtl名，定义在rtl.def中，rsqrt不是标准的rtl code,所以用 unspec定义
   UNSPEC_RSQRT
])
...
;;获取gridDim blockDim blockIdx threadIdx内部变量的汇编
(define_insn "mtcs_dim"
  [(set (match_operand:SI 0 "nvptx_register_operand" "")
   (unspec_volatile:SI [(match_operand:SI 1 "const_int_operand" "")]
             UNSPECV_MTCS_DIM))]
  ""
{
    return mtcs_ptx_builtins_get_asm_dim((MtcsPtxBuiltins *)mtcsTarget->mtcsBuiltins,INTVAL (operands[1]));
})

;;生成在函数内声明的 shared 变量的rtl
(define_expand "mtcs_local_shared"
  [(set (match_operand:SI 0 "nvptx_nonmemory_operand" "")
        (match_operand:SI 1 "general_operand" ""))
   (unspec_volatile:SI [(match_operand:SI 2 "const_int_operand" "")]
              UNSPECV_MTCS_LOCAL_SHARED)]
  ""
{
  if (operands[0] != const0_rtx)
    emit_move_insn (operands[0], operands[1]);
  mtcs_ptx_builtins_expand_local_shared ((MtcsPtxBuiltins *)mtcsTarget->mtcsBuiltins,INTVAL (operands[1]),INTVAL (operands[2]));
  DONE;
})

(define_insn "mtcs_shared"
  [(unspec_volatile:SI [(match_operand:SI 0 "const_int_operand" "")]
             UNSPECV_MTCS_LOCAL_SHARED)]
  ""
{
  return mtcs_var_assemble_local_shared((MtcsVar *)mtcsTarget->mtcsVar,INTVAL (operands[0]));
}
  [(set_attr "predicable" "no")])
  
; ;生成有符号的divmod指令,具体指令字符串的生成由mtcs_internal_fn_divmod创建。
(define_expand "divmod<mode>4"
  [(parallel
     [(set (match_operand:HSDIM 0 "nvptx_register_operand")             ;; q
           (div:SI (match_operand:HSDIM 1 "nvptx_register_operand")     ;;A
                   (match_operand:HSDIM 2 "nvptx_register_operand")))   ;;B
      (set (match_operand:HSDIM 3 "nvptx_register_operand")             ;; r
           (mod:HSDIM (match_dup 1)
                   (match_dup 2)))])]
  ""
{
  rtx q   = operands[0];
  ...
  rtx tmp = gen_reg_rtx (<MODE>mode); 
  mtcs_internal_fn_divmod(mtcsTarget->mtcsInternalFn,q,a,b,r,tmp,<MODE>mode,0);
  DONE;
})
  ...
  ""
{
  ...
  rtx tmp = gen_reg_rtx (<MODE>mode); 
  mtcs_internal_fn_divmod(mtcsTarget->mtcsInternalFn,q,a,b,r,tmp,<MODE>mode,1);
  DONE;
})

;;生成fms指令 "%# 对应 rn
;;a * b - c (fma + neg c) -> res = a * b + (-c)\)
...
;;生成fnma指令，neg %1: 这是核心,表示反转第一个操作数
;;-a * b + c (fnma) -> res = (-a) * b + c
...
;; 增加 shfl_xor_sync 与 nvptx_shuffle 类似.
(define_insn "shfl_xor_sync<mode>"
  [(set (match_operand:BITS 0 "nvptx_register_operand" "=R")
   (unspec:BITS
      [(match_operand:BITS 1 "nvptx_register_operand" "R")
       (match_operand:SI 2 "nvptx_nonmemory_operand" "Ri")
       (match_operand:SI 3 "nvptx_nonmemory_operand" "Ri")]
        UNSPEC_SHFL_XOR_SYNC))]
  ""
  {
      return "%.\\tshfl.sync.bfly.b32\\t%0, %1, %2, 31, %3;";
  })
