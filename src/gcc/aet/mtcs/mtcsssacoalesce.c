...
inline hashval_t coalesce_pair_hasher::hash (const coalesce_pair *pair)
{
...
inline bool coalesce_pair_hasher::equal (const coalesce_pair *p1, const coalesce_pair *p2)
{
...
{
   hash_table<coalesce_pair_hasher>/*!coalesce_table_type*/ *list;  /* Hash table.  */
...
static inline int coalesce_cost (int frequency, bool optimize_for_size)
{
...
static inline int coalesce_cost_bb (basic_block bb)
{
...
static inline int coalesce_cost_edge (edge e)
{
  int mult = 1;
  ...
  return coalesce_cost (EDGE_FREQUENCY (e), optimize_edge_for_size_p (e)) * mult;
}
...
static inline int pop_cost_one_pair (coalesce_list *cl, int *p1, int *p2)
{
   cost_one_pair *ptr;
...
static inline int pop_best_coalesce (coalesce_list *cl, int *p1, int *p2)
{
   coalesce_pair *node;
   int ret;
...
static inline coalesce_list *create_coalesce_list (void)
{
   coalesce_list *list;
   ...
   list->list = new hash_table<coalesce_pair_hasher>/*!coalesce_table_type*/(size);
   list->sorted = NULL;
   list->num_sorted = 0;
...
static inline void delete_coalesce_list (coalesce_list *cl)
{
...
static inline int num_coalesce_pairs (coalesce_list *cl)
{
...
static coalesce_pair *find_coalesce_pair (coalesce_list *cl, int p1, int p2, bool create)
{
...
static inline void add_cost_one_coalesce (coalesce_list *cl, int p1, int p2)
{
   cost_one_pair *pair;
...
static int compare_pairs (const void *p1, const void *p2)
{
   ...
   MtcsSsaCoalesce *self = mtcs_target_get_ssa_coalesce(mtcsTarget);
   ...
   if (result == 0){
      if (flag_expensive_optimizations){
         ...
         initialize_conflict_count (*pp2, self->conflicts_, self->map_);
         if ((*pp1)->conflict_count == 0)
            initialize_conflict_count (*pp1, self->conflicts_, self->map_);
...
static void sort_coalesce_list (MtcsSsaCoalesce *self,coalesce_list *cl, ssa_conflicts *conflicts, var_map map)
{
   unsigned x, num;
   coalesce_pair *p;
...
static inline ssa_conflicts *ssa_conflicts_new (unsigned size)
{
   ssa_conflicts *ptr;
...
static inline void ssa_conflicts_delete (ssa_conflicts *ptr)
{
...
static inline bool ssa_conflicts_test_p (ssa_conflicts *ptr, unsigned x, unsigned y)
{
...
static coalesce_list *create_coalesce_list_for_region (MtcsSsaCoalesce *self,var_map map, bitmap used_in_copy)
{
   ...
   for (unsigned j = 0; map->vec_bbs.iterate (j, &bb); ++j){
      ...
      for (gphi_iterator gpi = gsi_start_phis (bb); !gsi_end_p (gpi); gsi_next (&gpi)){
         ...
         for (i = 0; i < gimple_phi_num_args (phi); i++){
            ...
            if (mtcs_ssa_coalesce_gimple_can_coalesce_p/*!gimple_can_coalesce_p*/(self,arg, res) || (e->flags & EDGE_ABNORMAL)){
               saw_copy = true;
      ...
      for (gsi = gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi)){
         ...
         switch (gimple_code (stmt)){
            case GIMPLE_ASSIGN:
            {
               tree lhs = gimple_assign_lhs (stmt);
               tree rhs1 = gimple_assign_rhs1 (stmt);
               if (gimple_assign_ssa_name_copy_p (stmt)  && mtcs_ssa_coalesce_gimple_can_coalesce_p/*!gimple_can_coalesce_p*/(self,lhs, rhs1)){
            ...
            case GIMPLE_RETURN:
            {
               ...
               if (VOID_TYPE_P (TREE_TYPE (res)) || !mtcs_gimple_expr_is_gimple_reg/*!is_gimple_reg*/(mtcsGimpleExpr,res))
                  break;
               ...
               if (map->bitint && !lhs)
                  break;
               gcc_assert (lhs);
               if (TREE_CODE (rhs1) == SSA_NAME  && mtcs_ssa_coalesce_gimple_can_coalesce_p/*!gimple_can_coalesce_p*/(self,lhs, rhs1)){
                  v1 = SSA_NAME_VERSION (lhs);
                  v2 = SSA_NAME_VERSION (rhs1);
                  if (map->bitint && !bitmap_bit_p (map->bitint, v1))
                     break;
            ...
            case GIMPLE_ASM:
            {
               ...
               for (i = 0; i < ninputs; ++i){
                  ...
                  if (mtcs_ssa_coalesce_gimple_can_coalesce_p/*!gimple_can_coalesce_p*/(self,outputs[match], input)){
                     cost = coalesce_cost (REG_BR_PROB_BASE,optimize_bb_for_size_p (bb));
                     add_coalesce (cl, v1, v2, cost);
                     bitmap_set_bit (used_in_copy, v1);
                     bitmap_set_bit (used_in_copy, v2);
                  }
...
inline hashval_t ssa_name_var_hash::hash (const_tree n)
{
   return DECL_UID (SSA_NAME_VAR (n));
}

inline int ssa_name_var_hash::equal (const tree_node *n1, const tree_node *n2)
{
...
static void populate_coalesce_list_for_outofssa (MtcsSsaCoalesce *self,coalesce_list *cl, bitmap used_in_copy)
{
   tree var;
   tree first;
   int v1, v2, cost;
   ...
   FOR_EACH_SSA_NAME (i, var, cfun){
      if (!virtual_operand_p (var)){
         ...
         if (SSA_NAME_VAR (var)  && TREE_CODE (SSA_NAME_VAR (var)) == RESULT_DECL){
            bitmap_set_bit (used_in_copy, SSA_NAME_VERSION (var));
            if (first == NULL_TREE)
               first = var;
            else{
               gcc_assert (mtcs_ssa_coalesce_gimple_can_coalesce_p/*!gimple_can_coalesce_p*/(self,var, first));
         ...
         || !VAR_P (SSA_NAME_VAR (a)))){
            ...
            if (!*slot)
               *slot = a;
               ...
               const int cost= (VAR_P (SSA_NAME_VAR (a)) || in_lto_p) ? MUST_COALESCE_COST - 1 : MUST_COALESCE_COST;
               add_coalesce (cl, SSA_NAME_VERSION (a),SSA_NAME_VERSION (*slot), cost);
...
static inline bool attempt_coalesce (var_map map, ssa_conflicts *graph, int x, int y,FILE *debug)
{
   int z;
   tree var1, var2;
   int p1, p2;
...
static void coalesce_partitions (MtcsSsaCoalesce *self,var_map map, ssa_conflicts *graph, coalesce_list *cl,
           FILE *debug)
{
   int x = 0, y = 0;
   tree var1, var2;
      ...
      FOR_EACH_EDGE (e, ei, bb->preds)
         if (e->flags & EDGE_ABNORMAL){
            ...
            for (gsi = gsi_start_phis (bb); !gsi_end_p (gsi); gsi_next (&gsi)){
               ...
               if (SSA_NAME_IS_DEFAULT_DEF (arg) && (!SSA_NAME_VAR (arg) || TREE_CODE (SSA_NAME_VAR (arg)) != PARM_DECL))
                  continue;
   ...
   while ((cost = pop_best_coalesce (cl, &x, &y)) != NO_BEST_COALESCE){
      ...
      gcc_assert (mtcs_ssa_coalesce_gimple_can_coalesce_p/*!gimple_can_coalesce_p*/(self,var1, var2));
...
static void dump_part_var_map (FILE *f, partition part, var_map map)
{
   int t;
   unsigned x, y;
   int p;
   ...
   for (x = 0; x < map->num_partitions; x++){
      ...
      if (ssa_name (p) == NULL_TREE || virtual_operand_p (ssa_name (p)))
         continue;
      ...
      for (y = 1; y < num_ssa_names; y++){
         ...
         gcc_assert (map->partition_to_base_index[q] == map->partition_to_base_index[p]);

         if (p == (int)x){
            if (t++ == 0){
               fprintf (f, "Partition %d, base %d (", x, map->partition_to_base_index[q]);
...
bool mtcs_ssa_coalesce_gimple_can_coalesce_p (MtcsSsaCoalesce *self,tree name1, tree name2)
{
   ...
   if (t1 == t2){
      ...
      bool reg1 = mtcs_func_use_register_for_decl/*!use_register_for_decl*/(mtcsFunc,name1);
      bool reg2 = mtcs_func_use_register_for_decl/*!use_register_for_decl*/(mtcsFunc,name2);
      if (reg1 != reg2)
         return false;
      ...
      || ((mtcs_mode_promote_ssa_mode/*!promote_ssa_mode*/(mtcsMode,name1, &unsigned1)
      == mtcs_mode_promote_ssa_mode/*!promote_ssa_mode*/(mtcsMode,name2, &unsigned2))
      && unsigned1 == unsigned2);
   }

   /* If alignment requirements are different, we can't coalesce.  */
   if (mtcs_mode_get_mininum_alignment/*!MINIMUM_ALIGNMENT*/(mtcsMode,t1,
   var1 ? DECL_MODE (var1) : TYPE_MODE (t1),
   var1 ? LOCAL_DECL_ALIGNMENT (var1) : TYPE_ALIGN (t1))
   != mtcs_mode_get_mininum_alignment/*!MINIMUM_ALIGNMENT*/(mtcsMode,t2,
...
static void compute_optimized_partition_bases (var_map map, bitmap used_in_copies,
               coalesce_list *cl)
{
   ...
   if (map->bitint  && flag_tree_coalesce_vars && (optimize > 1 || parts < 500))
      for (i = 0; i < (unsigned) parts; ++i){
         ...
         for (unsigned j = i + 1; j < (unsigned) parts; ++j){
            ...
            if (s1 == s2)
               continue;
            if (tree_int_cst_equal (TYPE_SIZE (TREE_TYPE (s1)), TYPE_SIZE (TREE_TYPE (s2)))){
...
static void coalesce_bitint (var_map map, ssa_conflicts *graph)
{
   ...
   for (unsigned i = 0; i < n; ++i){
      ...
      for (unsigned j = i + 1; j < n; ++j){
         ...
         if (!types_compatible_p (TREE_TYPE (s1), TREE_TYPE (s2))){
            if (!try_same_size && tree_int_cst_equal (TYPE_SIZE (TREE_TYPE (s1)),TYPE_SIZE (TREE_TYPE (s2))))
               try_same_size = true;
            continue;
         }
         int v2 = SSA_NAME_VERSION (s2);
         if (attempt_coalesce (map, graph, v1, v2, debug_file) && partition_to_var (map, i) != s1)
            break;
      }
   }
   ...
   EXECUTE_IF_SET_IN_BITMAP (map->bitint, 0, i, bi){
      ...
      if (!SSA_NAME_VAR (s))
         continue;
      if (TREE_CODE (SSA_NAME_VAR (s)) != RESULT_DECL && (TREE_CODE (SSA_NAME_VAR (s)) != PARM_DECL || !SSA_NAME_IS_DEFAULT_DEF (s)))
         continue;
      if (same_type == NULL)
...
//原型 coalesce_ssa_name tree-ssa-coalesce.h tree-ssa-coalesce.cc
void mtcs_ssa_coalesce_coalesce_ssa_name (MtcsSsaCoalesce *self,var_map map)
{
  ...
  cl = create_coalesce_list_for_region(self,map, used_in_copies);
  if (map->outofssa_p)
    populate_coalesce_list_for_outofssa(self,cl, used_in_copies);
  ...
  sort_coalesce_list(self,cl, graph, map);
  ...
  coalesce_partitions(self,map, graph, cl,((dump_flags & TDF_DETAILS) ? dump_file : NULL));
...
static void mtcsSsaCoalesceInit(MtcsSsaCoalesce *self)
{
...
{
   MtcsSsaCoalesce *self = n_slice_alloc0 (sizeof(MtcsSsaCoalesce));
   mtcs_component_set_mode((MtcsComponent *)self,mtcsMode);
   mtcsSsaCoalesceInit(self);
   return self;
}
