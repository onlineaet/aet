/*
   ...
   This program based on gcc/c-family/c-warn.c
...
static tree unwrap_c_maybe_const (tree *tp, int *walk_subtrees, void *)
{
...
static bool conversion_warning (location_t loc, tree type, tree expr, tree result)
{
  ...
  switch (TREE_CODE (expr))
    ...
    case TRUTH_NOT_EXPR:
	 ...
	 it does change the value.  */
      if (TYPE_PRECISION (type) == 1 && !TYPE_UNSIGNED (type)){
			argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=conversion_to_x_from_boolean_expression;
      }
      return true;
	...
	if (conversion_kind == UNSAFE_SIGN){
	    bool cstresult = (result && TREE_CODE_CLASS (TREE_CODE (result)) == tcc_constant);
	    if (TYPE_UNSIGNED (type)){
	       if (cstresult)
	    	  argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=conversion_from_x_to_y_changes_value_from_x_to_y;
	       else
	    	  argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=conversion_from_x_to_y_changes_the_value_of_x;
	    }else{
		   if (cstresult)
			    argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=conversion_from_x_to_y_changes_value_from_x_to_y;
		   else
	...
	if (conversion_kind == UNSAFE_SIGN)
		   argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=conversion_to_x_from_y_may_change_the_sign_of_the_result;

	else if (conversion_kind == UNSAFE_IMAGINARY)
		   argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=conversion_from_x_to_y_discards_imaginary_component;

	else
		   argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=conversion_from_x_to_y_may_change_value;
...
void aet_warnings_for_convert_and_check (location_t loc, tree type, tree expr,tree result)
{
  ...
  printf("warnings_for_convert_and_check 00\n");
  tree exprtype = TREE_TYPE (expr);

  if (TREE_CODE (expr) == INTEGER_CST && (TREE_CODE (type) == INTEGER_TYPE || TREE_CODE (type) == ENUMERAL_TYPE) && !int_fits_type_p (expr, type)){
      /* Do not diagnose overflow in a constant expression merely
	 because a conversion overflowed.  */
	  printf("warnings_for_convert_and_check 11\n");
     ...
     if (TYPE_UNSIGNED (type)){
	  /* This detects cases like converting -129 or 256 to
	     unsigned char.  */
    	  printf("warnings_for_convert_and_check 22\n");
	    if (!int_fits_type_p (expr, c_common_signed_type (type))){
    	     printf("warnings_for_convert_and_check 33\n");

	       if (cst)
			   argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=TYPE_UNSIGNED (exprtype)?
						conversion_from_x_to_y_changes_value_from_x_to_y:unsigned_conversion_from_x_to_y_changes_value_from_x_to_y;
	       else
	    	   argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=TYPE_UNSIGNED (exprtype)?
	    			   conversion_from_x_to_y_changes_the_value_of_x:unsigned_conversion_from_x_to_y_changes_the_value_of_x;
	    }else
	      conversion_warning (loc, type, expr, result);
	 }else if (!int_fits_type_p (expr, c_common_unsigned_type (type))){
    	  printf("warnings_for_convert_and_check 44\n");
          if (cst)
    		argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=overflow_in_conversion_from_x_to_y_changes_value_from_x_to_y;
          else
       	    argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=overflow_in_conversion_from_x_to_y_changes_the_value_of_z;
	 }else if (pedantic && (TREE_CODE (exprtype) != INTEGER_TYPE || TYPE_PRECISION (exprtype)!= TYPE_PRECISION (type))){
    	  printf("warnings_for_convert_and_check 55\n");
          if (cst)
    		argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=overflow_in_conversion_from_x_to_y_changes_value_from_x_to_y;
          else
       	    argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=overflow_in_conversion_from_x_to_y_changes_the_value_of_z;
	    ...
	    conversion_warning (loc, type, expr, result);
  }else if ((TREE_CODE (result) == INTEGER_CST || TREE_CODE (result) == FIXED_CST) && TREE_OVERFLOW (result)){
	  printf("warnings_for_convert_and_check 66\n");
      if (cst)
		argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=overflow_in_conversion_from_x_to_y_changes_value_from_x_to_y;
      else
   	    argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=overflow_in_conversion_from_x_to_y_changes_the_value_of_z;
...
void aet_maybe_warn_bool_compare (location_t loc, enum tree_code code, tree op0,tree op1)
{
  if (TREE_CODE_CLASS (code) != tcc_comparison)
    return;
...
static tree expr_from_macro_expansion_r (tree *tp, int *, void *)
{
...
static void do_warn_duplicated_branches (tree expr)
{
...
tree aet_do_warn_duplicated_branches_r (tree *tp, int *, void *)
{
...
void aet_warn_for_multistatement_macros (location_t body_loc, location_t next_loc,location_t guard_loc, enum rid keyword)
{
...
static tree check_alignment_of_packed_member (tree type, tree field, bool rvalue)
{
...
static tree check_address_or_pointer_of_packed_member (tree type, tree rhs)
{
  bool rvalue = true;
  bool indirect = false;
      ...
      if (TYPE_PACKED (rhstype))
	{
	  ...
	  if (rhs_align < type_align)
			      ...
			      rhstype, rhs_align, type, type_align))
		{
		  ...
		  argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=converting_a_packed_x_pointer_alignment_y_to_a_x_pointer_alignment_y_may_result_in_an_unaligned_pointer_value;
...
static void check_and_warn_address_or_pointer_of_packed_member (tree type, tree rhs)
{
  bool nop_p = false;
  tree orig_rhs;
  ...
  if (TREE_CODE (rhs) == COND_EXPR)
    {
      /* Check the THEN path.  */
      check_and_warn_address_or_pointer_of_packed_member(type, TREE_OPERAND (rhs, 1));

      /* Check the ELSE path.  */
      check_and_warn_address_or_pointer_of_packed_member(type, TREE_OPERAND (rhs, 2));
    }
  else
    {
      if (nop_p)
	{
      ...
      tree context= check_address_or_pointer_of_packed_member (type, rhs);
      if (context){
         argsFuncsInfo.warns[argsFuncsInfo.warnCount++]=taking_address_of_packed_member_of_x_may_result_in_an_unaligned_pointer_value;
	  }
   }
}
