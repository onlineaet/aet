...
function_info::init_function_data ()
{
   ...
   int maxregs = mtcs_func_max_reg_num/*!max_reg_num*/(mtcsFunc);//这是主机的maxregs 不是ptx的

   m_next_artificial_uid = -1;
   m_next_phi_uid = 0;
   m_num_regs = maxregs/*!max_reg_num ()*/;
   ...
   m_free_phis = nullptr;
   m_clobbered_by_calls.count=mtcs_reg_get_hard_reg_element_count(mtcsReg);
}
...
//因为引用到mtcsreg 移到.c中实现.变成非内联函数,如果还是内联函数会报找不到引用的错误。
bool function_info::is_single_dominating_def (const set_info *set) const
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);

   return (set->is_first_def ()  && set->is_last_def ()
      && (!mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,set->regno ())
      || !mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&m_clobbered_by_calls, set->regno ())));
}

// Print a description of FUNCTION to PP.
void mtcs_rtl_ssa::pp_function (pretty_printer *pp, const function_info *function)
{
  function->print (pp);
}

// Print a description of FUNCTION to FILE.
void dump (FILE *file, const function_info *function)
{
