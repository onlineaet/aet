...
//代替 df_worklist_dataflow
static void dfWorklistDataFlow_cb(struct dataflow *dataflow,
                      ...
                      int n_blocks)
{
   ...
   mtcs_dfcore_df_worklist_dataflow/*!df_worklist_dataflow*/(mtcsDfcore,dataflow,blocks_to_consider,blocks_in_postorder,n_blocks);
}
...
static void printbb(basic_block block)
{
   ...
   FOR_BB_INSNS (block, insn){
      if (!INSN_P (insn))
         continue;
      n_debug("mtcsdfproblems.c 打印块中的指令 i:%d block:%p index:%d flags:%d insn:%p\n",i++,block,block->index,block->flags,insn);
...
static void printdl(MtcsDfproblems *self,basic_block bb)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);

   bitmap local_live = BITMAP_ALLOC (&mtcsDfcore->df_bitmap_obstack);
   ...
   FOR_BB_INSNS_REVERSE (bb, insn)
     if (INSN_P (insn)){
        FOR_EACH_INSN_DEF (def, insn)
          n_debug("mtcsdfproblems.c printdl bb:%p insn:%p %d\n",bb,insn,bitmap_bit_p (local_live, DF_REF_REGNO (def)));
     }
   BITMAP_FREE (local_live);

}

static void print_local_live(MtcsDfproblems *self)
{
   if(!cfun )
      return;
   basic_block bb;
   ...
   for (ref = refs; ref; ref = DF_REF_NEXT_REG (ref)){
     // gcc_assert (!DF_REF_IS_REG_MARKED (ref));
      ...
      if(DF_REF_IS_REG_MARKED (ref))
         n_debug("mtcsdfproblems.c DF_REF_IS_REG_MARKED (ref)=true 错误\n");

      /* If there are no def-use or use-def chains, make sure that all
      of the chains are clear.  */
      n_debug("mtcsdfproblems.c df_reg_chain_mark 00 ref:%p flags:%d is_def:%d is_eq_use:%d df_chain:%p regno:%d %d\n",
...
static void dfScanVerify (struct function *fn)
{
   ...
   for (i = 0; i < DF_REG_SIZE (df); i++){
      fprintf(stderr,"mtcsdfproblems.c dfScanVerify --- i:%d %d pass:%s\n",i,DF_REG_DEF_COUNT (i),passName);
...
static void df_rd_free_bb_info_cb(basic_block bb ATTRIBUTE_UNUSED,
          void *vbb_info)
{
...
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfproblems *self=mtcs_target_get_dfproblems(mtcsTarget);
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   ...
   if (df_rd->problem_data){
      ...
      bitmap_initialize (&problem_data->sparse_invalidated_by_eh, &problem_data->rd_bitmaps);
      bitmap_initialize (&problem_data->dense_invalidated_by_eh,  &problem_data->rd_bitmaps);
   }

   mtcs_dfcore_df_grow_bb_info/*!df_grow_bb_info*/(mtcsDfcore,df_rd);
...
/* Add the effect of the top artificial defs of BB to the reaching definitions
   bitmap LOCAL_RD.  */
static void df_rd_simulate_artificial_defs_at_top (MtcsDfproblems *self,basic_block bb, bitmap local_rd)
{
   ...
   FOR_EACH_ARTIFICIAL_DEF (def, bb_index)
      if (DF_REF_FLAGS (def) & DF_REF_AT_TOP){
         ...
         if (!(DF_REF_FLAGS (def) & (DF_REF_PARTIAL | DF_REF_CONDITIONAL)))
            bitmap_clear_range (local_rd,DF_DEFS_BEGIN (dregno),DF_DEFS_COUNT (dregno));
          ...
          bitmap local_rd)
{
   ...
   FOR_EACH_INSN_DEF (def, insn){
      ...
      || (dregno >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))){
         if (!(DF_REF_FLAGS (def) & (DF_REF_PARTIAL | DF_REF_CONDITIONAL)))
            bitmap_clear_range (local_rd,DF_DEFS_BEGIN (dregno),DF_DEFS_COUNT (dregno));
...
/* Process a list of DEFs for df_rd_bb_local_compute.  This is a bit
   ...
   of kill sets.   */
static void df_rd_bb_local_compute_process_def (MtcsDfproblems *self,class df_rd_bb_info *bb_info,
                df_ref def,int top_flag)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   for (; def; def = DF_REF_NEXT_LOC (def)){
      if (top_flag == (DF_REF_FLAGS (def) & DF_REF_AT_TOP)){
         ...
         || (regno >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))){
            ...
            effect.  */
            if (!bitmap_bit_p (&self->seen_in_block, regno)){
               /* The first def for regno in insn gets to knock out the
               defs from other instructions.  */
               if ((!bitmap_bit_p (&self->seen_in_insn, regno))
               /* If the def is to only part of the reg, it does
               not kill the other defs that reach here.  */
               && (!(DF_REF_FLAGS (def) & (DF_REF_PARTIAL | DF_REF_CONDITIONAL | DF_REF_MAY_CLOBBER)))){
               ...
               bitmap_set_bit (&self->seen_in_insn, regno);
...
static void df_rd_bb_local_compute (MtcsDfproblems *self,unsigned int bb_index)
{
   basic_block bb = BASIC_BLOCK_FOR_FN (cfun, bb_index);
   class df_rd_bb_info *bb_info = df_rd_get_bb_info (bb_index);
   rtx_insn *insn;

   bitmap_clear (&self->seen_in_block);
   bitmap_clear (&self->seen_in_insn);

   /* Artificials are only hard regs.  */
   if (!(df->changeable_flags & DF_NO_HARD_REGS))
      df_rd_bb_local_compute_process_def (self,bb_info, df_get_artificial_defs (bb_index), 0);
   ...
   FOR_BB_INSNS_REVERSE (bb, insn){
      ...
      df_rd_bb_local_compute_process_def(self,bb_info,DF_INSN_UID_DEFS (uid), 0);
      ...
      lost.  */
      bitmap_ior_into (&self->seen_in_block, &self->seen_in_insn);
      bitmap_clear (&self->seen_in_insn);
   }
   ...
   the start.  */
   if (!(df->changeable_flags & DF_NO_HARD_REGS))
      df_rd_bb_local_compute_process_def(self,bb_info, df_get_artificial_defs (bb_index),DF_REF_AT_TOP);
}
...
static void df_rd_local_compute (bitmap all_blocks)
{
   ...
   unsigned int bb_index;
   bitmap_iterator bi;
   class df_rd_problem_data *problem_data = (class df_rd_problem_data *) df_rd->problem_data;
   ...
   bitmap_initialize (&self->seen_in_block, &mtcsDfcore->df_bitmap_obstack);
   bitmap_initialize (&self->seen_in_insn, &mtcsDfcore->df_bitmap_obstack);

   mtcs_dfscan_df_maybe_reorganize_def_refs/*!df_maybe_reorganize_def_refs*/(mtcsDfscan,DF_REF_ORDER_BY_REG);

   EXECUTE_IF_SET_IN_BITMAP (all_blocks, 0, bb_index, bi){
      df_rd_bb_local_compute(self,bb_index);
   }
   ...
   if (!(df->changeable_flags & DF_NO_HARD_REGS))
      for (unsigned int regno = 0; regno < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); ++regno)
         if (abi/*!eh_edge_abi*/->clobbers_full_reg_p (regno)){
   ...
   bitmap_release (&self->seen_in_block);
   bitmap_release (&self->seen_in_insn);
}
...
static bool df_rd_confluence_n (edge e)
{
   ...
   if (e->flags & EDGE_EH){
      ...
      EXECUTE_IF_SET_IN_BITMAP (sparse_invalidated, 0, regno, bi){
         bitmap_clear_range (tmp, DF_DEFS_BEGIN (regno), DF_DEFS_COUNT (regno));
      }
...
static bool df_rd_transfer_function (int bb_index)
{
   ...
   if (df->changeable_flags & DF_RD_PRUNE_DEAD_DEFS){
      ...
      bitmap live_defs = BITMAP_ALLOC (&mtcsDfcore->df_bitmap_obstack);
      unsigned int regno;
      bitmap_iterator bi;

      EXECUTE_IF_SET_IN_BITMAP (regs_live_out, 0, regno, bi)
         bitmap_set_range (live_defs,DF_DEFS_BEGIN (regno),DF_DEFS_COUNT (regno));
...
static void df_rd_start_dump (FILE *file)
{
   ...
   for (regno = 0; regno < m; regno++)
      if (DF_DEFS_COUNT (regno))
         fprintf (file, "%d[%d,%d] ", regno,DF_DEFS_BEGIN (regno), DF_DEFS_BEGIN (regno) + DF_DEFS_COUNT (regno) - 1);
   fprintf (file, "\n");
}


static void df_rd_dump_defs_set (MtcsDfproblems *self,bitmap defs_set, const char *prefix, FILE *file)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   bitmap_initialize (&tmp, &mtcsDfcore->df_bitmap_obstack);
   for (regno = 0; regno < m; regno++){
      if (mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,regno)  && (df->changeable_flags & DF_NO_HARD_REGS))
         continue;
...
static void df_rd_top_dump (basic_block bb, FILE *file)
{
   ...
   df_rd_dump_defs_set(self,&bb_info->in, ";; rd  in  ", file);
   df_rd_dump_defs_set(self,&bb_info->gen, ";; rd  gen ", file);
   df_rd_dump_defs_set(self,&bb_info->kill, ";; rd  kill", file);
}
...
static void df_rd_bottom_dump (basic_block bb, FILE *file)
{
   ...
   df_rd_dump_defs_set(self,&bb_info->out, ";; rd  out ", file);
}
...
static const struct df_problem problem_RD =
{
  ...
  df_rd_free_bb_info_cb,         /* Free basic block info.  */
  df_rd_local_compute,        /* Local compute function.  */
  df_rd_init_solution,        /* Init the solution specific data.  */
  dfWorklistDataFlow_cb/*!df_worklist_dataflow*/,       /* Worklist solver.  */
...
void mtcs_dfproblems_df_rd_add_problem (MtcsDfproblems *self)
{
   ...
   mtcs_dfcore_df_add_problem/*!df_add_problem*/(mtcsDfcore,&problem_RD);
}
...
/* Free basic block info.  */
static void df_lr_free_bb_info (basic_block bb ATTRIBUTE_UNUSED, void *vbb_info)
{
   class df_lr_bb_info *bb_info = (class df_lr_bb_info *) vbb_info;
   if (bb_info){
      n_debug("mtcsdfproblems.c df_lr_free_bb_info %p %p %p %p\n",&bb_info->use,&bb_info->def,&bb_info->in,&bb_info->out);
...
/* Allocate or reset bitmaps for DF_LR blocks. The solution bits are
   not touched unless the block is new.  */
static void df_lr_alloc (bitmap all_blocks ATTRIBUTE_UNUSED)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfproblems *self=mtcs_target_get_dfproblems(mtcsTarget);
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   ...
   n_debug("mtcsdfproblems.c df_lr_alloc 分配 00 \n");
   mtcs_dfcore_df_grow_bb_info/*!df_grow_bb_info*/(mtcsDfcore,df_lr);
   if (df_lr->problem_data)
      problem_data = (struct df_lr_problem_data *) df_lr->problem_data;
   else{
      n_debug("mtcsdfproblems.c df_lr_alloc 分配 11 \n");
...
static void df_lr_bb_local_compute (MtcsDfproblems *self,unsigned int bb_index)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsDfscan *mtcsDfscan=mtcs_target_get_dfscan(mtcsTarget);
   ...
   scan.  */
   if (!df_live)
      mtcs_dfscan_df_recompute_luids/*!df_recompute_luids*/(mtcsDfscan,bb);
}

/* Compute local live register info for each basic block within BLOCKS.  */
static void df_lr_local_compute (bitmap all_blocks ATTRIBUTE_UNUSED)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfproblems *self=mtcs_target_get_dfproblems(mtcsTarget);
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   bitmap_iterator bi;
   n_debug("mtcsdfproblems.c df_lr_local_compute 00 清除  df->hardware_regs_used reload_completed:%d\n",reload_completed);
   bitmap_clear (&df->hardware_regs_used);
   /* The all-important stack pointer must always be live.  */
   bitmap_set_bit (&df->hardware_regs_used, mtcs_reg_get_stack_pointer_regnum/*!STACK_POINTER_REGNUM*/(mtcsReg));
   ...
   if (!reload_completed){
      unsigned int pic_offset_table_regnum = mtcs_reg_get_pic_offset_table_regnum/*!PIC_OFFSET_TABLE_REGNUM*/(mtcsReg);
      /* Any reference to any pseudo before reload is a potential
      reference of the frame pointer.  */
      bitmap_set_bit (&df->hardware_regs_used, mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg));

      /* Pseudos with argument area equivalences may require
      reloading via the argument pointer.  */
      if (mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg)
      ...
      && mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[mtcs_reg_get_arg_pointer_regnum/*!ARG_POINTER_REGNUM*/(mtcsReg)])
         bitmap_set_bit (&df->hardware_regs_used, mtcs_reg_get_arg_pointer_regnum/*!ARG_POINTER_REGNUM*/(mtcsReg));

      /* Any constant, or pseudo with constant equivalences, may
      require reloading from memory using the pic register.  */
      if (pic_offset_table_regnum != INVALID_REGNUM  && mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[pic_offset_table_regnum])
   ...
   EXECUTE_IF_SET_IN_BITMAP (df_lr->out_of_date_transfer_functions, 0, bb_index, bi){
      if (bb_index == EXIT_BLOCK){
         ...
         df_lr_bb_local_compute(self,bb_index);
   }
...
static void df_lr_init (bitmap all_blocks)
{
   unsigned int bb_index;
   bitmap_iterator bi;
   n_debug("mtcsdfproblems.c df_lr_init 类型是 dflr的问题 的初始化\n");
   EXECUTE_IF_SET_IN_BITMAP (all_blocks, 0, bb_index, bi){
      class df_lr_bb_info *bb_info = df_lr_get_bb_info (bb_index);
...
static void print_df_lr(int bb_index ,int lrbbType)
{
   return;
   bitmap_obstack obstack;
   bitmap_obstack_initialize (&obstack);

   bitmap local_live = BITMAP_ALLOC (&obstack);
   class df_lr_bb_info *bb_info = df_lr_get_bb_info (bb_index);
   basic_block bb = BASIC_BLOCK_FOR_FN (cfun, bb_index);

   char *type="OUT";
   if(lrbbType==0){
      bitmap_copy (local_live, &bb_info->out);
   }else if(lrbbType==1){
      bitmap_copy (local_live, &bb_info->in);
      type="IN";
   }else if(lrbbType==2){
      bitmap_copy (local_live, &bb_info->use);
      type="USE";
   }else if(lrbbType==3){
      bitmap_copy (local_live, &bb_info->def);
      type="DEF";
   }
   rtx_insn *insn;
   df_ref def;
   ...
   FOR_BB_INSNS_REVERSE (bb, insn)
      if (INSN_P (insn)){
         FOR_EACH_INSN_DEF (def, insn)
            fprintf(stderr,"打印 df_lr_get_bb_info 类型:%s bb:%p insn:%p %d regno:%d\n",
                  type,bb,insn,bitmap_bit_p (local_live, DF_REF_REGNO (def)),DF_REF_REGNO (def));
      }
   BITMAP_FREE (local_live);
...
static void df_lr_confluence_0 (basic_block bb)
{
   ...
   n_debug("mtcsdfproblems.c df_lr_confluence_0 df_problem中的函数域 con_fun_0  这是RTL不被dce删除的关键 bb:%p op1:%p exitbb:%p\n",
         bb,op1,EXIT_BLOCK_PTR_FOR_FN (cfun));
...
static bool df_lr_confluence_n (edge e)
{
   ...
   bool changed = false;
   n_debug("mtcsdfproblems.c df_lr_confluence_n 00 df_problem中的函数域 con_fun_n  这是RTL不被dce删除的关键 bb:%p op1:%p exitbb:%p\n",
         e->src,op1,EXIT_BLOCK_PTR_FOR_FN (cfun));
   print_df_lr(e->src->index,0);
   print_df_lr(e->dest->index,1);
   ...
   if (e->flags & EDGE_EH){
      n_debug("mtcsdfproblems.c df_lr_confluence_n 11 df_problem中的函数域 con_fun_n  这是RTL不被dce删除的关键 bb:%p op1:%p exitbb:%p\n",
            e->src,op1,EXIT_BLOCK_PTR_FOR_FN (cfun));
      mtcs_bitmap_view<HardRegSet/*!HARD_REG_SET*/> eh_kills (abi/*!eh_edge_abi*/->full_reg_clobbers ());
      changed = bitmap_ior_and_compl_into (op1, op2, eh_kills);
   }else{
      n_debug("mtcsdfproblems.c df_lr_confluence_n 22 e->src:%p %d op1:%p exitbb:%p e->src->index)->out 位图\n",
            e->src,e->src->index,op1,EXIT_BLOCK_PTR_FOR_FN (cfun));
//      if(n_log_is_debug_file(NULL,NULL)){
//         dump_bitmap (stderr, op1);
//         n_debug("mtcsdfproblems.c (e->src->index)->in 00\n");
//         dump_bitmap (stderr, &df_lr_get_bb_info (e->src->index)->in);
//         n_debug("mtcsdfproblems.c df_lr_confluence_n 33 e->dest:%p %d (e->dest->index)->in  位图\n", e->dest, e->dest->index);
//         dump_bitmap (stderr, op2);
//      }

      changed = bitmap_ior_into (op1, op2);

//      if(n_log_is_debug_file(NULL,NULL)){
//         n_debug("mtcsdfproblems.c df_lr_confluence_n 44 求交集后的  e->src->index)->out  位图\n");
//         dump_bitmap (stderr, op1);
//         n_debug("mtcsdfproblems.c  (e->src->index)->in 11 \n");
//         dump_bitmap (stderr, &df_lr_get_bb_info (e->src->index)->in);
//      }
...
//   if(n_log_is_debug_file(NULL,NULL)){
//      n_debug("mtcsdfproblems.c df_lr_confluence_n 55 与 df->hardware_regs_used求交集后e->src->index)->out 位图\n");
//      dump_bitmap (stderr, op1);
//      n_debug("mtcsdfproblems.c  df_lr_confluence_n 66 &df->hardware_regs_used的位图\n");
//      dump_bitmap (stderr, &df->hardware_regs_used);
//   }
   print_df_lr(e->src->index,0);
   print_df_lr(e->dest->index,1);
   return changed;
...
//实现   if (dflow->problem->finalize_fun) 中的 finalize_fun方法
static void df_lr_finalize (bitmap all_blocks)
{
   ...
   n_debug("mtcsdfproblems.c df_lr_finalize 00aa changeable_flags: %d\n",df->changeable_flags & DF_LR_RUN_DCE);
...
{
   struct df_lr_problem_data *problem_data = (struct df_lr_problem_data *) df_lr->problem_data;
   if (df_lr->block_info){
...
static void df_lr_top_dump (basic_block bb, FILE *file)
{
   ...
   fprintf (file, ";; lr  in  \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &bb_info->in);
   if (df_lr->problem_data){
      problem_data = (struct df_lr_problem_data *)df_lr->problem_data;
      if (problem_data->in){
         fprintf (file, ";;  old in  \t");
         mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &problem_data->in[bb->index]);
      }
   }
   fprintf (file, ";; lr  use \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &bb_info->use);
   fprintf (file, ";; lr  def \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &bb_info->def);
}
...
static void df_lr_bottom_dump (basic_block bb, FILE *file)
{
   ...
   fprintf (file, ";; lr  out \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &bb_info->out);
   if (df_lr->problem_data){
      problem_data = (struct df_lr_problem_data *)df_lr->problem_data;
      if (problem_data->out){
         fprintf (file, ";;  old out  \t");
         mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &problem_data->out[bb->index]);
      }
   }
}
...
static void df_lr_verify_solution_end (void)
{
   ...
   problem_data = (struct df_lr_problem_data *)df_lr->problem_data;
   n_debug("mtcsdfproblems.cc df_lr_verify_solution_end 00 out:%p\n",problem_data->out);
...
static const struct df_problem problem_LR =
{
  ...
  dfWorklistDataFlow_cb,      /*!df_worklist_dataflow dataflow_fun*/
...
static void print_df_lr_bb_bitmap()
{
   return;
   basic_block bb;
   FOR_ALL_BB_FN (bb, cfun){
      class df_lr_bb_info *bb_info = df_lr_get_bb_info (bb->index);
      if(n_log_is_debug_file(NULL,NULL)){
         n_debug("mtcsdfproblems.c print_df_lr_bb_bitmap 00 bb:%p index:%p\n",bb,bb->index);
         dump_bitmap (stderr, &bb_info->out);
         dump_bitmap (stderr, &bb_info->in);
         dump_bitmap (stderr, &bb_info->use);
         dump_bitmap (stderr, &bb_info->def);
      }
   }
}
...
static void df_lr_dce_finalize (bitmap all_blocks)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfproblems *self=mtcs_target_get_dfproblems(mtcsTarget);
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   MtcsDce *mtcsDce=mtcs_target_get_dce(mtcsTarget);
   n_debug("mtcsdfproblems.c df_lr_dce_finalize 00 changeable_flags:%d\n",(df->changeable_flags & DF_LR_RUN_DCE));

   if (!(df->changeable_flags & DF_LR_RUN_DCE))
      return;
   n_debug("mtcsdfproblems.c df_lr_dce_finalize 11 changeable_flags:%d \n",(df->changeable_flags & DF_LR_RUN_DCE));
      print_df_lr_bb_bitmap();
   /* Also clears df_lr_dce->solutions_dirty.  */
   mtcs_dce_run_fast_df_dce/*!run_fast_df_dce*/(mtcsDce);
   n_debug("mtcsdfproblems.c df_lr_dce_finalize 22 changeable_flags:%d \n",(df->changeable_flags & DF_LR_RUN_DCE));
   dfScanVerify(cfun);
      print_df_lr_bb_bitmap();
   ...
   if (df_lr->solutions_dirty){
      ...
      n_debug("mtcsdfproblems.c df_lr_dce_finalize 33 changeable_flags:%d\n",(df->changeable_flags & DF_LR_RUN_DCE));

      dfScanVerify(cfun);

      mtcs_dfcore_df_worklist_dataflow/*!df_worklist_dataflow*/(mtcsDfcore,df_lr, all_blocks, df->postorder, df->n_blocks);
      df_lr_finalize (all_blocks);
      mtcs_dfcore_df_set_flags/*!df_set_flags*/(mtcsDfcore,DF_LR_RUN_DCE);
      n_debug("mtcsdfproblems.c df_lr_dce_finalize 44 changeable_flags:%d\n",(df->changeable_flags & DF_LR_RUN_DCE));
...
void mtcs_dfproblems_df_lr_add_problem (MtcsDfproblems *self)
{
   ...
   mtcs_dfcore_df_add_problem/*!df_add_problem*/(mtcsDfcore,&problem_LR_DCE);
   /* These will be initialized when df_scan_blocks processes each
   block.  */
   df_lr->out_of_date_transfer_functions = BITMAP_ALLOC (&mtcsDfcore->df_bitmap_obstack);
}
...
void mtcs_dfproblems_df_lr_verify_transfer_functions (MtcsDfproblems *self)
{
   ...
   FOR_ALL_BB_FN (bb, cfun){
      ...
      if (bb_info){
         ...
         changed.  */
         if (!bitmap_bit_p (df_lr->out_of_date_transfer_functions,bb->index)){
            bitmap_copy (&saved_def, &bb_info->def);
            bitmap_copy (&saved_use, &bb_info->use);
            bitmap_clear (&bb_info->def);
            bitmap_clear (&bb_info->use);

            df_lr_bb_local_compute(self,bb->index);
         ...
         gcc_assert (bitmap_bit_p (df_lr->out_of_date_transfer_functions,bb->index));
      }
   ...
   gcc_assert (!bitmap_intersect_compl_p (df_lr->out_of_date_transfer_functions, &all_blocks));
...
/* Free basic block info.  */
static void df_live_free_bb_info (basic_block bb ATTRIBUTE_UNUSED,void *vbb_info)
{
   class df_live_bb_info *bb_info = (class df_live_bb_info *) vbb_info;
   n_debug("mtcsdfproblems.cc df_live_free_bb_info bb_info:%p\n",bb_info);
   if (bb_info){
      bitmap_clear (&bb_info->gen);
      bitmap_clear (&bb_info->kill);
      bitmap_clear (&bb_info->in);
...
/* Allocate or reset bitmaps for DF_LIVE blocks. The solution bits are
   not touched unless the block is new.  */
static void df_live_alloc (bitmap all_blocks ATTRIBUTE_UNUSED)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfproblems *self=mtcs_target_get_dfproblems(mtcsTarget);
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   ...
   if (df_live->problem_data){
      ...
      bitmap_initialize (&self->df_live_scratch, &problem_data->live_bitmaps);
   }

   mtcs_dfcore_df_grow_bb_info/*!df_grow_bb_info*/(mtcsDfcore,df_live);
   ...
   df_live->optional_p = (mtcsOptionsItem->x_optimize <= 1);
}
...
static void df_live_bb_local_compute (MtcsDfproblems *self,unsigned int bb_index)
{
   basic_block bb = BASIC_BLOCK_FOR_FN (cfun, bb_index);
   class df_live_bb_info *bb_info = df_live_get_bb_info (bb_index);
   rtx_insn *insn;
   ...
   FOR_BB_INSNS (bb, insn){
      ...
      n_debug("mtcsdfproblems.c df_live_bb_local_compute 11 bb_info:%p\n",bb_info);

      /* Inserting labels does not always trigger the incremental
      rescanning.  */
      if (!insn_info){
         gcc_assert (!INSN_P (insn));
         n_debug("mtcsdfproblems.c df_live_bb_local_compute 22 bb_info:%p\n",bb_info);
      ...
      FOR_EACH_INSN_INFO_DEF (def, insn_info){
         unsigned int regno = DF_REF_REGNO (def);
         n_debug("mtcsdfproblems.c df_live_bb_local_compute 33 bb_info:%p regno:%d\n",bb_info,regno);
         if (DF_REF_FLAGS_IS_SET (def,DF_REF_PARTIAL | DF_REF_CONDITIONAL)){
            n_debug("mtcsdfproblems.c df_live_bb_local_compute 44 bb_info:%p\n",bb_info);
...
static void df_live_local_compute (bitmap all_blocks ATTRIBUTE_UNUSED)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfproblems *self=mtcs_target_get_dfproblems(mtcsTarget);
   MtcsDfscan *mtcsDfscan=mtcs_target_get_dfscan(mtcsTarget);
   ...
   unsigned int bb_index;
   bitmap_iterator bi;
   mtcs_dfscan_df_grow_insn_info/*!df_grow_insn_info*/(mtcsDfscan);
   EXECUTE_IF_SET_IN_BITMAP (df_live->out_of_date_transfer_functions,0, bb_index, bi){
      df_live_bb_local_compute(self,bb_index);
   }
...
static bool df_live_transfer_function (int bb_index)
{
   ...
   bitmap_and (&self->df_live_scratch, gen, &bb_lr_info->out);
   /* No register may reach a location where it is not used.  Thus
   we trim the rr result to the places where it is used.  */
   bitmap_and_into (in, &bb_lr_info->in);

   return bitmap_ior_and_compl (out, &self->df_live_scratch, in, kill);
}
...
static void df_live_free (void)
{
   ...
   struct df_live_problem_data *problem_data  = (struct df_live_problem_data *) df_live->problem_data;
   if (df_live->block_info){
      df_live->block_info_size = 0;
      free (df_live->block_info);
      df_live->block_info = NULL;
      bitmap_release (&self->df_live_scratch);
...
static void df_live_top_dump (basic_block bb, FILE *file)
{
   ...
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &bb_info->gen);
   fprintf (file, ";; live  kill\t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &bb_info->kill);
}
...
void mtcs_dfproblems_df_live_add_problem (MtcsDfproblems *self)
{
   ...
   mtcs_dfcore_df_add_problem/*!df_add_problem*/(mtcsDfcore,&problem_LIVE);
   /* These will be initialized when df_scan_blocks processes each
   block.  */
   df_live->out_of_date_transfer_functions = BITMAP_ALLOC (&mtcsDfcore->df_bitmap_obstack);
}
...
void mtcs_dfproblems_df_live_set_all_dirty (MtcsDfproblems *self)
{
   basic_block bb;
   FOR_ALL_BB_FN (bb, cfun)
      bitmap_set_bit (df_live->out_of_date_transfer_functions,bb->index);
}
...
void mtcs_dfproblems_df_live_verify_transfer_functions (MtcsDfproblems *self)
{
   ...
   FOR_ALL_BB_FN (bb, cfun){
      ...
      if (bb_info){
         ...
         new ones to see if the transfer functions have
         changed.  */
         if (!bitmap_bit_p (df_live->out_of_date_transfer_functions,bb->index)){
            bitmap_copy (&saved_gen, &bb_info->gen);
            bitmap_copy (&saved_kill, &bb_info->kill);
            bitmap_clear (&bb_info->gen);
            bitmap_clear (&bb_info->kill);

            df_live_bb_local_compute(self,bb->index);
         ...
         the list of dirty blocks or else some one has added a
         block behind our backs. */
         gcc_assert (bitmap_bit_p (df_live->out_of_date_transfer_functions,bb->index));
      }
      /* Make sure no one created a block without following
      procedures.  */
      gcc_assert (df_scan_get_bb_info (bb->index));
   ...
   /* Make sure there are no dirty bits in blocks that have been deleted.  */
   gcc_assert (!bitmap_intersect_compl_p (df_live->out_of_date_transfer_functions, &all_blocks));
...
static void df_mir_free_bb_info (basic_block bb ATTRIBUTE_UNUSED,void *vbb_info)
{
...
static void df_mir_alloc (bitmap all_blocks)
{
   ...
   mtcs_dfcore_df_grow_bb_info/*!df_grow_bb_info*/(mtcsDfcore,df_mir);
...
/* Compute local uninitialized register info for basic block BB.  */
static void df_mir_bb_local_compute (MtcsDfproblems *self,unsigned int bb_index)
{
   basic_block bb = BASIC_BLOCK_FOR_FN (cfun, bb_index);
   class df_mir_bb_info *bb_info = df_mir_get_bb_info (bb_index);
   rtx_insn *insn;
   ...
   FOR_BB_INSNS (bb, insn){
      ...
      luid++;
      mtcs_dfproblems_df_mir_simulate_one_insn/*!df_mir_simulate_one_insn*/(self,bb, insn, &bb_info->kill, &bb_info->gen);
   }
}
...
static void df_mir_local_compute (bitmap all_blocks)
{
   ...
   mtcs_dfscan_df_grow_insn_info/*!df_grow_insn_info*/(mtcsDfscan);
   EXECUTE_IF_SET_IN_BITMAP (all_blocks, 0, bb_index, bi){
      df_mir_bb_local_compute(self,bb_index);
   }
}
...
{
   struct df_mir_problem_data *problem_data = (struct df_mir_problem_data *) df_mir->problem_data;
...
void mtcs_dfproblems_df_mir_add_problem (MtcsDfproblems *self)
{
   ...
   mtcs_dfcore_df_add_problem/*!df_add_problem*/(mtcsDfcore,&problem_MIR);
   /* These will be initialized when df_scan_blocks processes each
   block.  */
   df_mir->out_of_date_transfer_functions = BITMAP_ALLOC (&mtcsDfcore->df_bitmap_obstack);
}
...
{
   while (from_ref){
      mtcs_dfproblems_df_chain_create/*!df_chain_create*/(self,to_ref, from_ref->ref);
...
static void df_chain_alloc (bitmap all_blocks ATTRIBUTE_UNUSED)
{
   df_chain_remove_problem ();
...
static void df_chain_reset (bitmap blocks_to_clear ATTRIBUTE_UNUSED)
{
   df_chain_remove_problem ();
}

/* Create the chains for a list of USEs.  */
static void df_chain_create_bb_process_use (MtcsDfproblems *self,bitmap local_rd,df_ref use, int top_flag)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   for (; use; use = DF_REF_NEXT_LOC (use)){
      ...
      if ((!(df->changeable_flags & DF_NO_HARD_REGS))
      || (uregno >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))){
         ...
         if (count){
            if (top_flag == (DF_REF_FLAGS (use) & DF_REF_AT_TOP)){
               ...
               EXECUTE_IF_SET_IN_BITMAP (local_rd, first_index, def_index, bi){
                  ...
                  if (df_chain_problem_p (DF_DU_CHAIN))
                     mtcs_dfproblems_df_chain_create/*!df_chain_create*/(self,def, use);
                  if (df_chain_problem_p (DF_UD_CHAIN))
                     mtcs_dfproblems_df_chain_create/*!df_chain_create*/(self,use, def);
               }
            }
         }
      }
...
static void df_chain_create_bb (MtcsDfproblems *self,unsigned int bb_index)
{
   basic_block bb = BASIC_BLOCK_FOR_FN (cfun, bb_index);
   class df_rd_bb_info *bb_info = df_rd_get_bb_info (bb_index);
   rtx_insn *insn;
   ...
   /* Artificials are only hard regs.  */
   if (!(df->changeable_flags & DF_NO_HARD_REGS))
      df_chain_create_bb_process_use(self,&cpy,df_get_artificial_uses (bb->index),DF_REF_AT_TOP);
#endif
   ...
   FOR_BB_INSNS (bb, insn)
      if (INSN_P (insn)){
         ...
         in the cpy vector.  */
         df_chain_create_bb_process_use(self,&cpy, DF_INSN_UID_USES (uid), 0);
         if (df->changeable_flags & DF_EQ_NOTES)
            df_chain_create_bb_process_use(self,&cpy, DF_INSN_UID_EQ_USES (uid), 0);

         /* Since we are going forwards, process the defs second.  */
         mtcs_dfproblems_df_rd_simulate_one_insn/*!df_rd_simulate_one_insn*/(self,bb, insn, &cpy);
      }

   /* Create the chains for the artificial uses of the hard registers
   at the end of the block.  */
   if (!(df->changeable_flags & DF_NO_HARD_REGS))
      df_chain_create_bb_process_use(self,&cpy,df_get_artificial_uses (bb->index),0);
...
static void df_chain_finalize (bitmap all_blocks)
{
   ...
   bitmap_iterator bi;
   EXECUTE_IF_SET_IN_BITMAP (all_blocks, 0, bb_index, bi){
      df_chain_create_bb(self,bb_index);
   }
}
...
static void df_chain_bb_dump (basic_block bb, FILE *file, bool top)
{
   ...
   if (df_chain_problem_p (DF_UD_CHAIN)){
      ...
      fprintf (file,";;  UD chains for artificial uses at %s\n",top ? "top" : "bottom");
      FOR_EACH_ARTIFICIAL_USE (use, bb->index)
         if ((top && (DF_REF_FLAGS (use) & DF_REF_AT_TOP)) || (!top && !(DF_REF_FLAGS (use) & DF_REF_AT_TOP))){
            fprintf (file, ";;   reg %d ", DF_REF_REGNO (use));
            mtcs_dfproblems_df_chain_dump/*!df_chain_dump*/(self,DF_REF_CHAIN (use), file);
            fprintf (file, "\n");
         }
   }
   if (df_chain_problem_p (DF_DU_CHAIN)){
      ...
      fprintf (file,";;  DU chains for artificial defs at %s\n",top ? "top" : "bottom");
      FOR_EACH_ARTIFICIAL_DEF (def, bb->index)
      if ((top && (DF_REF_FLAGS (def) & DF_REF_AT_TOP)) || (!top && !(DF_REF_FLAGS (def) & DF_REF_AT_TOP))){
         fprintf (file, ";;   reg %d ", DF_REF_REGNO (def));
         mtcs_dfproblems_df_chain_dump/*!df_chain_dump*/(self,DF_REF_CHAIN (def), file);
         fprintf (file, "\n");
      }
   }
}
...
static void df_chain_insn_top_dump (const rtx_insn *insn, FILE *file)
{
   ...
   if (df_chain_problem_p (DF_UD_CHAIN) && INSN_P (insn)){
      ...
      FOR_EACH_INSN_INFO_USE (use, insn_info)
         if (!mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,DF_REF_REGNO (use))
...
void mtcs_dfproblems_df_chain_add_problem (MtcsDfproblems *self,unsigned int chain_flags)
{
   ...
   mtcs_dfcore_df_add_problem/*!df_add_problem*/(mtcsDfcore,&problem_CHAIN);
   df_chain->local_flags = chain_flags;
   df_chain->out_of_date_transfer_functions = BITMAP_ALLOC (&mtcsDfcore->df_bitmap_obstack);
}
...
/* Free basic block info.  */
static void df_word_lr_free_bb_info (basic_block bb ATTRIBUTE_UNUSED,
          void *vbb_info)
{
...
/* Allocate or reset bitmaps for DF_WORD_LR blocks. The solution bits are
   not touched unless the block is new.  */
static void df_word_lr_alloc (bitmap all_blocks ATTRIBUTE_UNUSED)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfproblems *self=mtcs_target_get_dfproblems(mtcsTarget);
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   ...
   bitmap_iterator bi;
   basic_block bb;
   struct df_word_lr_problem_data *problem_data = XNEW (struct df_word_lr_problem_data);

   df_word_lr->problem_data = problem_data;
   mtcs_dfcore_df_grow_bb_info/*!df_grow_bb_info*/(mtcsDfcore,df_word_lr);
...
bool mtcs_dfproblems_df_word_lr_mark_ref (MtcsDfproblems *self,df_ref ref, bool is_set, regset live)
{
   ...
   gcc_assert (DF_REF_FLAGS_IS_SET (ref, DF_REF_PARTIAL));
      if (mtcs_rtl_subreg_lowpart_p/*!subreg_lowpart_p*/(mtcsRTL,orig_reg))
         which_subword = 0;
      else
         which_subword = 1;
   }
...
/* Compute local live register info for basic block BB.  */
static void df_word_lr_bb_local_compute (MtcsDfproblems *self,unsigned int bb_index)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   FOR_EACH_ARTIFICIAL_DEF (def, bb_index)
      gcc_assert (DF_REF_REGNO (def) < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg));

   FOR_EACH_ARTIFICIAL_USE (use, bb_index)
      gcc_assert (DF_REF_REGNO (use) < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg));
   ...
   FOR_BB_INSNS_REVERSE (bb, insn){
      ...
      FOR_EACH_INSN_INFO_DEF (def, insn_info)
         ...
         if (!(DF_REF_FLAGS (def) & (DF_REF_CONDITIONAL))){
            mtcs_dfproblems_df_word_lr_mark_ref/*!df_word_lr_mark_ref*/(self,def, true, &bb_info->def);
            mtcs_dfproblems_df_word_lr_mark_ref/*!df_word_lr_mark_ref*/(self,def, false, &bb_info->use);
         }
      FOR_EACH_INSN_INFO_USE (use, insn_info)
         mtcs_dfproblems_df_word_lr_mark_ref/*!df_word_lr_mark_ref*/(self,use, true, &bb_info->use);
   }
}

/* Compute local live register info for each basic block within BLOCKS.  */
static void df_word_lr_local_compute (bitmap all_blocks ATTRIBUTE_UNUSED)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsDfproblems *self=mtcs_target_get_dfproblems(mtcsTarget);
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   EXECUTE_IF_SET_IN_BITMAP (df_word_lr->out_of_date_transfer_functions, 0, bb_index, bi){
      if (bb_index == EXIT_BLOCK){
         unsigned regno;
         bitmap_iterator bi;
         EXECUTE_IF_SET_IN_BITMAP (df->exit_block_uses,
               mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg),regno, bi)
         gcc_unreachable ();
      }else
         df_word_lr_bb_local_compute(self,bb_index);
   }
...
{
   struct df_word_lr_problem_data *problem_data  = (struct df_word_lr_problem_data *)df_word_lr->problem_data;
...
static void df_word_lr_top_dump (basic_block bb, FILE *file)
{
   ...
   fprintf (file, ";; blr  in  \t");
   mtcs_dfcore_df_print_word_regset/*!df_print_word_regset*/(mtcsDfcore,file, &bb_info->in);
   fprintf (file, ";; blr  use \t");
   mtcs_dfcore_df_print_word_regset/*!df_print_word_regset*/(mtcsDfcore,file, &bb_info->use);
   fprintf (file, ";; blr  def \t");
   mtcs_dfcore_df_print_word_regset/*!df_print_word_regset*/(mtcsDfcore,file, &bb_info->def);
}
...
static void df_word_lr_bottom_dump (basic_block bb, FILE *file)
{
   ...
   mtcs_dfcore_df_print_word_regset/*!df_print_word_regset*/(mtcsDfcore,file, &bb_info->out);
}
...
void mtcs_dfproblems_df_word_lr_add_problem (MtcsDfproblems *self)
{
   ...
   mtcs_dfcore_df_add_problem/*!df_add_problem*/(mtcsDfcore,&problem_WORD_LR);
   /* These will be initialized when df_scan_blocks processes each
   block.  */
   df_word_lr->out_of_date_transfer_functions = BITMAP_ALLOC (&mtcsDfcore->df_bitmap_obstack);
}
...
bool mtcs_dfproblems_df_word_lr_simulate_defs (MtcsDfproblems *self,rtx_insn *insn, bitmap live)
{
   ...
   FOR_EACH_INSN_DEF (def, insn)
      if (DF_REF_FLAGS (def) & DF_REF_CONDITIONAL)
         ...
         changed |= mtcs_dfproblems_df_word_lr_mark_ref/*!df_word_lr_mark_ref*/(self,def, false, live);
   return changed;
}
...
void mtcs_dfproblems_df_word_lr_simulate_uses (MtcsDfproblems *self,rtx_insn *insn, bitmap live)
{
   ...
   FOR_EACH_INSN_USE (use, insn)
      mtcs_dfproblems_df_word_lr_mark_ref/*!df_word_lr_mark_ref*/(self,use, true, live);
}
...
static void df_note_alloc (bitmap all_blocks ATTRIBUTE_UNUSED)
{
...
//}
//#else
static inline bool df_ignore_stack_reg (MtcsDfproblems *self,int regno ATTRIBUTE_UNUSED)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if(mtcs_config_ifdef(mtcsConfig,MTCS_STACK_REGS)){
      return mtcs_rtl_get_regstack_completed/*!regstack_completed*/(mtcsRTL)  && IN_RANGE (regno,
         mtcs_reg_get_first_stack_reg/*!FIRST_STACK_REG*/(mtcsReg),
         mtcs_reg_get_last_stack_reg/*!LAST_STACK_REG*/(mtcsReg));
   }else{
      return false;
   }
...
static void df_remove_dead_and_unused_notes (MtcsDfproblems *self,rtx_insn *insn)
{
   ...
   while (link){
      switch (REG_NOTE_KIND (link)){
         case REG_DEAD:
            ...
            if (df_ignore_stack_reg(self,REGNO (XEXP (link, 0)))){
               pprev = &XEXP (link, 1);
               link = *pprev;
            }else{
...
static void df_remove_dead_eq_notes (MtcsDfproblems *self,rtx_insn *insn, bitmap live)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
...
static inline void df_set_note (enum reg_note note_type, rtx_insn *insn, rtx reg)
{
...
static bool df_whole_mw_reg_unused_p (struct df_mw_hardreg *mws,bitmap live, bitmap artificial_uses)
{
   ...
   for (r = mws->start_regno; r <= mws->end_regno; r++)
      if (bitmap_bit_p (live, r) || bitmap_bit_p (artificial_uses, r))
         return false;
...
/* Set the REG_UNUSED notes for the multiword hardreg defs in INSN
   ...
   instruction.
*/
static void df_set_unused_notes_for_mw (rtx_insn *insn, struct df_mw_hardreg *mws,
...
static bool df_whole_mw_reg_dead_p (struct df_mw_hardreg *mws,
         bitmap live, bitmap artificial_uses, bitmap do_not_gen)
{
   unsigned int r;
...
/* Set the REG_DEAD notes for the multiword hardreg use in INSN based
   ...
   register.  */
static void df_set_dead_notes_for_mw (MtcsDfproblems *self,rtx_insn *insn, struct df_mw_hardreg *mws,
           bitmap live, bitmap do_not_gen,
           bitmap artificial_uses, bool *added_notes_p)
{
   ...
   *added_notes_p = false;
   ...
   if (REG_DEAD_DEBUGGING && dump_file){
      fprintf (dump_file, "mw_set_dead looking at mws[%d..%d]\n  do_not_gen =",mws->start_regno, mws->end_regno);
      mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,dump_file, do_not_gen);
      fprintf (dump_file, "  live =");
      mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,dump_file, live);
      fprintf (dump_file, "  artificial uses =");
      mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,dump_file, artificial_uses);
   }
   ...
   if (df_whole_mw_reg_dead_p (mws, live, artificial_uses, do_not_gen)){
      ...
      for (r = mws->start_regno; r <= mws->end_regno; r++)
         if (!bitmap_bit_p (live, r)  && !bitmap_bit_p (artificial_uses, r)  && !bitmap_bit_p (do_not_gen, r)){
            if (is_debug){
               *added_notes_p = true;
               return;
            }
...
static void df_create_unused_note (MtcsDfproblems *self,rtx_insn *insn, df_ref def,
             bitmap live, bitmap artificial_uses,struct dead_debug_local *debug)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsDfcore *mtcsDfcore=mtcs_target_get_dfcore(mtcsTarget);
   ...
   if (REG_DEAD_DEBUGGING && dump_file){
      ...
      mtcs_dfcore_df_ref_debug/*!df_ref_debug*/(mtcsDfcore,def, dump_file);
   }

   if (!((DF_REF_FLAGS (def) & DF_REF_MW_HARDREG)
   || bitmap_bit_p (live, dregno)
   || bitmap_bit_p (artificial_uses, dregno)
   || df_ignore_stack_reg(self,dregno))){
      rtx reg = (DF_REF_LOC (def)) ? *DF_REF_REAL_LOC (def): DF_REF_REG (def);
...
static void df_note_bb_compute (MtcsDfproblems *self,unsigned int bb_index,
          bitmap live, bitmap do_not_gen, bitmap artificial_uses)
{
   ...
   FOR_BB_INSNS_REVERSE (bb, insn){
      ...
      df_remove_dead_and_unused_notes(self,insn);
      ...
      if (CALL_P (insn)){
         ...
         FOR_EACH_INSN_INFO_MW (mw, insn_info)
            if ((DF_MWS_REG_DEF_P (mw)) && !df_ignore_stack_reg(self,mw->start_regno))
               df_set_unused_notes_for_mw (insn, mw, live, do_not_gen,artificial_uses, &debug);
         ...
         FOR_EACH_INSN_INFO_DEF (def, insn_info){
            unsigned int dregno = DF_REF_REGNO (def);
            if (!DF_REF_FLAGS_IS_SET (def, DF_REF_MUST_CLOBBER | DF_REF_MAY_CLOBBER)){
               df_create_unused_note(self,insn,def, live, artificial_uses, &debug);
      ...
      /* Process the uses.  */
      FOR_EACH_INSN_INFO_MW (mw, insn_info)
         if (DF_MWS_REG_USE_P (mw)  && !df_ignore_stack_reg(self,mw->start_regno)){
            bool really_add_notes = debug_insn != 0;

            df_set_dead_notes_for_mw(self,insn, mw, live, do_not_gen,
      ...
      FOR_EACH_INSN_INFO_USE (use, insn_info){
         ...
         if (REG_DEAD_DEBUGGING && dump_file && !debug_insn){
            ...
            mtcs_dfcore_df_ref_debug/*!df_ref_debug*/(mtcsDfcore,use, dump_file);
         }
         ...
         if (!bitmap_bit_p (live, uregno)){
            if (debug_insn){
               if (debug_insn > 0){
                  ...
                  debug insns either.  */
                  if (!bitmap_bit_p (artificial_uses, uregno) && !df_ignore_stack_reg(self,uregno))
                     dead_debug_add (&debug, use, uregno);
                  continue;
               }
               break;
            }else
               dead_debug_insert_temp (&debug, uregno, insn,DEBUG_TEMP_BEFORE_WITH_REG);

            if ( (!(DF_REF_FLAGS (use)
            & (DF_REF_MW_HARDREG | DF_REF_READ_WRITE)))
            && (!bitmap_bit_p (do_not_gen, uregno))
            && (!bitmap_bit_p (artificial_uses, uregno))
            && (!df_ignore_stack_reg(self,uregno))){
               rtx reg = (DF_REF_LOC (use))  ? *DF_REF_REAL_LOC (use) : DF_REF_REG (use);
      ...
      df_remove_dead_eq_notes(self,insn, live);
...
static void df_note_compute (bitmap all_blocks)
{
   ...
   bitmap_initialize (&live, &mtcsDfcore->df_bitmap_obstack);
   bitmap_initialize (&do_not_gen, &mtcsDfcore->df_bitmap_obstack);
   bitmap_initialize (&artificial_uses, &mtcsDfcore->df_bitmap_obstack);
   ...
   EXECUTE_IF_SET_IN_BITMAP (all_blocks, 0, bb_index, bi){
      ...
      df_note_bb_compute(self,bb_index, &live, &do_not_gen, &artificial_uses);
   }
...
void mtcs_dfproblems_df_note_add_problem (MtcsDfproblems *self)
{
   ...
   mtcs_dfcore_df_add_problem/*!df_add_problem*/(mtcsDfcore,&problem_NOTE);
}
...
//原型 df_simulate_find_noclobber_defs df.h df-problems.cc
void mtcs_dfproblems_df_simulate_find_noclobber_defs (MtcsDfproblems *self,rtx_insn *insn, bitmap defs)
{
   df_ref def;
   FOR_EACH_INSN_DEF (def, insn)
      if (!(DF_REF_FLAGS (def) & (DF_REF_MUST_CLOBBER | DF_REF_MAY_CLOBBER)))
...
void mtcs_dfproblems_df_simulate_defs (MtcsDfproblems *self,rtx_insn *insn, bitmap live)
{
   ...
   FOR_EACH_INSN_DEF (def, insn){
      unsigned int dregno = DF_REF_REGNO (def);
      n_debug("mtcsdfproblems.c df_simulate_defs dregno:%d live:%p insn:%p def_flags:%d %d\n",
      dregno,live,insn,DF_REF_FLAGS (def),DF_REF_FLAGS (def) & (DF_REF_PARTIAL | DF_REF_CONDITIONAL));
...
void mtcs_dfproblems_df_simulate_uses (MtcsDfproblems *self,rtx_insn *insn, bitmap live)
{
   ...
   FOR_EACH_INSN_USE (use, insn){
      n_debug("mtcsdfproblems.c df_simulate_uses dregno:%d live:%p insn:%p \n",
            DF_REF_REGNO (use),live,insn);
      /* Add use to set of uses in this BB.  */
...
static inline void df_simulate_fixup_sets (basic_block bb, bitmap live)
{
...
void mtcs_dfproblems_df_simulate_one_insn_backwards (MtcsDfproblems *self,basic_block bb, rtx_insn *insn, bitmap live)
{
   if (!NONDEBUG_INSN_P (insn))
      return;
   mtcs_dfproblems_df_simulate_defs/*!df_simulate_defs*/(self,insn, live);
   mtcs_dfproblems_df_simulate_uses/*!df_simulate_uses*/(self,insn, live);
...
void mtcs_dfproblems_df_simulate_one_insn_forwards (MtcsDfproblems *self,basic_block bb, rtx_insn *insn, bitmap live)
{
   ...
   situation.  */
   mtcs_dfproblems_df_simulate_find_noclobber_defs/*!df_simulate_find_noclobber_defs*/(self,insn, live);
...
typedef struct _FindMemoryData
{
...
/* A subroutine of can_move_insns_across_p called through note_stores.
   ...
   of either kind.  */
static void findMemoryStores_cb (rtx x, const_rtx pat ATTRIBUTE_UNUSED, void *userData ATTRIBUTE_UNUSED)
{
   FindMemoryData *findMemoryInfo=(FindMemoryData *)userData;
   MtcsDfproblems *self=(MtcsDfproblems *)findMemoryInfo->mtcsDfproblems;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);

   int *pflags = (int *)findMemoryInfo->value;
...
void mtcs_dfproblems_simulate_backwards_to_point (MtcsDfproblems *self,basic_block bb, regset live, rtx point)
{
   rtx_insn *insn;
   bitmap_copy (live, df_get_live_out (bb));
   mtcs_dfproblems_df_simulate_initialize_backwards/*df_simulate_initialize_backwards*/(self,bb, live);

   /* Scan and update life information until we reach the point we're
   interested in.  */
   for (insn = BB_END (bb); insn != point; insn = PREV_INSN (insn))
      mtcs_dfproblems_df_simulate_one_insn_backwards/*!df_simulate_one_insn_backwards*/(self,bb, insn, live);
}
             ...
             regset other_branch_live, rtx_insn **pmove_upto)
{
   ...
   for (insn = across_to; ; insn = next){
      ...
      if (NONDEBUG_INSN_P (insn)){
         ...
         FindMemoryData userData={self,&mem_sets_in_across};
         mtcs_rtlanal_note_stores/*!note_stores*/(mtcsRtlanal,insn, findMemoryStores_cb, (void*)&userData/*!&mem_sets_in_across*/);
         /* This is used just to find sets of the stack pointer.  */
         memrefs_in_across |= mem_sets_in_across;
         trapping_insns_in_across |= mtcs_rtlanal_may_trap_p/*!may_trap_p*/(mtcsRtlanal,PATTERN (insn));
      }
      next = PREV_INSN (insn);
      if (insn == across_from)
         break;
   ...
   mtcs_dfproblems_df_simulate_initialize_backwards/*!df_simulate_initialize_backwards*/(self,merge_bb, test_use);
   for (insn = across_to; ; insn = next){
      if (NONDEBUG_INSN_P (insn)){
         mtcs_dfproblems_df_simulate_find_defs/*!df_simulate_find_defs*/(self,insn, test_set);
         mtcs_dfproblems_df_simulate_defs/*!df_simulate_defs*/(self,insn, test_use);
         mtcs_dfproblems_df_simulate_uses/*!df_simulate_uses*/(self,insn, test_use);
      }
      next = PREV_INSN (insn);
      if (insn == across_from)
         break;
   ...
   for (insn = from; ; insn = next){
      ...
      if (NONDEBUG_INSN_P (insn)){
         ...
         if (other_branch_live != NULL || memrefs_in_across != 0){
            int mem_ref_flags = 0;
            int mem_set_flags = 0;
            FindMemoryData userData={self,&mem_set_flags};

            mtcs_rtlanal_note_stores/*!note_stores*/(mtcsRtlanal,insn, findMemoryStores_cb, (void*)&userData/*!&mem_set_flags*/);
            ...
            if ((memrefs_in_across & MEMREF_VOLATILE) && mem_ref_flags != 0)
               break;
            if (mem_set_flags != 0 || (mem_sets_in_across != 0 && mem_ref_flags != 0))
               break;
         }
         ...
         mtcs_dfproblems_df_simulate_find_defs/*!df_simulate_find_defs*/(self,insn, merge_set);
         if (bitmap_intersect_p (merge_set, test_use) || bitmap_intersect_p (merge_use, test_set))
            break;
         max_to = insn;
      }
      next = NEXT_INSN (insn);
   ...
   for (insn = to; insn != max_to; insn = PREV_INSN (insn))
      mtcs_dfproblems_df_simulate_one_insn_backwards/*!df_simulate_one_insn_backwards*/(self,merge_bb, insn, local_merge_live);
   ...
   if (! reload_completed
         && mtcsTarget/*!targetm.small_register_classes_for_mode_p*/->small_register_classes_for_mode_p(mtcsTarget,VOIDmode)){
...
static void df_md_free_bb_info (basic_block bb ATTRIBUTE_UNUSED,  void *vbb_info)
{
...
static void df_md_alloc (bitmap all_blocks)
{
   ...
   mtcs_dfcore_df_grow_bb_info/*!df_grow_bb_info*/(mtcsDfcore,df_md);
   ...
   bitmap_initialize (&self->df_md_scratch, &problem_data->md_bitmaps);
...
void df_md_simulate_artificial_defs_at_top (MtcsDfproblems *self,basic_block bb, bitmap local_md)
{
   ...
   FOR_EACH_ARTIFICIAL_DEF (def, bb_index)
      if (DF_REF_FLAGS (def) & DF_REF_AT_TOP){
         unsigned int dregno = DF_REF_REGNO (def);
         if (DF_REF_FLAGS (def) & (DF_REF_PARTIAL | DF_REF_CONDITIONAL | DF_REF_MAY_CLOBBER))
...
static void df_md_bb_local_compute_process_def (MtcsDfproblems *self,class df_md_bb_info *bb_info,df_ref def, int top_flag)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   for (; def; def = DF_REF_NEXT_LOC (def)){
      ...
      && top_flag == (DF_REF_FLAGS (def) & DF_REF_AT_TOP)){
         if (!bitmap_bit_p (&self->seen_in_insn, dregno)){
            if (DF_REF_FLAGS (def) & (DF_REF_PARTIAL | DF_REF_CONDITIONAL | DF_REF_MAY_CLOBBER)){
               ...
               bitmap_set_bit (&self->seen_in_insn, dregno);
...
static void df_md_bb_local_compute (MtcsDfproblems *self,unsigned int bb_index)
{
   basic_block bb = BASIC_BLOCK_FOR_FN (cfun, bb_index);
   class df_md_bb_info *bb_info = df_md_get_bb_info (bb_index);
   rtx_insn *insn;
   ...
   /* Artificials are only hard regs.  */
   if (!(df->changeable_flags & DF_NO_HARD_REGS))
      df_md_bb_local_compute_process_def(self,bb_info, df_get_artificial_defs (bb_index),DF_REF_AT_TOP);
   ...
   FOR_BB_INSNS (bb, insn){
      ...
      df_md_bb_local_compute_process_def(self,bb_info, DF_INSN_UID_DEFS (uid), 0);
   }

   if (!(df->changeable_flags & DF_NO_HARD_REGS))
      df_md_bb_local_compute_process_def(self,bb_info,df_get_artificial_defs (bb_index), 0);
}
...
static void df_md_local_compute (bitmap all_blocks)
{
   ...
   bitmap_initialize (&self->seen_in_insn, &bitmap_default_obstack);

   EXECUTE_IF_SET_IN_BITMAP (all_blocks, 0, bb_index, bi1){
      df_md_bb_local_compute(self,bb_index);
   }
   ...
   EXECUTE_IF_SET_IN_BITMAP (all_blocks, 0, bb_index, bi1){
      ...
      EXECUTE_IF_SET_IN_BITMAP (&frontiers[bb_index], 0, df_bb_index, bi2){
         ...
         if (bitmap_bit_p (all_blocks, df_bb_index))
            bitmap_ior_and_into (&df_md_get_bb_info (df_bb_index)->init, kill, df_get_live_in (bb));
      }
   }
...
static bool df_md_transfer_function (int bb_index)
{
   ...
   bitmap_and (&self->df_md_scratch, gen, df_get_live_out (bb));
   ...
   return bitmap_ior_and_compl (out, &self->df_md_scratch, in, kill);
}
...
static bool df_md_confluence_n (edge e)
{
   ...
   if (e->flags & EDGE_EH){
      ...
      they have.  */
      n_debug("mtcsdfproblems.c mtcs_df_md_confluence_n bitmap_view\n");
      mtcs_bitmap_view<HardRegSet/*!HARD_REG_SET*/> eh_kills (abi/*!eh_edge_abi*/->full_reg_clobbers ());
...
static void df_md_free (void)
{
   ...
   struct df_md_problem_data *problem_data = (struct df_md_problem_data *) df_md->problem_data;

   bitmap_release (&self->df_md_scratch);
...
static void df_md_top_dump (basic_block bb, FILE *file)
{
   ...
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &bb_info->init);
   fprintf (file, ";; md  gen \t");
   mtcs_dfcore_df_print_regset/*!df_print_regset*/(mtcsDfcore,file, &bb_info->gen);
...
void mtcs_dfproblems_df_md_add_problem (MtcsDfproblems *self)
{
   ...
   mtcs_dfcore_df_add_problem/*!df_add_problem*/(mtcsDfcore,&problem_MD);
}

static void mtcsDfproblemsInit(MtcsDfproblems *self)
{
}
...
{
   MtcsDfproblems *self = n_slice_alloc0 (sizeof(MtcsDfproblems));
   mtcs_component_set_mode((MtcsComponent *)self,mtcsMode);
   mtcsDfproblemsInit(self);
   return self;
}
