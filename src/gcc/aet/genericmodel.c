...
static GenericUnit *createGenUnit(tree generic,char *parentName,tree parentTree,location_t loc)
   ...
   if(TREE_CODE(generic)!=IDENTIFIER_NODE){
      ...
      //unit->pointerCount,unit->genericType);
   }else{
      unit->decl=generic;
...
static tree c_parser_parameter_declaration (GenericModel *self ,tree *expr,
      char **parentClass,tree *parentTree,nboolean needIdentifierNode,location_t *loc)
{
   ...
   && !aet_parser_c_parser_nth_token_starts_std_attributes/*!c_c_parser_nth_token_starts_std_attributes*/(aet_parser_get(),1)){
      c_token *token = c_parser_peek_token (parser);
   ...
   aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(aet_parser_get(),
         specs, true, true, true, true, false,false,true, cla_nonabstract_decl);
   finish_declspecs (specs);
   pending_xref_error ();
   prefix_attrs = specs->attrs;
   specs->attrs = NULL_TREE;
   declarator = aet_parser_c_parser_declarator/*!c_parser_declarator*/(aet_parser_get(),
...
static int c_parser_parms_list_declarator (GenericModel *self,c_parser *parser,nboolean needIdentifierNode)
{
   ...
   while (true){
      /* Parse a parameter.  */
      n_debug("genericmodel 循环分析每一个泛型参数 count:%d",count);
      char *parentClassName=NULL;
      tree parentTree=NULL_TREE;
      location_t loc;
      tree decl = c_parser_parameter_declaration (self,&expr,&parentClassName,&parentTree,needIdentifierNode,&loc);
      if (decl == NULL_TREE){
         c_parser_skip_until_found (parser, CPP_GREATER,"expected %<)%>");
         return 0;
      }
      self->genUnits[count]=createGenUnit(decl,parentClassName,parentTree,loc);
      count++;
      if(parentClassName)
...
/**
 * dest中的未定义在self中找到的位置。
 */
nboolean  generic_model_fill_undefine(GenericModel *self,GenericModel *first,GenericModel *second)
{

   int dest[2];
   int i,j;
   int destCount=generic_model_get_count(self);

   //printf("generic_model_fill_undefine -- %p %p\n",self,dest);
   for(i=0;i<destCount;i++){
      ...
      nboolean find=FALSE;
      if(first!=NULL){
         for(j=0;j<first->unitCount;j++){
            GenericUnit *selfUnit=first->genUnits[j];
            if(!selfUnit->isDefine && !strcmp(selfUnit->name,unit->name)){
               dest[0]=i;
               dest[1]=j;
               find=TRUE;
...
char   *generic_unit_tostring(GenericUnit *self)
{
   ...
   n_string_append_printf(str," gentype:%d define:%d",self->genericType,self->isDefine);
   sprintf(self->toString,"%s",str->str);
   n_string_free(str,TRUE);
   return self->toString;
}

//检查类声明或函数声明 检查有没有具体的类型
nboolean generic_model_class_or_func_decl_check(GenericModel *self)
{
   int i;
   for(i=0;i<generic_model_get_count(self);i++){
      GenericUnit  *unit=generic_model_get(self,i);
      if(!generic_unit_is_undefine(unit)){
         error_at(unit->loc,"泛型类或泛型函数中不能声明具体的类型。");
         return FALSE;
      }
   }
   return TRUE;
...
{
   if(self==NULL || !self->decl)
      return NULL;
   char *sysName=class_util_get_class_name(self->decl);
   if(sysName!=NULL){
      ClassInfo *classInfo=class_mgr_get_class_info(class_mgr_get(),sysName);
      return class_info_get_file(classInfo);
   }else{
      expanded_location xloc;
      if(DECL_P(self->decl)){
         xloc = expand_location (DECL_SOURCE_LOCATION (self->decl));
         return xloc.file;
      }
   }
   return NULL;
