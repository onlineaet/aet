...
/*获取正在编译的文件 classparser.c中设*/
typedef char *(*NLogGetCompileFile)(char **func);
...
/**
 * 设置获取正在编译的文件回调函数，由classparser.c调用。
 */
void             n_log_set_compile_file_callback(NLogGetCompileFile cb);
nboolean         n_log_is_debug();
char            *n_log_get_info();
nboolean         n_log_is_debug_file(char *file,char *func);

#define IF_N_LOG_FILE   if(n_log_is_debug_file(__FILE__,N_STRFUNC))
