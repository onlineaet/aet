...
static tree recursionDotCallLink(char *child,char *lastSysName,location_t loc,tree dot,int *find)
{
          ...
          if(childInfo->parentName.sysName!=NULL){
              ...
              tree componentRef = build_component_ref (loc, dot,id, loc,UNKNOWN_LOCATION);
...
static tree  createInfaceAtClass(tree root,ClassName *parent,ClassName *iface,location_t loc)
{
    ...
    tree componentRef = build_component_ref (loc, result,ifaceId, loc,UNKNOWN_LOCATION);
    return componentRef;
}
        ...
        tree ifaceField,location_t loc,tree *firstParm)
{
     ...
     if(TREE_CODE(func)==FUNCTION_DECL){
          ...
          tree firstRef= build_component_ref (loc, datum,DECL_NAME(ifaceVarInParentClass), loc,UNKNOWN_LOCATION);//
          tree ref= build_component_ref (loc, firstRef,DECL_NAME(ifaceField), loc,UNKNOWN_LOCATION);//
     ...
     if(!hasNamelessCall || hasNamelessCall==1){
       ...
       *firstParm=build1 (ADDR_EXPR, ifacePointerType, firstRef);
        ...
        tree componentRef = build_component_ref (loc, ref,DECL_NAME(ifaceField), loc,UNKNOWN_LOCATION);
         ...
         if(hasNamelessCall!=2){
             ...
             *firstParm=(hasNamelessCall==0)?castParent:null_pointer_node;
             //aet_print_tree(componentRef);
               ...
               if(find){
                   componentRef = build_component_ref (loc, result,DECL_NAME(parentField), loc,UNKNOWN_LOCATION);
                   *firstParm=null_pointer_node;
               }
         }
        ...
        ClassName *iface,tree ifaceField,location_t loc,tree *firstParm)
{
     ...
     if(TREE_CODE(func)==FUNCTION_DECL){
         ...
         tree firstRef= build_component_ref (loc, datum,DECL_NAME(ifaceVarInClass), loc,UNKNOWN_LOCATION);
         componentRef= build_component_ref (loc, firstRef,DECL_NAME(ifaceField), loc,UNKNOWN_LOCATION);
         *firstParm=build1 (ADDR_EXPR, ifacePointerType, firstRef);
         return componentRef;
     }
     ...
     if(TREE_CODE(valueType)==POINTER_TYPE){
       ...
       *firstParm=(hasNamelessCall==0)?build1 (ADDR_EXPR, ifacePointerType, firstRef):null_pointer_node;
         ...
         //从abc.getData变成 abc.iface.getData();
         tree firstRef= build_component_ref (loc, value,DECL_NAME(ifaceVarInClass), loc,UNKNOWN_LOCATION);
         componentRef = build_component_ref (loc, firstRef,DECL_NAME(ifaceField), loc,UNKNOWN_LOCATION);
         *firstParm=(hasNamelessCall==0)?build1 (ADDR_EXPR, ifacePointerType, firstRef):null_pointer_node;
     }
     return componentRef;
...
tree  func_help_create_itself_deref_new(FuncHelp *self,tree func,tree field,location_t loc,tree *firstParm)
{
        ...
        if(TREE_CODE(func)==FUNCTION_DECL){
            ...
            tree selfTree=lookup_name(aet_utils_create_ident("self"));
            tree datum=build_indirect_ref (loc,selfTree,RO_ARROW);
            componentRef= build_component_ref (loc, datum,DECL_NAME(field), loc,UNKNOWN_LOCATION);
            *firstParm=selfTree;
            return componentRef;
        }else{
        ...
        if(TREE_CODE(valueType)==POINTER_TYPE){
            ...
            if(TREE_CODE(value)==INDIRECT_REF)
                componentRef= build_component_ref (loc, value,DECL_NAME(field), loc,UNKNOWN_LOCATION);
            else{
              tree datum=build_indirect_ref (loc,value,RO_ARROW);
              componentRef= build_component_ref (loc, datum,DECL_NAME(field), loc,UNKNOWN_LOCATION);
            }
