...
inline resource_info full_register (unsigned int regno)
{
   ...
   return { GET_MODE (mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[regno]), regno };
...
accesses_include_hard_registers (const access_array &accesses)
{
  ...
  return accesses.size () && mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,accesses.front ()->regno ());

}

// Return true if ACCESSES includes a reference to a non-fixed hard register.
inline bool accesses_include_nonfixed_hard_registers (access_array accesses)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   for (access_info *access : accesses){
      if (!mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,access->regno ()))
         break;
      if (!mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[access->regno ()])
         return true;
   }
   return false;
}
...
inline bool accesses_include_memory (const access_array &accesses)
{
...
template<typename T>
inline auto memory_access (T accesses) -> decltype (accesses[0])
{
...
// unchanged.
template<typename T> inline T drop_memory_access (T accesses)
{
...
inline T filter_accesses (obstack_watermark &watermark,
		 T accesses,FilterPredicate predicate)
{
