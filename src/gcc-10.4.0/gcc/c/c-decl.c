...
/* Establish a binding between NAME, an IDENTIFIER_NODE, and DECL,
   ...
   in the scope SCOPE.  */
static void bind (tree name, tree decl, struct c_scope *scope, bool invisible, bool nested, location_t locus)
{
//  n_debug("bind 00 name:%s decl code:%d codeName:%s binding_freelist:%p scope:%p ",name?IDENTIFIER_POINTER(name):"unknown",
//				TREE_CODE(decl),get_tree_code_name(TREE_CODE(decl)),binding_freelist,scope);
...
//  n_debug("bind 22 name:%s decl code:%d codeName:%s binding_freelist:%p depth:%d scope:%p b:%p here:%p ",name?IDENTIFIER_POINTER(name):"unknown",
//				TREE_CODE(decl),get_tree_code_name(TREE_CODE(decl)),binding_freelist,b->depth,scope,b,*here);
  /* Locate the appropriate place in the chain of shadowed decls
     to insert this binding.  Normally, scope == current_scope and
     this does nothing.  */
  while (*here && (*here)->depth > scope->depth){
	 // n_debug("bind 33 name:%s decl code:%d codeName:%s binding_freelist:%p here->depth:%d scope:%p b:%p here:%p ",name?IDENTIFIER_POINTER(name):"unknown",
		//			TREE_CODE(decl),get_tree_code_name(TREE_CODE(decl)),binding_freelist,(*here)->depth ,scope,b,*here);
    here = &(*here)->shadowed;
  }
...
static void pushtag (location_t loc, tree name, tree type)
{
    /* Record the identifier as the type's name if it has none.  */
    if (name && !TYPE_NAME (type))
       TYPE_NAME (type) = name;
	n_debug("pushtag 00 name:%s code:%d %s current_scope:%p ",name?IDENTIFIER_POINTER(name):"unknown",
			TREE_CODE(type),get_tree_code_name(TREE_CODE(type)),current_scope);
  ...
  /* Create a fake NULL-named TYPE_DECL node whose TREE_TYPE will be the
     ...
     tagged type.  */
	n_debug("pushtag 11 name:%s code:%d %s current_scope:%p ",name?IDENTIFIER_POINTER(name):"unknown",
			TREE_CODE(type),get_tree_code_name(TREE_CODE(type)),current_scope);
   TYPE_STUB_DECL (type) = pushdecl (build_decl (loc,TYPE_DECL, NULL_TREE, type));
	n_debug("pushtag 22 name:%s code:%d %s current_scope:%p ",name?IDENTIFIER_POINTER(name):"unknown",
			TREE_CODE(type),get_tree_code_name(TREE_CODE(type)),current_scope);
  ...
  if (warn_cxx_compat && name != NULL_TREE)
	      ...
	      != TYPE_MAIN_VARIANT (type)))
	{
	  ...
	  if (warning_at (loc, OPT_Wc___compat,("using %qD as both a typedef and a tag is invalid in C++"), b->decl) && b->locus != UNKNOWN_LOCATION)
...
#define DECL_EXTERN_INLINE(DECL) (DECL_DECLARED_INLINE_P (DECL)  \
  ...
  *newtypep = newtype = TREE_TYPE (newdecl);
  if (oldtype == error_mark_node || newtype == error_mark_node)
    return false;
  n_debug("diagnose_mismatched_decls 00 %p %p\n",newdecl,olddecl);

  /* Two different categories of symbol altogether.  This is an error
     unless OLDDECL is a builtin.  OLDDECL will be discarded in any case.  */
  if (TREE_CODE (olddecl) != TREE_CODE (newdecl))
    {
	  n_debug("diagnose_mismatched_decls 11 TREE_CODE (olddecl) != TREE_CODE (newdecl) %p %p\n",newdecl,olddecl);
  ...
  if (TREE_CODE (olddecl) == CONST_DECL)
    {
	  n_debug("diagnose_mismatched_decls 22 %p %p\n",newdecl,olddecl);
  ...
  if (!comptypes (oldtype, newtype))
    {
	  n_debug("diagnose_mismatched_decls 33 %p %p\n",newdecl,olddecl);
	  ...
	  && !C_DECL_DECLARED_BUILTIN (olddecl))
	{
	  /* Accept "harmless" mismatches in function types such
	     ...
	     incompatible according to language rules.  */
    	  n_debug("diagnose_mismatched_decls 44 %p %p\n",newdecl,olddecl);
	  ...
	  if (mismatch_expect && extra_warnings)
	    {
    	  n_debug("diagnose_mismatched_decls 55 %p %p\n",newdecl,olddecl);
	...
	{
    	  n_debug("diagnose_mismatched_decls 66 %p %p\n",newdecl,olddecl);
	...
	{
    	  n_debug("diagnose_mismatched_decls 77 %p %p\n",newdecl,olddecl);
	...
	{
    	  n_debug("diagnose_mismatched_decls 88 %p %p\n",newdecl,olddecl);
	  ...
	  if (new_quals != old_quals){
	      ...
	      if (CLEAR_QUAL_ADDR_SPACE (new_quals) != CLEAR_QUAL_ADDR_SPACE (old_quals))
		     error ("conflicting type qualifiers for %q+D", newdecl);
	  }else{
    	  n_debug("diagnose_mismatched_decls ---88 %p %p newtype:%p oldType:%p\n",newdecl,olddecl,newtype,oldtype);
    	 // printNode(newtype);
    	  //printNode(newdecl);
  ...
  /* Function declarations can either be 'static' or 'extern' (no
	...
	{
    	  n_debug("diagnose_mismatched_decls 99 %p %p\n",newdecl,olddecl);
	...
	{
    	  n_debug("diagnose_mismatched_decls 100 %p %p\n",newdecl,olddecl);
      ...
      if (TREE_PUBLIC (olddecl) && !TREE_PUBLIC (newdecl))
	{
	  /* Two exceptions to the rule.  If olddecl is an extern
	     ...
	     Objective C use normal builtins.)  */
    	  n_debug("diagnose_mismatched_decls 101 %p %p\n",newdecl,olddecl);

	  if (!DECL_IS_BUILTIN (olddecl)
	      && !DECL_EXTERN_INLINE (olddecl))
	    {
    	  n_debug("diagnose_mismatched_decls 102 %p %p\n",newdecl,olddecl);
	...
	{
    	  n_debug("diagnose_mismatched_decls 103 %p %p\n",newdecl,olddecl);
	 ...
	 present on all inline decls.  */
	  ...
	  && DECL_DECLARED_INLINE_P (newdecl))
	{
    	  n_debug("diagnose_mismatched_decls 104 %p %p\n",newdecl,olddecl);
  ...
  else if (VAR_P (newdecl))
    {
	 ...
	 agree on this property.  */
	  n_debug("diagnose_mismatched_decls 105 %p %p\n",newdecl,olddecl);
	...
	{
    	  n_debug("diagnose_mismatched_decls 106 %p %p\n",newdecl,olddecl);
	...
	{
    	  n_debug("diagnose_mismatched_decls 107 %p %p\n",newdecl,olddecl);
	 ...
	 extern case only for -Wtraditional.  See generally 6.2.2p3-5,7.  */
      if (DECL_FILE_SCOPE_P (newdecl) && TREE_PUBLIC (newdecl) != TREE_PUBLIC (olddecl)){
    	  n_debug("diagnose_mismatched_decls 108 %p %p\n",newdecl,olddecl);

	  if (DECL_EXTERNAL (newdecl))
	    {
    	  n_debug("diagnose_mismatched_decls 109 %p %p\n",newdecl,olddecl);

	      if (!DECL_FILE_SCOPE_P (olddecl))
		{
	    	  n_debug("diagnose_mismatched_decls 110 %p %p\n",newdecl,olddecl);
		...
		{
	    	  n_debug("diagnose_mismatched_decls 111 %p %p\n",newdecl,olddecl);
	    ...
	    {
    	  n_debug("diagnose_mismatched_decls 112 %p %p\n",newdecl,olddecl);
      ...
      else if (!DECL_FILE_SCOPE_P (newdecl))
	{
	  if (DECL_EXTERNAL (newdecl))
	    ...
	    {
    	  n_debug("diagnose_mismatched_decls 113 %p %p\n",newdecl,olddecl);
	    ...
	    {
    	  n_debug("diagnose_mismatched_decls 114 %p %p\n",newdecl,olddecl);
  ...
  n_debug("diagnose_mismatched_decls 115 %p %p\n",newdecl,olddecl);
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
  ...
  else /* PARM_DECL, VAR_DECL */
    {
	 ...
	 mark_forward_parm_decls.  */
	  n_debug("diagnose_mismatched_decls 116 %p %p\n",newdecl,olddecl);
...
duplicate_decls (tree newdecl, tree olddecl)
{
  ...
  if(newdecl && TREE_CODE(newdecl)==VAR_DECL && DECL_NAME(newdecl))
			  n_debug("duplicate_decls 00 %p %s pub:%d static:%d extern:%d\n",
					  newdecl,IDENTIFIER_POINTER(DECL_NAME(newdecl)),TREE_PUBLIC(newdecl),TREE_STATIC(newdecl),DECL_EXTERNAL(newdecl));
  if(olddecl && TREE_CODE(olddecl)==VAR_DECL && DECL_NAME(olddecl))
  			  n_debug("duplicate_decls 11 %p %s pub:%d static:%d extern:%d\n",
  					olddecl,IDENTIFIER_POINTER(DECL_NAME(olddecl)),TREE_PUBLIC(olddecl),TREE_STATIC(olddecl),DECL_EXTERNAL(olddecl));

  if (!diagnose_mismatched_decls (newdecl, olddecl, &newtype, &oldtype))
    {
      /* Avoid `unused variable' and other warnings for OLDDECL.  */
	  if(newdecl && TREE_CODE(newdecl)==VAR_DECL && DECL_NAME(newdecl))
				  n_debug("duplicate_decls 22 %p %s pub:%d static:%d extern:%d\n",
						  newdecl,IDENTIFIER_POINTER(DECL_NAME(newdecl)),TREE_PUBLIC(newdecl),TREE_STATIC(newdecl),DECL_EXTERNAL(newdecl));
      TREE_NO_WARNING (olddecl) = 1;
      return false;
    }
  if(newdecl && TREE_CODE(newdecl)==VAR_DECL && DECL_NAME(newdecl))
			  n_debug("duplicate_decls 33 %p %s pub:%d static:%d extern:%d\n",
					  newdecl,IDENTIFIER_POINTER(DECL_NAME(newdecl)),TREE_PUBLIC(newdecl),TREE_STATIC(newdecl),DECL_EXTERNAL(newdecl));
  ...
  if (VAR_OR_FUNCTION_DECL_P (newdecl))
    {
	  if(newdecl && TREE_CODE(newdecl)==VAR_DECL && DECL_NAME(newdecl))
				  n_debug("duplicate_decls 44 %p %s pub:%d static:%d extern:%d\n",
						  newdecl,IDENTIFIER_POINTER(DECL_NAME(newdecl)),TREE_PUBLIC(newdecl),TREE_STATIC(newdecl),DECL_EXTERNAL(newdecl));
...
tree pushdecl (tree x)
{
  ...
  if (current_function_decl  && (!VAR_OR_FUNCTION_DECL_P (x) || DECL_INITIAL (x) || !TREE_PUBLIC (x)))
    DECL_CONTEXT (x) = current_function_decl;

  /* Anonymous decls are just inserted in the scope.  */
  if (!name){
	  //n_debug("xxpushdecl 00 !name 声明符不存在 bind scope current_scope:%p ",current_scope);
      bind (name, x, scope, /*invisible=*/false, /*nested=*/false,locus);
      return x;
  }
  ...
  if (b && B_IN_SCOPE (b, scope)) {
	  n_debug("xxpushdecl 22 已bind在当前current_scope：%p name:%s ",current_scope,name?IDENTIFIER_POINTER(name):"");
      struct c_binding *b_ext, *b_use;
      tree type = TREE_TYPE (x);
      tree visdecl = b->decl;
      tree vistype = TREE_TYPE (visdecl);
      if (TREE_CODE (TREE_TYPE (x)) == ARRAY_TYPE && COMPLETE_TYPE_P (TREE_TYPE (x)))
	     b->inner_comp = false;
      b_use = b;
      b_ext = b;
	 ...
	 information only.  */
      ...
      if (duplicate_decls (x, b_use->decl)){
	      if (b_use != b){
	         ...
	         if (TREE_CODE (b_use->decl) == FUNCTION_DECL && fndecl_built_in_p (b_use->decl))
		        thistype  = build_type_attribute_variant (thistype, TYPE_ATTRIBUTES(b_use->u.type));
  ...
  if ((DECL_EXTERNAL (x) || scope == file_scope)  && (VAR_OR_FUNCTION_DECL_P (x) || c_dialect_objc ())){
	  n_debug("xxpushdecl 33 已bind 如果是extern声明或者当前作用域与文件作用域相等：DECL_EXTERNAL (x):%d scope == file_scope:%d  %d scope:%p,",
			  DECL_EXTERNAL(x),scope == file_scope,VAR_OR_FUNCTION_DECL_P(x),current_scope);

      tree type = TREE_TYPE (x);
      tree vistype = NULL_TREE;
      tree visdecl = NULL_TREE;
      bool type_saved = false;
      if (b && !B_IN_EXTERNAL_SCOPE (b) && VAR_OR_FUNCTION_DECL_P (b->decl) && DECL_FILE_SCOPE_P (b->decl)){
	     visdecl = b->decl;
	     vistype = TREE_TYPE (visdecl);
	  }
      if (scope != file_scope && !DECL_IN_SYSTEM_HEADER (x))
	     warning_at (locus, OPT_Wnested_externs,"nested extern declaration of %qD", x);
	 ...
	 composite of the visible types only.  */
      if (b && (TREE_PUBLIC (x) || same_translation_unit_p (x, b->decl)) && b->u.type)
...
//	 we want to detect a duplicate decl for a construct like
	 ...
	 the static does not go in the externals scope.  */
      if (b && (TREE_PUBLIC (x) || same_translation_unit_p (x, b->decl)) && duplicate_decls (x, b->decl)){
	      tree thistype;
	      if (vistype){
	          if (comptypes (vistype, type))
		        thistype = composite_type (vistype, type);
	        ...
	        thistype = type;
	      b->u.type = TREE_TYPE (b->decl);
	     if (TREE_CODE (b->decl) == FUNCTION_DECL && fndecl_built_in_p (b->decl))
	        thistype = build_type_attribute_variant (thistype,TYPE_ATTRIBUTES (b->u.type));
		 ...
		 this scope.  */
	        ...
	        x = visdecl;
	    	 ...
	    	 n_debug("xxpushdecl 44 bind 绑定声明、标识符到expternal_scope name：%s scope:%p,",name?IDENTIFIER_POINTER(name):"",
	    			 current_scope);
	         bind (name, x, external_scope, /*invisible=*/true,/*nested=*/false, locus);
	     	 n_debug("xxpushdecl 55 bind 绑定声明、标识符到expternal_scope name：%s scope:%p,",name?IDENTIFIER_POINTER(name):"",
	    	    			 current_scope);
	         nested = true;
	      }
	  }
...
skip_external_and_shadow_checks:
	...
	//n_debug("xxpushdecl 66 bind 绑定到当前作用域：name:%s %p scope:%p ",name?IDENTIFIER_POINTER(name):"",name,current_scope);

    bind (name, x, scope, /*invisible=*/false, nested, locus);
	//n_debug("xxpushdecl 77 bind 绑定到当前作用域：name:%s %p scope:%p ",name?IDENTIFIER_POINTER(name):"",name,current_scope);
  ...
  if (TREE_TYPE (x) != error_mark_node  && !COMPLETE_TYPE_P (TREE_TYPE (x))){
      ...
      //n_debug("xxpushdecl 88--- %p\n",TREE_TYPE(x));
      while (TREE_CODE (element) == ARRAY_TYPE)
	      element = TREE_TYPE (element);
      element = TYPE_MAIN_VARIANT (element);
      //n_debug("xxpushdecl 99--- %p\n",TREE_TYPE(x));
      if ((RECORD_OR_UNION_TYPE_P (element) || TREE_CODE (element) == ENUMERAL_TYPE) && (TREE_CODE (x) != TYPE_DECL
	      || TREE_CODE (TREE_TYPE (x)) == ARRAY_TYPE)  && !COMPLETE_TYPE_P (element)){
	     C_TYPE_INCOMPLETE_VARS (element)= tree_cons (NULL_TREE, x, C_TYPE_INCOMPLETE_VARS (element));
	      n_debug("xxpushdecl 100--- %p\n",TREE_TYPE(x));

      }
  }
 // n_debug("xxpushdecl 101--- %p\n",TREE_TYPE(x));
 // printNode(TREE_TYPE(x));
  return x;
}

...
tree implicitly_declare (location_t loc, tree functionid)
{
    ...
    for (b = I_SYMBOL_BINDING (functionid); b; b = b->shadowed){
        n_debug("implicitly_declare 00 找绑定 id:%s ",IDENTIFIER_POINTER(functionid));
       if (B_IN_SCOPE (b, external_scope)){
	      decl = b->decl;
	      n_debug("implicitly_declare 11 找到了 id:%s %s ",
	    		  IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
    ...
    if (decl){
       if (TREE_CODE (decl) != FUNCTION_DECL){
    	      n_debug("implicitly_declare 22 不是函数声明返回 decl id:%s %s ",
    		    		  IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
	     return decl;
       }
       ...
       if (!fndecl_built_in_p (decl) && DECL_IS_BUILTIN (decl)){
    	   n_debug("implicitly_declare 33 !fndecl_built_in_p (decl) && DECL_IS_BUILTIN (decl) decl id:%s %s ",
    	    		    		  IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
	      bind (functionid, decl, file_scope,/*invisible=*/false, /*nested=*/true,DECL_SOURCE_LOCATION (decl));
	      return decl;
	   }else{
		   n_debug("implicitly_declare 44 newtype= default_function_type id:%s %s ",
		       	    		    		  IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
	      tree newtype = default_function_type;
	      if (b->u.type){
	    	   n_debug("implicitly_declare 55 b->u.type id:%s %s ",
	    			       	    		    		  IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
	      ...
	      if (!C_DECL_IMPLICIT (decl)){
	   	     n_debug("implicitly_declare 66 !C_DECL_IMPLICIT (decl) id:%s %s ",
	   	    			      IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
	         implicit_decl_warning (loc, functionid, decl);
	         C_DECL_IMPLICIT (decl) = 1;
	      }
	      if (fndecl_built_in_p (decl)){
	    	  n_debug("implicitly_declare 77 fndecl_built_in_p (decl) id:%s %s ",
	    	  	   	    			      IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
	         newtype = build_type_attribute_variant (newtype,TYPE_ATTRIBUTES(TREE_TYPE (decl)));
	         if (!comptypes (newtype, TREE_TYPE (decl))){
	        	 n_debug("implicitly_declare 88 !comptypes (newtype, TREE_TYPE (decl)) id:%s %s ",
	        	 	   	    			      IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
		        bool warned = warning_at (loc, 0, "incompatible implicit declaration of built-in function %qD", decl);
		        /* See if we can hint which header to include.  */
		        const char *header = header_for_builtin_fn (decl);
		        if (header != NULL && warned){
		           n_debug("implicitly_declare 99 !header != NULL && warned id:%s %s ",
		        		        	 IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
		           rich_location richloc (line_table, loc);
		           maybe_add_include_fixit (&richloc, header, true);
		           inform (&richloc,"include %qs or provide a declaration of %qD",header, decl);
		        }
		        newtype = TREE_TYPE (decl);
		     }
	      }else{
	    	   n_debug("implicitly_declare 100 fndecl_built_in_p (decl) id:%s %s ",
	    			        		        	 IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
	         if (!comptypes (newtype, TREE_TYPE (decl))){
	      	   n_debug("implicitly_declare 101 !comptypes (newtype, TREE_TYPE (decl) id:%s %s ",
	      	    			        		        	 IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
		        auto_diagnostic_group d;
	      ...
	      n_debug("implicitly_declare 102  id:%s %s ",
	   	      	    			   IDENTIFIER_POINTER(functionid),get_tree_code_name(TREE_CODE(decl)));
	      b->u.type = TREE_TYPE (decl);
	      TREE_TYPE (decl) = newtype;
	      bind (functionid, decl, current_scope,/*invisible=*/false, /*nested=*/true,
	      DECL_SOURCE_LOCATION (decl));
	      return decl;
	   } // end !fndecl_built_in_p (decl) && DECL_IS_BUILTIN (decl
    }// end decl

    /* Not seen before.  */
    n_debug("implicitly_declare 103  id:%s ",
...
{
  struct c_binding *b = I_SYMBOL_BINDING (name);
  //n_debug("lookup_name 00 bind:%p %s %s\n",b,get_tree_code_name(TREE_CODE(name)),IDENTIFIER_POINTER(name));
  if (b && !b->invisible)
    {
//	  n_debug("lookup_name 11 bind:%p %s decl:%p %s\n",
//			  b,get_tree_code_name(TREE_CODE(name)),b->decl,get_tree_code_name(TREE_CODE(b->decl)));
...
c_decl_attributes (tree *node, tree attributes, int flags)
{
  ...
  if (current_omp_declare_target_attribute
    ...
    {
	  //n_debug("c_decl_attributes 00 %s\n",IDENTIFIER_POINTER(DECL_NAME(*node)));
      if (VAR_P (*node)  && !lang_hooks.types.omp_mappable_type (TREE_TYPE (*node)))
	     attributes = tree_cons (get_identifier ("omp declare target implicit"),	NULL_TREE, attributes);
      else{
    	//n_debug("c_decl_attributes 11 %s\n",IDENTIFIER_POINTER(DECL_NAME(*node)));
	    attributes = tree_cons (get_identifier ("omp declare target"), NULL_TREE, attributes);
	    attributes = tree_cons (get_identifier ("omp declare target block"), NULL_TREE, attributes);
	  }
    }
  ...
  //n_debug("c_decl_attributes 22 %s %p\n",IDENTIFIER_POINTER(DECL_NAME(*node)),last_decl);
  if (!last_decl)
    last_decl = lookup_name_in_scope (DECL_NAME (*node), external_scope);
  //n_debug("c_decl_attributes 33 %s %p\n",IDENTIFIER_POINTER(DECL_NAME(*node)),last_decl);
  //printNode(attributes);
...
tree start_decl (struct c_declarator *declarator, struct c_declspecs *declspecs,
	    bool initialized, tree attributes)
{
 ...
 // n_debug("start_decl 00 ");
  ...
  if (!decl || decl == error_mark_node)
    return NULL_TREE;
//  n_debug("start_decl ---00 %p %p %s %s\n",decl,TREE_TYPE(decl),get_tree_code_name(TREE_CODE(decl)),get_tree_code_name(TREE_CODE(TREE_TYPE(decl))));
//  n_debug("start_decl ---11XX  %s\n",get_tree_code_name(TREE_CODE(DECL_NAME(decl))));
//  n_debug("start_decl ---11XXCCC  %s expr:%p\n",IDENTIFIER_POINTER(DECL_NAME(decl)),expr);
//  n_debug("start_decl ---22XXCCC :%p extern:%d static:%d pub:%d ",
//  		   DECL_CONTEXT(decl),DECL_EXTERNAL(decl),TREE_STATIC(decl),TREE_PUBLIC(decl));

  if (expr)
    add_stmt (fold_convert (void_type_node, expr));

  if (TREE_CODE (decl) != FUNCTION_DECL && MAIN_NAME_P (DECL_NAME (decl)) && TREE_PUBLIC (decl))
  ...
  if (VAR_P (decl)
      ...
      && !flag_no_common){
	  n_debug("start_decl --xxx DECL_COMMON (decl) = 1 %p\n",decl);
    DECL_COMMON (decl) = 1;
  }
...
void finish_decl (tree decl, location_t init_loc, tree init,tree origtype, tree asmspec_tree)
{
  ...
  //n_debug("finish_decl ----init %p",init);
  //if(decl && TREE_CODE(decl)==VAR_DECL)
 // n_debug("finish_decl -----00 %s external:%d pub:%d static:%d context:%p",
		  //IDENTIFIER_POINTER(DECL_NAME(decl)),DECL_EXTERNAL(decl),TREE_PUBLIC(decl),TREE_STATIC(decl),DECL_CONTEXT(decl));
  /* If a name was specified, get the string.  */
  if (VAR_OR_FUNCTION_DECL_P (decl) && DECL_FILE_SCOPE_P (decl))
    asmspec_tree = maybe_apply_renaming_pragma (decl, asmspec_tree);
  if (asmspec_tree)
    asmspec = TREE_STRING_POINTER (asmspec_tree);

  if (VAR_P (decl)  && TREE_STATIC (decl)  && global_bindings_p ())
  ...
  if (c_dialect_objc () && (VAR_OR_FUNCTION_DECL_P (decl) || TREE_CODE (decl) == FIELD_DECL))
  ...
  if (TREE_CODE (type) == ARRAY_TYPE  && TYPE_DOMAIN (type) == NULL_TREE &&
		  TREE_CODE (decl) != TYPE_DECL  && !(TREE_PUBLIC (decl) && current_scope != file_scope)){
	   /* Even if pedantic, an external linkage array
	      may have incomplete type at first.  */
      bool do_default= (TREE_STATIC (decl)? pedantic && !TREE_PUBLIC (decl) : !DECL_EXTERNAL (decl));
	...
	default:
      ...
      relayout_decl (decl);
  }//end TREE_CODE (type) == ARRAY_TYPE  && TYPE_DOMAIN (type) == NULL_TREE...
  ...
  if (VAR_OR_FUNCTION_DECL_P (decl))
      ...
      if (asmspec)
	{
      ...
      if (DECL_FILE_SCOPE_P (decl))
	{
          //n_debug("finish_decl ----66 %s",IDENTIFIER_POINTER(DECL_NAME(decl)));
     ...
     // n_debug("finish_decl ----77 rest_of_decl_compilation %s",IDENTIFIER_POINTER(DECL_NAME(decl)));
      ...
      else
	{
	  ...
	  if (TREE_CODE (decl) != FUNCTION_DECL)
		...
		{
	          //n_debug("finish_decl ----88 %s",IDENTIFIER_POINTER(DECL_NAME(decl)));
         ...
         // n_debug("finish_decl ----eee88 %s",IDENTIFIER_POINTER(DECL_NAME(decl)));
          //printNode(DECL_SIZE (decl));
  ...
  if (TREE_CODE (decl) == TYPE_DECL)
    {
      if (!DECL_FILE_SCOPE_P (decl)  && variably_modified_type_p (TREE_TYPE (decl), NULL_TREE)){
    	   n_debug("finish_decl 99 type_decl xxx 11 %s",get_tree_code_name(TREE_CODE(decl)));
  ...
  //if(decl && TREE_CODE(decl)==VAR_DECL){
 // n_debug("finish_decl -----xxx100 %s external:%d pub:%d static:%d",
		 // IDENTIFIER_POINTER(DECL_NAME(decl)),DECL_EXTERNAL(decl),TREE_PUBLIC(decl),TREE_STATIC(decl));
  //printNode(decl);
  //}
  ...
  if (warn_cxx_compat
      ...
      && DECL_INITIAL (decl) == NULL_TREE)
    {
	  ...
	  n_debug("finish_decl -----101 %s external:%d pub:%d static:%d",
				  IDENTIFIER_POINTER(DECL_NAME(decl)),DECL_EXTERNAL(decl),TREE_PUBLIC(decl),TREE_STATIC(decl));
  ...
  if (flag_openmp  && VAR_P (decl)  && lookup_attribute ("omp declare target implicit",DECL_ATTRIBUTES (decl)))
    {

	  n_debug("finish_decl -----102 %s external:%d pub:%d static:%d",
			  IDENTIFIER_POINTER(DECL_NAME(decl)),DECL_EXTERNAL(decl),TREE_PUBLIC(decl),TREE_STATIC(decl));
      DECL_ATTRIBUTES (decl)= remove_attribute ("omp declare target implicit",DECL_ATTRIBUTES (decl));
      if (!lang_hooks.types.omp_mappable_type (TREE_TYPE (decl)))
	     error ("%q+D in declare target directive does not have mappable type",decl);
      else if (!lookup_attribute ("omp declare target",DECL_ATTRIBUTES (decl))
	       && !lookup_attribute ("omp declare target link",DECL_ATTRIBUTES (decl)))
		...
		enum deprecated_states deprecated_state)
{
  ...
  *expr_const_operands = declspecs->expr_const_operands;
	  ...
	  /* FALL THRU.  */
	     ...
	     case cdk_pointer:
	       funcdef_syntax = (decl->kind == cdk_function);
	   	   n_debug("grokdeclarator  kind:%d %p funcdef_syntax:%d ",decl->kind,decl,funcdef_syntax);
  ...
  if (funcdef_flag && !funcdef_syntax){
	  n_debug("grokdeclarator 出错了(funcdef_flag && !funcdef_syntax)==TRUE 返回 NULL_TREE ");
      return NULL_TREE;
  }
  ...
  //n_debug("grokdeclarator temp info:%d %d %d %d\n",decl_context == NORMAL,decl_context == FIELD,current_scope == file_scope,variably_modified_type_p (type, NULL_TREE));


  if ((decl_context == NORMAL || decl_context == FIELD) && current_scope == file_scope && variably_modified_type_p (type, NULL_TREE)){
      if (name)
...
//	 tree tx=TYPE_NAME(type);
//	 n_debug("grokdeclarator cc88--11 %p\n",tx);
//	 n_debug("grokdeclarator cc88ww-- %s\n",get_tree_code_name(TREE_CODE(tx)));
//	 if((TREE_CODE(tx))==IDENTIFIER_NODE)
//		n_debug("还是 IDENTIFIER_NODE\n");
//	 else{
//		tree id=DECL_NAME(tx);
//		 n_debug("grokdeclarator cc88tt-- %s\n",IDENTIFIER_POINTER(id));
//	 }
//   }
  /* Warn about storage classes that are invalid for certain
     kinds of declarations (parameters, typenames, etc.).  */

  if (funcdef_flag && (threadp || storage_class == csc_auto || storage_class == csc_register || storage_class == csc_typedef)){
	  //n_debug("grokdeclarator 00 funcdef_flag storage_class== csc_auto,csc_register,csc_typedef ");
	  ...
	  || storage_class == csc_typedef)
	     ...
	     //n_debug("grokdeclarator 11 decl_context!=NORMAL storage_class!= csc_none||threadp ");

        if (decl_context == PARM && storage_class == csc_register)
	       ;
        else{
   	      //n_debug("grokdeclarator 11 aa decl_context!=NORMAL storage_class!= csc_none||threadp ");
	      switch (decl_context){
	         case FIELD:
	           if (name)
	     ...
	     //n_debug("grokdeclarator 22 storage_class == csc_extern  && initialized  && !funcdef_flag ");
	  ...
	  //n_debug("grokdeclarator 33 current_scope == file_scope ");
      if (storage_class == csc_auto)
	 ...
	 // n_debug("grokdeclarator 44 current_scope == file_scope ");

         if (storage_class == csc_extern && funcdef_flag)
	        error_at (loc, "nested function %qE declared %<extern%>", name);
         else if (threadp && storage_class == csc_none){
	        error_at (loc, "function-scope %qE implicitly auto and declared %qs", name,declspecs->thread_gnu_p ? "__thread" : "_Thread_local");
	        threadp = false;
	     }
  }//end funcdef_flag && (threadp || storage_class == csc_auto || storage_class == csc_register || storage_class == csc_typedef)
...
//  if(TREE_CODE(type)==RECORD_TYPE){
//	 tree tx=TYPE_NAME(type);
//	 n_debug("grokdeclarator cc88--22 %p\n",tx);
//	 n_debug("grokdeclarator cc88ww-- %s\n",get_tree_code_name(TREE_CODE(tx)));
//	 if((TREE_CODE(tx))==IDENTIFIER_NODE)
//		n_debug("还是 IDENTIFIER_NODE\n");
//	 else{
...
//   }
   ...
   while (declarator && declarator->kind != cdk_id){
    	  //n_debug("grokdeclarator 55   current_scope == file_scope 并且 declarator->kind != cdk_id  kind:%d %p ",
...
//          }
         ...
         /* Each level of DECLARATOR is either a cdk_array (for ...[..]),
          ...
          switch (declarator->kind){
	         case cdk_attrs:
	           {
	       	     n_debug("grokdeclarator 66  declarator->kind= cdk_attrs");
	         ...
	         case cdk_array:
	           {
	       	     n_debug("grokdeclarator 77  declarator->kind= cdk_array VLA:%d ",declarator->u.array.vla_unspec_p);
	            ...
	            if (size){
		            ...
		            bool this_size_varies = false;
		       	     n_debug("grokdeclarator 77-- 大小确定  cdk_array VLA:%d ",declarator->u.array.vla_unspec_p);
		          ...
		          /* Strip NON_LVALUE_EXPRs since we aren't using as an
		           lvalue.  */
		            ...
		            if (!INTEGRAL_TYPE_P (TREE_TYPE (size))){
		                ...
		                size_int_const = true;
		            }else if (!COMPLETE_TYPE_P (TREE_TYPE (size))) {/* This can happen with enum forward declaration.  */
		                if (name)
		             ...
		             }//end  TREE_CODE (size) == INTEGER_CST && size_maybe_const
					...
					/* Arrange for the SAVE_EXPR on the inside of the
					   ...
					   with the +1 that happens when building TYPE_SIZE.  */
		            	// printf("grokdeclarator 77-22 size_varies:%d this_size_varies:%d\n",size_varies,this_size_varies);
	            ...
	            }else if (decl_context == FIELD){//接 size
	       	         n_debug("grokdeclarator 77 aa 还是cdk_array decl_context == FIELD ");
		        ...
		        /* ISO C99 Flexible array members are effectively
		          identical to GCC's zero-length array extension.  */
		            if (flexible_array_member || array_parm_vla_unspec_p)
	       	        ...
	       	        n_debug("grokdeclarator bb 还是cdk_array decl_context == PARM ");
		            if (array_parm_vla_unspec_p){
		               itype = build_range_type (sizetype, size_zero_node, NULL_TREE);
		               size_varies = true;
		            }
	            }else if (decl_context == TYPENAME){
	       	         n_debug("grokdeclarator cc 还是cdk_array decl_context == TYPENAME ");
		             if (array_parm_vla_unspec_p){
		                /* C99 6.7.5.2p4 */
	          ...
	          /* Complain about arrays of incomplete types.  */
	            if (!COMPLETE_TYPE_P (type)){
		          ...
		          /* See if we can be more helpful.  */
		           if (TREE_CODE (type) == ARRAY_TYPE){
		             if (name)
		                inform (loc, "declaration of %qE as multidimensional array must have bounds for all dimensions except the first", name);
		             else
		                inform (loc, "declaration of multidimensional array must have bounds for all dimensions except the first");
		           }
		           type = error_mark_node;
	            }else{
	            ...
	            if (decl_context != PARM && (array_ptr_quals != TYPE_UNQUALIFIED || array_ptr_attrs != NULL_TREE || array_parm_static)){
					error_at (loc, "static or type qualifiers in non-parameter array declarator");
					array_ptr_quals = TYPE_UNQUALIFIED;
					array_ptr_attrs = NULL_TREE;
					array_parm_static = false;
	         ...
	         case cdk_function:
	          {
      	       n_debug("grokdeclarator 88   declarator->kind= cdk_function type:%s ",
				...
				n_debug("grokdeclarator 88 参数是 ----aa %p\n",arg_types);
				n_debug("grokdeclarator 88 参数是 ----aaee %p %d %d\n",arg_types,arg_types==NULL_TREE,arg_types==error_mark_node);
//
//				n_debug("grokdeclarator 88 参数是 ----aatt %s %d\n",get_tree_code_name(TREE_CODE(arg_types)),arg_types==NULL_TREE);
		          ...
		          if (specs_loc == UNKNOWN_LOCATION)
		           ...
		           if (quals_used && VOID_TYPE_P (type) && really_funcdef)
					...
					/* Ensure an error for restrict on invalid types; the
					   ...
					   this.  */
		            if (flag_isoc11  && (type_quals & TYPE_QUAL_RESTRICT)  && (!POINTER_TYPE_P (type)|| !C_TYPE_OBJECT_OR_INCOMPLETE_P (TREE_TYPE (type))))
		              error_at (loc, "invalid use of %<restrict%>");
		            type = c_build_qualified_type (type, quals_used);
	            }
	           type_quals = TYPE_UNQUALIFIED;
      	       n_debug("grokdeclarator 88 aa  declarator->kind= cdk_function build_function_type treecode:%s ",
      	    		 get_tree_code_name(TREE_CODE(type)));
	    ...
	    /* Set the TYPE_CONTEXTs for each tagged type which is local to
	       ...
	       the FUNCTION_TYPE node itself.  */
	         ...
	         case cdk_pointer:
	          {
	      	       //n_debug("grokdeclarator 99   declarator->kind= cdk_pointer treecode:%s %p type_quals:%d ",
	      	    		// get_tree_code_name(TREE_CODE(type)),type,type_quals);
//	               if(TREE_CODE(type)==RECORD_TYPE){
//					 tree tx=TYPE_NAME(type);
//					 n_debug("grokdeclarator 88-- %p\n",tx);
//					 n_debug("grokdeclarator 88ww-- %s\n",get_tree_code_name(TREE_CODE(tx)));
//					 tree id=DECL_NAME(tx);
//					 n_debug("grokdeclarator 88tt-- %s\n",IDENTIFIER_POINTER(id));
//	           	   }
	    ...
	    /* When the pointed-to type involves components of variable size,
	       ...
	       use BIND_EXPRs in TYPENAME contexts too.  */
	            if (!TYPE_NAME (type) && variably_modified_type_p (type, NULL_TREE)){
		      	       n_debug("grokdeclarator ---99   declarator->kind= cdk_pointer treecode:%d %p ",
		      	    		 get_tree_code_name(TREE_CODE(type)),type);
		           tree bind = NULL_TREE;
		           ...
		           if (bind){
	      	       ...
	      	       //n_debug("grokdeclarator ---99 eee  declarator->kind= cdk_pointer treecode:%s %p ",
	      	    		 //get_tree_code_name(TREE_CODE(type)),type);
	            type = c_build_pointer_type (type);
	      	      // n_debug("grokdeclarator ---99 www  declarator->kind= cdk_pointer treecode:%s %p ",
	      	    		// get_tree_code_name(TREE_CODE(type)),type);
  ...
  *decl_attrs = chainon (decl_id_attrs, *decl_attrs);
  ...
  if (!ADDR_SPACE_GENERIC_P (address_space)){
	  n_debug("下半部grokdeclarator 00 !ADDR_SPACE_GENERIC_P (address_space) ");

      if (decl_context == NORMAL){
    	  n_debug("下半部grokdeclarator 11 !ADDR_SPACE_GENERIC_P (address_space)  decl_context == NORMAL ");
	     switch (storage_class)
	      {
	       ...
	       if (name)
	         error ("%qs specified for structure field %qE",
		      ...
		      c_addr_space_name (address_space));
	   }//end decl_context == NORMAL
   }//end ADDR_SPACE_GENERIC_P (address_space)
  ...
  if (storage_class == csc_typedef){
	  n_debug("下半部grokdeclarator 22 storage_class == csc_typedef build_decl后返回decl了 ");
      ...
      if (type_quals)
	      type = c_build_qualified_type (type, type_quals, orig_qual_type,orig_qual_indirect);
      n_debug("------------xxxx %s type:%p\n",get_tree_code_name(TREE_CODE(type)),type);
      decl = build_decl (declarator->id_loc,TYPE_DECL, declarator->u.id.id, type);
	  n_debug("下半部grokdeclarator 22-- decl:%p ident:%s %p %s type:%p %p\n",decl,get_tree_code_name(TREE_CODE(declarator->u.id.id)),
			  declarator->u.id.id,IDENTIFIER_POINTER(declarator->u.id.id),type,TREE_TYPE (decl));
      ...
      if (warn_cxx_compat && declarator->u.id.id != NULL_TREE){
	     ...
	     n_debug("eeeeeeeeeeeeeeeeeeeeeee\n");
   	     if (b != NULL && b->decl != NULL_TREE && (B_IN_CURRENT_SCOPE (b)|| (current_scope == file_scope && B_IN_EXTERNAL_SCOPE (b)))
	         ...
	         auto_diagnostic_group d;
	         if (warning_at (declarator->id_loc, OPT_Wc___compat,("using %qD as both a typedef and a tag is "
			       "invalid in C++"), decl)  && b->locus != UNKNOWN_LOCATION)
		       inform (b->locus, "originally defined here");
	    }
	  }
      return decl;
  ...
  /* If this is a type name (such as, in a cast or sizeof),
   ...
   if (decl_context == TYPENAME){
	 ...
	 and fields.  */
		  n_debug("下半部grokdeclarator 22xxx sdecl_context == TYPENAME 返回type ");
  ...
  /* Aside from typedefs and type names (handle above),
   ...
   if (VOID_TYPE_P (type) && decl_context != PARM && !((decl_context != FIELD && TREE_CODE (type) != FUNCTION_TYPE)
	    && (storage_class == csc_extern || (current_scope == file_scope && !(storage_class == csc_static
			 || storage_class == csc_register))))) {
...

  {
    ...
    if (decl_context == PARM){
		...
		n_debug("下半部grokdeclarator 33 decl_context == PARM ");

	/* A parameter declared as an array of T is really a pointer to T.
	   One declared as a function is really a pointer to a function.  */
	   if (TREE_CODE (type) == ARRAY_TYPE){
		   n_debug("下半部grokdeclarator 44 decl_context == PARM TREE_CODE (type) == ARRAY_TYPE ");
	    ...
	    /* We don't yet implement attributes in this context.  */
		   ...
		   n_debug("下半部grokdeclarator 55 decl_context == PARM TREE_CODE (type) == FUNCTION_TYPE ");
	...
	/* Note that the grammar rejects storage classes in typenames
	   and fields.  */
		n_debug("下半部grokdeclarator 66 decl_context == FIELD build_decl 结构体struct内的内容通过grokfield调到这里 %s ",
				get_tree_code_name(TREE_CODE(type)));
	   gcc_assert (storage_class == csc_none && !threadp && !declspecs->inline_p && !declspecs->noreturn_p);
	...
	/* Structure field.  It may not be a function.  */
	  ...
	  if (size_varies)
	   C_DECL_VARIABLE_SIZE (decl) = 1;
   }else if (TREE_CODE (type) == FUNCTION_TYPE){ //decl_context == PARM的else if
		n_debug("下半部grokdeclarator 77 TREE_CODE (type) == FUNCTION_TYPE build_decl name:%s ",
	...
	/* An uninitialized decl with `extern' is a reference.  */
		  n_debug("下半部grokdeclarator 88 处理的是VAR-DECL ");
	...
	/* C99 6.2.2p7: It is invalid (compile-time undefined
	   ...
	   the 'extern' declaration is taken to refer to that decl.) */
	    ...
	    /* At file scope, the presence of a `static' or `register' storage
	     ...
	     }else{	/* Not at file scope, only `static' makes a static definition.  */
     ...
     if ((storage_class == csc_extern || (storage_class == csc_none  && TREE_CODE (type) == FUNCTION_TYPE
	     && !funcdef_flag))	&& variably_modified_type_p (type, NULL_TREE)){
	/* C99 6.7.5.2p2 */
	    if (TREE_CODE (type) == FUNCTION_TYPE)
...
static tree grokparms (struct c_arg_info *arg_info, bool funcdef_flag)
{
    ...
    if (arg_types == NULL_TREE && !funcdef_flag && !in_system_header_at (input_location))
        warning (OPT_Wstrict_prototypes,"function declaration isn%'t a prototype");
    ...
    if (arg_types == error_mark_node)
      ...
      /* In C2X, convert () in a function definition to (void).  */
        if (flag_isoc2x && funcdef_flag && !arg_types && !arg_info->parms)
	 ...
	 warn.  */
        ...
        for (parm = arg_info->parms, typelt = arg_types, parmno = 1;parm;parm = DECL_CHAIN (parm), typelt = TREE_CHAIN (typelt), parmno++){
	       ...
	       if (!COMPLETE_TYPE_P (type)){
	          if (funcdef_flag){
		        if (DECL_NAME (parm))
		           error_at (input_location,"parameter %u (%q+D) has incomplete type",parmno, parm);
		        else
		           error_at (DECL_SOURCE_LOCATION (parm),"parameter %u has incomplete type",parmno);
		        ...
		        arg_types = NULL_TREE;
		         ...
		         if (DECL_NAME (parm))
		             ...
		             warning_at (DECL_SOURCE_LOCATION (parm), 0,"parameter %u has void type",parmno);
		     }
	      }
...
struct c_arg_info *get_parm_info (bool ellipsis, tree expr)
{
...
tree start_struct (location_t loc, enum tree_code code, tree name,class c_struct_parse_info **enclosing_struct_parse_info)
{
  /* If there is already a tag defined at this scope
	...
	n_debug("struct_start 00 code：%d %s,行:%d 列:%d ",
		 			  code,get_tree_code_name(code),xloc.line, xloc.column);
  tree ref = NULL_TREE;
  location_t refloc = UNKNOWN_LOCATION;

  if (name != NULL_TREE){
    ref = lookup_tag (code, name, true, &refloc);
    xloc = expand_location(refloc);
    n_debug("struct_start 11 是否有重复的名字  code：%d,codeName:%s 行:%d 列:%d ref:%p",
    		 			  code,get_tree_code_name(code),xloc.line, xloc.column,ref);
  }
  if (ref && TREE_CODE (ref) == code){
  ...
  if (ref == NULL_TREE || TREE_CODE (ref) != code){
      ref = make_node (code);
      n_debug("struct_start 22 make_node  code：%d,codeName:%s 行:%d 列:%d DECL_LANG_SPECIFIC:%p %p",
         		 			  code,get_tree_code_name(code),xloc.line, xloc.column,DECL_LANG_SPECIFIC(ref),ref);
      pushtag (loc, name, ref);
      n_debug("struct_start ---22 make_node  code：%d,codeName:%s 行:%d 列:%d DECL_LANG_SPECIFIC:%p %p",
             		 			  code,get_tree_code_name(code),xloc.line, xloc.column,DECL_LANG_SPECIFIC(ref),ref);
  }
  ...
  *enclosing_struct_parse_info = struct_parse_info;
  ...
  if (warn_cxx_compat && (in_sizeof || in_typeof || in_alignof))
    warning_at (loc, OPT_Wc___compat,"defining type in %qs expression is invalid in C++",(in_sizeof ? "sizeof": (in_typeof ? "typeof" : "alignof")));
...
tree grokfield (location_t loc, struct c_declarator *declarator, struct c_declspecs *declspecs,
	   tree width, tree *decl_attrs)
{
   ...
   if (declarator->kind == cdk_id && declarator->u.id.id == NULL_TREE  && width == NULL_TREE){
	 ...
	 that took root before someone noticed the bug...  */
      ...
      tree type = declspecs->type;
      bool ok = false;
		n_debug("糅合成一个FIELD_DECL 22 declarator->kind:%d RECORD_OR_UNION_TYPE_P (type):%d ",
				declarator->kind,RECORD_OR_UNION_TYPE_P (type));

      if (RECORD_OR_UNION_TYPE_P (type)  && (flag_ms_extensions || flag_plan9_extensions || !declspecs->typedef_p)){
      ...
      if (flag_isoc99)
	      pedwarn_c99 (loc, OPT_Wpedantic,"ISO C99 doesn%'t support unnamed structs/unions");
      else
...
static void warn_cxx_compat_finish_struct (tree fieldlist, enum tree_code code,location_t record_loc)
{
  unsigned int ix;
  tree x;
  struct c_binding *b;
	       ...
	       class c_struct_parse_info *enclosing_struct_parse_info)
{
  ...
  if (pedantic){
      ...
      if (x == NULL_TREE){
    	  n_debug("finish_struct 11 没有fieldlist也就是没有内容 :%d :%d pedantic:%d ", xloc.line,xloc.column,pedantic);
  ...
  for (x = fieldlist; x; x = DECL_CHAIN (x)){
	 ...
	 treated in some ways as volatile.  */
      ...
      if (TYPE_PACKED (t) && (DECL_BIT_FIELD (x) || TYPE_ALIGN (TREE_TYPE (x)) > BITS_PER_UNIT))
	      DECL_PACKED (x) = 1;
	  ...
	  && TYPE_MAX_VALUE (TYPE_DOMAIN (TREE_TYPE (x))) == NULL_TREE){
	      if (TREE_CODE (t) == UNION_TYPE){
	         error_at (DECL_SOURCE_LOCATION (x),"flexible array member in union");
	         TREE_TYPE (x) = error_mark_node;
	       } else if (DECL_CHAIN (x) != NULL_TREE){
	         error_at (DECL_SOURCE_LOCATION (x),"flexible array member not at end of struct");
	         TREE_TYPE (x) = error_mark_node;
	       } else if (!saw_named_field) {
	          error_at (DECL_SOURCE_LOCATION (x),"flexible array member in a struct with no named "
			"members");
	          TREE_TYPE (x) = error_mark_node;
	       }
	  }

      if (pedantic && TREE_CODE (t) == RECORD_TYPE  && flexible_array_type_p (TREE_TYPE (x)))
	      pedwarn (DECL_SOURCE_LOCATION (x), OPT_Wpedantic, "invalid use of structure with flexible array member");

      if (DECL_NAME (x)  || RECORD_OR_UNION_TYPE_P (TREE_TYPE (x)))
...

  {
    ...
    if (len > 15){
	   ...
	   len += list_length (x);
	   n_debug("finish_struct 66 :%d :%d  pedantic:%d DECL_LANG_SPECIFIC:%p t:%p", xloc.line,xloc.column,pedantic,DECL_LANG_SPECIFIC(t),t);
	...
	/* Use the same allocation policy here that make_node uses, to
	  ...
	  All decls in an inline function need to be saved.  */
	   ...
	   space2 = (sorted_fields_type *) ggc_internal_alloc(sizeof (struct sorted_fields_type) + len * sizeof (tree));
	   len = 0;
	   space->s = space2;
  ...
  if (TREE_CODE (t) == UNION_TYPE  && TYPE_TRANSPARENT_AGGR (t)
  ...
  if (building_stmt_list_p () && variably_modified_type_p (t, NULL_TREE)){
  	  n_debug("finish_struct 101 :%d :%d  pedantic:%d ", xloc.line,xloc.column,pedantic);

    add_stmt (build_stmt (loc,DECL_EXPR, build_decl (loc, TYPE_DECL, NULL, t)));
  }

  if (warn_cxx_compat){
  	  n_debug("finish_struct 102 :%d :%d  pedantic:%d ", xloc.line,xloc.column,pedantic);
  ...
  /* If this struct is defined inside a struct, add it to
     struct_types.  */
  if (warn_cxx_compat  && struct_parse_info != NULL   && !in_sizeof && !in_typeof && !in_alignof){
  	  n_debug("finish_struct 103 :%d :%d  pedantic:%d ", xloc.line,xloc.column,pedantic);

    struct_parse_info->struct_types.safe_push (t);
  }
	  n_debug("finish_struct 104 :tree_code:%d t:%p %d :%d pedantic:%d ",
			  TREE_CODE(t),t,xloc.line,xloc.column,pedantic);
...
bool start_function (struct c_declspecs *declspecs, struct c_declarator *declarator,tree attributes)
{
  tree decl1, old_decl;
  ...
  n_debug("开始函数定义 00 在grokdeclarator 把声明上下文(enum decl_context)FUNCDEF 改成了 NORMAL %s ",
		  decl1?get_tree_code_name(TREE_CODE(decl1)):"null");
  ...
  if (DECL_DECLARED_INLINE_P (decl1) && DECL_UNINLINABLE (decl1)  && lookup_attribute ("noinline", DECL_ATTRIBUTES (decl1)))
  ...
  if (warn_about_return_type)
    warn_defaults_to (loc, flag_isoc99 ? OPT_Wimplicit_int: (warn_return_type > 0 ? OPT_Wreturn_type: OPT_Wimplicit_int),"return type defaults to %<int%>");
  ...
  if (old_decl && TREE_CODE (old_decl) != FUNCTION_DECL){
	n_debug("开始函数定义 11 函数名在当前范围已存在了。%s ",IDENTIFIER_POINTER(decl1),
	    	 	  				TREE_CODE(decl1),get_tree_code_name(TREE_CODE(decl1)));
    old_decl = NULL_TREE;
  }
  ...
  if (!prototype_p (TREE_TYPE (decl1))){
	  n_debug("开始函数定义 22 %s ",get_tree_code_name(TREE_CODE(decl1)));
      if (old_decl != NULL_TREE && TREE_CODE (TREE_TYPE (old_decl)) == FUNCTION_TYPE && comptypes (TREE_TYPE (TREE_TYPE (decl1)),
			TREE_TYPE (TREE_TYPE (old_decl)))){
    	  n_debug("开始函数定义 33 %s ",get_tree_code_name(TREE_CODE(decl1)));
	     ...
	     TREE_TYPE (decl1) = composite_type (TREE_TYPE (old_decl),TREE_TYPE (decl1));
	     current_function_prototype_locus = DECL_SOURCE_LOCATION (old_decl);
	     current_function_prototype_built_in  = C_DECL_BUILTIN_PROTOTYPE (old_decl);
	     current_function_prototype_arg_types = TYPE_ARG_TYPES (TREE_TYPE (decl1));
	  }
      if (TREE_PUBLIC (decl1)){
    	  n_debug("开始函数定义 44 %s ",get_tree_code_name(TREE_CODE(decl1)));
	     ...
	     if (b){
	         ...
	         if (TREE_CODE (ext_type) == FUNCTION_TYPE  && comptypes (TREE_TYPE (TREE_TYPE (decl1)),TREE_TYPE (ext_type))){
  ...
  /* If #pragma weak was used, mark the decl weak now.  */
  if (current_scope == file_scope){
	  n_debug("开始函数定义 55 ------current_scope == file_scope------- %s ",
			  get_tree_code_name(TREE_CODE(decl1)));
      maybe_apply_pragma_weak (decl1);
  }

  /* Warn for unlikely, improbable, or stupid declarations of `main'.  */
  if (warn_main && MAIN_NAME_P (DECL_NAME (decl1))){
	  n_debug("开始函数定义 66 %s ",get_tree_code_name(TREE_CODE(decl1)));
  ...
  /* Record the decl so that the function name is defined.
     ...
     use the old decl.  */
  n_debug("开始函数定义 77 开始pushdecl  decl1:%p %s %s",decl1,get_tree_code_name(TREE_CODE(decl1)),IDENTIFIER_POINTER(DECL_NAME(decl1)));
  current_function_decl = pushdecl (decl1);
  n_debug("开始函数定义 88  decl1:%p current_function_decl:%p %s ",
		  decl1,current_function_decl,get_tree_code_name(TREE_CODE(decl1)));
  push_scope ();
  declare_parm_level ();
...
finish_function (location_t end_loc)
{
  ...
  if (TREE_CODE (fndecl) == FUNCTION_DECL  && targetm.calls.promote_prototypes (TREE_TYPE (fndecl))){
	  //n_debug("完成函数  11 %s ",get_tree_code_name(TREE_CODE(fndecl)));
  ...
  if (MAIN_NAME_P (DECL_NAME (fndecl)) && flag_hosted  && TYPE_MAIN_VARIANT (TREE_TYPE (TREE_TYPE (fndecl))) == integer_type_node && flag_isoc99){
      ...
      */
	  n_debug("完成函数  44 c_finish_return %s ",get_tree_code_name(TREE_CODE(fndecl)));
  ...
  n_debug("完成函数  77 %s ",get_tree_code_name(TREE_CODE(fndecl)));

  /* Complain about parameters that are only set, but never otherwise used.  */
  if (warn_unused_but_set_parameter) {
      tree decl;
      n_debug("完成函数  88 %s ",get_tree_code_name(TREE_CODE(fndecl)));
  ...
  if (DECL_INITIAL (fndecl) && DECL_INITIAL (fndecl) != error_mark_node && !undef_nested_function){
   	     //n_debug("完成函数  101 %s ",get_tree_code_name(TREE_CODE(fndecl)));
...
struct c_declspecs *declspecs_add_type (location_t loc, struct c_declspecs *specs, struct c_typespec spec)
{
  tree type = spec.spec;
  specs->non_sc_seen_p = true;
  specs->declspecs_seen_p = true;
  specs->non_std_attrs_seen_p = true;
  specs->typespec_kind = spec.kind; //征对struct type是RECORD_TYPE的,kind是ctsk_tagdef specs->locations不会被设置
	         ...
	         switch (i){
	            case RID_LONG:
	                ...
	                if (specs->long_p){
		                if(specs->typespec_word == cts_double){
		                   error_at (loc,("both %<long long%> and %<double%> in "
	                ...
	                if (specs->short_p)
		                ...
		                error_at (loc,("both %<long%> and %<__int%d%> in declaration specifiers"),int_n_data[specs->int_n_idx].bitsize);
		               ...
		               error_at (loc, ("both %<long%> and %<_Float%d%s%> in declaration specifiers"), floatn_nx_types[specs->floatn_nx_idx].n, (floatn_nx_types[specs->floatn_nx_idx].extended ? "x"  : ""));
	            ...
	            case RID_SHORT:
	                ...
	                if (specs->long_p)
		              ...
		              error_at (loc,("both %<short%> and %<__int%d%> in declaration specifiers"),int_n_data[specs->int_n_idx].bitsize);
	               else if (specs->typespec_word == cts_bool)
		              ...
		              error_at (loc,("both %<short%> and %<_Float%d%s%> in declaration specifiers"),floatn_nx_types[specs->floatn_nx_idx].n,(floatn_nx_types[specs->floatn_nx_idx].extended ? "x": ""));
	               else if (specs->typespec_word == cts_dfloat32)
  ...
  if (specs->type || specs->typespec_word != cts_none || specs->long_p || specs->short_p || specs->signed_p
      ...
      if (TREE_TYPE (type) == error_mark_node)
	     ...
	     n_debug("进入 TYPE_DECL type:%p type code:%s tree_type:%p %s\n",type,get_tree_code_name(TREE_CODE(type)),
	    		 specs->type,get_tree_code_name(TREE_CODE(specs->type)));
...
struct c_declspecs *declspecs_add_scspec (location_t loc,struct c_declspecs *specs,tree scspec)
{
  enum rid i;
  ...
  switch (i)
    ...
    case RID_TYPEDEF:
      n = csc_typedef;
  	  n_debug("加入声明说明符 RID_TYPEDEF specs->storage_class：%d n:%d ",specs->storage_class,n);
      break;
    default:
      gcc_unreachable ();
    }
...
struct c_declspecs * declspecs_add_attrs (location_t loc, struct c_declspecs *specs, tree attrs)
{
...
/* Add an _Alignas specifier (expression ALIGN, or type whose
   ...
   SPECS.  */
struct c_declspecs *declspecs_add_alignas (location_t loc,struct c_declspecs *specs, tree align)
{
...
struct c_declspecs *finish_declspecs (struct c_declspecs *specs)
{
...
void c_c_decl_bind_file_scope (tree  decl)
{
   ...
   struct c_binding *b;
   bool nested = false;
   location_t locus = DECL_SOURCE_LOCATION (decl);
   bind (name, decl, scope,/*invisible=*/false, nested, locus);
}
