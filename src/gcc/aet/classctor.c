...
{
   c_parser *parser=self->parser->parser;
   if(self->parser->isAet && isConstructor){
...
static struct c_arg_info *createParam (c_parser *parser, char *className)
{
   ...
   tree decl=class_impl_push_parm_decl/*!push_parm_decl*/(class_impl_get(),parm, &expr);
...
static nboolean  haveFieldOrDefine(ClassCtor *self,ClassName *className,nboolean field)
{
   ...
   for(i=0;i<array->len;i++){
      ...
      if(strcmp(item->orgiName,className->userName)==0
            && aet_utils_valid_tree(field?item->fieldDecl:item->fromImplDefine)){
...
tree   class_ctor_create_default_decl(ClassCtor *self,ClassName *className,tree structType)
{
   ...
   // nboolean change=func_mgr_change_class_func_decl(func_mgr_get(),pointerLast,className,structType,NULL);
   int errInfo=0;
   ClassFunc *newFunc=func_mgr_change_class_func_decl(func_mgr_get(),pointerLast,className,structType,&errInfo);

   tree decls = NULL_TREE;
   tree postfix_attrs = NULL_TREE;
   tree d = grokfield (c_parser_peek_token (parser)->location,pointerLast, specs, NULL_TREE, &all_prefix_attrs,NULL);
...
static void superToConstructor(ClassCtor *self,ClassName *className)
{
   ...
   aet_print_token_in_parser("class_ctor superToConstructor className ---- %s superConstructorCount:%d",
         className->sysName,self->superOfSelfParseing);
}
        ...
        vec<tree, va_gc> *origtypes,vec<location_t> arg_loc,location_t expr_loc,SelectFunc *selectFunc)
{
    ...
    if(class_func_is_func_generic(classFunc) || class_func_have_query_param(classFunc)){
        n_debug("class_ctor_select 进这里了------因为%s是一个泛型类或带问号的构造函数。",funName);
        nboolean ok= generic_call_check/*!generic_call_add_fpgi_parm*/(generic_call_get(),
              classFunc,className,exprlist,funcGenericDefine,expr_loc);
    }
