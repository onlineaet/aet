...
void insn_change::print (pretty_printer *pp) const
{
      if (m_is_deletion){
...
// temporary obstack.
access_array function_info::temp_access_array (access_array accesses)
{
   if (accesses.empty ())
      return accesses;
   ...
   return { static_cast<access_info **> (obstack_finish (&m_temp_obstack)),accesses.size () };
}

// See the comment above the declaration.
bool function_info::verify_insn_changes (array_slice<insn_change *const> changes)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);

   HardRegSet /*!HARD_REG_SET*/ defined_hard_regs = {mtcs_reg_get_hard_reg_element_count(mtcsReg)};
   HardRegSet /*!HARD_REG_SET*/ clobbered_hard_regs = {mtcs_reg_get_hard_reg_element_count(mtcsReg)};

   mtcs_reg_clear_hard_reg_set/*!CLEAR_HARD_REG_SET*/(&defined_hard_regs);
   mtcs_reg_clear_hard_reg_set/*!CLEAR_HARD_REG_SET*/(&clobbered_hard_regs);
      ...
      if (!change->is_deletion ()){
         ...
         for (use_info *use : change->new_uses){
            ...
            if (mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,regno)
                  && mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&clobbered_hard_regs, regno)){
               if (dump_file && (dump_flags & TDF_DETAILS))
                  fprintf (dump_file, "register %d would be clobbered while it is still live\n", regno);
               return false;
            }
         }
         for (def_info *def : change->new_defs){
            ...
            if (mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,regno)){
               if (def->m_is_temp){
                  ...
                  if (mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&defined_hard_regs, regno)){
                     if (dump_file && (dump_flags & TDF_DETAILS))
                        fprintf (dump_file, "conflicting definitions of register %d\n", regno);
                     return false;
                  }
                  mtcs_reg_set_hard_reg_bit/*!SET_HARD_REG_BIT*/(mtcsReg,&clobbered_hard_regs, regno);
               }else if (is_a<set_info *> (def)){
               // REGNO now has a defined value.
                  mtcs_reg_set_hard_reg_bit/*!SET_HARD_REG_BIT*/(mtcsReg,&defined_hard_regs, regno);
                  mtcs_reg_clear_hard_reg_bit/*!CLEAR_HARD_REG_BIT*/(mtcsReg,&clobbered_hard_regs, regno);
               }
            }
         }
      }
...
// See the comment above the declaration.gcc-15
bool mtcs_rtl_ssa::changes_are_worthwhile (array_slice<insn_change *const> changes,
             bool strict_p)
{
   ...
   auto entry_count = ENTRY_BLOCK_PTR_FOR_FN (cfun)->count;
   {
      ...
      for (insn_change *change : changes){
         ...
         if (optimize_bb_for_speed_p (cfg_bb))
            weighted_old_cost += (cfg_bb->count.to_sreal_scale (entry_count) * change->old_cost ());
      }
   }
   ...
   for (insn_change *change : changes){
      ...
      if (!change->is_deletion ()  && INSN_CODE (change->rtl ()) != NOOP_MOVE_INSN_CODE){
         change->new_cost = insn_cost (change->rtl (), for_speed);
         /* If the cost is unknown, replacement is not worthwhile.  */
         if (!change->new_cost){
            if (dump_file && (dump_flags & TDF_DETAILS))
               fprintf (dump_file, "Reject replacement due to unknown insn cost.\n");
            return false;
         }
         new_cost += change->new_cost;
         if (for_speed)
            weighted_new_cost += (cfg_bb->count.to_sreal_scale (entry_count) * change->new_cost);
      }
   }
   bool ok_p;
...
void function_info::process_uses_of_deleted_def (set_info *set)
{
   if (!set->has_any_uses ())
      return;
...
static void update_notes (rtx_insn *insn)
{
...
static insn_info *choose_insn_placement (insn_change &change)
{
...
void function_info::possibly_queue_changes (insn_change &change)
{
   insn_info *insn = change.insn ();
   ...
   auto needs_pending_update = [&]()
   {
      ...
      if (GET_CODE (pattern) == TRAP_IF   && XEXP (pattern, 0) == const1_rtx)
         return true;

      return false;
   };

   if (needs_pending_update ()   && bitmap_set_bit (m_queued_insn_update_uids, insn->uid ())){
...
static void delete_insn (insn_change &change)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   ...
   mtcs_rtl_set_insn_deleted/*!set_insn_deleted*/(mtcsRTL,rtl);
}

// Move the RTL instruction associated with CHANGE so that it comes
// immediately after AFTER.
static void move_insn (insn_change &change, insn_info *after)
{
   rtx_insn *rtl = change.rtl ();
...
static void update_insn_in_place (insn_change &change)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   ...
   mtcs_dfscan_df_insn_rescan/*!df_insn_rescan*/(mtcsDfscan,change.rtl ());
}
...
void function_info::finalize_new_accesses (insn_change &change, insn_info *pos, hash_set<def_info *> &new_sets)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
      ...
      if (ref.is_write ()){
         ...
         if (def->m_is_temp){
            if (is_a<clobber_info *> (def))
               ...
               def_info *perm_def = allocate<set_info> (change.insn (), def->resource ());
   ...
   // removed later.
   for (def_info *def : change.new_defs)
      if (def->m_has_been_superceded && (def->is_call_clobber () || def->is_mem ())){
         def->m_has_been_superceded = false;
         def->set_insn (insn);
         m_temp_defs.safe_push (def);
      }
   ...
   auto *unshared_uses_base = XOBNEWVEC (&m_temp_obstack, access_info *,change.new_uses.size ());
   unsigned int i = 0;
   for (use_info *use : change.new_uses){
      ...
      if (ref.is_read ()){
         ...
         if (use->m_has_been_superceded){
            ...
            *use = use_info (insn, resource_info { mode, regno }, use->def ());
            ...
            if (mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,regno)
                  && mtcs_mode_partial_subreg_p/*!partial_subreg_p*/(mtcsMode,use->mode (), mode))
               use->set_mode (mode);
...
void function_info::apply_changes_to_insn (insn_change &change,hash_set<def_info *> &new_sets)
{
   insn_info *insn = change.insn ();
   if (change.is_deletion ()){
...
insn_info *function_info::add_placeholder_after (insn_info *after)
{
...
// See the comment above the declaration.
void function_info::change_insns (array_slice<insn_change *> changes)
{
   ...
   for (unsigned int i = changes.size (); i-- > 0;){
      ...
      if (change.is_deletion ())
         ...
         if (following_insn){
            change.move_range = move_earlier_than (change.move_range,following_insn);
            if (!canonicalize_move_range (change.move_range,change.insn ()))
   ...
   for (unsigned i = 0; i < changes.size (); i++){
      ...
      if (!change.is_deletion ())
         finalize_new_accesses (change, placeholder ? placeholder : change.insn (),new_sets);
   }
...
// See the comment above the declaration.
void function_info::change_insn (insn_change &change)
{
...
static bool add_clobber (insn_change &change, add_regno_clobber_fn add_regno_clobber,
        rtx clobber)
{
   ...
   if (GET_CODE (dest) == SCRATCH){
      if (reload_completed){
         if (dump_file && (dump_flags & TDF_DETAILS)){
            ...
            fprintf (dump_file, "instruction requires a scratch after reload:\n");
...
static access_info *find_clobbered_access (access_array accesses, rtx pat)
{
   rtx subpat;
      ...
      if (GET_CODE (subpat = XVECEXP (pat, 0, i)) == CLOBBER){
         ...
         if (REG_P (x))
            for (auto *access : accesses)
               if (access->regno () >= REGNO (x) && access->regno () < END_REGNO (x))
                  return access;
      }
   return nullptr;
}

static void printRtx(rtx x1)
{
      rtx x2, x3, x4;
      x2 = XEXP (x1, 1);
      x3 = XEXP (x1, 0);
      n_debug("mtcsptxrecog.c recog_cb  x1:%d %s %s mode:%d x2:%p x3:%p\n",GET_CODE (x1),
            GET_RTX_NAME(GET_CODE(x1)),GET_RTX_FORMAT(GET_CODE(x1)), GET_MODE (x1),x2,x3);
//      if(x2)
//         fprintf(stderr,"mtcsptxrecog.c  recog_cb  x2:%d %s %s mode:%d\n",
//               GET_CODE (x2),GET_RTX_NAME(GET_CODE(x2)),GET_RTX_FORMAT(GET_CODE(x2)),GET_MODE (x2));
//      if(x3)
//         fprintf(stderr,"mtcsptxrecog.c  recog_cb  x3:%d %s %s mode:%d\n",
//               GET_CODE (x3),GET_RTX_NAME(GET_CODE(x3)),GET_RTX_FORMAT(GET_CODE(x3)),GET_MODE (x3));
...
// ADD_REGNO_CLOBBER is a specialization of function_info::add_regno_clobber
// for a specific caller-provided predicate.
static bool recog_level2 (insn_change &change, add_regno_clobber_fn add_regno_clobber)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);

   mtcs_insn_change_watermark insn_watermark(mtcsRecog);
   rtx_insn *rtl = change.rtl ();
   rtx pat = PATTERN (rtl);
   int num_clobbers = 0;
   int icode = -1;
   bool asm_p = asm_noperands (pat) >= 0;
   n_debug("mtcschanges.c recog_level2 00\n");
   if (asm_p){
      ...
      n_debug("mtcschanges.c recog_level2 11 noop_move_p (rtl) instruction becomes a no-op:\n");
      ...
      n_debug("mtcschanges.c recog_level2 22 执行 ::recog \n");
      printRtx(pat);
      icode = mtcs_recog_recog/*!::recog*/(mtcsRecog,pat, rtl, &num_clobbers);
      n_debug("mtcschanges.c recog_level2 33 icode:%d\n",icode);
      if (icode < 0){
         if (dump_file && (dump_flags & TDF_DETAILS)){
   ...
   n_debug("mtcschanges.c recog_level2 44 num_clobbers:%d\n",num_clobbers);
   ...
   if (num_clobbers > 0){
      ...
      n_debug("mtcschanges.c recog_level2 55 num_clobbers:%d\n",num_clobbers);

      rtx newpat = gen_rtx_PARALLEL (VOIDmode, newvec);
      mtcs_recog_add_clobbers/*!add_clobbers*/(mtcsRecog,newpat, icode);
      mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,rtl, &PATTERN (rtl), newpat, true);
      for (int i = 0; i < num_clobbers; ++i)
         if (!add_clobber (change, add_regno_clobber,XVECEXP (newpat, 0, oldlen + i))){
   ...
   INSN_CODE (rtl) = icode;
   if (mtcsRecog->recog_data.insn == rtl)
      mtcsRecog->recog_data.insn = nullptr;
   n_debug("mtcschanges.c recog_level2 66 pat:%d %s\n",GET_CODE (pat),GET_RTX_NAME(GET_CODE (pat)));
   ...
   if (GET_CODE (pat) == PARALLEL)
      for (int i = 0; i < XVECLEN (pat, 0); ++i)
         if (GET_CODE (subpat = XVECEXP (pat, 0, i)) == CLOBBER && REG_P (XEXP (subpat, 0))){
            // Stub out all operands, so that we can tell which registers
            // are hard-coded.
            mtcs_recog_extract_insn/*!extract_insn*/(mtcsRecog,rtl);
            for (int j = 0; j < mtcsRecog->recog_data.n_operands; ++j)
               *mtcsRecog->recog_data.operand_loc[j] = pc_rtx;

            auto *use = find_clobbered_access (change.new_uses, pat);

            // Restore the operands.
            for (int j = 0; j < mtcsRecog->recog_data.n_operands; ++j)
               *mtcsRecog->recog_data.operand_loc[j] = mtcsRecog->recog_data.operand[j];

            if (use){
               if (dump_file && (dump_flags & TDF_DETAILS)){
                  fprintf (dump_file, "register %d is both clobbered and used as an input:\n", use->regno ());
                  print_rtl_single (dump_file, pat);
               }
               return false;
            }
   ...
   n_debug("mtcschanges.c recog_level2 77 reload_completed:%d asm_p:%d\n",reload_completed,asm_p);
   ...
   if (reload_completed && !asm_p){
      ...
      if (!mtcs_recog_constrain_operands/*!constrain_operands*/(mtcsRecog,1,
            mtcs_recog_get_preferred_alternatives/*!get_preferred_alternatives*/(mtcsRecog,rtl))){
         n_debug("mtcschanges.c recog_level2 88 reload_completed:%d asm_p:%d\n",reload_completed,asm_p);

         if (dump_file && (dump_flags & TDF_DETAILS)){
            if (asm_p)
               fprintf (dump_file, "asm does not match its constraints:\n");
            else if (const char *name = get_insn_name (icode))
               fprintf (dump_file, "instruction does not match the constraints for %s:\n", name);
            else
               fprintf (dump_file, "instruction does not match its constraints:\n");
            print_rtl_single (dump_file, pat);
         }
         change.new_defs = prev_new_defs;
         change.move_range = prev_move_range;
...
// ADD_REGNO_CLOBBER is a specialization of function_info::add_regno_clobber
// for a specific caller-provided predicate.
bool mtcs_rtl_ssa::recog_internal (insn_change &change, add_regno_clobber_fn add_regno_clobber)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);
   // Accept all changes to debug instructions.
   n_debug("mtcschanges.c recog_internal 00\n");
   insn_info *insn = change.insn ();
   ...
   if (GET_CODE (pat) == PARALLEL && asm_noperands (pat) < 0){
      n_debug("mtcschanges.c recog_internal 11\n");
      ...
      int old_num_changes = mtcs_recog_num_validated_changes/*!num_validated_changes*/(mtcsRecog);
      mtcs_recog_validate_change_xveclen/*!validate_change_xveclen*/(mtcsRecog,rtl, &PATTERN (rtl), new_len, true);
      if (recog_level2 (change, add_regno_clobber))
         return true;
      n_debug("mtcschanges.c recog_internal 22\n");
      ...
      while (new_len > 0  && GET_CODE (XVECEXP (pat, 0, new_len - 1)) == CLOBBER)
         new_len -= 1;
      if (new_len != prev_len){
         n_debug("mtcschanges.c recog_internal 33\n");
         mtcs_recog_validate_change_xveclen/*!validate_change_xveclen*/(mtcsRecog,rtl, &PATTERN (rtl), new_len, true);
         if (recog_level2 (change, add_regno_clobber))
            return true;
         n_debug("mtcschanges.c recog_internal 44\n");

         mtcs_recog_cancel_changes/*!cancel_changes*/(mtcsRecog,old_num_changes);
      }
      return false;
   }
   n_debug("mtcschanges.c recog_internal 55\n");

   return recog_level2 (change, add_regno_clobber);
}

// See the comment above the declaration.
bool function_info::perform_pending_updates ()
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);
   ...
   for (insn_info *insn : m_queued_insn_updates){
      ...
      if (NOTE_P (rtl))
         ...
         if (INSN_CODE (rtl) == NOOP_MOVE_INSN_CODE){
            ...
            mtcs_dojump_mark_jump_label/*!mark_jump_label*/(mtcsDojump,PATTERN (rtl), rtl, 0);
            mtcs_cfg_rtl_update_cfg_for_uncondjump/*!update_cfg_for_uncondjump*/(mtcsCfgRtl,rtl);
            changed_cfg = true;
            changed_jumps = true;
         }
         ...
         if (GET_CODE (pattern) == TRAP_IF && XEXP (pattern, 0) == const1_rtx){
            mtcs_cfg_context_remove_edge/*!remove_edge*/(mtcsCfgContext,
                  mtcs_cfg_context_split_block/*!split_block*/(mtcsCfgContext,BLOCK_FOR_INSN (rtl), rtl));
            mtcs_cfg_rtl_emit_barrier_after_bb/*!emit_barrier_after_bb*/(mtcsCfgRtl,BLOCK_FOR_INSN (rtl));
            changed_cfg = true;
         }
      }
   }
   ...
   EXECUTE_IF_SET_IN_BITMAP (m_need_to_purge_dead_edges, 0, index, bi)
      if (mtcs_cfg_rtl_purge_dead_edges/*!purge_dead_edges*/(mtcsCfgRtl,BASIC_BLOCK_FOR_FN (m_fn, index)))
         changed_cfg = true;
...
insn_info *function_info::create_insn (obstack_watermark &watermark,
             rtx_code insn_code,rtx pat)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   ...
   switch (insn_code){
      case INSN:
         rti = mtcs_emit_make_insn_raw/*!make_insn_raw*/(mtcsEmit,pat);
         break;
      default:
         gcc_unreachable ();
   }
...
void mtcs_rtl_ssa::pp_insn_change (pretty_printer *pp, const insn_change &change)
{
   change.print (pp);
}

// Print a description of CHANGE to FILE.
void dump (FILE *file, const insn_change &change)
{
