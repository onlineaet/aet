...
bool mtcs_outof_ssa_ssa_is_replaceable_p (MtcsOutofSsa *self,gimple *stmt)
{
   ...
   if (flag_float_store   && FLOAT_TYPE_P (TREE_TYPE (def)))
      return false;
...
class mtcs_elim_graph
{
public:
  mtcs_elim_graph (var_map map);
...
static void set_location_for_edge (MtcsOutofSsa *self,edge e)
{
...
static inline rtx_insn * emit_partition_copy (MtcsOutofSsa *self,rtx dest, rtx src, int unsignedsrcp, tree sizeexp)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsEmit *mtcsEmit=mtcs_target_get_emit(mtcsTarget);
   ...
   if (GET_MODE (src) != VOIDmode && GET_MODE (src) != GET_MODE (dest))
      src = mtcs_expr_convert_to_mode/*!convert_to_mode*/(mtcsExpr,GET_MODE (dest), src, unsignedsrcp);
   if (GET_MODE (src) == mtcsMode->modes.M_BLKmode){
      gcc_assert (GET_MODE (dest) == mtcsMode->modes.M_BLKmode);
      mtcs_expr_emit_block_move/*!emit_block_move*/(mtcsExpr,dest, src, expr_size (sizeexp), BLOCK_OP_NORMAL);
   }else
...
static void insert_partition_copy_on_edge (MtcsOutofSsa *self,edge e, int dest, int src, location_t locus)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   set_location_for_edge(self,e);
   /* If a locus is provided, override the default.  */
   if (locus)
      mtcs_emit_set_curr_insn_location/*!set_curr_insn_location*/(mtcsEmit,locus);

   var = partition_to_var (SA.map, src);
   rtx_insn *seq = emit_partition_copy(self,copy_rtx (SA.partition_to_pseudo[dest]),
         copy_rtx (SA.partition_to_pseudo[src]), TYPE_UNSIGNED (TREE_TYPE (var)),var);

   mtcs_cfg_rtl_insert_insn_on_edge/*!insert_insn_on_edge*/(mtcsCfgRtl,seq, e);
}

/* Insert a copy instruction from expression SRC to partition DEST
   onto edge E.  */

static void insert_value_copy_on_edge (MtcsOutofSsa *self,edge e, int dest, tree src, location_t locus)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsEmit *mtcsEmit=mtcs_target_get_emit(mtcsTarget);
   ...
   n_debug("mtcsoutofssa.c insert_value_copy_on_edge 00 TYPE_MODE (TREE_TYPE (src)):%d\n",src_mode);
   aet_print_tree(src);
   n_debug("mtcsoutofssa.c insert_value_copy_on_edge 11 TYPE_MODE (TREE_TYPE (name))):%d\n",TYPE_MODE (TREE_TYPE (name)));
   aet_print_tree(name);
   ...
   gcc_assert (!REG_P (dest_rtx) || dest_mode == mtcs_mode_promote_ssa_mode/*!promote_ssa_mode*/(mtcsMode,name, &unsignedp));

   if (src_mode != dest_mode){
      x = mtcs_expr_expand_expr/*!expand_expr*/(mtcsExpr,src, NULL, src_mode, EXPAND_NORMAL);
      x = mtcs_expr_convert_modes/*!convert_modes*/(mtcsExpr,dest_mode, src_mode, x, unsignedp);
   }else if (src_mode == mtcsMode->modes.M_BLKmode){
      x = dest_rtx;
      mtcs_expr_store_expr/*!store_expr*/(mtcsExpr,src, x, 0, false, false);
   }else
      x = mtcs_expr_expand_expr/*!expand_expr*/(mtcsExpr,src, dest_rtx, dest_mode, EXPAND_NORMAL);

   if (x != dest_rtx)
      mtcs_expr_emit_move_insn/*!emit_move_insn*/(mtcsExpr,dest_rtx, x);
   mtcs_dojump_do_pending_stack_adjust/*!do_pending_stack_adjust*/(mtcsDojump);
...
/* Insert a copy instruction from RTL expression SRC to partition DEST
   onto edge E.  */
static void insert_rtx_to_part_on_edge (MtcsOutofSsa *self,edge e, int dest, rtx src, int unsignedsrcp,
             location_t locus)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   rtx_insn *seq = emit_partition_copy(self,copy_rtx (SA.partition_to_pseudo[dest]),
         src, unsignedsrcp,partition_to_var (SA.map, dest));
...
static void insert_part_to_rtx_on_edge (MtcsOutofSsa *self,edge e, rtx dest, int src, location_t locus)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgRtl *mtcsCfgRtl=mtcs_target_get_cfg_rtl(mtcsTarget);
   ...
   var = partition_to_var (SA.map, src);
   rtx_insn *seq = emit_partition_copy(self,dest,
         copy_rtx (SA.partition_to_pseudo[src]),TYPE_UNSIGNED (TREE_TYPE (var)),var);
...
mtcs_elim_graph::mtcs_elim_graph (var_map map) :
...
static inline void clear_elim_graph (mtcs_elim_graph *g)
{
   g->nodes.truncate (0);
...
static inline int elim_graph_size (mtcs_elim_graph *g)
{
...
static inline void elim_graph_add_node (mtcs_elim_graph *g, int node)
{
   int x;
   int t;
...
static inline void elim_graph_add_edge (mtcs_elim_graph *g, int pred, int succ, location_t locus)
{
...
static inline int elim_graph_remove_succ_edge (mtcs_elim_graph *g, int node, location_t *locus)
{
   int y;
   unsigned x;
...
static inline void eliminate_name (mtcs_elim_graph *g, int T)
{
...
static inline bool queue_phi_copy_p (var_map map, tree t)
{
...
/* Build elimination graph G for basic block BB on incoming PHI edge
   G->e.  */
static void eliminate_build (mtcs_elim_graph *g)
{
   tree Ti;
   int p0, pi;
   gphi_iterator gsi;
   ...
   for (gsi = gsi_start_phis (g->e->dest); !gsi_end_p (gsi); gsi_next (&gsi)){
      ...
      n_debug("mtcsoutofssa.c eliminate_build 函数参数 保存在SA分区中 e:%p result:%p Ti=src:%p\n",g->e,gimple_phi_result (phi),Ti);
      ...
      if (queue_phi_copy_p (g->map, Ti)){
         ...
         on this edge.  */
         g->const_dests.safe_push (p0);   //dest
         g->const_copies.safe_push (Ti); //src =Ti
...
static void elim_forward (mtcs_elim_graph *g, int T)
{
   int S;
   location_t locus;
...
static int elim_unvisited_predecessor (mtcs_elim_graph *g, int T)
{
   int P;
   location_t locus;
...
static void elim_backward (MtcsOutofSsa *self,mtcs_elim_graph *g, int T)
{
   int P;
   location_t locus;
   ...
   FOR_EACH_ELIM_GRAPH_PRED (g, T, P, locus,{
      if (!bitmap_bit_p (g->visited, P)){
         elim_backward(self,g, P);
         insert_partition_copy_on_edge(self,g->e, P, T, locus);
      }
   });
}
...
static rtx get_temp_reg (MtcsOutofSsa *self,tree name)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsEmit *mtcsEmit=mtcs_target_get_emit(mtcsTarget);
   ...
   int unsignedp;
   machine_mode reg_mode = mtcs_mode_promote_ssa_mode/*!promote_ssa_mode*/(mtcsMode,name, &unsignedp);
   if (reg_mode == mtcsMode->modes.M_BLKmode)
      return mtcs_func_assign_temp/*!assign_temp*/(mtcsFunc,type, 0, 0);
   rtx x = mtcs_emit_gen_reg_rtx/*!gen_reg_rtx*/(mtcsEmit,reg_mode);
   if (POINTER_TYPE_P (type))
      mtcs_rtl_mark_reg_pointer/*!mark_reg_pointer*/(mtcsRTL,x, TYPE_ALIGN (TREE_TYPE (type)));
   return x;
}
...
static void elim_create (MtcsOutofSsa *self,mtcs_elim_graph *g, int T)
{
   int P, S;
   location_t locus;
   ...
   if (elim_unvisited_predecessor (g, T)){
      ...
      rtx U = get_temp_reg(self,var);
      int unsignedsrcp = TYPE_UNSIGNED (TREE_TYPE (var));

      insert_part_to_rtx_on_edge(self,g->e, U, T, UNKNOWN_LOCATION);
      FOR_EACH_ELIM_GRAPH_PRED (g, T, P, locus,{
         if (!bitmap_bit_p (g->visited, P)){
            elim_backward(self,g, P);
            insert_rtx_to_part_on_edge(self,g->e, P, U, unsignedsrcp, locus);
         }
      });
   }else{
      ...
      if (S != -1){
         bitmap_set_bit (g->visited, T);
         insert_partition_copy_on_edge(self,g->e, T, S, locus);
      }
   }
}
...
static void eliminate_phi (MtcsOutofSsa *self,edge e, mtcs_elim_graph *g)
{
   int x;
   ...
   while (g->const_copies.length () > 0){
      ...
      insert_value_copy_on_edge(self,e, dest, src, locus);
   }
}
      ...
      for (gsi = gsi_start_phis (bb); !gsi_end_p (gsi); gsi_next (&gsi)){
         ...
         if (T0 == NULL_TREE){
            ...
            for (i = 0; i < gimple_phi_num_args (phi); i++){
               ...
               if (TREE_CODE (arg) == SSA_NAME  && var_to_partition (map, arg) != NO_PARTITION){
...
typedef struct _ForAllParmsData{
   MtcsOutofSsa *mtcsOutofSsa;
   void *arg;
}ForAllParmsData;

/* Create a default def for VAR.  */
static void create_default_def (tree var, void *userData ATTRIBUTE_UNUSED)
{
   ForAllParmsData *data=(ForAllParmsData *)userData;
   void *arg = data->arg;
   MtcsOutofSsa *self = data->mtcsOutofSsa;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
...
static void for_all_parms (MtcsOutofSsa *self,void (*callback)(tree var, void *arg), void *arg)
{
   ForAllParmsData userData={self,arg};
   for (tree var = DECL_ARGUMENTS (current_function_decl); var; var = DECL_CHAIN (var))
      callback (var, &userData/*!arg*/);
   if (!VOID_TYPE_P (TREE_TYPE (DECL_RESULT (current_function_decl)))){
      n_debug("mtcsoutofssa.c for_all_parms 00 -- arg:%p cfun->static_chain_decl:%p %p\n",
            arg,cfun->static_chain_decl,DECL_RESULT (current_function_decl));
      callback (DECL_RESULT (current_function_decl), &userData/*!arg*/);
   }
   n_debug("mtcsoutofssa.c for_all_parms 11 -- arg:%p cfun->static_chain_decl:%p \n",arg,cfun->static_chain_decl);

   if (cfun->static_chain_decl)
      callback (cfun->static_chain_decl, &userData/*!arg*/);
}
...
static void set_parm_default_def_partition (tree var, void *userData)
{
   ForAllParmsData *data=(ForAllParmsData *)userData;
   void *arg_ = data->arg;
   MtcsOutofSsa *self = data->mtcsOutofSsa;
...
static bitmap get_parm_default_def_partitions (MtcsOutofSsa *self,var_map map)
{
   ...
   for_all_parms(self,set_parm_default_def_partition, &arg);
...
static bitmap get_undefined_value_partitions (var_map map)
{
   ...
   for (unsigned int i = 1; i < num_ssa_names; i++){
      tree var = ssa_name (i);
      if (var  && !virtual_operand_p (var) && !has_zero_uses (var) && ssa_undefined_value_p (var)){
...
{
   basic_block bb;
   mtcs_elim_graph g (sa->map);

   FOR_BB_BETWEEN (bb, ENTRY_BLOCK_PTR_FOR_FN (cfun)->next_bb,EXIT_BLOCK_PTR_FOR_FN (cfun), next_bb)
      if (!gimple_seq_empty_p (phi_nodes (bb))){
         edge e;
         edge_iterator ei;
         FOR_EACH_EDGE (e, ei, bb->preds)
            eliminate_phi(self,e, &g);
         ...
         for (ei = ei_start (bb->preds); (e = ei_safe_edge (ei)); ){
            if (e->insns.r && (e->flags & EDGE_EH)  && !single_pred_p (e->dest)){
...
/* Remove the ssa-names in the current function and translate them into normal
   ...
   should also be used.  */
static void remove_ssa_form (MtcsOutofSsa *self,bool perform_ter, struct ssaexpand *sa)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsSsaCoalesce *mtcsSsaCoalesce = mtcs_target_get_ssa_coalesce(mtcsTarget);

   bitmap values = NULL;
   var_map map;

   for_all_parms(self,create_default_def, NULL);
   map = init_var_map (num_ssa_names);
   //fprintf(stderr,"mtvcsoutofssa.c remove_ssa_form---cfun:%p num_ssa_names:%d\n",cfun,num_ssa_names);
   mtcs_ssa_coalesce_coalesce_ssa_name/*!coalesce_ssa_name*/(mtcsSsaCoalesce,map);
   ...
   sa->map = map;
   sa->values = values;
   sa->partitions_for_parm_default_defs = get_parm_default_def_partitions(self,map);
   ...
   FOR_EACH_BB_FN (bb, cfun){
      ...
      for (gsi = gsi_start_phis (bb); !gsi_end_p (gsi); gsi_next (&gsi)){
         ...
         for (i = 0; i < gimple_phi_num_args (phi); i++){
            ...
            backedges.  */
            if (!(e->flags & EDGE_DFS_BACK) || !EDGE_CRITICAL_P (e))
               continue;
            ...
            if (TREE_CODE (arg) != SSA_NAME
            || (gimple_code (SSA_NAME_DEF_STMT (arg)) == GIMPLE_PHI  && trivially_conflicts_p (bb, result, arg))){
               tree name;
               gassign *stmt;
               gimple *last = NULL;
               ...
               if (last && stmt_ends_bb_p (last)){
                  ...
                  anything after it.  */
                  if (TREE_CODE (arg) == SSA_NAME && SSA_NAME_DEF_STMT (arg) == last)
                     continue;
               }
               ...
               stmt = gimple_build_assign (name, gimple_phi_arg_def (phi, i));

               /* copy location if present.  */
               if (gimple_phi_arg_has_location (phi, i))
                  gimple_set_location (stmt, gimple_phi_arg_location (phi, i));
            ...
            and adjust all conflicting uses.  */
               ...
               if (gimple_nop_p (def) || gimple_code (def) == GIMPLE_PHI)
                  continue;
               imm_use_iterator imm_iter;
               gimple *use_stmt;
               ...
               FOR_EACH_IMM_USE_STMT (use_stmt, imm_iter, result){
                  if (gimple_bb (use_stmt) != bb  || (gimple_code (use_stmt) != GIMPLE_PHI
                  && (maybe_renumber_stmts_bb (bb), true)  && gimple_uid (use_stmt) > gimple_uid (def))){
                     use_operand_p use;
               ...
               && TREE_CODE (gimple_assign_rhs2 (def)) == INTEGER_CST){
                  ...
                  tree newval = gimple_build (&gsi, true, GSI_SAME_STMT,UNKNOWN_LOCATION,gimple_assign_rhs_code (def),
                        TREE_TYPE (*adj), *adj, gimple_assign_rhs2 (def));
                  *adj = newval;
...
unsigned int mtcs_outof_ssa_rewrite_out_of_ssa (MtcsOutofSsa *self,struct ssaexpand *sa)
{
   ...
   remove_ssa_form(self,flag_tree_ter, sa);

   return 0;
}

static void mtcsOutofSsaInit(MtcsOutofSsa *self)
{
...
{
   MtcsOutofSsa *self = n_slice_alloc0 (sizeof(MtcsOutofSsa));
   mtcs_component_set_mode((MtcsComponent*)self,mtcsMode);
   mtcsOutofSsaInit(self);
   return self;
}
