...
static char *callFiles=NULL;        //调试语句，如n_debug调用所在的文件,即 __FILE__
static char *compileFiles=NULL;     //正在编译的文件，
static char *callFileFuncs=NULL;    //调试语句，如n_debug调用所在的函数,即 __FUNCTION__
static char *compileFileFuncs=NULL; //正在编译的文件的函数名。

#define STRATEGY_FRONT     1<<1  //前端
#define STRATEGY_END       1<<2  //后端
#define STRATEGY_MTCS      1<<3  //MTCS
static int debugStrategy = STRATEGY_FRONT|STRATEGY_END|STRATEGY_MTCS;//调试策略 默认全部
static int debug_level= 1 ;//默认
...
void n_log_structured (NLogLevelFlags  log_level,...)
{
 ...
 // NArray *array = NULL;
  char *array=(char *)n_malloc(sizeof(NLogField)*20);
  int arrayCount=0;
...
{
   //如果 log_level =N_LOG_LEVEL_DEBUG,但设置的AET_LOG_LEVELG不包含DEBUG,返回
  if(log_level==N_LOG_LEVEL_DEBUG && !(N_LOG_LEVEL_DEBUG & n_log_msg_prefix))
       return;
  NLogField fields[] =
    {
...
{
  struct timeval r;
#ifdef CONFIG_NLIB_HOST //不进入gcc环境
  time_t tx;
  time(&tx);
  r.tv_sec=tx;
#else
...
/**
 * 参数 fileStr 调用n_debug n_info...的文件,来自__FILE__
 * 或者是正在编译的文件，来自回调函数 NLogGetCompileFile
 */
static nboolean exitsFile(char *src,const char *fileStr)
{
	 if(!fileStr)
		return FALSE;
	 char fn[512];
	 ...
	 if(ptr==NULL)
		...
		sprintf(fn,"%s",ptr+1);
	 return strstr(src,fn);
}

static  nboolean filterFile(char *currentCallFile,char * currentCallFunc)
{
   if(callFiles==NULL &&  callFileFuncs==NULL && compileFiles==NULL && compileFileFuncs==NULL)
      return FALSE;
   nchar *currentCompileFile=NULL; //正在编译的文件
   nchar *currentCompileFunc=NULL;//正在编译的函数
   if(nLogGetCompileFile){
      currentCompileFile=nLogGetCompileFile(&currentCompileFunc);
   }
   nboolean callFileOk=callFiles==NULL?TRUE:exitsFile(callFiles,currentCallFile);
   nboolean callFuncOk=callFileFuncs==NULL?TRUE:exitsFile(callFileFuncs,currentCallFunc);
   nboolean compileFileOk=compileFiles==NULL?TRUE:exitsFile(compileFiles,currentCompileFile);
   nboolean compileFuncOk=(compileFileFuncs==NULL || currentCompileFunc==NULL)?TRUE:exitsFile(compileFileFuncs,currentCompileFunc);
   return !(callFileOk && callFuncOk && compileFileOk && compileFuncOk);
}

/**
 * domain包含有两类文件，一是debug调用所在文件，一是编译文件
 */
static  nboolean filterField(const NLogField *fields,nsize n_fields)
{
   if(callFiles==NULL &&  callFileFuncs==NULL && compileFiles==NULL && compileFileFuncs==NULL)
      return FALSE;
   const nchar *currentCallFile=NULL;    //调用n_debug所在的文件
   const nchar *currentCallFunc=NULL; //调用n_debug所在的函数
   int i;
   for (i = 0; i < n_fields; i++){
      if (n_strcmp0 (fields[i].key, "CODE_FILE") == 0){
         currentCallFile = fields[i].value;
      }
      if (n_strcmp0 (fields[i].key, "CODE_FUNC") == 0){
         currentCallFunc = fields[i].value;
      }
   }
   return filterFile(currentCallFile,currentCallFunc);
}
...
static nboolean shouldDropMessage (NLogLevelFlags log_level,const NLogField *fields,nsize n_fields)
{
   ...
   if(log_level==N_LOG_LEVEL_ERROR)
      return FALSE;
   return filterField(fields,n_fields);
}
...
handled:
   /* Abort if the message was fatal. */
   if ((log_level & N_LOG_FLAG_FATAL) || (log_level & N_LOG_LEVEL_ERROR)){
...
/**
 * 设置获取正在编译的文件的回调函数，由classparser.c调用。
 */
void   n_log_set_compile_file_callback(NLogGetCompileFile cb)
{
   nLogGetCompileFile=cb;
}

static void setValue(char *src,char **dest)
{
   if(*dest){
      n_free(*dest);
      *dest=NULL;
   }
   if(src && n_ascii_strncasecmp(src,"all",3))
      *dest=n_strdup(src);
}

/**
 * 匹配调用n_debug,n_info等的文件
 */
void  n_log_set_callfiles(char *files)
{
   setValue(files,&callFiles);
}

void  n_log_set_callfuncs(char *funcs)
{
   setValue(funcs,&callFileFuncs);
}

void  n_log_set_compilefiles(char *files)
{
   setValue(files,&compileFiles);
}

void  n_log_set_compilefuncs(char *funcs)
{
   setValue(funcs,&compileFileFuncs);
}
...
void  n_log_set_param_from_env()
{
	...
	if(level!=NULL){
	   debug_level=atoi(level);
		n_log_set_level(debug_level);
	}
	n_log_set_callfiles(getenv("AET_LOG_CALLFILE"));
   n_log_set_callfuncs(getenv("AET_LOG_CALLFUNC"));
   n_log_set_compilefiles(getenv("AET_LOG_FILE"));
   n_log_set_compilefuncs(getenv("AET_LOG_FUNC"));
	...
	char *strategy=getenv("AET_LOG_STRATEGY");
	if(strategy){
	   if(!strcmp(strategy,"ALL") || !strcmp(strategy,"all") || !strcmp(strategy,"All"))
         debugStrategy=STRATEGY_FRONT|STRATEGY_END|STRATEGY_MTCS;
      else{
         char **items=n_strsplit(strategy,"|",3);
         int len=n_strv_length(items);
         int i;
         debugStrategy=0;
         for(i=0;i<len;i++)
            debugStrategy|=atoi(items[i]);
         n_strfreev(items);
      }
	}
}
 ...
 * 进入前端
 */
void n_log_enter_front()
{
   if(debugStrategy&STRATEGY_FRONT){
      n_log_set_level(debug_level);
   }else{
      n_log_set_level(1);
   }
...
void n_log_enter_end()
{
   if(debugStrategy&STRATEGY_END){
      n_log_set_level(debug_level);
   }else{
      n_log_set_level(1);
   }
...
void n_log_enter_mtcs()
{
   if(debugStrategy&STRATEGY_MTCS){
      n_log_set_level(debug_level);
   }else{
      n_log_set_level(1);
   }
...
char  *n_log_get_info()
{
   sprintf(infos,"level:%d file_func_line:%d compilefiles:%s compilefuncs:%s callfiles:%s callfuncs:%s",
         n_log_msg_prefix,need_FILE_FUN_LINE,compileFiles,compileFileFuncs,callFiles,callFileFuncs);
   return (char *)infos;
}
...
nboolean n_log_is_debug_file(char *callFile,char *callFunc)
{
   if(!n_log_is_debug())
      return FALSE;
   return !filterFile(callFile,callFunc);
}
