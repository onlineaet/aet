...
impl$ ALoop{
    ...
    void run (){
      ...
      //printf("g_main_loop_run 00 thread:%p\n",selfThread);
      if(!mgr->acquire()){
          aboolean gotOwnership = FALSE;
          /* 另外的线程正在使用这个管理器 */
          mgr->lock();
          a_atomic_int_set (&isRunning, TRUE);
          //printf("g_main_loop_run 11\n");
      ...
      if (mgr->in_check_or_prepare){
          a_warning ("g_main_loop_run(): called recursively from within a source's "
             "check() or prepare() member, iteration not possible.");
          return;
      }
      //printf("g_main_loop_run 44\n");
