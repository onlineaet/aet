...
expand_delays (void)
{
  ...
  fprintf(stderr,"mtcsgenattrtab.c expand_delays num_delay_slots_str:%s\n",num_delay_slots_str);
...
write_length_unit_log (FILE *outf)
{
  ...
  fprintf (outf, "EXPORTED_CONST int %s_length_unit_log = %u;\n", platName,length_unit_log);
}
...
write_attr_get (FILE *outf, class attr_desc *attr)
{
  ...
  if (attr->name[0] == '*')
    fprintf (outf, "%s_%s (MtcsInsnAttr *self,rtx_insn *insn ATTRIBUTE_UNUSED)\n", platName,&attr->name[1]);
  else if (attr->is_const == 0)
    fprintf (outf, "%s_get_attr_%s (MtcsInsnAttr *self,rtx_insn *insn ATTRIBUTE_UNUSED)\n", platName,attr->name);
  else
    {
      fprintf (outf, "get_attr_%s (MtcsInsnAttr *self)\n", attr->name);
      fprintf (outf, "{\n");
      fprintf (outf, "   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);\n");
      fprintf (outf, "   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;\n");
      fprintf (outf, "   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);\n");
  ...
  fprintf (outf, "  switch (mtcs_recog_recog_memoized/*!recog_memoized*/(mtcsRecog,insn))\n");
  fprintf (outf, "    {\n");
...
{
   fprintf(stderr,"mtcsgenattrtab.c write_attr_set 00 prefix:%s suffix:%s \n",prefix,suffix);
  ...
  if (GET_CODE (value) == COND)
      ...
      if (! first_if)
   {
  ...
  else
    {
      ...
      fprintf(stderr,"mtcsgenattrtab.c write_attr_set 11 prefix:%s suffix:%s \n",prefix,suffix);
       ...
       int indent, rtx known_true)
{
  ...
  if (av->has_asm_insn)
    {
      ...
      fprintf(stderr,"mtcsgenattrtab.c write_attr_case 00 prefix:%s suffix:%s write_case_lines:%d\n",prefix,suffix,write_case_lines);
    }
  ...
  fprintf(stderr,"mtcsgenattrtab.c write_attr_case 11 prefix:%s suffix:%s av->num_insns:%d\n",prefix,suffix,av->num_insns);
  ...
  if (attr->is_numeric)
   ...
   fprintf (outf, " /* %#x */", num);
  ...
  else
    {
      ...
      fprintf (outf, "%s_",platUpperName);
      write_upcase (outf, attr->enum_name ? attr->enum_name : attr->name);
      //fprintf(stderr,"mtcsgenattrtab.c write_attr_valueq %s %s s:%s\n",attr->enum_name,attr->name,s);
      fprintf (outf, "_");
      write_upcase (outf, s);
    }
}
...
write_attr_value (FILE *outf, class attr_desc *attr, rtx value)
{
  ...
  switch (GET_CODE (value))
    {
    case CONST_STRING:
      //fprintf(stderr,"mtcsgenattrtab.c write_attr_value 00\n");
      write_attr_valueq (outf, attr, XSTR (value, 0));
      break;

    case CONST_INT:
       //fprintf(stderr,"mtcsgenattrtab.c write_attr_value 11\n");

      fprintf (outf, HOST_WIDE_INT_PRINT_DEC, INTVAL (value));
      break;

    case SYMBOL_REF:
       //fprintf(stderr,"mtcsgenattrtab.c write_attr_value 22\n");
...
write_dummy_eligible_delay (FILE *outf, const char *kind)
{
  ...
  fprintf (outf, "int\n");
  fprintf (outf, "%s_eligible_for_%s (rtx_insn *delay_insn ATTRIBUTE_UNUSED,\n"
...
write_eligible_delay (FILE *outf, const char *kind)
{
  ...
  fprintf (outf, "int\n");
  fprintf (outf, "%s_eligible_for_%s (MtcsInsnAttr *self,rtx_insn *delay_insn ATTRIBUTE_UNUSED, int slot, \n"
       "       rtx_insn *candidate_insn, int flags ATTRIBUTE_UNUSED)\n",
       platName,kind);
  fprintf (outf, "{\n");
  fprintf (outf, "  rtx_insn *insn ATTRIBUTE_UNUSED;\n");
  fprintf (outf, "\n");
  fprintf (outf, "  if (%s_num_delay_slots (self,delay_insn) == 0)\n",platName);
  fprintf (outf, "    return 0;");
  fprintf (outf, "\n");
...
write_const_num_delay_slots (FILE *outf)
{
  ...
  if (attr)
    {
      fprintf (outf, "int\n%s_const_num_delay_slots (MtcsInsnAttr *self,rtx_insn *insn)\n",platName);
      fprintf (outf, "{\n");
      fprintf (outf, "   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);\n");
      fprintf (outf, "   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;\n");
      fprintf (outf, "   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);\n");
...
write_header (FILE *outf)
{
  ...
  fprintf (outf, "/* Generated automatically by the program `mtcsgenattrtab'\n"
  ...
  fprintf (outf, "#include \"%s-insn-attr.h\"\n",platName);
  ...
  fprintf (outf, "\n");
  fprintf (outf, "#include \"../../mtcstarget.h\"\n");
  fprintf (outf, "\n");

  fprintf (outf, "#define operands mtcsRecog->recog_data.operand\n\n");
}
...
handle_arg (const char *arg)
{
  switch (arg[1])
    ...
    case 'L':
      ...
      return true;
    case 'W':
      return mtcs_gen_handle_arg(arg);
    default:
      return false;
    }
}
