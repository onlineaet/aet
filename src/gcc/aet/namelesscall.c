...
static tree getSelfParm(tree varDecl,location_t loc)
{
           ...
           if(class_info_is_interface(info)){
               ...
               if(isPointer){
                   ...
                   tree ref= build_component_ref (loc, datum,id, loc,UNKNOWN_LOCATION);
                   id=aet_utils_create_ident(IFACE_AT_CLASS);//
                   firstParms= build_component_ref (loc, ref,id, loc,UNKNOWN_LOCATION);
               }else{
                   tree id=aet_utils_create_ident(IFACE_COMMON_STRUCT_VAR_NAME);//
                   tree ref= build_component_ref (loc, varDecl,id, loc,UNKNOWN_LOCATION);
                   id=aet_utils_create_ident(IFACE_AT_CLASS);//
                   firstParms= build_component_ref (loc, ref,id, loc,UNKNOWN_LOCATION);
               }
           }else{
...
static tree createNewCall00(tree call,location_t loc,tree varDecl,tree fieldDecl)
{
        ...
        if(isPointer){
            tree datum=build_indirect_ref (loc,varDecl,RO_ARROW);
            fndecl= build_component_ref (loc, datum,DECL_NAME(fieldDecl), loc,UNKNOWN_LOCATION);
        }else{
            fndecl = build_component_ref (loc, varDecl,DECL_NAME(fieldDecl), loc,UNKNOWN_LOCATION);
        }
...
/**
 * 在新建的bind_expr中创建临时变量并赋值
 * getDefault()->add()
 * 相当于把 getDefault()-add()中的 getDefault()
 *拆分为 :
 *1.ASourceMgr *tempVarName_index;
 *2.tempVarName_index= getDefault();
 *返回的是 tempVarName_index= getDefault(); modify_expr 表达式
 */
...
static tree createNewRef(location_t loc,tree varDecl,tree fieldDecl)
{
        ...
        if(isPointer){
            tree datum=build_indirect_ref (loc,varDecl,RO_ARROW);
            ref= build_component_ref (loc, datum,DECL_NAME(fieldDecl), loc,UNKNOWN_LOCATION);
        }else{
            ref = build_component_ref (loc, varDecl,DECL_NAME(fieldDecl), loc,UNKNOWN_LOCATION);
        }
        return ref;
}
...
/**
 * 返回新建的监时变量
 * createModify_new创建 tempVarName_index= getDefault() 表达式。
 * 并加入语句列表中。
 */
static tree createVarDeclStmt(tree rvalue,NamelessBuffer *bufs,location_t loc,tree *stmtList)
{
    ...
    tree varDecl=  TREE_OPERAND (modifyVarDecl, 0);
    tree stmt0 = build_stmt (loc, DECL_EXPR, varDecl);
    ...
    //加入调用函数printgen来调试 pringgen实现在Aobject.h中
...
static tree process(tree call)
{
        ...
        if(result>0){
            ...
            if(TREE_CODE(item)==COMPONENT_REF){
                if(!aet_utils_valid_tree(prevVal)){
                 ...
                 int continueRef=0;
                 tree newCallOrNewRef=processComponentRef(call,prevVal,item,loc,&continueRef);
                 n_debug("引用 COMPONENT_REF 但返回值不是类 call_expr i:%d\n",i);
                 if(i!=0){
                     if(continueRef==1){
    ...
    //把临时变量加到bind_expr中的treeoperand[0]中
    for(i=bufs.count-1;i>=0;i--){
           tree modify=bufs.modifys[i];
           tree varDecl =TREE_OPERAND (modify, 0);
           tree vars = BIND_EXPR_VARS (bindExpr);
           DECL_CHAIN (varDecl) = vars;
           BIND_EXPR_VARS (bindExpr) = varDecl;
    }
    if(!hasRtn)
        return bindExpr;
...
tree nameless_call_parser(NamelessCall *self,tree call)
{
    ...
    */
   if(token1->type==CPP_DOT || token1->type==CPP_DEREF
         || (token1->type==CPP_CLOSE_PAREN && (token2->type==CPP_DOT || token2->type==CPP_DEREF)) ){
       return call;
   }
   int hasNamelessCall= class_util_has_nameless_call(call);
   if(hasNamelessCall==0)
       return call;
   n_debug("nameless_call_parser ------00 %p %s hasNamelessCall:%d\n",call,in_fnames[0],hasNamelessCall);
