...
inline insn_range_info move_later_than (insn_range_info range, insn_info *insn)
{
...
// than INSN.  (The movement range is the range of instructions after which
// an instruction can be placed.)
inline insn_range_info move_no_earlier_than (insn_range_info range, insn_info *insn)
{
...
// than INSN.  (The movement range is the range of instructions after which
// an instruction can be placed.)
inline insn_range_info move_no_later_than (insn_range_info range, insn_info *insn)
{
...
// than INSN.  (The movement range is the range of instructions after which
// an instruction can be placed.)
inline insn_range_info move_earlier_than (insn_range_info range, insn_info *insn)
{
...
bool can_insert_after (insn_info *insn);
/*
inline bool can_insert_after (insn_info *insn)
{
   return (insn->is_bb_head () || (insn->is_real () && !control_flow_insn_p (insn->rtl ())));
}
*/
...
inline bool canonicalize_move_range (insn_range_info &move_range, insn_info *insn)
{
...
bool restrict_movement_for_dead_range (insn_range_info &move_range,
              ...
              IgnorePredicates ignore)
{
   ...
   def_lookup dl =mtcsRtlData/*!crtl*/->ssa->find_def (resource, insn);
...
// the same extended basic block.
template<typename IgnorePredicates>
bool restrict_movement_for_defs (insn_range_info &move_range, def_array defs,
             IgnorePredicates ignore)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   ...
   for (def_info *def : defs){
      ...
      if (!mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,regno) || is_clobber)
         continue;
...
template<typename IgnorePredicates>
bool restrict_movement_for_uses (insn_range_info &move_range, use_array uses,
             IgnorePredicates ignore)
{
   MtcsTarget *mtcsTarget=mtcs_compile_get_current_target(mtcs_compile_get());
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   for (const use_info *use : uses){
      ...
      if (!ignore.should_ignore_def (set) && !ignore.should_ignore_insn (insn))
         move_range = move_later_than (move_range, insn);
      ...
      for (ebb_call_clobbers_info *call_group : ebb->call_clobbers ()){
         ...
         insn_info *insn = next_call_clobbers (*call_group, use->insn (),ignore);
         if (insn)
            move_range = move_earlier_than (move_range, insn);
      }
   }
