...
//      item->loc,item->order,input_location,item->line_base,pfile->buffer->line_base,pfile->buffer->cur);
   ...
   pfile->line_table->m_num_optimized_ranges=item->num_optimized_ranges;
   pfile->line_table->m_num_unoptimized_ranges=item->num_unoptimized_ranges;
...
/**
 * 为了恢复token在文件的位置。看restorelocation.c
 * directives.cc 使用"aet"作为判断标志。
 */
...
void *aet_utils_create_restore_location_data(cpp_reader *pfile,location_t loc)
{
   ...
   if(!pfile->buffer){
      ...
      if(errors>0){
         printf("aet_utils_create_restore_location_data --出错次数 %d\n",errors);
         return NULL;
      }
   }
   ...
   restore->num_optimized_ranges=pfile->line_table->m_num_optimized_ranges;
   restore->num_unoptimized_ranges=pfile->line_table->m_num_unoptimized_ranges;
