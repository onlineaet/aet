...
class mtcs_ssa_block_ranges
{
public:
  mtcs_ssa_block_ranges (tree t) : m_type (t) { }
...
void mtcs_ssa_block_ranges::dump (FILE *f)
{
   basic_block bb;
...
class mtcs_sbr_vector : public mtcs_ssa_block_ranges
{
public:
   mtcs_sbr_vector (tree t, vrange_allocator *allocator, bool zero_p = true);
...
mtcs_sbr_vector::mtcs_sbr_vector (tree t, vrange_allocator *allocator, bool zero_p)
  : mtcs_ssa_block_ranges (t)
{
   ...
   fprintf(stderr,"mtcsrangecache.c mtcs_sbr_vector 执行对象构造函数\n");

   m_tab = static_cast <vrange_storage **> (allocator->alloc (m_tab_size * sizeof (vrange_storage *)));
   if (zero_p)
...
void mtcs_sbr_vector::grow ()
{
   ...
   vrange_storage **t = static_cast <vrange_storage **>(m_range_allocator->alloc (new_size * sizeof (vrange_storage *)));
   memcpy (t, m_tab, m_tab_size * sizeof (vrange_storage *));
   if (m_zero_p)
      memset (t + m_tab_size, 0, (new_size - m_tab_size) * sizeof (vrange_storage *));
   fprintf(stderr,"mtcs_sbr_vector::grow () 分配 storage\n");
   m_tab = t;
   m_tab_size = new_size;
}

// Set the range for block BB to be R.
bool mtcs_sbr_vector::set_bb_range (const_basic_block bb, const vrange &r)
{
   vrange_storage *m;
...
// there is no range.
bool mtcs_sbr_vector::get_bb_range (vrange &r, const_basic_block bb)
{
   if (bb->index >= m_tab_size)
      return false;
...
bool mtcs_sbr_vector::bb_range_p (const_basic_block bb)
{
   if (bb->index < m_tab_size)
...
class mtcs_sbr_lazy_vector : public mtcs_sbr_vector
{
public:
   mtcs_sbr_lazy_vector (tree t, vrange_allocator *allocator, bitmap_obstack *bm);
...
mtcs_sbr_lazy_vector::mtcs_sbr_lazy_vector (tree t, vrange_allocator *allocator, bitmap_obstack *bm)
  : mtcs_sbr_vector (t, allocator, false)
{
   m_has_value = BITMAP_ALLOC (bm);
}

bool mtcs_sbr_lazy_vector::set_bb_range (const_basic_block bb, const vrange &r)
{
   mtcs_sbr_vector::set_bb_range (bb, r);
   bitmap_set_bit (m_has_value, bb->index);
   return true;
}

bool mtcs_sbr_lazy_vector::get_bb_range (vrange &r, const_basic_block bb)
{
   if (bitmap_bit_p (m_has_value, bb->index))
      return mtcs_sbr_vector::get_bb_range (r, bb);
   return false;
}

bool mtcs_sbr_lazy_vector::bb_range_p (const_basic_block bb)
{
...
class mtcs_sbr_sparse_bitmap : public mtcs_ssa_block_ranges
{
public:
   mtcs_sbr_sparse_bitmap (tree t, vrange_allocator *allocator, bitmap_obstack *bm);
   virtual bool set_bb_range (const_basic_block bb, const vrange &r) override;
   virtual bool get_bb_range (vrange &r, const_basic_block bb) override;
   virtual bool bb_range_p (const_basic_block bb) override;
private:
...
// Initialize a block cache for an ssa_name of type T.
mtcs_sbr_sparse_bitmap::mtcs_sbr_sparse_bitmap (tree t, vrange_allocator *allocator,bitmap_obstack *bm)
  : mtcs_ssa_block_ranges (t)
{
   fprintf(stderr,"mtcsrangecache.c mtcs_sbr_sparse_bitmap 执行对象构造函数\n");
   gcc_checking_assert (TYPE_P (t));
   m_type = t;
...
inline void mtcs_sbr_sparse_bitmap::bitmap_set_quad (bitmap head, int quad, int quad_value)
{
...
// function as a sparse array of 4 bit values.
// QUAD is the index.
inline int mtcs_sbr_sparse_bitmap::bitmap_get_quad (const_bitmap head, int quad)
{
   return (int) bitmap_get_aligned_chunk (head, quad, 4);
}

// Set the range on entry to basic block BB to R.
bool mtcs_sbr_sparse_bitmap::set_bb_range (const_basic_block bb, const vrange &r)
{
   if (r.undefined_p ()){
...
bool mtcs_sbr_sparse_bitmap::get_bb_range (vrange &r, const_basic_block bb)
{
...
bool mtcs_sbr_sparse_bitmap::bb_range_p (const_basic_block bb)
{
   return (bitmap_get_quad (&bitvec, bb->index) != 0);
}

// -------------------------------------------------------------------------
//实现类 mtcs_block_range_cache 原型是 block_range_cache gimple-range-cache.h
// Initialize the block cache.

mtcs_block_range_cache::mtcs_block_range_cache ()
{
   bitmap_obstack_initialize (&m_bitmaps);
   m_ssa_ranges.create (0);
   m_ssa_ranges.safe_grow_cleared (num_ssa_names);
   m_range_allocator = new mtcs_vrange_allocator/*!vrange_allocator*/;
}
...
void mtcs_block_range_cache::setMtcsMode (MtcsMode *mtcsMode)
{
   this->mtcsMode = mtcsMode;
}

MtcsMode *mtcs_block_range_cache::getMtcsMode()
{
   return mtcsMode;
}
...
bool mtcs_block_range_cache::set_bb_range (tree name, const_basic_block bb,const vrange &r)
{
   ...
   if (!m_ssa_ranges[v]){
      ...
      if (last_basic_block_for_fn (cfun) > param_vrp_sparse_threshold){
         void *r = m_range_allocator->alloc (sizeof (mtcs_sbr_sparse_bitmap));
         m_ssa_ranges[v] = new (r) mtcs_sbr_sparse_bitmap (TREE_TYPE (name),m_range_allocator,&m_bitmaps);
      }else if (last_basic_block_for_fn (cfun) < param_vrp_vector_threshold){
         // For small CFGs use the basic vector implemntation.
         void *r = m_range_allocator->alloc (sizeof (mtcs_sbr_vector));
         m_ssa_ranges[v] = new (r) mtcs_sbr_vector (TREE_TYPE (name),m_range_allocator);
      }else{
         // Otherwise use the sparse vector implementation.
         void *r = m_range_allocator->alloc (sizeof (mtcs_sbr_lazy_vector));
         m_ssa_ranges[v] = new (r) mtcs_sbr_lazy_vector (TREE_TYPE (name),m_range_allocator, &m_bitmaps);
      }
   }
...
inline mtcs_ssa_block_ranges *mtcs_block_range_cache::query_block_ranges (tree name)
{
   unsigned v = SSA_NAME_VERSION (name);
...
bool mtcs_block_range_cache::get_bb_range (vrange &r, tree name, const_basic_block bb)
{
   mtcs_ssa_block_ranges *ptr = query_block_ranges (name);
   if (ptr)
...
bool mtcs_block_range_cache::bb_range_p (tree name, const_basic_block bb)
{
   mtcs_ssa_block_ranges *ptr = query_block_ranges (name);
   if (ptr)
...
void mtcs_block_range_cache::dump (FILE *f)
{
   unsigned x;
...
void mtcs_block_range_cache::dump (FILE *f, basic_block bb, bool print_varying)
{
   unsigned x;
...
mtcs_ssa_cache::mtcs_ssa_cache (MtcsMode *mtcsMode)
{
   fprintf(stderr,"mtcs_ssa_cache::mtcs_ssa_cache m_tab.create (0); mtcsMode:%p\n",mtcsMode);
   m_tab.create (0);
   m_range_allocator = new mtcs_vrange_allocator/*!vrange_allocator*/;
   ((mtcs_vrange_allocator *) m_range_allocator)->mtcsMode = mtcsMode;
}

// Deconstruct an ssa cache.

mtcs_ssa_cache::~mtcs_ssa_cache ()
{
   m_tab.release ();
   delete m_range_allocator;
}
...
bool mtcs_ssa_cache::range_of_expr (vrange &r, tree expr, gimple *stmt)
{
...
bool mtcs_ssa_cache::has_range (tree name) const
{
   unsigned v = SSA_NAME_VERSION (name);
   if (v >= m_tab.length ())
      return false;
...
//stow进入vrange_storage的get_vrange方法
//static void vrange_storage::get_vrange (vrange_storage *stow,vrange &r, tree type) const
//{
//  if (is_a <irange> (r))
//    {
...
bool mtcs_ssa_cache::get_range (vrange &r, tree name) const
{
   unsigned v = SSA_NAME_VERSION (name);
   fprintf(stderr,"mtcs_ssa_cache::get_range 00---r:%p v:%d name:%p\n",&r,v,name);
   if (v >= m_tab.length ())
      return false;
   ...
   if (!stow)
      return false;
   fprintf(stderr,"mtcs_ssa_cache::get_range 11 ---stow:%p r:%p v:%d name:%p type:%p tree_code:%s\n",
         stow,&r,v,name,TREE_TYPE (name),get_tree_code_name(TREE_CODE(TREE_TYPE (name))));
   if (is_a <MtcsFrange> (r)){
      const MtcsFrangeStorage *s = static_cast <const MtcsFrangeStorage *> (stow);
       s->get_frange (as_a <MtcsFrange> (r), TREE_TYPE (name));
   }else{
...
bool mtcs_ssa_cache::set_range (tree name, const vrange &r)
{
   unsigned v = SSA_NAME_VERSION (name);
   if (v >= m_tab.length ())
   ...
   fprintf(stderr,"mtcs_ssa_cache::set_range 分配 vrange_storage :%p name:%p v:%d %s\n",m,name,v,get_tree_code_name(TREE_CODE(TREE_TYPE(name))));
   if (m && m->fits_p (r))
      m->set_vrange (r);
   else{
      m_tab[v] = ((mtcs_vrange_allocator *)m_range_allocator)->clone (r);
      fprintf(stderr,"mtcs_ssa_cache::set_range 调用 m_range_allocator)->clone 分配 vrange_storage :%p name:%p v:%d %s\n",
            m_tab[v],name,v,get_tree_code_name(TREE_CODE(TREE_TYPE(name))));
   }
...
bool mtcs_ssa_cache::merge_range (tree name, const vrange &r)
{
   unsigned v = SSA_NAME_VERSION (name);
   if (v >= m_tab.length ())
      m_tab.safe_grow_cleared (num_ssa_names + 1);
...
// Set the range for NAME to R in the ssa cache.
void mtcs_ssa_cache::clear_range (tree name)
{
   unsigned v = SSA_NAME_VERSION (name);
   if (v >= m_tab.length ())
      return;
...
void mtcs_ssa_cache::dump (FILE *f)
{
...
mtcs_ssa_lazy_cache::mtcs_ssa_lazy_cache (MtcsMode *mtcsMode,bitmap_obstack *ob):mtcs_ssa_cache(mtcsMode)
{
   if (!ob){
      bitmap_obstack_initialize (&m_bitmaps);
      m_ob = &m_bitmaps;
...
mtcs_ssa_lazy_cache::~mtcs_ssa_lazy_cache ()
{
   if (m_ob == &m_bitmaps)
      bitmap_obstack_release (&m_bitmaps);
   else
...
bool mtcs_ssa_lazy_cache::has_range (tree name) const
{
...
// If NAME has a range, intersect it with R, otherwise set it to R.
// Return TRUE if the range is new or changes.
bool mtcs_ssa_lazy_cache::merge_range (tree name, const vrange &r)
{
   unsigned v = SSA_NAME_VERSION (name);
   if (!bitmap_set_bit (active_p, v)){
      // There is already an entry, simply merge it.
      gcc_checking_assert (v < m_tab.length ());
      return mtcs_ssa_cache::merge_range (name, r);
   }
   if (v >= m_tab.length ())
      m_tab.safe_grow (num_ssa_names + 1);
   m_tab[v] = m_range_allocator->clone (r);
...
void mtcs_ssa_lazy_cache::merge (const mtcs_ssa_lazy_cache &cache)
{
   unsigned x;
   bitmap_iterator bi;
...
inline mtcs_temporal_cache::mtcs_temporal_cache ()
{
   m_current_time = 1;
...
inline int mtcs_temporal_cache::temporal_value (unsigned ssa) const
{
...
bool mtcs_temporal_cache::current_p (tree name, tree dep1, tree dep2) const
{
...
inline void mtcs_temporal_cache::set_timestamp (tree name)
{
   unsigned v = SSA_NAME_VERSION (name);
...
inline void mtcs_temporal_cache::set_always_current (tree name, bool value)
{
   unsigned v = SSA_NAME_VERSION (name);
   if (v >= m_timestamp.length ())
      m_timestamp.safe_grow_cleared (num_ssa_names + 20);
...
inline bool mtcs_temporal_cache::always_current_p (tree name) const
{
   unsigned v = SSA_NAME_VERSION (name);
   if (v >= m_timestamp.length ())
      return false;
...
mtcs_update_list::mtcs_update_list ()
{
...
void mtcs_update_list::add (basic_block bb)
{
   int i = bb->index;
...
basic_block mtcs_update_list::pop ()
{
...
// --------------------------------------------------------------------------
//类 mtcs_ranger_cache 实现 原型 ranger_cache
mtcs_ranger_cache::mtcs_ranger_cache (MtcsMode *mtcsMode,int not_executable_flag, bool use_imm_uses):
      m_globals(mtcsMode)
{
   this->mtcsMode = mtcsMode;
   m_workback = vNULL;
   m_temporal = new mtcs_temporal_cache;
   ...
   m_update = new mtcs_update_list ();
}

mtcs_ranger_cache::~mtcs_ranger_cache ()
{
   delete m_update;
...
// gori map as well.
void mtcs_ranger_cache::dump (FILE *f)
{
...
void mtcs_ranger_cache::dump_bb (FILE *f, basic_block bb)
{
...
bool mtcs_ranger_cache::get_global_range (vrange &r, tree name) const
{
   fprintf(stderr,"gimple-range-cache.cc get_global_range 00 r:%p,name:%p %s\n",&r,name,get_tree_code_name(TREE_CODE(name)));

   if (m_globals.get_range (r, name))
      return true;
   fprintf(stderr,"gimple-range-cache.cc get_global_range 11 r:%p name:%p %s\n",&r,name,get_tree_code_name(TREE_CODE(name)));
...
bool mtcs_ranger_cache::get_global_range (vrange &r, tree name, bool &current_p)
{
   ...
   current_p = false;
   if (had_global)
      current_p = r.singleton_p ()  || m_temporal->current_p (name, gori_ssa ()->depend1 (name), gori_ssa ()->depend2 (name));
   else{
...
void mtcs_ranger_cache::set_global_range (tree name, const vrange &r, bool changed)
{
   ...
   if (!changed){
      ...
      if (!m_temporal->current_p (name, gori_ssa ()->depend1 (name),gori_ssa ()->depend2 (name)))
   ...
   if (r.singleton_p () || (POINTER_TYPE_P (TREE_TYPE (name)) && r.nonzero_p ()))
...
void mtcs_ranger_cache::range_of_def (vrange &r, tree name, basic_block bb)
{
...
// lookups.
void mtcs_ranger_cache::entry_range (vrange &r, tree name, basic_block bb, enum rfd_mode mode)
{
...
void mtcs_ranger_cache::exit_range (vrange &r, tree name, basic_block bb, enum rfd_mode mode)
{
   if (bb == ENTRY_BLOCK_PTR_FOR_FN (cfun)){
      gimple_range_global (r, name);
      return;
...
bool mtcs_ranger_cache::edge_range (vrange &r, edge e, tree name, enum rfd_mode mode)
{
...
bool mtcs_ranger_cache::range_of_expr (vrange &r, tree name, gimple *stmt)
{
...
bool mtcs_ranger_cache::range_on_edge (vrange &r, edge e, tree expr)
{
   if (gimple_range_ssa_p (expr))
...
bool mtcs_ranger_cache::block_range (vrange &r, basic_block bb, tree name, bool calc)
{
   gcc_checking_assert (gimple_range_ssa_p (name));
...
void mtcs_ranger_cache::propagate_cache (tree name)
{
   basic_block bb;
   edge_iterator ei;
   edge e;
...
// entry.
void mtcs_ranger_cache::propagate_updated_value (tree name, basic_block bb)
{
   edge e;
   edge_iterator ei;
...
void mtcs_ranger_cache::fill_block_cache (tree name, basic_block bb, basic_block def_bb)
{
   edge_iterator ei;
   edge e;
   tree type = TREE_TYPE (name);
   ...
   if (range_from_dom (block_result, name, bb, RFD_FILL)){
      ...
      FOR_EACH_PARTIAL_AND_FULL_EQUIV (m_relation, bb, name, equiv_name, rel){
         ...
         if (equiv_bb == bb || (equiv_bb && !dominated_by_p (CDI_DOMINATORS, bb, equiv_bb)))
            continue;
...
void mtcs_ranger_cache::resolve_dom (vrange &r, tree name, basic_block bb)
{
...
bool mtcs_ranger_cache::range_from_dom (vrange &r, tree name, basic_block start_bb,
			      enum rfd_mode mode)
{
   ...
   for ( ; bb; prev_bb = bb, bb = get_immediate_dominator (CDI_DOMINATORS, bb)){
      ...
      if (gori ().has_edge_range_p (name, bb))
         ...
         if (EDGE_COUNT (prev_bb->preds) > 1 && gori ().has_edge_range_p (name, EDGE_PRED (prev_bb, 0)->src)){
            edge e;
            edge_iterator ei;
            bool all_dom = true;
            FOR_EACH_EDGE (e, ei, prev_bb->preds)
            if (e->src != bb   && !dominated_by_p (CDI_DOMINATORS, e->src, bb)){
               all_dom = false;
               break;
            }
            if (all_dom)
...
void mtcs_ranger_cache::register_inferred_value (const vrange &ir, tree name,basic_block bb)
{
   value_range r (TREE_TYPE (name));
   if (!m_on_entry.get_bb_range (r, name, bb))
...
void mtcs_ranger_cache::apply_inferred_ranges (gimple *s)
{
   bool update = true;
