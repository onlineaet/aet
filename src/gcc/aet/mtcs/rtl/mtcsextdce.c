...
static int group_limit (MtcsExtDce *self,const_rtx reg)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);

   machine_mode mode = GET_MODE (reg);

   if (!mtcs_mode_get_bitsize_poly/*!GET_MODE_BITSIZE*/(mtcsMode,mode).is_constant ())
      return 4;

   int size = mtcs_mode_get_size_poly/*!GET_MODE_SIZE*/(mtcsMode,mode).to_constant ();
...
static void make_reg_live (MtcsExtDce *self,bitmap bmap, int regno)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
...
static bool ext_dce_process_sets (MtcsExtDce *self,rtx_insn *insn, rtx obj, bitmap live_tmp)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   FOR_EACH_SUBRTX (iter, array, obj, NONCONST){
      ...
      if (GET_CODE (x) == SET || GET_CODE (x) == CLOBBER){
         ...
         if (!mtcs_mode_is_a <scalar_mode>(mtcsMode,GET_MODE (x), &outer_mode)
         || mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,outer_mode) > HOST_BITS_PER_WIDE_INT){
         ...
         if (GET_CODE (x) == STRICT_LOW_PART){
            ...
            paradoxical SUBREG.  */
            gcc_assert (SUBREG_P (x) && !mtcs_rtl_paradoxical_subreg_p/*!paradoxical_subreg_p*/(mtcsRTL,x) && SUBREG_BYTE (x).is_constant ());
            ...
            if (!mtcs_mode_is_a <scalar_mode>(mtcsMode,GET_MODE (SUBREG_REG (x)), &outer_mode)
            || mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,outer_mode) > HOST_BITS_PER_WIDE_INT){
            ...
            int limit = group_limit(self,SUBREG_REG (x));
            for (HOST_WIDE_INT i = 4 * rn; i < 4 * rn + limit; i++)
               if (bitmap_bit_p (self->livenow, i))
                  bitmap_set_bit (live_tmp, i);

            if (bitmap_empty_p (live_tmp))
               make_reg_live(self,live_tmp, rn);

            /* The mode of the SUBREG tells us how many bits we can
            clear.  */
            machine_mode mode = GET_MODE (x);
            HOST_WIDE_INT size = exact_log2 (mtcs_mode_get_size_poly/*!GET_MODE_SIZE*/(mtcsMode,mode).to_constant ()) + 1;
            bitmap_clear_range (self->livenow, 4 * rn, size);
         ...
         unsigned HOST_WIDE_INT mask  = mtcs_mode_get_mask/*!GET_MODE_MASK*/(mtcsMode,
               mtcs_mode_get_inner/*!GET_MODE_INNER*/(mtcsMode,GET_MODE (x)));
         if (SUBREG_P (x)){
            ...
            if (mtcs_rtl_paradoxical_subreg_p/*!paradoxical_subreg_p*/(mtcsRTL,x))
               ...
               bit = mtcs_rtlanal_subreg_lsb/*!subreg_lsb*/(mtcsRtlanal,x).to_constant ();
               mask = mtcs_mode_get_mask/*!GET_MODE_MASK*/(mtcsMode,GET_MODE (SUBREG_REG (x))) << bit;
               gcc_assert (mask);
               x = SUBREG_REG (x);
            }else
               gcc_unreachable ();
         ...
         if (REG_P (x)){
            ...
            : (mask & 0xff00) ? 2 : 1);
            bitmap_clear_range (self->livenow, 4 * rn + start, end - start);
         }
...
static void ext_dce_try_optimize_insn (MtcsExtDce *self,rtx_insn *insn, rtx set)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   new_pattern = mtcs_simplify_rtx_gen_subreg/*!simplify_gen_subreg*/(mtcsSimplifyRtx,GET_MODE (src), inner, GET_MODE (inner), 0);
   ...
   if (new_pattern){
      int ok = mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &SET_SRC (set), new_pattern, false);
      ...
      gcc_assert (REG_P (x));
      if (ok)
         bitmap_set_bit (self->changed_pseudos, REGNO (x));
...
unsigned HOST_WIDE_INT carry_backpropagate (MtcsExtDce *self,unsigned HOST_WIDE_INT mask, enum rtx_code code, rtx x)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);
   ...
   enum machine_mode mode = mtcs_mode_get_inner/*!GET_MODE_INNER*/(mtcsMode,GET_MODE (x));
   unsigned HOST_WIDE_INT mmask = mtcs_mode_get_mask/*!GET_MODE_MASK*/(mtcsMode,mode);
   ...
   if (!mtcs_mode_is_a <scalar_int_mode>(mtcsMode,mode, &smode)
   || mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,smode) > HOST_BITS_PER_WIDE_INT)
      return mmask;
   ...
   switch (code){
      ...
      case ASHIFT:
         ...
         && UINTVAL (XEXP (x, 1)) < mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,smode))
      ...
      case ASHIFTRT:
         ...
         && UINTVAL (XEXP (x, 1)) < mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,smode)){
            ...
            > mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,smode))
               sign = HOST_WIDE_INT_1U << (mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,smode) - 1);
      ...
      case UMUL_HIGHPART:
         ...
         if (CONST_INT_P (XEXP (x, 1))){
            if (pow2p_hwi (INTVAL (XEXP (x, 1))))
               return mmask & (mask << (mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,smode) -
                     exact_log2 (INTVAL (XEXP (x, 1)))));

            int bits = (HOST_BITS_PER_WIDE_INT + mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,smode) -
                  clz_hwi (mask) - ctz_hwi (INTVAL (XEXP (x, 1))));
            if (bits < mtcs_mode_get_bitsize/*!GET_MODE_BITSIZE*/(mtcsMode,smode))
               return (HOST_WIDE_INT_1U << bits) - 1;
         }
         return mmask;

      case SIGN_EXTEND:
         if (!mtcs_mode_get_bitsize_poly/*!GET_MODE_BITSIZE*/(mtcsMode,GET_MODE (x)).is_constant ()
         || !mtcs_mode_get_bitsize_poly/*!GET_MODE_BITSIZE*/(mtcsMode,GET_MODE (XEXP (x, 0))).is_constant ())
            return -1;

         /* We want the mode of the inner object.  We need to ensure its
         sign bit is on in MASK.  */
         mode = mtcs_mode_get_inner/*!GET_MODE_INNER*/(mtcsMode,GET_MODE (XEXP (x, 0)));
         if (mask & ~mtcs_mode_get_mask/*!GET_MODE_MASK*/(mtcsMode,mode))
            mask |= HOST_WIDE_INT_1U << (mtcs_mode_get_bitsize_poly/*!GET_MODE_BITSIZE*/(mtcsMode,mode).to_constant () - 1);

         /* Recurse into the operand.  */
         return carry_backpropagate(self,mask, GET_CODE (XEXP (x, 0)), XEXP (x, 0));
...
static void ext_dce_process_uses (MtcsExtDce *self,rtx_insn *insn, rtx obj,
   ...
   FOR_EACH_SUBRTX_VAR (iter, array_var, obj, NONCONST){
      ...
      if (xcode == SET){
         ...
         if (SUBREG_P (dst) && SUBREG_BYTE (dst).is_constant ()){
            bit = mtcs_rtlanal_subreg_lsb/*!subreg_lsb*/(mtcsRtlanal,dst).to_constant ();
         ...
         if (REG_P (dst) && safe_for_live_propagation (code)){
            ...
            unsigned HOST_WIDE_INT mask_array[] = { 0xff, 0xff00, HOST_WIDE_INT_UC (0xffff0000),-HOST_WIDE_INT_UC (0x100000000) };
            for (int i = 0; i < 4; i++)
            ...
            dst_mask = carry_backpropagate(self,dst_mask, code, src);
            ...
            if (code == SIGN_EXTEND || code == ZERO_EXTEND){
               rtx inner = XEXP (src, 0);
               unsigned HOST_WIDE_INT src_mask =mtcs_mode_get_mask/*!GET_MODE_MASK*/(mtcsMode,
                     mtcs_mode_get_inner/*!GET_MODE_INNER*/(mtcsMode,GET_MODE (inner)));

               /* DST_MASK could be zero if we had something in the SET
               that we couldn't handle.  */
               if (self->modify && !skipped_dest && (dst_mask & ~src_mask) == 0)
                  ext_dce_try_optimize_insn(self,insn, x);
            ...
            for (;;){
               ...
               if (mtcs_rtl_paradoxical_subreg_p/*!paradoxical_subreg_p*/(mtcsRTL,y))
                  y = XEXP (y, 0);
                  ...
                  bit = mtcs_rtlanal_subreg_lsb/*!subreg_lsb*/(mtcsRtlanal,y).to_constant ();
               ...
               if (REG_P (y)){
                  ...
                  if (!safe_for_live_propagation (code))
                     tmp_mask =mtcs_mode_get_mask/*!GET_MODE_MASK*/(mtcsMode,mtcs_mode_get_inner/*!GET_MODE_INNER*/(mtcsMode,GET_MODE (y)));

                  if (tmp_mask & 0xff)
                     bitmap_set_bit (self->livenow, rn);
                  if (tmp_mask & 0xff00)
                     bitmap_set_bit (self->livenow, rn + 1);
                  if (tmp_mask & HOST_WIDE_INT_UC (0xffff0000))
                     bitmap_set_bit (self->livenow, rn + 2);
                  if (tmp_mask & -HOST_WIDE_INT_UC (0x100000000))
                     bitmap_set_bit (self->livenow, rn + 3);
            ...
            if (REG_P (y) || CONSTANT_P (y))
               iter.skip_subrtxes ();
         }//end  if (REG_P (dst) && safe_for_live_propagation (code)){
      } //end    if (xcode == SET){
      ...
      && mtcs_mode_get_bitsize_poly/*!GET_MODE_BITSIZE*/(mtcsMode,GET_MODE (x)).is_constant ()
      && mtcs_mode_get_bitsize_poly/*!GET_MODE_BITSIZE*/(mtcsMode,GET_MODE (x)).to_constant () <= 32)
      {
         HOST_WIDE_INT size = mtcs_mode_get_bitsize_poly/*!GET_MODE_BITSIZE*/(mtcsMode,GET_MODE (x)).to_constant ();
         ...
         if (SUBREG_PROMOTED_VAR_P (x))
            size = mtcs_mode_get_bitsize_poly/*!GET_MODE_BITSIZE*/(mtcsMode,GET_MODE (SUBREG_REG (x))).to_constant ();
      ...
      just assume all the chunks are live.  */
         ...
         bitmap_set_range (self->livenow, REGNO (x) * 4, group_limit(self,x));
   }
}
...
static void ext_dce_process_bb (MtcsExtDce *self,basic_block bb)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   FOR_BB_INSNS_REVERSE (bb, insn){
      ...
      bool skipped_dest = ext_dce_process_sets(self,insn, PATTERN (insn), live_tmp);

      /* CALL_INSNs need processing their fusage data.  */
      if (CALL_P (insn))
         skipped_dest |= ext_dce_process_sets(self,insn, CALL_INSN_FUNCTION_USAGE (insn), live_tmp);

      /* And now uses, optimizing away SIGN/ZERO extensions as we go.  */
      ext_dce_process_uses(self,insn, PATTERN (insn), live_tmp, skipped_dest);

      /* A nonlocal goto implicitly uses the frame pointer.  */
      if (JUMP_P (insn) && find_reg_note (insn, REG_NON_LOCAL_GOTO, NULL_RTX)){
         bitmap_set_range (self->livenow, mtcs_reg_get_frame_pointer_regnum/*!FRAME_POINTER_REGNUM*/(mtcsReg) * 4, 4);
         if (!mtcs_reg_hard_frame_pointer_is_frame_pointer/*!HARD_FRAME_POINTER_IS_FRAME_POINTER*/(mtcsReg))
            bitmap_set_range (self->livenow, mtcs_reg_get_hard_frame_pointer_regnum/*!HARD_FRAME_POINTER_REGNUM*/(mtcsReg) * 4, 4);
      }

      /* And process fusage data for the use as well.  */
      if (CALL_P (insn)){
         if (!FAKE_CALL_P (insn))
            bitmap_set_range (self->livenow, mtcs_reg_get_stack_pointer_regnum/*!STACK_POINTER_REGNUM*/(mtcsReg) * 4, 4);

         /* If this is not a call to a const fucntion, then assume it
         can read any global register.  */
         if (!RTL_CONST_CALL_P (insn))
            for (unsigned i = 0; i < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); i++)
               if (mtcsReg->global_regs/*!global_regs*/[i])
                  bitmap_set_range (self->livenow, i * 4, 4);

         ext_dce_process_uses(self,insn, CALL_INSN_FUNCTION_USAGE (insn), live_tmp, false);
      }
...
static void maybe_clear_subreg_promoted_p (MtcsExtDce *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsFunc *mtcsFunc=mtcs_target_get_func(mtcsTarget);
...
static void reset_subreg_promoted_p (MtcsExtDce *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsFunc *mtcsFunc=mtcs_target_get_func(mtcsTarget);
   ...
   for (rtx_insn *insn = mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData); insn; insn = NEXT_INSN (insn)){
      ...
      FOR_EACH_SUBRTX_VAR (iter, array, pat, NONCONST){
         ...
         if (bitmap_bit_p (self->changed_pseudos, REGNO (x)))
...
static void ext_dce_init (MtcsExtDce *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsDfscan *mtcsDfscan=mtcs_target_get_dfscan(mtcsTarget);

   self->livein.create (last_basic_block_for_fn (cfun));
   self->livein.quick_grow_cleared (last_basic_block_for_fn (cfun));
   for (int i = 0; i < last_basic_block_for_fn (cfun); i++)
      bitmap_initialize (&self->livein[i], &bitmap_default_obstack);

   auto_bitmap refs (&bitmap_default_obstack);
   mtcs_dfscan_df_get_exit_block_use_set/*!df_get_exit_block_use_set*/(mtcsDfscan,refs);

   unsigned i;
   bitmap_iterator bi;
   EXECUTE_IF_SET_IN_BITMAP (refs, 0, i, bi)
      make_reg_live (self,&self->livein[EXIT_BLOCK], i);

   self->livenow = BITMAP_ALLOC (NULL);
   self->all_blocks = BITMAP_ALLOC (NULL);
   self->changed_pseudos = BITMAP_ALLOC (NULL);

   for (int i = 0; i < last_basic_block_for_fn (cfun); i++)
      if (i != ENTRY_BLOCK && i != EXIT_BLOCK)
         bitmap_set_bit (self->all_blocks, i);
...
static void ext_dce_finish (MtcsExtDce *self)
{
   for (unsigned i = 0; i < self->livein.length (); i++)
      bitmap_clear (&self->livein[i]);
   self->livein.release ();

   BITMAP_FREE (self->livenow);
   BITMAP_FREE (self->changed_pseudos);
...
static bool ext_dce_rd_transfer_n (int bb_index)
{
   ...
   MtcsExtDce *self = (MtcsExtDce *) mtcs_pass_mgr_get_pass(mtcsPassMgr,RTL_PASS,"ext_dce");
   gcc_assert(self!=NULL);
   ...
   bitmap_clear (self->livenow);
   edge_iterator ei;
   edge e;

   FOR_EACH_EDGE (e, ei, bb->succs)
      bitmap_ior_into (self->livenow, &self->livein[e->dest->index]);

   ext_dce_process_bb(self,bb);
   /* We only allow widening the set of objects live at the start
   of a block.  Otherwise we run the risk of not converging.  */
   return bitmap_ior_into (&self->livein[bb_index], self->livenow);
}
...
static void ext_dce_execute (MtcsExtDce *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   /* Limit the amount of memory we use for self->livein, with 4 bits per
   reg per basic-block including overhead that maps to one byte
   per reg per basic-block.  */
   uint64_t memory_request = (uint64_t)n_basic_blocks_for_fn (cfun) * mtcs_func_max_reg_num/*!max_reg_num*/(mtcsFunc);
   if (memory_request / 1024 > (uint64_t)mtcsOptionsItem->x_param_max_gcse_memory){
      warning (OPT_Wdisabled_optimization,
         "ext-dce disabled: %d basic blocks and %d registers; "
         "increase %<--param max-gcse-memory%> above %wu",
         n_basic_blocks_for_fn (cfun), mtcs_func_max_reg_num/*!max_reg_num*/(mtcsFunc),
         memory_request / 1024);
      return;
   }
   ...
   maybe_clear_subreg_promoted_p(self);
   mtcs_dfcore_df_analyze/*!df_analyze*/(mtcsDfcore);
   ext_dce_init(self);

   do{
      mtcs_dfcore_df_simple_dataflow/*!df_simple_dataflow*/(mtcsDfcore,DF_BACKWARD, NULL, NULL,
            ext_dce_rd_confluence_n, ext_dce_rd_transfer_n, self->all_blocks,
   ...
   reset_subreg_promoted_p(self);

   ext_dce_finish(self);
}

//原型 NEXT_PASS (pass_ext_dce, 1); RTL_PASS ext-dce.cc ext_dce n 有条件执行 (flag_ext_dce && optimize > 0);ext_dce_execute
static nboolean ext_dce_gate_cb(MtcsPass *mtcsPass,function *fun)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(mtcsPass);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsOptions *mtcsOptions=mtcs_target_get_options(mtcsTarget);
   MtcsOptionsItem *mtcsOptionsItem=mtcsOptions->global_options;
   return mtcsOptionsItem->x_optimize>0 && mtcsOptionsItem->x_flag_ext_dce;
}

static nuint ext_dce_execute_cb(MtcsPass *mtcsPass,function *func)
{
   ext_dce_execute ((MtcsExtDce *)mtcsPass);
   return 0;
}

static void mtcsExtDceInit(MtcsExtDce *self)
{
   MtcsPass *mtcsPass= (MtcsPass *)self;
   mtcsPass->execute =ext_dce_execute_cb;
   mtcsPass->gate =ext_dce_gate_cb;
...
{
   MtcsExtDce *self = n_slice_alloc0 (sizeof(MtcsExtDce));
   mtcs_component_set_mode((MtcsComponent*)self,mtcsMode);
   mtcs_pass_init((MtcsPass *)self,RTL_PASS,"ext_dce");
   mtcsExtDceInit(self);
   return self;
}
