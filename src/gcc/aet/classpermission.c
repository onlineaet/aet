...
static FieldDecorate createFieldDecorate(int keyword,int classType,location_t loc)
{
	...
	if(keyword==RID_STATIC){
		FieldDecorate dr={per,TRUE,FALSE,loc,FALSE};
		return dr;
	}else if(keyword==RID_AET_FINAL){
		 FieldDecorate dr={per,FALSE,TRUE,loc,FALSE};
	    return dr;
	}else{
	    ...
	    FieldDecorate dr={per,FALSE,FALSE,loc,(keyword==RID_AET_PUBLIC || keyword==RID_AET_PROTECTED || keyword==RID_AET_PRIVATE)};
	    return dr;
	}
}
...
FieldDecorate   class_permission_try_parser(ClassPermission *self,int classType)
{
   ...
   if(first==-1){
      ...
      FieldDecorate dr={per,FALSE,FALSE,0,FALSE};
      return dr;
   }else{
      if(second==-1){
 ...
 */
static nboolean checkParentFieldPermission(ClassPermission *self,location_t loc,ClassName *className,ClassFunc *dest)
{
   ClassInfo *classInfo=class_mgr_get_class_info_by_class_name(class_mgr_get(),className);
   if(classInfo->parentName.sysName){
      NPtrArray  *funcs=func_mgr_get_funcs(func_mgr_get(), &classInfo->parentName);
      if(funcs!=NULL){
         ...
         for(i=0;i<funcs->len;i++){
            ClassFunc *item=(ClassFunc *)n_ptr_array_index(funcs,i);
            if(strcmp(item->rawMangleName,dest->rawMangleName)==0){
               int a = item->permission==CLASS_PERMISSION_DEFAULT?CLASS_PERMISSION_PROTECTED:item->permission;
               int b = dest->permission==CLASS_PERMISSION_DEFAULT?CLASS_PERMISSION_PROTECTED:dest->permission;
               if(b>a){
                  error_at(loc,"类%qs的方法%qs不能比父类%qs的方法可见性低。",
                                           className->userName,item->orgiName,classInfo->parentName.userName);
                  return FALSE;
               }
            }
         }
      }
      return checkParentFieldPermission(self,loc,&classInfo->parentName,dest);
   }
   return TRUE;
}
...
void class_permission_set_field_decorate(ClassPermission *self,tree decls,ClassName *className)
{
   ...
   if(!is){
      ...
      //检查方法权限
      checkParentFieldPermission(self,DECL_SOURCE_LOCATION(decls),className,entity);
   }
}
