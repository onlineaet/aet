	    ...
	    if (DECL_NAME (field) == NULL_TREE){
	      ...
	      while (DECL_NAME (field_array[bot]) == NULL_TREE){
		     ...
		     if (RECORD_OR_UNION_TYPE_P (TREE_TYPE (field))){
		        ...
		        if (flag_plan9_extensions && TYPE_NAME (TREE_TYPE (field)) != NULL_TREE && (TREE_CODE (TYPE_NAME (TREE_TYPE (field)))   == TYPE_DECL)
		        		&& (DECL_NAME (TYPE_NAME (TREE_TYPE (field)))== component))
			     break;
		     }
		  }
...
tree build_component_ref (location_t loc, tree datum, tree component,location_t component_loc)
{
  ...
  if (code == RECORD_TYPE || code == UNION_TYPE){
      ...
      //n_debug("build_component_ref 11 name:%s code:%s field:%p ",
  		    			//IDENTIFIER_POINTER(component),field?get_tree_code_name(TREE_CODE(field)):"NULL",field);
      if (!field){
	     ...
	     if (guessed_id){
	        ...
	        location_t reported_loc= (component_loc != UNKNOWN_LOCATION) ? component_loc : loc;
	 ...
	 behavior (C11 6.5.2.3#5).  */
      if (TYPE_ATOMIC (type) && c_inhibit_evaluation_warnings == 0){
	     if (code == RECORD_TYPE)
	         warning_at (loc, 0, "accessing a member %qE of an atomic structure %qE", component, datum);
	    else
	         warning_at (loc, 0, "accessing a member %qE of an atomic union %qE", component, datum);
	  }
	 ...
	 recursively call itself.  We can't do that here.  */
      do{
	     ...
	     if (TREE_TYPE (subdatum) == error_mark_node)
	        return error_mark_node;
	     n_debug("build_component_ref 22 field:%s subdatum:%s TREE_TYPE (subdatum):%s ",
	    		 get_tree_code_name(TREE_CODE(field)),get_tree_code_name(TREE_CODE(subdatum)),
				 get_tree_code_name(TREE_CODE(TREE_TYPE (subdatum))));
	   ...
	   n_debug("build_component_ref 33 sub:%p datum:%p subdatum:%p ref:%p field:%p type:%p reftype:%p ",
			   subtype, datum, subdatum,ref,field,type,TREE_TYPE(ref));
	   ...
	   field = TREE_CHAIN (field);
	     ...
	     n_debug("build_component_ref 44 name:%s code:%s field:%p ref:%p datum:%p reftype:%p ",
	       		    			IDENTIFIER_POINTER(component),get_tree_code_name(code),field,ref,datum,TREE_TYPE(ref));
      return ref;
	 ...
	 where the user has confused "." vs "->".  */
      ...
      error_at (loc, "request for member %qE in something not a structure or union",component);
...
tree build_indirect_ref (location_t loc, tree ptr, ref_operator errstring)
{

  tree pointer = default_conversion (ptr);
  tree type = TREE_TYPE (pointer);
  tree ref;
  //n_debug("build_indirect_ref 00 pointer:%p type:%p %s %s ref_operator:%d",
		  //pointer,type,get_tree_code_name(TREE_CODE(pointer)),get_tree_code_name(TREE_CODE(type)),errstring);

  if (TREE_CODE (type) == POINTER_TYPE)  {
      if (CONVERT_EXPR_P (pointer) || TREE_CODE (pointer) == VIEW_CONVERT_EXPR){
	  /* If a warning is issued, mark it to avoid duplicates from
	     the backend.  This only needs to be done at
	     warn_strict_aliasing > 2.  */
    	  n_debug("build_indirect_ref 11 %s %s",get_tree_code_name(TREE_CODE(pointer)),get_tree_code_name(TREE_CODE(type)));
	     if (warn_strict_aliasing > 2)
		     if (strict_aliasing_warning (EXPR_LOCATION (pointer),type, TREE_OPERAND (pointer, 0)))
	           TREE_NO_WARNING (pointer) = 1;
	  }

      if (TREE_CODE (pointer) == ADDR_EXPR && (TREE_TYPE (TREE_OPERAND (pointer, 0)) == TREE_TYPE (type))){
	     ref = TREE_OPERAND (pointer, 0);
	     protected_set_expr_location (ref, loc);
	     n_debug("build_indirect_ref 22 %s %s",get_tree_code_name(TREE_CODE(pointer)),get_tree_code_name(TREE_CODE(ref)));
	     return ref;
	  }else{
	     tree t = TREE_TYPE (type);
	     ref = build1 (INDIRECT_REF, t, pointer);
	     n_debug("build_indirect_ref 33 t:%p %s %s",t,get_tree_code_name(TREE_CODE(t)),get_tree_code_name(TREE_CODE(ref)));
...
build_external_ref (location_t loc, tree id, bool fun, tree *type)
{
    ...
    *type = NULL;
    if (decl && decl != error_mark_node){
    	n_debug("build_external_ref 11 找到了声明的变量 decl code:%s name:%s ",
    			get_tree_code_name(TREE_CODE(decl)),IDENTIFIER_POINTER(id));
//       if(TREE_CODE (decl) == PARM_DECL){
//    	  tree dd=DECL_NAME(decl);
//    	  tree pointer=TREE_TYPE(decl);
//      	  printf("build_external_ref 22aa 参数声明 PARM_DECL pointer:%s  %s %s %d\n",get_tree_code_name(TREE_CODE(pointer)),__FILE__,__FUNCTION__,__LINE__);
//      	  if(TREE_CODE(pointer)==POINTER_TYPE){
//			  tree record=TREE_TYPE(pointer);
//			  printf("build_external_ref 22bb 参数声明 PARM_DECL pointer:%s  %s %s %d\n",get_tree_code_name(TREE_CODE(record)),__FILE__,__FUNCTION__,__LINE__);
//	      	  if(TREE_CODE(record)==RECORD_TYPE){
//				  tree rna=TYPE_NAME(record);
//				  printf("build_external_ref 22cc 参数声明 PARM_DECL pointer:%s  %s %s %d\n",get_tree_code_name(TREE_CODE(rna)),__FILE__,__FUNCTION__,__LINE__);
//				  tree classId=DECL_NAME(rna);
//				  printf("build_external_ref 22dd 参数声明 PARM_DECL pointer:%s  %s %s %d\n",get_tree_code_name(TREE_CODE(classId)),__FILE__,__FUNCTION__,__LINE__);
//				  printf("build_external_ref 22ee 参数声明 PARM_DECL pointer:%s  %s %s %d\n",IDENTIFIER_POINTER(classId),__FILE__,__FUNCTION__,__LINE__);
//				  printf("build_external_ref 22 参数声明 PARM_DECL %s decl:%p name:%s %s %s %d\n",
//						  IDENTIFIER_POINTER(dd),decl,IDENTIFIER_POINTER(id),__FILE__,__FUNCTION__,__LINE__);
//	      	  }
//      	  }
//       }
      ref = decl;
      *type = TREE_TYPE (ref);
    	...
    	//printf("build_external_ref 33 name:%s %s %s %d\n",IDENTIFIER_POINTER(id),__FILE__,__FUNCTION__,__LINE__);

       /* Implicit function declaration.  */
       ref = implicitly_declare (loc, id);
    }else if (decl == error_mark_node){
    	printf("build_external_ref 44 decl是错的 error_mark_node name:%s %s %s %d\n",IDENTIFIER_POINTER(id),__FILE__,__FUNCTION__,__LINE__);

       /* Don't complain about something that's already been
        complained about.  */
      return error_mark_node;
    }else {
    	printf("build_external_ref 55 找不到声明的变量name:%s %s %s %d\n",IDENTIFIER_POINTER(id),__FILE__,__FUNCTION__,__LINE__);
  ...
  /* Recursive call does not count as usage.  */
    ...
    if (TREE_CODE (ref) == FUNCTION_DECL && !in_alignof){
    	//printf("build_external_ref 66 name:%s %s %s %d\n",IDENTIFIER_POINTER(id),__FILE__,__FUNCTION__,__LINE__);

       if (!in_sizeof && !in_typeof){
       	//printf("build_external_ref 77 name:%s %s %s %d\n",IDENTIFIER_POINTER(id),__FILE__,__FUNCTION__,__LINE__);

	      C_DECL_USED (ref) = 1;
       }else if (DECL_INITIAL (ref) == NULL_TREE && DECL_EXTERNAL (ref) && !TREE_PUBLIC (ref)){
       	//printf("build_external_ref 88 name:%s %s %s %d\n",IDENTIFIER_POINTER(id),__FILE__,__FUNCTION__,__LINE__);
    ...
    if (TREE_CODE (ref) == CONST_DECL){
    	n_debug("build_external_ref 99 name:%s",IDENTIFIER_POINTER(id));

       used_types_insert (TREE_TYPE (ref));
       if (warn_cxx_compat  && TREE_CODE (TREE_TYPE (ref)) == ENUMERAL_TYPE  && C_TYPE_DEFINED_IN_STRUCT (TREE_TYPE (ref))){
	      warning_at (loc, OPT_Wc___compat,("enum constant defined in struct or union is not visible in C++"));
      ...
      TREE_CONSTANT (ref) = 1;
	   ...
	   && (VAR_OR_FUNCTION_DECL_P (ref) || TREE_CODE (ref) == PARM_DECL)){
    	//printf("build_external_ref 100 name:%s %s %s %d\n",IDENTIFIER_POINTER(id),__FILE__,__FUNCTION__,__LINE__);
  ...
  /* C99 6.7.4p3: An inline definition of a function with external
     ...
     internal linkage.  */
	   ...
	   && DECL_CONTEXT (ref) != current_function_decl){
     	...
     	printf("build_external_ref 101 name:%s %s %s %d\n",IDENTIFIER_POINTER(id),__FILE__,__FUNCTION__,__LINE__);
			 ...
			 vec<tree, va_gc> *origtypes, tree orig_fundecl)
{
 ...
 // n_debug("build_function_call_vec 33 %s fntype code:%s function:%s",funName,get_tree_code_name(TREE_CODE (fntype)),
  ...
  if (name != NULL_TREE  && !strncmp (IDENTIFIER_POINTER (name), "__builtin_", 10)){
	  n_debug("build_function_call_vec 55 %s",name?IDENTIFIER_POINTER(name):"unknown");
	   ...
	   n_debug("build_function_call_vec 66 主要的方法在这里 %s\n",name?IDENTIFIER_POINTER(name):"unknown");
  ...
  /* In this improbable scenario, a nested function returns a VM type.
   ...
   if (variably_modified_type_p (TREE_TYPE (fntype), NULL_TREE)){
	   n_debug("build_function_call_vec 77 %s",name?IDENTIFIER_POINTER(name):"unknown");
		   ...
		   vec<tree, va_gc> *values, vec<tree, va_gc> *origtypes, tree function, tree fundecl)
{
  ...
  /* For type-generic built-in functions, determine whether excess
    ...
    if (fundecl   && fndecl_built_in_p (fundecl, BUILT_IN_NORMAL)){
    	n_debug("convert_arguments 00 fundecl:%s origtypes:%p %s %s %d\n",
	  ...
	  /* For a call to a built-in function declared without a prototype
	     ...
	     match those of the arguments to.  */
    	   n_debug("convert_arguments 22 fundecl:%s origtypes:%p code:%d %s %s %d\n",
	 ...
	 (comparison).  */
       if (type_generic){
    	   n_debug("convert_arguments 33 fundecl:%s origtypes:%p %s %s %d\n",
    	  	        		 get_tree_code_name(TREE_CODE(fundecl)),origtypes,__FILE__,__FUNCTION__,__LINE__);
	      switch (code){
  ...
  /* Scan the given expressions (VALUES) and types (TYPELIST), producing
    ...
    for (typetail = typelist,builtin_typetail = builtin_typelist,parmnum = 0; values && values->iterate (parmnum, &val);++parmnum){
	  ...
	  ? expansion_point_location_if_in_system_header (arg_loc[parmnum]): input_location;
       if (type == void_type_node){
	      ...
	      inform_declaration (fundecl);
	       //printf("convert_arguments 55 进入循环 错误 type == void_type_node 直接返回 parmnum:%d error_args:%d %s %s %d\n",
	    		  // parmnum,error_args,__FILE__,__FUNCTION__,__LINE__);
	      return error_args ? -1 : (int) parmnum;
	   }
       if (builtin_type == void_type_node){
	      if (warning_at (loc, OPT_Wbuiltin_declaration_mismatch, "too many arguments to built-in function %qE expecting %d", function, parmnum))
	         inform_declaration (fundecl);
	 ...
	 (ARM __fp16), but not to _FloatN or _FloatNx types.  */
	  ...
	  /* Promote this argument, unless it has a _FloatN or
	     _FloatNx type.  */
    	   n_debug("convert_arguments 77 进入循环  parmnum:%d error_args:%d %s %s %d\n",
    		     	    		   parmnum,error_args,__FILE__,__FUNCTION__,__LINE__);
       ...
       if (type != NULL_TREE){
	      ...
	      if(origtype==NULL_TREE){
	     	   n_debug("convert_arguments origtype ---origtype==NULL_TREE---xx %d %p",parmnum,origtypes);
	      }
	      n_debug("convert_arguments 88 进入循环从origtypes取实参 并调convert_argument code:%s  parmnum:%d error_args:%d",
	         			  origtype?get_tree_code_name(TREE_CODE(origtype)):"NULL", parmnum,error_args);
	      ...
	      /* Convert `float' to `double'.  */
	    	  n_debug("convert_arguments 99 进入循环 Convert `float' to `double  parmnum:%d error_args:%d",parmnum,error_args);
			...
			/* A "double" argument with excess precision being passed
			   ...
			   promoted.  */
		   n_debug("convert_arguments 100 进入循环 还是关于double  parmnum:%d error_args:%d",
	    		     	     parmnum,error_args);
	         parmval = convert (valtype, val);
	   }else if ((invalid_func_diag =targetm.calls.invalid_arg_for_unprototyped_fn (typelist, fundecl, val))){
		   n_warning("convert_arguments 101 进入循环 出错了返回-1 invalid_func_diag =targetm.calls.invalid_a... parmnum:%d error_args:%d %s %s %d\n",
		  	    		     	     parmnum,error_args,__FILE__,__FUNCTION__,__LINE__);
	      error (invalid_func_diag);
	      return -1;
	   }else if (TREE_CODE (val) == ADDR_EXPR && reject_gcc_builtin (val)){
		   n_warning("convert_arguments 102 进入循环 出错了返回-1 TREE_CODE (val) == ADDR_EXPR && reject_gcc_builtin (val) parmnum:%d error_args:%d %s %s %d\n",
		  		  	    		     	     parmnum,error_args,__FILE__,__FUNCTION__,__LINE__);
	      return -1;
	   }else{
	  ...
	  /* For a call to a built-in function declared without a prototype,
	   ...
	   n_debug("convert_arguments 105 进入循环  结束一次循环 c-typeck.c typetail:%p builtin_typetail:%p parmnum:%d error_args:%d",
    ...
    if (builtin_typetail && TREE_VALUE (builtin_typetail) != void_type_node){
       ...
       if (warning_at (loc, OPT_Wbuiltin_declaration_mismatch,"too few arguments to built-in function %qE expecting %u", function, nargs - 1))
	      inform_declaration (fundecl);
			...
			struct c_expr arg1, struct c_expr arg2)
{
  ...
  //printf("parser_build_binary_op 00 %s %s arg1.original_typ:%p arg2.original_typ:%p\n",
		 // get_tree_code_name(code1),get_tree_code_name(code2),arg1.original_type,arg2.original_type);
...
build_c_cast (location_t loc, tree type, tree expr)
{
  ...
  if (TREE_CODE (value) == REAL_CST
	  ...
	  && !((TREE_CODE (expr) == INTEGER_CST
	    ...
	    // printf("build_c_cast NOP_EXPR %p %p\n",type,value);
		   ...
		   location_t rhs_loc, tree rhs, tree rhs_origtype)
{
  ...
  bool npc;
  bool is_atomic_op;
//   printf("build_modify_expr 00 将左值与右值组合成一个赋值表达式,组建树结构 lhs:%p rhs:%p %s %s %d\n",lhs,rhs,__FILE__,__FUNCTION__,__LINE__);
//   printf("build_modify_expr 00xx lhs:%s rhs:%s modifycode:%s %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//		   get_tree_code_name(TREE_CODE(rhs)),get_tree_code_name(modifycode),__FILE__,__FUNCTION__,__LINE__);

  /* Types that aren't fully specified cannot be used in assignments.  */
  lhs = require_complete_type (location, lhs);
//  printf("build_modify_expr 00YY TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//		   lhs,__FILE__,__FUNCTION__,__LINE__);
  ...
  if (TREE_CODE (lhs) == C_MAYBE_CONST_EXPR)
    {
//	  printf("build_modify_expr 11 TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//			   lhs,__FILE__,__FUNCTION__,__LINE__);
  ...
  if (modifycode != NOP_EXPR)
    {
//	  printf("build_modify_expr 22 TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//			   lhs,__FILE__,__FUNCTION__,__LINE__);
...
//  printf("build_modify_expr 33 TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//		   lhs,__FILE__,__FUNCTION__,__LINE__);
...
//  printf("build_modify_expr 44 TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//		   lhs,__FILE__,__FUNCTION__,__LINE__);
...
//  printf("build_modify_expr 55 TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//		   lhs,__FILE__,__FUNCTION__,__LINE__);
...
//      printf("build_modify_expr 66 TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//    		   lhs,__FILE__,__FUNCTION__,__LINE__);
      newrhs = convert_for_assignment (location, rhs_loc, lhstype, newrhs,
				       rhs_origtype, ic_assign, npc, NULL_TREE, NULL_TREE, 0);
      if (TREE_CODE (newrhs) == ERROR_MARK)
	    return error_mark_node;
   }
 // printf("build_modify_expr 77 TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//		   lhs,__FILE__,__FUNCTION__,__LINE__);
  ...
  if (is_atomic_op)
    ...
    {
//	  printf("build_modify_expr 88 TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
//			   lhs,__FILE__,__FUNCTION__,__LINE__);
  ...
  if (olhstype == TREE_TYPE (result)){
	  //printf("build_modify_expr 99 TREE_TYPE(lhs):%s lhs:%p rhseval:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
	//		   lhs,rhseval,__FILE__,__FUNCTION__,__LINE__);
    goto return_result;
  }

  //printf("build_modify_expr 100 TREE_TYPE(lhs):%s lhs:%p %s %s %d\n",get_tree_code_name(TREE_CODE(lhs)),
	//	   lhs,__FILE__,__FUNCTION__,__LINE__);
...
#define WARNING_FOR_QUALIFIERS(LOCATION, PLOC, OPT, AR, AS, IN, RE, QUALS) \
  ...
  if (c_dialect_objc ()){
   ...
   if (warn_cxx_compat) {
    ...
    if (warn_enum_conversion) {
      tree checktype = origtype != NULL_TREE ? origtype : rhstype;
      if (checktype != error_mark_node  && TREE_CODE (checktype) == ENUMERAL_TYPE && TREE_CODE (type) == ENUMERAL_TYPE
  ...
  /* A non-reference type can convert to a reference.  This handles
   ...
   if (codel == REFERENCE_TYPE && coder != REFERENCE_TYPE){
      ...
      if (rhs == error_mark_node)
	      return error_mark_node;
      n_debug("convert_for_assignment NOP_EXPR %p %p\n",type,rhs);
  ...
  /* Arithmetic types all interconvert, and enum is treated like int.  */
	   ...
	   && (coder == INTEGER_TYPE || coder == REAL_TYPE
	       ...
	       || coder == BOOLEAN_TYPE))
    {
	  n_debug("convert_for_assignment ----cccc codel == INTEGER_TYPE || codel == REAL_TYPE warnopt:%d %d\n",warnopt,errtype == ic_argpass);
  ...
  if ((codel == RECORD_TYPE || codel == UNION_TYPE)   && codel == coder   && comptypes (type, rhstype))
      ...
      for (memb = TYPE_FIELDS (type); memb ; memb = DECL_CHAIN (memb)){
	     ...
	     if (TREE_CODE (memb_type) != POINTER_TYPE)
          ...
          if ((VOID_TYPE_P (ttl) && !TYPE_ATOMIC (ttl))  || (VOID_TYPE_P (ttr) && !TYPE_ATOMIC (ttr)) || comp_target_types (location, memb_type, rhstype)){
                ...
                if (lquals == rquals || ((TREE_CODE (ttr) == FUNCTION_TYPE && TREE_CODE (ttl) == FUNCTION_TYPE)
                  ? ((lquals | rquals) == rquals) : ((lquals | rquals) == lquals)))
                  break;
  ...
  if (memb || marginal_memb){
	  if (!memb){
	   ...
	   }//end TREE_CODE (ttr) == FUNCTION_TYPE  && TREE_CODE (ttl) == FUNCTION_TYPE
  }/* Conversions among pointers */else if ((codel == POINTER_TYPE || codel == REFERENCE_TYPE)  && (coder == codel)){
	 ...
	 and RHSTYPE is set to the actual type of the built-in.  */
      ...
      if (TREE_CODE (mvl) != ARRAY_TYPE)
        mvl = (TYPE_ATOMIC (mvl) ? c_build_qualified_type (TYPE_MAIN_VARIANT (mvl),TYPE_QUAL_ATOMIC) : TYPE_MAIN_VARIANT (mvl));
      if (TREE_CODE (mvr) != ARRAY_TYPE)
	     mvr = (TYPE_ATOMIC (mvr) ? c_build_qualified_type (TYPE_MAIN_VARIANT (mvr),TYPE_QUAL_ATOMIC): TYPE_MAIN_VARIANT (mvr));
	 ...
	 within the struct.  */
      ...
      if (VOID_TYPE_P (ttr) && rhs != null_pointer_node && !VOID_TYPE_P (ttl))
	      warning_at (errtype == ic_argpass ? expr_loc : location,
		    OPT_Wc___compat, "request for implicit conversion from %qT to %qT not permitted in C++", rhstype, type);
	 ...
	 Meanwhile, the lhs target must have all the qualifiers of the rhs.  */
      if ((VOID_TYPE_P (ttl) && !TYPE_ATOMIC (ttl))  || (VOID_TYPE_P (ttr) && !TYPE_ATOMIC (ttr))
		 ...
		 qualifier on the element type. */
	      ...
	      /* Assignments between atomic and non-atomic objects are OK.  */
              if (TYPE_QUALS_NO_ADDR_SPACE_NO_ATOMIC (ttr)	  & ~TYPE_QUALS_NO_ADDR_SPACE_NO_ATOMIC (ttl)){
                ...
                no warning.  */
              else if (VOID_TYPE_P (ttl) || VOID_TYPE_P (ttr) || target_cmp)
		         ;
...
//zclei
void  aet_record_maybe_used_decl(tree decl)
{
   record_maybe_used_decl(decl);
}
