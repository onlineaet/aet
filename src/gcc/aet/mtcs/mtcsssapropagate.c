...
static void mtcsSsaPropagateInit(MtcsSsaPropagate *self)
{
...
static void add_ssa_edge (MtcsSsaPropagate *self, tree var)
{
   ...
   FOR_EACH_IMM_USE_FAST (use_p, iter, var){
      ...
      && !(EDGE_PRED (use_bb, PHI_ARG_INDEX_FROM_USE (use_p))->flags  & EDGE_EXECUTABLE))
         continue;

      if (bitmap_set_bit (self->ssa_edge_worklist, gimple_uid (use_stmt))){
         self->uid_to_stmt[gimple_uid (use_stmt)] = use_stmt;
         if (dump_file && (dump_flags & TDF_DETAILS)){
            fprintf (dump_file, "self->ssa_edge_worklist: adding SSA use in ");
...
static void add_control_edge (MtcsSsaPropagate *self,edge e)
{
   ...
   int bb_order = self->bb_to_cfg_order[bb->index];
   bitmap_set_bit (self->cfg_blocks, bb_order);
...
//private
void mtcs_ssa_propagation_engine::simulate_stmt (gimple *stmt)
{
   enum mtcs_ssa_prop_result val = MTCS_SSA_PROP_NOT_INTERESTING;
   edge taken_edge = NULL;
   tree output_name = NULL_TREE;

   /* Pull the stmt off the SSA edge worklist.  */
   bitmap_clear_bit (self->ssa_edge_worklist, gimple_uid (stmt));
   ...
   if (val == SSA_PROP_VARYING){
      ...
      if (taken_edge)
         add_control_edge(self,taken_edge);
   }
   ...
   if (gimple_code  (stmt) == GIMPLE_PHI){
      ...
      FOR_EACH_SSA_USE_OPERAND (use_p, stmt, iter, SSA_OP_USE){
         ...
         if (!gimple_nop_p (def_stmt) && prop_simulate_again_p (def_stmt)){
            has_simulate_again_uses = true;
            break;
         }
      }
...
void mtcs_ssa_propagation_engine::simulate_block (basic_block block)
{
   ...
   if (! (block->flags & BB_VISITED)){
      ...
      if (normal_edge_count == 1)
         add_control_edge(self,normal_edge);
   }
}


/* Initialize local data structures and work lists.  */
static void ssa_prop_init (MtcsSsaPropagate *self)
{
   edge e;
   edge_iterator ei;
   basic_block bb;
   ...
   self->ssa_edge_worklist = BITMAP_ALLOC (NULL);
   bitmap_tree_view (self->ssa_edge_worklist);

   /* Worklist of basic-blocks.  */
   self->bb_to_cfg_order = XNEWVEC (int, last_basic_block_for_fn (cfun) + 1);
   self->cfg_order_to_bb = XNEWVEC (int, n_basic_blocks_for_fn (cfun));
   int n = pre_and_rev_post_order_compute_fn (cfun, NULL,self->cfg_order_to_bb, false);
   for (int i = 0; i < n; ++i)
      self->bb_to_cfg_order[self->cfg_order_to_bb[i]] = i;
   self->cfg_blocks = BITMAP_ALLOC (NULL);
   ...
   for (int i = 0; i < n; ++i){
      ...
      bb = BASIC_BLOCK_FOR_FN (cfun, self->cfg_order_to_bb[i]);
   ...
   self->uid_to_stmt.safe_grow (gimple_stmt_max_uid (cfun), true);
}


/* Free allocated storage.  */
static void ssa_prop_fini (MtcsSsaPropagate *self)
{
   BITMAP_FREE (self->cfg_blocks);
   free (self->bb_to_cfg_order);
   free (self->cfg_order_to_bb);
   BITMAP_FREE (self->ssa_edge_worklist);
   self->uid_to_stmt.release ();
}
...
void mtcs_ssa_propagation_engine::ssa_propagate (void)
{
   ssa_prop_init(self);
   ...
   while (1){
      int next_block_order = (bitmap_empty_p (self->cfg_blocks) ? -1 : bitmap_first_set_bit (self->cfg_blocks));
      int next_stmt_uid = (bitmap_empty_p (self->ssa_edge_worklist) ? -1 : bitmap_first_set_bit (self->ssa_edge_worklist));
      ...
      if (next_stmt_uid != -1){
         next_stmt = self->uid_to_stmt[next_stmt_uid];
         next_stmt_bb_order = self->bb_to_cfg_order[gimple_bb (next_stmt)->index];
      }

      /* Pull the next block to simulate off the worklist if it comes first.  */
      if (next_block_order != -1  && (next_stmt_bb_order == -1 || next_block_order <= next_stmt_bb_order)){
         self->curr_order = next_block_order;
         bitmap_clear_bit (self->cfg_blocks, next_block_order);
         basic_block bb  = BASIC_BLOCK_FOR_FN (cfun, self->cfg_order_to_bb [next_block_order]);
         simulate_block (bb);
      }else{/* Else simulate from the SSA edge worklist.  */
         self->curr_order = next_stmt_bb_order;
         if (dump_file && (dump_flags & TDF_DETAILS)){
...
bool mtcs_ssa_propagate_stmt_makes_single_store (MtcsSsaPropagate *self,gimple *stmt)
{
   ...
   return (!TREE_THIS_VOLATILE (lhs) && (DECL_P (lhs) || REFERENCE_CLASS_P (lhs)));
}
...
tree mtcs_substitute_and_fold_engine::value_on_edge (edge, tree expr)
{
   return value_of_expr (expr);
}

tree mtcs_substitute_and_fold_engine::value_of_stmt (gimple *stmt, tree name)
{
   if (!name)
...
bool mtcs_substitute_and_fold_engine::range_of_expr (vrange &, tree, gimple *)
{
   return false;
}
...
bool mtcs_substitute_and_fold_engine::replace_uses_in (gimple *stmt)
{
   bool replaced = false;
   use_operand_p use;
   ssa_op_iter iter;
   ...
   FOR_EACH_SSA_USE_OPERAND (use, stmt, iter, SSA_OP_USE){
      ...
      if (gimple_code (stmt) == GIMPLE_ASM  && !may_propagate_copy_into_asm (tuse))
         continue;
...
bool mtcs_substitute_and_fold_engine::replace_phi_args_in (gphi *phi)
{
   size_t i;
   bool replaced = false;
   ...
   for (i = 0; i < gimple_phi_num_args (phi); i++){
      ...
      if (TREE_CODE (arg) == SSA_NAME){
         ...
         if (val && val != arg && may_propagate_copy (arg, val)){
            ...
            accordingly.  */
            if (TREE_CODE (val) == SSA_NAME  && e->flags & EDGE_ABNORMAL  && !SSA_NAME_OCCURS_IN_ABNORMAL_PHI (val)){
...
class mtcs_substitute_and_fold_dom_walker : public dom_walker
{
public:
   mtcs_substitute_and_fold_dom_walker (cdi_direction direction,
                class mtcs_substitute_and_fold_engine *engine)
   : dom_walker (direction),
    ...
    ~mtcs_substitute_and_fold_dom_walker ()
    {
    ...
    class mtcs_substitute_and_fold_engine *substitute_and_fold_engine;
private:
...
void mtcs_substitute_and_fold_dom_walker::foreach_new_stmt_in_bb (gimple_stmt_iterator old_gsi,gimple_stmt_iterator new_gsi)
{
...
bool mtcs_substitute_and_fold_engine::propagate_into_phi_args (basic_block bb)
{
   edge e;
   edge_iterator ei;
   ...
   FOR_EACH_EDGE (e, ei, bb->succs){
      for (gphi_iterator gpi = gsi_start_phis (e->dest); !gsi_end_p (gpi); gsi_next (&gpi)){
         gphi *phi = gpi.phi ();
         ...
         if (TREE_CODE (arg) != SSA_NAME || virtual_operand_p (arg))
            continue;
         tree val = value_on_edge (e, arg);
         if (val  && is_gimple_min_invariant (val)  && may_propagate_copy (arg, val)) {
...
edge mtcs_substitute_and_fold_dom_walker::before_dom_children (basic_block bb)
{
   substitute_and_fold_engine->pre_fold_bb (bb);

   /* Propagate known values into PHI nodes.  */
   for (gphi_iterator i = gsi_start_phis (bb); !gsi_end_p (i); gsi_next (&i)){
      gphi *phi = i.phi ();
   ...
   for (gimple_stmt_iterator i = gsi_start_bb (bb);!gsi_end_p (i); gsi_next (&i)){
      bool did_replace;
      ...
      if (lhs && TREE_CODE (lhs) == SSA_NAME){
         fprintf(stderr,"mtcsssapropagate.c before_dom_children 00 stmt:%p lhs:%p\n",stmt,lhs);
      ...
      if (gimple_code (stmt) == GIMPLE_COND){
         if ((EDGE_SUCC (bb, 0)->flags & EDGE_EXECUTABLE)^ (EDGE_SUCC (bb, 1)->flags & EDGE_EXECUTABLE)){
            if (((EDGE_SUCC (bb, 0)->flags & EDGE_TRUE_VALUE) != 0) == ((EDGE_SUCC (bb, 0)->flags & EDGE_EXECUTABLE) != 0))
      ...
      if (did_replace){
         ...
         if (can_make_abnormal_goto && !stmt_can_make_abnormal_goto (stmt))
            bitmap_set_bit (need_ab_cleanup, bb->index);

         /* If we turned a not noreturn call into a noreturn one
         schedule it for fixup.  */
         if (!was_noreturn && is_gimple_call (stmt) && gimple_call_noreturn_p (stmt))
...
bool mtcs_substitute_and_fold_engine::substitute_and_fold (basic_block block)
{
   if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf (dump_file, "\nSubstituting values and folding statements\n\n");

   memset (&prop_stats, 0, sizeof (prop_stats));
   fprintf(stderr,"mtcsssapropagate.c substitute_and_fold 00 block:%p\n",block);
   ...
   mtcs_substitute_and_fold_dom_walker walker (CDI_DOMINATORS, this);
   ...
   statistics_counter_event (cfun, "Constants propagated",prop_stats.num_const_prop);
   statistics_counter_event (cfun, "Copies propagated",prop_stats.num_copy_prop);
   statistics_counter_event (cfun, "Statements folded",prop_stats.num_stmts_folded);
...
bool mtcs_ssa_propagate_may_propagate_copy (MtcsSsaPropagate *self,tree dest, tree orig, bool dest_not_abnormal_phi_edge_p)
{
   ...
   be propagated.  */
   else if (TREE_CODE (orig) == SSA_NAME && SSA_NAME_OCCURS_IN_ABNORMAL_PHI (orig))
      return false;
   ...
   does not apply.  */
   else if (!dest_not_abnormal_phi_edge_p && TREE_CODE (dest) == SSA_NAME && SSA_NAME_OCCURS_IN_ABNORMAL_PHI (dest))
      return false;
...
bool mtcs_ssa_propagate_may_propagate_copy_into_stmt (MtcsSsaPropagate *self,gimple *dest, tree orig)
{
   ...
   else if (is_gimple_call (dest) && gimple_call_lhs (dest) != NULL_TREE)
...
void mtcs_ssa_propagate_propagate_value (MtcsSsaPropagate *self,use_operand_p op_p, tree val)
{
   if (flag_checking){
      bool ab = (is_a <gphi *> (USE_STMT (op_p))
            && (gimple_phi_arg_edge (as_a <gphi *> (USE_STMT (op_p)), PHI_ARG_INDEX_FROM_USE (op_p))->flags & EDGE_ABNORMAL));
      gcc_assert (may_propagate_copy (USE_FROM_PTR (op_p), val, !ab));
   }
   mtcs_ssa_propagate_replace_exp/*!replace_exp*/(self,op_p, val);
}
...
void mtcs_ssa_propagate_propagate_tree_value_into_stmt (MtcsSsaPropagate *self,gimple_stmt_iterator *gsi, tree val)
{
   ...
   }else if (is_gimple_call (stmt)  && gimple_call_lhs (stmt) != NULL_TREE){
      tree expr = NULL_TREE;
      propagate_tree_value (&expr, val);
...
{
    MtcsSsaPropagate *self = n_slice_alloc0 (sizeof(MtcsSsaPropagate));
    mtcs_component_set_mode((MtcsComponent *)self,mtcsMode);
    mtcsSsaPropagateInit(self);
    return self;
}
