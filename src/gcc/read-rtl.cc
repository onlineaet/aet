...
//zclei 重设machine_mode
struct replaceData{
    int maxModeNumber;
    const char **newModeNames;
    const char **newLowModeNames;
};

static int replaceModeName(void **slot, void *data ATTRIBUTE_UNUSED)
{
  struct mapping *iterator;
  iterator = (struct mapping *) *slot;
  fprintf(stderr,"read-rtl.cc replaceModeName 00-------- %p %s\n",iterator,iterator->name);
  if(!strcmp(iterator->name,"mode") || !strcmp(iterator->name,"MODE")){
      struct replaceData *rpd=(struct replaceData  *)data;
      const char **modeNames=!strcmp(iterator->name,"mode")?rpd->newLowModeNames:rpd->newModeNames;
      iterator->values=NULL;
      struct map_value **valuesPtr;
      valuesPtr=&iterator->values;
      int i;
      for(i=0;i<rpd->maxModeNumber;i++){
          //fprintf(stderr,"read-rtl.cc replaceModeName 11-------- %p %s modeNames[i]:%s\n",iterator,iterator->name,modeNames[i]);
          valuesPtr = add_map_value (valuesPtr, i, modeNames[i]);
      }
  }
  return 1;
}
...
//zclei 实现read-md.h中声明的方法 替换modes.attrs中的主机mode为设备modes
//并设置modes的get_c_token和find_builtin两个回调函数
void read_rtl_reset_modes(const char **modeNames,int maxModeNumber,
        void *get_mode_token_cb,void *find_mode_cb)
{
    fprintf(stderr,"read-rtl.cc read_rtl_reset_modes--- %d\n",maxModeNumber);
    int i;
    char *newLowModeNames[maxModeNumber];
    char *p;
    for(i=0;i<maxModeNumber;i++){
        char *copy = xstrdup (modeNames[i]);
        for (p = copy; *p != 0; p++)
           *p = TOLOWER (*p);
        newLowModeNames[i]=copy;
    }
    fprintf(stderr,"read-rtl.cc read_rtl_reset_modes---111 %d %d %p\n",maxModeNumber,getpid(),modes.attrs);

    modes.get_c_token = get_mode_token_cb;
    struct replaceData userData={maxModeNumber,modeNames,newLowModeNames};
    fprintf(stderr,"read-rtl.cc read_rtl_reset_modes---222 %d %d\n",maxModeNumber,getpid());

    htab_traverse (modes.attrs, replaceModeName, &userData);
    fprintf(stderr,"read-rtl.cc read_rtl_reset_modes---333 %d\n",maxModeNumber);

    /*
    for(i=0;i<maxModeNumber;i++){
        free(newLowModeNames[i]);
        newLowModeNames[i]=NULL;
    }
    */
    modes.find_builtin = find_mode_cb;//赋值新的查找mode的函数 替换原来的find_mode回调方法
}
