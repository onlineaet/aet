...
static void mtcsCseInit(MtcsCse *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   self->hard_regs_in_table.count=mtcs_reg_get_hard_reg_element_count(mtcsReg);
}
...
static void printbb(basic_block block)
{
   ...
   FOR_BB_INSNS (block, insn){
      if (!INSN_P (insn))
         continue;
      n_debug("mtcscse.c 打印块中的指令 i:%d  block:%p index:%d flags:%d insn:%p\n",
...
/* Count the number of times registers are used (not set) in X.
   ...
   deleted here.  */
static void count_reg_usage (MtcsCse *self,rtx x, int *counts, rtx dest, int incr)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   if (x == 0)
      return;
   n_debug("mtcscse.c count_reg_usage  00 x:%p dest:%p name:%s\n",x,dest,GET_RTX_NAME(GET_CODE (x)));

   switch (code = GET_CODE (x)){
      case REG:
         if (x != dest){
            n_debug("mtcscse.c count_reg_usage  11 设counts x:%p dest:%p name:%s REGNO (x):%d\n",x,dest,GET_RTX_NAME(code),REGNO (x));
            counts[REGNO (x)] += incr;
      ...
      case SYMBOL_REF:
      case LABEL_REF:
         n_debug("mtcscse.c count_reg_usage  22 x:%p dest:%p name:%s\n",x,dest,GET_RTX_NAME(code));
         return;
      ...
      case CLOBBER:
         ...
         as being used.  */
         if (MEM_P (XEXP (x, 0)))
            count_reg_usage(self,XEXP (XEXP (x, 0), 0), counts, NULL_RTX, incr);
         return;

      case SET:
         /* Unless we are setting a REG, count everything in SET_DEST.  */
         if (!REG_P (SET_DEST (x)))
            count_reg_usage(self,SET_DEST (x), counts, NULL_RTX, incr);
         count_reg_usage(self,SET_SRC (x), counts, dest ? dest : SET_DEST (x), incr);
         return;
      ...
      case JUMP_INSN:
         ...
         if ((!cfun->can_delete_dead_exceptions && !insn_nothrow_p (x)) || side_effects_p (PATTERN (x)))
            dest = mtcsRTL->pc_rtx/*!pc_rtx*/;
         if (code == CALL_INSN)
            count_reg_usage(self,CALL_INSN_FUNCTION_USAGE (x), counts, dest, incr);
         count_reg_usage(self,PATTERN (x), counts, dest, incr);
         ...
         if (note){
            ...
            if (GET_CODE (eqv) == EXPR_LIST)
               ...
               do{
                  count_reg_usage(self,XEXP (eqv, 0), counts, dest, incr);
                  eqv = XEXP (eqv, 1);
               }while (eqv && GET_CODE (eqv) == EXPR_LIST);
            else
               count_reg_usage(self,eqv, counts, dest, incr);
         }
         return;
      ...
      case EXPR_LIST:
         ...
         || GET_CODE (XEXP (x, 0)) == CLOBBER)
            count_reg_usage(self,XEXP (x, 0), counts, NULL_RTX, incr);

         count_reg_usage(self,XEXP (x, 1), counts, NULL_RTX, incr);
         return;

      case ASM_OPERANDS:
         /* Iterate over just the inputs, not the constraints as well.  */
         for (i = ASM_OPERANDS_INPUT_LENGTH (x) - 1; i >= 0; i--)
            count_reg_usage(self,ASM_OPERANDS_INPUT (x, i), counts, dest, incr);
         return;
   ...
   for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--){
      if (fmt[i] == 'e')
         count_reg_usage(self,XEXP (x, i), counts, dest, incr);
      else if (fmt[i] == 'E')
         for (j = XVECLEN (x, i) - 1; j >= 0; j--)
            count_reg_usage(self,XVECEXP (x, i, j), counts, dest, incr);
   }
}

typedef struct _CountStoresData{
    MtcsCse *self;
    void *data;
}CountStoresData;

/* Count the number of stores into pseudo.  Callback for note_stores.  */
//原型 count_stores cse.cc
static void count_stores_cb (rtx x, const_rtx set ATTRIBUTE_UNUSED, void *userData)
{
   CountStoresData *data=(CountStoresData *)userData;
   MtcsCse *self=data->self;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);

   int *counts = (int *) data->data;
   if (REG_P (x) && REGNO (x) >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))
      counts[REGNO (x)]++;
}

/* Return true if X is a dead register.  */
static inline bool is_dead_reg (MtcsCse *self,const_rtx x, int *counts)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   if(REG_P (x)){
      n_debug("mtcscse.c is_dead_reg 00 x:%p\n",x);
      if(REGNO (x) >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)){
         n_debug("mtcscse.c is_dead_reg 11 x:%p REGNO (x):%d FIRST_PSEUDO_REGISTER:%d counts[REGNO (x)] == 0:%d\n",
               x,REGNO (x),mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg),counts[REGNO (x)] == 0);
...
//原型 set_live_p cse.cc
static bool set_live_p (MtcsCse *self,rtx set, int *counts)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRtlanal *mtcsRtlanal=mtcs_target_get_rtlanal(mtcsTarget);

   if (mtcs_rtlanal_set_noop_p/*!set_noop_p*/(mtcsRtlanal,set)){
      n_debug("mtcscse.c set_live_p 00 是死insn:%p\n",set);
      return false;
   }
   n_debug("mtcscse.c set_live_p 11 是活的insn:%p is_dead_reg：%d side_effects_p：%d\n",set,
         is_dead_reg(self,SET_DEST (set), counts),side_effects_p (SET_SRC (set)));

   if (!is_dead_reg(self,SET_DEST (set), counts) || side_effects_p (SET_SRC (set))){
      return true;
   }
...
//原型 insn_live_p cse.cc
static bool insn_live_p (MtcsCse *self,rtx_insn *insn, int *counts)
{
   int i;
   if (!cfun->can_delete_dead_exceptions && !insn_nothrow_p (insn))
      return true;
   else if (GET_CODE (PATTERN (insn)) == SET){
      n_debug("mtcscse.c insn_live_p--- %p\n",insn);
      return set_live_p(self,PATTERN (insn), counts);
   }else if (GET_CODE (PATTERN (insn)) == PARALLEL){
      for (i = XVECLEN (PATTERN (insn), 0) - 1; i >= 0; i--){
         rtx elt = XVECEXP (PATTERN (insn), 0, i);

         if (GET_CODE (elt) == SET){
            if (set_live_p(self,elt, counts))
               return true;
      ...
      if (DEBUG_BIND_INSN_P (insn)  && TREE_VISITED (INSN_VAR_LOCATION_DECL (insn)))
         return false;
...
/* Return if DEBUG_INSN pattern PAT needs to be reset because some dead
   ...
   a replacement.  */
static bool is_dead_debug_insn (MtcsCse *self,const_rtx pat, int *counts, rtx *replacements,
          bool *seen_repl)
{
   subrtx_iterator::array_type array;
   FOR_EACH_SUBRTX (iter, array, pat, NONCONST){
      const_rtx x = *iter;
      if (is_dead_reg(self,x, counts)){
...
//原型 replace_dead_reg cse.cc
static rtx replace_dead_reg_cb (rtx x, const_rtx old_rtx ATTRIBUTE_UNUSED, void *userData)
{
   CountStoresData *data=(CountStoresData *)userData;
   MtcsCse *self=data->self;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   ...
   rtx *replacements = (rtx *) data->data;
   ...
   && replacements[REGNO (x)] != NULL_RTX){
      ...
      return mtcs_simplify_rtx_lowpart_subreg/*!lowpart_subreg*/(mtcsSimplifyRtx,
            GET_MODE (x), replacements[REGNO (x)], GET_MODE (replacements[REGNO (x)]));
   }
   return NULL_RTX;
}
...
//原型 #define FIXED_REGNO_P(N)  \
//  ((N) == FRAME_POINTER_REGNUM || (N) == HARD_FRAME_POINTER_REGNUM \
//   || fixed_regs[N] || global_regs[N]) cse.cc
static nboolean fixed_regno_p(MtcsCse *self,int n)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);

   int framePointerRegnum= mtcs_reg_get_frame_pointer_regnum(mtcsReg);
   int hardFramePointerRegnum=mtcs_reg_get_hard_frame_pointer_regnum(mtcsReg);
   return (n==framePointerRegnum || n==hardFramePointerRegnum ||
      mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[n] || mtcsReg->global_regs/*!global_regs*/[n]);
}
...
//原型 #define CHEAP_REGNO(N)                    \
//(REGNO_PTR_FRAME_P (N)                  \
//|| (HARD_REGISTER_NUM_P (N)                  \
//&& FIXED_REGNO_P (N) && REGNO_REG_CLASS (N) != NO_REGS)) cse.cc
static nboolean cheap_regno(MtcsCse *self,int n)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   return (mtcs_reg_regnum_ptr_frame_p(mtcsReg,n) || (mtcs_reg_is_hard(mtcsReg,n) && fixed_regno_p(self,n)
         && mtcs_reg_get_class(mtcsReg,n)!=NO_REGS));
}
...
#define COST(X, MODE)                     \
  (REG_P (X) ? 0 : notreg_cost (self,X, MODE, SET, 1))
#define COST_IN(X, MODE, OUTER, OPNO)              \
  (REG_P (X) ? 0 : notreg_cost (self,X, MODE, OUTER, OPNO))

/* Get the number of times this register has been updated in this
   basic block.  */

#define REG_TICK(N) (get_cse_reg_info (self,N)->reg_tick)

/* Get the point at which REG was recorded in the table.  */

#define REG_IN_TABLE(N) (get_cse_reg_info (self,N)->reg_in_table)

/* Get the SUBREG set at the last increment to REG_TICK (-1 if not a
   SUBREG).  */

#define SUBREG_TICKED(N) (get_cse_reg_info (self,N)->subreg_ticked)

/* Get the quantity number for REG.  */

#define REG_QTY(N) (get_cse_reg_info (self,N)->reg_qty)
...
static bool fixed_base_plus_p (MtcsCse *self,rtx x);
static int notreg_cost (MtcsCse *self,rtx, machine_mode, enum rtx_code, int);
static int preferable (int, int, int, int);
static void new_basic_block (MtcsCse *self);
static void make_new_qty (MtcsCse *self,unsigned int, machine_mode);
static void make_regs_eqv (MtcsCse *self,unsigned int, unsigned int);
static void delete_reg_equiv (MtcsCse *self,unsigned int);
static bool mention_regs (MtcsCse *self,rtx);
static bool insert_regs (MtcsCse *self,rtx, struct table_elt *, bool);
static void remove_from_table (MtcsCse *self,struct table_elt *, unsigned);
static void remove_pseudo_from_table (MtcsCse *self,rtx, unsigned);
static struct table_elt *lookup (MtcsCse *self,rtx, unsigned, machine_mode);
static struct table_elt *lookup_for_remove (MtcsCse *self,rtx, unsigned, machine_mode);
static rtx lookup_as_function (MtcsCse *self,rtx, enum rtx_code);
static struct table_elt *insert_with_costs (MtcsCse *self,rtx, struct table_elt *, unsigned,
                   machine_mode, int, int);
static struct table_elt *insert (MtcsCse *self,rtx, struct table_elt *, unsigned,
             machine_mode);
static void merge_equiv_classes (MtcsCse *self,struct table_elt *, struct table_elt *);
static void invalidate (MtcsCse *self,rtx, machine_mode);
static void remove_invalid_refs (MtcsCse *self,unsigned int);
static void remove_invalid_subreg_refs (MtcsCse *self,unsigned int, poly_uint64,
               machine_mode);
static void rehash_using_reg (MtcsCse *self,rtx);
static void invalidate_memory (MtcsCse *self);
static rtx use_related_value (MtcsCse *self,rtx, struct table_elt *);

static inline unsigned canon_hash (MtcsCse *self,rtx, machine_mode);
static inline unsigned safe_hash (MtcsCse *self,rtx, machine_mode);
static inline unsigned hash_rtx_string (const char *);

static rtx canon_reg (MtcsCse *self,rtx, rtx_insn *);
static enum rtx_code find_comparison_args (MtcsCse *self,enum rtx_code, rtx *, rtx *,
                  machine_mode *,machine_mode *);
static rtx fold_rtx (MtcsCse *self,rtx, rtx_insn *);
static rtx equiv_constant (MtcsCse *self,rtx);
static void record_jump_equiv (MtcsCse *self,rtx_insn *, bool);
static void record_jump_cond (MtcsCse *self,enum rtx_code, machine_mode, rtx, rtx);
static void cse_insn (MtcsCse *self,rtx_insn *);
static void cse_prescan_path (struct cse_basic_block_data *);//不移植
static void invalidate_from_clobbers (MtcsCse *self,rtx_insn *);
static void invalidate_from_sets_and_clobbers (MtcsCse *self,rtx_insn *);
static void cse_extended_basic_block (MtcsCse *self,struct cse_basic_block_data *);
extern void mtcs_cse_dump_class (struct table_elt*);
static void get_cse_reg_info_1 (MtcsCse *self,unsigned int regno);
static struct cse_reg_info * get_cse_reg_info (MtcsCse *self,unsigned int regno);

static void flush_hash_table (MtcsCse *self);
static void cse_change_cc_mode_insn (MtcsCse *self,rtx_insn *, rtx);
static void cse_change_cc_mode_insns (MtcsCse *self,rtx_insn *, rtx_insn *, rtx);
static machine_mode cse_cc_succs (MtcsCse *self,basic_block, basic_block, rtx, rtx, bool);


/* Compute hash code of X in mode M.  Special-case case where X is a pseudo
   register (hard registers may require `do_not_record' to be set).  */
static inline unsigned HASH (MtcsCse *self,rtx x, machine_mode mode)
{
  MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
  MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
  MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
  unsigned h = (REG_P (x) && REGNO (x) >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)
      ? (((unsigned) REG << 7) + (unsigned) REG_QTY (REGNO (x)))
      : canon_hash(self,x, mode));
  return (h ^ (h >> HASH_SHIFT)) & HASH_MASK;
}

/* Like HASH, but without side-effects.  */

static inline unsigned SAFE_HASH (MtcsCse *self,rtx x, machine_mode mode)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   switch (GET_CODE (x)){
      case REG:
         ...
         || x == mtcs_rtl_get_hard_frame_pointer_rtx/*!hard_frame_pointer_rtx*/(mtcsRTL))
            return true;
         if (x == mtcs_rtl_get_arg_pointer_rtx/*!arg_pointer_rtx*/(mtcsRTL)
      ...
      case PLUS:
         if (!CONST_INT_P (XEXP (x, 1)))
            return false;
         return fixed_base_plus_p(self,XEXP (x, 0));
...
DEBUG_FUNCTION void mtcs_cse_dump_class (struct table_elt *classp)
{
   struct table_elt *elt;

   fprintf (stderr, "Equivalence chain for ");
   mtcs_print_rtl (stderr, classp->exp);
   fprintf (stderr, ": \n");

   for (elt = classp->first_same_value; elt; elt = elt->next_same_value){
      mtcs_print_rtl (stderr, elt->exp);
      fprintf (stderr, "\n");
   }
}
...
static int approx_reg_cost (MtcsCse *self,const_rtx x)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   FOR_EACH_SUBRTX (iter, array, x, NONCONST){
      ...
      if (REG_P (x)){
         ...
         if (!cheap_regno/*!CHEAP_REGNO*/(self,regno)){
            if (regno < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)){
               if (mtcsTarget/*!targetm.small_register_classes_for_mode_p*/->small_register_classes_for_mode_p(mtcsTarget,GET_MODE (x)))
                  return MAX_COST;
               cost += 2;
            }else
               cost += 1;
...
/* Return a negative value if an rtx A, whose costs are given by COST_A
   ...
   equally good.  */
//不移植
static int preferable (int cost_a, int regcost_a, int cost_b, int regcost_b)
{
...
static int notreg_cost (MtcsCse *self,rtx x, machine_mode mode, enum rtx_code outer, int opno)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);
   ...
   return ((GET_CODE (x) == SUBREG
      ...
      && mtcs_mode_is_int_mode/*!is_int_mode*/(mtcsMode,GET_MODE (SUBREG_REG (x)), &inner_mode)
      && mtcs_mode_get_size/*!GET_MODE_SIZE*/(mtcsMode,int_mode) < mtcs_mode_get_size/*!GET_MODE_SIZE*/(mtcsMode,inner_mode)
      && mtcs_rtl_subreg_lowpart_p/*!subreg_lowpart_p*/(mtcsRTL,x)
      && mtcs_mode_truly_noop_truncation_p/*!TRULY_NOOP_TRUNCATION_MODES_P*/(mtcsMode,int_mode, inner_mode))
      ? 0
      : mtcs_rtlanal_rtx_cost/*!rtx_cost*/(mtcsRtlanal,x, mode, outer, opno, self->optimize_this_for_speed_p) * 2);
}


/* Initialize CSE_REG_INFO_TABLE.  */

static void init_cse_reg_info (MtcsCse *self,unsigned int nregs)
{
   /* Do we need to grow the table?  */
   if (nregs > self->cse_reg_info_table_size){
      unsigned int new_size;

      if (self->cse_reg_info_table_size < 2048){
         /* Compute a new size that is a power of 2 and no smaller
         than the large of NREGS and 64.  */
         new_size = (self->cse_reg_info_table_size  ? self->cse_reg_info_table_size : 64);
      ...
      free (self->cse_reg_info_table);
      self->cse_reg_info_table = XNEWVEC (struct cse_reg_info, new_size);
      self->cse_reg_info_table_size = new_size;
      self->cse_reg_info_table_first_uninitialized = 0;
   }

   /* Do we have all of the first NREGS entries initialized?  */
   if (self->cse_reg_info_table_first_uninitialized < nregs){
      unsigned int old_timestamp = self->cse_reg_info_timestamp - 1;
      unsigned int i;
      ...
      virtual memory.  */
      for (i = self->cse_reg_info_table_first_uninitialized; i < nregs; i++)
         self->cse_reg_info_table[i].timestamp = old_timestamp;

      self->cse_reg_info_table_first_uninitialized = nregs;
   }
}
...
static void get_cse_reg_info_1 (MtcsCse *self,unsigned int regno)
{
  ...
  self->cse_reg_info_table[regno].timestamp = self->cse_reg_info_timestamp;

  /* Initialize the rest of the entry.  */
  self->cse_reg_info_table[regno].reg_tick = 1;
  self->cse_reg_info_table[regno].reg_in_table = -1;
  self->cse_reg_info_table[regno].subreg_ticked = -1;
  self->cse_reg_info_table[regno].reg_qty = -regno - 1;
}

/* Find a cse_reg_info entry for REGNO.  */

static inline struct cse_reg_info *get_cse_reg_info (MtcsCse *self,unsigned int regno)
{
   struct cse_reg_info *p = &self->cse_reg_info_table[regno];

   /* If this entry has not been initialized, go ahead and initialize
   it.  */
   if (p->timestamp != self->cse_reg_info_timestamp)
      get_cse_reg_info_1(self,regno);
...
static void new_basic_block (MtcsCse *self)
{
   ...
   self->cse_reg_info_timestamp++;

   /* Clear out hash table state for this pass.  */
   mtcs_reg_clear_hard_reg_set/*!CLEAR_HARD_REG_SET*/(&self->hard_regs_in_table);
   ...
   for (i = 0; i < HASH_SIZE; i++){
      ...
      if (first != NULL){
         ...
         last->next_same_hash = self->free_element_chain;
         self->free_element_chain = first;
      }
   }
}
...
static void make_new_qty (MtcsCse *self,unsigned int reg, machine_mode mode)
{
  int q;
  struct qty_table_elem *ent;
  struct reg_eqv_elem *eqv;

  gcc_assert (self->next_qty < self->max_qty);

  q = REG_QTY (reg) = self->next_qty++;
  ent = &self->qty_table[q];
  ent->first_reg = reg;
  ent->last_reg = reg;
  ent->mode = mode;
...
static void make_regs_eqv (MtcsCse *self,unsigned int new_reg, unsigned int old_reg)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (! (firstr < firstPseudoRegister/*!FIRST_PSEUDO_REGISTER*/ && fixed_regno_p/*!FIXED_REGNO_P*/(self,firstr))
   /* Certain fixed registers might be of the class NO_REGS.  This means
   that not only can they not be allocated by the compiler, but
   they cannot be used in substitutions or canonicalizations
   either.  */
   && (new_reg >= firstPseudoRegister/*!FIRST_PSEUDO_REGISTER*/ || REGNO_REG_CLASS (new_reg) != NO_REGS)
   && ((new_reg < firstPseudoRegister/*!FIRST_PSEUDO_REGISTER*/ && fixed_regno_p/*!FIXED_REGNO_P*/(self,new_reg))
   || (new_reg >= firstPseudoRegister/*!FIRST_PSEUDO_REGISTER*/
   && (firstr < firstPseudoRegister/*!FIRST_PSEUDO_REGISTER*/
   || (bitmap_bit_p (self->cse_ebb_live_out, new_reg)
   && !bitmap_bit_p (self->cse_ebb_live_out, firstr))
   || (bitmap_bit_p (self->cse_ebb_live_in, new_reg)
   && !bitmap_bit_p (self->cse_ebb_live_in, firstr)))))){
      self->reg_eqv_table[firstr].prev = new_reg;
      self->reg_eqv_table[new_reg].next = firstr;
      self->reg_eqv_table[new_reg].prev = -1;
      ...
      while (lastr < firstPseudoRegister/*!FIRST_PSEUDO_REGISTER*/ && self->reg_eqv_table[lastr].prev >= 0
      && (REGNO_REG_CLASS (lastr) == NO_REGS || ! fixed_regno_p/*!FIXED_REGNO_P*/(self,lastr))
      && new_reg >= firstPseudoRegister/*!FIRST_PSEUDO_REGISTER*/)
         lastr = self->reg_eqv_table[lastr].prev;
      self->reg_eqv_table[new_reg].next = self->reg_eqv_table[lastr].next;
      if (self->reg_eqv_table[lastr].next >= 0)
         self->reg_eqv_table[self->reg_eqv_table[lastr].next].prev = new_reg;
      else
         self->qty_table[q].last_reg = new_reg;
      self->reg_eqv_table[lastr].next = new_reg;
      self->reg_eqv_table[new_reg].prev = lastr;
   }
}

/* Remove REG from its equivalence class.  */
static void delete_reg_equiv (MtcsCse *self,unsigned int reg)
{
   struct qty_table_elem *ent;
   int q = REG_QTY (reg);
   int p, n;
   ...
   p = self->reg_eqv_table[reg].prev;
   n = self->reg_eqv_table[reg].next;

   if (n != -1)
      self->reg_eqv_table[n].prev = p;
   else
      ent->last_reg = p;
   if (p != -1)
      self->reg_eqv_table[p].next = n;
   else
      ent->first_reg = n;
...
/* Remove any invalid expressions from the hash table
   ...
   the hash code of X.  */
static bool mention_regs (MtcsCse *self,rtx x)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   && REGNO (SUBREG_REG (x)) >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)){
      ...
      if (REG_IN_TABLE (i) >= 0 && REG_IN_TABLE (i) != REG_TICK (i)){
         ...
         if (REG_TICK (i) - REG_IN_TABLE (i) > 1 || SUBREG_TICKED (i) != REGNO (SUBREG_REG (x)))
            remove_invalid_refs(self,i);
         else
            remove_invalid_subreg_refs(self,i, SUBREG_BYTE (x), GET_MODE (x));
      }
   ...
   if (code == COMPARE || COMPARISON_P (x)){
      if (REG_P (XEXP (x, 0)) && ! REGNO_QTY_VALID_P (REGNO (XEXP (x, 0))))
         if (insert_regs(self,XEXP (x, 0), NULL, false)){
            rehash_using_reg(self,XEXP (x, 0));
            changed = true;
         }

      if (REG_P (XEXP (x, 1)) && ! REGNO_QTY_VALID_P (REGNO (XEXP (x, 1))))
         if (insert_regs(self,XEXP (x, 1), NULL, false)){
            rehash_using_reg(self,XEXP (x, 1));
            changed = true;
         }
   }
   ...
   for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
      if (fmt[i] == 'e'){
         if (mention_regs(self,XEXP (x, i)))
            changed = true;
      }else if (fmt[i] == 'E')
         for (j = 0; j < XVECLEN (x, i); j++)
            if (mention_regs(self,XVECEXP (x, i, j)))
               changed = true;
...
static bool insert_regs (MtcsCse *self,rtx x, struct table_elt *classp, bool modified)
{
   if (REG_P (x)){
      unsigned int regno = REGNO (x);
      int qty_valid;
      ...
      if (qty_valid){
         struct qty_table_elem *ent = &self->qty_table[REG_QTY (regno)];
      ...
      if (modified || ! qty_valid){
         if (classp)
            for (classp = classp->first_same_value; classp != 0; classp = classp->next_same_value)
               if (REG_P (classp->exp)   && GET_MODE (classp->exp) == GET_MODE (x)){
                  ...
                  if (self->qty_table[REG_QTY (c_regno)].mode != GET_MODE (x))
                     continue;

                  make_regs_eqv(self,regno, c_regno);
                  return true;
               }
         ...
         if (! modified && REG_IN_TABLE (regno) >= 0  && REG_TICK (regno) == REG_IN_TABLE (regno) + 1)
            REG_TICK (regno)++;
         make_new_qty(self,regno, GET_MODE (x));
         return true;
      }
      return false;
   }
   ...
   && ! REGNO_QTY_VALID_P (REGNO (SUBREG_REG (x)))){
      insert_regs(self,SUBREG_REG (x), NULL, false);
      mention_regs(self,x);
      return true;
   }else
      return mention_regs(self,x);
}
...
static bool compute_const_anchors (MtcsCse *self,rtx cst,
             ...
             HOST_WIDE_INT *upper_base, HOST_WIDE_INT *upper_offs)
{
   ...
   *lower_base = n & ~(mtcsTarget/*!targetm.const_anchor*/->const_anchor - 1);
   if ((unsigned HOST_WIDE_INT) *lower_base == n)
      return false;

   *upper_base = ((n + (mtcsTarget/*!targetm.const_anchor*/->const_anchor - 1))
                  & ~(mtcsTarget/*!targetm.const_anchor*/->const_anchor - 1));
...
static void insert_const_anchor (MtcsCse *self,HOST_WIDE_INT anchor, rtx reg, HOST_WIDE_INT offs,
           machine_mode mode)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   anchor_exp = mtcs_rtl_gen_int_mode/*!gen_int_mode*/(mtcsRTL,anchor, mode);
   hash = HASH(self,anchor_exp, mode);
   elt = lookup(self,anchor_exp, hash, mode);
   if (!elt)
      elt = insert(self,anchor_exp, NULL, hash, mode);

   exp = mtcs_rtl_plus_constant/*!plus_constant*/(mtcsRTL,mode, reg, offs);
   /* REG has just been inserted and the hash codes recomputed.  */
   mention_regs(self,exp);
   hash = HASH(self,exp, mode);
   ...
   insert_with_costs(self,exp, elt, hash, mode, COST (reg, mode), 1);
}
...
static void insert_const_anchors (MtcsCse *self,rtx reg, rtx cst, machine_mode mode)
{
   HOST_WIDE_INT lower_base, lower_offs, upper_base, upper_offs;
   if (!compute_const_anchors(self,cst, &lower_base, &lower_offs,&upper_base, &upper_offs))
      return;
   /* Ignore anchors of value 0.  Constants accessible from zero are
   simple.  */
   if (lower_base != 0)
      insert_const_anchor(self,lower_base, reg, -lower_offs, mode);
   if (upper_base != 0)
      insert_const_anchor(self,upper_base, reg, -upper_offs, mode);
}
...
static rtx find_reg_offset_for_const (MtcsCse *self,struct table_elt *anchor_elt, HOST_WIDE_INT offs,
            unsigned *old)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   elt; elt = elt->next_same_value, idx++){
      ...
      && GET_CODE (XEXP (elt->exp, 1)) == CONST_INT)){
         ...
         if (!REG_P (elt->exp) && !mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,elt->exp, elt->exp, 1, false))
            continue;

         x = mtcs_rtl_plus_constant/*!plus_constant*/(mtcsRTL,GET_MODE (elt->exp), elt->exp, offs);
         if (REG_P (x) || (GET_CODE (x) == PLUS && IN_RANGE (INTVAL (XEXP (x, 1)),
         -mtcsTarget/*!targetm.const_anchor*/->const_anchor,
         mtcsTarget/*!targetm.const_anchor*/->const_anchor - 1))){
            match = x;
            match_elt = elt;
            *old = idx;
...
static rtx try_const_anchors (MtcsCse *self,rtx src_const, machine_mode mode)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);
   ...
   if (!compute_const_anchors(self,src_const, &lower_base, &lower_offs,&upper_base, &upper_offs))
      return NULL_RTX;

   lower_anchor_rtx = mtcs_rtl_GEN_INT/*!GEN_INT*/(mtcsRTL,lower_base);
   upper_anchor_rtx = mtcs_rtl_GEN_INT/*!GEN_INT*/(mtcsRTL,upper_base);
   lower_elt = lookup(self,lower_anchor_rtx, HASH(self,lower_anchor_rtx, mode), mode);
   upper_elt = lookup(self,upper_anchor_rtx, HASH(self,upper_anchor_rtx, mode), mode);

   if (lower_elt)
      lower_exp = find_reg_offset_for_const(self,lower_elt, lower_offs, &lower_old);
   if (upper_elt)
      upper_exp = find_reg_offset_for_const(self,upper_elt, upper_offs, &upper_old);
...
static void remove_from_table (MtcsCse *self,struct table_elt *elt, unsigned int hash)
{
   if (elt == 0)
      return;
   ...
   /* Remove the table element from its hash bucket.  */
   {
      ...
      else if (self->table[hash] == elt)
         self->table[hash] = next;
      else{
   ...
   elt->next_same_hash = self->free_element_chain;
   self->free_element_chain = elt;
}

/* Same as above, but X is a pseudo-register.  */

static void remove_pseudo_from_table (MtcsCse *self,rtx x, unsigned int hash)
{
   struct table_elt *elt;

   /* Because a pseudo-register can be referenced in more than one
   mode, we might have to remove more than one table entry.  */
   while ((elt = lookup_for_remove(self,x, hash, VOIDmode)))
      remove_from_table(self,elt, hash);
}
...
static struct table_elt * lookup (MtcsCse *self,rtx x, unsigned int hash, machine_mode mode)
{
  struct table_elt *p;
  for (p = self->table[hash]; p; p = p->next_same_hash)
    if (mode == p->mode && ((x == p->exp && REG_P (x))
          || mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,x, p->exp, !REG_P (x), false)))
      return p;
...
static struct table_elt * lookup_for_remove (MtcsCse *self,rtx x, unsigned int hash, machine_mode mode)
{
   struct table_elt *p;
   ...
   if (REG_P (x)){
      ...
      for (p = self->table[hash]; p; p = p->next_same_hash)
         if (REG_P (p->exp) && REGNO (p->exp) == regno)
            return p;
   }else{
      for (p = self->table[hash]; p; p = p->next_same_hash)
         if (mode == p->mode  && (x == p->exp || mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,x, p->exp, 0, false)))
            return p;
   }
...
static rtx lookup_as_function (MtcsCse *self,rtx x, enum rtx_code code)
{
   struct table_elt *p= lookup(self,x, SAFE_HASH(self,x, VOIDmode), GET_MODE (x));
   if (p == 0)
      return 0;
   for (p = p->first_same_value; p; p = p->next_same_value)
      if (GET_CODE (p->exp) == code
      /* Make sure this is a valid entry in the table.  */
      && mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,p->exp, p->exp, 1, false))
         return p->exp;
...
static struct table_elt *insert_with_costs (MtcsCse *self,rtx x, struct table_elt *classp, unsigned int hash,
         machine_mode mode, int cost, int reg_cost)
{
   ...
   if (REG_P (x) && REGNO (x) < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))
      mtcs_reg_add_to_hard_reg_set/*!add_to_hard_reg_set*/(mtcsReg,&self->hard_regs_in_table, GET_MODE (x), REGNO (x));

   /* Put an element for X into the right hash bucket.  */

   elt = self->free_element_chain;
   if (elt)
      self->free_element_chain = elt->next_same_hash;
   else
   ...
   elt->next_same_hash = self->table[hash];
   elt->prev_same_hash = 0;
   elt->related_value = 0;
   elt->in_memory = 0;
   elt->mode = mode;
   elt->is_const = (CONSTANT_P (x) || fixed_base_plus_p(self,x));

   if (self->table[hash])
      self->table[hash]->prev_same_hash = elt;
   self->table[hash] = elt;

   /* Put it into the proper value-class.  */
   if (classp){
      classp = classp->first_same_value;
      if (CHEAPER (elt, classp)) /* Insert at the head of the class.  */{
         struct table_elt *p;
   ...
   if (elt->is_const && classp && REG_P (classp->exp) && !REG_P (x)){
      int exp_q = REG_QTY (REGNO (classp->exp));
      struct qty_table_elem *exp_ent = &self->qty_table[exp_q];

      exp_ent->const_rtx = gen_lowpart (exp_ent->mode, x);
      exp_ent->const_insn = self->this_insn;
   }else if (REG_P (x)
   && classp
   && ! self->qty_table[REG_QTY (REGNO (x))].const_rtx
   && ! elt->is_const){
      struct table_elt *p;
      ...
      for (p = classp; p != 0; p = p->next_same_value){
         if (p->is_const && !REG_P (p->exp)){
            ...
            struct qty_table_elem *x_ent = &self->qty_table[x_q];

            x_ent->const_rtx  = gen_lowpart (GET_MODE (x), p->exp);
            x_ent->const_insn = self->this_insn;
            break;
         }
      }
   }else if (REG_P (x)
   && self->qty_table[REG_QTY (REGNO (x))].const_rtx
   && GET_MODE (x) == self->qty_table[REG_QTY (REGNO (x))].mode)
      self->qty_table[REG_QTY (REGNO (x))].const_insn = self->this_insn;
   ...
   if (GET_CODE (x) == CONST){
      ...
      if (subexp != 0){
         ...
         subhash = SAFE_HASH(self,subexp, mode);
         subelt = lookup(self,subexp, subhash, mode);
         if (subelt == 0)
            subelt = insert(self,subexp, NULL, subhash, mode);
...
static struct table_elt *insert (MtcsCse *self,rtx x, struct table_elt *classp,
      unsigned int hash,machine_mode mode)
{
  return insert_with_costs(self,x, classp, hash, mode, COST (x, mode), approx_reg_cost(self,x));
}
...
static void merge_equiv_classes (MtcsCse *self,struct table_elt *class1, struct table_elt *class2)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   for (elt = class2; elt; elt = next){
      ...
      if (REG_P (exp) || mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,exp, exp, 1, false)){
         bool need_rehash = false;

         self->hash_arg_in_memory = 0;
         hash = HASH(self,exp, mode);

         if (REG_P (exp)){
            need_rehash = REGNO_QTY_VALID_P (REGNO (exp));
            delete_reg_equiv(self,REGNO (exp));
         }

         if (REG_P (exp) && REGNO (exp) >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))
            remove_pseudo_from_table(self,exp, hash);
         else
            remove_from_table(self,elt, hash);

         if (insert_regs(self,exp, class1, false) || need_rehash){
            rehash_using_reg(self,exp);
            hash = HASH(self,exp, mode);
         }
         new_elt = insert(self,exp, class1, hash, mode);
         new_elt->in_memory = self->hash_arg_in_memory;
...
static void flush_hash_table (MtcsCse *self)
{
   ...
   for (i = 0; i < HASH_SIZE; i++)
      for (p = self->table[i]; p; p = self->table[i]){
         /* Note that invalidate can remove elements
         after P in the current hash chain.  */
         if (REG_P (p->exp))
            invalidate(self,p->exp, VOIDmode);
         else
            remove_from_table(self,p, i);
      }
}
...
static bool check_dependence (MtcsCse *self,const_rtx x, rtx exp, machine_mode mode, rtx addr)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   FOR_EACH_SUBRTX (iter, array, x, NONCONST){
      const_rtx x = *iter;
      if (MEM_P (x) && mtcs_alias_canon_anti_dependence/*!canon_anti_dependence*/(mtcsAlias,x, true, exp, mode, addr))
         return true;
   }
   return false;
}
...
static void invalidate_reg (MtcsCse *self,rtx x)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   unsigned int regno = REGNO (x);
   unsigned int hash = HASH(self,x, GET_MODE (x));
   ...
   delete_reg_equiv(self,regno);
   REG_TICK (regno)++;
   SUBREG_TICKED (regno) = -1;

   if (regno >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))
      remove_pseudo_from_table(self,x, hash);
   else{
      HOST_WIDE_INT in_table = mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&self->hard_regs_in_table, regno);
      unsigned int endregno = END_REGNO (x);
      unsigned int rn;
      struct table_elt *p, *next;

      mtcs_reg_clear_hard_reg_bit/*!CLEAR_HARD_REG_BIT*/(mtcsReg,&self->hard_regs_in_table, regno);

      for (rn = regno + 1; rn < endregno; rn++){
         in_table |= mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&self->hard_regs_in_table, rn);
         mtcs_reg_clear_hard_reg_bit/*!CLEAR_HARD_REG_BIT*/(mtcsReg,&self->hard_regs_in_table, rn);
         delete_reg_equiv(self,rn);
         REG_TICK (rn)++;
         SUBREG_TICKED (rn) = -1;
      }
      ...
      if (in_table)
         for (hash = 0; hash < HASH_SIZE; hash++)
            for (p = self->table[hash]; p; p = next){
               next = p->next_same_hash;

               if (!REG_P (p->exp) || REGNO (p->exp) >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))
                  continue;

               unsigned int tregno = REGNO (p->exp);
               unsigned int tendregno = END_REGNO (p->exp);
               if (tendregno > regno && tregno < endregno)
                  remove_from_table(self,p, hash);
            }
   }
}
...
static void invalidate (MtcsCse *self,rtx x, machine_mode full_mode)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsAlias *mtcsAlias=mtcs_target_get_alias(mtcsTarget);
   ...
   switch (GET_CODE (x)){
      case REG:
         invalidate_reg(self,x);
         return;

      case SUBREG:
         invalidate(self,SUBREG_REG (x), VOIDmode);
         return;

      case PARALLEL:
         for (i = XVECLEN (x, 0) - 1; i >= 0; --i)
            invalidate(self,XVECEXP (x, 0, i), VOIDmode);
         return;

      case EXPR_LIST:
         /* This is part of a disjoint return value; extract the location in
         question ignoring the offset.  */
         invalidate(self,XEXP (x, 0), VOIDmode);
         return;

      case MEM:
         addr = mtcs_alias_canon_rtx/*!canon_rtx*/(mtcsAlias,mtcs_alias_get_addr/*!get_addr*/(mtcsAlias,XEXP (x, 0)));
         /* Calculate the canonical version of X here so that
         true_dependence doesn't generate new RTL for X on each call.  */
         x = mtcs_alias_canon_rtx/*!canon_rtx*/(mtcsAlias,x);
         ...
         for (i = 0; i < HASH_SIZE; i++){
            ...
            for (p = self->table[i]; p; p = next){
               ...
               if (p->in_memory){
                  ...
                  expression again.  */
                  if (!p->canon_exp)
                     p->canon_exp = mtcs_alias_canon_rtx/*!canon_rtx*/(mtcsAlias,p->exp);
                  if (check_dependence(self,p->canon_exp, x, full_mode, addr))
                     remove_from_table(self,p, i);
               }
            }
         }
...
static void invalidate_dest (MtcsCse *self,rtx dest)
{
   if (REG_P (dest) || GET_CODE (dest) == SUBREG || MEM_P (dest))
      invalidate(self,dest, VOIDmode);
   else if (GET_CODE (dest) == STRICT_LOW_PART  || GET_CODE (dest) == ZERO_EXTRACT)
      invalidate(self,XEXP (dest, 0), GET_MODE (dest));
}
...
static void remove_invalid_refs (MtcsCse *self,unsigned int regno)
{
   unsigned int i;
   struct table_elt *p, *next;
...
/* Likewise for a subreg with subreg_reg REGNO, subreg_byte OFFSET,
   and mode MODE.  */
static void remove_invalid_subreg_refs (MtcsCse *self,unsigned int regno,
      poly_uint64 offset,machine_mode mode)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   ...
   for (i = 0; i < HASH_SIZE; i++)
      for (p = self->table[i]; p; p = next){
         ...
         mtcs_mode_get_size/*!GET_MODE_SIZE*/(mtcsMode,GET_MODE (exp)),
         offset, mtcs_mode_get_size/*!GET_MODE_SIZE*/(mtcsMode,mode)))
...
static void rehash_using_reg (MtcsCse *self,rtx x)
{
   unsigned int i;
   struct table_elt *p, *next;
   unsigned hash;
   ...
   for (i = 0; i < HASH_SIZE; i++)
      for (p = self->table[i]; p; p = next){
         ...
         && mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,p->exp, p->exp, 1, false)
         && i != (hash = SAFE_HASH(self,p->exp, p->mode))){
            if (p->next_same_hash)
            ...
            if (p->prev_same_hash)
               ...
               self->table[i] = p->next_same_hash;

            p->next_same_hash = self->table[hash];
            p->prev_same_hash = 0;
            if (self->table[hash])
               self->table[hash]->prev_same_hash = p;
            self->table[hash] = p;
         }
      }
}
...
static void invalidate_for_call (MtcsCse *self,rtx_insn *insn)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   MTCS_EXECUTE_IF_SET_IN_HARD_REG_SET/*!EXECUTE_IF_SET_IN_HARD_REG_SET*/(mtcsReg,&callee_clobbers, 0, regno, hrsi){
      ...
      in_table |= (mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&self->hard_regs_in_table, regno) != 0);
   }
      ...
      for (hash = 0; hash < HASH_SIZE; hash++)
         for (p = self->table[hash]; p; p = next){
            ...
            if (mtcs_reg_overlaps_hard_reg_set_p/*!overlaps_hard_reg_set_p*/(mtcsReg,
                  &callee_clobbers, GET_MODE (p->exp),REGNO (p->exp)))
               remove_from_table(self,p, hash);
         }
}
...
static rtx use_related_value (MtcsCse *self,rtx x, struct table_elt *elt)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);
   ...
   if (elt != 0 && elt->related_value != 0)
      ...
      rtx subexp = get_related_value (x);
      if (subexp != 0)
         relt = lookup(self,subexp,SAFE_HASH(self,subexp, GET_MODE (subexp)),GET_MODE (subexp));
   }
   if (relt == 0)
      return 0;
   ...
   return mtcs_rtl_plus_constant/*!plus_constant*/(mtcsRTL,q->mode, q->exp, offset);
}


/* Hash a string.  Just add its bytes up.  */
//不移植
static inline unsigned hash_rtx_string (const char *ps)
{
  unsigned hash = 0;
...
repeat:
   ...
   if (cb != NULL  && ((*cb) (x, mode, &newx, &newmode))){
      hash += mtcs_cse_hash_rtx(self,newx, newmode, do_not_record_p,
   ...
   switch (code){
      case REG:
      {
         ...
         if (do_not_record_p && !reload_completed){
            ...
            if (regno >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))
               record = true;
            else if (x == mtcs_rtl_get_frame_pointer_rtx/*!frame_pointer_rtx*/(mtcsRTL)
            || x == mtcs_rtl_get_hard_frame_pointer_rtx/*!hard_frame_pointer_rtx*/(mtcsRTL)
            ...
            || x == mtcs_rtl_get_pic_offset_table_rtx/*!pic_offset_table_rtx*/(mtcsRTL))
               record = true;
            else if ( mtcsReg->global_regs/*!global_regs*/[regno])
               record = false;
            else if (mtcsReg->hardRegs.x_fixed_regs/*!fixed_regs*/[regno])
               record = true;
            else if (mtcs_mode_get_class/*!GET_MODE_CLASS*/(mtcsMode,GET_MODE (x)) == MODE_CC)
               record = true;
            else if (mtcsTarget/*!targetm.small_register_classes_for_mode_p*/->small_register_classes_for_mode_p(mtcsTarget,GET_MODE (x)))
               record = false;
            else if (mtcsTarget/*!targetm.class_likely_spilled_p*/->class_likely_spilled_p(mtcsTarget,REGNO_REG_CLASS (regno)))
               record = false;
            else
               record = true;
      ...
      case SUBREG:
      {
         if (REG_P (SUBREG_REG (x))){
            hash += (((unsigned int) SUBREG << 7) + REGNO (SUBREG_REG (x)) +
                  (constant_lower_bound (SUBREG_BYTE (x))/ UNITS_PER_WORD));
            return hash;
         }
         break;
      }
      ...
      case CONST_INT:
         hash += (((unsigned int) CONST_INT << 7) + (unsigned int) mode + (unsigned int) INTVAL (x));
         return hash;
      ...
      case CONST_DOUBLE:
         ...
         if (TARGET_SUPPORTS_WIDE_INT == 0 && GET_MODE (x) == VOIDmode)
            hash += ((unsigned int) CONST_DOUBLE_LOW (x) + (unsigned int) CONST_DOUBLE_HIGH (x));
         else
      ...
      case CONST_VECTOR:
      {
         ...
         for (i = 0; i < units; ++i){
            ...
            hash += mtcs_cse_hash_rtx(self,elt, GET_MODE (elt), do_not_record_p, hash_arg_in_memory_p, have_reg_qty, cb);
         }
      ...
      case MEM:
         ...
         if (do_not_record_p && (MEM_VOLATILE_P (x) || GET_MODE (x) == mtcsMode->modes.M_BLKmode)){
            *do_not_record_p = 1;
            return 0;
         }
      ...
      case USE:
         ...
         if (MEM_P (XEXP (x, 0))  && ! MEM_VOLATILE_P (XEXP (x, 0))){
            hash += (unsigned) USE;
            x = XEXP (x, 0);
            ...
            *do_not_record_p = 1;
            ...
            if (ASM_OPERANDS_INPUT_LENGTH (x)){
               for (i = 1; i < ASM_OPERANDS_INPUT_LENGTH (x); i++){
                  hash += (mtcs_cse_hash_rtx(self,ASM_OPERANDS_INPUT (x, i), GET_MODE (ASM_OPERANDS_INPUT (x, i)),
                        do_not_record_p, hash_arg_in_memory_p,have_reg_qty, cb) +
                        hash_rtx_string (ASM_OPERANDS_INPUT_CONSTRAINT (x, i)));
               }
   ...
   for (; i >= 0; i--){
      switch (fmt[i]){
         case 'e':
            ...
            hash += mtcs_cse_hash_rtx(self,XEXP (x, i), VOIDmode, do_not_record_p,
            hash_arg_in_memory_p,have_reg_qty, cb);
            break;

         case 'E':
            for (j = 0; j < XVECLEN (x, i); j++)
               hash += mtcs_cse_hash_rtx(self,XVECEXP (x, i, j), VOIDmode, do_not_record_p, hash_arg_in_memory_p,have_reg_qty, cb);
            break;
...
static inline unsigned canon_hash (MtcsCse *self,rtx x, machine_mode mode)
{
  return mtcs_cse_hash_rtx(self,x, mode, &self->do_not_record, &self->hash_arg_in_memory, true);
}
...
static inline unsigned safe_hash (MtcsCse *self,rtx x, machine_mode mode)
{
  ...
  return mtcs_cse_hash_rtx(self,x, mode, &dummy_do_not_record, NULL, true);
}
...
bool mtcs_cse_exp_equiv_p (MtcsCse *self,const_rtx x, const_rtx y, int validate, bool for_gcse)
{
   ...
   if (code == MEM && mtcs_rtl_get_mem_addr_space/*!MEM_ADDR_SPACE*/(mtcsRTL,x)
         != mtcs_rtl_get_mem_addr_space/*!MEM_ADDR_SPACE*/(mtcsRTL,y))
      return false;
   ...
   switch (code){
      ...
      case MEM:
         if (for_gcse){
            ...
            if (cfun->can_throw_non_call_exceptions && (MEM_NOTRAP_P (x) != MEM_NOTRAP_P (y)))
               return false;
         }
         break;
      ...
      case NE:
      case EQ:
         return ((mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,XEXP (x, 0), XEXP (y, 0),validate, for_gcse)
               && mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,XEXP (x, 1), XEXP (y, 1),validate, for_gcse))
         || (mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,XEXP (x, 0), XEXP (y, 1), validate, for_gcse)
               && mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,XEXP (x, 1), XEXP (y, 0), validate, for_gcse)));
      ...
      case ASM_OPERANDS:
         ...
         if (ASM_OPERANDS_INPUT_LENGTH (x)){
            for (i = ASM_OPERANDS_INPUT_LENGTH (x) - 1; i >= 0; i--)
               if (! mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,
                     ASM_OPERANDS_INPUT (x, i), ASM_OPERANDS_INPUT (y, i), validate, for_gcse)
                     || strcmp (ASM_OPERANDS_INPUT_CONSTRAINT (x, i), ASM_OPERANDS_INPUT_CONSTRAINT (y, i)))
                  return false;
         }
   ...
   for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--){
      switch (fmt[i]){
         case 'e':
            if (! mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,XEXP (x, i), XEXP (y, i),validate, for_gcse))
               return false;
            break;
         ...
         case 'E':
            if (XVECLEN (x, i) != XVECLEN (y, i))
               return 0;
            for (j = 0; j < XVECLEN (x, i); j++)
               if (! mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,XVECEXP (x, i, j), XVECEXP (y, i, j),validate, for_gcse))
                  return false;
            break;
...
static void validate_canon_reg (MtcsCse *self,rtx *xloc, rtx_insn *insn)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   if (*xloc){
      rtx new_rtx = canon_reg(self,*xloc, insn);

      /* If replacing pseudo with hard reg or vice versa, ensure the
      insn remains valid.  Likewise if the insn has MATCH_DUPs.  */
      gcc_assert (insn && new_rtx);
      mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, xloc, new_rtx, 1);
   }
}
...
static rtx canon_reg (MtcsCse *self,rtx x, rtx_insn *insn)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   switch (code){
      ...
      case REG:
      {
         ...
         ent = &self->qty_table[q];
         first = ent->first_reg;
         return (first >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)
               ? mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[first]
               : REGNO_REG_CLASS (first) == NO_REGS ? x : mtcs_rtl_gen_rtx_REG/*!gen_rtx_REG*/(mtcsRTL,ent->mode, first));
      }
   ...
   for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--){
      ...
      if (fmt[i] == 'e')
         validate_canon_reg(self,&XEXP (x, i), insn);
      else if (fmt[i] == 'E')
         for (j = 0; j < XVECLEN (x, i); j++)
            validate_canon_reg(self,&XVECEXP (x, i, j), insn);
   }
...
static enum rtx_code find_comparison_args (MtcsCse *self,enum rtx_code code, rtx *parg1, rtx *parg2,
            machine_mode *pmode1, machine_mode *pmode2)
{
   ...
   while (arg2 == CONST0_RTX (GET_MODE (arg1))){
      ...
      STORE_FLAG_VALUE, get the inner arguments.  */
         ...
         if(mtcs_config_ifdef(mtcsConfig,MTCS_FLOAT_STORE_FLAG_VALUE))
            cond = mtcs_mode_is_scalar_float_p(mtcsMode,GET_MODE (arg1))
               && (fsfv=mtcs_real_float_store_flag_value(mtcsReal,GET_MODE (arg1)),REAL_VALUE_NEGATIVE (fsfv));

         if (code == NE || (mtcs_mode_get_class/*!GET_MODE_CLASS*/(mtcsMode,GET_MODE (arg1)) == MODE_INT
         && code == LT && mtcs_real_get_store_flag_value/*!STORE_FLAG_VALUE*/(mtcsReal) == -1) || cond)
            //#ifdef FLOAT_STORE_FLAG_VALUE
            ...
            //         )
            x = arg1;
         else if (code == EQ || (mtcs_mode_get_class/*!GET_MODE_CLASS*/(mtcsMode,GET_MODE (arg1)) == MODE_INT
         && code == GE && mtcs_real_get_store_flag_value/*!STORE_FLAG_VALUE*/(mtcsReal) == -1) || cond)
            //#ifdef FLOAT_STORE_FLAG_VALUE
            //         || (SCALAR_FLOAT_MODE_P (GET_MODE (arg1))
            //             && (fsfv = FLOAT_STORE_FLAG_VALUE (GET_MODE (arg1)),
            //            REAL_VALUE_NEGATIVE (fsfv)))
      ...
      if (x == 0)
         ...
         p = lookup(self,arg1, SAFE_HASH(self,arg1, GET_MODE (arg1)), GET_MODE (arg1));
      if (p){
      ...
      for (; p; p = p->next_same_value){
         ...
         if (! mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,p->exp, p->exp, 1, false))
            continue;
         ...
         bool cond2=false;
         if(mtcs_config_ifdef(mtcsConfig,MTCS_FLOAT_STORE_FLAG_VALUE)){
            cond1= code == LT && mtcs_mode_is_scalar_float_p(mtcsMode,inner_mode)
               && (fsfv=mtcs_real_float_store_flag_value(mtcsReal,GET_MODE (arg1)),REAL_VALUE_NEGATIVE (fsfv));
            cond2= code == GE && mtcs_mode_is_scalar_float_p(mtcsMode,inner_mode)
               && (fsfv=mtcs_real_float_store_flag_value(mtcsReal,GET_MODE (arg1)),REAL_VALUE_NEGATIVE (fsfv));
         }
         ...
         || ((code == NE  || (code == LT  && mtcs_simplify_rtx_val_signbit_known_set_p/*!val_signbit_known_set_p*/(mtcsSimplifyRtx,
               inner_mode, mtcs_real_get_store_flag_value/*!STORE_FLAG_VALUE*/(mtcsReal))) || cond1)
         //#ifdef FLOAT_STORE_FLAG_VALUE
         //         || (code == LT
         ...
         && COMPARISON_P (p->exp))){
            x = p->exp;
            break;
         }else if ((code == EQ  || (code == GE  && mtcs_simplify_rtx_val_signbit_known_set_p/*!val_signbit_known_set_p*/(mtcsSimplifyRtx,
               inner_mode,mtcs_real_get_store_flag_value/*!STORE_FLAG_VALUE*/(mtcsReal))) || cond2)
         //#ifdef FLOAT_STORE_FLAG_VALUE
         //          || (code == GE
         //         && SCALAR_FLOAT_MODE_P (inner_mode)
         //         && (fsfv = FLOAT_STORE_FLAG_VALUE (GET_MODE (arg1)),
      ...
      if (reverse_code){
         enum rtx_code reversed = mtcs_dojump_reversed_comparison_code/*!reversed_comparison_code*/(mtcsDojump,x, NULL);
         if (reversed == UNKNOWN)
            break;
         else
            code = reversed;
   ...
   *parg1 = fold_rtx(self,arg1, 0), *parg2 = fold_rtx(self,arg2, 0);
...
static rtx fold_rtx (MtcsCse *self,rtx x, rtx_insn *insn)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);
   ...
   switch (code){
      ...
      case ZERO_EXTRACT:
      case SIGN_EXTRACT:
         if ((new_rtx = equiv_constant(self,x)) != NULL_RTX)
            return new_rtx;
         return x;
      ...
      case ASM_OPERANDS:
         if (insn){
            for (i = ASM_OPERANDS_INPUT_LENGTH (x) - 1; i >= 0; i--)
               mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &ASM_OPERANDS_INPUT (x, i),
                     fold_rtx(self,ASM_OPERANDS_INPUT (x, i), insn), 0);
         }
         return x;
   ...
   for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
      if (fmt[i] == 'e'){
         ...
         switch (GET_CODE (folded_arg)){
            ...
            case REG:
            case SUBREG:
               const_arg = equiv_constant(self,folded_arg);
               break;
            ...
            default:
               folded_arg = fold_rtx(self,folded_arg, insn);
               const_arg = equiv_constant(self,folded_arg);
               break;
         }
         ...
         && (COST_IN (const_arg, mode_arg, code, i) <= COST_IN (folded_arg, mode_arg, code, i))
         /* It's not safe to substitute the operand of a conversion
         operator with a constant, as the conversion's identity
         depends upon the mode of its operand.  This optimization
         is handled by the call to simplify_unary_operation.  */
         && (GET_RTX_CLASS (code) != RTX_UNARY || GET_MODE (const_arg) == mode_arg0 || (code != ZERO_EXTEND
         && code != SIGN_EXTEND
         && code != TRUNCATE
         && code != FLOAT_TRUNCATE
         && code != FLOAT_EXTEND
         ...
         if (insn == NULL_RTX && !changed)
            x = copy_rtx (x);
         changed = true;
         mtcs_recog_validate_unshare_change/*!validate_unshare_change*/(mtcsRecog,insn, &XEXP (x, i), folded_arg, 1);
      }//end       if (fmt[i] == 'e'){
   ...
   switch (GET_RTX_CLASS (code)){
      case RTX_UNARY:
      {
         ...
         new_rtx = mtcs_simplify_rtx_unary_operation/*!simplify_unary_operation*/(mtcsSimplifyRtx,code, mode,
               const_arg0 ? const_arg0 : folded_arg0,mode_arg0);
      }
         break;
      ...
      case RTX_COMM_COMPARE:
         ...
         if (const_arg0 == 0 || const_arg1 == 0){
            ...
            if (mtcs_mode_is_scalar_float_p/*!SCALAR_FLOAT_MODE_P*/(mtcsMode,mode)){
               true_rtx = NULL_RTX;
               if(mtcs_config_ifdef(mtcsConfig,MTCS_FLOAT_STORE_FLAG_VALUE))
                  true_rtx = (mtcs_rtl_const_double_from_real_value/*!const_double_from_real_value*/(mtcsRTL,
                        mtcs_real_float_store_flag_value/*!FLOAT_STORE_FLAG_VALUE*/(mtcsReal,mode), mode));
               /*!
               #ifdef FLOAT_STORE_FLAG_VALUE
            ...
            code = find_comparison_args(self,code, &folded_arg0, &folded_arg1,&mode_arg0, &mode_arg1);
            /* If the mode is VOIDmode or a MODE_CC mode, we don't know
            what kinds of things are being compared, so we can't do
            anything with this comparison.  */
            if (mode_arg0 == VOIDmode || mtcs_mode_get_class/*!GET_MODE_CLASS*/(mtcsMode,mode_arg0) == MODE_CC)
               break;
            const_arg0 = equiv_constant(self,folded_arg0);
            const_arg1 = equiv_constant(self,folded_arg1);
            ...
            if (const_arg0 == 0 || const_arg1 == 0){
               if (const_arg1 != NULL){
                  ...
                  p = lookup(self,folded_arg0, SAFE_HASH(self,folded_arg0, mode_arg0),mode_arg0);
                  ...
                  if (p != NULL){
                     ...
                     for (p = p->first_same_value; p != NULL; p = p->next_same_value){
                        ...
                        simp_result = mtcs_simplify_rtx_relational_operation/*!simplify_relational_operation*/(mtcsSimplifyRtx,
                              code, mode,mode_arg0,p->exp,const_arg1);
                     ...
                     if (cheapest_simplification != x)
                        return fold_rtx(self,copy_rtx (cheapest_simplification),insn);
                  }//end  if (p != NULL){
               }//end  if (const_arg1 != NULL){
               ...
               || ((p0 = lookup(self,folded_arg0, SAFE_HASH(self,folded_arg0, mode_arg0),mode_arg0))
               && (p1 = lookup(self,folded_arg1,SAFE_HASH(self,folded_arg1, mode_arg0), mode_arg0))
               ...
               (we only check the reverse if not floating-point).  */
                  ...
                  if (REGNO_QTY_VALID_P (REGNO (folded_arg0))){
                     struct qty_table_elem *ent = &self->qty_table[qty];

                     if ((comparison_dominates_p (ent->comparison_code, code)
                           || (! mtcs_mode_is_float_p/*!FLOAT_MODE_P*/(mtcsMode,mode_arg0)
                     && comparison_dominates_p (ent->comparison_code, reverse_condition (code))))
                     && (rtx_equal_p (ent->comparison_const, folded_arg1)
                     || (const_arg1  && rtx_equal_p (ent->comparison_const, const_arg1))
                     || (REG_P (folded_arg1)  && (REG_QTY (REGNO (folded_arg1)) == ent->comparison_qty)))){
                        if (comparison_dominates_p (ent->comparison_code, code)){
                           if (true_rtx)
                              return true_rtx;
                           else
            ...
            }//end   if (const_arg0 == 0 || const_arg1 == 0){
         }//end    if (const_arg0 == 0 || const_arg1 == 0){
         ...
         if (const_arg1 == const0_rtx && !const_arg0){
            rtx y = lookup_as_function(self,folded_arg0, IOR);
            rtx inner_const;

            if (y != 0  && (inner_const = equiv_constant(self,XEXP (y, 1))) != 0
            && CONST_INT_P (inner_const)  && INTVAL (inner_const) != 0)
         ...
         switch (code){
            case PLUS:
               ...
               if (const_arg1 && GET_CODE (const_arg1) == LABEL_REF){
                  rtx y = GET_CODE (folded_arg0) == MINUS ? folded_arg0 : lookup_as_function(self,folded_arg0, MINUS);
                  ...
                  if ((y = (GET_CODE (folded_arg0) == CONST ? folded_arg0: lookup_as_function(self,folded_arg0, CONST))) != 0
               ...
               if (const_arg0 && GET_CODE (const_arg0) == LABEL_REF){
                  rtx y = GET_CODE (folded_arg1) == MINUS ? folded_arg1 : lookup_as_function(self,folded_arg1, MINUS);

                  if (y != 0 && GET_CODE (XEXP (y, 1)) == LABEL_REF   && label_ref_label (XEXP (y, 1)) == label_ref_label (const_arg0))
                     return XEXP (y, 0);

                  /* Now try for a CONST of a MINUS like the above.  */
                  if ((y = (GET_CODE (folded_arg1) == CONST ? folded_arg1 : lookup_as_function(self,folded_arg1, CONST))) != 0
                  && GET_CODE (XEXP (y, 0)) == MINUS
                  && GET_CODE (XEXP (XEXP (y, 0), 1)) == LABEL_REF
               ...
               if (const_arg1 != 0 && CONST_INT_P (const_arg1) && INTVAL (const_arg1) < 0
               /* This used to test
               ...
               && INTVAL (const_arg1) !=  (HOST_WIDE_INT_1 << (HOST_BITS_PER_WIDE_INT - 1))
               && REG_P (folded_arg1)){
                  rtx new_const = mtcs_rtl_GEN_INT/*!GEN_INT*/(mtcsRTL,-INTVAL (const_arg1));
                  struct table_elt *p = lookup(self,new_const, SAFE_HASH(self,new_const, mode), mode);

                  if (p)
                     for (p = p->first_same_value; p; p = p->next_same_value)
                        if (REG_P (p->exp))
                           return mtcs_simplify_rtx_gen_binary/*!simplify_gen_binary*/(mtcsSimplifyRtx,
                                 MINUS, mode, folded_arg0,canon_reg(self,p->exp, NULL));
               }
               goto from_plus;
            ...
            case MINUS:
               ...
               if (const_arg1 != 0 && poly_int_rtx_p (const_arg1, &xval)){
                  rtx y = lookup_as_function(self,XEXP (x, 0), PLUS);
                  if (y && poly_int_rtx_p (XEXP (y, 1)))
                     return fold_rtx(self,mtcs_rtl_plus_constant/*!plus_constant*/(mtcsRTL,mode, copy_rtx (y), -xval),NULL);
               }
...
from_plus:
            ...
            case ASHIFT:  case LSHIFTRT:  case ASHIFTRT:
               ...
               if (REG_P (folded_arg0) && const_arg1 && CONST_INT_P (const_arg1)){
                  int is_shift = (code == ASHIFT || code == ASHIFTRT || code == LSHIFTRT);
                  rtx y, inner_const, new_const;
                  rtx canon_const_arg1 = const_arg1;
                  enum rtx_code associate_code;

                  if (is_shift && (INTVAL (const_arg1) >= mtcs_mode_get_unit_precision/*!GET_MODE_UNIT_PRECISION*/(mtcsMode,mode)
                        || INTVAL (const_arg1) < 0)){
                     if (SHIFT_COUNT_TRUNCATED)
                        canon_const_arg1 = mtcs_rtl_gen_int_shift_amount/*!gen_int_shift_amount*/(mtcsRTL,mode, (INTVAL (const_arg1) &
                              (mtcs_mode_get_unit_bitsize/*!GET_MODE_UNIT_BITSIZE*/(mtcsMode,mode) - 1)));
                     else
                        break;
                  }

                  y = lookup_as_function(self,folded_arg0, code);
                  if (y == 0)
                     break;
                  ...
                  inner_const = equiv_constant(self,fold_rtx(self,XEXP (y, 1), 0));
                  ...
                  if (is_shift && mtcs_mode_is_vector_p/*!VECTOR_MODE_P*/(mtcsMode,mode))
                     break;

                  if (is_shift && (INTVAL (inner_const) >= mtcs_mode_get_unit_precision/*!GET_MODE_UNIT_PRECISION*/(mtcsMode,mode)
                  || INTVAL (inner_const) < 0)){
                     if (SHIFT_COUNT_TRUNCATED)
                        inner_const = mtcs_rtl_gen_int_shift_amount/*!gen_int_shift_amount*/(mtcsRTL,mode,
                              (INTVAL (inner_const) & (mtcs_mode_get_unit_bitsize/*!GET_MODE_UNIT_BITSIZE*/(mtcsMode,mode) - 1)));
                     else
                        break;
                  }
                  ...
                  new_const = mtcs_simplify_rtx_binary_operation/*!simplify_binary_operation*/(mtcsSimplifyRtx,
                        associate_code, mode,canon_const_arg1,inner_const);
                  ...
                  if (is_shift   && CONST_INT_P (new_const)  && INTVAL (new_const) >=
                        mtcs_mode_get_unit_precision/*!GET_MODE_UNIT_PRECISION*/(mtcsMode,mode)){
                     /* As an exception, we can turn an ASHIFTRT of this
                     form into a shift of the number of bits - 1.  */
                     if (code == ASHIFTRT)
                        new_const = gen_int_shift_amount(mode, mtcs_mode_get_unit_bitsize/*!GET_MODE_UNIT_BITSIZE*/(mtcsMode,mode) - 1);
                  ...
                  if (! reg_mentioned_p (folded_arg0, y))
                     y = fold_rtx(self,y, insn);

                  return mtcs_simplify_rtx_gen_binary/*!simplify_gen_binary*/(mtcsSimplifyRtx,code, mode, y, new_const);
               }
               break;
         ...
         new_rtx = mtcs_simplify_rtx_binary_operation/*!simplify_binary_operation*/(mtcsSimplifyRtx,code, mode,
      ...
      case RTX_BITFIELD_OPS:
         new_rtx = mtcs_simplify_rtx_ternary_operation/*!simplify_ternary_operation*/(mtcsSimplifyRtx,code, mode, mode_arg0,
                                                      const_arg0 ? const_arg0 : folded_arg0,
                                                      const_arg1 ? const_arg1 : folded_arg1,
...
static rtx equiv_constant (MtcsCse *self,rtx x)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   if (GET_CODE (x) == SUBREG){
      ...
      if ((new_rtx = lookup_as_function(self,x, CONST_INT)) != 0
      || (new_rtx = lookup_as_function(self,x, CONST_WIDE_INT)) != 0
      || (NUM_POLY_INT_COEFFS > 1
      && (new_rtx = lookup_as_function(self,x, CONST_POLY_INT)) != 0)
      || (new_rtx = lookup_as_function(self,x, CONST_DOUBLE)) != 0
      || (new_rtx = lookup_as_function(self,x, CONST_FIXED)) != 0)
         return new_rtx;

      /* If we didn't and if doing so makes sense, see if we previously
      assigned a constant value to the enclosing word mode SUBREG.  */
      if (known_lt (mtcs_mode_get_size/*!GET_MODE_SIZE*/(mtcsMode,mode), UNITS_PER_WORD)
      && known_lt (UNITS_PER_WORD, mtcs_mode_get_size/*!GET_MODE_SIZE*/(mtcsMode,imode))){
         poly_int64 byte = (SUBREG_BYTE (x) - mtcs_mode_subreg_lowpart_offset/*!subreg_lowpart_offset*/(mtcsMode,mode, word_mode));
         if (known_ge (byte, 0) && multiple_p (byte, UNITS_PER_WORD)){
            rtx y = mtcs_rtl_gen_rtx_SUBREG/*!gen_rtx_SUBREG*/(mtcsRTL,mtcsMode->word_mode, SUBREG_REG (x), byte);
            new_rtx = lookup_as_function(self,y, CONST_INT);
            if (new_rtx)
      ...
      && (new_rtx = equiv_constant(self,SUBREG_REG (x))) != 0)
         return mtcs_simplify_rtx_subreg/*!simplify_subreg*/(mtcsSimplifyRtx,mode, new_rtx, imode, SUBREG_BYTE (x));
   ...
   if (MEM_P (x)) {
      ...
      elt = lookup(self,x, SAFE_HASH(self,x, GET_MODE (x)), GET_MODE (x));
      if (elt == 0)
         return 0;
...
static void record_jump_equiv (MtcsCse *self,rtx_insn *insn, bool taken)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);
   ...
   if (taken)
      cond_known_true = (XEXP (SET_SRC (set), 2) == mtcsRTL->pc_rtx/*!pc_rtx*/);
   else
      cond_known_true = (XEXP (SET_SRC (set), 1) ==  mtcsRTL->pc_rtx/*!pc_rtx*/);
   ...
   op0 = fold_rtx(self,XEXP (XEXP (SET_SRC (set), 0), 0), insn);
   op1 = fold_rtx(self,XEXP (XEXP (SET_SRC (set), 0), 1), insn);

   /* If fold_rtx returns NULL_RTX, there's nothing to record.  */
   if (op0 == NULL_RTX || op1 == NULL_RTX)
      return;

   code = find_comparison_args(self,code, &op0, &op1, &mode0, &mode1);
   if (! cond_known_true){
      code = mtcs_dojump_reversed_comparison_code_parts/*!reversed_comparison_code_parts*/(mtcsDojump,code, op0, op1, insn);
   ...
   record_jump_cond(self,code, mode, op0, op1);
}
...
static rtx record_jump_cond_subreg (MtcsCse *self,machine_mode mode, rtx op)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsSimplifyRtx *mtcsSimplifyRtx=mtcs_target_get_simplify_rtx(mtcsTarget);
   ...
   if (op_mode == mode || op_mode == VOIDmode)
      return op;
   return mtcs_simplify_rtx_lowpart_subreg/*!lowpart_subreg*/(mtcsSimplifyRtx,mode, op, op_mode);
}
...
static void record_jump_cond (MtcsCse *self,enum rtx_code code, machine_mode mode, rtx op0, rtx op1)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);
   ...
   if (code == EQ && mtcs_rtl_paradoxical_subreg_p/*!paradoxical_subreg_p*/(mtcsRTL,op0)){
      machine_mode inner_mode = GET_MODE (SUBREG_REG (op0));
      rtx tem = record_jump_cond_subreg(self,inner_mode, op1);
      if (tem)
         record_jump_cond(self,code, mode, SUBREG_REG (op0), tem);
   }

   if (code == EQ && mtcs_rtl_paradoxical_subreg_p/*!paradoxical_subreg_p*/(mtcsRTL,op1)){
      machine_mode inner_mode = GET_MODE (SUBREG_REG (op1));
      rtx tem = record_jump_cond_subreg(self,inner_mode, op0);
      if (tem)
         record_jump_cond(self,code, mode, SUBREG_REG (op1), tem);
   }
   ...
   if (code == NE
   && mtcs_rtl_partial_subreg_p/*!partial_subreg_p*/(mtcsRTL,op0)
   ...
   if (code == NE
   && mtcs_rtl_partial_subreg_p/*!partial_subreg_p*/(mtcsRTL,op1)
   ...
   self->do_not_record = 0;
   self->hash_arg_in_memory = 0;
   op0_hash = HASH(self,op0, mode);
   op0_in_memory = self->hash_arg_in_memory;

   if (self->do_not_record)
      return;

   self->do_not_record = 0;
   self->hash_arg_in_memory = 0;
   op1_hash = HASH(self,op1, mode);
   op1_in_memory = self->hash_arg_in_memory;

   if (self->do_not_record)
      return;

   /* Look up both operands.  */
   op0_elt = lookup(self,op0, op0_hash, mode);
   op1_elt = lookup(self,op1, op1_hash, mode);
   ...
   if (code != EQ || mtcs_mode_is_float_p/*!FLOAT_MODE_P*/(mtcsMode,GET_MODE (op0))){
      struct qty_table_elem *ent;
      int qty;
      ...
      new quantity number.  */
      if (op0_elt == 0){
         if (insert_regs(self,op0, NULL, false)){
            rehash_using_reg(self,op0);
            op0_hash = HASH(self,op0, mode);
         ...
         op0_elt = insert(self,op0, NULL, op0_hash, mode);
         op0_elt->in_memory = op0_in_memory;
      }

      qty = REG_QTY (REGNO (op0));
      ent = &self->qty_table[qty];
      ...
      if (REG_P (op1)){
         ...
         if (op1_elt == 0){
            if (insert_regs(self,op1, NULL, false)){
               rehash_using_reg(self,op1);
               op1_hash = HASH(self,op1, mode);
            }

            op1_elt = insert(self,op1, NULL, op1_hash, mode);
   ...
   merge_equiv_classes(self,op0_elt, op1_elt);
}
...
static void try_back_substitute_reg (MtcsCse *self,rtx set, rtx_insn *insn)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);
   ...
   if (REG_P (dest)
   && REG_P (src) && ! mtcs_reg_is_hard_rtx/*!HARD_REGISTER_P*/(mtcsReg,src)
   && REGNO_QTY_VALID_P (REGNO (src))){
      int src_q = REG_QTY (REGNO (src));
      struct qty_table_elem *src_ent = &self->qty_table[src_q];
      ...
      if (src_ent->first_reg == REGNO (dest)){
         ...
         && ! find_reg_note (prev, REG_EQUIV, NULL_RTX)){
            ...
            mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,prev, &SET_DEST (PATTERN (prev)), dest, 1);
            mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &SET_DEST (set), src, 1);
            mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &SET_SRC (set), dest, 1);
            mtcs_recog_apply_change_group/*!apply_change_group*/(mtcsRecog);
            ...
            if (note != 0   && (reg_mentioned_p (dest, XEXP (note, 0))|| rtx_equal_p (src, XEXP (note, 0))))
            ...
            if (note != 0){
               ...
               mtcs_rtl_set_unique_reg_note/*!set_unique_reg_note*/(mtcsRTL,prev, REG_ARGS_SIZE, XEXP (note, 0));
            }
         }
      }
   }
...
/* Add an entry containing RTL X into SETS.  IS_FAKE_SET is true if X is
   ...
   effect.  */
//不移植
static inline void add_to_set (vec<struct set> *sets, rtx x, bool is_fake_set)
{
  struct set entry = {};
  entry.rtl = x;
...
static int find_sets_in_insn (MtcsCse *self,rtx_insn *insn, vec<struct set> *psets)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsReg *mtcsReg=mtcs_target_get_reg(mtcsTarget);
   ...
   if (GET_CODE (x) == SET){
      ...
      if (SET_DEST (x) == mtcsRTL->pc_rtx/*!pc_rtx*/ && GET_CODE (SET_SRC (x)) == LABEL_REF)
         ;
      ...
      && mtcs_mode_get_class/*!GET_MODE_CLASS*/(mtcsMode,GET_MODE (SET_SRC (x))) != MODE_VECTOR_BOOL
      ...
      && !(SUBREG_P (SET_DEST (x))
      && known_eq (mtcs_mode_get_nunits/*!GET_MODE_NUNITS*/(mtcsMode,GET_MODE (SET_SRC (x))), 1))){
         ...
         for (unsigned i = 0; i < const_vector_encoded_nelts (src) ; i++){
            ...
            rtx y = mtcs_simplify_rtx_simplify_gen_vec_select/*!simplify_gen_vec_select*/(mtcsSimplifyRtx,SET_DEST (x), i);
            gcc_assert (y);
      ...
      for (i = 0; i < lim; i++){
         ...
         if (GET_CODE (y) == SET){
            ...
            if (SET_DEST (y) == mtcsRTL->pc_rtx/*!pc_rtx*/  && GET_CODE (SET_SRC (y)) == LABEL_REF)
               ;
...
static void canon_asm_operands (MtcsCse *self,rtx x, rtx_insn *insn)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);
   ...
   for (int i = ASM_OPERANDS_INPUT_LENGTH (x) - 1; i >= 0; i--){
      ...
      if (!(REG_P (input) && mtcs_reg_is_hard_rtx/*!HARD_REGISTER_P*/(mtcsReg,input))){
         input = canon_reg(self,input, insn);
         mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &ASM_OPERANDS_INPUT (x, i), input, 1);
      }
   }
}
...
static void canonicalize_insn (MtcsCse *self,rtx_insn *insn, vec<struct set> *psets)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);
   ...
   if (CALL_P (insn)){
      for (tem = CALL_INSN_FUNCTION_USAGE (insn); tem; tem = XEXP (tem, 1))
         if (GET_CODE (XEXP (tem, 0)) != SET)
            XEXP (tem, 0) = canon_reg(self,XEXP (tem, 0), insn);
   }

   if (GET_CODE (x) == SET && GET_CODE (SET_SRC (x)) == CALL){
      canon_reg(self,SET_SRC (x), insn);
      mtcs_recog_apply_change_group/*!apply_change_group*/(mtcsRecog);
      fold_rtx(self,SET_SRC (x), insn);
   }else if (GET_CODE (x) == CLOBBER){
      /* If we clobber memory, canon the address.
      This does nothing when a register is clobbered
      because we have already invalidated the reg.  */
      if (MEM_P (XEXP (x, 0)))
         canon_reg(self,XEXP (x, 0), insn);
   }else if (GET_CODE (x) == USE  && ! (REG_P (XEXP (x, 0))
   && REGNO (XEXP (x, 0)) < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)))
      /* Canonicalize a USE of a pseudo register or memory location.  */
      canon_reg(self,x, insn);
   else if (GET_CODE (x) == ASM_OPERANDS)
      canon_asm_operands(self,x, insn);
   else if (GET_CODE (x) == CALL){
      canon_reg(self,x, insn);
      mtcs_recog_apply_change_group/*!apply_change_group*/(mtcsRecog);
      fold_rtx(self,x, insn);
   }else if (DEBUG_INSN_P (insn))
      canon_reg(self,PATTERN (insn), insn);
   else if (GET_CODE (x) == PARALLEL){
      for (i = XVECLEN (x, 0) - 1; i >= 0; i--){
         rtx y = XVECEXP (x, 0, i);
         if (GET_CODE (y) == SET && GET_CODE (SET_SRC (y)) == CALL){
            canon_reg(self,SET_SRC (y), insn);
            mtcs_recog_apply_change_group/*!apply_change_group*/(mtcsRecog);
            fold_rtx(self,SET_SRC (y), insn);
         }else if (GET_CODE (y) == CLOBBER){
            if (MEM_P (XEXP (y, 0)))
               canon_reg(self,XEXP (y, 0), insn);
         }else if (GET_CODE (y) == USE  && ! (REG_P (XEXP (y, 0))
         && REGNO (XEXP (y, 0)) < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)))
            canon_reg(self,y, insn);
         else if (GET_CODE (y) == ASM_OPERANDS)
            canon_asm_operands(self,y, insn);
         else if (GET_CODE (y) == CALL){
            canon_reg(self,y, insn);
            mtcs_recog_apply_change_group/*!apply_change_group*/(mtcsRecog);
            fold_rtx(self,y, insn);
         }
      }
   }
   ...
   && (tem = find_reg_note (insn, REG_EQUAL, NULL_RTX)) != 0){
      ...
      && rtx_equal_p (XEXP (tem, 0), SET_SRC (sets[0].rtl)))
         mtcs_rtlanal_remove_note/*!remove_note*/(mtcsRtlanal,insn, tem);
      else{
         canon_reg(self,XEXP (tem, 0), insn);
         mtcs_recog_apply_change_group/*!apply_change_group*/(mtcsRecog);
         XEXP (tem, 0) = fold_rtx(self,XEXP (tem, 0), insn);
   ...
   for (i = 0; i < n_sets; i++){
      ...
      rtx new_rtx = canon_reg(self,src, insn);

      mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &SET_SRC (sets[i].rtl), new_rtx, 1);

      if (GET_CODE (dest) == ZERO_EXTRACT){
         mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &XEXP (dest, 1),
         canon_reg(self,XEXP (dest, 1), insn), 1);
         mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &XEXP (dest, 2),
         canon_reg(self,XEXP (dest, 2), insn), 1);
      }
...
static void cse_insn (MtcsCse *self,rtx_insn *insn)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);
   ...
   invalidate_from_sets_and_clobbers(self,insn);

   /* Record all the SETs in this instruction.  */
   auto_vec<struct set, 8> sets;
   n_sets = find_sets_in_insn(self,insn, (vec<struct set>*)&sets);

   /* Substitute the canonical register where possible.  */
   canonicalize_insn(self,insn, (vec<struct set>*)&sets);
   ...
   if (n_sets == 1 && REG_NOTES (insn) != 0  && (tem = find_reg_note (insn, REG_EQUAL, NULL_RTX)) != 0){
      ...
      && CONST_INT_P (XEXP (SET_DEST (sets[0].rtl), 2))){
         ...
         scalar_int_mode dest_mode = mtcs_mode_as_a <scalar_int_mode> (mtcsMode,GET_MODE (dest_reg));
         ...
         unsigned int shift;
         if (BITS_BIG_ENDIAN)
            shift = (mtcs_mode_get_precision/*!GET_MODE_PRECISION*/(mtcsMode,dest_mode) - INTVAL (pos) - INTVAL (width));
         else
            shift = INTVAL (pos);
         ...
         val = (val >> shift) & mask;
         src_eqv = mtcs_rtl_GEN_INT/*!GEN_INT*/(mtcsRTL,val);
      }
   }
   ...
   for (i = 0; i < n_sets; i++){
      ...
      if (!is_fake_set && src_eqv){
         ...
         self->do_not_record = 0;
         self->hash_arg_in_memory = 0;
         src_eqv_hash = HASH(self,src_eqv, eqvmode);

         /* Find the equivalence class for the equivalent expression.  */

         if (!self->do_not_record)
            src_eqv_elt = lookup(self,src_eqv, src_eqv_hash, eqvmode);

         src_eqv_volatile = self->do_not_record;
         src_eqv_in_memory = self->hash_arg_in_memory;
      }
      ...
      src_folded = fold_rtx(self,src, NULL);
      ...
      if (GET_CODE (SET_DEST (sets[i].rtl)) == ZERO_EXTRACT){
         ...
         src_folded = mtcs_rtl_GEN_INT/*!GEN_INT*/(mtcsRTL,INTVAL (src) & ((HOST_WIDE_INT_1 << INTVAL (width)) - 1));
      }
#endif
      ...
      self->do_not_record = (REG_P (dest) && REG_P (src) && find_reg_note (insn, REG_UNUSED, dest));
      self->hash_arg_in_memory = 0;

      sets[i].src = src;
      sets[i].src_hash = HASH(self,src, mode);
      sets[i].src_volatile = self->do_not_record;
      sets[i].src_in_memory = self->hash_arg_in_memory;
      ...
      else if (GET_CODE (src) == ASM_OPERANDS   && GET_CODE (x) == PARALLEL){
      ...
      if (!sets[i].src_volatile)
         elt = lookup(self,src, sets[i].src_hash, mode);
      ...
      if (elt && src_eqv_here && src_eqv_elt){
         if (elt->first_same_value != src_eqv_elt->first_same_value){
            ...
            merge_equiv_classes(self,elt, src_eqv_elt);
            src_eqv_hash = HASH(self,src_eqv, elt->mode);
            src_eqv_elt = lookup(self,src_eqv, src_eqv_hash, elt->mode);
         }
      ...
      if (src_const && src_const_elt == 0){
         sets[i].src_const_hash = HASH(self,src_const, mode);
         src_const_elt = lookup(self,src_const, sets[i].src_const_hash, mode);
      }
      ...
      if (src_const_elt && elt && src_const_elt->first_same_value != elt->first_same_value)
         merge_equiv_classes(self,elt, src_const_elt);
      ...
      equivalent of SRC.  */
      if (src_const
      && (GET_CODE (src_const) == CONST  || (src_const_elt && src_const_elt->related_value != 0))){
         src_related = use_related_value(self,src_const, src_const_elt);
         if (src_related){
            struct table_elt *src_related_elt = lookup(self,src_related, HASH(self,src_related, mode), mode);
            if (src_related_elt && elt){
               if (elt->first_same_value != src_related_elt->first_same_value)
                  ...
                  merge_equiv_classes(self,elt, src_related_elt);
      ...
      && mtcs_mode_get_precision/*!GET_MODE_PRECISION*/(mtcsMode,int_mode) < BITS_PER_WORD){
         ...
         MTCS_FOR_EACH_WIDER_MODE/*!FOR_EACH_WIDER_MODE*/(mtcsMode,wider_mode_iter, int_mode){
            scalar_int_mode wider_mode = wider_mode_iter.require ();
            if (mtcs_mode_get_precision/*!GET_MODE_PRECISION*/(mtcsMode,wider_mode) > BITS_PER_WORD)
               break;

            struct table_elt *const_elt = lookup(self,src_const, HASH(self,src_const, wider_mode), wider_mode);

            if (const_elt == 0)
               continue;

            for (const_elt = const_elt->first_same_value; const_elt; const_elt = const_elt->next_same_value)
      ...
      if (mtcsOptionsItem->x_flag_expensive_optimizations && ! src_related
      && mtcs_mode_is_a <scalar_int_mode> (mtcsMode,mode, &int_mode)
      && GET_CODE (src) == AND && CONST_INT_P (XEXP (src, 1))
      && mtcs_mode_get_size/*!GET_MODE_SIZE*/(mtcsMode,int_mode) < UNITS_PER_WORD){
         opt_scalar_int_mode tmode_iter;
         rtx new_and = gen_rtx_AND (VOIDmode, NULL_RTX, XEXP (src, 1));

         MTCS_FOR_EACH_WIDER_MODE/*!FOR_EACH_WIDER_MODE*/(mtcsMode,tmode_iter, int_mode){
            scalar_int_mode tmode = tmode_iter.require ();
            if (mtcs_mode_get_size/*!GET_MODE_SIZE*/(mtcsMode,tmode) > UNITS_PER_WORD)
               break;
            ...
            if (inner){
               mtcs_rtl_put_mode/*!PUT_MODE*/(mtcsRTL,new_and, tmode);
               XEXP (new_and, 0) = inner;
               larger_elt = lookup(self,new_and, HASH(self,new_and, tmode), tmode);
               if (larger_elt == 0)
                  continue;

               for (larger_elt = larger_elt->first_same_value; larger_elt; larger_elt = larger_elt->next_same_value)
      ...
      && MEM_P (src) && ! self->do_not_record
      && mtcs_mode_is_a <scalar_int_mode> (mtcsMode,mode, &int_mode)
      && (extend_op = mtcs_rtl_load_extend_op/*!load_extend_op*/(mtcsRTL,int_mode)) != UNKNOWN){
         #if GCC_VERSION >= 5000
         ...
         MTCS_FOR_EACH_WIDER_MODE/*!FOR_EACH_WIDER_MODE*/(mtcsMode,tmode_iter, int_mode){
            ...
            mtcs_rtl_put_mode/*!PUT_MODE*/(mtcsRTL,memory_extend_rtx, tmode);
            larger_elt = lookup(self,memory_extend_rtx, HASH(self,memory_extend_rtx, tmode), tmode);
            if (larger_elt == 0)
               continue;
      ...
      if (mtcsTarget/*!targetm.const_anchor*/->const_anchor
      && !src_related
      && src_const
      && GET_CODE (src_const) == CONST_INT){
         src_related = try_const_anchors(self,src_const, mode);
         src_related_is_const_anchor = src_related != NULL_RTX;
      }

      /* Try to re-materialize a vec_dup with an existing constant.   */
      rtx src_elt;
      if ((!src_eqv_here || CONSTANT_P (src_eqv_here))  && const_vec_duplicate_p (src, &src_elt)){
         machine_mode const_mode = mtcs_mode_get_inner/*!GET_MODE_INNER*/(mtcsMode,GET_MODE (src));
         struct table_elt *related_elt = lookup(self,src_elt, HASH(self,src_elt, const_mode), const_mode);
         if (related_elt){
            for (related_elt = related_elt->first_same_value;related_elt; related_elt = related_elt->next_same_value)
               if (REG_P (related_elt->exp)) {
                  ...
                  src_eqv_here = gen_rtx_VEC_DUPLICATE (GET_MODE (src), related_elt->exp);
                  break;
               }
         }
      }
      ...
      for (p = elt; p; p = p->next_same_value){
         ...
         if (code != REG && ! mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,p->exp, p->exp, 1, false))
            continue;

         /* Also skip paradoxical subregs, unless that's what we're
         looking for.  */
         if (mtcs_rtl_paradoxical_subreg_p/*!paradoxical_subreg_p*/(mtcsRTL,p->exp)
         && ! (src != 0
         && GET_CODE (src) == SUBREG
         && GET_MODE (src) == GET_MODE (p->exp)
         && mtcs_mode_partial_subreg_p/*!partial_subreg_p*/(mtcsMode,
               GET_MODE (SUBREG_REG (src)), GET_MODE (SUBREG_REG (p->exp)))))
            continue;
         ...
         if (src && GET_CODE (src) == code && rtx_equal_p (src, p->exp))
            ...
            src_folded = 0;
         else if (src_eqv_here && GET_CODE (src_eqv_here) == code && rtx_equal_p (src_eqv_here, p->exp))
            src_eqv_here = 0;
         else if (src_related && GET_CODE (src_related) == code  && rtx_equal_p (src_related, p->exp))
            src_related = 0;

         /* This is the same as the destination of the insns, we want
         to prefer it.  The code below will then give it a negative
         cost.  */
         if (!dest_related && GET_CODE (dest) == code && rtx_equal_p (p->exp, dest))
            dest_related = p->exp;
      }
      ...
      if (src){
         if (rtx_equal_p (src, dest))
            ...
            src_regcost = approx_reg_cost(self,src);
         }
      }
      ...
      if (src_eqv_here){
         if (rtx_equal_p (src_eqv_here, dest))
            ...
            src_eqv_regcost = approx_reg_cost(self,src_eqv_here);
         }
      }
      ...
      if (src_folded){
         if (rtx_equal_p (src_folded, dest))
            ...
            src_folded_regcost = approx_reg_cost(self,src_folded);
         }
      }
      ...
      if (dest_related){
         ...
         src_related_regcost = approx_reg_cost(self,src_related);
         ...
         loops.  */
         if (src_related_is_const_anchor  && src_related_cost == src_cost && src_eqv_here)
            src_related_cost--;
      }

      /* If this was an indirect jump insn, a known label will really be
      cheaper even though it looks more expensive.  */
      if (dest == mtcsRTL->pc_rtx/*!pc_rtx*/ && src_const && GET_CODE (src_const) == LABEL_REF)
      ...
      while (!is_fake_set){
         ...
         while (elt && !REG_P (elt->exp) && ! mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,elt->exp, elt->exp, 1, false))
         ...
         if (elt != 0
         && mtcs_rtl_paradoxical_subreg_p/*!paradoxical_subreg_p*/(mtcsRTL,elt->exp)
         ...
         && mtcs_mode_partial_subreg_p/*!partial_subreg_p*/(mtcsMode,
               GET_MODE (SUBREG_REG (src)),GET_MODE (SUBREG_REG (elt->exp))))){
            elt = elt->next_same_value;
            continue;
         }
         ...
         && (known_ge
         (mtcs_mode_get_precision/*!GET_MODE_PRECISION*/(mtcsMode,GET_MODE (SET_DEST (sets[i].rtl))),
         INTVAL (XEXP (SET_DEST (sets[i].rtl), 1))))
         && ((unsigned) INTVAL (XEXP (SET_DEST (sets[i].rtl), 1)) + (unsigned) INTVAL (XEXP (SET_DEST (sets[i].rtl), 2))
         <= HOST_BITS_PER_WIDE_INT)){
            rtx dest_reg = XEXP (SET_DEST (sets[i].rtl), 0);
            rtx width = XEXP (SET_DEST (sets[i].rtl), 1);
            rtx pos = XEXP (SET_DEST (sets[i].rtl), 2);
            unsigned int dest_hash = HASH(self,dest_reg, GET_MODE (dest_reg));
            struct table_elt *dest_elt = lookup(self,dest_reg, dest_hash, GET_MODE (dest_reg));
            rtx dest_cst = NULL;
            ...
            if (dest_cst){
               ...
               val = mtcs_mode_trunc_int_for_mode/*!trunc_int_for_mode*/(mtcsMode,val, dest_mode);
               mtcs_recog_validate_unshare_change/*!validate_unshare_change*/(mtcsRecog,insn, &SET_DEST (sets[i].rtl),dest_reg, 1);
               mtcs_recog_validate_unshare_change/*!validate_unshare_change*/(mtcsRecog,insn, &SET_SRC (sets[i].rtl),
                     mtcs_rtl_GEN_INT/*!GEN_INT*/(mtcsRTL,val), 1);
         ...
         barriers).  */
         if (n_sets == 1 && dest == mtcsRTL->pc_rtx/*!pc_rtx*/
         && (trial == mtcsRTL->pc_rtx/*!pc_rtx*/
         || (GET_CODE (trial) == LABEL_REF
         && ! condjump_p (insn)))){
            /* Don't substitute non-local labels, this confuses CFG.  */
            if (GET_CODE (trial) == LABEL_REF && LABEL_REF_NONLOCAL_P (trial))
               continue;

            SET_SRC (sets[i].rtl) = trial;
            self->cse_jumps_altered = true;
            break;
         }
         ...
         && (!MEM_P (trial)
         || ((mtcs_rtl_get_mem_alias/*!MEM_ALIAS_SET*/(mtcsRTL,dest) == mtcs_rtl_get_mem_alias/*!MEM_ALIAS_SET*/(mtcsRTL,trial)
         || mtcs_alias_alias_set_subset_of/*!alias_set_subset_of*/(mtcsAlias,mtcs_rtl_get_mem_alias/*!MEM_ALIAS_SET*/(mtcsRTL,dest),
               mtcs_rtl_get_mem_alias/*!MEM_ALIAS_SET*/(mtcsRTL,trial)))
         && (!mtcs_rtl_get_mem_expr/*!MEM_EXPR*/(mtcsRTL,trial)
         || mtcs_alias_refs_same_for_tbaa_p/*!refs_same_for_tbaa_p*/(mtcsAlias,mtcs_rtl_get_mem_expr/*!MEM_EXPR*/(mtcsRTL,trial),
               mtcs_rtl_get_mem_expr/*!MEM_EXPR*/(mtcsRTL,dest)))))) {
            SET_SRC (sets[i].rtl) = trial;
            noop_insn = true;
            break;
         }
         ...
         else if (mtcs_recog_validate_unshare_change/*!validate_unshare_change*/(mtcsRecog,
               insn, &SET_SRC (sets[i].rtl),trial, 0)){
            rtx new_rtx = canon_reg(self,SET_SRC (sets[i].rtl), insn);
         ...
         likely came from.  */
         else if (mtcsRtlData/*!crtl*/->uses_const_pool
         && CONSTANT_P (trial)
         && !CONST_INT_P (trial)
         && (src_folded == 0 || !MEM_P (src_folded))
         && mtcs_mode_get_class/*!GET_MODE_CLASS*/(mtcsMode,mode) != MODE_CC
         && mode != VOIDmode){
            src_folded = mtcs_asm_force_const_mem/*!force_const_mem*/(mtcsAsm,mode, trial);
            if (src_folded){
               src_folded_cost = COST (src_folded, mode);
               src_folded_regcost = approx_reg_cost(self,src_folded);
            }
         }
      }//end    while (!is_fake_set){
      ...
      && REGNO_QTY_VALID_P (REGNO (dest))){
         ...
         struct qty_table_elem *dest_ent = &self->qty_table[dest_q];
         ...
         SET_SRC or SET_DEST.  */
         && (!REG_P (sets[i].src)
         || REGNO (sets[i].src) >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))
         ...
         SRC is a hard register.  */
         {
            ...
            rtx new_src = (first >= mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg)
            ? mtcsRtlData->regno_reg_rtx/*!regno_reg_rtx*/[first] : mtcs_rtl_gen_rtx_REG/*!gen_rtx_REG*/(mtcsRTL,GET_MODE (src), first));

            /* We must use validate-change even for this, because this
            might be a special no-op instruction, suitable only to
            tag notes onto.  */
            if (mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &SET_SRC (sets[i].rtl), new_src, 0)){
               src = new_src;
               /* If we had a constant that is cheaper than what we are now
               setting SRC to, use that constant.  We ignored it when we
               thought we could make this into a no-op.  */
               if (src_const && COST (src_const, mode) < COST (src, mode)
               && mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, &SET_SRC (sets[i].rtl),src_const, 0))
                  src = src_const;
            }
         }
      }
      ...
      if (src != sets[i].src){
         ...
         sets[i].src_volatile = self->do_not_record;
         sets[i].src_in_memory = self->hash_arg_in_memory;
         sets[i].src_elt = lookup(self,src, sets[i].src_hash, mode);
      }
      ...
      && !rtx_equal_p (src, src_const)){
         ...
         mtcs_rtl_set_unique_reg_note/*!set_unique_reg_note*/(mtcsRTL,insn, REG_EQUAL, src_const);
         mtcs_dfscan_df_notes_rescan/*!df_notes_rescan*/(mtcsDfscan,insn);
      }
      ...
      if (MEM_P (dest)){
         ...
         && XEXP (addr, 0) == mtcs_rtl_get_stack_pointer_rtx/*!stack_pointer_rtx*/(mtcsRTL))
            invalidate(self,mtcs_rtl_get_stack_pointer_rtx/*!stack_pointer_rtx*/(mtcsRTL), VOIDmode);
#endif
         dest = fold_rtx(self,dest, insn);
      }
      ...
      sets[i].dest_hash = HASH(self,dest, mode);
      ...
      the insn.  */
      else if (n_sets == 1 && dest == mtcsRTL->pc_rtx/*!pc_rtx*/ && src == mtcsRTL->pc_rtx/*!pc_rtx*/){
         /* One less use of the label this insn used to jump to.  */
         self->cse_cfg_altered |= mtcs_cfg_rtl_delete_insn_and_edges/*!delete_insn_and_edges*/(mtcsCfgRtl,insn);
         self->cse_jumps_altered = true;
      ...
      else if (dest == mtcsRTL->pc_rtx/*!pc_rtx*/ && GET_CODE (src) == LABEL_REF
      && !LABEL_REF_NONLOCAL_P (src)){
         ...
         if (n_sets == 1){
            ...
            rtx_insn *seq = target_rtx_gen_jump/*!targetm.gen_jump*/(mtcsMachine->tmrtx,XEXP (src, 0));
            new_rtx = mtcs_emit_emit_jump_insn_before/*!emit_jump_insn_before*/(mtcsEmit,seq, insn);
      ...
      else if (self->do_not_record){
         invalidate_dest(self,dest);
         sets[i].rtl = 0;
      }

      if (sets[i].rtl != 0 && dest != SET_DEST (sets[i].rtl)){
         self->do_not_record = 0;
         sets[i].dest_hash = HASH(self,SET_DEST (sets[i].rtl), mode);
         if (self->do_not_record){
            invalidate_dest(self,SET_DEST (sets[i].rtl));
            sets[i].rtl = 0;
         }
      }
   }//end    for (i = 0; i < n_sets; i++){
   ...
   && ! rtx_equal_p (src_eqv, SET_DEST (sets[0].rtl))){
      ...
      if (insert_regs(self,src_eqv, classp, false)){
         rehash_using_reg(self,src_eqv);
         src_eqv_hash = HASH(self,src_eqv, eqvmode);
      }
      elt = insert(self,src_eqv, classp, src_eqv_hash, eqvmode);
   ...
   for (i = 0; i < n_sets; i++)
      if (sets[i].rtl && ! sets[i].src_volatile && ! rtx_equal_p (SET_SRC (sets[i].rtl), SET_DEST (sets[i].rtl))){
         if (GET_CODE (SET_DEST (sets[i].rtl)) == STRICT_LOW_PART){
            ...
            rtx src = sets[i].src;
            rtx dest = SET_DEST (sets[i].rtl);
            machine_mode mode = GET_MODE (src) == VOIDmode ? GET_MODE (dest) : GET_MODE (src);
            ...
            if (sets[i].src_elt == 0){
               ...
               if (insert_regs(self,src, classp, false)){
                  rehash_using_reg(self,src);
                  sets[i].src_hash = HASH(self,src, mode);
               }
               elt = insert(self,src, classp, sets[i].src_hash, mode);
               ...
               if (GET_CODE (src) == ASM_OPERANDS && GET_CODE (x) == PARALLEL)
                  elt->cost = MAX_COST;
               sets[i].src_elt = classp = elt;
            }
            if (sets[i].src_const && sets[i].src_const_elt == 0 && src != sets[i].src_const && ! rtx_equal_p (sets[i].src_const, src))
               sets[i].src_elt = insert(self,sets[i].src_const, classp, sets[i].src_const_hash, mode);
         }
   ...
   for (i = 0; i < n_sets; i++){
      if (sets[i].rtl){
         ...
         if (MEM_P (x)){
            x = XEXP (x, 0);
            mode = GET_MODE (x);
            hash = HASH(self,x, mode);
            elt = lookup(self,x, hash, mode);
            if (!elt){
               if (insert_regs(self,x, NULL, false)){
                  rtx dest = SET_DEST (sets[i].rtl);

                  rehash_using_reg(self,x);
                  hash = HASH(self,x, mode);
                  sets[i].dest_hash = HASH(self,dest, GET_MODE (dest));
               }
               elt = insert(self,x, NULL, hash, mode);
            }
   ...
   invalidate_from_clobbers(self,insn);
   ...
   if (CALL_P (insn)){
      if (!(RTL_CONST_OR_PURE_CALL_P (insn)))
         invalidate_memory(self);
      else
         /* For const/pure calls, invalidate any argument slots, because
         those are owned by the callee.  */
         for (tem = CALL_INSN_FUNCTION_USAGE (insn); tem; tem = XEXP (tem, 1))
            if (GET_CODE (XEXP (tem, 0)) == USE && MEM_P (XEXP (XEXP (tem, 0), 0)))
               invalidate(self,XEXP (XEXP (tem, 0), 0), VOIDmode);
      invalidate_for_call(self,insn);
   }
   ...
   for (i = 0; i < n_sets; i++)
      if (sets[i].rtl){
         ...
         if (REG_P (dest) || GET_CODE (dest) == SUBREG){
            n_debug("mtcscse.c cse_insn invalidate 00\n");
            mtcs_print_rtl(stderr,dest);


            invalidate(self,dest, VOIDmode);
         }else if (MEM_P (dest)){
            n_debug("mtcscse.c cse_insn invalidate 11\n");
            mtcs_print_rtl(stderr,dest);

            invalidate(self,dest, VOIDmode);
         }else if (GET_CODE (dest) == STRICT_LOW_PART || GET_CODE (dest) == ZERO_EXTRACT){
            n_debug("mtcscse.c cse_insn invalidate 22\n");
            mtcs_print_rtl(stderr,dest);
   ...
   for (i = 0; i < n_sets; i++)
      if (sets[i].rtl){
         if (sets[i].dest_addr_elt && sets[i].dest_addr_elt->first_same_value == 0){
   ...
   for (i = 0; i < n_sets; i++)
      if (sets[i].rtl){
         ...
         memory.  */
         if ((mtcsOptionsItem->x_flag_float_store
         && MEM_P (dest)
         && mtcs_mode_is_float_p/*!FLOAT_MODE_P*/(mtcsMode,GET_MODE (dest)))
         /* Don't record BLKmode values, because we don't know the
         size of it, and can't be sure that other BLKmode values
         have the same or smaller size.  */
         || GET_MODE (dest) == mtcsMode->modes.M_BLKmode
         ...
         if (REG_P (dest) || GET_CODE (dest) == SUBREG)
            ...
            if (insert_regs(self,dest, sets[i].src_elt, true)){
               /* If `insert_regs' changes something, the hash code must be
               recalculated.  */
               rehash_using_reg(self,dest);
               sets[i].dest_hash = HASH(self,dest, GET_MODE (dest));
            }
         ...
         if (mtcs_rtl_paradoxical_subreg_p/*!paradoxical_subreg_p*/(mtcsRTL,dest))
            continue;

         elt = insert(self,dest, sets[i].src_elt,
         ...
         if (mtcsTarget/*!targetm.const_anchor*/->const_anchor
         && REG_P (dest)
         && mtcs_mode_is_scalar_int_p/*!SCALAR_INT_MODE_P*/(mtcsMode,GET_MODE (dest))
         && GET_CODE (sets[i].src_elt->exp) == CONST_INT)
            insert_const_anchors(self,dest, sets[i].src_elt->exp, GET_MODE (dest));

         elt->in_memory = (MEM_P (sets[i].inner_dest)  && !MEM_READONLY_P (sets[i].inner_dest));
         ...
         (mtcs_mode_get_size_poly/*!GET_MODE_SIZE*/(mtcsMode,GET_MODE (SUBREG_REG (dest))) - 1,
               mtcs_mode_get_size_poly/*!GET_MODE_SIZE*/(mtcsMode,GET_MODE (dest)) - 1,UNITS_PER_WORD))
         && !mtcs_rtl_partial_subreg_p/*!partial_subreg_p*/(mtcsRTL,dest)
         && sets[i].src_elt != 0){
            ...
            for (elt = sets[i].src_elt->first_same_value; elt; elt = elt->next_same_value){
               ...
               if (GET_MODE (elt->exp) == new_mode)
                  ...
                  poly_uint64 byte = mtcs_mode_subreg_lowpart_offset/*!subreg_lowpart_offset*/(mtcsMode,
                        new_mode, GET_MODE (dest));
                  new_src = mtcs_simplify_rtx_gen_subreg/*!simplify_gen_subreg*/(mtcsSimplifyRtx,
                        new_mode, elt->exp,GET_MODE (dest), byte);
               }
               ...
               src_hash = HASH(self,new_src, new_mode);
               src_elt = lookup(self,new_src, src_hash, new_mode);

               /* Put the new source in the hash table is if isn't
               already.  */
               if (src_elt == 0){
                  if (insert_regs(self,new_src, classp, false)){
                     rehash_using_reg(self,new_src);
                     src_hash = HASH(self,new_src, new_mode);
                  }
                  src_elt = insert(self,new_src, classp, src_hash, new_mode);
                  src_elt->in_memory = elt->in_memory;
                  if (GET_CODE (new_src) == ASM_OPERANDS && elt->cost == MAX_COST)
                     src_elt->cost = MAX_COST;
               }else if (classp && classp != src_elt->first_same_value)
                  /* Show that two things that we've seen before are
                  actually the same.  */
                  merge_equiv_classes(self,src_elt, classp);

               classp = src_elt->first_same_value;
               /* Ignore invalid entries.  */
               while (classp && !REG_P (classp->exp)
                     && ! mtcs_cse_exp_equiv_p/*!exp_equiv_p*/(self,classp->exp, classp->exp, 1, false))
                  classp = classp->next_same_value;
            }//end for (elt = sets[i].src_elt->first_same_value; elt; elt = elt->next_same_value){
         }
      }//end  if (sets[i].rtl){
   ...
   if (n_sets == 1 && sets[0].rtl)
      try_back_substitute_reg(self,sets[0].rtl, insn);
...
static void invalidate_from_clobbers (MtcsCse *self,rtx_insn *insn)
{
   rtx x = PATTERN (insn);
   ...
   if (GET_CODE (x) == CLOBBER){
      rtx ref = XEXP (x, 0);
      if (ref){
         if (REG_P (ref) || GET_CODE (ref) == SUBREG || MEM_P (ref))
            invalidate(self,ref, VOIDmode);
         else if (GET_CODE (ref) == STRICT_LOW_PART || GET_CODE (ref) == ZERO_EXTRACT)
            invalidate(self,XEXP (ref, 0), GET_MODE (ref));
      }
   }else if (GET_CODE (x) == PARALLEL){
      int i;
      for (i = XVECLEN (x, 0) - 1; i >= 0; i--){
...
static void invalidate_from_sets_and_clobbers (MtcsCse *self,rtx_insn *insn)
{
   rtx tem;
   rtx x = PATTERN (insn);
   ...
   if (CALL_P (insn)){
      for (tem = CALL_INSN_FUNCTION_USAGE (insn); tem; tem = XEXP (tem, 1)){
         ...
         if (GET_CODE (temx) == CLOBBER)
            invalidate(self,SET_DEST (temx), VOIDmode);
      }
   }
   ...
   if (GET_CODE (x) == SET && GET_CODE (SET_SRC (x)) == CALL)
      invalidate(self,SET_DEST (x), VOIDmode);
      ...
      for (i = XVECLEN (x, 0) - 1; i >= 0; i--){
         ...
         if (GET_CODE (y) == CLOBBER){
            ...
            if (REG_P (clobbered) || GET_CODE (clobbered) == SUBREG)
               invalidate(self,clobbered, VOIDmode);
            else if (GET_CODE (clobbered) == STRICT_LOW_PART || GET_CODE (clobbered) == ZERO_EXTRACT)
               invalidate(self,XEXP (clobbered, 0), GET_MODE (clobbered));
         } else if (GET_CODE (y) == SET && GET_CODE (SET_SRC (y)) == CALL)
            invalidate(self,SET_DEST (y), VOIDmode);
      }
   }
}

static rtx cse_process_note (MtcsCse *self,rtx);
...
static rtx cse_process_note_1 (rtx x, const_rtx, void *userData)
{
   MtcsCse *self=(MtcsCse *)userData;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);

   if (MEM_P (x)){
      mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,x, &XEXP (x, 0),
            cse_process_note(self,XEXP (x, 0)), false);
      return x;
   }
   ...
   if (REG_P (x)){
      ...
      if (REGNO_QTY_VALID_P (REGNO (x))){
         struct qty_table_elem *ent = &self->qty_table[i];
         if (ent->const_rtx != NULL_RTX && (CONSTANT_P (ent->const_rtx) || REG_P (ent->const_rtx))){
...
static rtx cse_process_note (MtcsCse *self,rtx x)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsSimplifyRtx *mtcsSimplifyRtx=mtcs_target_get_simplify_rtx(mtcsTarget);
   ...
   return mtcs_simplify_rtx_simplify_replace_fn_rtx/*!simplify_replace_fn_rtx*/(mtcsSimplifyRtx,
     x, NULL_RTX, cse_process_note_1, (void*)self/*!NULL*/);
}
...
static bool cse_find_path (MtcsCse *self,basic_block first_bb, struct cse_basic_block_data *data,
          int follow_jumps)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   bitmap_set_bit (self->cse_visited_basic_blocks, first_bb->index);
   ...
   if (path_size == 0)
      ...
      while (path_size >= 2){
         ...
         && e == BRANCH_EDGE (previous_bb_in_path)){
            ...
            if (bb != EXIT_BLOCK_PTR_FOR_FN (cfun) && single_pred_p (bb)
            ...
            && !bitmap_bit_p (self->cse_visited_basic_blocks, bb->index)){
               bitmap_set_bit (self->cse_visited_basic_blocks, bb->index);
   ...
   if (follow_jumps){
      ...
      while (bb && path_size < mtcsOptionsItem->x_param_max_cse_path_length){
         if (single_succ_p (bb))
            e = single_succ_edge (bb);
         else if (EDGE_COUNT (bb->succs) == 2 && any_condjump_p (BB_END (bb))){
         ...
         && !bitmap_bit_p (self->cse_visited_basic_blocks, e->dest->index)){
            basic_block bb2 = e->dest;
            bitmap_set_bit (self->cse_visited_basic_blocks, bb2->index);
...
static void cse_extended_basic_block (MtcsCse *self,struct cse_basic_block_data *ebb_data)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCfgRtl *mtcsCfgRtl=mtcs_target_get_cfg_rtl(mtcsTarget);
   ...
   self->qty_table = XNEWVEC (struct qty_table_elem, self->max_qty);

   new_basic_block(self);
   self->cse_ebb_live_in = df_get_live_in (ebb_data->path[0].bb);
   self->cse_ebb_live_out = df_get_live_out (ebb_data->path[path_size - 1].bb);
   for (path_entry = 0; path_entry < path_size; path_entry++){
      basic_block bb;
      rtx_insn *insn;
      ...
      if (bb_has_eh_pred (bb)){
         ...
         FOR_EACH_ARTIFICIAL_DEF (def, bb->index)
            if (DF_REF_FLAGS (def) & DF_REF_AT_TOP)
               invalidate(self,DF_REF_REG (def), GET_MODE (DF_REF_REG (def)));
      }

      self->optimize_this_for_speed_p = optimize_bb_for_speed_p (bb);
      FOR_BB_INSNS (bb, insn){
         ...
         way.  */
         if (NONDEBUG_INSN_P (insn)  && num_insns++ > param_max_cse_insns){
            flush_hash_table(self);
            num_insns = 0;
         }
         ...
         if (INSN_P (insn)){
            ...
            for (rtx note = REG_NOTES (insn); note; note = XEXP (note, 1))
               if (REG_NOTE_KIND (note) == REG_EQUAL){
                  rtx newval = cse_process_note(self,XEXP (note, 0));
            ...
            check this one.  */
            if (INSN_P (insn) && !self->recorded_label_ref && check_for_label_ref (insn))
               self->recorded_label_ref = true;
         }
      }
      ...
      if (cfun->can_throw_non_call_exceptions && have_eh_succ_edges (bb))
         self->cse_cfg_altered |= mtcs_cfg_rtl_purge_dead_edges/*!purge_dead_edges*/(mtcsCfgRtl,bb);
      ...
      if (path_entry < path_size - 1){
         ...
         if (!find_edge (bb, next_bb)){
            do{
               ...
               bitmap_clear_bit (self->cse_visited_basic_blocks, ebb_data->path[path_size].bb->index);
      ...
      && any_condjump_p (insn)){
         ...
         record_jump_equiv(self,insn, taken);
      }
   }
...
///实现接口rtlhooks.h RTL_HOOKS_GEN_LOWPART 方法 在cse_main中运行完后，需要恢复在mtcsrtl.c中定义的RTL_HOOKS_GEN_LOWPART--------------------
static rtx genLowpart_cb (machine_mode mode, rtx x)
{
    ...
    MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);
    return mtcs_rtl_gen_lowpart_if_possible/*!gen_lowpart_if_possible*/(mtcsRTL,mode,x);
}
...
/* Perform cse on the instructions of a function.
   ...
   Return 0 otherwise.  */
static int cse_main (MtcsCse *self,rtx_insn *f ATTRIBUTE_UNUSED, int nregs)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   ...
   n_debug("mtcscse.c cse_main 00 经过 mtcs_dfcore_df_analyze 分析 rtl可能被清除\n");
   testprint(cfun);

   mtcs_dfcore_df_analyze/*!df_analyze*/(mtcsDfcore);
   n_debug("mtcscse.c cse_main 11\n");
   //printrtl();
    testprint(cfun);

   mtcs_dfcore_df_set_flags/*!df_set_flags*/(mtcsDfcore,DF_DEFER_INSN_RESCAN);

   mtcs_rtl_reg_scan/*!reg_scan*/(mtcsRTL,mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData),
         mtcs_func_max_reg_num/*!max_reg_num*/(mtcsFunc));

   init_cse_reg_info(self,nregs);
   ebb_data.path = XNEWVEC (struct branch_path,mtcsOptionsItem->x_param_max_cse_path_length);

   self->cse_cfg_altered = false;
   self->cse_jumps_altered = false;
   self->recorded_label_ref = false;
   ebb_data.path_size = 0;
   ebb_data.nsets = 0;
   //重新设置rtl_hooks中的RTL_HOOKS_GEN_LOWPART函数指针到genLowpart_cb
   //等同于原型中的三句
   //1.#undef RTL_HOOKS_GEN_LOWPART
   //2.#define RTL_HOOKS_GEN_LOWPART    gen_lowpart_if_possible
   //3.static const struct rtl_hooks cse_rtl_hooks = RTL_HOOKS_INITIALIZER;
   //4. rtl_hooks = cse_rtl_hooks;
   void *old_rlt_hooks_gen_lowpart=gen_lowpart;//备份原rtl_hooks中的函数指针
   gen_lowpart/*!rtl_hooks.gen_lowpart*/=genLowpart_cb;

   n_debug("mtcscse.c cse_main 44\n");
   testprint(cfun);

   mtcs_recog_init_recog/*!init_recog*/(mtcsRecog);
   n_debug("mtcscse.c cse_main 55\n");

   mtcs_alias_init_alias_analysis/*!init_alias_analysis*/(mtcsAlias);
   n_debug("mtcscse.c cse_main 66\n");

   self->reg_eqv_table = XNEWVEC (struct reg_eqv_elem, nregs);

   /* Set up the table of already visited basic blocks.  */
   self->cse_visited_basic_blocks = sbitmap_alloc (last_basic_block_for_fn (cfun));
   bitmap_clear (self->cse_visited_basic_blocks);
   n_debug("mtcscse.c cse_main 77\n");
   testprint(cfun);
   ...
   while (i < n_blocks){
      ...
      }while (bitmap_bit_p (self->cse_visited_basic_blocks, bb->index) && i < n_blocks);

      /* Find all paths starting with BB, and process them.  */
      while (cse_find_path(self,bb, &ebb_data,mtcsOptionsItem->x_flag_cse_follow_jumps)){
         /* Pre-scan the path.  */
         ...
         self->max_qty = ebb_data.nsets * MAX_RECOG_OPERANDS;

         /* Dump the path we're about to process.  */
         if (dump_file)
            cse_dump_path (&ebb_data, ebb_data.nsets, dump_file);
         n_debug("mtcscse.c cse_main 77aa\n");
         cse_extended_basic_block(self,&ebb_data);
         n_debug("mtcscse.c cse_main 77bb\n");

      }
   }
   n_debug("mtcscse.c cse_main 88\n");
   testprint(cfun);
   /* Clean up.  */
   mtcs_alias_end_alias_analysis/*!end_alias_analysis*/(mtcsAlias);
   n_debug("mtcscse.c cse_main 99\n");

   free (self->reg_eqv_table);
   free (ebb_data.path);
   sbitmap_free (self->cse_visited_basic_blocks);
   free (rc_order);
   //恢复被替换的方法
   //rtl_hooks = general_rtl_hooks;
   gen_lowpart/*!rtl_hooks.gen_lowpart*/=old_rlt_hooks_gen_lowpart;

   if (self->cse_jumps_altered || self->recorded_label_ref)
      return 2;
   else if (self->cse_cfg_altered)
      return 1;
   else
      return 0;
}
...
static void cse_change_cc_mode (MtcsCse *self,subrtx_ptr_iterator::array_type &array,
          rtx *loc, rtx_insn *insn, rtx newreg)
{
   ...
   FOR_EACH_SUBRTX_PTR (iter, array, loc, NONCONST){
      ...
      && GET_MODE (x) != GET_MODE (newreg)){
         mtcs_recog_validate_change/*!validate_change*/(mtcsRecog,insn, loc, newreg, 1);
         iter.skip_subrtxes ();
      }
   }
}
...
static void cse_change_cc_mode_insn (MtcsCse *self,rtx_insn *insn, rtx newreg)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRecog *mtcsRecog=mtcs_target_get_recog(mtcsTarget);
   ...
   cse_change_cc_mode(self,array, &PATTERN (insn), insn, newreg);
   cse_change_cc_mode(self,array, &REG_NOTES (insn), insn, newreg);
   ...
   success =  mtcs_recog_apply_change_group/*!apply_change_group*/(mtcsRecog);
   gcc_assert (success);
}
...
static void cse_change_cc_mode_insns (MtcsCse *self,rtx_insn *start, rtx_insn *end, rtx newreg)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRtlanal *mtcsRtlanal=mtcs_target_get_rtlanal(mtcsTarget);
   ...
   for (insn = start; insn != end; insn = NEXT_INSN (insn)){
      if (! INSN_P (insn))
         continue;
      if (mtcs_rtlanal_reg_set_p/*!reg_set_p*/(mtcsRtlanal,newreg, insn))
         return;
      cse_change_cc_mode_insn(self,insn, newreg);
   }
}
...
static machine_mode cse_cc_succs (MtcsCse *self,basic_block bb, basic_block orig_bb, rtx cc_reg, rtx cc_src,
         bool can_change_mode)
{
   ...
   FOR_EACH_EDGE (e, ei, bb->succs){
      ...
      for (insn = BB_HEAD (e->dest); insn != end; insn = NEXT_INSN (insn)){
         ...
         && REGNO (SET_DEST (set)) == REGNO (cc_reg)){
            ...
            XEXP (SET_SRC (set), 1))){
               comp_mode = mtcsTarget/*!targetm.cc_modes_compatible*/->cc_modes_compatible(mtcsTarget,mode, set_mode);
               if (comp_mode != VOIDmode   && (can_change_mode || comp_mode == mode))
                  found = true;
            }
            ...
            if (found){
               ...
               if (insn_count < ARRAY_SIZE (insns)){
                  ...
                  if (mode != comp_mode){
                     ...
                     mtcs_rtl_put_mode/*!PUT_MODE*/(mtcsRTL,cc_src, mode);
                  }
               }else{
                  if (set_mode != mode){
         ...
         farther.  */
         if (mtcs_rtlanal_reg_set_p/*!reg_set_p*/(mtcsRtlanal,cc_reg, insn))
            break;
      }//end    for (insn = BB_HEAD (e->dest); insn != end; insn = NEXT_INSN (insn)){
      ...
      if (insn == end){
         ...
         submode = cse_cc_succs(self,e->dest, orig_bb, cc_reg, cc_src, false);
         if (submode != VOIDmode){
   ...
   }//end    FOR_EACH_EDGE (e, ei, bb->succs){
      ...
      if (modes[i] != mode){
         ...
         if (! newreg) {
            if (GET_MODE (cc_reg) == mode)
               ...
               newreg = mtcs_rtl_gen_rtx_REG/*!gen_rtx_REG*/(mtcsRTL,mode, REGNO (cc_reg));
         }
         cse_change_cc_mode_insns(self,NEXT_INSN (insns[i]), last_insns[i],newreg);
      }

      self->cse_cfg_altered |= mtcs_cfg_rtl_delete_insn_and_edges/*!delete_insn_and_edges*/(mtcsCfgRtl,insns[i]);
   }
...
static void cse_condition_code_reg (MtcsCse *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsRTL *mtcsRTL=mtcs_target_get_rtl(mtcsTarget);
   ...
   if (! mtcsTarget/*!targetm.fixed_condition_code_regs*/->fixed_condition_code_regs(mtcsTarget,&cc_regno_1, &cc_regno_2))
      return;

   cc_reg_1 = mtcs_rtl_gen_rtx_REG/*!gen_rtx_REG*/(mtcsRTL,mtcsMode->modes.M_CCmode, cc_regno_1);
   if (cc_regno_2 != INVALID_REGNUM)
      cc_reg_2 = mtcs_rtl_gen_rtx_REG/*!gen_rtx_REG*/(mtcsRTL,mtcsMode->modes.M_CCmode, cc_regno_2);
   else
      cc_reg_2 = NULL_RTX;
   ...
   FOR_EACH_BB_FN (bb, cfun){
      ...
      if (mtcs_rtlanal_reg_referenced_p/*!reg_referenced_p*/(mtcsRtlanal,cc_reg_1, PATTERN (last_insn)))
         cc_reg = cc_reg_1;
      else if (cc_reg_2 && mtcs_rtlanal_reg_referenced_p/*!reg_referenced_p*/(mtcsRtlanal,cc_reg_2, PATTERN (last_insn)))
         cc_reg = cc_reg_2;
      else
         continue;
      ...
      cc_src_insn = NULL;
      cc_src = NULL_RTX;
      for (insn = PREV_INSN (last_insn); insn && insn != PREV_INSN (BB_HEAD (bb)); insn = PREV_INSN (insn)){
         rtx set;

         if (! INSN_P (insn))
            continue;
         set = single_set (insn);
         if (set  && REG_P (SET_DEST (set))  && REGNO (SET_DEST (set)) == REGNO (cc_reg)){
            cc_src_insn = insn;
            cc_src = SET_SRC (set);
            break;
         }else if (mtcs_rtlanal_reg_set_p/*!reg_set_p*/(mtcsRtlanal,cc_reg, insn))
            break;
      }
      ...
      if (mtcs_rtlanal_modified_between_p/*!modified_between_p*/(mtcsRtlanal,cc_src, cc_src_insn, NEXT_INSN (last_insn)))
         continue;
      ...
      mode = cse_cc_succs(self,bb, bb, cc_reg, cc_src, true);
      if (mode != VOIDmode){
         gcc_assert (mode == GET_MODE (cc_src));
         if (mode != orig_mode){
            rtx newreg = mtcs_rtl_gen_rtx_REG/*!gen_rtx_REG*/(mtcsRTL,mode, REGNO (cc_reg));

            cse_change_cc_mode_insn(self,cc_src_insn, newreg);

            /* Do the same in the following insns that use the
            current value of CC_REG within BB.  */
            cse_change_cc_mode_insns(self,NEXT_INSN (cc_src_insn), NEXT_INSN (last_insn),newreg);
         }
      }
   }
}
...
static unsigned int rest_of_handle_cse (MtcsCse *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsFunc *mtcsFunc=mtcs_target_get_func(mtcsTarget);
   ...
   n_debug("mtcscse.c rest_of_handle_cse 00 cse_not_expected:%d\n",cse_not_expected);
   tem = cse_main(self,mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData), mtcs_func_max_reg_num/*!max_reg_num*/(mtcsFunc));
   n_debug("mtcscse.c rest_of_handle_cse 11 tem:%d\n",tem);
   testprint(cfun);
   /* If we are not running more CSE passes, then we are no longer
   expecting CSE to be run.  But always rerun it in a cheap mode.  */
   cse_not_expected = !mtcsOptionsItem->x_flag_rerun_cse_after_loop && !mtcsOptionsItem->x_flag_gcse;

   if (tem == 2){
      mtcs_dojump_rebuild_jump_labels/*!rebuild_jump_labels*/(mtcsDojump,mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData));
      self->cse_cfg_altered |= mtcs_cfg_cleanup_cleanup_cfg/*!cleanup_cfg*/(mtcsCfgCleanup,CLEANUP_CFG_CHANGED);
   }else if (tem == 1 || mtcsOptionsItem->x_optimize > 1)
      self->cse_cfg_altered |= mtcs_cfg_cleanup_cleanup_cfg/*!cleanup_cfg*/(mtcsCfgCleanup,0);

   return 0;
}

/* Run second CSE pass after loop optimizations.  */
static unsigned int rest_of_handle_cse2 (MtcsCse *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsFunc *mtcsFunc=mtcs_target_get_func(mtcsTarget);
   ...
   bypassed safely.  */
   cse_condition_code_reg(self);

   mtcs_cse_delete_trivially_dead_insns/*!delete_trivially_dead_insns*/(self,
   mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData), mtcs_func_max_reg_num/*!max_reg_num*/(mtcsFunc));

   if (tem == 2){
      mtcs_dojump_rebuild_jump_labels/*!rebuild_jump_labels*/(mtcsDojump,mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData));
      self->cse_cfg_altered |= mtcs_cfg_cleanup_cleanup_cfg/*!cleanup_cfg*/(mtcsCfgCleanup,CLEANUP_CFG_CHANGED);
   }else if (tem == 1 || self->cse_cfg_altered)
      self->cse_cfg_altered |= mtcs_cfg_cleanup_cleanup_cfg/*!cleanup_cfg*/(mtcsCfgCleanup,0);
...
/* Run second CSE pass after loop optimizations.  */
static unsigned int rest_of_handle_cse_after_global_opts (MtcsCse *self)
{
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsFunc *mtcsFunc=mtcs_target_get_func(mtcsTarget);
   ...
   save_cfj = mtcsOptionsItem->x_flag_cse_follow_jumps;
   mtcsOptionsItem->x_flag_cse_follow_jumps = 0;

   mtcs_dojump_rebuild_jump_labels/*!rebuild_jump_labels*/(mtcsDojump,mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData));
   tem = cse_main(self,mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData), mtcs_func_max_reg_num/*!max_reg_num*/(mtcsFunc));
   self->cse_cfg_altered |= mtcs_cfg_rtl_purge_all_dead_edges/*!purge_all_dead_edges*/(mtcsCfgRtl);
   delete_trivially_dead_insns (mtcs_rtl_data_get_insns/*!get_insns*/(mtcsRtlData), mtcs_func_max_reg_num/*!max_reg_num*/(mtcsFunc));

   cse_not_expected = !mtcsOptionsItem->x_flag_rerun_cse_after_loop;
   ...
   mtcsOptionsItem->x_flag_cse_follow_jumps = save_cfj;
   return 0;
}
//
//namespace {
...
int mtcs_cse_delete_trivially_dead_insns (MtcsCse *self,rtx_insn *insns, int nreg)
{
   ...
   int ndead = 0;
   n_debug("mtcscse.c mtcs_cse_delete_trivially_dead_insns 00 nreg:%d insns:%p\n",nreg,insns);
   mtcs_print_rtl_single(stderr,insns);
   /* First count the number of times each register is used.  */
   if (MAY_HAVE_DEBUG_BIND_INSNS){
      counts = XCNEWVEC (int, nreg * 3);
      for (insn = insns; insn; insn = NEXT_INSN (insn))
         if (DEBUG_BIND_INSN_P (insn)){
            n_debug("mtcscse.c mtcs_cse_delete_trivially_dead_insns 11 nreg:%d insns:%p\n",nreg,insn);
            mtcs_print_rtl_single(stderr,insn);

            count_reg_usage(self,INSN_VAR_LOCATION_LOC (insn), counts + nreg,NULL_RTX, 1);
            TREE_VISITED (INSN_VAR_LOCATION_DECL (insn)) = 0;
         }else if (INSN_P (insn)){
            n_debug("mtcscse.c mtcs_cse_delete_trivially_dead_insns 22 nreg:%d insns:%p\n",nreg,insn);
            mtcs_print_rtl_single(stderr,insn);

            count_reg_usage(self,insn, counts, NULL_RTX, 1);
            CountStoresData userData={self,(void*)(counts + nreg * 2)};
            mtcs_rtlanal_note_stores/*!note_stores*/(mtcsRtlanal,insn, count_stores_cb, &userData/*!counts + nreg * 2*/);
         }
      ...
      for (insn = insns; insn; insn = NEXT_INSN (insn))
         if (INSN_P (insn)){
            n_debug("mtcscse.c mtcs_cse_delete_trivially_dead_insns 33 nreg:%d insns:%p \n",nreg,insn);
            mtcs_print_rtl_single(stderr,insn);
            count_reg_usage(self,insn, counts, NULL_RTX, 1);
         }
   ...
   n_debug("mtcscse.c mtcs_cse_delete_trivially_dead_insns 44 nreg:%d insns:%p reload_completed:%d\n",nreg,insns,reload_completed);

   /* Pseudo PIC register should be considered as used due to possible
   new usages generated.  */
   if (!reload_completed
   && mtcsRTL->x_pic_offset_table_rtx/*!pic_offset_table_rtx*/
   ...
   mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg))
      counts[REGNO (mtcsRTL->x_pic_offset_table_rtx/*!pic_offset_table_rtx*/)]++;
   ...
   for (insn = mtcs_rtl_data_get_last_insn/*!get_last_insn*/(mtcsRtlData); insn; insn = prev){
      ...
      live_insn = insn_live_p(self,insn, counts);

      /* If this is a dead insn, delete it and show registers in it aren't
      being used.  */
      n_debug("mtcscse.c mtcs_cse_delete_trivially_dead_insns 55 nreg:%d insn:%p live_insn:%d %d\n",
               nreg,insn,live_insn ,dbg_cnt (delete_trivial_dead));
      mtcs_print_rtl_single(stderr,insn);
      if (! live_insn && dbg_cnt (delete_trivial_dead)){
         if (DEBUG_INSN_P (insn)){
            ...
            && is_dead_reg(self,SET_DEST (set), counts)
            ...
            && asm_noperands (PATTERN (insn)) < 0){
               ...
               n_debug("mtcscse.c mtcs_cse_delete_trivially_dead_insns 66 nreg:%d insn:%p dval:%p\n",
                     nreg,insn,dval);

               /* Emit a debug bind insn before the insn in which
               reg dies.  */
               bind_var_loc = gen_rtx_VAR_LOCATION (GET_MODE (SET_DEST (set)),
                                 DEBUG_EXPR_TREE_DECL (dval),SET_SRC (set),VAR_INIT_STATUS_INITIALIZED);
               count_reg_usage(self,bind_var_loc, counts + nreg, NULL_RTX, 1);

               bind = emit_debug_insn_before (bind_var_loc, insn);
               mtcs_dfscan_df_insn_rescan/*!df_insn_rescan*/(mtcsDfscan,bind);
         ...
         } else if (DEBUG_BIND_INSN_P (insn) && !TREE_VISITED (INSN_VAR_LOCATION_DECL (insn))){
   ...
   if (MAY_HAVE_DEBUG_BIND_INSNS){
      n_debug("mtcscse.c mtcs_cse_delete_trivially_dead_insns 77 nreg:%d insns:%p ndead:%d\n",
                      nreg,insns,ndead);
      for (insn = mtcs_rtl_data_get_last_insn/*!get_last_insn*/(mtcsRtlData); insn; insn = PREV_INSN (insn))
         if (DEBUG_BIND_INSN_P (insn)){
            ...
            if (is_dead_debug_insn(self,INSN_VAR_LOCATION_LOC (insn),counts, replacements, &seen_repl)){
               INSN_VAR_LOCATION_LOC (insn) = gen_rtx_UNKNOWN_VAR_LOC ();
               mtcs_dfscan_df_insn_rescan/*!df_insn_rescan*/(mtcsDfscan,insn);
            }else if (seen_repl){
               CountStoresData userData={self,(void*)replacements};
               INSN_VAR_LOCATION_LOC (insn)
               = mtcs_simplify_rtx_simplify_replace_fn_rtx/*!simplify_replace_fn_rtx*/(mtcsSimplifyRtx,
                     INSN_VAR_LOCATION_LOC (insn), NULL_RTX, replace_dead_reg_cb,&userData/*!replacements*/);
               mtcs_dfscan_df_insn_rescan/*!df_insn_rescan*/(mtcsDfscan,insn);
            }
         }
      free (replacements);
   }
   n_debug("mtcscse.c mtcs_cse_delete_trivially_dead_insns 88 nreg:%d insns:%p ndead:%d\n",
                   nreg,insns,ndead);
   if (dump_file && ndead)
...
{
     MtcsCse *self = n_slice_alloc0 (sizeof(MtcsCse));
     mtcs_component_set_mode((MtcsComponent *)self,mtcsMode);
     mtcsCseInit(self);
     return self;
}

/*****************以下是其于mtcscse的rtl pass--------------------------------*/

//原型 NEXT_PASS (pass_cse, 1); RTL_PASS cse.cc cse1 n 有条件执行 optimize > 0 rest_of_handle_cse
static nboolean pass_cse1_gate_cb(MtcsPass *mtcsPass,function *fun)
{
   MtcsPassCse1 *self=(MtcsPassCse1 *)mtcsPass;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsOptions *mtcsOptions=mtcs_target_get_options(mtcsTarget);
   MtcsOptionsItem *mtcsOptionsItem=mtcsOptions->global_options;
...
static nuint pass_cse1_execute_cb(MtcsPass *mtcsPass,function *func)
{
   MtcsPassCse1 *self=(MtcsPassCse1 *)mtcsPass;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCse *mtcsCse=mtcs_target_get_cse(mtcsTarget);
   return rest_of_handle_cse(mtcsCse);
}


static void mtcsPassCse1Init(MtcsPassCse1 *self)
{
   MtcsPass *mtcsPass=(MtcsPass *)self;
   mtcsPass->execute =pass_cse1_execute_cb;
   mtcsPass->gate =pass_cse1_gate_cb;
...
{
   MtcsPassCse1 *self = n_slice_alloc0 (sizeof(MtcsPassCse1));
   mtcs_component_set_mode((MtcsComponent*)self,mtcsMode);
   mtcs_pass_init((MtcsPass *)self,RTL_PASS,"cse1");
   mtcsPassCse1Init(self);
   return self;
}

//原型 NEXT_PASS (pass_cse2, 1); RTL_PASS cse.cc cse2 n 有条件执行 optimize > 0 && flag_rerun_cse_after_loop; rest_of_handle_cse2
static nboolean pass_cse2_gate_cb(MtcsPass *mtcsPass,function *fun)
{
   MtcsPassCse1 *self=(MtcsPassCse1 *)mtcsPass;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsOptions *mtcsOptions=mtcs_target_get_options(mtcsTarget);
   MtcsOptionsItem *mtcsOptionsItem=mtcsOptions->global_options;
   return mtcsOptionsItem->x_optimize>0 && mtcsOptionsItem->x_flag_rerun_cse_after_loop;
}

static nuint pass_cse2_execute_cb(MtcsPass *mtcsPass,function *func)
{
   MtcsPassCse1 *self=(MtcsPassCse1 *)mtcsPass;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCse *mtcsCse=mtcs_target_get_cse(mtcsTarget);
   return rest_of_handle_cse2(mtcsCse);
}


static void mtcsPassCse2Init(MtcsPassCse2 *self)
{
   MtcsPass *mtcsPass=(MtcsPass *)self;
   mtcsPass->execute =pass_cse2_execute_cb;
   mtcsPass->gate =pass_cse2_gate_cb;
...
{
   MtcsPassCse2 *self = n_slice_alloc0 (sizeof(MtcsPassCse2));
   mtcs_component_set_mode((MtcsComponent*)self,mtcsMode);
   mtcs_pass_init((MtcsPass *)self,RTL_PASS,"cse2");
   mtcsPassCse2Init(self);
   return self;
}
...
static nboolean pass_cse_local_gate_cb(MtcsPass *mtcsPass,function *fun)
{
   MtcsPassCse1 *self=(MtcsPassCse1 *)mtcsPass;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsOptions *mtcsOptions=mtcs_target_get_options(mtcsTarget);
   MtcsOptionsItem *mtcsOptionsItem=mtcsOptions->global_options;
   return mtcsOptionsItem->x_optimize>0  && flag_rerun_cse_after_global_opts;
}

static nuint pass_cse_local_execute_cb(MtcsPass *mtcsPass,function *func)
{
   MtcsPassCse1 *self=(MtcsPassCse1 *)mtcsPass;
   MtcsMode *mtcsMode=MTCS_GET_MODE_OBJECT(self);
   MtcsTarget *mtcsTarget=(MtcsTarget *)mtcsMode->target;
   MtcsCse *mtcsCse=mtcs_target_get_cse(mtcsTarget);
   return rest_of_handle_cse_after_global_opts(mtcsCse);
}

static void mtcsPassCseLocalInit(MtcsPassCseLocal *self)
{
   MtcsPass *mtcsPass=(MtcsPass *)self;
   mtcsPass->execute =pass_cse_local_execute_cb;
   mtcsPass->gate =pass_cse_local_gate_cb;
...
{
   MtcsPassCseLocal *self = n_slice_alloc0 (sizeof(MtcsPassCseLocal));
   mtcs_component_set_mode((MtcsComponent*)self,mtcsMode);
   mtcs_pass_init((MtcsPass *)self,RTL_PASS,"cse_local");
   mtcsPassCseLocalInit(self);
   return self;
}
   ...
   for (ref = refs; ref; ref = DF_REF_NEXT_REG (ref)){
     // gcc_assert (!DF_REF_IS_REG_MARKED (ref));
      ...
      if(DF_REF_IS_REG_MARKED (ref))
         n_debug("mtcscse.c DF_REF_IS_REG_MARKED (ref)=true 错误\n");

      /* If there are no def-use or use-def chains, make sure that all
      of the chains are clear.  */
      n_debug("mtcscse.c df_reg_chain_mark 00 ref:%p flags:%d is_def:%d is_eq_use:%d df_chain:%p regno:%d %d\n",
...
static void dfScanVerify (struct function *fn)
{
   ...
   if (!df)
      return;
   n_debug("mtcscse.c dfScanVerify 00 DF_REG_SIZE (df):%d\n",DF_REG_SIZE (df));
   FOR_ALL_BB_FN (bb, cfun){
      struct df_scan_bb_info *bbinfo= df_scan_get_bb_info (bb->index);
      df_ref adef=bbinfo->artificial_defs;
      n_debug("mtcscse.c dfScanVerify 11 flags bb:%p bb->index:%d adef:%p\n",bb,bb->index,adef);
      if(adef){
         struct df_artificial_ref  dfaref=adef->artificial_ref;
         struct df_base_ref base =dfaref.base;
         n_debug("mtcscse.c dfScanVerify 22 bb->index:%d flags:%d def:%p\n",bb->index,base.flags,adef);
      }
   }
   for (i = 0; i < DF_REG_SIZE (df); i++){
      fprintf(stderr,"mtcscse.c dfScanVerify --- i:%d %d\n",i,DF_REG_DEF_COUNT (i));
