...
ClassInfo *class_info_new()
{
   ...
   self->isFinal=FALSE;
   self->file=NULL;
   self->declLoc =0;//声明的位置  class A
   self->implLoc =0; //实现的位置 impl$ A
   return self;
}
...
{
   ClassInfo *parentInfo=class_mgr_get_class_info(class_mgr_get(),self->parentName.sysName);
  // printf("class_info_check_generic_decl -- %s parentInfo:%p\n",self->className.sysName,parentInfo);
   if(!parentInfo)
      return TRUE;
   if(class_info_is_generic_class(parentInfo)){
      //父类本身的声明泛型，子类为父类设定的泛型,是否匹配
      if(self->parentGenericModel==NULL){
         error_at(self->parentLoc,"父类没有设定泛型模型。例如:class$ A<E,F> extends$ B<F,int>");
         return FALSE;
      }
      int parentDeclCount = generic_model_get_count(parentInfo->genericModel);
      int parentDefineCount = generic_model_get_count(self->parentGenericModel);
      if(parentDeclCount!=parentDefineCount){
         error_at(self->parentLoc,"子类为父类设定的泛型单元数量与父类声明的不符。");
         return FALSE;
      }
      int i,j;
      for(i=0;i<parentDefineCount;i++){
         GenericUnit *parentUnit=generic_model_get(self->parentGenericModel,i);
         if(parentUnit->isDefine) //有具体类型
            continue;
         //父类定义的泛型单元
         nboolean find=FALSE;
         for(j=0;j<generic_model_get_count(self->genericModel);j++){
            GenericUnit *childUnit=generic_model_get(self->genericModel,j);
            if(generic_unit_equal(childUnit,parentUnit)){
               find=TRUE;
               break;
            }
         }
         if(!find){
            error_at(parentUnit->loc,"父类的泛型单元%qs在子类中没有声明。",generic_unit_tostring(parentUnit));
            return FALSE;
         }
      }
   }else{
      if(self->parentGenericModel!=NULL){
         error_at(self->parentLoc,"不是泛型类。");
         return FALSE;
      }
   }
   return TRUE;
...
nboolean   class_info_have_iface(ClassInfo *self,ClassName *ifaceName)
{
   if(self->ifaceCount==0)
      return FALSE;
   int i;
   for(i=0;i<self->ifaceCount;i++){
      if(!strcmp(self->ifaces[i].sysName,ifaceName->sysName))
         return TRUE;
   }
   return FALSE;
}
