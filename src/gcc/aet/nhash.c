...
static inline nuint lookupNodeByHash(NHashTable *hash_table,nconstpointer  key,nuint hash, nuint *hash_return)
{
  nuint node_index;
  nuint node_hash;
  nuint hash_value;
  ...
  *hash_return = hash_value;
  node_index = n_hash_table_hash_to_index (hash_table, hash_value);
  node_hash = hash_table->hashes[node_index];
  //printf("lookupNodeByHash 00 is key:%p node_hash:%d node_index:%d hash_value:%d\n",key,node_hash,node_index,hash_value);
  while (!HASH_IS_UNUSED (node_hash)){
      /* We first check if our full hash values
       * are equal so we can avoid calling the full-blown
       * key equality function in most cases.
       */
     // printf("lookupNodeByHash 11 is key:%p node_hash:%d node_index:%d hash_value:%d\n",key,node_hash,node_index,hash_value);

      if (node_hash == hash_value){
          npointer node_key = n_hash_table_fetch_key_or_value (hash_table->keys, node_index, hash_table->have_big_keys);
          //printf("lookupNodeByHash 22 is key:%p node_hash:%d node_index:%d %p\n",key,node_hash,node_index,hash_table->key_equal_func);
          if (hash_table->key_equal_func){
              if (hash_table->key_equal_func(node_key, key))
                return node_index;
          }else if (node_key == key){
...
//通过外部的hash值和key的比较功能，获取value,比forearch 快
npointer    n_hash_table_lookup_by_hash(NHashTable *hash_table,nconstpointer key,nuint hash)
{
     nuint node_index;
     nuint node_hash;
     n_return_val_if_fail (hash_table != NULL, NULL);
     node_index = lookupNodeByHash (hash_table, key,hash, &node_hash);
     return HASH_IS_REAL (hash_table->hashes[node_index])
       ? n_hash_table_fetch_key_or_value (hash_table->values, node_index, hash_table->have_big_values): NULL;
}
