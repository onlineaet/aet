...
//原型 c_parser_type_name c-parser.h c-parser.cc
static struct c_type_name * parserTypeName (AetParser *self, bool alignas_ok = false);
//4个方法运行在 c_parser_declspecs 中
static int      aet_parser_c_parser_declspecs_c_id (AetParser *self,struct c_typespec *t,tree ident);
static void     aet_parser_c_parser_declspecs_enum_dot (AetParser *self,struct c_typespec *t);
//在 c_parser_declspecs中解析 AET 关键字
static nboolean aet_parser_c_parser_declspecs_keyword (AetParser *self,location_t loc,
                        bool typespec_ok,struct c_declspecs *specs,bool *attrs_ok,bool *seen_type);
//原调用 parser_build_binary_op 的地方改为调用aet_parser_build_binary_op
//可以为泛型块函数中的变量进行转化
struct c_expr aet_parser_build_binary_op (location_t location, enum tree_code code,
         struct c_expr arg1, struct c_expr arg2);
//zclei
static ClassParser *classParser=NULL;
static ClassImpl   *classImpl  =NULL;
static AetParser   *aetParser = NULL;
static MtcsParser  *mtcsParser = NULL;
...
//typedef struct _Abc{
//typedef struct 是关键字在 460行标为关键字，然后跳到out    if (C_IS_RESERVED_WORD (token->value)) 判断是不是keyword
//_Abc是标识符 符号类型 是CP_NAME  符号类型是CP_NAME 还要分标识符类型 普通的分为  token->id_kind=C_ID_ID 、然后走到471号
//"{"的符号类型是 CPP_OPEN_BRACE 也是标识符，但标识符类型是 C_ID_NONE
static void c_lex_one_token (c_parser *parser, c_token *token, bool raw = false)
{
   if (raw || vec_safe_length (parser->raw_tokens) == 0){
      token->type = c_lex_with_flags (&token->value, &token->location,
      &token->flags, (parser->lex_joined_string ? 0 : C_LEX_STRING_NO_JOIN));
      ...
      case CPP_NAME:
      {
         ...
         if (C_IS_RESERVED_WORD (token->value)){
            ...
            if (rid_code == RID_CXX_COMPAT_WARN){
               warning_at (token->location, OPT_Wc___compat, "identifier %qE conflicts with C++ keyword", token->value);
            }else if (rid_code >= RID_FIRST_ADDR_SPACE  && rid_code <= RID_LAST_ADDR_SPACE){
               addr_space_t as;
               ...
               if (parser->objc_pq_context){
                  ...
                  token->keyword = rid_code;
                  break;
               }else if (parser->objc_could_be_foreach_context   && rid_code == RID_IN){
            ...
            */
            }else if (c_dialect_objc ()  && (OBJC_IS_AT_KEYWORD (rid_code) || OBJC_IS_CXX_KEYWORD (rid_code))){
         ...
         if (decl){
            ...
            declarations.  */
            if (objc_interface_decl   && (!objc_force_identifier || global_bindings_p ())) {
...
static c_token *c_parser_peek_nth_token_raw (c_parser *parser, unsigned int n)
{
...
static inline bool c_parser_next_tokens_start_typename (c_parser *parser, enum c_lookahead_kind la)
{
...
static bool c_token_starts_compound_literal (c_token *token)
{
   switch (token->type){
      case CPP_KEYWORD:
...
static bool c_token_is_qualifier (c_token *token)
{
   switch (token->type){
      case CPP_NAME:
...
/* Return true if the next token from PARSER is a type qualifier,
   false otherwise.  */
static inline bool c_parser_next_token_is_qualifier (c_parser *parser)
{
   c_token *token = c_parser_peek_token (parser);
...
static bool c_token_starts_declspecs (c_token *token)
{
   switch (token->type)   {
      case CPP_NAME:
         switch (token->id_kind){
      ...
      case CPP_KEYWORD:
         ...
         //      printf("判断是否是声明说明符 keyowrd:%d RID_TYPEDEF:%d %s %s %d\n",
         //              token->keyword,RID_TYPEDEF, __FILE__, __FUNCTION__, __LINE__);
...
/* Return true if TOKEN can start declaration specifiers (not
   ...
   otherwise.  */
static bool c_token_starts_declaration (c_token *token)
{
   if (c_token_starts_declspecs (token)  || token->keyword == RID_STATIC_ASSERT)
      return true;
   else
      return false;
}
...
static inline void c_parser_set_source_position_from_token (c_token *token)
{
...
static bool c_parser_peek_conflict_marker (c_parser *parser, enum cpp_ttype tok1_kind,
                location_t *out_loc)
{
...
static bool c_parser_error_richloc (c_parser *parser, const char *gmsgid,
...
struct aet_matching_paren_traits
{
...
const char * const aet_matching_paren_traits::open_gmsgid = "expected %<(%>";
const char * const aet_matching_paren_traits::close_gmsgid = "expected %<)%>";

/* "matching_parens" is a token_pair<T> class for tracking matching
   pairs of parentheses.  */

typedef token_pair<aet_matching_paren_traits> matching_parens;

/* Traits for token_pair<T> for tracking matching pairs of braces.  */

struct aet_matching_brace_traits
{
...
const char * const aet_matching_brace_traits::open_gmsgid = "expected %<{%>";
const char * const aet_matching_brace_traits::close_gmsgid = "expected %<}%>";

/* "matching_braces" is a token_pair<T> class for tracking matching
   pairs of braces.  */

typedef token_pair<aet_matching_brace_traits> matching_braces;

/* Get a description of the matching symbol to TYPE e.g. "(" for
   CPP_CLOSE_PAREN.  */

static const char *get_matching_symbol (enum cpp_ttype type)
{
   switch (type){
      default:
         gcc_unreachable ();
...
static bool c_parser_require_keyword (c_parser *parser,
           enum rid keyword,
           const char *msgid)
{
...
static bool c_parser_skip_to_closing_brace (c_parser *parser)
{
   unsigned nesting_depth = 0;
...
static void c_parser_skip_to_end_of_block_or_statement (c_parser *parser,
   ...
   while (true){
      ...
      switch (token->type){
         ...
         case CPP_CLOSE_BRACE:
            ...
            if ((nesting_depth == 0 || --nesting_depth == 0) && (!metadirective_p || bracket_depth <= 0)){
               c_parser_consume_token (parser);
               goto finished;
            }
            break;
...
static inline int disable_extension_diagnostics (void)
{
  int ret = (pedantic
...
static inline void restore_extension_diagnostics (int flags)
{
...
static tree c_parser_declaration_or_fndef (c_parser *, bool, bool, bool,
                  ...
                  bool * = NULL);
static bool c_parser_handle_statement_omp_attributes (c_parser *, tree &, bool *);
...
static struct c_expr c_parser_braced_init (c_parser *, tree, bool, struct obstack *, tree);
...
static void c_parser_while_statement (c_parser *, bool, unsigned short, bool,   bool *, tree);
static void c_parser_do_statement (c_parser *, bool, unsigned short, bool, tree);
static void c_parser_for_statement (c_parser *, bool, unsigned short, bool, bool *, tree);
...
static struct c_expr c_parser_postfix_expression_after_paren_type (c_parser *,
                           struct c_declspecs *, struct c_type_name *,location_t);
...
static bool c_parser_omp_next_tokens_can_be_canon_loop (c_parser *, enum tree_code, bool);
static tree c_parser_omp_loop_nest (c_parser *, bool *);
static tree c_parser_omp_for_loop (location_t, c_parser *, enum tree_code, tree, tree *, bool *);
...
static void c_parser_translation_unit (c_parser *parser)
{
   if (c_parser_next_token_is (parser, CPP_EOF)){
      pedwarn (c_parser_peek_token (parser)->location, OPT_Wpedantic,"ISO C forbids an empty translation unit");
   }else{
      void *obstack_position = obstack_alloc (&parser_obstack, 0);
      mark_valid_location_for_stdc_pragma (false);
      int testcount=0;
      do{
        // printf("外部声明  00 --- count:%d  %s, %s, %d \n", testcount,__FILE__, __FUNCTION__, __LINE__);
         ggc_collect ();
         c_parser_external_declaration (parser);
         //printf("外部声明  11 --- count:%d  %s, %s, %d \n", testcount,__FILE__, __FUNCTION__, __LINE__);
         obstack_free (&parser_obstack, obstack_position);
         testcount++;
      }while (c_parser_next_token_is_not (parser, CPP_EOF));
     // printf("外部声明结束 --- 并处理static count:%d  %s, %s, %d \n", testcount++,__FILE__, __FUNCTION__, __LINE__);
      class_impl_compile_over(classImpl); //zclei 10.4.0
   }
   ...
   if (vec_safe_length (current_omp_declare_target_attribute)){
      c_omp_declare_target_attr   a = current_omp_declare_target_attribute->pop ();
      if (!errorcount)
         error ("%qs without corresponding %qs", a.device_type >= 0 ? "#pragma omp begin declare target"
   ...
   if (vec_safe_length (current_omp_begin_assumes)){
      if (!errorcount)
         error ("%qs without corresponding %qs","#pragma omp begin assumes", "#pragma omp end assumes");
...
/* Parse an external declaration (C90 6.7, C99 6.9, C11 6.9).
   ...
   external-declaration:
     ...
     @end
*/
static void c_parser_external_declaration (c_parser *parser)
{
   int ext;
      ...
      case CPP_KEYWORD:
         switch (c_parser_peek_token (parser)->keyword){
            ...
            case RID_AET_REALIZE_CLASS:
               ...
               class_impl_parser(classImpl);//zclei
               break;
...
decl_or_fndef:
        ...
        // printf("处理关键词 decl_or_fndef:00 --- %s, %s, %d\n", __FILE__, __FUNCTION__, __LINE__);
         c_parser_declaration_or_fndef (parser, true, true, true, false, true,false);
         break;
   }
}

static void c_parser_handle_directive_omp_attributes (tree &, vec<c_token> *&, vec<c_token> *);
...
static void add_debug_begin_stmt (location_t loc)
{
...
static tree handle_assume_attribute (location_t here, tree attrs, bool nested)
{
  if (nested)
...
static tree c_parser_declaration_or_fndef (c_parser *parser, bool fndef_ok,
                ...
                bool *fallthru_attr_p /* = NULL */)
{
   ...
   if(tok->type==CPP_KEYWORD && (tok->keyword==RID_AET_PUBLIC
      ...
      if(class_parser_add_include(classParser))
         return result;
   ...
   if(aetParser->isAet){
      if(c_parser_next_token_is (parser, CPP_LESS)){
         funcGenericModel= generic_model_new (TRUE,GEN_FROM_DECLARATION_OR_FNDEF);
         classImpl->currentFuncModel = funcGenericModel;//generic_func_parameter_declaration 需要
         n_debug("分析声明说明符 --- 在声明说明符之前有一个泛型函数定义或声明---- %p\n",funcGenericModel);
      }
      if(c_parser_next_token_is (parser, CPP_COMPL)){
         n_info("进这里处理析构函数====finalize class:%s",classImpl->className->sysName);
         class_finalize_parser(classImpl->classFinalize,classImpl->className);
      }else{
         if(c_parser_next_token_is (parser, CPP_NAME)){
            c_token *token=c_parser_peek_token (parser);
         ...
         class_ctor_parser_constructor(classImpl->classCtor,classImpl->className);
      }
   }
   //zclei
   if(c_parser_next_token_is_keyword (parser,RID_AET_PACKAGE)){
      class_package_parser(classParser->classPackage);
      return;
   }
   if(c_parser_next_token_is_keyword (parser,RID_AET_GOTO)){
      genericBlockFuncDefine=class_parser_goto(class_parser_get(),start_attr_ok,NULL);
      if(!genericBlockFuncDefine)
         return;
   }
    //进入编译泛型块函数，把泛型声明替换成泛型定义
   if(aet_parser_is_generic_state(aetParser)){
      if(c_parser_next_token_is (parser, CPP_NAME)){
         c_token *token=c_parser_peek_token (parser);
         //printf("是一xxxxxTTT个泛型吗------ %s\n",IDENTIFIER_POINTER(token->value));
         if(generic_util_valid_id(token->value)){
            //printf("是一xxxxxTTT个泛型吗----xxxxxxx-- %s\n",IDENTIFIER_POINTER(token->value));
            const char *genStr=IDENTIFIER_POINTER(token->value);
            c_parser_consume_token (parser);
            generic_parser_replace(generic_parser_get(),genStr);
         }
      }
   }
   ...
   && (!nested || !lookup_name (c_parser_peek_token (parser)->value))){
      ...
      n_debug("分析声明说明符 11：一般不到这里 name:%s nested:%d", IDENTIFIER_POINTER(name),nested);
      aet_print_token(c_parser_peek_token (parser));
      ...
      if (tag_exists_p (RECORD_TYPE, name)){
         ...
         //zclei 解决函数定义有两个连续的mtcs修饰符问题  __device__ __host__ void testkernel() bug 015
         if(aetParser->isAet && mtcs_parser_is_attribute(mtcs_parser_get(),c_parser_peek_token (parser))){
            goto mtcs_label;
         }
         auto_diagnostic_group d;
   ...
   if (c_parser_nth_token_starts_std_attributes (parser, 1))
      have_attrs = true;
   n_debug("分析声明说明符 22： have_attrs:%d ", have_attrs);
   //在这里匹配的是public$  __global__  int   setData(void *data);
   if(mtcs_parser_is_attribute(mtcs_parser_get(),c_parser_peek_token (parser))){
      n_debug("分析声明说明符 是一个MTCS 属性");
      mtcs_parser_add_attribute(mtcs_parser_get());
   }
   // 分析声明说明符,也就是 struct 或 fun 函数声明语句中的符号“ int ”
   aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(aetParser, specs, true, true, start_attr_ok,
         true, true, start_attr_ok, true, cla_nonabstract_decl);
   if (parser->error){
      n_debug("分析声明说明符 出错了 22： have_attrs:%d ", have_attrs);
      c_parser_skip_to_end_of_block_or_statement (parser);
      return result;
   }
   if (nested && !specs->declspecs_seen_p){
      n_debug("分析声明说明符 出错了 22： nested:%d ", nested);
   ...
   if(aetParser->isAet && mtcs_parser_is_attribute(mtcs_parser_get(),c_parser_peek_token (parser))){
      n_debug("分析声明说明符 是一个MTCS 属性");
      //解决两个连续的MTCS修饰问题 __device__ __host__ void testkernel(
mtcs_label:
      mtcs_parser_add_attribute(mtcs_parser_get());
      aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(aetParser,specs, true, true, start_attr_ok,
            true, true, start_attr_ok, true, cla_nonabstract_decl);
   ...
   genericDefineModel=generic_impl_pop_generic_from_declspecs(generic_impl_get(),specs);//zclei 10.4.0
   if(genericDefineModel!=NULL){
      nboolean isClass=class_util_is_class(specs);
      if(!isClass){
   ...
   if (c_parser_next_token_is (parser, CPP_SEMICOLON)){
      n_debug("分析声明说明符 44:下一个是分号吗：是的： have_attrs:%d", have_attrs);
      bool handled_assume = false;
      if (specs->attrs
      && !nested
      && specs->typespec_kind == ctsk_none
      && c_parser_handle_statement_omp_attributes (parser, specs->attrs,NULL)){
         if (specs->attrs)
      ...
      if (specs->typespec_kind == ctsk_none  && lookup_attribute ("gnu", "assume", specs->attrs)){
         handled_assume = true;
         specs->attrs  = handle_assume_attribute (here, specs->attrs, nested);
      }
      if (any_auto_type_p)
         error_at (here, "%qs in empty declaration", auto_type_keyword);
      else if (specs->typespec_kind == ctsk_none && attribute_fallthrough_p (specs->attrs)){
         if (fallthru_attr_p != NULL)
            *fallthru_attr_p = true;
         if (nested){
            tree fn = build_call_expr_internal_loc (here, IFN_FALLTHROUGH, void_type_node, 0);
            add_stmt (fn);
         }else
            pedwarn (here, OPT_Wattributes,"%<fallthrough%> attribute at top level");
      }else if (empty_ok  && !(have_attrs && specs->non_std_attrs_seen_p)  && !handled_assume){
         if(class_parser_is_add_include(classParser)){
            c_parser_consume_token (parser);
            return result;
         }
         shadow_tag (specs);
      ...
      class_parser_complete_enum(classParser,specs,haveAccessControl,permission,NULL);//zclei 估计是枚举也可能不是 必须在add_stmt之前
   ...
   n_debug("分析声明说明符 55： have_attrs:%d", have_attrs);
   ...
   && !c_parser_next_token_is (parser, CPP_NAME)){
      ...
      parser->error = false;
      shadow_tag_warned (specs, 1);
      n_debug("分析声明说明符 66： have_attrs:%d", have_attrs);
      return result;
   }else if (c_dialect_objc () && !any_auto_type_p){
      /* Prefix attributes are an error on method decls.  */
      n_debug("分析声明说明符 77： have_attrs:%d", have_attrs);
      switch (c_parser_peek_token (parser)->type){
         case CPP_PLUS:
         case CPP_MINUS:
            ...
            if (specs->attrs){
               ...
               OPT_Wattributes,"prefix attributes are ignored for methods");
      ...
      ((deprecated)).
      */
      n_debug("分析声明说明符 88： have_attrs:%d", have_attrs);
      switch (c_parser_peek_token (parser)->keyword){
         case RID_AT_INTERFACE:
         {
            if (c_parser_objc_diagnose_bad_element_prefix (parser, specs))
         ...
         case RID_AT_IMPLEMENTATION:
         {
            ...
            if (specs->attrs){
               warning_at (c_parser_peek_token (parser)->location,
                     OPT_Wattributes, "prefix attributes are ignored for implementations");
               specs->attrs = NULL_TREE;
            }
            c_parser_objc_class_definition (parser, NULL_TREE);
            return result;
      ...
      warning_at (here, OPT_Wattributes, "%<assume%> attribute not followed by %<;%>");

   auto_vec<c_token> omp_declare_simd_attr_clauses;
   c_parser_handle_directive_omp_attributes (specs->attrs,
   omp_declare_simd_clauses, &omp_declare_simd_attr_clauses);
   pending_xref_error ();
   ...
   int testcount=0;
   //zclei 在这里加参数Abc *self
   if(aetParser->isAet && !genericBlockFuncDefine && nested==0){
      n_info("分析声明说明符 00aa:进入了impl Abc{};实现模式 如果是函数，要加入Abc *self： count:%d name:%s",
      testcount++,classImpl->className->sysName);
      nboolean ok=class_impl_add_self_to_param(classImpl);
      if(ok){
         ok=class_impl_add_static_to_declspecs(classImpl,here,specs);
         n_info("分析声明说明符 00cc:进入了impl Abc{};实现模式 如果是函数，加入static count:%d name:%s ok:%d",
         testcount++,classImpl->className->sysName,ok);
      }
   }
   while (true){
      n_debug("分析声明说明符 99: 在这里分析声明符： count:%d fndef_ok:%d", testcount++,fndef_ok);
      aet_print_token(c_parser_peek_token (parser));
      ...
      declarator = aet_parser_c_parser_declarator/*!c_parser_declarator*/(aetParser,
      specs->typespec_kind != ctsk_none, C_DTR_NORMAL, &dummy);
      if (declarator == NULL){
         n_debug("分析声明说明符 100： count:%d declarator:%p", testcount,declarator);
         if (omp_declare_simd_clauses)
            c_finish_omp_declare_simd (parser, NULL_TREE, NULL_TREE, omp_declare_simd_clauses);
         if (oacc_routine_data)
            c_finish_oacc_routine (oacc_routine_data, NULL_TREE, false);
      ...
      if (flag_openmp || flag_openmp_simd){
         ...
         c_parser_handle_directive_omp_attributes (d->u.id.attrs, dummy, &omp_dsimd_idattr_clauses);
      }
      if (gnu_auto_type_p && declarator->kind != cdk_id){
         error_at (here,  "%<__auto_type%> requires a plain identifier as declarator");
         c_parser_skip_to_end_of_block_or_statement (parser);
         n_debug("分析声明说明符 101： count:%d declarator:%p", testcount,declarator);
         return result;
      }
      if (std_auto_type_p){
         struct c_declarator *d = declarator;
      ...
      || c_parser_next_token_is_keyword (parser, RID_IN)){
         ...
         if (!diagnosed_no_specs && !specs->declspecs_seen_p){
            n_debug("分析声明说明符 102： count:%d", testcount);
            diagnosed_no_specs = true;
            pedwarn (here, 0, "data definition has no type or storage class");
         }
         n_debug("分析声明说明符 103： count:%d declarator:%p", testcount,declarator);
         ...
         if (c_parser_next_token_is_keyword (parser, RID_ATTRIBUTE)){
            n_debug("分析声明说明符 104： count:%d", testcount);
         ...
         if (c_parser_next_token_is (parser, CPP_EQ)){
            n_debug("分析声明说明符 105： 下一个token是等号 count:%d", testcount);
            tree d;
            struct c_expr init;
            location_t init_loc;
            c_parser_consume_token (parser);
            if (any_auto_type_p){
               ...
               if (std_auto_type_p)
                  underspec_state = start_underspecified_init (init_loc, underspec_name);
               start_init (NULL_TREE, asm_name, (global_bindings_p ()  || specs->storage_class == csc_static
                     || specs->constexpr_p),  specs->constexpr_p, &richloc);
               ...
               if (std_auto_type_p)
                  finish_underspecified_init (underspec_name, underspec_state);
               flag_sanitize = flag_sanitize_save;
               if (gnu_auto_type_p
               && TREE_CODE (init.value) == COMPONENT_REF
               && DECL_C_BIT_FIELD (TREE_OPERAND (init.value, 1)))
                  error_at (here, "%<__auto_type%> used with a bit-field initializer");
               init = convert_lvalue_to_rvalue (init_loc, init, true, true, true);
               ...
               if (specs->postfix_attrs){
                  ...
                  specs->postfix_attrs  = c_warn_type_attributes (specs->type, specs->postfix_attrs);
                  decl_attributes (&specs->type, specs->postfix_attrs, 0);
                  specs->postfix_attrs = NULL_TREE;
               }
               if (vm_type){
                  bool maybe_const = true;
                  tree type_expr = c_fully_fold (init.value, false, &maybe_const);
                  specs->expr_const_operands &= maybe_const;
                  if (specs->expr)
                     specs->expr = build2 (COMPOUND_EXPR, TREE_TYPE (type_expr), specs->expr, type_expr);
                  else
                     specs->expr = type_expr;
               }
               n_debug("分析声明说明符 106 start_decl 00 ： count:%d declarator:%p", testcount,declarator);
               d = start_decl (declarator, specs, true, chainon (postfix_attrs, all_prefix_attrs));
               if (!d)
                  d = error_mark_node;
               if (omp_declare_simd_clauses)
                  c_finish_omp_declare_simd (parser, d, NULL_TREE, omp_declare_simd_clauses);
               if (!omp_dsimd_idattr_clauses.is_empty ())
                  c_finish_omp_declare_simd (parser, d, NULL_TREE, &omp_dsimd_idattr_clauses);
            }else{ //auto_type_p
               /* The declaration of the variable is in effect while
               its initializer is parsed, except for a constexpr
               variable.  */
               n_debug("分析声明说明符 107 start_decl 11 ： token是等号 得到变量声明树 VAL_DECL count:%d", testcount);
               init_loc = c_parser_peek_token (parser)->location;
               rich_location richloc (line_table, init_loc);
               unsigned int underspec_state = 0;
               if (specs->constexpr_p)
                  underspec_state =  start_underspecified_init (init_loc, underspec_name);
               d = start_decl (declarator, specs, true, chainon (postfix_attrs, all_prefix_attrs), !specs->constexpr_p);
               if (!d)
                  d = error_mark_node;
               if (!specs->constexpr_p && omp_declare_simd_clauses)
                  c_finish_omp_declare_simd (parser, d, NULL_TREE, omp_declare_simd_clauses);

               if (!specs->constexpr_p && !omp_dsimd_idattr_clauses.is_empty ())
                  c_finish_omp_declare_simd (parser, d, NULL_TREE, &omp_dsimd_idattr_clauses);
               n_debug("分析声明说明符 108 ： token是等号 count:%d", testcount);
               new_object_parser(new_object_get(),init_loc,d,genericDefineModel);//zclei
               start_init (d, asm_name, TREE_STATIC (d) || specs->constexpr_p, specs->constexpr_p, &richloc);
               n_debug("分析声明说明符 109：初始化变量 token是等号=EQ 所以进入 c_parser_initializer count:%d", testcount);
               //zclei
               ...
               finish_init ();
               if(genblock) //zclei 10.4.0
                  block_mgr_set_lhs(block_mgr_get(),NULL_TREE);//zclei 10.4.0
               n_debug("分析声明说明符 110：完成初始化变量 token是等号 count:%d init:%p %s",
                     testcount,init.value,get_tree_code_name(TREE_CODE(init.value)));
            }//end auto_type_p
            ...
            if (d != error_mark_node){
               ...
               init.value=class_impl_modify_or_init_func_pointer(classImpl,
                     init_loc,d,init.value,init.original_type,false); //zclei 10.4.0
               ...
               class_impl_in_finish_decl(classImpl,init_loc,d);
            }
         }else{ //c_parser_next_token_is (parser, CPP_EQ) 声明符右边不是等号
            if (any_auto_type_p || specs->constexpr_p){
               error_at (here,"%qs requires an initialized data declaration",any_auto_type_p ? auto_type_keyword : "constexpr");
               c_parser_skip_to_end_of_block_or_statement (parser);
               return result;
            }
            n_debug("分析声明说明符 114 start_decl 22 ：构建完整的 fun 函数声明语句的树结构 specs中有int 声明说明符树 declarator有fun及参数据的树 count:%d",testcount);
            nboolean setFundecl=FALSE;//zclei 10.4.0
            if(aetParser->isAet && !genericBlockFuncDefine && nested==0){ //zclei 10.4.0
               nboolean isQueryFunction=generic_query_is_function_declarator(generic_query_get(),classImpl->className,declarator);
               n_debug("分析声明说明符 115 :把函数声明符改了 count:%d name:%s isQueryFunction:%d",testcount,classImpl->className->sysName,isQueryFunction);
               if(isQueryFunction && !funcGenericModel)
                  generic_query_add_parm_to_declarator(generic_query_get(),declarator);
               setFundecl=TRUE;//func_mgr_change_class_impl_func_decl(func_mgr_get(),declarator,classImpl->className);
            }//zclei 10.4.0

            location_t lastloc = UNKNOWN_LOCATION;
            tree attrs = chainon (postfix_attrs, all_prefix_attrs);
            tree d = start_decl (declarator, specs, false, attrs, true, &lastloc);
            if (d && TREE_CODE (d) == FUNCTION_DECL){
               /* Find the innermost declarator that is neither cdk_id
               nor cdk_attrs.  */
               const struct c_declarator *decl = declarator;
               const struct c_declarator *last_non_id_attrs = NULL;
               n_debug("分析声明说明符 116： count:%d", testcount);
               while (decl)
                  switch (decl->kind){
                     case cdk_array:
                     case cdk_function:
               ...
               if (last_non_id_attrs && last_non_id_attrs->kind == cdk_function){
                  n_debug("分析声明说明符 -----117：设参数 count:%d decl:%p", testcount,d);
                  tree parms = last_non_id_attrs->u.arg_info->parms;
                  if (DECL_ARGUMENTS (d) == NULL_TREE  && DECL_INITIAL (d) == NULL_TREE)
                     DECL_ARGUMENTS (d) = parms;

                  warn_parm_array_mismatch (lastloc, d, parms);
               }
            } //结束   if (d && TREE_CODE (d) == FUNCTION_DECL){
            if (omp_declare_simd_clauses || !omp_dsimd_idattr_clauses.is_empty ()){
               n_debug("分析声明说明符 117： count:%d", testcount);
               tree parms = NULL_TREE;
               if (d && TREE_CODE (d) == FUNCTION_DECL){
                  n_debug("分析声明说明符 118： FUNCTION_DECL ount:%d", testcount);
               ...
               if (omp_declare_simd_clauses)
                  c_finish_omp_declare_simd (parser, d, parms, omp_declare_simd_clauses);
               if (!specs->constexpr_p && !omp_dsimd_idattr_clauses.is_empty ())
                  c_finish_omp_declare_simd (parser, d, parms, &omp_dsimd_idattr_clauses);
               if (parms)
                  temp_pop_parm_decls ();
            }
            if (oacc_routine_data)
               c_finish_oacc_routine (oacc_routine_data, d, false);
            if (d){
               if(aetParser->isAet && TREE_CODE (d) == FUNCTION_DECL && c_parser_next_token_is (parser, CPP_SEMICOLON)){//zclei
                  // error_at(here,"类实现中不能声明函数。类%qs，函数%qs。",classImpl->className->sysName,IDENTIFIER_POINTER(DECL_NAME(d)));
                  error_at(here,"Functions cannot be declared in class implementations %qs %qs",
                        classImpl->className->sysName,IDENTIFIER_POINTER(DECL_NAME(d)));
               }
               if(aetParser->isAet && setFundecl && !genericBlockFuncDefine && nested==0){//zclei
                  n_debug("分析声明说明符 119:把函数声明符改了 count:%d name:%s",testcount,classImpl->className->sysName);
                  generic_impl_check_and_set_func(generic_impl_get(),d,genericDefineModel,funcGenericModel);
               }//zclei
               finish_decl (d, UNKNOWN_LOCATION, NULL_TREE, NULL_TREE, asm_name);
               mtcs_parser_check(mtcs_parser_get(),d,TRUE);//zclei 检查是不是mtcs变量
               n_debug("分析声明说明符 119-xx context:%p extern:%d static:%d pub:%d ",
                     DECL_CONTEXT(d),DECL_EXTERNAL(d),TREE_STATIC(d),TREE_PUBLIC(d));
               if(aetParser->isAet && setFundecl){//zclei
                  ...
                  if(genericDefineModel){//zclei
                     ...
                     c_aet_set_generics_model(d,NULL);//重要，因为d可能会重用，原来的d中存有泛型
                  }
               }//zclei
            }
         ...
         }//结束 _parser_next_token_is (parser, CPP_EQ)
         if (c_parser_next_token_is (parser, CPP_COMMA)){
            n_debug("分析声明说明符 122： count:%d declarator:%p", testcount,declarator);
            ...
            c_parser_consume_token (parser);
            if (c_parser_next_token_is_keyword (parser, RID_ATTRIBUTE))
               all_prefix_attrs = chainon (c_parser_gnu_attributes (parser), prefix_attrs);
            else
               all_prefix_attrs = prefix_attrs;
            continue;
         }else if (c_parser_next_token_is (parser, CPP_SEMICOLON)){
            n_debug("分析声明说明符 123 这是一个分号 simple_ok:%d", simple_ok);
            if (!simple_ok)
               c_parser_consume_token (parser);
            return result;
            ...
            if (!simple_ok){
               ...
               c_parser_skip_to_end_of_block_or_statement (parser);
            }else if (more_than_one_decl)/* It's not valid to use if (int i = 2, j = 3).  */
               error_at (here, "declaration in condition can only declare a single object");
            return result;
         }
         //结否 if (c_parser_next_token_is (parser, CPP_EQ)  || c_parser_next_token_is (parser, CPP_COMMA)...
      }else if (any_auto_type_p || specs->constexpr_p){
         error_at (here, "%qs requires an initialized data declaration",
               any_auto_type_p ? auto_type_keyword : "constexpr");
         c_parser_skip_to_end_of_block_or_statement (parser);
         return result;
      }else if (!fndef_ok){
      ...
      nboolean changeNameOk=FALSE; //zclei
      if(aetParser->isAet && !genericBlockFuncDefine && nested==0){//zclei
         changeNameOk=class_impl_start_function(classImpl,
               specs,declarator,funcGenericModel!=NULL,permissionLoc,haveAccessControl,permission);
      }
      n_debug("分析声明说明符 124XX： 分析函数定义 count:%d declarator:%p", testcount,declarator);
      ...
      if (!start_function (specs, declarator, all_prefix_attrs)){
         ...
         if (c_parser_next_token_starts_declspecs (parser)){
            ...
            n_debug("分析声明说明符 126： count:%d", testcount);
      ...
      n_debug("分析声明说明符 127： count:%d declarator:%p", testcount,declarator);
      c_parser_maybe_reclassify_token (parser);
      while (c_parser_next_token_is_not (parser, CPP_EOF) && c_parser_next_token_is_not (parser, CPP_OPEN_BRACE))
         c_parser_declaration_or_fndef (parser, false, false, false,true, false, false);
      n_debug("分析声明说明符 128：保存函数定义的参数 count:%d declarator:%p", testcount,declarator);
      debug_nonbind_markers_p = save_debug_nonbind_markers_p;
      store_parm_decls ();
      if (omp_declare_simd_clauses)
         c_finish_omp_declare_simd (parser, current_function_decl, NULL_TREE, omp_declare_simd_clauses);
      if (!omp_dsimd_idattr_clauses.is_empty ())
         c_finish_omp_declare_simd (parser, current_function_decl, NULL_TREE, &omp_dsimd_idattr_clauses);
      if (oacc_routine_data)
      ...
      tree fndecl = current_function_decl;//zclei 10.4.0
      ...
      if (specs->declspec_il == cdil_rtl){
         n_debug("分析声明说明符 129： count:%d declarator:%p", testcount,declarator);
      ...
      function body as GIMPLE.  */
         ...
         n_debug("分析声明说明符 130： count:%d declarator:%p", testcount,declarator);
         ...
         n_debug("分析声明说明符 131：对函数内部的复合语句进行分析 如果是MTCS函数，记录状态 count:%d declarator:%p", testcount,declarator);
         aet_print_token(c_parser_peek_token (parser));
         if(func_mgr_is_mtcs_func(func_mgr_get(),fndecl)){
            n_debug("分析声明说明符 131-1 进入mtcs函数\n");
            mtcs_parser_enter_function_body(mtcsParser);
         }
         fnbody = c_parser_compound_statement (parser, &endloc);
         if(func_mgr_is_mtcs_func(func_mgr_get(),fndecl)){  //zclei
            n_debug("分析声明说明符 131-2 退出mtcs函数\n");
             mtcs_parser_exit_function_body(mtcsParser);
         }
         //如果函数定义是类中的final函数或是所在类是final$,如果是公共函数和protected，把函数tree_public设为1
         if(class_impl_can_global(classImpl,fndecl)){
            TREE_PUBLIC(fndecl) = 1;
         }
      }
      //tree fndecl = current_function_decl;//zclei 10.4.0没有这句
      if (nested){
         ...
         if (fnbody)
            add_stmt (fnbody);
         class_impl_finish_function(class_impl_get(),fndecl,endloc);//zclei
         finish_function (endloc);
      }
   ...
   classImpl->currentFuncModel = NULL;
   return result;
}
...
static void c_parser_static_assert_declaration (c_parser *parser)
{
   c_parser_static_assert_declaration_no_semi (parser);
   if (parser->error || !c_parser_require (parser, CPP_SEMICOLON, "expected %<;%>"))
      c_parser_skip_to_end_of_block_or_statement (parser);
}
  ...
  if (c_parser_next_token_is (parser, CPP_COLON)
      ...
      else
   {
     ...
     ret.has_enum_type_specifier = true;
      ...
      aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(aetParser, specs, false, true, false, false, false,
...
static struct c_typespec c_parser_struct_or_union_specifier (c_parser *parser)
{
   struct c_typespec ret;
   bool have_std_attrs;
   tree std_attrs = NULL_TREE;
   ...
   if (have_std_attrs)
      std_attrs = c_parser_std_attribute_specifier_sequence (parser);
   attrs = c_parser_gnu_attributes (parser);
   n_debug("分析struct 00 code:%d RECORD_TYPE:%d have_std_attrs:%d--- attrs==NULL_TREE:%d",
         code,RECORD_TYPE,have_std_attrs,attrs==NULL_TREE);
   ...
   if (c_parser_next_token_is (parser, CPP_NAME)){
      ...
      c_parser_consume_token (parser);
      n_debug("分析struct 11 下一个token是CPP_NAME code:%d RECORD_TYPE:%d have_std_attrs:%d--- str:%s",
            code,RECORD_TYPE,have_std_attrs,str1);
   }

   if (c_parser_next_token_is (parser, CPP_OPEN_BRACE)){
      n_debug("分析struct 22 下一个token是CPP_OPEN_BRACE code:%d RECORD_TYPE:%d have_std_attrs:%d---",
      code,RECORD_TYPE,have_std_attrs);
      /* Parse a struct or union definition.  Start the scope of the
      tag before parsing components.  */
      class c_struct_parse_info *struct_info;
      //type 在start_struct中的make_node创建 make_node里根据传入的code选择不同的TREE类型 code=struct 或union
      //ident是struct 后跟的名字是一个ident tree在start_struct中pushtag把 struct 类型tree 与ident tree绑在一起了。
      ...
      if (c_parser_next_token_is_keyword (parser, RID_AT_DEFS)){
         ...
         if (!parens.require_open (parser))
            goto end_at_defs;
         if (c_parser_next_token_is (parser, CPP_NAME)  && c_parser_peek_token (parser)->id_kind == C_ID_CLASSNAME){
...
end_at_defs:
      ...
      while (true){
         ...
         if (c_parser_next_token_is (parser, CPP_SEMICOLON)){
            n_debug("分析struct 33 下一个token是CPP_SEMICOLON  continue count:%d ",testcount);
         ...
         n_debug("分析struct 55 应该在这里mangle 如果结构体内没内容不会进这里，下一个token是什么? count:%d---",testcount);
         //       tree name=DECL_NAME(decls);
         //       tree type00=TREE_TYPE(decls);
         //       n_debug("分析struct 55 应该在这里mangle code:%s %s %s count:%d---",
         //               get_tree_code_name(TREE_CODE(decls)),
         //               get_tree_code_name(TREE_CODE(name)),
         //               get_tree_code_name(TREE_CODE(type00)),
         //               testcount);
         ...
         pedwarn.  */
         if (c_parser_next_token_is (parser, CPP_SEMICOLON)){
            n_debug("分析struct 66 下一个token是CPP_SEMICOLON count:%d ",testcount);
            c_parser_consume_token (parser);
         }else{
            if (c_parser_next_token_is (parser, CPP_CLOSE_BRACE))
               ...
               c_parser_error (parser, "expected %<;%>");
               c_parser_skip_until_found (parser, CPP_CLOSE_BRACE, NULL);
               n_debug("分析struct CCCC 下一个token是CPP_OPEN_BRACE code:%d RECORD_TYPE:%d have_std_attrs:%d---",
               code,RECORD_TYPE,have_std_attrs);
               break;
            }
      ...
      postfix_attrs = c_parser_gnu_attributes (parser);
      //返回的tree 还是type tree_code=RECORD_TYPE
      ret.spec = finish_struct (struct_loc, type, nreverse (contents),
            chainon (std_attrs,chainon (attrs, postfix_attrs)),struct_info, &expr);
      n_debug("分析struct 77 完成finish_struct count:%d--- ret.spec:%p postfix_attrs:%p std_attrs:%p",
            testcount,ret.spec,postfix_attrs,std_attrs);
      aet_print_token(c_parser_peek_token (parser));
      ret.kind = ctsk_tagdef;//将会存在struct c_declspecs 中的typespec_kind字段
      ret.expr = expr;
      ret.expr_const_operands = true;
      ret.has_enum_type_specifier = false;
      timevar_pop (TV_PARSE_STRUCT);
      return ret;
      //end    if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
   }else if (!ident){
      n_debug("分析struct 88 ident是空的 code:%d RECORD_TYPE:%d have_std_attrs:%d---",
            code,RECORD_TYPE,have_std_attrs);
      c_parser_error (parser, "expected %<{%>");
      ret.spec = error_mark_node;
      ret.kind = ctsk_tagref;
   ...
   n_debug("分析struct over %s---",IDENTIFIER_POINTER (ident));
...
/*zclei
 * 函数c_parser_struct_declaration处理是以分号为单位的结构体成员声明。
 * 也就是说这些声明符共用一个声明说明符，但声明符和位域都是需要分别处理，其注释说明如下：
 * 共用的是struct 存储类型的声明说明符 一个声明执行该方法一次，多个声明是在c_parser_struct_or_union_specifier
 * 方法中循环的
 */
static tree
c_parser_struct_declaration (c_parser *parser, tree *expr)
{
  ...
  if (c_parser_next_token_is_keyword (parser, RID_STATIC_ASSERT))
    {
      c_parser_static_assert_declaration_no_semi (parser);
      n_debug("struct内的声明 -------11 RID_STATIC_ASSERT 返回");
      return NULL_TREE;
    }
  specs = build_null_declspecs ();
  decl_loc = c_parser_peek_token (parser)->location;
  n_debug("struct内的声明 00 建立空的声明说明符");
  aet_print_token(c_parser_peek_token (parser));
  ...
  aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(aetParser, specs, false, true, true,
  ...
  finish_declspecs (specs);
  n_debug("struct内的声明 11 到这里specs已有声明说明符了比如 int 完成finish_declspecs");
  aet_print_token(c_parser_peek_token (parser));
  if (c_parser_next_token_is (parser, CPP_SEMICOLON)
      || c_parser_next_token_is (parser, CPP_CLOSE_BRACE))
    {
      n_debug("struct内的声明 22 如果是CPP_SEMICOLON或CPP_CLOSE_BRACE 返回");
      tree ret;
      ...
      else
   {
     /* Support for unnamed structs or unions as members of
        ...
        MS P-SDK).  */
     tree attrs = NULL;
         n_debug("struct内的声明 33 Support for unnamed structs ");
  ...
  n_debug("struct内的声明 44 不是CPP_SEMICOLON或CPP_CLOSE_BRACE 继续");
  ...
  if (specs->typespec_kind == ctsk_tagdef
      ...
      && !c_parser_next_token_is (parser, CPP_NAME))
    {
      c_parser_error (parser, "expected %<;%>, identifier or %<(%>");
      parser->error = false;
      n_debug("struct内的声明 55 返回 NULL_TREE");
      return NULL_TREE;
    }
  ...
  while (true)
   ...
   declarator = build_id_declarator (NULL_TREE);
        ...
        n_debug("struct内的声明 66 一定进这里 这时peek token 是标识符getName也就是声明符 count:%d ", testcount);
   declarator =aet_parser_c_parser_declarator/*!c_parser_declarator*/(aetParser,
                 specs->typespec_kind != ctsk_none,
                 C_DTR_NORMAL, &dummy);
        n_debug("struct内的声明 ----66  count:%d  declarator:%p", testcount,declarator);
      }
      if (declarator == NULL)
   {
          n_debug("struct内的声明 77 出错了 break count:%d ", testcount);
     c_parser_skip_to_end_of_block_or_statement (parser);
     break;
   }
      if (c_parser_next_token_is (parser, CPP_COLON)
     ...
     || c_parser_next_token_is_keyword (parser, RID_ATTRIBUTE))
   {
     ...
     if (c_parser_next_token_is_keyword (parser, RID_ATTRIBUTE))
       postfix_attrs = c_parser_gnu_attributes (parser);
          n_debug("struct内的声明 88 grokfield count:%d  生成的声明kind=%d %p",testcount,declarator->kind,declarator);
     ...
     if (c_parser_next_token_is (parser, CPP_COMMA))
         ...
         || c_parser_next_token_is (parser, CPP_CLOSE_BRACE))
       {
         /* Semicolon consumed in caller.  */
           n_debug("struct内的声明 99 下一个 token 是CPP_SEMICOLON or CPP_CLOSE_BRACE 退出循环 count:%d ", testcount);
         break;
       }
     else
       {
  ...
  n_debug("struct内的声明 100 over count:%d ", testcount);
  return decls;
}
...
c_parser_typeof_specifier (c_parser *parser)
{
  ...
  if (!parens.require_open (parser))
   ...
   generic_parser_parser_typeof(generic_parser_get());//zclei

  if (c_parser_next_tokens_start_typename (parser, cla_prefer_id))
    {
      struct c_type_name *type = parserTypeName/*!c_parser_type_name*/(aetParser);
      c_inhibit_evaluation_warnings--;
      in_typeof--;
      if (type != NULL)
   {
...
static tree c_parser_alignas_specifier (c_parser * parser)
{
   ...
   c_parser_consume_token (parser);
   if (flag_isoc99)
      pedwarn_c99 (loc, OPT_Wpedantic, "ISO C99 does not support %qE", spelling);
   else
      pedwarn_c99 (loc, OPT_Wpedantic, "ISO C90 does not support %qE", spelling);
   matching_parens parens;
   if (!parens.require_open (parser))
      return ret;
   if (c_parser_next_tokens_start_typename (parser, cla_prefer_id)){
      struct c_type_name *type =parserTypeName/*!c_parser_type_name*/(aetParser);
      if (type != NULL)
         ret = c_sizeof_or_alignof_type (loc, groktypename (type, NULL, NULL),false, true, 1);
   }else
      ret = convert_lvalue_to_rvalue (loc,c_parser_expr_no_commas (parser, NULL), true, true).value;
   parens.skip_until_found_close (parser);
   return ret;
}
...
static struct c_declarator * c_parser_direct_declarator (c_parser *parser,
      bool type_seen_p, c_dtr_syn kind, bool *seen_id)
{
   ...
   || c_parser_peek_token (parser)->id_kind == C_ID_ID)){
      const char *str1=IDENTIFIER_POINTER ((c_parser_peek_token (parser)->value));
      struct c_declarator *inner = build_id_declarator (c_parser_peek_token (parser)->value);
      n_debug("直接解析声明 00  name: %s 创建声明符树节点(kind=cdk_id) inner:%p", str1,inner);
      *seen_id = true;
      inner->id_loc = c_parser_peek_token (parser)->location;
      c_parser_consume_token (parser);
      if (c_parser_nth_token_starts_std_attributes (parser, 1))
         inner->u.id.attrs = c_parser_std_attribute_specifier_sequence (parser);
      n_debug("直接解析声明 11 进入此函数开始分析声明实体,即 fun 函数声明语句中“ ( ”“ ) ”之间的内容 c_parser_direct_declarator_inner 并返回");
   ...
   && !c_parser_nth_token_starts_std_attributes (parser, 1)){
      n_debug("直接解析声明 22 进入 c_parser_direct_declarator_inner ");
   ...
   if (c_parser_next_token_is (parser, CPP_OPEN_PAREN)){
      n_debug("直接解析声明 33 下一个token是CPP_OPEN_PAREN");
      aet_print_token(c_parser_peek_token (parser));
      tree attrs;
      struct c_declarator *inner;
      c_parser_consume_token (parser);
      bool have_gnu_attrs = c_parser_next_token_is_keyword (parser,RID_ATTRIBUTE);
      attrs = c_parser_gnu_attributes (parser);
      if (kind != C_DTR_NORMAL
      && (c_parser_next_token_starts_declspecs (parser)
      || (!have_gnu_attrs
      ...
      || c_parser_next_token_is (parser, CPP_CLOSE_PAREN))){
         n_debug("直接解析声明 44 下一个token是CPP_OPEN_PAREN ");
         struct c_arg_info *args = c_parser_parms_declarator (parser, kind == C_DTR_NORMAL,attrs, have_gnu_attrs);
         n_debug("直接解析声明 44 --- 下一个token是CPP_OPEN_PAREN args:%p",args);
         if (args == NULL)
            return NULL;
         else{
            ...
            n_debug("直接解析声明 55 进入 c_parser_direct_declarator_inner");
            inner = build_function_declarator (args, inner);
            return c_parser_direct_declarator_inner (parser, *seen_id,inner);
         }
      }
      /* A parenthesized declarator.  */
      n_debug("直接解析声明 66 调用c_parser_declarator 会不会死循环？");
      inner = aet_parser_c_parser_declarator/*!c_parser_declarator*/(aetParser, type_seen_p, kind, seen_id);
      if (inner != NULL && attrs != NULL)
         inner = build_attrs_declarator (attrs, inner);
      if (c_parser_next_token_is (parser, CPP_CLOSE_PAREN)){
         n_debug("直接解析声明 77 CPP_CLOSE_PAREN 完成了 int (*fun)的分析返回的declarator是一个指针里面的target是fun的declarator inner %p ",inner);
         c_parser_consume_token (parser);
         if (inner == NULL)
            return NULL;
         else
      ...
      n_debug("直接解析声明 88 kind == C_DTR_NORMAL %d 返回空否则build_id_declarator", kind == C_DTR_NORMAL);
      if (kind == C_DTR_NORMAL){
         c_parser_error (parser, "expected identifier or %<(%>");
         return NULL;
      }else
              ...
              struct c_declarator *inner)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_SQUARE)
      && !c_parser_nth_token_starts_std_attributes (parser, 1))
    {
      n_debug("inner直接解析声明 00 是数组 CPP_OPEN_SQUARE");
      ...
      c_parser_consume_token (parser);
      aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(aetParser, quals_attrs, false, false, true,
   ...
   {
        n_debug("inner直接解析声明 11 不是CPP_CLOSE_SQUARE");
      ...
      if (c_parser_nth_token_starts_std_attributes (parser, 1))
   {
     ...
     if (std_attrs)
       ...
       n_debug("inner直接解析声明 33 创建数组");
      inner = set_array_declarator_inner (declarator, inner);
       //zclei 如果变量的声明是CLASS并且下一个是"("退出
      return c_parser_direct_declarator_inner (parser, id_present, inner);
    }
  else if (c_parser_next_token_is (parser, CPP_OPEN_PAREN))
    {
  n_debug("inner直接解析声明 44 下一个token是CPP_OPEN_PAREN 如果符号“ fun ”后面跟的是“ ( ”,说明是函数声明或定义 做参数据解析");
        if(inner->kind == cdk_array){//zclei 10.4.0
            n_debug("inner直接解析声明 --44 这是一个数组后跟'(',返回。");
            return inner;
        }
      tree attrs;
      ...
      args = c_parser_parms_declarator (parser, id_present, attrs,
               have_gnu_attrs);
      if (args == NULL){
            n_debug("inner直接解析声明 55 下一个token是CPP_OPEN_PAREN 参数是空的返回NUL");
   return NULL;
      }else
   {
     if (!(args->types
    ...
    //这里的inner的kind是
           struct c_declarator *temp=inner;//zclei 10.4.0
     inner = build_function_declarator (args, inner);
  n_debug("inner直接解析声明 66 用指针类型的(kind=cdk_pointer)和参数构成一个kind=cdk_funtion的新声明符 新:inner:%p 老inner:%p",
                   inner,temp);
     return c_parser_direct_declarator_inner (parser, id_present, inner);
   }
    }
...
static struct c_arg_info *c_parser_parms_declarator (c_parser *parser, bool id_list_ok, tree attrs,
            bool have_gnu_attrs)
{
   push_scope ();
   n_debug("参数声明 00 id_list_ok:%d have_gnu_attrs:%d",id_list_ok,have_gnu_attrs);
   declare_parm_level ();
   ...
   && c_parser_peek_2nd_token (parser)->type != CPP_KEYWORD){
      n_debug("参数声明 11 id_list_ok:%d have_gnu_attrs:%d",id_list_ok,have_gnu_attrs);
      tree list = NULL_TREE, *nextp = &list;
      while (c_parser_next_token_is (parser, CPP_NAME)  && c_parser_peek_token (parser)->id_kind == C_ID_ID){
         n_debug("参数声明 22 id_list_ok:%d have_gnu_attrs:%d",id_list_ok,have_gnu_attrs);
      ...
      if (c_parser_next_token_is (parser, CPP_CLOSE_PAREN)){
         n_debug("参数声明 33 id_list_ok:%d have_gnu_attrs:%d",id_list_ok,have_gnu_attrs);
         ...
         n_debug("参数声明 44 id_list_ok:%d have_gnu_attrs:%d",id_list_ok,have_gnu_attrs);
         c_parser_skip_until_found (parser, CPP_CLOSE_PAREN,"expected %<)%>");
         pop_scope ();
         return NULL;
      }
   }else{
      n_debug("参数声明 55 分析参数列表 id_list_ok:%d have_gnu_attrs:%d",id_list_ok,have_gnu_attrs);
      struct c_arg_info *ret = c_parser_parms_list_declarator (parser, attrs, NULL, have_gnu_attrs);
      pop_scope ();
      return ret;
   }
}
...
static struct c_arg_info *c_parser_parms_list_declarator (c_parser *parser, tree attrs, tree expr, bool have_gnu_attrs)
{
   bool bad_parm = false;
   ...
   declarations.  */
   if (c_parser_next_token_is (parser, CPP_CLOSE_PAREN)){
      n_debug("创建参数声明列表 00 没有参数  have_gnu_attrs:%d",have_gnu_attrs);
      struct c_arg_info *ret = build_arg_info ();
      c_parser_consume_token (parser);
      return ret;
   }
   if (c_parser_next_token_is (parser, CPP_ELLIPSIS) && !have_gnu_attrs){
      n_debug("创建参数声明列表 11 可变参数  have_gnu_attrs:%d",have_gnu_attrs);
      struct c_arg_info *ret = build_arg_info ();

      ret->types = NULL_TREE;
      pedwarn_c11 (c_parser_peek_token (parser)->location, OPT_Wpedantic,"ISO C requires a named argument before %<...%> before C23");
      c_parser_consume_token (parser);
      if (c_parser_next_token_is (parser, CPP_CLOSE_PAREN)){
   ...
   while (true){
      /* Parse a parameter.  */
      n_debug("创建参数声明列表 22 循环分析每一个参数 count:%d have_gnu_attrs:%d",count,have_gnu_attrs);
      ...
      if (parm == NULL)
         ...
         //zclei
         GenericModel *genericsModel=generic_impl_pop_generic_from_declspecs(generic_impl_get(),parm->specs);
         //zclei 该方法原来没有返回值，为了支持泛型现在加上
         tree decl=class_impl_push_parm_decl/*!push_parm_decl*/(classImpl,parm, &expr);
         if(genericsModel!=NULL){
            n_debug("参数设泛型了------- 00    等完成函数声明后再检查泛型是否有效。%p",decl);
            c_aet_set_generics_model(decl,genericsModel);
         }else{
            c_aet_set_generics_model(decl,NULL);//zclei 必设为空，decl可能重用
         }
      }
      if (c_parser_next_token_is (parser, CPP_SEMICOLON)){
         n_debug("创建参数声明列表 33 循环分析每一个参数 下一个是CPP_SEMICOLON,count:%d have_gnu_attrs:%d",count,have_gnu_attrs);
         tree new_attrs;
         c_parser_consume_token (parser);
      ...
      if (c_parser_next_token_is (parser, CPP_CLOSE_PAREN)){
         n_debug("创建参数声明列表 44 循环分析每一个参数 下一个是 CPP_CLOSE_PAREN 识别到符号“ ) ”,参数列表分析完毕,count:%d have_gnu_attrs:%d",
         count,have_gnu_attrs);
         c_parser_consume_token (parser);
         if (bad_parm)
            return NULL;
         else
      ...
      if (c_parser_next_token_is (parser, CPP_ELLIPSIS)){
         n_debug("创建参数声明列表 55 循环分析每一个参数 下一个是 CPP_ELLIPSIS,count:%d have_gnu_attrs:%d",count,have_gnu_attrs);
         c_parser_consume_token (parser);
         if (c_parser_next_token_is (parser, CPP_CLOSE_PAREN)){
            c_parser_consume_token (parser);
            if (bad_parm)
...
static struct c_parm * c_parser_parameter_declaration (c_parser *parser, tree attrs,
            bool have_gnu_attrs)
{
   struct c_declspecs *specs;
   struct c_declarator *declarator;
   ...
   && !c_parser_nth_token_starts_std_attributes (parser, 1)){
      ...
      if (c_parser_next_tokens_start_typename (parser, cla_prefer_type)){
         auto_diagnostic_group d;
         name_hint hint = lookup_name_fuzzy (token->value,FUZZY_LOOKUP_TYPENAME,token->location);
         if (const char *suggestion = hint.suggestion ()){
            gcc_rich_location richloc (token->location);
            richloc.add_fixit_replace (suggestion);
            error_at (&richloc, "unknown type name %qE; did you mean %qs?", token->value, suggestion);
         }else
   ...
   aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(aetParser,
         specs, true, true, true, true, false,!have_gnu_attrs, true, cla_nonabstract_decl);
   finish_declspecs (specs);
   pending_xref_error ();
   prefix_attrs = specs->attrs;
   specs->attrs = NULL_TREE;
   ...
   //  if (c_parser_next_token_is (parser, CPP_LESS)){
   //    printf("进这里参数据泛型----\n");
   //    generic_impl_push_generic_from_c_parm(generic_impl_get(),specs);
   //  }
   declarator = aet_parser_c_parser_declarator/*!c_parser_declarator*/(aetParser,
   specs->typespec_kind != ctsk_none, C_DTR_PARM, &dummy);
   if (declarator == NULL){
   ...
   */
   ...
   xloc = expand_location(caret_loc);
   fprintf(stderr,"caret is --- %d %d %s\n",xloc.line,xloc.column,xloc.file);
   //  xloc = expand_location(pure_loc);
   //  printf("pure_loc is --- %d %d %s\n",xloc.line,xloc.column,xloc.file);

   xloc = expand_location(start_loc);
   fprintf(stderr,"src_range.m_start is --- %d %d %s\n",xloc.line,xloc.column,xloc.file);
   xloc = expand_location(end_loc);
   fprintf(stderr,"src_range.m_finish is --- %d %d %s\n",xloc.line,xloc.column,xloc.file);
   */
  ...
  if (c_parser_next_token_is (parser, CPP_KEYWORD))
    {
      ...
      bool ok;
        n_debug("c-parser.c c_parser_gnu_attribute_any_word %d %d",c_parser_peek_token (parser)->keyword,RID_TYPEDEF);
...
static tree c_parser_gnu_attribute (c_parser *parser, tree attrs,
         bool expect_comma = false, bool empty_ok = true)
{
   ...
   tree attr_args = c_parser_attribute_arguments (parser,
         attribute_takes_identifier_p (attr_name),false,is_attribute_p ("assume", attr_name),true);
...
static tree c_parser_gnu_attributes (c_parser *parser)
{
   tree attrs = NULL_TREE;
...
static struct c_expr c_parser_initializer (c_parser *parser, tree decl)
{
   if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
...
static tree c_parser_compound_statement (c_parser *parser, location_t *endlocp)
{
   tree stmt;
   ...
   if (endlocp)
      *endlocp = end_loc;
   n_debug("aetparser.c c_parser_compound_statement 00\n");
   tree ret= c_end_compound_stmt (brace_loc, stmt, true);
   return ret;
}
...
static location_t c_parser_compound_statement_nostart (c_parser *parser)
{
   bool last_stmt = false;
   bool last_label = false;
   bool save_valid_for_pragma = valid_location_for_stdc_pragma_p ();
   location_t label_loc = UNKNOWN_LOCATION;  /* Quiet warning.  */
   class_impl_nest_op(classImpl,TRUE);//zclei 10.4.0
   struct omp_for_parse_data *omp_for_parse_state  = parser->omp_for_parse_state;
   bool in_omp_loop_block = omp_for_parse_state ? omp_for_parse_state->want_nested_loop : false;
   tree sl = NULL_TREE;
   attr_state a = {};

   if (c_parser_next_token_is (parser, CPP_CLOSE_BRACE)){
      n_debug("函数复合语句 遇到CPP_CLOSE_BRACE 返回");
      class_impl_nest_op(classImpl,FALSE);//zclei 10.4.0
      aet_print_token(c_parser_peek_token(parser));
      if(aetParser->isAet && classImpl->nest==0 && classImpl->isConstructor){//zclei 10.4.0
         //如果是构造函数加入返回语句
         nboolean re=class_ctor_add_return_stmt(classImpl->classCtor);
         n_debug("函数复合语句 99xx：goto statement  re:%d", re);
         if(re)
            goto again;
      }
   ...
   if (c_parser_next_token_is_keyword (parser, RID_LABEL)){
      ...
      while (c_parser_next_token_is_keyword (parser, RID_LABEL)){
         ...
         while (true){
            ...
            label = declare_label (c_parser_peek_token (parser)->value);
...
again:
   ...
   while (c_parser_next_token_is_not (parser, CPP_CLOSE_BRACE)){
      ...
      bool want_nested_loop = (omp_for_parse_state ? omp_for_parse_state->want_nested_loop : false);
      ...
      if (in_omp_loop_block && !last_label){
         ...
         if (want_nested_loop  && c_parser_omp_next_tokens_can_be_canon_loop (parser, code, false)){
            ...
            continue;
         }else if (want_nested_loop && c_parser_next_token_is (parser, CPP_OPEN_BRACE)){
      ...
      || (c_parser_next_token_is (parser, CPP_NAME)
      && c_parser_peek_2nd_token (parser)->type == CPP_COLON)){
         n_debug("函数复合语句 22： c_parser_label count:%d", testcount);
         aet_print_token(c_parser_peek_token(parser));
         if (c_parser_next_token_is_keyword (parser, RID_CASE))
         ...
         nboolean ret=class_impl_parser_package_dot_class(classImpl);//zclei
         if(!ret){
            goto statement;
         }else{
            goto class_dot_access;
         }
      }else if(mtcs_parser_is_attribute(mtcs_parser_get(),c_parser_peek_token (parser))){
         //fprintf(stderr,"处理 __shared__ ----\n");
         //跳到 class_do_access 利用 c_parser_declaration_or_fndef 处理在mtcs_parser_add_attribute加入的 _attribute_((global)) 属性
         mtcs_parser_add_attribute (mtcs_parser_get());
         goto class_dot_access;
      ...
      && c_parser_next_token_is (parser, CPP_SEMICOLON)
      && (have_std_attrs = true)) || c_parser_next_token_is_keyword (parser, RID_AET_GOTO)/*zclei*/)
      {
   class_dot_access://zclei
         nboolean isClassAndDot=class_impl_next_tokens_is_class_and_dot (classImpl);//zclei 10.4.0
         if(isClassAndDot){
         ...
         c_parser_declaration_or_fndef (parser,
               true, !have_std_attrs,true, true, true, false, NULL, NULL, have_std_attrs, std_attrs, NULL, &fallthru_attr_p);
         ...
         || c_parser_nth_token_starts_std_attributes (parser, 2))){
            ...
            mark_valid_location_for_stdc_pragma (false);
            c_parser_declaration_or_fndef (parser, true, true, true, true, true, false);
            if (in_omp_loop_block)
               omp_for_parse_state->want_nested_loop = want_nested_loop;
         ...
         if (omp_for_parse_state)
            omp_for_parse_state->want_nested_loop = false;
         if (c_parser_pragma (parser, last_label ? pragma_stmt : pragma_compound, NULL, before_labels)){
            last_label = false;
            last_stmt = true;
            if (omp_for_parse_state)
               check_omp_intervening_code (parser);
   ...
   statement:
         n_debug("函数复合语句 101：goto statement  count:%d before_labels:%P", testcount,before_labels);
         c_warn_unused_attributes (std_attrs);
         last_label = false;
         last_stmt = true;
         mark_valid_location_for_stdc_pragma (false);
         if (!omp_for_parse_state)
            c_parser_statement_after_labels (parser, NULL, before_labels, NULL, a);
         else{
   ...
   if (last_label)
      pedwarn_c11 (label_loc, OPT_Wfree_labels,"label at end of compound statement");
   location_t endloc = c_parser_peek_token (parser)->location;
   aet_print_token(c_parser_peek_token(parser));
   n_debug("函数复合语句 88aa：goto statement  count:%d  nest:%d isctor:%d", testcount,classImpl->nest,classImpl->isConstructor);
   class_impl_nest_op(classImpl,FALSE);//zclei 10.4.0
   ...
   if(aetParser->isAet && classImpl->nest==0 && classImpl->isConstructor){//zclei 10.4.0
      //如果是构造函数加入返回语句
      nboolean re=class_ctor_add_return_stmt(classImpl->classCtor);//zclei 10.4.0
...
static attr_state c_parser_all_labels (c_parser *parser)
{
   attr_state attr = {};
   bool have_std_attrs;
   tree std_attrs = NULL;
   ...
   if (std_attrs
   && (!c_parser_handle_statement_omp_attributes (parser, std_attrs, &have_std_attrs)  || std_attrs)){
...
/* Helper function for c_parser_label: mangle a metadirective region
   label NAME.  */
static tree mangle_metadirective_region_label (c_parser *parser, tree name)
{
   if (parser->omp_metadirective_state->body_labels->contains (name)){
      ...
      sprintf (new_name, "%s_MDR%u", old_name, parser->omp_metadirective_state->region_num);
...
/* Parse a label (C90 6.6.1, C99 6.8.1, C11 6.8.1).
   ...
   function.  */
static void c_parser_label (c_parser *parser, tree std_attrs)
{
   ...
   if (c_parser_next_token_is_keyword (parser, RID_CASE)){
      tree exp1, exp2;
      c_parser_consume_token (parser);
      exp1 = convert_lvalue_to_rvalue (loc1, c_parser_expr_no_commas (parser, NULL), true, true).value;
      if (c_parser_next_token_is (parser, CPP_COLON)){
         c_parser_consume_token (parser);
         label = do_case (loc1, exp1, NULL_TREE, std_attrs);
      }else if (c_parser_next_token_is (parser, CPP_ELLIPSIS)){
         c_parser_consume_token (parser);
         exp2 = convert_lvalue_to_rvalue (loc1, c_parser_expr_no_commas (parser, NULL),true, true).value;
      ...
      if (attrs && c_parser_next_tokens_start_declaration (parser))
...
static void c_parser_statement (c_parser *parser, bool *if_p, location_t *loc_after_labels)
{
   tree before_labels = get_before_labels ();
...
static tree c_parser_bc_name (c_parser *parser, bool is_break)
{
   if (!c_parser_next_token_is (parser, CPP_NAME))
      return NULL_TREE;
...
static void c_parser_statement_after_labels (c_parser *parser, bool *if_p,
             tree before_labels,  vec<tree> *chain, attr_state astate)
{
   location_t loc = c_parser_peek_token (parser)->location;
   tree stmt = NULL_TREE;
...
restart:
   switch (c_parser_peek_token (parser)->type){
      case CPP_OPEN_BRACE:
         n_debug("语句标签后 11 CPP_OPEN_BRACE");
         add_stmt (c_parser_compound_statement (parser));
         break;
      case CPP_KEYWORD:
         n_debug("语句标签后 22 CPP_KEYWORD");
         switch (c_parser_peek_token (parser)->keyword){
            case RID_IF:
            ...
            case RID_RETURN:
               ...
               if (c_parser_next_token_is (parser, CPP_SEMICOLON)){
                  stmt = c_finish_return (loc, NULL_TREE, NULL_TREE, astate.musttail_p);
                  c_parser_consume_token (parser);
               }else{
                  location_t xloc = c_parser_peek_token (parser)->location;
                  //zclei return new$ Abc 说明返回的是一个指针对象类对象变量
                  if(c_parser_next_token_is_keyword(parser,RID_AET_NEW)){
                     new_object_set_return_newobject(new_object_get(),TRUE);
                  }else if(c_parser_next_token_is_keyword(parser,RID_AET_GENERIC_BLOCK)){
                     //zclei 说明 return genericblock$(... 泛型返回的类型与所在函数返回一样
                     gcc_assert(current_function_decl);
                     block_mgr_set_lhs(block_mgr_get(),TREE_TYPE(current_function_decl));
                  }
            ...
            case RID_ATTRIBUTE:
            {
               ...
               if (has_assume){
                  if (c_parser_next_token_is (parser, CPP_SEMICOLON))
                     ...
                     warning_at (loc, OPT_Wattributes, "%<assume%> attribute not followed by %<;%>");
                     has_assume = false;
                  }
               }
               gcc_assert (!astate.musttail_p);
               attrs = c_parser_handle_musttail (parser, attrs, astate);
               n_debug("c_parser_statement_after_labels 00");
               ...
               if (attribute_fallthrough_p (attrs)){
                  n_debug("c_parser_statement_after_labels 22");
                  if (c_parser_next_token_is (parser, CPP_SEMICOLON)){
                     tree fn = build_call_expr_internal_loc (loc,IFN_FALLTHROUGH,void_type_node, 0);
                     add_stmt (fn);
                     /* Eat the ';'.  */
                     c_parser_consume_token (parser);
                  }else{
                     auto_urlify_attributes sentinel;
                     warning_at (loc, OPT_Wattributes,"%<fallthrough%> attribute not followed by %<;%>");
                  }
               }else if (has_assume)
                  /* Eat the ';'.  */
                  c_parser_consume_token (parser);
            ...
            case RID_AET_NEW:
               n_debug("语句标签后 55 分析new$");
               new_object_parser_new$(new_object_get());//zclei
               break;
            case RID_AET_SUPER:
               n_debug("语句标签后 66 分析super$ ");
               if(class_impl_parser_super(classImpl))
                 break;
               else
                  goto expr_stmt;

            case RID_AET_GENERIC_BLOCK:
            n_debug("语句标签后 00nn 分析genericblock$ ");
            {
               struct c_expr expr=block_mgr_parser(block_mgr_get());
               stmt = c_finish_expr_stmt (loc, expr.value);
            }
               break;
            default:
               goto expr_stmt;
      ...
      case CPP_PRAGMA:
         if (!c_parser_pragma (parser, pragma_stmt, if_p, before_labels))
            goto restart;
         break;
      case CPP_NAME://zclei 处理 self,__shared__等 不要break,进入default处理。
         if(class_impl_replace_self_call_at_statement_after_labels(classImpl))//zclei 解决self()调用
            goto restart;
      default:
expr_stmt:
         n_debug("语句标签后 100xx 把整个赋值表达式语句的树结构挂接到语句列表上 ");
         stmt = c_finish_expr_stmt (loc, c_parser_expression_conv (parser).value);
         //zclei
         class_impl_in_finish_stmt(classImpl,loc,stmt);
expect_semicolon:
         c_parser_skip_until_found (parser, CPP_SEMICOLON, "expected %<;%>");
...
c_parser_selection_header (c_parser *parser, bool switch_p)
{
  ...
  tree std_attrs;
  bool have_std_attrs = c_parser_nth_token_starts_std_attributes (parser, 1);
  n_debug("c_parser_selection_header  00 have_std_attrs:%d",have_std_attrs);
  if (have_std_attrs)
    std_attrs = c_parser_std_attribute_specifier_sequence (parser);
  else
    std_attrs = NULL_TREE;
  if (c_parser_next_tokens_start_declaration (parser))
    {
     n_debug("c_parser_selection_header 11 c_parser_next_tokens_start_declaration:%d",have_std_attrs);
     aet_print_token(c_parser_peek_token (parser));

      pedwarn_c23 (loc, OPT_Wpedantic,
         "ISO C does not support if declarations before C2Y");
      expr.value = c_parser_declaration_or_fndef (parser,
                /*fndef_ok=*/false,
  ...
  n_debug("c_parser_selection_header  22 have_std_attrs:%d %d",have_std_attrs,parse_expr);
...
c_parser_paren_selection_header (c_parser *parser)
{
  ...
  if (c_parser_next_tokens_start_declaration (parser)){//zclei
     tree id =c_parser_peek_token (parser)->value;
...
static void c_parser_for_statement (c_parser *parser, bool ivdep, unsigned short unroll,
         bool novector, bool *if_p, tree before_labels)
{
   ...
   if (parser->omp_for_parse_state){
      error_at (for_loc,"loop not permitted in intervening code in OpenMP loop body");
      parser->omp_for_parse_state->fail = true;
   }
   ...
   if (parens.require_open (parser)){
      ...
      || c_parser_nth_token_starts_std_attributes (parser, 1)){
         c_parser_declaration_or_fndef (parser, true, true, true, true, true, false, &object_expression);
         parser->objc_could_be_foreach_context = false;
   ...
   init_expr:
      ...
      if (!is_foreach_statement){
         if (c_parser_next_token_is (parser, CPP_SEMICOLON)){
            if (ivdep){
               c_parser_error (parser, "missing loop condition in loop with %<GCC ivdep%> pragma");
               cond = error_mark_node;
            }else if (unroll){
               c_parser_error (parser, "missing loop condition in loop with %<GCC unroll%> pragma");
               cond = error_mark_node;
            }else{
               c_parser_consume_token (parser);
               cond = NULL_TREE;
         ...
         if (ivdep && cond != error_mark_node)
            cond = build3 (ANNOTATE_EXPR, TREE_TYPE (cond), cond,
         build_int_cst (integer_type_node,annot_expr_ivdep_kind),integer_zero_node);
         if (unroll && cond != error_mark_node)
            cond = build3 (ANNOTATE_EXPR, TREE_TYPE (cond), cond,
                  build_int_cst (integer_type_node, annot_expr_unroll_kind),build_int_cst (integer_type_node, unroll));
         if (novector && cond && cond != error_mark_node)
            cond = build3 (ANNOTATE_EXPR, TREE_TYPE (cond),
                  cond, build_int_cst (integer_type_node, annot_expr_no_vector_kind), integer_zero_node);
      }
      ...
      if (c_parser_next_token_is (parser, CPP_CLOSE_PAREN)){
         ...
         if (is_foreach_statement)
            collection_expression = c_fully_fold (c_parser_expression (parser).value, false, NULL);
         else{
   ...
   if (is_foreach_statement){
      ...
      if (loop_name){
         gcc_checking_assert (!DECL_CHAIN (loop_name) && !DECL_CHAIN (objc_foreach_break_label));
   ...
   token_indent_info body_tinfo = get_token_indent_info (c_parser_peek_token (parser));
   ...
   if (is_foreach_statement)
      objc_finish_foreach_loop (for_loc, object_expression,
            collection_expression, body,objc_foreach_break_label,objc_foreach_continue_label);
   else
      add_stmt (build_stmt (for_loc, FOR_STMT, NULL_TREE, cond, incr,body, NULL_TREE,
      loop_name && C_DECL_LOOP_SWITCH_NAME_USED (loop_name) ? loop_name : NULL_TREE, NULL_TREE, NULL_TREE));

   add_stmt (c_end_compound_stmt (for_loc, block,flag_isoc99 || c_dialect_objc ()));
   c_parser_maybe_reclassify_token (parser);

   token_indent_info next_tinfo = get_token_indent_info (c_parser_peek_token (parser));
   warn_for_misleading_indentation (for_tinfo, body_tinfo, next_tinfo);

   if (next_tinfo.type != CPP_SEMICOLON && !open_brace)
      warn_for_multistatement_macros (loc_after_labels, next_tinfo.location,for_tinfo.location, RID_FOR);
...
static struct c_expr c_parser_expr_no_commas (c_parser *parser, struct c_expr *after,
          tree omp_atomic_lhs)
{
   ...
   c_parser_peek_2nd_token (parser)->type==CPP_KEYWORD
   && c_parser_peek_2nd_token (parser)->keyword==RID_AET_NEW){
      //如果直接返回，会报‘警告：语句不起作用 [-Wunused-value]“ 是因为
      //abc=new$ Abc();变成了 abc;为了取消这个警告，在vale
      n_debug("abc=new$ Abc()这样的语句处理 用到c_aet_set_modify_stack_new 在c_parser_expression_conv用到 c_aet_get_is_modify_stack_new");
      new_object_modify(new_object_get(),op_location,lhs.value);
   }
   ...
   switch (c_parser_peek_token (parser)->type){
      ...
      default:
         ...
         //n_debug("c_parser_expr_no_commas 22 左边作赋值表达式处理，赋值运算符:%s code:%s lhs.value:%p",
              //cpp_type2name(c_parser_peek_token (parser)->type,0),get_tree_code_name(code),lhs.value);
         //zclei
         if(c_parser_next_token_is_keyword(parser,RID_AET_VAROF)){
            lhs=class_impl_varof_parser(classImpl,lhs);
         }else{
            lhs=class_impl_nameless_call(classImpl,lhs);
         }
         mtcs_parser_modify_check(mtcsParser,op_location,NULL_TREE,lhs.value);//zclei
         set_c_expr_source_range (&lhs, op_location, c_parser_peek_token (parser)->location);
         return lhs;
   }
   c_parser_consume_token (parser);
   ...
   if(c_parser_next_token_is_keyword(parser,RID_AET_GENERIC_BLOCK)){
      n_debug("c_parser_expr_no_commas  发现genericblock\n");
      block_mgr_set_lhs(block_mgr_get(),lhs.value);
   }
   rhs = c_parser_expr_no_commas (parser, NULL);
   //zclei 解决函数指针赋值问题 AHashFunc var=Abc.strHashFunc;
   rhs.value=class_impl_modify_or_init_func_pointer(classImpl,exp_location,lhs.value,rhs.value,rhs.original_type,true);
...
static struct c_expr c_parser_conditional_expression (c_parser *parser, struct c_expr *after,
             tree omp_atomic_lhs)
{
   struct c_expr cond, exp1, exp2, ret;
   location_t start, cond_loc, colon_loc;
   n_debug("c_parser_conditional_expression 00 条件表达式 ");
   bool save_c_omp_array_section_p = c_omp_array_section_p;
   ...
   if (c_parser_next_token_is_not (parser, CPP_QUERY)){
      n_debug("c_parser_conditional_expression 11 下一个token不是 CPP_QUERY 返回 after:%p",after);
      return cond;
   }
   ...
   c_parser_consume_token (parser);
   if (c_parser_next_token_is (parser, CPP_COLON)){
      n_debug("c_parser_conditional_expression 22 条件表达式  CPP_COLON ");
      tree eptype = NULL_TREE;

      location_t middle_loc = c_parser_peek_token (parser)->location;
      pedwarn (middle_loc, OPT_Wpedantic, "ISO C forbids omitting the middle term of a %<?:%> expression");
      ...
      n_debug("c_parser_conditional_expression 33 条件表达式   ");
      cond.value = c_objc_common_truthvalue_conversion (cond_loc, default_conversion (cond.value));
      c_inhibit_evaluation_warnings += cond.value == truthvalue_false_node;
      exp1 = c_parser_expression_conv (parser);
      mark_exp_read (exp1.value);
      c_inhibit_evaluation_warnings += ((cond.value == truthvalue_true_node) - (cond.value == truthvalue_false_node));
   }

   colon_loc = c_parser_peek_token (parser)->location;
   if (!c_parser_require (parser, CPP_COLON, "expected %<:%>")){
      n_debug("c_parser_conditional_expression 44 条件表达式    ");
      c_inhibit_evaluation_warnings -= cond.value == truthvalue_true_node;
      ret.set_error ();
      ret.original_code = ERROR_MARK;
      ret.original_type = NULL;
...
#define POP                            \
  do {                                 \
   ...
   stack[0].expr.m_decimal = 0;                    \
      ...
      stack[sp - 1].expr = aet_parser_build_binary_op (stack[sp].loc,       \
                     stack[sp].op,        \
                     stack[sp - 1].expr,        \
                     stack[sp].expr);        \
    sp--;                              \
  ...
  gcc_assert (!after || c_dialect_objc ());
  n_debug("c_parser_binary_expression 00 二元表达式 ");
 ...
 out:
  while (sp > 0)
    POP;
  n_debug("c_parser_binary_expression 33 二元表达式 expr.value:%p %s ",
          stack[0].expr.value,stack[0].expr.value?get_tree_code_name(TREE_CODE(stack[0].expr.value)):"NULL");
...
static struct c_declspecs *c_parser_compound_literal_scspecs (c_parser *parser)
{
...
static struct c_expr c_parser_cast_expression (c_parser *parser, struct c_expr *after)
{
   location_t cast_loc = c_parser_peek_token (parser)->location;
   gcc_assert (!after || c_dialect_objc ());
   n_debug("c_parser_cast_expression 00 强转表达式 c_expr *after %p ",after);
   aet_print_token(c_parser_peek_token (parser));
   if (after)
      return c_parser_postfix_expression_after_primary (parser,cast_loc, *after);
   //zclei 把泛型声明 E 转成aet_generic_E类型 或替换成实际类型
   if (c_parser_next_token_is (parser, CPP_OPEN_PAREN)
         && generic_impl_is_cast_token(generic_impl_get(),c_parser_peek_2nd_token (parser))){
   ...
   && c_token_starts_compound_literal (c_parser_peek_2nd_token (parser))){
      //n_debug("c_parser_cast_expression 11 强转表达式 重要!!! 强转(XXX *)CPP_OPEN_PAREN 第二是类型名 %p ",after);
      //zclei 判断是不是AObject.xxxx表达式。
      if(class_impl_is_class_dot_expression(classImpl)){
         return c_parser_unary_expression (parser);
      }
      //n_debug("c_parser_cast_expression 11xx 强转表达式 重要!!! 强转(XXX *)CPP_OPEN_PAREN 第二是类型名 %p ",after);
      ...
      type_name = parserTypeName/*!c_parser_type_name*/(aetParser,true);
      parens.skip_until_found_close (parser);
     // n_debug("c_parser_cast_expression 11yy 强转表达式 重要!!! 强转(XXX *)CPP_OPEN_PAREN 第二是类型名 %p ",after);
      if (type_name == NULL){
         ret.set_error ();
         ret.original_code = ERROR_MARK;
         ret.original_type = NULL;
      ...
      if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
         return c_parser_postfix_expression_after_paren_type (parser, scspecs, type_name,cast_loc);
      //n_debug("c_parser_cast_expression 11tt 强转表达式 重要!!! 强转(XXX *)CPP_OPEN_PAREN 第二是类型名 %p ",after);
      ...
      expr.value= generic_impl_cast(generic_impl_get(),type_name,expr.value);//zclei 10.4.0
      ret.value = c_cast_expr (cast_loc, type_name, expr.value);
      ret.value=class_cast_cast(classImpl->classCast,cast_loc,type_name,ret.value);//zclei 10.4.0
      generic_impl_ready_check_cast(generic_impl_get(),type_name,ret.value);//zclei 10.4.0
     // n_debug("c_parser_cast_expression 33 %p %s",ret.value,get_tree_code_name(TREE_CODE(ret.value)));
      ...
      n_debug("c_parser_cast_expression 44 强转表达式 进入一元表达式处理流程 ");
      return c_parser_unary_expression (parser);
   }
}
   ...
   switch (c_parser_peek_token (parser)->type){
      ...
      case CPP_PLUS:
         if (!c_dialect_objc () && !in_system_header_at (input_location))
            warning_at (op_loc, OPT_Wtraditional, "traditional C rejects the unary plus operator");
         c_parser_consume_token (parser);
         exp_loc = c_parser_peek_token (parser)->location;
         op = c_parser_cast_expression (parser, NULL);
         op = convert_lvalue_to_rvalue (exp_loc, op, true, true);
      ...
      case CPP_AND_AND:
         ...
         c_parser_consume_token (parser);
         if (c_parser_next_token_is (parser, CPP_NAME)){
            ret.value = finish_label_address_expr (c_parser_peek_token (parser)->value, op_loc);
            set_c_expr_source_range (&ret, op_loc, c_parser_peek_token (parser)->get_finish ());
            c_parser_consume_token (parser);
         }else{
            c_parser_error (parser, "expected identifier");
            ret.set_error ();
      ...
      case CPP_KEYWORD:
         switch (c_parser_peek_token (parser)->keyword){
            ...
            case RID_TRANSACTION_ATOMIC:
            case RID_TRANSACTION_RELAXED:
               return c_parser_transaction_expression (parser, c_parser_peek_token (parser)->keyword);
            //zclei
            case RID_AET_NEW:
            ...
            case RID_AET_GOTO:
            {
               ...
               if(!re && action==GOTO_MTCS_CREATE_OBJ){
                  c_token *tok = c_parser_peek_token (parser);
                  wide_int aw=wi::to_wide(tok->value);
                  unsigned long address=aw.to_shwi();
                  tree value=new_object_get_mtcs_plat_and_dev(new_object_get(),address);
                  if(value==NULL_TREE)
                     value=build_int_cst(unsigned_type_node,0);
                  c_parser_consume_token (parser);//consume   ADDRESS
                  ret.value=value;
                  return ret;//c_parser_postfix_expression (parser);
               }else
                  c_parser_error (parser, "RID_AET_GOTO关键字，不应该在c_parser_unary_expression这里出现，报告此问题");
            }
               return ret;
            default:
...
static struct c_expr c_parser_sizeof_expression (c_parser *parser)
{
   struct c_expr expr;
   struct c_expr result;
   location_t expr_loc;
   nboolean aetGeneric=FALSE;
   ...
   && c_token_starts_compound_literal (c_parser_peek_2nd_token (parser))){
      ...
      struct c_type_name *type_name;
      matching_parens parens;
      n_debug("c_parser_sizeof_expression 00 处理sizeof 并且()中是一个已知的类型");
      parens.consume_open (parser);
      ...
      if (c_parser_next_token_is (parser, CPP_OPEN_BRACE)){
         expr = c_parser_postfix_expression_after_paren_type (parser, scspecs, type_name, expr_loc);
      ...
      if (type_name->specs->alignas_p)
         error_at (type_name->specs->locations[cdw_alignas], "alignment specified for type name in %<sizeof%>");
      c_inhibit_evaluation_warnings--;
      in_sizeof--;
...
sizeof_expr:
      ...
      in_sizeof--;
      mark_exp_read (expr.value);
      if (TREE_CODE (expr.value) == COMPONENT_REF && DECL_C_BIT_FIELD (TREE_OPERAND (expr.value, 1)))
...
static struct c_expr c_parser_alignof_expression (c_parser *parser)
{
   struct c_expr expr;
   location_t start_loc = c_parser_peek_token (parser)->location;
   location_t end_loc;
   ...
   bool is_c11_alignof = (strcmp (IDENTIFIER_POINTER (alignof_spelling), "_Alignof") == 0
   || strcmp (IDENTIFIER_POINTER (alignof_spelling), "alignof") == 0);
   /* A diagnostic is not required for the use of this identifier in
   the implementation namespace; only diagnose it for the C11 or C23
   spelling because of existing code using the other spellings.  */
   if (is_c11_alignof){
      if (flag_isoc99)
         pedwarn_c99 (start_loc, OPT_Wpedantic, "ISO C99 does not support %qE", alignof_spelling);
      else
         pedwarn_c99 (start_loc, OPT_Wpedantic, "ISO C90 does not support %qE",alignof_spelling);
   }
   c_parser_consume_token (parser);
   c_inhibit_evaluation_warnings++;
   in_alignof++;
   ...
   && c_token_starts_compound_literal (c_parser_peek_2nd_token (parser))){
      ...
      if (c_parser_next_token_is (parser, CPP_OPEN_BRACE)){
         expr = c_parser_postfix_expression_after_paren_type (parser, scspecs, type_name,loc);
         goto alignof_expr;
      }
      /* alignof ( type-name ).  */
      if (scspecs)
      ...
      if (type_name->specs->alignas_p)
         error_at (type_name->specs->locations[cdw_alignas],"alignment specified for type name in %qE", alignof_spelling);
      c_inhibit_evaluation_warnings--;
      in_alignof--;
      ret.value = c_sizeof_or_alignof_type (loc, groktypename (type_name, NULL, NULL),false, is_c11_alignof, 1);
      ret.original_code = ERROR_MARK;
      ret.original_type = NULL;
...
alignof_expr:
      ...
      in_alignof--;
      if (is_c11_alignof)
         pedwarn (start_loc, OPT_Wpedantic, "ISO C does not allow %<%E (expression)%>", alignof_spelling);
...
c_parser_has_attribute_expression (c_parser *parser)
{
  ...
  if (c_parser_next_tokens_start_typename (parser, cla_prefer_id))
    {
      struct c_type_name *tname = parserTypeName/*!c_parser_type_name*/(aetParser);
      in_typeof--;
      if (tname)
   {
...
c_parser_generic_selection (c_parser *parser)
{
  ...
  while (1)
      ...
      else
   {
     ...
     type_name = parserTypeName/*!c_parser_type_name*/(aetParser);
     if (type_name == NULL)
       {
         c_parser_skip_until_found (parser, CPP_CLOSE_PAREN, NULL);
         return error_expr;
...
/* Helper function for c_parser_postfix_expression.  Parse predefined
   identifiers.  */
static struct c_expr c_parser_predefined_identifier (c_parser *parser)
{
   location_t loc = c_parser_peek_token (parser)->location;
   switch (c_parser_peek_token (parser)->keyword){
      ...
      case RID_C99_FUNCTION_NAME:
         ...
         n_debug("aetparser.c c_parser_predefined_identifier RID_C99_FUNCTION_NAME\n");
         break;
      default:
         gcc_unreachable ();
   }
   ...
   expr.value = fname_decl (loc, c_parser_peek_token (parser)->keyword,c_parser_peek_token (parser)->value);
   n_debug("检查两个函数中的__func__变量声明是不是同一个:%p %d %d\n",expr.value,c_parser_peek_token (parser)->keyword,RID_C99_FUNCTION_NAME);
...
static struct c_expr c_parser_postfix_expression (c_parser *parser)
{
   ...
   switch (c_parser_peek_token (parser)->type){
      case CPP_NUMBER:
         ...
         c_parser_consume_token (parser);
         if (TREE_CODE (expr.value) == FIXED_CST   && !targetm.fixed_point_supported_p ()){
      ...
      case CPP_WSTRING:
      case CPP_UTF8STRING:
         expr = c_parser_string_literal (parser, parser->translate_strings_p,true);
         break;
      case CPP_OBJC_STRING:
         gcc_assert (c_dialect_objc ());
         expr.value = objc_build_string_object (c_parser_peek_token (parser)->value);
         set_c_expr_source_range (&expr, tok_range);
         c_parser_consume_token (parser);
         break;
      case CPP_NAME:
         /*
         * 如果在aet中，如果是函数声明，前面没有指针引用，算是impl class中的函数调用要传self进入
         */
         switch (c_parser_peek_token (parser)->id_kind){
            case C_ID_ID:
            {
               tree id = c_parser_peek_token (parser)->value;
               location_t idLoc = c_parser_peek_token (parser)->location;
               c_parser_consume_token (parser);
               GenericModel *genericDefineModel=NULL;//zclei 10.4.0
               if(c_parser_peek_token (parser)->type== CPP_LESS){
                  genericDefineModel=class_impl_get_func_generic_model(classImpl,id);//zclei
               }
               nboolean isFun=(c_parser_peek_token (parser)->type== CPP_OPEN_PAREN);
               int action=0;
               expr= class_impl_process_expression(classImpl,expr,loc,genericDefineModel,id,isFun,&action);//zclei
               n_debug("c_parser_postfix_expression 33 后缀表达式 CPP_NAME:%s action:%d original_type:%p expr:%p action:%d",
                     IDENTIFIER_POINTER(id),action,expr.original_type,&expr,action);
               if(action==2){
                  n_debug("c_parser_postfix_expression 44 在aet中函数解析 CPP_NAME:%s action:%d ",
                        IDENTIFIER_POINTER(id),action);
                  break;
               }
               if(action==1){
                  n_debug("c_parser_postfix_expression 55 在aet中变量解析 变成指针操作了.CPP_NAME:%s action:%d ",
                        IDENTIFIER_POINTER(id),action);
                  id = c_parser_peek_token (parser)->value;//把refCount变量变成self->refCount 这里id变成了"self"
                  c_parser_consume_token (parser);
               }

               if(action==3){
                  n_debug("c_parser_postfix_expression 66 这是在aet中调用implicitly函数:%s action:%d ",IDENTIFIER_POINTER(id),action);
                  set_c_expr_source_range (&expr, tok_range);
                  break;
               }
               if(action==4){
               ...
               if(aetParser->isAet && mtcs_parser_is_compiling(mtcsParser)){
                   ...
                   n_debug("正在编译的是MTCS函数，可能token是一个内置变量 %s decl:%p",IDENTIFIER_POINTER(id),decl);
                   if(decl){
                      expr.original_code = ERROR_MARK;
                      expr.original_type = NULL;
                      expr.value = decl;
               ...
               expr.value = build_external_ref (loc, id,isFun, &expr.original_type);
               generic_parser_record_const_decl(generic_parser_get(), idLoc,id,expr.value);
               mtcs_parser_postfix_expression (mtcsParser,loc,expr.value);
               n_debug("c_parser_postfix_expression 77 后缀表达式 id:%s expr.value:%p ",
                     IDENTIFIER_POINTER(id),expr.value);
               set_c_expr_source_range (&expr, tok_range);
               break;
            }
            case C_ID_CLASSNAME:
            {
               ...
               c_parser_consume_token (parser);
               expr.value = objc_build_class_component_ref (class_name,component);
...
classAccessLabel:
               ...
               n_info("注意 这里是Class.name的调用 或者是Enum.name的调用 isClassAndDot:%d isEnumAndDot:%d",
                     isClassAndDot,isEnumAndDot);
               if(isClassAndDot){
                  class_impl_build_class_dot(classImpl,loc,&expr);
               }else if(isEnumAndDot){
                  class_impl_build_enum_dot(classImpl,loc,&expr);
               }else{
      ...
      case CPP_OPEN_PAREN:
         ...
         if (c_parser_peek_2nd_token (parser)->type == CPP_OPEN_BRACE){
            ...
            if (!building_stmt_list_p () || old_style_parameter_scope ()){
               error_at (loc, "braced-group within expression allowed only inside a function");
               parser->error = true;
               c_parser_skip_until_found (parser, CPP_CLOSE_BRACE, NULL);
               c_parser_skip_until_found (parser, CPP_CLOSE_PAREN, NULL);
               expr.set_error ();
            ...
            c_parser_skip_until_found (parser, CPP_CLOSE_PAREN,"expected %<)%>");
            pedwarn (loc, OPT_Wpedantic,  "ISO C forbids braced-groups within expressions");
            ...
            if (expr.original_code != C_MAYBE_CONST_EXPR  && expr.original_code != SIZEOF_EXPR)
               expr.original_code = ERROR_MARK;
            ...
            c_parser_skip_until_found (parser, CPP_CLOSE_PAREN,"expected %<)%>", loc_open_paren);
         }
         break;
      case CPP_KEYWORD:
         switch (c_parser_peek_token (parser)->keyword){
            ...
            case RID_VA_ARG:
            {
               ...
               loc = c_parser_peek_token (parser)->location;
               t1 = parserTypeName/*!c_parser_type_name*/(aetParser);
               ...
               if (t1 == NULL){
                  ...
                  if (type_expr){
                     expr.value = build2 (C_MAYBE_CONST_EXPR, TREE_TYPE (expr.value), type_expr,expr.value);
            ...
            case RID_C23_VA_START:
            {
               ...
               if (!c_parser_next_token_is (parser, CPP_CLOSE_PAREN)){
                  ...
                  if (c_parser_next_token_is (parser, CPP_NAME)  && c_parser_peek_token (parser)->id_kind == C_ID_ID
                  && (c_parser_peek_2nd_token (parser)->type == CPP_CLOSE_PAREN)){
                     ...
                     if (!last_parm || decl != last_parm)
                        warning_at (nloc, OPT_Wvarargs, "optional second parameter of %<va_start%> not last named argument");
               ...
               expr.value = c_build_function_call_vec (loc, arg_loc, fndecl,params, NULL);
               set_c_expr_source_range (&expr, loc,parser->tokens_buf[0].get_finish ());
               expr.m_decimal = 0;
               expr.original_code = ERROR_MARK;
               expr.original_type = NULL;
            ...
            case RID_OFFSETOF:
            {
               ...
               if (c_parser_next_token_is (parser, CPP_NAME)){
                  ...
                  offsetof_ref = build_component_ref (loc, offsetof_ref, comp_tok->value, comp_tok->location, UNKNOWN_LOCATION,false);
                  c_parser_consume_token (parser);
                  ...
                  || c_parser_next_token_is (parser,CPP_DEREF)){
                     if (c_parser_next_token_is (parser, CPP_DEREF)){
                        loc = c_parser_peek_token (parser)->location;
                        offsetof_ref = build_array_ref (loc, offsetof_ref, integer_zero_node);
                        goto do_dot;
                        ...
                        offsetof_ref = build_component_ref (loc, offsetof_ref,
                        comp_tok->value, comp_tok->location,UNKNOWN_LOCATION, false);
                        c_parser_consume_token (parser);
                     }else{
                        struct c_expr ce;
                        tree idx;
            ...
            case RID_CHOOSE_EXPR:
            {
               ...
               c_parser_consume_token (parser);
               if (!c_parser_get_builtin_args (parser, "__builtin_choose_expr", &cexpr_list, true, &close_paren_loc)){
                  expr.set_error ();
                  break;
               }

               if (vec_safe_length (cexpr_list) != 3){
                  error_at (loc, "wrong number of arguments to %<__builtin_choose_expr%>");
                  expr.set_error ();
                  break;
               }
               ...
               if (TREE_CODE (c) != INTEGER_CST  || !INTEGRAL_TYPE_P (TREE_TYPE (c)))
                  error_at (loc, "first argument to %<__builtin_choose_expr%> not a constant");
            ...
            case RID_TYPES_COMPATIBLE_P:
            {
               ...
               t2 = parserTypeName/*!c_parser_type_name*/(aetParser);
               if (t2 == NULL){
                  expr.set_error ();
                  break;
               }
               ...
               expr.value = comptypes (e1, e2) ? integer_one_node : integer_zero_node;
               set_c_expr_source_range (&expr, loc, close_paren_loc);
            }
               break;
            case RID_BUILTIN_TGMATH:
            {
               ...
               c_parser_consume_token (parser);
               if (!c_parser_get_builtin_args (parser,  "__builtin_tgmath", &cexpr_list, false, &close_paren_loc)){
                  expr.set_error ();
                  break;
               }
               ...
               for (unsigned int j = 1; j < num_functions; j++){
                  unsigned int this_nargs  = check_tgmath_function (&(*cexpr_list)[j], j + 1);
                  if (this_nargs == 0){
                     expr.set_error ();
                     goto out;
                  }
               ...
               for (unsigned int j = 1; j < num_functions; j++){
                  ...
                  if (ret != parm_first[0]){
                     parm_varies[0] = true;
                     if (!SCALAR_FLOAT_TYPE_P (parm_first[0])  && !COMPLEX_FLOAT_TYPE_P (parm_first[0])){
                     ...
                     if (!SCALAR_FLOAT_TYPE_P (ret)  && !COMPLEX_FLOAT_TYPE_P (ret)){
                        error_at ((*cexpr_list)[j].get_location (),
                        "invalid type-generic return type for "
                        "argument %u of %<__builtin_tgmath%>",
                        j + 1);
                  ...
                  FOREACH_FUNCTION_ARGS (type, t, iter){
                     ...
                     if (t != parm_first[argpos]){
                        ...
                        if (!SCALAR_FLOAT_TYPE_P (parm_first[argpos]) && !COMPLEX_FLOAT_TYPE_P (parm_first[argpos])){
                           error_at ((*cexpr_list)[0].get_location (),
                        ...
                        if (!SCALAR_FLOAT_TYPE_P (t)  && !COMPLEX_FLOAT_TYPE_P (t)){
                           error_at ((*cexpr_list)[j].get_location (),
                           "invalid type-generic type for "
                           "argument %u of argument %u of "
               ...
               if (max_variation == tgmath_fixed){
                  error_at (loc, "function arguments of %<__builtin_tgmath%> all have the same type");
                  expr.set_error ();
                  break;
               }
               ...
               if (tgarg == 0){
                  error_at (loc, "function arguments of %<__builtin_tgmath%> lack type-generic parameter");
                  expr.set_error ();
                  break;
               }
               ...
               for (unsigned int j = 0; j < num_functions; j++){
                  ...
                  FOREACH_FUNCTION_ARGS (type, t, iter){
                     if (t == void_type_node)
                        break;
                     t = TYPE_MAIN_VARIANT (t);
                     if ((parm_kind[argpos] == tgmath_complex  && t != exp_type)
                     || (parm_kind[argpos] == tgmath_real  && t != exp_real_type)){
                        error_at ((*cexpr_list)[j].get_location (),
               ...
               for (unsigned int j = 1; j <= nargs; j++){
                  ...
                  if (!INTEGRAL_TYPE_P (type)   && !SCALAR_FLOAT_TYPE_P (type)  && TREE_CODE (type) != COMPLEX_TYPE){
               ...
               tree arg_type = (arg_complex  ? build_complex_type (arg_real) : arg_real);
               ...
               if (fn == NULL && parm_kind[0] == tgmath_fixed && SCALAR_FLOAT_TYPE_P (parm_first[0])){
                  ...
                  type.  */
                  for (unsigned int j = 0; j < num_functions; j++){
                     if (arg_complex != (TREE_CODE (tg_type[j]) == COMPLEX_TYPE))
                        continue;
                     tree real_tg_type = (arg_complex ? TREE_TYPE (tg_type[j]) : tg_type[j]);
                     if (DECIMAL_FLOAT_TYPE_P (arg_real) != DECIMAL_FLOAT_TYPE_P (real_tg_type))
                        continue;
                     scalar_float_mode arg_mode = SCALAR_FLOAT_TYPE_MODE (arg_real);
                     scalar_float_mode tg_mode = SCALAR_FLOAT_TYPE_MODE (real_tg_type);
                     const real_format *arg_fmt = REAL_MODE_FORMAT (arg_mode);
                     const real_format *tg_fmt = REAL_MODE_FORMAT (tg_mode);
                     if (arg_fmt->b == tg_fmt->b  && arg_fmt->p <= tg_fmt->p
                     && arg_fmt->emax <= tg_fmt->emax  && (arg_fmt->emin - arg_fmt->p >= tg_fmt->emin - tg_fmt->p)){
                        fn = &(*cexpr_list)[j];
                        break;
                     }
                  }
               ...
               expr.value = c_build_function_call_vec (loc, arg_loc, fn->value,args, origtypes);
               set_c_expr_source_range (&expr, loc, close_paren_loc);
               break;
            } //结束 case RID_BUILTIN_TGMATH:
            ...
            case RID_BUILTIN_CALL_WITH_STATIC_CHAIN:
            {
               ...
               c_parser_consume_token (parser);
               if (!c_parser_get_builtin_args (parser, "__builtin_call_with_static_chain", &cexpr_list, false, &close_paren_loc)){
                  expr.set_error ();
                  break;
               }
               if (vec_safe_length (cexpr_list) != 2){
                  error_at (loc, "wrong number of arguments to %<__builtin_call_with_static_chain%>");
                  expr.set_error ();
                  break;
               }
            ...
            case RID_BUILTIN_COMPLEX:
            {
               ...
               c_parser_consume_token (parser);
               if (!c_parser_get_builtin_args (parser, "__builtin_complex", &cexpr_list, false, &close_paren_loc)){
                  expr.set_error ();
                  break;
               }

               if (vec_safe_length (cexpr_list) != 2){
                  error_at (loc, "wrong number of arguments to %<__builtin_complex%>");
                  expr.set_error ();
                  break;
               }
               ...
               *e1_p = convert_lvalue_to_rvalue (loc, *e1_p, true, true);
               if (TREE_CODE (e1_p->value) == EXCESS_PRECISION_EXPR)
                  e1_p->value = convert (TREE_TYPE (e1_p->value), TREE_OPERAND (e1_p->value, 0));
               *e2_p = convert_lvalue_to_rvalue (loc, *e2_p, true, true);
               if (TREE_CODE (e2_p->value) == EXCESS_PRECISION_EXPR)
                  e2_p->value = convert (TREE_TYPE (e2_p->value),TREE_OPERAND (e2_p->value, 0));
               ...
               if (TYPE_MAIN_VARIANT (TREE_TYPE (e1_p->value))  != TYPE_MAIN_VARIANT (TREE_TYPE (e2_p->value))){
                  error_at (loc, "%<__builtin_complex%> operands of different types");
                  expr.set_error ();
                  break;
               }
               pedwarn_c90 (loc, OPT_Wpedantic,"ISO C90 does not support complex types");
               expr.value = build2_loc (loc, COMPLEX_EXPR,
                     build_complex_type (TYPE_MAIN_VARIANT (TREE_TYPE (e1_p->value))), e1_p->value, e2_p->value);
               set_c_expr_source_range (&expr, loc, close_paren_loc);
               break;
            }
            case RID_BUILTIN_COUNTED_BY_REF:
            {
               ...
               c_parser_consume_token (parser);
               if (!c_parser_get_builtin_args (parser, "__builtin_counted_by_ref", &cexpr_list, false, &close_paren_loc)){
                  expr.set_error ();
                  break;
               }
               if (vec_safe_length (cexpr_list) != 1){
                  error_at (loc, "wrong number of arguments to %<__builtin_counted_by_ref%>");
                  expr.set_error ();
                  break;
               }
               ...
               if (TREE_CODE (TREE_TYPE (ref)) != ARRAY_TYPE){
                  error_at (loc, "the argument to %<__builtin_counted_by_ref%> must be an array");
                  expr.set_error ();
                  break;
               }
            ...
            case RID_BUILTIN_SHUFFLE:
            {
               ...
               c_parser_consume_token (parser);
               if (!c_parser_get_builtin_args (parser, "__builtin_shuffle",&cexpr_list, false, &close_paren_loc)){
                  expr.set_error ();
                  break;
               }
               ...
               *p = convert_lvalue_to_rvalue (loc, *p, true, true);
               ...
               if (vec_safe_length (cexpr_list) == 2)
                  expr.value = c_build_vec_perm_expr (loc, (*cexpr_list)[0].value, NULL_TREE,(*cexpr_list)[1].value);

               else if (vec_safe_length (cexpr_list) == 3)
                  expr.value = c_build_vec_perm_expr (loc, (*cexpr_list)[0].value, (*cexpr_list)[1].value,(*cexpr_list)[2].value);
               else{
                  error_at (loc, "wrong number of arguments to %<__builtin_shuffle%>");
                  expr.set_error ();
               }
               set_c_expr_source_range (&expr, loc, close_paren_loc);
               break;
            ...
            case RID_BUILTIN_SHUFFLEVECTOR:
            {
               ...
               c_parser_consume_token (parser);
               if (!c_parser_get_builtin_args (parser, "__builtin_shufflevector", &cexpr_list, false, &close_paren_loc)){
                  expr.set_error ();
                  break;
               }
               ...
               if (vec_safe_length (cexpr_list) < 3){
                  ...
                  expr.value = c_build_shufflevector (loc, (*cexpr_list)[0].value, (*cexpr_list)[1].value, mask);
               }
               set_c_expr_source_range (&expr, loc, close_paren_loc);
               break;
            }
            case RID_BUILTIN_CONVERTVECTOR:
            {
               ...
               if (t1 == NULL)
                  ...
                  tree type_expr = NULL_TREE;
                  expr.value = c_build_vec_convert (start_loc, e1.value, loc, groktypename (t1, &type_expr,NULL));
                  set_c_expr_source_range (&expr, start_loc, end_loc);
               }
            }
               break;
            ...
            case RID_BUILTIN_STDC:
            {
               ...
               const char *name = IDENTIFIER_POINTER (c_parser_peek_token (parser)->value);
               unsigned num_args = 1;
               ...
               c_parser_consume_token (parser);
               if (!c_parser_get_builtin_args (parser, name, &cexpr_list, false, &close_paren_loc)){
                  expr.set_error ();
                  break;
               }
               ...
               *arg_p = convert_lvalue_to_rvalue (loc, *arg_p, true, true);
               ...
               if (TREE_CODE (TREE_TYPE (arg_p->value)) == ENUMERAL_TYPE){
                  error_at (loc, "argument %u in call to function %qs has enumerated type", 1, name);
                  expr.set_error ();
                  break;
               }
               if (TREE_CODE (TREE_TYPE (arg_p->value)) == BOOLEAN_TYPE){
                  error_at (loc, "argument %u in call to function %qs has boolean type", 1, name);
                  expr.set_error ();
                  break;
               }
               if (!TYPE_UNSIGNED (TREE_TYPE (arg_p->value))){
                  error_at (loc, "argument 1 in call to function %qs has signed type", name);
                  expr.set_error ();
                  break;
               }
               if (TYPE_MAIN_VARIANT (TREE_TYPE (arg_p->value)) == char_type_node){
                  error_at (loc, "argument 1 in call to function %qs has %<char%> type", name);
                  expr.set_error ();
                  break;
               }
               ...
               if (num_args == 2){
                  ...
                  *arg_p = convert_lvalue_to_rvalue (loc, *arg_p, true, true);
                  ...
                  if (TREE_CODE (TREE_TYPE (arg_p->value)) == ENUMERAL_TYPE){
                     error_at (loc, "argument %u in call to function %qs has enumerated type", 2, name);
                     expr.set_error ();
                     break;
                  }
                  ...
                  if (TREE_CODE (arg2) == INTEGER_CST  && tree_int_cst_sgn (arg2) < 0)
                     warning_at (loc, OPT_Wshift_count_negative,"rotate count is negative");

                  tree instrument_expr = NULL_TREE;
                  if (sanitize_flags_p (SANITIZE_SHIFT))
                     instrument_expr = ubsan_instrument_shift (loc, code, arg1, arg2);
                  ...
                  if (TYPE_PRECISION (TREE_TYPE (arg2))  < TYPE_PRECISION (integer_type_node))
                     arg2 = fold_convert (unsigned_type_node, arg2);

                  if (TYPE_UNSIGNED (TREE_TYPE (arg2)))
                     arg2 = build2_loc (loc, TRUNC_MOD_EXPR, TREE_TYPE (arg2), arg2, build_int_cst (TREE_TYPE (arg2),  prec));
                  else{
                     ...
                     arg2 = build2_loc (loc, TRUNC_MOD_EXPR, utype, fold_convert (utype, arg2), build_int_cst (utype, prec));
                  }

                  /* The middle-end isn't prepared to handle {L,R}ROTATE_EXPR
                  on types without mode precision, except for large/huge
                  _BitInt types.  */
                  if (type_has_mode_precision_p (TREE_TYPE (arg1)) || (TREE_CODE (TREE_TYPE (arg1)) == BITINT_TYPE
                  && prec > MAX_FIXED_MODE_SIZE))
                     expr.value = build2_loc (loc, code, TREE_TYPE (arg1), arg1, arg2);
                  else{
                     arg2 = save_expr (arg2);
                     tree t1 = build2_loc (loc, (code == LROTATE_EXPR ? LSHIFT_EXPR : RSHIFT_EXPR),TREE_TYPE (arg1), arg1, arg2);
                     tree t2 = build2_loc (loc, MINUS_EXPR, TREE_TYPE (arg2), build_int_cst (TREE_TYPE (arg2),prec), arg2);
                     t2 = build2_loc (loc, (code == LROTATE_EXPR  ? RSHIFT_EXPR : LSHIFT_EXPR), TREE_TYPE (arg1), arg1, t2);
                     suppress_warning (t2, OPT_Wshift_count_overflow);
                     tree t3 = build2_loc (loc, BIT_IOR_EXPR, TREE_TYPE (arg1), t1, t2);
                     tree t4 = build2_loc (loc, NE_EXPR, boolean_type_node, arg2, build_zero_cst (TREE_TYPE (arg2)));
                     t4 = build2_loc (loc, COMPOUND_EXPR, boolean_type_node, arg1, t4);
                     expr.value = build3_loc (loc, COND_EXPR,TREE_TYPE (arg1), t4, t3, arg1);
                  }
                  if (instrument_expr)
                     expr.value = build2_loc (loc, COMPOUND_EXPR, TREE_TYPE (expr.value),  instrument_expr, expr.value);
                  set_c_expr_source_range (&expr, loc, close_paren_loc);
                  break;
               }
               tree barg1 = arg;
               switch (stdc_rid){
                  case C_BUILTIN_STDC_BIT_CEIL:
                     arg = save_expr (arg);
                     barg1 = build2_loc (loc, PLUS_EXPR, type, arg, build_int_cst (type, -1));
                     break;
               ...
               expr.value = c_build_function_call_vec (loc, arg_loc, fndecl, args, origtypes);
               set_c_expr_source_range (&expr, loc, close_paren_loc);
               ...
               switch (stdc_rid){
                  ...
                  /* FALLTHRU */
                  case C_BUILTIN_STDC_BIT_WIDTH:
                     expr.value = build2_loc (loc, MINUS_EXPR, integer_type_node, build_int_cst (integer_type_node, prec), expr.value);
                     break;
                  case C_BUILTIN_STDC_FIRST_LEADING_ONE:
                  case C_BUILTIN_STDC_FIRST_LEADING_ZERO:
                  case C_BUILTIN_STDC_FIRST_TRAILING_ONE:
                  case C_BUILTIN_STDC_FIRST_TRAILING_ZERO:
                     expr.value = build2_loc (loc, PLUS_EXPR, integer_type_node, expr.value, integer_one_node);
                     break;
                  case C_BUILTIN_STDC_HAS_SINGLE_BIT:
                     expr.value = build2_loc (loc, EQ_EXPR, boolean_type_node, expr.value, integer_one_node);
                     break;
                  default:
                     break;
               }

               if (stdc_rid != C_BUILTIN_STDC_BIT_CEIL  && stdc_rid != C_BUILTIN_STDC_BIT_FLOOR){
                  if (stdc_rid != C_BUILTIN_STDC_HAS_SINGLE_BIT)
                     expr.value = fold_convert_loc (loc, unsigned_type_node, expr.value);
                  break;
               }
               ...
               if (integer_zerop (arg) || (stdc_rid == C_BUILTIN_STDC_BIT_CEIL && integer_onep (arg)))
                  expr.value = build_int_cst (type, 0);
               else
                  expr.value = build2_loc (loc, LSHIFT_EXPR, type,
                        build_int_cst (type, (stdc_rid == C_BUILTIN_STDC_BIT_CEIL ? 2 : 1)), expr.value);
               if (stdc_rid == C_BUILTIN_STDC_BIT_CEIL)
                  expr.value = build3_loc (loc, COND_EXPR, type,
                                       build2_loc (loc, LE_EXPR,  boolean_type_node, arg, build_int_cst (type, 1)),
                                       build_int_cst (type, 1),  expr.value);
               else
                  expr.value = build3_loc (loc, COND_EXPR, type,
                                       build2_loc (loc, EQ_EXPR, boolean_type_node, arg,  build_int_cst (type, 0)),
                                       build_int_cst (type, 0),  expr.value);
               break;
            } //结束      case RID_BUILTIN_STDC:
            ...
            case RID_AET_SUPER://zclei
               n_debug("c_parser_postfix_expression super 00 后缀表达式 RID_AET_SUPER ");
               expr=class_impl_parser_super_at_postfix_expression(classImpl,expr);//zclei
               break;
...
out:
   return c_parser_postfix_expression_after_primary (parser, EXPR_LOC_OR_LOC (expr.value, loc), expr);
}
...
static struct c_expr c_parser_postfix_expression_after_primary (c_parser *parser,
                  location_t expr_loc,
                  struct c_expr expr)
{
   ...
   location_t start;
   location_t finish;
   char *tempname=NULL;//zclei 测试用的
   n_debug("c_parser_postfix_expression_after_primary 00 type:%s expr:%p %s, ",
         cpp_type2name(c_parser_peek_token (parser)->type,0),expr.value, get_tree_code_name(TREE_CODE(expr.value)));
   while (true){
      location_t op_loc = c_parser_peek_token (parser)->location;
      SelectFunc selectFunc;//zclei
      ...
      switch (c_parser_peek_token (parser)->type){
         case CPP_OPEN_SQUARE:
            ...
            c_parser_consume_token (parser);
            idx = len = NULL_TREE;
            if (!c_omp_array_section_p  || c_parser_next_token_is_not (parser, CPP_COLON))
               idx = c_parser_expression (parser).value;

            if (c_omp_array_section_p && c_parser_next_token_is (parser, CPP_COLON)){
               c_parser_consume_token (parser);
               if (c_parser_next_token_is_not (parser, CPP_CLOSE_SQUARE))
                  len = c_parser_expression (parser).value;

               expr.value = build_omp_array_section (op_loc, expr.value, idx, len);
            }else
            ...
            finish = parser->tokens_buf[0].location;
            n_debug("c_parser_postfix_expression_after_primary --00 数组引用");
         ...
         /* Function call.  */
         {
            ...
            n_debug("c_parser_postfix_expression_after_primary 11 进入函数参数处理 %p %s ",
                  expr.value, get_tree_code_name(TREE_CODE(expr.value)));
            new_object_set_parser_parms_state(new_object_get(),TRUE);//zclei 10.4.0
            literal_zero_mask = 0;
            if (c_parser_next_token_is (parser, CPP_CLOSE_PAREN))
               exprlist = NULL;
            else if (TREE_CODE (expr.value) == FUNCTION_DECL
            && fndecl_built_in_p (expr.value, BUILT_IN_CLASSIFY_TYPE)
            && c_parser_next_tokens_start_typename (parser,  cla_prefer_id)){
               ...
               if (type != NULL)
                  ret.spec = groktypename (type, &ret.expr, &ret.expr_const_operands);
               parens.skip_until_found_close (parser);
               expr.value = build_int_cst (integer_type_node,  type_to_class (ret.spec));
               break;
            }else
               exprlist = c_parser_expr_list (parser, true, false,
                     &origtypes,sizeof_arg_loc, sizeof_arg, &arg_loc, &literal_zero_mask);
             /*
            {
              int i;
              for(i=0;i<arg_loc.length();i++){
                 printf("位置是:%d\n",i);
                 aet_print_location(arg_loc[i]);
              }
            }
            */
            ...
            /*
            {
               if(exprlist){
                 ...
                 for (ix = 0; exprlist->iterate (ix, &arg); ++ix){
                    n_debug("c_parser_postfix_expression_after_primary ttcc %p %s %d\n",
                          arg,get_tree_code_name(TREE_CODE(TREE_TYPE(arg))),TYPE_MODE(TREE_TYPE(arg)));
            ...
            */
            ...
            //parens.skip_until_found_close (parser);
            new_object_set_parser_parms_state(new_object_get(),FALSE);//zclei 10.4.0
            parens.skip_until_found_close (parser);//zclei 10.4.0源代码是： c_parser_skip_until_found (parser, CPP_CLOSE_PAREN,"expected %<)%>");
            ...
            if(class_impl_is_aet_class_component_ref_call(classImpl,expr)){
               if(!class_util_add_self(expr,&exprlist,&origtypes,&arg_loc)){
                  n_debug("需要加入参数self到参数列表中 参数个数:%d,但失败了。\n",vec_safe_length (exprlist));
                  break;
               }
            }
            expr=class_impl_replace_func_id(classImpl,expr,exprlist,origtypes,arg_loc,expr_loc,&selectFunc);//zclei
         }
         orig_expr = expr;
         n_debug("c_parser_postfix_expression_after_primary 33 函数参数处理完成 expr.value:%s :%p origtypes:%p 参数个数:%d",
               get_tree_code_name(TREE_CODE(expr.value)),expr.value,origtypes,vec_safe_length (exprlist));
         mark_exp_read (expr.value);
         if (warn_sizeof_pointer_memaccess)
            sizeof_pointer_memaccess_warning (sizeof_arg_loc,  expr.value, exprlist, sizeof_arg, sizeof_ptr_memacc_comptypes);
         if (TREE_CODE (expr.value) == FUNCTION_DECL){
            if (fndecl_built_in_p (expr.value, BUILT_IN_MEMSET)  && vec_safe_length (exprlist) == 3){
            ...
            && omp_runtime_api_call (expr.value)){
               error_at (expr_loc, "calls to the OpenMP runtime API are not permitted in intervening code");
               parser->omp_for_parse_state->fail = true;
            }
            if (warn_calloc_transposed_args)
               if (tree attr = lookup_attribute ("alloc_size", TYPE_ATTRIBUTES(TREE_TYPE (expr.value))))
                  if (TREE_VALUE (attr) && TREE_CHAIN (TREE_VALUE (attr)))
                     warn_for_calloc (sizeof_arg_loc, expr.value, exprlist, sizeof_arg, attr);
         }
            start = expr.get_start ();
            finish = parser->tokens_buf[0].get_finish ();
            GenericModel *defineGenModel;//zclei 如果是调用泛型函数，返回定义的泛型模型
            //zclei
            expr.value =class_impl_build_function_call_vec(classImpl,expr_loc, arg_loc,
                  expr.value, exprlist, origtypes,&selectFunc,&defineGenModel);
            if(selectFunc.classFunc){
               n_debug("c_parser_postfix_expression_after_primary 44 整合 expr.value:%p %s origtypes:%p classfunc:%p name:%s",
                     expr.value,get_tree_code_name(TREE_CODE(expr.value)),origtypes,
                     selectFunc.classFunc,selectFunc.classFunc->mangleFunName);
            }
            expr.value=generic_call_convert_generic_to_user(generic_call_get(),expr.value);//zclei 10.4.0
            expr=class_impl_nameless_call(classImpl,expr);//zclei 10.4.0
            class_impl_add_implicitly_actual_param(classImpl,expr.value, exprlist,origtypes);//zclei
            //处理核函数，转为MtcsLanuch的lanuch调用。
            expr.value=class_impl_record_mtcs_call(classImpl,expr_loc,selectFunc.funcdecl,
         ...
         case CPP_DOT:
            ...
            n_debug("c_parser_postfix_expression_after_primary 55 DOT引用. %p %s ",
            expr.value, get_tree_code_name(TREE_CODE(expr.value)));
            c_parser_consume_token (parser);
            ...
            c_parser_consume_token (parser);
            if(class_dot_is_class_ref(classImpl->classDot,expr.value)){//zclei 10.4.0
               n_debug("c_parser_postfix_expression_after_primary 66 DOT引用.name:%s是一个aet class %p %s ",
               IDENTIFIER_POINTER(ident),expr.value, get_tree_code_name(TREE_CODE(expr.value)));
               expr.value =class_dot_build_ref(classImpl->classDot, op_loc,comp_loc,ident,expr.value);
            }else{
         ...
         case CPP_DEREF:
            /* Structure element reference.  */
            n_debug("c_parser_postfix_expression_after_primary 77 指针引用-> %p %s ",
            expr.value, get_tree_code_name(TREE_CODE(expr.value)));
            c_parser_consume_token (parser);
            ...
            if (c_parser_next_token_is (parser, CPP_NAME)){
               ...
               ident = comp_tok->value;
               comp_loc = comp_tok->location;
               n_debug("c_parser_postfix_expression_after_primary 88 指针引用-> name:%s expr.value:%p %s ",
                     IDENTIFIER_POINTER(ident),expr.value,get_tree_code_name(TREE_CODE(expr.value)));
            }else{
               c_parser_error (parser, "expected identifier");
               expr.set_error ();
            ...
            finish = c_parser_peek_token (parser)->get_finish ();
            c_parser_consume_token (parser);
            //如果expr.value是aetclass类型由 classref处理，否则系统处理
            if(class_ref_is_class_ref(classImpl->classRef,expr.value)){
               expr.value =class_impl_build_deref(classImpl, op_loc,comp_loc,ident,expr.value);
            }else{
               expr.value = build_component_ref (op_loc,
                     build_indirect_ref (op_loc,expr.value, RO_ARROW), ident, comp_loc, expr.get_location ());
               n_debug("c_parser_postfix_expression_after_primary 99 指针引用-> name:%s expr.value:%p ",IDENTIFIER_POINTER(ident),expr.value);
            }
            set_c_expr_source_range (&expr, start, finish);
            expr.original_code = ERROR_MARK;
            if (TREE_CODE (expr.value) != COMPONENT_REF)
         ...
         case CPP_PLUS_PLUS:
            /* Postincrement.  */
            n_debug("c_parser_postfix_expression_after_primary 100 CPP_PLUS_PLUS %s ",
         ...
         case CPP_MINUS_MINUS:
            ...
            expr.original_type = NULL;
            break;
         //zclei 原本没有处理CPP_RSHIFT的代码，为了实现<<<...>>>加入。
         case CPP_RSHIFT:
            if(c_parser_peek_2nd_token (parser)->type==CPP_GREATER){
               n_debug("c_parser_postfix_expression_after_primary 101  进这里了-------把 >>>替换成)\n");
               mtcs_parser_replace_rshift_and_greater(mtcs_parser_get());
            }
            return expr;
         default:
            return expr;
...
c_parser_expression (c_parser *parser)
{
  ...
  struct c_expr expr;
     n_debug("c_parser_expression:00 对表达式实质性分析 先把即将分析的表达式当作赋值表达式处理");
    aet_print_token(c_parser_peek_token(parser));
  expr = c_parser_expr_no_commas (parser, NULL);
  if (c_parser_next_token_is (parser, CPP_COMMA))
  ...
  while (c_parser_next_token_is (parser, CPP_COMMA))
    {
           n_debug("c_parser_expression:11 有CPP_COMMA");
      struct c_expr next;
      tree lhsval;
      location_t loc = c_parser_peek_token (parser)->location;
      location_t expr_loc;
...
/* Parse an expression and convert functions or arrays to pointers and
   lvalues to rvalues.  */
static struct c_expr c_parser_expression_conv (c_parser *parser)
{
   struct c_expr expr;
   location_t loc = c_parser_peek_token (parser)->location;
   expr = c_parser_expression (parser);
   expr = convert_lvalue_to_rvalue (loc, expr, true, false);
   if(expr.value && (VAR_P(expr.value) || TREE_CODE(expr.value)==COMPONENT_REF) && c_aet_get_is_modify_stack_new(expr.value)){
      n_info("c_parser_expression_conv 44 保护确保只有一次 c_aet_set_modify_stack_new(expr.value,0) %p",expr.value);
      c_aet_set_modify_stack_new(expr.value,0);//zclei 10.4.0
      expr.value=NULL_TREE;
   }
   return expr;
}
...
static void c_parser_objc_try_catch_finally_statement (c_parser *parser)
{
   location_t location;
   tree stmt;
...
*/
static tree c_parser_omp_dispatch_body (c_parser *parser)
{
   struct c_expr lhs, rhs, ret;
   location_t expr_loc = c_parser_peek_token (parser)->location;
   source_range tok_range = c_parser_peek_token (parser)->get_range ();
   ...
   if (TREE_CODE (rhs.value) != CALL_EXPR){
      error_at (EXPR_LOC_OR_LOC (rhs.value, expr_loc),"expected target-function call");
      return error_mark_node;
   }

   /* Build assignment. */
   rhs = convert_lvalue_to_rvalue (expr_loc, rhs, true, true);
   ret.value = build_modify_expr (op_location, lhs.value, lhs.original_type, NOP_EXPR,expr_loc, rhs.value, rhs.original_type);
   ret.m_decimal = 0;
   set_c_expr_source_range (&ret, lhs.get_start (), rhs.get_finish ());
   ret.original_code = MODIFY_EXPR;
   ret.original_type = NULL;
...
static tree c_parser_omp_context_selector_specification (c_parser *parser, tree parms)
{
   tree ret = NULL_TREE;
   do{
      const char *setp = "";
...
static void c_parser_omp_declare_reduction (c_parser *parser, enum pragma_context context)
{
   ...
   while (true){
      location_t loc = c_parser_peek_token (parser)->location;
      struct c_type_name *ctype =parserTypeName/*!c_parser_type_name*/(aetParser);
      if (ctype != NULL){
         ...
         && orig_reduc_id == NULL_TREE)
            error_at (loc, "predeclared arithmetic type in %<#pragma omp declare reduction%>");
         else if (TREE_CODE (type) == FUNCTION_TYPE
         || TREE_CODE (type) == ARRAY_TYPE)
            error_at (loc, "function or array type in %<#pragma omp declare reduction%>");
         else if (TYPE_ATOMIC (type))
            error_at (loc, "%<_Atomic%> qualified type in %<#pragma omp declare reduction%>");
         else if (TYPE_QUALS_NO_ADDR_SPACE (type))
            error_at (loc, "const, volatile or restrict qualified type in %<#pragma omp declare reduction%>");
         else{
            tree t;
            for (t = DECL_INITIAL (reduc_decl); t; t = TREE_CHAIN (t))
               if (comptypes (TREE_PURPOSE (t), type)){
                  error_at (loc, "redeclaration of %qs %<#pragma omp declare reduction%> for type %qT",
                        IDENTIFIER_POINTER (reduc_id) + sizeof ("omp declare reduction ") - 1,type);
                  location_t ploc = DECL_SOURCE_LOCATION (TREE_VEC_ELT (TREE_VALUE (t),0));
                  error_at (ploc, "previous %<#pragma omp declare reduction%>");
                  break;
               }
            if (t == NULL_TREE)
   ...
   if (!c_parser_require (parser, CPP_COLON, "expected %<:%>")  || types.is_empty ()){
fail:
      clauses.release ();
      types.release ();
      while (true){
   ...
   FOR_EACH_VEC_ELT (types, i, type){
      ...
      if (nested)
         c_push_function_context ();
      tree fndecl = build_decl (BUILTINS_LOCATION, FUNCTION_DECL,reduc_id, default_function_type);
      ...
      warn about these.  */
      tree omp_out = build_decl (BUILTINS_LOCATION, VAR_DECL, get_identifier ("omp_out"), type);
      DECL_ARTIFICIAL (omp_out) = 1;
      DECL_CONTEXT (omp_out) = fndecl;
      pushdecl (omp_out);
      tree omp_in = build_decl (BUILTINS_LOCATION, VAR_DECL,  get_identifier ("omp_in"), type);
      ...
      if (!c_parser_require (parser, CPP_CLOSE_PAREN, "expected %<)%>"))
         bad = true;
      else if (c_parser_next_token_is (parser, CPP_COMMA) && c_parser_peek_2nd_token (parser)->type == CPP_NAME)
         c_parser_consume_token (parser);
      if (!bad && (c_parser_next_token_is (parser, CPP_NAME)
      && strcmp (IDENTIFIER_POINTER(c_parser_peek_token (parser)->value),"initializer") == 0)){
         c_parser_consume_token (parser);
         pop_scope ();
         push_scope ();
         ...
         if (!c_parser_require (parser, CPP_OPEN_PAREN, "expected %<(%>"))
            ...
            c_parser_error (parser, "expected %<omp_priv%> or function-name");
            bad = true;
         }else if (strcmp (IDENTIFIER_POINTER (c_parser_peek_token (parser)->value), "omp_priv") != 0){
            if (c_parser_peek_2nd_token (parser)->type != CPP_OPEN_PAREN
                  || c_parser_peek_token (parser)->id_kind != C_ID_ID){
               c_parser_error (parser, "expected function-name %<(%>");
               bad = true;
            ...
            if (initializer.value && TREE_CODE (initializer.value) == CALL_EXPR){
               int j;
               tree c = initializer.value;
               for (j = 0; j < call_expr_nargs (c); j++){
                  tree a = CALL_EXPR_ARG (c, j);
                  STRIP_NOPS (a);
                  if (TREE_CODE (a) == ADDR_EXPR && TREE_OPERAND (a, 0) == omp_priv)
                     break;
               }
            ...
            if (!c_parser_require (parser, CPP_EQ, "expected %<=%>"))
               ...
               finish_init ();
               finish_decl (omp_priv, loc, init.value, init.original_type, NULL_TREE);
               pop_stmt_list (st);
            }
         }
         if (!bad && !c_parser_require (parser, CPP_CLOSE_PAREN, "expected %<)%>"))
            bad = true;
      }
      ...
      if (!bad){
         ...
         tree t = tree_cons (type, make_tree_vec (omp_priv ? 6 : 3),DECL_INITIAL (reduc_decl));
         ...
         walk_tree (&combiner.value, c_check_omp_declare_reduction_r, &TREE_VEC_ELT (TREE_VALUE (t), 0), NULL);
         if (omp_priv){
            ...
            walk_tree (&initializer.value, c_check_omp_declare_reduction_r, &TREE_VEC_ELT (TREE_VALUE (t), 3), NULL);
            walk_tree (&DECL_INITIAL (omp_priv), c_check_omp_declare_reduction_r, &TREE_VEC_ELT (TREE_VALUE (t), 3), NULL);
         }
      }
...
static bool c_parser_omp_declare (c_parser *parser, enum pragma_context context)
{
   c_parser_consume_pragma (parser);
   if (c_parser_next_token_is (parser, CPP_NAME)){
      const char *p = IDENTIFIER_POINTER (c_parser_peek_token (parser)->value);
   ...
   c_parser_error (parser, "expected %<simd%>, %<reduction%>, %<target%> or %<variant%>");
   c_parser_skip_to_pragma_eol (parser);
   return false;
}
...
static void c_parser_omp_requires (c_parser *parser)
{
...
static tree c_finish_taskloop_clauses (tree clauses)
{
   tree *pc = &clauses;
   for (tree c = clauses; c; c = *pc){
      ...
      if (OMP_CLAUSE_CODE (c) == OMP_CLAUSE_REDUCTION){
         ...
         if (integer_zerop (TYPE_SIZE_UNIT (type))){
            error_at (OMP_CLAUSE_LOCATION (c),"zero sized type %qT in %<reduction%> clause", type);
            remove = true;
         }else if (TREE_CODE (TYPE_SIZE_UNIT (type)) != INTEGER_CST){
            error_at (OMP_CLAUSE_LOCATION (c),"variable sized type %qT in %<reduction%> clause",type);
            remove = true;
         }
      }
      if (remove)
...
location_t c_parser_parse_rtl_body (c_parser *parser, char *start_with_pass)
{
   if (!c_parser_require (parser, CPP_OPEN_BRACE, "expected %<{%>")){
...
static void aet_parser_c_parser_declspecs_enum_dot (AetParser *self,struct c_typespec *t)
{
   ...
   //解析 AObject.EnumName
   if(c_parser_next_token_is (parser, CPP_DOT))
      class_parser_parser_enum_dot(classParser,t);
}

//在 declspec 解析是否有泛型声明
void aet_parser_c_parser_declspecs_generic (AetParser *self,struct c_declspecs *specs)
{
   ...
   if(c_parser_next_token_is (parser, CPP_LESS)){
      ...
      bool typeName=c_token_starts_typename (secondToken);
      if(!typeName){
         if(secondToken->type==CPP_NAME || secondToken->type==CPP_QUERY){
            if(secondToken->type==CPP_NAME){
               if(generic_util_valid_all(secondToken->value)){
                  n_debug("发现泛型语句 c_parser_declspecs 00 是 ? A-Z specs:%p\n",specs);
                  generic_impl_push_generic_from_declspecs(generic_impl_get(),specs);
               }
            }else{
               n_debug("发现泛型语句 c_parser_declspecs 11 是 ? A-Z specs:%p\n",specs);
               generic_impl_push_generic_from_declspecs(generic_impl_get(),specs);
            }
         }
      }else{
         n_debug("发现泛型语句 c_parser_declspecs 22 是 类型 specs:%p\n",specs);
         generic_impl_push_generic_from_declspecs(generic_impl_get(),specs);
      }
   }
}
      ...
      bool typespec_ok,struct c_declspecs *specs,bool *attrs_ok,bool *seen_type)
{
   ...
   struct c_typespec t;
   switch (c_parser_peek_token (parser)->keyword){
      case RID_AET_PACKAGE://zclei 10.4.0
         c_parser_consume_token (parser); //consume package
         break;
      case RID_AET_CLASS:
      case RID_AET_INTERFACE:
         //n_debug("aet_parser_c_parser_declspecs_keyword AET CLASS说明符：  keyword:%d specs:%p id_kind!=C_ID_CLASSNAME:%d",
         //c_parser_peek_token (parser)->keyword,specs, c_parser_peek_token (parser)->id_kind!=C_ID_CLASSNAME);
         //触发加入<aet.h>,如果需要加入，返回FALSE，留到加完#include <aet.h>后再进入这里。
         if(class_parser_add_include(classParser))
            return FALSE;
         if(c_parser_peek_token (parser)->id_kind!=C_ID_CLASSNAME){
            class_parser_replace_class_to_typedef(classParser);
            *attrs_ok = true;
            declspecs_add_scspec (loc, specs,c_parser_peek_token (parser)->value);
            //aet_print_specs(specs);
            c_parser_consume_token (parser); //consume typedef
         }else{
            //c_parser_peek_token 在class_parser_parser_class_specifier中被consume
            t=class_parser_parser_class_specifier(classParser);
            declspecs_add_type (loc, specs, t);
         }
         break;
      case RID_AET_ABSTRACT:
         //触发加入<aet.h>,如果需要加入，返回FALSE，留到加完#include <aet.h>后再进入这里。
         if(class_parser_add_include(classParser))
            return FALSE;
         c_parser_consume_token (parser); //
         class_parser_abstract_keyword(classParser);
         break;
      case RID_AET_PUBLIC:
      case RID_AET_PROTECTED:
      case RID_AET_PRIVATE:
      case RID_AET_FINAL:
         //printf("进这里的RID_AET_FINAL\n");
         class_parser_decorate(classParser);
         break;
      case RID_AET_ENUM:
         if (!typespec_ok)
            return FALSE;
         *attrs_ok = true;
         *seen_type = true;
         t=class_parser_enum(classParser,loc);
         declspecs_add_type (loc, specs, t);
         specs->expr=get_identifier ("enum$");//作一个标记
         break;
      default:
         return FALSE;
   }
...
static void aetParserInit(AetParser *self)
{
   classParser = class_parser_get();
   classImpl = class_impl_get();
   self->isAet=FALSE;
   self->isGenericState = FALSE;
   self->isTestGenericBlockState=FALSE;
   self->includeCodes=n_string_new("");
}

/**
 * 0 是系统      <...>
 * 1 是当前项目  "..."
 * 生成泛型信息是需当前正大编译的头文件。只取depth=1
 */
static void cb_include (cpp_reader *pfile ATTRIBUTE_UNUSED, location_t line,
       const unsigned char *dir, const char *header, int angle_brackets,
       const cpp_token **comments)
{
   if(pfile->line_table->depth==1){
      //printf("---cb_include -- %d %s %s\n",pfile->line_table->depth,dir,header);
      if (angle_brackets)
         n_string_append_printf(aet_parser_get()->includeCodes,"0%s\n", header);
      else
         n_string_append_printf(aet_parser_get()->includeCodes,"1%s\n",header);
   }
}

/**
 * 在 c-ppoutput.cc中也有回调，会不会冲突?
 * if (flag_dump_includes)
 *     cb->include  = cb_include;
 */
static void setIncludeCallback()
{
   cpp_callbacks *cb = cpp_get_callbacks (parse_in);
   if (flag_dump_includes)
      error("flag_dump_includes 选项屏蔽了对引用头文件的回调。请从当前编译项目的makefile 中移走flag_dump_includes\n");
   cb->include  = cb_include;
}
...
{
   return disable_extension_diagnostics();
}

//原型 c_parser_expr_no_commas c-parser.cc
struct c_expr aet_parser_c_parser_expr_no_commas (AetParser *self, struct c_expr *after,tree omp_atomic_lhs)
{
   return c_parser_expr_no_commas(self->parser,after,omp_atomic_lhs);
}

//原型 c_parser_declaration_or_fndef c-parser.cc
void aet_parser_c_parser_declaration_or_fndef (AetParser *self, bool fndef_ok,
                   bool static_assert_ok, bool empty_ok,
                   bool nested, bool start_attr_ok,
                   bool simple_ok,
                   tree *objc_foreach_object_declaration,
...
{
   c_parser_skip_to_end_of_block_or_statement(self->parser);
}
...
{
   c_parser_static_assert_declaration_no_semi(self->parser);
}
...
{
   return c_parser_nth_token_starts_std_attributes(self->parser,n);
}
...
{
   return c_parser_gnu_attributes(self->parser);
}
...
{
   self->parser=parser;
   aet_print_set_parser(parser);
   mtcsParser = mtcs_parser_get();
   mtcsParser->aetParser = self;
}
           ...
           bool *seen_id)
{
   ...
   if (c_parser_next_token_is (parser, CPP_MULT)){
      struct c_declspecs *quals_attrs = build_null_declspecs ();
      n_debug("解析声明符：00 是*号 --- specs:%p ", quals_attrs);
      struct c_declarator *inner;
      c_parser_consume_token (parser);
      aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(self, quals_attrs, false, false, true,
            false, false, true, false, cla_prefer_id);
      inner = aet_parser_c_parser_declarator/*!c_parser_declarator*/(self, type_seen_p, kind, seen_id);
      n_debug("解析声明符：11 是*号 --- inner:%p ", inner);
      if (inner == NULL)
         return NULL;
      else{
         struct c_declarator *pointer= make_pointer_declarator (quals_attrs, inner);
         n_debug("解析声明符：22 返回指针类型的c_declarator  新的declarator:%p inner:%p specs:%p ",pointer,inner,quals_attrs);
         return pointer;
      }
   }
   /* Now we have a direct declarator, direct abstract declarator or
   nothing (which counts as a direct abstract declarator here).  */
   n_debug("解析声明符：33 进入  c_parser_direct_declarator---");
          ...
          enum c_lookahead_kind la)
{
   ...
   if (start_std_attr_ok   && c_parser_nth_token_starts_std_attributes (parser, 1)){
   ...
   || (c_dialect_objc () && c_parser_next_token_is (parser, CPP_LESS))){
      ...
      tree align;
      location_t loc = c_parser_peek_token (parser)->location;
      //n_debug("声明说明符语法 00 count:%d  %d",count,(!typespec_ok || specs->typespec_kind == ctsk_tagdef));
      //aet_print_token(c_parser_peek_token (parser));
      if(mtcs_parser_is_attribute(mtcs_parser_get(),c_parser_peek_token (parser))){
         if(mtcs_parser_add_attribute(mtcs_parser_get()))
            continue;
      }
      ...
      && !c_parser_next_token_is_keyword (parser, RID_ALIGNAS)){
         //n_debug("声明说明符语法 11 break  count:%d keyword:%d",count,c_parser_peek_token (parser)->keyword);
         break;
      }
      ...
      if (c_parser_next_token_is (parser, CPP_NAME)){
        ...
        // n_debug("声明说明符语法 22  count:%d seen_type:%d kind:%d",count,seen_type,kind);
         if (kind == C_ID_ADDRSPACE){
            addr_space_t as = name_token->keyword - RID_FIRST_ADDR_SPACE;
            declspecs_add_addrspace (name_token->location, specs, as);
            c_parser_consume_token (parser);
            attrs_ok = true;
            //n_debug("声明说明符语法 33 continue  count:%d keyword:%d ",count,c_parser_peek_token (parser)->keyword);
            continue;
         }
         ...
         if (seen_type || !c_parser_next_tokens_start_typename (parser, la)){
            //n_debug("声明说明符语法 44 fun 不是一个类型名了,退出循环 count:%d keyword:%d seen_type:%d ",
            //count,c_parser_peek_token (parser)->keyword,seen_type);
            break;
         }
         tree ident=c_parser_peek_token (parser)->value;//zclei
         ...
         attrs_ok = true;
         if (kind == C_ID_ID){
            int ret = aet_parser_c_parser_declspecs_c_id(aetParser,&t,value);//zclei
            if(ret ==0){ //zclei
               auto_diagnostic_group d;
               name_hint hint = lookup_name_fuzzy (value, FUZZY_LOOKUP_TYPENAME,loc);
               if (const char *suggestion = hint.suggestion ()){
                  gcc_rich_location richloc (loc);
                  richloc.add_fixit_replace (suggestion);
                  error_at (&richloc,"unknown type name %qE; did you mean %qs?", value, suggestion);
               }else
            ...
            n_debug("类型 是 t.spec:%p %s value :%s  %s",
                  t.spec,get_tree_code_name(TREE_CODE(t.spec)),IDENTIFIER_POINTER(value),get_tree_code_name(TREE_CODE(TREE_TYPE(t.spec) )));
         }else{
            tree proto = NULL_TREE;
            gcc_assert (c_dialect_objc ());
            t.kind = ctsk_objc;
         ...
         //zclei 解析 AObject.EnumName
         aet_parser_c_parser_declspecs_enum_dot(aetParser,&t);//zclei
         declspecs_add_type (name_token->location, specs, t);
         //n_debug("声明说明符语法 55 continue count:%d keyword:%d",count,c_parser_peek_token (parser)->keyword);
         aet_parser_c_parser_declspecs_generic(aetParser,specs);//zclei
         continue;
      }
      if (c_parser_next_token_is (parser, CPP_LESS)){
         ...
         if (!typespec_ok || seen_type){
           // n_debug("声明说明符语法 66 break count:%d keyword:%d ",count,c_parser_peek_token (parser)->keyword);
            break;
         }
         proto = c_parser_objc_protocol_refs (parser);
         t.kind = ctsk_objc;
         ...
         t.has_enum_type_specifier = false;
         declspecs_add_type (loc, specs, t);
         //n_debug("声明说明符语法 77 continue count:%d keyword:%d",count,c_parser_peek_token (parser)->keyword);
         continue;
      }
      gcc_assert (c_parser_next_token_is (parser, CPP_KEYWORD));
      //n_debug("声明说明符语法：88 count:%d keyword:%d",count,c_parser_peek_token (parser)->keyword);
      switch (c_parser_peek_token (parser)->keyword){
         case RID_STATIC:
         case RID_EXTERN:
         case RID_REGISTER:
...
//            n_debug("声明说明符语法 99 存储类声明说明符：count:%d keyword:%d specs:%p %s name:%s code:%d ",
//                  count,c_parser_peek_token (parser)->keyword,specs,
//                  get_tree_code_name(TREE_CODE(c_parser_peek_token (parser)->value)),
//                  IDENTIFIER_POINTER (c_parser_peek_token (parser)->value),C_RID_CODE(c_parser_peek_token (parser)->value));
            declspecs_add_scspec (loc, specs, c_parser_peek_token (parser)->value);
            c_parser_consume_token (parser);
            break;
         case RID_AUTO_TYPE:
         ...
         case RID_UNION:
            ...
            attrs_ok = true;
            seen_type = true;
            //n_debug("声明说明符语法 103 结构体与联合声明说明符语法开始： count:%d specs:%p ",count,specs);
            //aet_print_token(c_parser_peek_token (parser));
            //t中的spec就是struct 关键字的tree
            t = c_parser_struct_or_union_specifier (parser);
            invoke_plugin_callbacks (PLUGIN_FINISH_TYPE, t.spec);
            declspecs_add_type (loc, specs, t);
           // n_debug("声明说明符语法 103 结构体与联合声明说明符语法结束了： count:%d specs:%p t:%p", count,specs,t.spec);
            break;
         case RID_TYPEOF:
         case RID_TYPEOF_UNQUAL:
         ...
         case RID_BITINT:
            if (!typespec_ok)
               ...
               if (parens.require_open (parser)){
                  c_expr expr = c_parser_expr_no_commas (parser, NULL);
                  t.expr = convert_lvalue_to_rvalue (loc, expr, true,true).value;
                  parens.skip_until_found_close (parser);
               }
               declspecs_add_type (loc, specs, t);
            }
         ...
         case RID_ATOMIC:
            ...
            if (flag_isoc99)
               pedwarn_c99 (loc, OPT_Wpedantic,"ISO C99 does not support the %<_Atomic%> qualifier");
            else
               pedwarn_c99 (loc, OPT_Wpedantic,"ISO C90 does not support the %<_Atomic%> qualifier");
            attrs_ok = true;
            tree value;
            value = c_parser_peek_token (parser)->value;
            c_parser_consume_token (parser);
            if (typespec_ok && c_parser_next_token_is (parser, CPP_OPEN_PAREN)){
               ...
               t.has_enum_type_specifier = false;
               if (type != NULL)
                  t.spec = groktypename (type, &t.expr, &t.expr_const_operands);
               c_parser_skip_until_found (parser, CPP_CLOSE_PAREN, "expected %<)%>");
         ...
         case RID_RTL:
            ...
            break;
         default:
            if(aet_parser_c_parser_declspecs_keyword(aetParser,loc,typespec_ok,specs,&attrs_ok,&seen_type)) //zclei
               break;
            goto out;
      }
   } //end while
out:
   if (end_std_attr_ok
   && c_parser_nth_token_starts_std_attributes (parser, 1)){
     // n_debug("声明说明符语法 104 count:%d keyword:%d ",count,c_parser_peek_token (parser)->keyword);
      specs->postfix_attrs = c_parser_std_attribute_specifier_sequence (parser);
   }
   //n_debug("声明说明符语法 105 count:%d keyword:%d ",count,c_parser_peek_token (parser)->keyword);
}
...
*/
//原型 c_parser_type_name c-parser.h c-parser.cc
static struct c_type_name * parserTypeName (AetParser *self, bool alignas_ok)
{
   c_parser *parser = self->parser;
   struct c_declspecs *specs = build_null_declspecs ();
   struct c_declarator *declarator;
   ...
   aet_parser_c_parser_declspecs/*!c_parser_declspecs*/(self,
         specs, false, true, true, alignas_ok, false,false, true, cla_prefer_type);
   if (!specs->declspecs_seen_p){
      c_parser_error (parser, "expected specifier-qualifier-list");
      return NULL;
   }
   ...
   declarator = aet_parser_c_parser_declarator/*!c_parser_declarator*/(self,
         specs->typespec_kind != ctsk_none,C_DTR_ABSTRACT, &dummy);
   if (declarator == NULL)
      return NULL;
...
vec<tree, va_gc> *aet_parser_c_parser_expr_list (AetParser *self, bool convert_p, bool fold_p,
          vec<tree, va_gc> **p_orig_types,
          location_t *sizeof_arg_loc, tree *sizeof_arg,
          vec<location_t> *locations,
          unsigned int *literal_zero_mask)
{
    return c_parser_expr_list(self->parser,convert_p,fold_p,p_orig_types,sizeof_arg_loc,sizeof_arg,locations,literal_zero_mask);
}
...
{
   static AetParser *singleton = NULL;
   if (!singleton){
      singleton =n_slice_alloc0 (sizeof(AetParser));
      aetParserInit(singleton);
      aetParser = singleton;
   }
   return singleton;
}
