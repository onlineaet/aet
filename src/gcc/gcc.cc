...
//zclei 找出-mtcs参数，进入gcc报错，因为gcc不能解析-mtcs 取出后单独在GCC_AET_MTCS_ARGV环境变量中。 argv中会有重复的 -mtcs
//-mtcs=sm_75,ptx
//-noinclude 不要自动包含头文件aet.h，该选项是用来调试的。
static int setAetArgv (int argc, char **argv)
{
   int i;
   static const char *SEPARATION="#$%";
   char buffer[64*1024];
   buffer[0]='\0';
   char mtcsbuffer[1024];
   mtcsbuffer[0]='\0';
   //来自aetcollect.c
   static char *aet_param_1 ="-mtcs";
...
#define FOR2 \
   ...
   for(i=0;i<argc;i++){
      char *arg=argv[i];
      //fprintf(stderr,"run param ---- %d %s\n",i,arg);
      if(startswith(arg,aet_param_1)){
         FOR1;
         strcat(mtcsbuffer,arg);
         strcat(mtcsbuffer,SEPARATION);
         FOR2;
      }else if(startswith(arg,aet_param_2)){
         FOR1;
         xputenv (concat ("GCC_AET_NO_INCLUDE=", "yes", NULL));
         FOR2;
      }else if(startswith(arg,aet_param_3)){
         //汇编ptx代码保存在.o的同目录下
         FOR1;
         xputenv (concat ("GCC_AET_DUMP=", "yes", NULL));
         FOR2;
      }else if(startswith(arg,aet_param_4)){
         FOR1;
         char *path=arg+strlen(aet_param_4);
         xputenv (concat ("GCC_AET_LIB_PATH=", path, NULL));
      ...
      strcat(buffer,arg);
      strcat(buffer,SEPARATION);
      // fprintf(stderr,"gcc.cc makefile传给gcc的参数 %d %s pid:%d\n",i,arg,getpid());
   }
   xputenv (concat ("GCC_AET_ARGV=", buffer, NULL));
   xputenv (concat ("GCC_AET_MTCS_ARGV=", mtcsbuffer, NULL));
#undef FOR1
#undef FOR2
   return argc;
}
...
//zclei main方法中加入  setAetVar(argc,argv);
int
driver::main (int argc, char **argv)
{
  bool early_exit;
  argc = setAetArgv(argc,argv);//zclei
  set_progname (argv[0]);
