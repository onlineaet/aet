...
//采用回调方法后可以取消这种依赖。
static ConvertMangleCallBack convertMangleCallBack;//zclei
void pp_set_convert_managle_callback(ConvertMangleCallBack callback,void *userData)
{
    convertMangleCallBack=callback;
}
...
{
   if(convertMangleCallBack){//zclei
                 char newStart[2048];//zclei
                 int newLen=convertMangleCallBack(start,length,newStart);//zclei
                 if(newLen<=0)//zclei
                    output_buffer_append_r (pp_buffer (pp), start, length);//zclei
                 else{//zclei
                    output_buffer_append_r (pp_buffer (pp), newStart, newLen);//zclei
                 }//zclei
         }else{
