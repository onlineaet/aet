...
// m_temp_uses.
void function_info::record_use (build_info &bi, insn_info *insn,
            rtx_obj_reference ref)
{
  ...
  if (!use)
   ...
   {
     if (!ordered_p (mtcs_mode_get_precision/*!GET_MODE_PRECISION*/(mtcsMode,use->mode ()),
           mtcs_mode_get_precision/*!GET_MODE_PRECISION*/(mtcsMode,mode)))
       use->set_mode (reg_raw_mode[regno]);
     else if (mtcs_mode_partial_subreg_p/*!partial_subreg_p*/(mtcsMode,use->mode (), mode))
       use->set_mode (mode);
   }
                 ...
                 rtx_call_insn *rtl)
{
   ...
   mtcs_function_abi abi = mtcs_func_abi_insn_callee_abi/*!insn_callee_abi*/(mtcsFuncAbi,rtl);
   ...
   || abi.full_and_partial_reg_clobbers () != abi.full_reg_clobbers ()){
      ...
      for (unsigned int regno = 0; regno < mtcs_reg_get_first_pseudo_register/*!FIRST_PSEUDO_REGISTER*/(mtcsReg); ++regno)
         if (mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&abi.full_reg_clobbers (), regno)){
...
int
mtcs_rtl_ssa::lookup_call_clobbers (insn_call_clobbers_tree &tree, insn_info *insn)
{
...
void
mtcs_rtl_ssa::pp_insn (pretty_printer *pp, const insn_info *insn)
{
  if (!insn)
    pp_string (pp, "<null>");
  else
