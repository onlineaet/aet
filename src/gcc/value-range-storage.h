...
public:
  ...
  void dump () const;
  static size_t size (const irange &r);//zclei 移为改为公共方法
  irange_storage (const irange &r);//zclei 移为改为公共方法
private:
...
public:
  ...
  void dump () const;
  prange_storage (const prange &r);//zclei 改为公共方法
private:
  DISABLE_COPY_AND_ASSIGN (prange_storage);
  //prange_storage (const prange &r);//zclei 改为公共方法了
