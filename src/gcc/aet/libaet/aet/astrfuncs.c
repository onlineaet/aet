...
static auint64 a_parse_long_long (const achar  *nptr,const achar **endptr,auint  base,aboolean     *negative)
{
   ...
   */
   ...
   a_return_val_if_fail (nptr != NULL, 0);
   ...
   *negative = FALSE;
   ...
   save = s;
   cutoff = A_MAXUINT64 / base;
   cutlim = A_MAXUINT64 % base;
...
auint64 a_ascii_strtoull (const achar *nptr,achar **endptr,auint base)
{
   aboolean negative;
   auint64 result;
   result = a_parse_long_long (nptr, (const achar **) endptr, base, &negative);
...
aint64 a_ascii_strtoll (const achar *nptr,achar **endptr,auint base)
{
   ...
   if (negative && result > (auint64) A_MININT64){
      errno = ERANGE;
      return A_MININT64;
   }else if (!negative && result > (auint64) A_MAXINT64){
      errno = ERANGE;
      return A_MAXINT64;
   }else if (negative)
      return - (aint64) result;
   else
      return (aint64) result;
}
