...
#include \"function.h\"");
  ...
  if(mtcs_gen_is_ptx(mtcs_gen_get())){
      puts ("#include \"../../mtcstarget.h\"");
      puts ("#include \"../mtcsptxrecog.h\"");
      puts ("#include \"../../mtcsoptions.h\"");
  }
...
/* This variable is used as the \"location\" of any missing operand\n\
  ...
  puts("static rtx junk ATTRIBUTE_UNUSED;\n");

  char *platName=mtcs_gen_get_platform_name(mtcs_gen_get());
  printf("/*\n*函数%s_insn_extract_cb 由平台mtcs%srecog.h声明\n*/\n",platName,platName);
  printf("void %s",platName);
  //函数ptx_insn_extract_cb 由平台mtcsptxrecog.h声明
  puts("_insn_extract_cb (MtcsRecog *self,rtx_insn *insn)\n{");
  puts("\
  ...
  int maxRecogOperands=mtcs_recog_get_max_recog_operands(self);");

  puts ("\
  rtx *ro = self->recog_data.operand;\n\
  rtx **ro_loc = self->recog_data.operand_loc;\n\
  rtx pat = PATTERN (insn);\n\
  int i ATTRIBUTE_UNUSED; /* only for peepholes */\n\
\n\
  if (mtcsOptionsItem->x_flag_checking)\n\
    {\n\
      memset (ro, 0xab, sizeof (*ro) * maxRecogOperands/*!MAX_RECOG_OPERANDS*/);\n\
      memset (ro_loc, 0xab, sizeof (*ro_loc) * maxRecogOperands/*!MAX_RECOG_OPERANDS*/);\n\
    }\n");
...
int main (int argc, const char **argv)
{
  ...
  for (p = extractions; p; p = p->next)
   ...
   {
     printf ("      self->recog_data.dup_loc[%d] = &", i);
     print_path (p->duplocs[i]);
     puts (";");
     printf ("      self->recog_data.dup_num[%d] = %d;\n", i, p->dupnums[i]);
   }
