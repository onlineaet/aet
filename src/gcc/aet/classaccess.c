...
static tree setNopExpr(tree nopexpr,int *nest)
{
   ...
   n_debug("classaccess.c setNopExpr 00 %s op:%s",get_tree_code_name(nopexprTypeCode),get_tree_code_name(TREE_CODE(op)));
   if(nopexprTypeCode==POINTER_TYPE && TREE_CODE(op)==CALL_EXPR){
...
static tree getLastVar(tree decl,int *nest)
{
	if(TREE_CODE(decl)==VAR_DECL){
		*nest=*nest+1;
		...
		n_debug("classaccess.c getLastVar 33 TREE_CODE(decl)==NOP_EXPR nest:%d",*nest);
		*nest=*nest+1;
		...
		n_debug("classaccess.c getLastVar 44 TREE_CODE(decl)==NON_LVALUE_EXPR nest:%d",*nest);
		*nest=*nest+1;
    	return setNopExpr(decl,nest);
	}else if(TREE_CODE(decl)==ADDR_EXPR){
		n_debug("classaccess.c getLastVar 55 TREE_CODE(decl)==ADDR_EXPR nest:%d",*nest);
		*nest=*nest+1;
    	return setAddrExpr(decl,nest);
	}else if(TREE_CODE(decl)==COMPONENT_REF){
		n_debug("classaccess.c getLastVar 66 TREE_CODE(decl)==COMPONENT_REF nest:%d",*nest);
		*nest=*nest+1;
		return setComponentRef(decl,nest);
	}else if(TREE_CODE(decl)==CALL_EXPR){
		n_debug("classaccess.c getLastVar 77 TREE_CODE(decl)==CALL_EXPR nest:%d",*nest);
		*nest=*nest+1;
...
char *class_access_get_class_name(ClassAccess *self,tree exprValue)
{
  ...
  if(TREE_CODE(exprValue)==VAR_DECL || TREE_CODE(exprValue)==PARM_DECL){
	  ...
	  n_debug("classaccess.c exprValue 是变量或参数 %s className:%s",get_tree_code_name(TREE_CODE(exprValue)),className);
  }else if(TREE_CODE(exprValue)==NOP_EXPR){
	  className=class_util_get_class_name(TREE_TYPE(exprValue));
	  tree op=TREE_OPERAND (exprValue, 0);
	  n_debug("classaccess.c exprValue 是NOP_EXPR %s className:%s",get_tree_code_name(TREE_CODE(op)),className);
  }else if(TREE_CODE(exprValue)==NON_LVALUE_EXPR){
	  className=class_util_get_class_name(TREE_TYPE(exprValue));
	  tree op=TREE_OPERAND (exprValue, 0);
	  n_debug("classaccess.c exprValue 是NON_LVALUE_EXPR %s className:%s",get_tree_code_name(TREE_CODE(op)),className);
  }else if(TREE_CODE(exprValue)==ADDR_EXPR){
	  tree op=TREE_OPERAND (exprValue, 0);
	  className=class_util_get_class_name(TREE_TYPE(op));
	  n_debug("classaccess.c exprValue 是ADDR_EXPR %s className:%s",get_tree_code_name(TREE_CODE(exprValue)),className);
  }else if(TREE_CODE(exprValue)==COMPONENT_REF){
	  //表达式可能是self->obj或self->obj->obj1;要找的是最后一个obj
	  tree op=TREE_OPERAND (exprValue, 1);
	  className=class_util_get_class_name(TREE_TYPE(op));
	  n_debug("classaccess.c exprValue 是引用 %s className:%s",get_tree_code_name(TREE_CODE(op)),className);
  }else if(TREE_CODE(exprValue)==CALL_EXPR){
	  n_debug("classaccess.c exprValue 是CALL_EXPR %s className:%s",get_tree_code_name(TREE_CODE(exprValue)),className);
	  return class_util_get_class_name(TREE_TYPE(exprValue));
  }else if(TREE_CODE(exprValue)==INDIRECT_REF){
      tree op=TREE_OPERAND (exprValue, 0);
      className=class_util_get_class_name(TREE_TYPE(op));
      n_debug("classaccess.c exprValue 是INDIRECT_REF %s className:%s",get_tree_code_name(TREE_CODE(exprValue)),className);
...
int class_access_get_last_class_and_var(ClassAccess *self,location_t loc,tree component,tree exprValue,char **result)
{
   ...
   if(lows[0]==NULL){
      //bug 077
      if(class_util_have_error())
         n_warning("有语法错识,引用的函数名是:%s，变量名:%s 引用的类型未知",IDENTIFIER_POINTER(component),lows[1]);
      else{
