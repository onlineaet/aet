...
#define IFACE_IMPL_O_FILE     "<ofile>" //目标文件
#define IFACE_IMPL_O_FILE_END "</ofile>"

static void appendIfaceClass(char *sysName,NString *codes)
{
       n_string_append_printf(codes,"%s %s=%s>\n",  CLASS_CONTENT_HALF,XML_SYS_NAME,sysName);
}

static void appendbeQuoted(char *beQuoted,NString *codes)
{
       n_string_append_printf(codes,"%s%s%s\n",IFACE_BE_QUOTED,beQuoted,IFACE_BE_QUOTED_END);
}

static void appendCFile(char *cfile,NString *codes)
{
       n_string_append_printf(codes,"%s%s%s\n",IFACE_IMPL_C_FILE,cfile,IFACE_IMPL_C_FILE_END);
}

static void appendOFile(char *ofile,NString *codes)
{
...
/**
 *从库中取IFACE的数据，这些数据是IfaceSaveData
 */
...
static char *createIfsaveIface(XmlFile *self,NPtrArray *array)
{
     NString *codes=n_string_new("");
     int i;
     for(i=0;i<array->len;i++){
         IFaceSaveData *item=(IFaceSaveData *)n_ptr_array_index(array,i);
         appendIfaceClass(item->sysName,codes);
         if(item->cFile && item->oFile){
             appendCFile(item->cFile,codes);
             appendOFile(item->oFile,codes);
         }
         n_string_append_printf(codes,"%s\n",CLASS_CONTENT_END);
     }
     char sec[255];
