...
using add_regno_clobber_fn = std::function<bool (insn_change &, unsigned int)>;
       ...
       IgnorePredicates ignore)
{
   ...
   {
      return mtcsRtlData/*!crtl*/->ssa->add_regno_clobber (watermark, change, regno, ignore);
   };
...
// As above, but ignore only the instruction that is being changed.
inline bool recog (obstack_watermark &watermark, insn_change &change)
{
