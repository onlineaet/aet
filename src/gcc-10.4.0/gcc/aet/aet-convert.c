/*
   ...
   This program based on gcc/c/c-convert.c
  ...
  switch (TREE_CODE (TREE_TYPE (expr)))
    ...
    case REFERENCE_TYPE:
      {
	   ...
	   to be done via a ADDR_SPACE_CONVERT_EXPR instead of a NOP_EXPR.  */
		...
		if (to_as == from_as)
		  ...
		  return maybe_fold_build1_loc (fold_p, loc, ADDR_SPACE_CONVERT_EXPR,type, expr);
      }
	...
	/* If the input precision differs from the target pointer type
	   ...
	   sizes to coexist so the latter isn't necessarily POINTER_SIZE.  */
 	   ...
 	   printf("aet-convert_to_pointer_1 POINTER_TYPE eprec != pprec:%d \n",eprec != pprec);

		if (eprec != pprec)
		  expr = maybe_fold_build1_loc (fold_p, loc, NOP_EXPR,lang_hooks.types.type_for_size (pprec, 0),expr);
      }
      return maybe_fold_build1_loc (fold_p, loc, CONVERT_EXPR, type, expr);
    default:
     //zclei error ("cannot convert to a pointer type");
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=cannot_convert_to_a_pointer_type;
      return expr;//convert_to_pointer_1 (type, integer_zero_node, fold_p);
    }
}
...
tree aet_convert_to_pointer (tree type, tree expr)
{
...
tree aet_convert_to_pointer_maybe_fold (tree type, tree expr, bool dofold)
{
  tree result
    = convert_to_pointer_1 (type, expr,
			    dofold || CONSTANT_CLASS_OR_WRAPPER_P (expr));
  return aet_preserve_any_location_wrapper (result, expr);
}
...
#undef CASE_MATHFN
  ...
  switch (TREE_CODE (TREE_TYPE (expr)))
    ...
    case POINTER_TYPE:
    case REFERENCE_TYPE:
      //zclei error ("pointer value used where a floating-point was expected");
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=pointer_value_used_where_a_floating_point_was_expected;
      return convert_to_real_1 (type, integer_zero_node, fold_p);

    default:
      //zclei error ("aggregate value used where a floating-point was expected");
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=aggregate_value_used_where_a_floating_point_was_expected;
      return convert_to_real_1 (type, integer_zero_node, fold_p);
    }
}
...
tree aet_convert_to_real (tree type, tree expr)
{
...
tree aet_convert_to_real_maybe_fold (tree type, tree expr, bool dofold)
{
  tree result
...
static tree do_narrow (location_t loc,
      ...
      else
	{
	 ...
	 type promotion/demotion pass, see PR45397.  */
      expr = maybe_fold_build2_loc (dofold, loc, ex_form, typex,
				    aet_convert (typex, arg0),
				    aet_convert (typex, arg1));
...
static tree convert_to_integer_1 (tree type, tree expr, bool dofold)
{
  ...
  if (!COMPLETE_TYPE_P (type))
    {
      //zclei error ("conversion to incomplete type");
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=conversion_to_incomplete_type;
  ...
  switch (TREE_CODE (intype))
	...
	switch (ex_form)
	  ...
	  CASE_CONVERT:
	    ...
	    return aet_convert (type, get_unwidened (TREE_OPERAND (expr, 0), type));
	  ...
	  case COND_EXPR:
	    ...
	    return
	      fold_build3 (COND_EXPR, type, TREE_OPERAND (expr, 0),
			   ...
			   ? TREE_OPERAND (expr, 1)
			   : aet_convert (type, TREE_OPERAND (expr, 1)),
			   VOID_TYPE_P (TREE_TYPE (TREE_OPERAND (expr, 2)))
			   ? TREE_OPERAND (expr, 2)
			   : aet_convert (type, TREE_OPERAND (expr, 2)));
	...
	{
	  //zcleierror ("cannot convert a vector of type %qT to type %qT which has different size", TREE_TYPE (expr), type);
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=cannot_convert_a_vector_of_type_xx_to_type_xx_which_has_different_size;
	  return error_mark_node;
	}
      return build1 (VIEW_CONVERT_EXPR, type, expr);
    default:
      //zclei error ("aggregate value used where an integer was expected");
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=aggregate_value_used_where_an_integer_was_expected;
      return aet_convert (type, integer_zero_node);
    }
}
...
tree aet_convert_to_integer (tree type, tree expr)
{
...
tree aet_convert_to_integer_maybe_fold (tree type, tree expr, bool dofold)
{
  tree result
...
static tree convert_to_complex_1 (tree type, tree expr, bool fold_p)
{
  ...
  switch (TREE_CODE (TREE_TYPE (expr)))
    ...
    case ENUMERAL_TYPE:
    case BOOLEAN_TYPE:
      return build2 (COMPLEX_EXPR, type, aet_convert (subtype, expr),
		     aet_convert (subtype, integer_zero_node));
	...
	if (TYPE_MAIN_VARIANT (elt_type) == TYPE_MAIN_VARIANT (subtype))
	...
	else
	  {
	    ...
	    tree imagp = maybe_fold_build1_loc (fold_p, loc, IMAGPART_EXPR,
						...
						expr);
	    return maybe_fold_build2_loc (fold_p, loc, COMPLEX_EXPR, type,
	    		aet_convert (subtype, realp),
				aet_convert (subtype, imagp));
	  }
      }

    case POINTER_TYPE:
    case REFERENCE_TYPE:
      //zclei error ("pointer value used where a complex was expected");
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=pointer_value_used_where_a_complex_was_expected;
      return convert_to_complex_1 (type, integer_zero_node, fold_p);

    default:
      //zclei error ("aggregate value used where a complex was expected");
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=aggregate_value_used_where_a_complex_was_expected;
      return convert_to_complex_1 (type, integer_zero_node, fold_p);
    }
}
...
tree aet_convert_to_complex (tree type, tree expr)
{
...
tree aet_convert_to_complex_maybe_fold (tree type, tree expr, bool dofold)
{
  tree result
...
tree aet_convert_to_vector (tree type, tree expr)
{
  switch (TREE_CODE (TREE_TYPE (expr)))
    {
    case INTEGER_TYPE:
    case VECTOR_TYPE:
      if (!tree_int_cst_equal (TYPE_SIZE (type), TYPE_SIZE (TREE_TYPE (expr))))
	{
	  //zclei error ("cannot convert a value of type %qT to vector type %qT which has different size",TREE_TYPE (expr), type);
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=cannot_convert_a_value_of_type_xx_to_vector_type_xx_which_has_different_size;
	  return error_mark_node;
	}
      return build1 (VIEW_CONVERT_EXPR, type, expr);

    default:
      //zclei error ("cannot convert value to a vector");
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=cannot_convert_value_to_a_vector;
      return error_mark_node;
    }
}
...
tree aet_convert_to_fixed (tree type, tree expr)
{
  if (integer_zerop (expr))
    {
  ...
  switch (TREE_CODE (TREE_TYPE (expr)))
    ...
    default:
      //zclei error ("aggregate value used where a fixed-point was expected");
      argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=aggregate_value_used_where_a_fixed_point_was_expected;
      return error_mark_node;
    }
}
...
tree aet_convert (tree type, tree expr)
{
    ...
    if ((invalid_conv_diag= targetm.invalid_conversion (TREE_TYPE (expr), type))){
        //zclei error (invalid_conv_diag);
        argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=invalid_conv_diag;
        return error_mark_node;
    }
    ...
    if (TYPE_MAIN_VARIANT (type) == TYPE_MAIN_VARIANT (TREE_TYPE (expr)) && (TREE_CODE (TREE_TYPE (expr)) != COMPLEX_TYPE  || TREE_CODE (e) == COMPLEX_EXPR))
       return fold_convert_loc (loc, type, expr);
    if (TREE_CODE (TREE_TYPE (expr)) == ERROR_MARK)
      return error_mark_node;
    if (TREE_CODE (TREE_TYPE (expr)) == VOID_TYPE){
        //zclei error ("void value not ignored as it ought to be");
    ...
    switch (code){
       ...
       case INTEGER_TYPE:
       case ENUMERAL_TYPE:
          if (sanitize_flags_p(SANITIZE_FLOAT_CAST) && current_function_decl != NULL_TREE && TREE_CODE (TREE_TYPE (expr)) == REAL_TYPE && COMPLETE_TYPE_P (type)){
	         expr = save_expr (expr);
	         expr = aet_c_fully_fold (expr, false, NULL);
	         tree check = ubsan_instrument_float_cast (loc, type, expr);
          ...
          ret = aet_convert_to_integer (type, e);
          goto maybe_fold;
       case BOOLEAN_TYPE:
          return fold_convert_loc(loc, type, aet_c_objc_common_truthvalue_conversion (input_location, expr));

       case POINTER_TYPE:
       case REFERENCE_TYPE:
    	   //printf("aet-convert POINTER_TYPE \n");
          ret = aet_convert_to_pointer (type, e);
          goto maybe_fold;
       case REAL_TYPE:
          ret = aet_convert_to_real (type, e);
          goto maybe_fold;
       case FIXED_POINT_TYPE:
          ret = aet_convert_to_fixed (type, e);
          goto maybe_fold;
       case COMPLEX_TYPE:
          ret = aet_convert_to_complex (type, e);
          goto maybe_fold;
       case VECTOR_TYPE:
          if (gnu_vector_type_p (type) || gnu_vector_type_p (TREE_TYPE (e))
	  /* Allow conversions between compatible non-GNU vector types
	     ...
	     nonconforming code.  */
	        || (flag_lax_vector_conversions && VECTOR_TYPE_P (TREE_TYPE (e)) && vector_types_convertible_p (type, TREE_TYPE (e), false))){
	         ret = aet_convert_to_vector (type, e);
	         goto maybe_fold;
	      }
          break;
       case RECORD_TYPE:
    ...
    //zclei error ("conversion to non-scalar type requested");
    argsFuncsInfo.errors[argsFuncsInfo.errorCount++]=conversion_to_non_scalar_type_requested;
    return error_mark_node;
}
