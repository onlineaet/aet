...
static int  clear_warning_modify(tree lhtype,tree rhs)
{
...
static int checktype(location_t loc,tree formulaType,tree actualType,int parmnum,tree rname,int *haveError)
{
   ...
   if(formulaSysName==NULL  && actualSysName!=NULL){
      ...
      if(type==void_type_node){
         //说明型参是 void *
         n_debug("clearwqrning.c formulaSysName -vvv- %s %s\n",formulaSysName,actualSysName);
      ...
      if(type==void_type_node){
         //说明实参是 void *
         n_debug("clearwqrning.c formulaSysName -eee- %s %s\n",formulaSysName,actualSysName);
         return COMPARE_ARGUMENT_ORIG_PARM_OK;
      }else{
         printf("clearwarning.c checktype --- %p\n",rname);
         if(rname)
            error_at(loc, "passing argument %d of %qE from incompatible pointer type", parmnum, rname);//zclei
         else
...
{
   int  ret =  clear_warning_modify(lhtype,rhs);
   n_debug("clear_warning_modify_new 00 %d\n",ret);
   ...
   switch(errtype){
      case ic_argpass:
         ok=checktype(loc,lhtype,rhstype,parmnum,rname,&haveError);
         if(ok==ARGUMENT_SKIP){
            pedwarn (loc, OPT_Wpointer_sign,"passing argument %d of %qE from incompatible pointer type", parmnum, rname);
            ret=1;
         }else if(ok==ARGUMENT_OK){
            ret=1;
         }else{
            if(haveError)
               ret=1;
      ...
      case ic_assign:
         ok=checktype(loc,lhtype,rhstype,parmnum,rname,&haveError);
         if(ok==ARGUMENT_SKIP){
            pedwarn (loc, OPT_Wpointer_sign,"assignment to %e from incompatible pointer type %e",&e_type, &e_rhstype);
            ret=1;
         }else if(ok==ARGUMENT_OK){
            ret=1;
         }else{
