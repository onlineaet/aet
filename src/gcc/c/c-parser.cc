...
struct GTY(()) c_parser {
  ...
  c_token tokens_buf[30];//zclei 原来是4改为30 c_parser_consume_token也要改
	...
	else if (c_dialect_objc ())
	  {
	   tree objc_interface_decl = objc_is_class_name (token->value);
	         ...
	         if (objc_interface_decl  && (!objc_force_identifier || global_bindings_p ())){
	              ...
	              if(aet_parser_set_class_or_enum_type(aet_parser_get(),token))//zclei
	                     break;//zclei
	       }//zclei
      ...
      if (parser->tokens_avail >= 3)
        {
          ...
          AET_PARSER_CONSUME_TOKEN //zclei
        }
    }
  parser->tokens_avail--;
...
c_parse_file (void)
{
  ...
  *the_parser = tparser;
  ...
  aet_parser_set_parser(aet_parser_get(),the_parser);//zclei
  aet_parser_c_parser_translation_unit/*!c_parser_translation_unit (the_parser)*/(aet_parser_get());//zclei
  the_parser = NULL;
}
