 ...
 */
char *class_build_create_codes(ClassBuild *self,ClassName *className)
{
   ...
   nboolean isMtcsClass =class_mgr_is_mtcs_class(class_mgr_get(),className);//是不是mtcs类
   ...
   char class_className[512];
   sprintf(class_className,"_class_%s",className->userName);
   n_string_append_printf(codes,"\t\t temp=%s.newObject(sizeof(%s),0,0,\"%s\");\n",cName->sysName,cName->sysName,class_className);
   ...
   n_string_append_printf(codes,"\tobj->isMtcs=%d;\n",isMtcsClass);//设是不是mtcs类
   n_string_append_printf(codes,"\tobj->size=sizeof(%s);\n",className->sysName);//设是不是mtcs类
