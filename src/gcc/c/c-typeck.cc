...
#include "aet/selectfield.h" //zclei
#include "aet/aetprinttree.h" //zclei
      ...
      if (type != NULL_TREE)
	{
     ...
     /*!
       parmval = convert_argument (ploc, function, fundecl, type, origtype,
                    ...
                    excess_precision, 0);
                    */
     parmval=  select_field_call_back (ploc, function, fundecl, type, origtype,
                               val, valtype, npc, rname, parmnum, argnum,
                               excess_precision);//zclei
	}
  ...
  /* Conversions among pointers */
      ...
      else if (!objc_ok)
	{
	  ...
	  switch (errtype)
	    ...
	    case ic_argpass:
	      {
	         if(clear_warning_modify_new(errtype,location,type ,rhs,rhstype,parmnum,rname,TRUE))//zclei
	                   break;
		range_label_for_type_mismatch rhs_label (rhstype, type);
		gcc_rich_location richloc (expr_loc, &rhs_label,
					   highlight_colors::actual);
	    ...
	    case ic_init:
	    case ic_init_const:
	       if(clear_warning_modify_new(errtype,location,type ,rhs,rhstype,parmnum,rname,FALSE))//zclei
	                        break;
	      if (bltin)
		warned
...
//zclei
tree aet_digest_init (location_t init_loc, tree decl,tree type, tree init, tree origtype,
             bool null_pointer_constant, bool int_const_expr,
         bool arith_const_expr, bool strict_string,
         bool require_constant, bool require_constexpr)
{
    return digest_init(init_loc,decl,type,init,origtype,null_pointer_constant,
            int_const_expr,arith_const_expr,strict_string,require_constant,require_constexpr);
}

//zclei
tree aet_convert_for_assignment (location_t location, location_t expr_loc, tree type,
            tree rhs, tree origtype,  int errtype,
            bool null_pointer_constant, tree fundecl,
            tree function, int parmnum, int warnopt /* = 0 */)
{
    enum impl_conv et=ic_init_const;
     if(errtype==ic_argpass)
         et=ic_argpass;
     else if(errtype==ic_assign)
         et=ic_assign;
     else if(errtype==ic_init)
            et=ic_init;
     else if(errtype==ic_init_const)
            et=ic_init_const;
     else if(errtype==ic_return)
             et=ic_return;

    return convert_for_assignment (location,  expr_loc,  type,
                 rhs,  origtype,  et,
                 null_pointer_constant,  fundecl,
                 function,  parmnum,  warnopt /* = 0 */);
}

//zclei
//转化单个参数
tree aet_convert_argument (location_t ploc, tree function, tree fundecl,
          tree type, tree origtype, tree val, tree valtype,
          bool npc, tree rname, int parmnum, int argnum,
          bool excess_precision, int warnopt)
{
    return convert_argument ( ploc,  function,  fundecl,
               type,  origtype,  val,  valtype,
               npc,  rname,  parmnum,  argnum,
