...
function_info::add_entry_block_defs (build_info &bi)
{
  ...
  EXECUTE_IF_SET_IN_BITMAP (&lr_info->out, 0, regno, in_bi)
    {
     n_debug("mtcsblocks.c function_info::add_entry_block_defs 00\n");
      auto *set = allocate<set_info> (insn, full_register (regno));
      n_debug("mtcsblocks.c function_info::add_entry_block_defs 11 %p\n",set);
...
function_info::add_artificial_accesses (build_info &bi, df_ref_flags flags)
{
  ...
  HardRegSet /*!HARD_REG_SET*/ added_regs = {mtcs_reg_get_hard_reg_element_count(mtcsReg)};
  ...
  FOR_EACH_ARTIFICIAL_USE (ref, cfg_bb->index)
   ...
   if (mtcs_reg_is_hard/*!HARD_REGISTER_NUM_P*/(mtcsReg,regno))
      mtcs_reg_set_hard_reg_bit/*!SET_HARD_REG_BIT*/(mtcsReg,&added_regs, regno);
  ...
  if (flags == 0 && EDGE_COUNT (cfg_bb->succs) == 0)
   ...
   if (global_regs[i] && !mtcs_reg_test_hard_reg_bit/*!TEST_HARD_REG_BIT*/(&added_regs, i))
     {
...
function_info::process_all_blocks ()
{
  ...
  for (edge e : EXIT_BLOCK_PTR_FOR_FN (m_fn)->preds)
    if (bi.exit_block_dominator)
      bi.exit_block_dominator = nearest_common_dominator (CDI_DOMINATORS, bi.exit_block_dominator, e->src);
    else
...
void
mtcs_rtl_ssa::pp_ebb_call_clobbers (pretty_printer *pp,
...
void
mtcs_rtl_ssa::pp_bb (pretty_printer *pp, const bb_info *bb)
{
  if (!bb)
    pp_string (pp, "<null>");
  else
...
void
mtcs_rtl_ssa::pp_ebb (pretty_printer *pp, const ebb_info *ebb)
{
  if (!ebb)
    pp_string (pp, "<null>");
  else
...
//void
//mtcs_rtl_ssa_dump (FILE *file, const ebb_call_clobbers_info *call_clobbers)
//{
...
//void
//mtcs_rtl_ssa_dump (FILE *file, const bb_info *bb)
//{
...
//void
//mtcs_rtl_ssa_dump (FILE *file, const ebb_info *ebb)
//{
//  dump_using (file, pp_ebb, ebb);
//}
//
//// Debug interfaces to the dump routines above.
//void mtcs_rtl_ssa_debug (const ebb_call_clobbers_info *x) { mtcs_rtl_ssa_dump (stderr, x); }
//void mtcs_rtl_ssa_debug (const bb_info *x) { mtcs_rtl_ssa_dump (stderr, x); }
//void mtcs_rtl_ssa_debug (const ebb_info *x) { mtcs_rtl_ssa_dump (stderr, x); }
