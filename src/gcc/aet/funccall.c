...
static int findStaticFuncAtClass(FuncCall *self,ClassName *className,char *orgiName)
{
   ...
   n_debug("findStaticFuncAtClass findFuncAtClass 00 %s %s exists:%d",className->sysName,orgiName,exists);
   //取一个来匹配
   if(!exists){
...
static CandidateFunc *selectFunc(FuncCall *self,ClassName *className,char *orgiName,vec<tree, va_gc> *exprlist,
        ...
        GenericModel *generics,GenericModel * funcGenericDefine,FuncPointerError **errors,SelectFunc *selectFunc)
{
   ...
   if(item!=NULL){
      if(class_func_is_func_generic(item->classFunc) || class_func_have_query_param(item->classFunc)){
         ...
         nboolean ok=generic_call_check/*!generic_call_add_fpgi_parm*/(generic_call_get(),
               item->classFunc,selectClassName,exprlist,funcGenericDefine,expr_loc);
      }
   }
   return item;
}
 ...
 * 指针调用形式
 */
tree func_call_deref_select(FuncCall *self,tree func,vec<tree, va_gc> *exprlist,vec<tree, va_gc> *origtypes,
      vec<location_t> arg_loc,location_t expr_loc,FuncPointerError **errors,SelectFunc *selRet)
{
   AET_LANG_FLAG_6(func)=0;
   ...
   if(class_mgr_is_interface(class_mgr_get(),className)){
      ...
      if(class_func_is_func_generic(item->classFunc) || class_func_have_query_param(item->classFunc)){
         ...
         nboolean ok=generic_call_check/*!generic_call_add_fpgi_parm*/(generic_call_get(),
               item->classFunc,className,exprlist,funcGenericDefine,expr_loc);
      }
      tree firstParm=NULL_TREE;
      tree selfVarOrParm = (*exprlist)[0];
 ...
 */
tree func_call_select(FuncCall *self,tree func,vec<tree, va_gc> *exprlist, vec<tree, va_gc> *origtypes,
                       vec<location_t> arg_loc,location_t expr_loc,FuncPointerError **errors,SelectFunc *selRet)
{
   AET_LANG_FLAG_2(func)=0;
   ...
   GenericModel *funcGenericDefine=c_aet_get_func_generics_model(func);//如果funcGenericDefine是有效的说明这是一个泛型函数
   item=selectFunc(self,className,funName,exprlist,origtypes,arg_loc,expr_loc,TRUE,generics,funcGenericDefine,errors,selRet);
...
tree func_call_super_select(FuncCall *self,tree func,vec<tree, va_gc> *exprlist,
		         vec<tree, va_gc> *origtypes,vec<location_t> arg_loc,location_t expr_loc,FuncPointerError **errors,SelectFunc *selRet)
{
   AET_LANG_FLAG_5(func)=0;
   tree field=TREE_OPERAND(func,1);
   tree id=DECL_NAME (field);
   tree last=NULL_TREE;
   char currentClassName[256];
...
FuncAndVarMsg func_call_get_process_express_method(FuncCall *self,tree id, ClassName *className)
{
   ...
   if(self->parser->isAet){
      ...
      char *currentFunName=NULL;
      if(current_function_decl)
         currentFunName = IDENTIFIER_POINTER(DECL_NAME(current_function_decl));
      //重要!!!如果是在类中的静态函数内
      int re=fun_call_find_func(self,className,id);
      n_debug("func_call_get_process_express_method 33 在isAet中找函数结果是 %d classname:%s id:%s currentFunName:%s lookupExists:%d",
            re,className->sysName,idStr,currentFunName,lookupExists);
      if(re==ID_NOT_FIND && lookupExists)
         return ID_EXISTS;
      if(re==ISAET_FIND_FUNC && lookupExists){
         //在类中找到方法，在外部也找到方法，如果当前的函数是静态的，返回外部
         if(current_function_decl){
           ClassFunc    *classFunc= func_mgr_get_static_method(func_mgr_get(),className->sysName,currentFunName);
           if(classFunc){
               n_debug("在类中找到方法，在外部也有同名的函数，并且代码在类中的静态函数-----currentFunName：%s\n",currentFunName);
               return ID_EXISTS;
           }
         }
      }
