...
static void createSizeofToken(GenericImpl *self,int index,nboolean fromFuncGen)
{
   ...
   if(fromFuncGen){
      //AET_GENERIC_FUNC_THREAD_BLOCK_ADDR _gen_func_block_addr_128347
      refName=AET_GENERIC_FUNC_THREAD_BLOCK_ADDR;
      varName=AET_GENERIC_ARRAY;
   }
...
/**
 * 生成在类中的数组field
 * aet_generic_info _generic_1234_array[genericCount];
 * 数组内容格式如下：
 ...
 * 在泛型函数中，泛型模型存在线程变量 AET_GENERIC_FUNC_THREAD_BLOCK_ADDR  "_gen_func_block_addr_128347
 */
static tree getGenericInfo(location_t loc,tree objExpr,tree id2)
{
   ...
   //如果是泛型函数来自AET_GENERIC_FUNC_THREAD_BLOCK_ADDR，否则来自类中的aet_generic_info _generic_1234_array数组
   nboolean from=FALSE;
   int index=-1;
   if(funcGen){
...
void generic_impl_cast_by_token(GenericImpl *self,c_token *token)
{
  c_parser *parser=self->parser->parser;
  if(self->parser->isAet){
	  if(aet_parser_is_test_generic_block_state(self->parser)){
		  c_parser_consume_token (parser); //consume (
		  c_parser_consume_token (parser); //E
	  ...
	  generic_parser_cast_by_token(generic_parser_get(),token);
  }
}
 ...
 */
static nboolean checkgen(location_t loc,char *funcName,GenericModel *checkedGen,
      tree retnOrParm,ClassInfo *belongInfo,GenericModel *funcGen)
{
	if(!checkedGen)
		return TRUE;
    ClassName *className=getClassName(retnOrParm);
    if(!className){
       aet_print_tree_skip_debug(retnOrParm);
...
fxyd = fndecl 所以要判断是不是FUNCTION_DECL
*/
void  generic_impl_check_and_set_func(GenericImpl *self,tree fndecl,GenericModel *genModel,GenericModel *funcGenModel)
{
   ...
   if(!fndecl || TREE_CODE(fndecl)!=FUNCTION_DECL)
      return;
   n_debug("generic_impl_check_and_set_func 000 %s func:%p declGen:%p funcGen:%p",
         IDENTIFIER_POINTER(DECL_NAME(fndecl)),fndecl,genModel,funcGenModel);
...
{
   n_debug("generic_impl_push_generic_from_declspecs 00 %p\n",specs);
   c_parser *parser=self->parser->parser;
...
void generic_impl_push_generic_from_c_parm(GenericImpl *self,struct c_declspecs *specs)
{
   ...
   n_debug("generic_impl_push_generic_from_c_parm 00 %p\n",specs);
...
GenericModel *generic_impl_pop_generic_from_c_parm(GenericImpl *self,struct c_parm *parm)
{
   ...
   n_debug("generic_impl_pop_generic_from_c_parm 00 %p %p\n",specs,self->currentModelOfSpecs.specs);
   ...
   if(self->currentModelOfSpecs.specs && self->currentModelOfSpecs.specs==specs){
      n_debug("generic_impl_pop_generic_from_c_parm 11 %p\n",specs);
...
{
   GenericModel *funcModel=NULL;
   if(atFunc && class_func_is_func_generic(atFunc))
      funcModel=class_func_get_func_generic(atFunc);
   GenericModel  *classModel=NULL;
   if(atInfo && class_info_is_generic_class(atInfo))
      classModel=class_info_get_generic_model(atInfo);

   int i,j;
   int destCount=generic_model_get_count(genericDefine);
   RunGenericInfo **infos=(RunGenericInfo **)xmalloc(destCount*sizeof(unsigned long)) ;
   for(i=0;i<destCount;i++){
      ...
      if(!strcmp(unit->name,"all") || unit->isDefine){
         info->genUnit=generic_unit_clone(unit);
         info->from=UNIT_FROM_NEW_OBJECT;
         info->fromPos=i;
         info->file=generic_unit_get_decl_file(unit);
         infos[i]=info;
         continue;
      }
      if(funcModel!=NULL){
         int index=generic_model_get_index_by_unit(funcModel,unit);//在泛型函数模型中的索引
         if(index>=0){
            GenericUnit *funUnit=generic_model_get(funcModel,index);
            info->genUnit=generic_unit_clone(funUnit);
            info->from=UNIT_FROM_GENERIC_FUNC;
            info->fromPos=index;
            info->fromFunction=atFunc;
            infos[i]=info;
            continue;
         }
      }
      if(classModel!=NULL){
         int index=generic_model_get_index_by_unit(classModel,unit);//在泛型类模型中的索引
         if(index>=0){
            GenericUnit *selfUnit=generic_model_get(classModel,index);
            info->genUnit=generic_unit_clone(selfUnit);
            info->from=UNIT_FROM_SELF_PARM;
            info->fromPos=index;
            info->fromClass=atInfo;
            infos[i]=info;
            continue;
         }
      }
...
/**
 * 收集new对象的父类泛型信息。
 * 1.A不是泛型类，但继承了泛型类B，一定定义了泛型类B的泛型模型，所以B的泛型模型全部定义。
...
{
   if(child->parentName.sysName==NULL)
      return NULL;

   ClassInfo *parentInfo=class_mgr_get_class_info(class_mgr_get(),child->parentName.sysName);
   if(!class_info_is_generic_class(parentInfo))
      return NULL;
   gcc_assert(child->parentGenericModel!=NULL);

   int i;

   int parentCount=generic_model_get_count(parentInfo->genericModel);
   RunGenericInfo **infos=(RunGenericInfo **)xmalloc(parentCount*sizeof(unsigned long)) ;
   for(i=0;i<parentCount;i++){
      GenericUnit *unit=generic_model_get(child->parentGenericModel,i);
      RunGenericInfo *info=n_slice_new0(RunGenericInfo);
      if(!strcmp(unit->name,"all") || unit->isDefine){
         info->genUnit=generic_unit_clone(unit);
         info->from=UNIT_FROM_NEW_OBJECT;
         ...
         //父类有未定义的泛型，说明子类一定是一个泛型类
         gcc_assert(class_info_is_generic_class(child));
         int index=generic_model_get_index_by_unit(child->genericModel,unit);
         if(index<0){
            error_at(unit->loc,"未知泛型类型。");
         }
         GenericUnit *childUnit=childInfos[index]->genUnit;
         info->genUnit=generic_unit_clone(childUnit);
         info->from=UNIT_FROM_CHILD;//来自子类
         info->fromPos=index;
         info->fromClass=child;
         infos[i]=info;
      }
   }
   return infos;
